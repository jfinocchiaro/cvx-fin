...
\jessie{Is there a better way to say this? ex: ``For all $r' \in \R'$, there is an $r\in \R$ such that $\Gamma_r = \gamma_{r'}$'' ... this suggests the properties are equivalent-ish.}
\end{notation}
\jessie{In the latter definition, if $\Gamma$ embeds $\gamma$, then $\gamma \preceq \Gamma$ since $\Gamma_{\varphi(r')} = \gamma_{r'}$ for all $r' \in \R'$.}

\begin{definition}
	The elicitable property $\Gamma:\P\toto \R$ is an \emph{embedding} of $\gamma: \P \toto \R'$ if there exists an injection $\varphi:\R' \to \R$ such that $\varphi(r) \in \Gamma(p) \iff r \in \gamma(p)$. 
\end{definition}

We slightly abuse notation to say that if $\Gamma$ is an embedding of $\gamma$, then $\gamma \preceq \Gamma$ if and only if $\varphi(\gamma(p)) = \Gamma(p) \cap \varphi(\R')$\footnote{If $S$ is a set and $\phi$ a function, we write $\phi(S) := \{ \phi(x) : x \in S \}$.}.
\jessiet{Do we want the above statement? Meh}
...
\begin{definition}
A property $\Gamma: \P \toto \R$ is \emph{finite} if $\R$ is finite. % $\im(\Gamma) := \{r\in\Gamma(p) : p\in\P, r \in \R \} = \bigcup_{p\in\P} \Gamma(p)$ is a finite set.
\end{definition}
...
%
%\begin{proof}
%	\jessie{Uses: Lemma~\ref{lem:gam-prime-exists}.}
%	
%	We define $\trim(\Gamma)$ in terms of the property $\gamma: \P \toto \R'$ such that $\gamma \preceq \Gamma$ and $\R'$ is as constructed in Lemma~\ref{lem:gam-prime-exists}.
...
%Since $\Gamma$ has a finite set of full-dimensional level sets comprising $\trim(\Gamma)$ that union to $\P$ (by nondegeneracy of $\Gamma$), we can apply Lemma~\ref{lem:gam-prime-exists} to observe there is a nondegenerate, nonredundant $\gamma \preceq \Gamma$, and we show $\Gamma$ embeds $\gamma$.
