...
\jessiet{TODO: REVISIT THIS.  See Notation 3}
  ...
  %\jessie{Technically, if labeled, we also want a bijection $\phi: \R' \to \varphi(\R)$ so that $\phi(\gamma) = \Gamma \cap \varphi(\R')$, where $\phi = \varphi$ restricted to the range of $\varphi$.  We would then say $\gamma \preceq \Gamma$ if $\phi(\gamma) = \Gamma \cap \varphi(\R)$...?}
\end{notation}

\begin{notation}
\jessie{Attempt 2 of the above}
Let $\gamma: \P \toto\R'$ and $\Gamma: \P \toto\R$.
We say $\gamma \preceq \Gamma$ if there exists an injection $\varphi: \R' \to \R$ so that for all $p \in\P$, we have $\varphi(\gamma(p)) \in \Gamma(p)$. 
%\jessie{The $\cap \varphi(\R')$ isn't necessary anymore, right...?}
\end{notation}
