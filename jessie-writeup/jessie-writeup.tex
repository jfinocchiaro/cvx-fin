...
Observe that $\Theta$ is a set of level sets that divide the simplex, and not a function.\raft{Careful: $\Theta$ does not need to divide anything; $\Theta = \{\emptyset\}$ is technically an unlabeled (degenerate) property}
...
\begin{notation}
  ...
  \raft{Let's use $\preceq$ instead of $\leq$, since it's clearly different from the usual $\leq$, and easier to find--replace later.}
\end{notation}
...
\begin{definition}\label{def:unlabeled-nonredundant}
  ...
  \raft{Have not defined elicitability for unlabeled properties.  Also, what does ``almost everywhere'' mean in this context, since $\Theta$ is just a set of sets?  I think we just want to say that there are no distinct $\theta_1,\theta_2 \in \Theta$ such that $\theta_1 \subseteq \theta_2$.}
...
Equivalently, $\theta$ is full-dimensional if there are distributions $p_1, p_2, \ldots, p_n \in \theta$ so that $p_1, \ldots, p_n$ are affinely independent.\raft{Careful; I think you need convexity here.  Probably you want to define full-dimensional as having nonempty interior, and prove this as a consequence of convexity.}
