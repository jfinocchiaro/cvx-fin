...
We proceed in a cycle to show $5 \implies 1 \implies 6 \implies 2 \implies 3 \implies 4 \implies 5$.
...
Then there is a nondegenerate, nonredundant, finite property $\gamma$ so that $\{ \gamma_r : r \in \R' \} = \strip(\gamma) = \trim(\Gamma)$, and we take $\gamma$ as constructed in Conjecture~\ref{conj:gam-prime-exists}.
We want to show that $\Gamma$ embeds this property $\gamma$.
First, we know $\gamma$ is nondegenerate and finite by construction.
...
Since there is an injection $\varphi$ from $\R' \to \R$, then $r \in \gamma(p) \implies \varphi(r) \in \varphi(\gamma(p)) \subseteq \Gamma(p) \implies \varphi(\gamma(p)) \in \Gamma(p)$.

Now consider $\trim(\Gamma) = \strip(\gamma)$.
Therefore, for every level set $\theta\in\trim(\Gamma)$, there is a corresponding level set $\theta' \in \strip(\gamma)$ so that $\theta = \theta'$.
...
Therefore, if $\trim(\Gamma)$ is finite, then $\Gamma$ embeds the finite, nondegenerate property $\gamma$ such that $\trim(\Gamma) = \strip(\gamma)$ .
Thus, $5 \implies 1$.
...
\item [$6 \implies 2$]
Since $\Gamma$ is non degenerate and every $\theta \in \trim(\Gamma)$ is full dimensional (Conjecture~\ref{conj:trim-full-dim}), it is clear that the union of the  (full-dimensional) level sets of $\trim(\Gamma)$ union to $\P$.
...
Since $G$ is convex, it is differentiable almost everywhere, so the optimal report is unique almost everywhere.
...
\item [$3 \implies 4$]
	...
	Thus, we concern ourselves with the case $|\partial G(p)| > 1$.
	We know this only happens at lower dimensional level sets $\theta$ by assumption (since the optimal report is unique on the interior of level sets).
	Since $\theta$ is not full dimensional and there is only a finite set of full dimensional level sets, we know there is a finite set of full dimensional level sets $\{\theta_i\}_{i=1}^m$ in the $\epsilon$-ball around each $p \in \theta$ so that for any $p_i \in \inter{\theta_i}$, we observe $\partial G(p_i) \neq \partial G(p_j)$ for all $i \neq j$ and $\partial G(p_i) = \Gamma(p_i) = \{r_i\}$ for all $1 \leq i \leq m$.
	Additionally, as level sets are closed, we claim $p \in \Gamma_{r_i}$ for all $1 \leq i \leq m$.
	Therefore, $p \in \theta \implies p \in \bigcap_{r' \in \{r_i\}_{i=1}^m} \Gamma_{r'}$ for some $\{r_i\}_{i=1}^m \subseteq \R'$.
	As $\R'$ is finite, so is the power set $2^{\R'}$, so the set $S = \{\Gamma_r : r \in \R \setminus \R' \}$ is finite.
	Therefore $\strip(\Gamma) = \strip(\Gamma') \cup S$ is finite. 
	\jessie{Come back to this- should be esay-ish though.}
...
\begin{conjecture}\label{conj:prec-if-embeds}
	If the elicitable property $\Gamma$ embeds the nondegenerate finite $\gamma$, then $\gamma \preceq \Gamma$ and $\trim(\Gamma) = \strip(\gamma)$.
\end{conjecture}
\begin{proof}
	...
	\jessie{This gets at the idea, but I don't think it's correct.  It also might rely on convexity of $\R$, which we may not always have, although I'd be ok with assuming it, otherwise embedding is kind of pointless.}
