...
%	Since level sets are closed, for any $r \in \R'$, we know that $\varphi(r) \in \Gamma(q)$ for all $q \in bd(\Gamma_{\varphi(r)})$, and therefore there is at least one report $r \in \R'$ such that $\varphi(r) \in \Gamma(p)$ for all $p \in \P$, so $\gamma$ is nondegenerate. 
	...
	Any level set with a redundant report must either (a.) equal to another level set or (b.) a proper subset of another level set. %, or (c.) span multiple level sets. %not by current definition of redundancy
...
%	We then know that $p \in \gamma_{r'}$.
	...
	%Since $\zeta \preceq \Gamma$, we know for all $p \in \P$ that $\zeta(p) \subseteq \Gamma(p)$.
	...
	For $r' \in \R$, we know $p \in \Gamma_{r'} \implies p \in \{ p \in \P : r' \in \Gamma(p) \cap \R \} = \{ p \in \P : r' \in \zeta(p) \} = \zeta_{r'}$. %as $\zeta(p) \subseteq \Gamma(p)$ by closure of level sets (since if a level set is lower-dimensional, there is some full-dimensional level set that it is a subset of).
...
\begin{proof}
	...
	\begin{align*}
		...
		\implies 
%		\lambda[\partial_r \bar L(r) + p(z) \partial_r L(r,z)] + (1-\lambda) [\partial_r \tilde L(r) + q(z) \partial_r L(r,z)] = 0
	\end{align*}
\end{proof}
