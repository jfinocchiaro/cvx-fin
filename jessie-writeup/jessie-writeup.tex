...
Observe that $\Theta$ is a set of level sets, and not a function.\raft{Careful: $\Theta$ does not need to divide anything; $\Theta = \{\emptyset\}$ is technically an unlabeled (degenerate) property}
...
\begin{notation}
  ...
  We say $\Gamma' \preceq \Gamma$ if removing reports from $\Gamma$ yields $\Gamma'$, that is, if $\Gamma' = \Gamma \cap \R'$ for some $\R' \subseteq \R$.
  \jessiet{Changed $\R \subseteq \R$ to $\R' \subseteq \varphi(\R)$.}
\end{notation}
...
\begin{definition}\label{def:unlabeled-nonredundant}
  ...
  \raft{Have not defined elicitability for unlabeled properties.  Also, what does ``almost everywhere'' mean in this context, since $\Theta$ is just a set of sets?  I think we just want to say that there are no distinct $\theta_1,\theta_2 \in \Theta$ such that $\theta_1 \subseteq \theta_2$.- you're right... shoot}
...
\item A level set $\theta$ is \emph{full-dimensional} if it has nonempty interior. %% old: $dim(\aff(\theta)) = dim(\P)$. 
...
\jessiet{What is the formal, ``official'' definition of full-dimensional, if one exists?}
...
\begin{proposition}\label{prop:optimal-reports-per-level-set}
  ...
  \item \jessie{Added after fplc-elic.tex}
	\begin{itemize}
		...
		\item[5.] There is a finite set of full dimensional level sets of $\Gamma$ that union to $\P$.
		\item[6.] There is a nondegenerate, finite property $\Gamma':\P\toto\R'$ such that there is an injection $\varphi:\R'\to\R$ such that $r\in\gamma(p) \iff \varphi(r) \in \Gamma(p)$. (i.e. $\Gamma$ embeds finite $\Gamma'$)
	\end{itemize}
  \end{enumerate}
\end{proposition}

\begin{proof}
First, we show $2 \iff 3$ then proceed in a cycle to show $6 \implies 1 \implies 5 \implies 4 \implies 2 \implies 6$.
...
\item[$2 \implies 6$] Let $\trim(\Gamma)$ be finite.
...
Since we know $\card(\trim(\Gamma)) = \card(\R')$ is finite and $\trim(\Gamma)$ is a subset of the codomain of $\Gamma$, $\R$, then $\card(\R')) \leq \card(\R) \jessie{?= \card(\reals^d)}$, there is an injection $\varphi : \R' \to \R$ from Folland Section 0.
...
\begin{proof}
	...
	As $\varphi(\R') \subseteq \R$ and $\gamma$ is nondegenerate, we know that for all $r \in \R'$
	
	
	Therefore, $\gamma(p) = \Gamma(p) \cap \R'$
	
	
	Finally, we show $\gamma$ is nonredundant. \jessie{TODO...}
