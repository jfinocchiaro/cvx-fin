...
\btw{Note: Strip includes the boundaries of level sets / lower dimensional level sets.}
...
\begin{definition}
	The elicitable property $\Gamma:\P\toto \R$ is an \emph{embedding} of $\gamma: \P \toto \R'$ if there exists a property $\zeta : \P \toto \R$ so that $\zeta \preceq \Gamma$ and $\gamma \approx \zeta$.
\end{definition}
