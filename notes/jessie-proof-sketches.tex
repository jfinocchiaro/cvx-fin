...
\begin{proof}[Property version]
  ...
  \jessie{I don't see a way that properties make this easier.}
...
\begin{proof}[Property version]
  ...
  By \jessie{COLT Lemma 11}, we know $\Gamma$ embeds $\gamma := \Gamma \cap \R$, \jessie{for appropriate choice of $\R$} elicited by the discrete loss $\ell$. 
  Thus, $L$ embeds $\ell$.
\end{proof}
\hrulefill
\begin{proof}[Non-property version]
...
\hrulefill
\begin{proof}[Property version]
  To see this, we construct one such discrete loss, namely $L|_\R$, where $\R := \{u_f\}_{f \in F}$ is the set of reports that are uniquely optimal on the interior of each full-dimensional level set.
...
\begin{proof}[Property version]
  ...
  The result is a corollary of Theorem~\ref{thm:general-duality-embedding}, where $L$ is as constructed in the proof.
\end{proof}
\hrulefill
\begin{proof}
