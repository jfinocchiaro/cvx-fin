...
%	Since the infimal convolution is polyhedral (and therefore bounded from below), the epigraph is closed \jessie{what we want?  is this true?}
	...
	We know the infimum attained by taking a convex combination over $\{f_y\}_{y \in \Y}$ is the same as any over $\{L'_y\}_{y \in \Y}$, since, for all $y \in \Y$, we have $\inf_{u \in \reals^n} L'(u)_y = \inf_{u \in [0,1]^n} L'(u)_y$, so we cannot strictly improve expected loss by changing the minimizing report on any outcome.
...
\begin{proof}
  ...
  We do this by showing that for $u$ such that $u_{[\ell]} < 0$, changing the report to $u'$ where $u'_{[i]} = 0$ for $i = \ell, \ldots, n$ does not increase the expected loss. 
  (Let $\ell$ be the ordered index of the first such negative entry.)
  ...
  Now, with some more manipulation, we can see that if $y \in [1, \ldots, k]$, let $\rho_y := (u_y - 1 + \epsilon)_+$.
  Then we have
  \begin{align*}
...
\end{align*}
\jessie{Haven't show this is always nonnegative, but every example I've worked out has been that way.}
...
This $u_y$ can only ``decrease'' loss if
\begin{align*}
...
On the other hand, if $p_y \geq \frac 1 {k+1}$, then we want $u_y \leq 0$ in order to minimize expected loss.
