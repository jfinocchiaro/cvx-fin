...
\begin{proof}[Property version]
  ...
  We know $\gamma' := \Gamma(p) \cap \R'$ is nonempty for all $p \in \simplex$ since $\varphi(\gamma(p)) \subseteq \R' \subseteq \Gamma(p)$.
...
%  Applying \jessie{COLT Lemma 11}, we then observe that $\risk{L} = \risk{L|_\R} = \risk{\ell}$.
  ...
  The projection of their epigraphs onto $\simplex$ must form the same power diagram \jessie{COLT Theorem 33}.  %, corresponding to the level sets of the properties each loss elicits.
  ...
  Since $\ell$ is discrete, the coefficients of the supporting hyperplane exactly describe the loss over each outcome given the report $r = \gamma(p)$.  \jessie{??}
  %Similarly, for some $u \in \reals^d$, as the risks are the same, we claim that $L(u,p) = \ell(\gamma(p), p)$, as the risks are equal. 
  
  As there is some $u_f \in \argmin_{u'} L(u', p)$, we can design $\varphi : r \mapsto u_f \in \Gamma(p)$, and by the risks being equal, we also have $r \in \argmin_{r' \in \R} \ell(r', p)$, and thus $r \in \gamma(p)$.
  The reverse implication follows similarly, and we conclude that $\Gamma$ embeds $\gamma$.
  
  Since $\risk{L}$ is polyhedral, the supporting hyperplane to $\risk{L}$ is unique for each $p \in \inter{f}$ and $f \in F$, and so is the loss $L(u_f)$ is unique for all such $p$.
  As both the loss values of $\ell(r)$ and $L(u_f)$ are equal and minimize their respective losses,  so we conclude that $L$ embeds $\ell$.
...
\begin{proof}[Property version]
  ...
  Since $\ell$ is discrete, we know that $\risk{\ell}$ is polyhedral as $\risk{\ell}$ is the max of a finite set of linear functions in $\simplex$. \raf{do we know? okay, we do---but why?}.
