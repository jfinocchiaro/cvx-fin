...
Throughout this section, consider $F$ to be the facets of the projection of the surface of $\risk{\ell}$.
For a facet $f$, we also consider $u_f = \argmin_u L(u, p)$ for any $p \in \inter{f}$.
(We know that this $u_f$ is the same regardless of the choice of $p$ since $\risk{\ell}$ has a constant derivative on $\inter{f}$) 
\hrule
\hrulefill
...
\begin{proof}[Property version]
  For the forward implication, suppose $L$ embeds the discrete loss $\ell$.
  Then $\Gamma := \prop{L}$ embeds the finite property $\gamma$ elicited by $\ell$.

  Generating $\R := \{u_f\}_{f\in F}$, we know that $\gamma = \phi(\Gamma \cap \R)$, where the two are the same up to relabeling \jessie{need more thorough argument on this?} and $\Gamma \cap \R$ is elicited by $L|_\R$ by \jessie{COLT Lemma 11}, and thus has the same Bayes Risk as $L|_\R$ by \jessie{cite FK General Truthfulness Theorem 2}.
  Applying \jessie{COLT Lemma 11}, we then observe that $\risk{L} = \risk{L|_\R} = \risk{\ell}$.
  
  For the reverse implication, we use contradiction. \jessie{But no properties... I didn't see how.  Different technique from below though.}
  ...
  That means, for some $p \in \simplex$ on the interior of a cell in $P_\ell$ \jessie{define}, we know $\nabla \risk{\ell}(p) \neq \nabla \risk{L}$.
...
\begin{proof}[Property version]
  ...
  For the set of facets $F$ projected from the surface of $\risk{L}$ (cells of $P_L$), construct $\R := \{u_f\}_{f \in F}$, with $u_f$ as described earlier. 
  By \jessie{COLT Lemma 11}, we know $\Gamma$ embeds $\gamma := \Gamma \cap \R$ elicited by the discrete loss $L|_\R$. 
...
\begin{proof}[Non-property version]
  ...
  For each facet $f \in F$, consider the report $u_f$ as constructed above.
  Since $F$ is finite, the set $\R = \{u_f\}_{f \in F}$ is finite.
...
\begin{proof}[Non-property version]
  ...
  \jessie{Going to refresh my brain-- come back to this, but if the other proof is insufficient, this idea is chasing down the correct path.}
\end{proof}
\hrulefill
\begin{proof}[Property version]
...
\end{theorem}
\begin{proof}[Property version]
  Consider $\gamma := \prop{\ell}$, which is a finite property as $\dom(\ell)$ is finite.
...
\begin{proof}[Property version]
	...
	Consider the finite property $\gamma$ embedded by the discrete loss $\ell$. \jessie{Need a proof that $\ell$ is discrete?}
	By Theorem~\ref{thm:discrete-loss-poly-embeddable}, we know that there is a polyhedral loss $L$ that embeds $\ell$.
	Therefore, $\Gamma := \prop{L}$ embeds $\gamma$.
	Since $L$ elicits $\Gamma$, we can then see that $L|_\R$ elicits $\gamma' := \Gamma \cap \R$ by \jessie{COLT Lemma 11... hold up.  We need something else here.  This statement isn't quite true.}.
	Thus, $\gamma$ is elicitable by some relabeling of $L|_\R$.
\end{proof}
\hrulefill
\begin{proof}[Non-property version]
