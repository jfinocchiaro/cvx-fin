...
\begin{proof}
  ...
  Since $\cup f = \simplex$, and each $f = \Gamma_{u_f}$ for some $u_f \in \reals^d$, we invoke Proposition~\ref{prop:embed-trim} \jessie{COLT Prop 12} to note $\Gamma:= \prop{L}$ embeds a finite property $\gamma := \prop{ell}$, and their loss values match up, since $\ell$ is exactly described by the same supporting hyperplanes for each $p \in \simplex$.
