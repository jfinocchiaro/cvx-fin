...
\begin{proof}
	...
	Given this, we observe $L'(u)_y = \left(\tfrac 1 k (u-e_y)_{[1,\ldots,k]}\right)_+$.
	...
	Additionally, if $u_{[k+1]} <0$, then the average $\tfrac 1 k (u - e_y)_{[1, \ldots, k]} = \tfrac 1 k u_{[2,\ldots,k]}$, so setting $u_{[k+1]} = 0$ can only decrease the loss.  (By the same argument used for $u_{[k+2]}$.)
...
\hrule
\begin{lemma}
	For all $p \in \simplex$, there is a $u \in [0,1]^n$ optimizing $L'$.
\end{lemma}
\begin{proof}
	...
	Since every $p \in \simplex$ is a convex combination of point mass distributions, we claim the expected loss can be minmized by reporting some $u \in [0,1]^n$: the Minkowski sum of the $e_y$ vectors.
	
	Note that the Minkowski sum of the $e_y$ vectors is $[0,1]^n$ and is a subset of the full set of minimizers of $L'$ over $\simplex$.
	
	\jessie{Another thought: Sufficient to show $u_{[k+1]} \geq u_{[1]} - 1$?}
	
	Call $f_i(u) := L'(u)_i|_{[0,1]^n}$ the loss on outcome $i$ restricted to $[0,1]^n$.
	We know that all $f_i$ are polyhedral, since they are simply a (convex) restriction of polyhedral losses.
	By Rockafellar~\cite[Theorem 19.2]{rockafellar1997convex}, we then know $f^*_i$ is also polyhedral.
	...
	We know that $\epi(f_1 \Box \ldots \Box f_n) = \epi(f_1) + \epi(f_2) + \ldots + \epi(f_n)$, where $+$ is the Minkowski sum. \jessiet{cite... current source is Wikipedia}
	Moreover, $\epi(f_1) + \ldots \epi(f_n)$ is defined on $[0,1]^n$ and the infimum of the function sum is attained, since the epigraph sum is bounded from below  \jessie{not sure if that's what I want to say, but something like it.} on the function value.
	
	We know the infimum attained by taking a convex combination over $\{f_y\}_{y \in \Y}$ is the same as any over $\{L'_y\}_{y \in \Y}$, since, for all $y \in \Y$, we have $\inf_{u \in \reals^n} L'(u)_y = \inf_{u \in [0,1]^n} L'(u)_y$, so we cannot strictly improve expected loss by changing the minimizing report on any outcome.\jessie{??}
\end{proof}
