...
Throughout this section, consider $F$ to be the facets of the projection of the surface of $\risk{L}$.
For a facet $f$, we also consider $u_f = \argmin_u L(u, p)$ for any $p \in \inter{f}$.
(We know that this $u_f$ is the same regardless of the choice of $p$ since $\risk{L}$ has a constant derivative on $\inter{f}$) \jessie{This implicitly assumes $\risk{L}$ is polyhedral, although this isn't stated in Prop 1.}
...
\begin{proof}[Property version]
  ...
  For the set of facets $F$ projected from the surface of $\risk{L}$, construct $\R := \{u_f\}_{f \in F}$, with $u_f$ as described earlier. 
...
\begin{proof}[Property version]
	...
	Since $L$ elicits $\Gamma$, we can then see that $L|_\R$ elicits $\gamma$ by \jessie{COLT Lemma 11... hold up.  We need something else here.  This statement isn't quite true.}.
	Thus, $\gamma$ is elicitable.
	\jessie{This seems like a lot of running in circles, and it's not quite right.}
\end{proof}
\hrulefill
\begin{proof}[Non-property version]
