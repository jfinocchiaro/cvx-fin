...
\begin{proof}[Property version]
  ...
  Call $\R' := \varphi(\R)$.
  For all $p \in \simplex$, we know that $\Gamma(p) = \varphi(\gamma(p))$.
  We know $\gamma' := \Gamma(p) \cap \R'$ is nonempty for all $p \in \simplex$ since $\Gamma(p) = \varphi(\gamma(p)) \in \R'$.
...
%  Applying \jessie{COLT Lemma 11}, we then observe that $\risk{L} = \risk{L|_\R} = \risk{\ell}$.
  ...
  As $\ell$ is discrete, we know $\gamma$ is finite, and so the level sets of $\gamma$ are full-dimensional \jessie{by COLT Lemma 10} and union to the simplex by nondegeneracy of $\gamma$.
  Since the risks of $L$ and $L$ are the same (and therefore, so are their projections onto $\simplex$), this same [finite] subset $\Theta$ of the level sets of $\Gamma$ that union to the simplex and are full-dimensional.
  By \jessie{COLT Prop 12}, we can then conclude that $\Gamma$ embeds the finite property $\gamma$, and thus $L$ embeds the discrete loss $\ell$.
