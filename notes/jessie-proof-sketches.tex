...
For a facet $f$, we also consider $u_f = \argmin_r \ell(r, p)$ for any $p \in \inter{f}$.
(We know that this $u_f$ is the same regardless of the choice of $p$ since $\risk{\ell}$ has a constant, well-defined derivative on $\inter{f}$) 
\hrule
\hrulefill
