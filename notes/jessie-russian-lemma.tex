...
\begin{definition}[Measurable (set-valued function)]
	Let $\B(\reals^d)$ is the Borel $\sigma$-algebra of $\reals^d$.
	Suppose $(\Omega, \B(\Omega))$ is a measurable space and $f:\Omega \toto \reals^d$ a multifunction taking values in the set of nonempty closed subsets of $\reals^d$.
...
Fix $z$.
Consider the set-valued function $Q_z : \Omega \to 2^{\reals^d}$ defined by $Q_z(\omega) := \{V : \inprod{z}{V(\omega)} = L'(u,\omega;z), V \in \V_u \}$. \jessie{type check: okay when you think about $V$ as a RV instead of function.}
...
1.  
If $Q_z$ is measurable and closed-valued, then by \jessie{Castaing, also Rockafellar VA 14.6}, we have a measurable selection $V \in \V_u$ such that $\inprod{z}{V(\omega)} = L'(u,\omega; z)$ for all $\omega \in \Omega$.

Thus, we show $Q_z$ is measurable and closed-valued.
First, we have $Q_z$ closed-valued since $L$ is convex and therefore its directional derivative is continuous. \jessie{Being lazy; will fill in later.}

As measurable functions are closed under (countable) intersections, we can simply show $Q_z$ intersection of measurable functions; namely, since we have $Q_z(\omega) = \V_u \cap f: \omega \mapsto \partial L(u,\omega) \cap g : \omega \mapsto \{c : \inprod{z}{c} = L'(u,\omega;z)\}$.

$\V_u$ is measurable by construction as it is a collection of measurable sets.
Moreover, the function $f$ is measurable by \jessie{Rockafellar: Measurable Dependence, Corollary 4.6}.
Now, we aim to show the function $g$ is measurable, which follows from measurability of $L'$ and the fact $\inprod{z}{\cdot}$ is continuous in its second argument, hence Borel, the set is measurable. \jessie{Copying Russian argument}
...
For all closed $U \in \reals^d$, have $g^{-1}(U) = \{\omega : \exists c \in \reals^d : \inprod{z}{c} = L'(u,\omega;z)\} \in \B(\Omega)$
