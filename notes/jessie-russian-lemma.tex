...
In order to show that $Q_z$ is $B$-measurable, it is equivalent to show $gr(Q_z)$ is $B \otimes \B(\reals^d)$-measurable. \jessie{Kinda hand-wavy} \jessie{ASSUMPTION: $B$ is a complete $\sigma$-algebra with respect to $p$.}
Now, consider $gr(Q_z) = gr(Q) \cap gr(R)$, where $Q: \omega \mapsto \partial L(u,\omega)$ and $R : \omega \mapsto \{x \mid \inprod{z}{x} = L'(u,\omega;z)\}$.
As $Q$ is measurable \jessie{Rockafellar: Measurable Dependence, Cor 4.6}, so is $gr(Q)$.
Moreover, $gr(R)$ is measurable as it is the graph of a linear multifunction, hence measurable.
Now we can take $gr(Q_z) = gr(Q) \cap gr(R)$, which is measurable by closure under intersections, and have the result.
