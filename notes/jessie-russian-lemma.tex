...
As measurable functions are closed under (countable) intersections, we can simply show $Q_z$ intersection of measurable functions; namely, since we have $Q_z(\omega) = \V_u \cap f: \omega \mapsto \partial L(u,\omega) \cap g : \omega \mapsto \{c \in \reals^d : \inprod{z}{c} = L'(u,\omega;z)\}$.
...
Now, we aim to show the function $g$ is measurable, which follows from measurability of $L'$ and the fact $\inprod{z}{\cdot}$ is continuous in its second argument, hence Borel, the set is measurable. \jessiet{Copying Russian argument}
