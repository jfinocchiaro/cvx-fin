...
\begin{proof}
  ...
  Recall that each face of the epigraph is uniquely identified by a subset of constraints which are tight for exactly that face.
  ...
  We claim $U^*$ is the union of the relative interiors of these faces.

  \vskip2em
  Outline:
  \begin{itemize}
    \item $L(u;p)-  L^* \geq \max_d ~ d \cdot (u - u^*)$ where $u^*$ is the projection of $u$ onto $U^*$ and $d$ ranges over subgradients of $L$ at $u^*$.
    \item For each face of $U^*$, we have a...
  \end{itemize}
