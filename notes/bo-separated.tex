...
\end{lemma}
\begin{proof}
  For each report $r$, write $u_r$ for its embedding point.
  By construciton, we have $u_r \in \gamma(p) \iff r \in \Gamma(p)$.
  Therefore, for each $U \in \mathcal{U}$, let $R_U = \{r : u_r \in U\}$.
  By construction, $R_U$ is nonempty because some $r$ is always optimal for any $p$, hence some $u_r \in U$ for all $U \in \mathcal{U}$.
  Furthermore, because $R_U$ are simultaneously optimal when $U$ is, i.e. when $\gamma(p) = U$, we must have that $r \in \Gamma(p) \iff u_r \in U$.
\end{proof}
...
\begin{proof}
  ...
  Because both $D$ and $S'$ are polyhedra, their Minkowski sum $D' = D + S'$ is a polyhedron.\bo{Cite}
