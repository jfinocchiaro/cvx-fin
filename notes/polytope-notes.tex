...
%  
  ...
  \jessiet{Later: Talk about why $c = \vec 0$ and not some other vector.}
  ...
  \begin{proof}
	\begin{itemize}
		...
		We can re-write $D$ as $\{p \in A : [E;I] p \geq \vec 0\}$, building simplex constraints into the inequality rather than the domain assumption.
		By the assumed equality, every row of $B$ must then appear in $[E;I]$, where $I$ is the $n$-element identity matrix.
		As $\{p \in A : I p \geq \vec 0\}$ strictly defines the simplex constraints, any non-simplex constraint of $B$ must then be described by a row of $E$, up to rescaling.
		...
		As $D$ is defined by distributions on the simplex and $C$ on the affine span of the simplex, and $\simplex \subset A$, we have the rest of the rows in $E$ necessarily being redundant with respect to $B$.  \jessie{revisit.}
		...
		By adding simplex constraints to the affine span of the simplex, we then have $\{p \in A : Bp \geq \vec 0\} = \{p \in A : [E; I] \geq \vec 0 \}$. \jessiet{Double check we can claim this at this point...}
		The latter set is equal to $D$ once we change the domain of $p$, so that we have $D = \{p \in \simplex : Ep \geq \vec 0\} = \{p \in A : [E;I] p \geq \vec 0\}$.
