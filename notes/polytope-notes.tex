...
\end{definition}
  ...
  For each face $F_{jy}$ that is in the decomposition of the face $F_j$, we have $\inprod {v_j}{x} \leq e^y_j$ forming a tight bound on $F_j$. 
...
%  
  ...
  Throughout, assume we have $V$ which is complete for $\T$.
  
  Observe that as $T_y = \{x : Vx \leq E_{;y}\}$, we can multiply the right side of the inequality by the constant $p_y \geq  0$ to yield $p_y T_y = \{x : Vx \leq p_y E_{;y}\}$.
  ...
  \begin{definition}
  	We say a vector $v$ is \emph{redundant} with respect to matrix $Y$ describing the polytope $P^Y := \{z \in A : Yz \geq \vec 0\}$ if we have $P^Y = \{z \in A : [Y;v]z \geq \vec 0\}$.
  \end{definition}
