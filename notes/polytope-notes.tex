...
\end{definition}
  ...
  Since we know $T_1, \ldots, T_n$ are polytopes with $\H$-representations, one of the first natural steps is to find a set of valid inequalities describing each $T_i$.
  ...
  For ease of notation, we concatenate the $e^y$ vectors to form the matrix $E$ according to a consistent ordering on $\Y$.
...
%  	Thus, 
  ...
  In cases where $\T$ is obvious from context, we forego the subscript and just write $D$.
  ...
  \begin{proof}
    ...
    As each halfspace is bounded by the support function of the weighted polytope by construction of $E$, then the support of the weighted polytope defined by an inequality on $v_i$ can be described as $\inprod{v_i}{z} \leq \inprod{E_i}{p}$.
