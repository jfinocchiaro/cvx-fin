...
%    , so $\vec 0$ being in those Minkowski sums implies that $\vec 0$ is also in the weighted Minkowski sum of any convex combination. \jessiet{Too hand wavy}
  ...
  \begin{proof}
    ...
    In particular, by the halfspace representation of this set, $x \in \oplus_p \T^r$ if and only if, for all $w$, $\inprod{w}{x} \leq \max_{x' \in \oplus_p \T^r} \inprod{w}{x'}$.
  ...
  \end{lemma}
  \begin{proof}
    [Bo: ?? We seem to use this above, but is there a citation?]
  \end{proof}

  \begin{theorem}\label{thm:nasc-optimality-conditions}
    $\Opt(\T^r, C)$ holds if and only if $\T^r$ and $C$ satisfy both the halfspace condition (\ref{cond:H-condition}) and the vertex condition (\ref{cond:V-condition}).
...
%  	There exists $\T = \{T_1, \ldots, T_n\}$ such that $D^\T = C$ if and only if there exist normals $v_1, \ldots, v_k$, and points $x_{jy}$ such that the Conditions~\ref{cond:H-condition} and~\ref{cond:V-condition} hold.
  \end{theorem}
  \begin{proof}
    [Bo: rewritten, needs a look.]
    ($\implies$)
    First, we note that the vertex condition is immediately implied by $\Opt(\T^r,C)$.

    [Bo: feel this is a bit shaky where it cites $E$ and $B$ stuff.] To show the halfspace condition is satisfied, we first construct a matrix $E$ such that $Ep \geq 0 \iff Bp \geq 0$, then use our construction to pick out the necessary vectors $v_1,\dots,v_k$.
    ...
    ($\impliedby$)
    The vertex condition gives that for all $j \in [\ell]$, there exist witnesses $\{X^j_y \in T^r_y : y \in \Y\}$ such that $\vec 0 = \sum_y p^j(y) X^j_y$.
    Every $p \in C$ is a convex combination of $\{p^1,\ldots,p^{\ell}\}$, so taking the corresponding convex combination of the witnesses proves that $\vec 0 \in \oplus_p \T^r$.

    It remains to show that if $\vec 0 \in \oplus_p \T^r$, then $p \in C$.
    By Lemma \ref{lemma:minkowski-support-all}, $\vec 0 \in \oplus_p \T^r$ implies that for all $v$, $\sum_y p(y) \max_{x \in T^r_y} \inprod{v}{x} \geq 0$.
    By the halfspace condition, there exist $v_1,\dots,v_k$ with $\max_{x \in T^r_y} \inprod{v_i}{x} \leq B_{iy}$, which implies $Bp \geq 0$, so $p \in C$.
    %% Bo: QED
...
%  	v_i, x_{jy} \in \reals^d && \forall i \in [k], j \in [\ell], y \in [n]
  ...
  \begin{theorem}
  	Given a convex polytope $C \subseteq \simplex$, there exist polytopes $\T^r$ such that $\Opt(\T^r, C)$ if and only if there is a feasible solution to the above quadratic program.
  \end{theorem}
  \begin{proof}
