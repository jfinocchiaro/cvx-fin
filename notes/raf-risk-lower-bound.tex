...
\begin{proof}
  ...
  In particular, if $\elic_\C(\Gamma') = \ell$\jessiet{Nitpick: $\ell$ used for target loss, so can we change later}, then we have such a $\hat\Gamma:\P\to 2^{\reals^\ell}$, and hence $\elic_\C(\Gamma) \leq \ell$.
...
\begin{lemma}\label{lem:lin-alg-nested-level-sets}
  ...
  for all $v\in S \defeq C \cap \ker f$, there exists a linear $\hat f_v:\V\to\reals^\ell$ with $v \in C \cap \ker \hat f_v \subseteq S$. \jessiet{Sounds like there is an $f_v$ per $v$, but $\ell$ the same for all $v$, and isn't able to be \emph{chosen}. Ok I think I'm just talking to myself now}
  Then $\ell \geq k$.
...
%  \raft{We want to show that the $\beta_i$ must be 0 (that's one definition of affine independence)}
  ...
  Now $v_\lambda \in C \cap \ker \hat f \subseteq S = C \cap \ker f$, and in particular $v_\lambda \in \ker f$.\jessiet{Typo?  $\ker \hat f \to \ker \hat f_v$?}
