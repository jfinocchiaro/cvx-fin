...
\begin{definition}
  ...
  Given loss function $L:\A\times\Y \to \reals$ for some report set $\A$, the \emph{Bayes risk} of $L$ is defined as $\lbar(p) := \inf_{a \in \A} \E_pL(a,Y)$.
\end{definition}
...
\begin{proof}
  ...
  In particular, if $\elic_\C(\Gamma') = \ell$, then we have such a $\hat\Gamma:\P\to 2^{\reals^\ell}$, and hence $\elic_\C(\Gamma) \leq \ell$.
...
\begin{lemma}\label{lem:lin-alg-nested-level-sets}
  ...
  for all $v\in S \defeq C \cap \ker f$, there exists a linear $\hat f_v:\V\to\reals^\ell$ with $v \in C \cap \ker \hat f_v \subseteq S$.
  Then $\ell \geq k$.
...
%  \raft{We want to show that the $\beta_i$ must be 0 (that's one definition of affine independence)}
  ...
  Now $v_\lambda \in C \cap \ker \hat f \subseteq S = C \cap \ker f$, and in particular $v_\lambda \in \ker f$.
