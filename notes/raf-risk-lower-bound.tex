...
%  \raf{We only reference this in the discussion and appendix (mode for finite $\Y$); let's remove it from this main def.}
  ...
  We define the \emph{range} of a property by $\range \Gamma = \bigcup_{p\in\P} \Gamma(p) \subseteq \R$. \jessiet{Technically, $\range \Gamma = \bigcup_{p \in \P} \cup \Gamma(p)$, right?}\raft{I'm not sure what you mean; check again?}
