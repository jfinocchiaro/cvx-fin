...
%  \raf{We only reference this in the discussion and appendix (mode for finite $\Y$); let's remove it from this main def.}
  ...
  We define the \emph{range} of a property by $\range \Gamma = \bigcup_{p\in\P} \Gamma(p) \subseteq \R$. \jessiet{Technically, $\range \Gamma = \bigcup_{p \in \P} \cup \Gamma(p)$, right?}\raft{I'm not sure what you mean; check again?}\jessiet{I was thinking about any value the property could possibly take, even if not uniquely, but I think that's wrong... feel free to delete/ignore this.}
...
\begin{proof}
  ...
  In particular, if $\elic_\C(\Gamma') = \ell$\jessiet{Nitpick: $\ell$ used for target loss, so can we change later}\raft{Ah, good point; let's change it when we integrate the files}, then we have such a $\hat\Gamma:\P\to 2^{\reals^\ell}$, and hence $\elic_\C(\Gamma) \leq \ell$.
  \raft{The logic for this whole proof could use a very careful check}
\end{proof}

\begin{lemma}
  \label{lem:elic-complex-bayes-concave}
  \label{LEM:ELIC-COMPLEX-BAYES-CONCAVE} % hilarious (sad) bug in imsart (refs in sec title)
...
\begin{lemma}\label{lem:lin-alg-nested-level-sets}
  ...
  Let $f:\V\to\reals^k$ be linear and $C\subseteq \V$ convex with $\spn C = \V$, and let $\ell\in\N$.
  Suppose that $0 \in \interior f(C)$, and
  for all $v\in S \defeq C \cap \ker f$, there exists a linear $\hat f_v:\V\to\reals^\ell$ with $v \in C \cap \ker \hat f_v \subseteq S$. \jessiet{Sounds like there is an $f_v$ per $v$, but $\ell$ the same for all $v$, and isn't able to be \emph{chosen}. Ok I think I'm just talking to myself now}\raft{Correct -- maybe you missed that $\ell$ was instantiated in the 2nd line?}
  Then $\ell \geq k$.
  If $\ell=k$, we additionally have $0\in\interior\hat f_v(C)$ for some $v\in S$.
...
\begin{proof}
  ...
  Let $\hat f_v:\V\to\reals^\ell$ be linear with $v \in \hat S := C \cap \ker \hat f_v \subseteq S$.
...
%  \raft{We want to show that the $\beta_i$ must be 0 (that's one definition of affine independence)}
  ...
  As $\hat f_v(C)$ contains $k+1$ affinely independent points, we have $\ell \geq \dim \im \hat f_v \geq k$.
  When $\ell=k$,
  ...
  \btw{This was surprisingly hard to pin down; perhaps there is a more direct way}
\end{proof}
...
\begin{proof}
  ...
  Let $\ell = \elic_{\C^*}(\lbar)$, so that we have $\hat\Gamma\in\C^*_\ell$ which refines $\lbar$.
  ...
  % ``Via the identity link function, we also have $\elic_{\C^*}(\hat\Gamma) \leq \ell$.
  % Moreover, from Lemma~\ref{lem:refine}, $\elic_{\C^*}(\lbar) = \ell \geq \elic_{\C^*}(\hat\Gamma) \geq \elic_{\C^*}(\Gamma)$.''
  ...
  We conclude $\ell \geq d$, and moreover, if $\ell=d$, then there exists some $q\in\Gamma_r$ such that $0 \in\interior\hat f_q(\P)$.
    
  Now suppose $\ell = d$ for a contradiction.
...
\end{proof}
  ...
  % \raf{OLD VERSION}
  % By refinement, if $p\in\Gamma_r$ then there exists $\hat r \in \reals^\ell$ with $p \in \hat \Gamma_{\hat r} \subseteq \Gamma_r$.
  ...
  % Thus, we have some $p^*\in\P$, $u\in\hat\Gamma(p^*)$, and $\hat V:\Y\to\reals^\ell$, such that, letting $\hat S\defeq \{p\in\P:\E_p\hat V = 0\}$, we have $p^* \in \hat S \subseteq \hat\Gamma_u \subseteq \Gamma_r$ and $0\in\interior\{\E_p\hat V : p\in\P\}$.
