...
%  \raf{We only reference this in the discussion and appendix (mode for finite $\Y$); let's remove it from this main def.}
  ...
  We define the \emph{range} of a property by $\range \Gamma = \bigcup_{p\in\P} \Gamma(p) \subseteq \R$. \jessiet{Technically, $\range \Gamma = \bigcup_{p \in \P} \cup \Gamma(p)$, right?}
...
%  \raft{We want to show that the $\beta_i$ must be 0 (that's one definition of affine independence)}
  ...
  Now $v_\lambda \in C \cap \ker \hat f_v \subseteq S = C \cap \ker f$, and in particular $v_\lambda \in \ker f$.\jessiet{Typo?  $\ker \hat f \to \ker \hat f_v$?}\raft{Yep, fixed}
