...
\begin{definition}[Strong Convexity and Strong Smoothness]
  Let $f:\reals^d\to\reals$ be a differentiable convex function.
  We say $f$ is $\alpha$-\emph{strongly convex} if
  \begin{equation}
...
\begin{theorem}
  Let $L:\reals^d \to \reals^\Y_+$ be a surrogate such that for all $y\in\Y$, the function $L(\cdot)_y$ is differentiable, $\alpha$-strongly convex, and $\beta$-strongly smooth.\raft{I think differentiability follows from smoothness...}
\end{theorem}
