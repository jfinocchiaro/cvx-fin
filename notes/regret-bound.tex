...
\begin{theorem}[new version; weaker assumptions]
  ...
  Suppose that there exist reports $r,r'$, distribution $p_0 \in \gamma_r \cap \gamma_{r'}$, and surrogate report $u_0 \in \Gamma(p_0)$ such that $u \mapsto \inprod{p}{L(u)}$ is $\alpha$-strongly convex in an open neighborhood of $u_0$.
...
\begin{proof}
  ...
  Let $\delta >0$ such that the above open neighborhood of $u_0$ contains the ball $B_\delta(u_0)$ of radius $\delta$.
  ...
  $\lambda^* := \alpha \delta^2 / (\alpha \delta^2 + 2L_1(u_0) - 2L_1(u_1))$,
  we have that $L_\lambda$ is minimized within $B_\delta(u_0)$.
  By $\alpha$-strong convexity of $L_0$ on $B_\delta(u_0)$, $L_\lambda$ is therefore $\alpha^*$-strongly convex on $B_\delta(u_0)$ for $\lambda \leq \lambda^*$, where $\alpha^* = (1-\lambda^*)\alpha$.\raft{Need to double-check this statement}
  ...
  We now upper bound $\|u_\lambda - u_0\|_2$, and then apply strong smoothness \raft{last hole to fill: use \emph{locally} Lipschitz instead of smoothness; should be fine since we apply it in a bounded region, e.g. around $u_1$} to upper bound $L_\lambda(u_0) - L_\lambda(u_\lambda)$.
