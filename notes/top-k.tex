...
\begin{align*}
B_p &:= \left\{i : p_i \leq \frac{1}{k+1} \wedge \frac{p_H k- |\bar H|}{k(k- |\bar H|)} \geq \frac 1 k - p_i\left(\frac{k+1}{k}\right) \right\}\\
...
\begin{proof}
	...
	\jessie{TODO}
	Observe that $p_H > \frac H k$, and therefore, $H < k$.
	Moreover, $M \leq k$, and if $M = k$, then $(1 - p_M) = p_H + p_B + p_L \leq \frac 1 {k+1}$.
	Additionally, we have $p_M > \frac {M}{k+1}$
	
	Now we have $p_B > \frac{Bk(1-p_H)}{(k+1)(k-H)}$.
	
	Since $p_H + p_M + p_B \leq 1$, I am hoping we get to a contradiction.
	
	\begin{align*}
	p_H + p_M + p_B &> 
	\frac{H}{k} + \frac M {k+1} + \frac{Bk(1-p_H)}{(k+1)(k-H)} \\
	&= \frac{H(k+1)(k-H)}{k(k+1)(k-H)} + \frac{Mk(k-H)}{k(k+1)(k-H)} + \frac{Bk^2(1-p_H)}{k(k+1)(k-H)}\\
	\end{align*}
...
\begin{claim}
	$\inprod{L(\hat \Gamma(p))}{p} = (1-p_H)(\bar u - \frac 1 k) + p_L)$. 
\end{claim}
\begin{proof}
	\begin{align*}
	\inprod{L(\hat \Gamma(p))}{p} &=
	\sum_{y \in H_p} L(\hat \Gamma(p))_y + \sum_{y \in M_p, B_p} L(\hat \Gamma(p))_y + \sum_{y \in L_p}L(\hat \Gamma(p))_y \\
	&= \sum_{y \in H_p} 0 + \sum_{y \in M_p, B_p} p_y(\bar u - \frac{1}{k}) + \sum_{y \in L_p} p_y(1 + \bar u - \frac 1 k) \\
...
Since $L^{top-k}$ is polyhedral, $\inprod{L^{top-k}(\hat\Gamma(p))}{p}$ is linear in any direction $\epsilon v$ for small enough $\varepsilon$, and therefore, we have 
\begin{align*}
...
&= L^*
\end{align*}
for any $\lambda = \{\lambda_i\} \in \simplex$.
Since $\{\varepsilon_{\hat y}\}_{\hat y \in \Y}$ forms a basis for $\reals^n$, we have the following corollaries.

\begin{proposition}
	For all $p \in \simplex$, $\hat \Gamma(p) \in \Gamma(p) \cap \U$.
\end{proposition}

\begin{corollary}
	$\hat \Gamma(\simplex)$ is a finite representative set.
\end{corollary}


This yields the finite loss $L|_{\hat \Gamma(\simplex)}$ embedded by $L^{top-k}$ by
\begin{align}
