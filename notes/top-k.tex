...
\jessie{WLOG $\to$ for all $p$, there exists $u \in \Gamma(p)$ such that....}
\jessie{Storyboard this a bit... err on the side of being too modular since it will be in the appendix.}
...
Throughout, we make two assumptions for ease of exposition, but for all $p \in \simplex$, there is a minimizer of $\inprod{p}{L^{top-k}(\cdot)}$ such that both statements hold.  %, as well as a third assumption when thinking about minimizing the expected loss.
...
\end{assumption}
This partially follows from the invariance of $L^{top-k}$ in the $\ones$ direction shown in Lemma~\ref{lem:invar-ones}, setting $u_{k+1} = 0$, and then showing weak dominance of the nonnegative $u'$ over any $u$ that contains negative components as in Lemma~\ref{lem:extra-elts-zero}.

\begin{lemma}\label{lem:extra-elts-zero}
	For all $y \in \Y$, there is a minimizer $u$ of $\inprod{p}{L^{top-k}(\cdot)}$ such that $u_{k+1} = \ldots = u_{n-1} = u_n = 0$.
\end{lemma}
\begin{proof}
	By Lemma~\ref{lem:invar-ones}, we can set $u_{k+1} = 0$.
	Given Assumption~\ref{assum:ordered-u}, we then have $u_i \leq 0$ for all $i \geq k+1$.
	...
	&\geq 1 + \frac {1}{k} \sum_{i=1}^k (u' - e_y)_{[i]}\\
	&= L^{top-k}(u')_y~.~
	\end{align*}
	The inequality comes from the equality of the first $k$ elements of $u$ and $u'$, combined with the assumption that $u_y \leq u'_y = 0$ in this case.
	
	Now, if $y < k+1$, then 
	\begin{align*}
	L^{top-k}(u)_y &= \left(1 - u_y + \frac 1 k \sum_{i=1}^k (u - e_y)_{[i]}\right)_+ \\
	&= \left(1 - u_y + \frac 1 k \sum_{i=1}^{k-1} (u - e_y)_{[i]} + \frac 1 k (u-e_y)_{[k]}\right)_+\\
	&= \left(1 - u'_y + \frac 1 k \sum_{i=1}^{k-1} (u' - e_y)_{[i]} + \frac 1 k (u-e_y)_{[k]}\right)_+
	\end{align*}
	If $u_y \geq 1$, then this is equal to $L^{top-k}(u')$.
	
	Now suppose that $u_y \in [0,1)$; we now observe $(u - e_y)_{[k]} = u_{[k+1]} = 0$. 
	\begin{align*}
...
\begin{lemma}\label{lem:top-threshold}
	For all $p \in \simplex$, there is a minimizer $u$ of $\inprod{L^{top-k}(\cdot)}{p}$ satisfying Assumptions~\ref{assum:ordered-u} and~\ref{assum:nonneg} so that for all $u_i$, we have $u_i \leq \bar u_{-i} +1$. 
\end{lemma}
\begin{proof}
...
\jessie{Revisit}
By Assumption~\ref{assum:nonneg}, if $u_y > \bar u_{-y} + 1$, then we have $y < k+1$.
Now, for any outcome $j \neq y$, we have 
\begin{align*}
L^{top-k}(u)_j &= \left( 1 - u_j + \frac 1 k \sum_{i=1}^k (u-e_j)_{[i]} \right)_+\\
	&= \left( 1 - u_j + \frac 1 k \sum_{i=1}^k (u'-e_j)_{[i]} + \frac {\epsilon}{k}\right)_+\\
	&= \left( 1 - u'_j + \frac 1 k \sum_{i=1}^k (u'-e_j)_{[i]} + \frac {\epsilon}{k}\right)_+	\\
	&> \left( 1 - u'_j + \frac 1 k \sum_{i=1}^k (u'-e_j)_{[i]}\right)_+\\
	&= L^{top-k}(u')_j
\end{align*}
Therefore, if $u$ minimizes $L(\cdot)_y$, then so does $u'$.
\end{proof}
...
\begin{proof}
	...
	This holds since $u_y \leq u_k \leq \bar u \leq \bar u$ by Assumptions~\ref{assum:ordered-u} and~\ref{assum:nonneg}.
	
	Now if $y < k+1$ and $u_y \in [0,1]$, then we have $(u-e_y)_{[k]} = 0$, and the above term becomes
	\begin{align*}
	&1 -u_y + \frac{1}{k} \sum_{i=1}^{k+1} u_{[i]} \geq 0\\
	...
	Now if $u_y > 1$, then $\sum_{i=1}^k (u-e_y)_{[i]} = \bar u - \frac 1 k$.
	Therefore, we have 
	\begin{align*}
	...
	\end{align*}
	with the final inequality coming from Assumption~\ref{assum:top-threshold}.
...
With Assumptions~\ref{assum:ordered-u}, \ref{assum:nonneg}, and \ref{assum:top-threshold}, we can rewrite the given surrogate loss with the above Lemma.
...
\begin{align*}
	...
	\end{align*}
	Observe here that $u_y = \min(u_y, 1)$.

	If $u_y > 1$, then $(u-e_y)_{[k]} = \frac{1}{k}(u_y - 1)$.
	This yields
	\begin{align*}
	L^{top-k}(u)_y &= 1 - u_y + \bar u - \frac{1}{k}~,~
	\end{align*}
	where, in a parallel manner to the above case, $1 = \min(u_y, 1)$, and this is again the term subtracted from $1 - u_y + \bar u$.
...
Because the upper bound on $u_y$ depends on $\ubar$, we cannot determine $u$ iteratively.
