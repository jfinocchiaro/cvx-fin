...
We will start with $\U$ as we show it is representative in Lemma~\ref{lem:top-threshold}, and narrow down to a finite representative set $\R$ for $L^{top-k}$.
...
\begin{proof}
	...
	Without loss of generality, we will fix a partition $\pi$ of the elements of $u$, and let $T_k(u)$ be the top $k$ elements of $u$ according to the partition $\pi$.
	Since $u \in \U \supseteq R^{\tt low}$, we have $u_{\pi_{k+1}} = \ldots = u_{\pi_n} = 0$, and take $k$ constraints constructing $R^{\tt low}$ given by $0 \leq u_{\pi_i}$ for $i \in [k]$.
	Call $T$ the top $k$ elements of the partition $\pi$.
	
	
	Moreover, there are $k$ constraints constructing $R^{\tt high}$, given by $	\forall i \in T_k(u), \,\,
	u_{i} \leq 1 + \frac{1}{k-1}\sum_{j \in T_k(u) \setminus \{i\}} u_j$.


	%need to introduce the segments where L is affine.
	Within each permutation $\pi$, we know that the third term is affine on each set $\{u \in \U \mid $ the top $k$ elements of $u$ are equal to $T\}$.
	...
	We have $L$ affine on the region $\{u \in \U \mid u \in V\}$ for each choice of $V \in \{A,B\}^k$.
	We will generate such sets using $T$ and $S$, the latter of which is introduced below.
	...
	For a fixed $T$ (the top $k$ elements of $\pi$), iterate over every $S \subseteq T$.
	Suppose $S$ is the set of elements of $u$ such that $0 \leq u_y \leq 1$ for all $y \in S$, and additionally have the constraints $1 \leq u_y \leq 1 + \frac 1 {k-1} \sum_{j \in T_k(u) \setminus \{i\}} u_j$.
	For each $T,S$ combination, we then have vertices at each of these $2^k$ possible equalities.
	\begin{align*}
	\forall i \in S, \,\, 0 &\leq u_i \leq 1 \\
	\forall i \in T \setminus S, \,\, 1 &\leq u_i \leq 1 + \frac 1 {k-1} \sum_{j \in T_k(u) \setminus \{i\}} u_j 
	\end{align*}
	Iterating over each of these $2^k$ inequalities, we see that vertices are generated at points $0,1$ or some constant $c_{TS}$ for each choice of inequalities for $T$ and $S$.

	Now, for each $T$ and $S$ and choice of equalities in each, let $M = \{y \mid u_y = 1\}$, and $H = \{y \in \Y \mid u_y = 1 + \sum_{j \in T_k(u) \setminus \{i\}} u_j\}$.
