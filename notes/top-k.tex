...
\begin{proof}
	...
	\implies B &\leq k- H - M\\
	%\implies B&< \frac{\epsilon (k-H)}{\epsilon + \frac M {k+1}}\\
	%\implies B&< \frac{(1 - \frac {H}{k} - \frac{M}{k+1}) (k-H)}{1 - \frac H k}\\
	%\iff B&< \frac{ (k - H - (k+1)M)(k-H)}{k-H}\\
	%\iff B&< k - H - M - kM \\
	%% &< \frac{(k+1)(k-H)}{k}
	%% &< \frac{k^2 + k - Hk - H}{k}\\
	%% &= k + 1 - H - \frac H k
...
\end{lemma}
\begin{proof}
	We denote $L^* := \inprod{L(\hat \Gamma(p))}{p}$ and $\bar u := \bar \sum_{i \in T_k(\hat\Gamma(p))} u_i$.
	We proceed in cases:
	...
	\begin{align*}
	\E_p L(\hat \Gamma(p) - \varepsilon_{\hat y}, Y) &=1 + \sum_{y \in \Y}p_y({\color{olive}\bar u - \frac 1 k} -{\color{blue} \frac{\varepsilon_{\hat y}}{k}}) + {\color{teal} p_{\hat y}\varepsilon_{\hat y}} \\%+ \sum_{y \in L_p}({\color{olive}p_y (1 + \bar u - \frac 1 k} -{\color{blue} \frac{\varepsilon_{\hat y}}{k}})\\
	\end{align*}
...
\implies \inprod{p}{L(\hat \Gamma(p))} &\leq \sum_i \lambda'_i \inprod{p}{L(a_i)} & \text{since $L$ convex} \\
...
\begin{proof}
	...
	Now for $r \in \R$ generated by the sets $H,M$ such that $H, M \subseteq [n]$, $H \cap M = \emptyset$, $|H| + |M| \leq k$, and $|H| < k$ with $k <n$, consider the distribution 
	...
	For any $H,M$ combination, we have the generated $p \in \simplex$ \jessie{flesh out that claim}, and therefore $r \in \hat \Gamma(p)$. 
	Thus, $\R \subseteq \hat \Gamma(\simplex)$.
	
	%$p_{HM} = (\frac{1}{k} + \varepsilon)\ones_{H} + (\frac{1}{k+1} + \varepsilon)\ones_M + (1 - ((\frac{|H|}{k} + |H| \varepsilon) + (\frac{|M|}{k+1} + |M| \varepsilon))) \ones_{[n] \setminus H\cup M} $.
\end{proof}

\begin{corollary}
	$\R$ is a finite representative set.
\end{corollary}

\section{Minimum-ness}
\begin{claim}
	For all $r \in \R$, there is a $p \in \simplex$ such that $\hat \Gamma(p)$ uniquely minimizes $L^{top-k}$.
\end{claim}
\begin{proof}[Sketch]
	Fix $r \in \R$, and consider $H,M$ so that $r$ is generated by $H$ and $M$.
	
	Since $L^{top-k}$ is affine on the regions $A^{T,S}$ for each $T,S$ affiliated with $H$ and $M$, we just need to show strict inequality in each of the cases of Lemma~\ref{lem:hat-gamma-opt-one-dir}.
	
	
	\jessie{Want to formalize, but here's the idea.}
	In Case 1, we would leave $\U$, so no ned to worry.
	In Case 2, we construct $p$ so that $p_{y} < \frac 1 k$, and thus the inequality is strict.
	In Case 3, since $y \in H_p$, we additionally have $p_y > \frac 1 k$.
	In Case 4a, we have $p_y < \frac 1 k$, and case 4b does not happen by construction.
