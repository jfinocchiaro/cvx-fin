...
\begin{lemma}\label{lem:extra-elts-zero}
  \raf{This statement seems off.  I think you want to say that for all $u\in\reals^d$, there is a $u'$ satisfying assumption~\ref{assum:nonneg} such that $L(u')_y \leq L(u)_y$ for all $y$ (pardon lazy notation).}
...
\begin{theorem}\label{thm:finite-set}
	...
	\end{align*}
	\jessie{Check the $|H|+|M| \leq k$ condition. Worst case, the set $\R$ is still finite without it.}\raf{follows from assump~\ref{assum:nonneg}, no?}
\end{theorem}
