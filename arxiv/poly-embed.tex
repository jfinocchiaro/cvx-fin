...
Thus, $L^2$ embeds the loss $\ell^2(r)_y = 1 - r_y + \tfrac 1 2 \inprod{r}{\ones-e_y}$ if $r_y = 1$ or $0$, and $0$ otherwise.
...
%\raf{ACKNOWLEDGEMENTS (commented out for submission)}
 ...
 We thank Peter Bartlett for several discussions early on, which led to our 1-dimensional characterization.  %to a proof of %\raf{1-d reduction} among other insights.
 Additionally, we thank Arpit Agarwal for early insights and bringing out attention to the abstain property.
 We thank Eric Balkanski for the insight that led to the proof of Lemma~\ref{lem:bar-f}.
 JF and RF were funded by NSF grant \#1657598.
\newpage
