...
\proposedadd{However, we should observe that this statement is not immediately an ``if and only if'' because property embedding does not capture the requirement of matching losses on embedded points.  See Lemma~\ref{lem:embed-defs-equiv} to see an equivalence between the two definitions of embedding.}
...
\end{lemma}
\begin{proof}
  \raft{All the important pieces are here, but take it slower.  State $\ell$ as a definition (let $\ell$ be given by...) and show why $L$ embeds it.}
  ...
  \raft{Spell this out; I don't think these distributions are enough, for the same reason as above}
...
\begin{proof}
  ...
  \raft{Again, a bit too brief.  Consider adding some of this statement into the Lemma.}
  ...
  \raft{Why is it nonempty though?}
  ...
  \raft{These next two directions together seem like what we did before for $2\Rightarrow 3$, correct?  (Not as complete though; see the step where we justify the conditions of Lemma~\ref{lem:loss-restrict}.)  I'm actually happy for them to be separated, but let's make sure the argument is just as complete.}
	2 $\Rightarrow$ 1:
...
\begin{proof}
  ...
  \raft{Implicitly? Justify the claim}
