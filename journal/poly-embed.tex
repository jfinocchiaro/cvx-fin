...
\begin{corollary}
	The necessary and sufficient conditions for calibrated surrogates from Ramaswamy, but in terms of the unlabeled property.
\end{corollary}
\hrule
...
\jessie{Everything below here is added from the COLT version}
...
\begin{proof}
	Let $\gamma: \simplex \toto \R$.
	Proposition~\ref{prop:embed-trim} gives us that $\trim(\Gamma) = \{\gamma_r : r \in \R\}$.
	We conclude that for any $u \in \reals^d$, there is a calibrated link from $\Gamma$ to $\gamma$.\jessie{define calibrated link for properties?}
	\jessie{TODO}
\end{proof}

\jessie{It looks like this was in the body of the NeurIPS paper ot one point.  Not sure if we want to move it back there.}
The following result emphasizes the significance and sufficiency of polyhedral losses in the context of studying embedding.
\begin{theorem}
	Let $\gamma$ be a finite property.  
	The following are equivalent.
	\begin{enumerate}
		\item $\gamma$ is elicitable.
		\item $\gamma$ is embeddable.
		\item $\gamma$ is embeddable by a polyhedral loss.
	\end{enumerate}
\end{theorem}
\begin{proof}
	Trivially, we have $3 \implies 2$, and $2 \implies 1$ follows as a result of Proposition~\ref{prop:embed-trim}.
	We then show $1 \implies 3$.
	For any non-redundant property $\gamma'$, even if not finite, elicited by a loss with a Bayes Risk which is a closed function, is embeddable in $d = n-1$ dimensions.
	By Lemma \jessie{ref}, the (negative) Bayes risk $G$ of the given finite elicitable property $\gamma$ is polyhedral and therefore closed.
	Take $L(u) = C(u)\ones - u$, where $C = G^*$ is the convex conjugate of $G$, which is also polyhedral by~\cite[Theorem 19.2]{rockafellar1997convex}.
	Therefore, $L$ is polyhedral.
\end{proof}
...
\jessie{Other results on refined properties?}
