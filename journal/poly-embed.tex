...
\end{proof}
\begin{proof}[Via properties]
	\jessie{($L$ embeds $\ell \implies \risk{L} = \risk{\ell}$.)  $L$ embeds $\ell$ implies $\Gamma$ embeds $\gamma$, which is finite as $\ell$ is discrete.  By Prop 3, then we have $\{\gamma_r : r \in \R\} = \trim(\Gamma) = \{\Gamma_u : u \in \U \subset \reals^d \}$, where $\U$ is finite. Since $L$ embeds $\ell$}
	\jessie{$(\risk{L} = \risk{L} \implies L$ embeds $\ell$).  $\risk{L} = \risk{\ell}$ implies $\{\gamma_r : r \in \R\} = \trim(\Gamma)$, and therefore $\Gamma$ embeds $\gamma$, satisfying the first two conditions.  For the matching losses, this follows from the risks being equal...?}
\end{proof}
...
Before we state the Proposition needed to prove many of the statements in Section~\ref{sec:poly-loss-embed}, we will need to general lemmas about properties and their losses. \jessie{I think this is what's throwing me off 2.20.20}
...
\begin{proof}
	...
	1 $\Rightarrow$ 2:
        \jessie{Intuition: $\Gamma$ embeds $\gamma$ implies their level sets are the same on facets, and $\trim(\Gamma)$ gets rid of any redundancies in $\Gamma$'s level sets.  Moreover, $\cup \trim(\Gamma) = \simplex$ since for all $\theta \in \trim(\Gamma)$, there is a $\gamma_r \in \gamma(\simplex)$ so that $\theta = \gamma_r$ and $\gamma$ is non redundant.}
        \jessie{Loss intuition: If $\Gamma$ embeds a finite $\gamma$, $L$ must embed a discrete loss $\ell$ such that $\ell(r) = L(\varphi(r))$.  Therefore, $\risk{L} = \risk{\ell}$ is polyhedral. }
	Since $\Gamma$ embeds $\gamma$, we know that $\gamma_r = \Gamma_{\varphi(r)}$.
	If we can show $\trim(\Gamma) = \{\Gamma_{\varphi(r)}\}_{r \in \R}$, then it suffices to see the result in terms of $\{\Gamma_{\varphi(r)}\}_{r \in \R}$.
	\hrule
	First, to see $\trim(\Gamma) \subseteq \Gamma_{\varphi(\R)}$.
	Take $\theta \in \trim(\Gamma)$.
	There must be some $u$ such that $\theta = \Gamma_u$.
	
	
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	Take some $u \not \in \varphi(\R)$.
	We want to show that there exists a $r \in \R$ such that $\Gamma_u \subseteq \Gamma_{\varphi(r)}$, and therefore, $\Gamma_u \not \in \trim(\Gamma)$.
	
	Consider any $p$ such that $p \in \Gamma(u)$.
	Since $\gamma$ is nondegenerate, we know $p \in \gamma(r)$ for some $r \in \R$, and by $\Gamma$ embedding $\gamma$, we also have $p \in \Gamma(\varphi(r))$.
	Therefore, $\Gamma(u) \subseteq \Gamma(\varphi(r))$.
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	
	
	Next, to see $\Gamma_{\varphi(\R)} \subseteq \trim(\Gamma)$.
	Take $r \in \R$, and call $\theta = \Gamma_{\varphi(r)} = \gamma_r$.
	As $\gamma$ is nonredundant, we know there is no $r' \in \R$ such that $\gamma_{r'} = \Gamma_{\varphi(r')} \subseteq \theta$, and therefore $\theta \in \trim(\Gamma)$.
	\hrule
	Now that we have established $\trim(\Gamma) = \Gamma_{\varphi(\R)}$, we we show the result.
	As $\R$ is a finite set, so is $\Gamma_{\varphi(\R)}$ as it is finitely generated.
	Moreover, $\gamma$ is nondegenerate, we then have $\cup_\R \Gamma_{\varphi(r)} = \cup_\R \gamma_r = \simplex$.
	
	2 $\Rightarrow$ 3: \jessie{Intuition: Each $\theta \in \trim(\Gamma)$ is full dimensional.  Take $\Theta = \trim(\Gamma)$.}
    
    \hrule \jessie{New}
    Each $\theta_i \in \Theta$ has some $u_i$ such that $\theta_i = \Gamma_{u_i}$.
    As $\cup \trim(\Gamma) = \cup_i \theta_i = \cup_i \Gamma_{u_i}$, we then have $\cup \Gamma_{u_i} = \simplex$
    
    If $\Gamma_{u_i}$ was not full-dimensional, then it would not be in $\trim(\Gamma)$
    
    \hrule
    
    Let $\R = \{u_1,\ldots,u_k\} \subseteq\reals^d$ be a set of distinct reports such that $\trim(\Gamma) = \{\Gamma_{u_1},\ldots,\Gamma_{u_k}\}$.
