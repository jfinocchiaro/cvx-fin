...
While property elicitation has by now an extensive literature~\citep{savage1971elicitation,osband1985information-eliciting,lambert2008eliciting,gneiting2011making,steinwart2014elicitation,frongillo2015vector-valued,fissler2016higher,lambert2018elicitation}\raft{others? already a bit much...}, these works are mostly concerned with point estimation problems.
Literature directly connecting property elicitation to consistency is sparse, with the main reference being~\citet{agarwal2015consistent}; note however that they consider single-valued properties in finite outcome settings, whereas properties elicited by general convex losses are necessarily set-valued.
...
\end{proposition}
\begin{proof}
  \raft{$\implies$: Expand a bit more; you want to give the intuition for the ``show'', i.e., why/how can one show this? \jessie{Done}}
...
\end{proposition}
\begin{proof}
  \raft{Great.  I'd add the piece about taking representatives (how do we construct the finite property?) though. \jessie{Added}}
...
\jessiet{The commented out section is now in colt-20, but I'm adding a bit to say why this is cool.}
...
\proposedadd{However, we should observe that this statement is not immediately an ``if and only if'' because property embedding does not capture the requirement of matching losses on embedded points; i.e. that $L(\varphi(r)) = \ell(r)$ for all $r \in \R$.  Lemma~\ref{lem:embed-defs-equiv} shows an equivalence between the two definitions of embedding.}

\jessiet{Lemma new to journal version here:}
