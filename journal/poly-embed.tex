...
\end{proposition}
\begin{proof}
  \raft{$\implies$: Expand a bit more; you want to give the intuition for the ``show'', i.e., why/how can one show this?}
  \raft{$\impliedby$: First 2 sentences are good, but then it fogs up a bit.  3rd needs another careful pass, e.g., supporting hyperplane at what point?   4th cannot be right since you don't cover all reports $r$}
	\jessie{Intuition: $\implies$: Take the injection given and take $\U = \varphi(\R)$.
		Since we embed, the loss values match at these embedding points, and we know the surrogate has no ``strictly better'' report than the embedding, otherwise we would contradict the fact that $L$ embeds $\ell$.  
		At any $p \in \simplex$, suppose $r \in \argmin_{r'} \inprod{\ell{r}}{p}$.
		As $\ell(r) = L(\varphi(r))$, we then have $\inprod{p}{\ell(r)} = \inprod{p}{L(\varphi(r))}$.
	The property $\gamma'$ elicited by $L|_\U$ where $\risk{L} = \risk{L|_\U} = \risk{\ell}$.
	...
	Since each $\gamma_r$ is full-dimensional, we know there is a report $u_r$ such that $u_r \in \Gamma(\tilde p)$ for some $\tilde p \in \inter{\gamma_r}$.
...
\end{proposition}
\begin{proof}
  \raft{Great.  I'd add the piece about taking representatives (how do we construct the finite property?) though.}
	\jessie{Intuition: $\Leftarrow$: There are a finite set of full-dimensional level sets, which we enumerate.  We then use these enumerated reports to define a finite property (i.e. $u_i \in \Gamma(p) \implies i \in \gamma(p)$), and take the discrete loss values equal to original loss values at the enumerated points.}
...
\jessie{New Lemma here}
\begin{lemma}\label{lem:embed-defs-equiv}
%  \raft{Introduce $L$}
	Let $L : \reals^d \to \reals^\Y_+$ be a loss function.
	If the elicitable property $\Gamma := \prop{L}$ embeds the finite property $\gamma : \simplex \toto \R$, then there is a discrete loss $\ell$ such that $\ell$ elicits $\gamma$ and $L$ embeds $\ell$.
\end{lemma}
\begin{proof}
  \raft{All the important pieces are here, but take it slower.  State $\ell$ as a definition (let $\ell$ be given by...) and show why $L$ embeds it.\jessie{Took another pass but changed technique, adding in this detail.}}
  	Consider a loss $\ell : \R \to \reals_+^\Y$, where $\R$ is the same as range of the property $\gamma$.
    Define $\ell : r \mapsto L(\varphi(r))$, where $\varphi$ is the same embedding given by $\Gamma$ embedding $\gamma$.
        
    Now, we claim that $L$ embeds $\ell$.
    To see this, consider that Condition (i.) is met by construction of $\ell$.
    Moreover, by Proposition~\ref{prop:embed-bayes-risks}, showing $\risk{L} = \risk{\ell}$ suffices to show that $L$ embeds $\ell$.
    
    We aim to show that for all $p \in \simplex$, we have $\inf_{u \in \reals^d}{p}{L(u)} (1) = (2) \inf_{r \in \R}{p}{L(\varphi(r))}$, and the equality of risks follows.
    $(1) \leq (2)$ follows from the fact that $\varphi(\R) \subset \reals^d$. 
    Consider that if we did not have $(1) \geq (2)$ for some $p \in \simplex$, then there would be no $r$ such that $\varphi(r) \in \Gamma(p)$, and therefore the property it embeds, $\gamma$ is degenerate at $p$, i.e. $\gamma(p) = \emptyset$, yielding a contradiction.
    Therefore, we must have $\risk{L} = \risk{\ell}$.
    
    Now, to verify $\ell$ elicits $\gamma$, consider $r \in \gamma(p) \iff \varphi(r) \in \Gamma(p) \iff \varphi(r) \in \argmin_{u \in \reals^d} \inprod{p}{L(u)} \iff r \in \argmin_{r \in \R} \inprod{p}{\ell(r)}$.
