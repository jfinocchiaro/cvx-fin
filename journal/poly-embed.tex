...
%	However, if there was a level set $\Gamma_{\varphi(\hat r)}$ that was not in $\trim(\Gamma)$ \jessie{\ldots}
	...
	1 $\Rightarrow$ 3: Since $\Gamma$ embeds the finite property $\gamma$, Lemma~\ref{lem:embed-defs-equiv} yields a loss $\ell$ eliciting $\gamma$ embedded by $L$, and therefore $\risk{L} = \risk{\ell}$.
