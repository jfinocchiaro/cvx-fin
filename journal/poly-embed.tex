...
\begin{proof}
	...
	1 $\Rightarrow$ 2:
        ...
        We then observe that $\trim(\Gamma) = \{\Gamma_{\varphi(r)} : r \in \R \} = \{\gamma_r : r \in \R\}$ (chaining Lemma~\ref{lem:elicitable-level-sets} and a corollary of the embedding definition).
...
%	However, if there was a level set $\Gamma_{\varphi(\hat r)}$ that was not in $\trim(\Gamma)$ \jessie{\ldots}
	...
	3 $\Rightarrow$ 1: \jessie{Intuition: Take $\gamma = \Gamma|_{\varphi(\R)}$.}
...
\begin{proof}
	\begin{itemize}
		\item [$\subseteq$] As $\gamma$ is finite (and implicitly non-redundant), we know that each level set of $\gamma$ must be full-dimensional. For all $r \in \R$, we know $\gamma_r = \gamma_{\varphi(r)}$ by the definition of embedding, so $\Gamma_{\varphi(r)} \in \trim(\Gamma)$ by Lemma~\ref{lem:fdls-in-trim}.
...
\begin{lemma}\label{lem:elicitable-level-sets}
	Let $\Gamma$ be an elicitable property.
	For all $p \in \simplex$, we have $|\Gamma(p)| > 1 \iff \exists r \in \Gamma(p)$ such that $\Gamma_{r'} \subseteq \Gamma_r$ for all $r' \in \Gamma(p)$.
\end{lemma}
\begin{proof}
	\jessie{TODO: argument on subgradients of the concave score over the loss.}
\end{proof}
...
\end{theorem}
\begin{proof}
	Let $\L$ be a polyhedral loss, and $\Gamma$ by the unique property it elicits.
	By Theorem~\ref{thm:poly-embeds-discrete}, we know there is a discrete loss $\ell$ embedded by $L$.
	Take $\gamma$ to be the finite property elicited by $\ell$.
	\jessie{TODO After clarifying with Raf and Bo}
\end{proof}
\begin{proof}[Original]
