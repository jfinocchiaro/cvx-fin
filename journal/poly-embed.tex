...
\jessie{
	\begin{itemize}
		\item Add figures of Bayes risks of 0-1, logistic, and hinge losses (Jessie: Have made them, not sure where would be the best place to insert them/if you think this would be helpful)
		...
		\item Theorem 2: move from n-dimensional construction to n-1 dimensional construction (or add n-1 dim construction in appendix if it gets too hairy?)
		\item Links: cool to point out that when L is polyhedral, \prop{L} has a finite range (this result) and so does its (multivalued map) inverse (trim result)
		\item Links: sketch vs full proof
		\item Commentary on how any loss embedding ell must be polyhedral ish
	\end{itemize}}
...
Moreover~\citet{zhang2020bayes} describes the impact of the hypothesis class has on consistency, and when consistency relative to the hypothesis class differs from Bayes consistency; the latter is what we describe in this paper when we say ``consistency.''
...
\citet{finocchiaro2020indirect}\jessiet{cvx-flats paper; might need to throw on arxiv if we want this ref.} additionally relates indirect property elicitation to consistency in finite outcome settings\jessiet{flag} when one is given either a target loss or property in both discrete and continuous prediction settings, assuming surrogates attain their infimum in expectation over all distributions over the outcomes.
...
After introducing notation, and terminology from property elicitation, we study \emph{calibration} (Definition~\ref{def:calibrated}), which is equivalent to consistency in finite outcome settings~\citep{bartlett2006convexity,tewari2007consistenc,ramaswamy2016convex} and depends solely on the conditional distribution over $\Y$.
...
\end{definition}
It is well-known in finite-outcome settings that calibration implies\jessiet{is equivalent to?} \emph{consistency}, in the following sense (cf.~\citep{agarwal2015consistent}).
...
\end{proposition}
\begin{proof}
  \raft{$\impliedby$: First 2 sentences are good, but then it fogs up a bit.  3rd needs another careful pass, e.g., supporting hyperplane at what point?   4th cannot be right since you don't cover all reports $r$ \jessie{Took another pass}}
	\jessie{Intuition: $\implies$: Take the injection given and take $\U = \varphi(\R)$.
		Since we embed, the loss values match at these embedding points, and we know the surrogate has no ``strictly better'' report than the embedding, otherwise we would contradict the fact that $L$ embeds $\ell$, so the embedded report must be in the argmin of the surrogate restricted to $\U$, that is, $L|_\U$.  
		At any $p \in \simplex$, suppose $r \in \argmin_{r'} \inprod{\ell(r)}{p}$.
		As $\ell(r) = L(\varphi(r))$, we then have $\inprod{p}{\ell(r)} = \inprod{p}{L(\varphi(r))}$, and therefore we observe $\risk{L|_\U} = \risk{\ell}$ on $\simplex$.
	The property $\gamma'$ elicited by $L|_\U$ is then nonempty for all $p \in \simplex$.
	Moreover, we have $\risk{L} = \risk{L|_\U}$ by construction of $\gamma'$ and its nondegeneracy.
	$\impliedby$: We know $\risk{\ell}$ is polyhedral, and classic results (Aurenhammer) show that the projection of the facets of the epigraph of $\risk{\ell}$ form a power diagram on the simplex.  Each cell of this power diagram corresponds to a level set of $\gamma_r$.  A corollary of each level set being a cell in a power diagram is that each $\gamma_r$ is full-dimensional.
	We know there is a report $u_r$ such that $u_r \in \Gamma(\tilde p)$ for some $\tilde p \in \inter{\gamma_r}$.
	Moreover, the point $(p, -\inprod{p}{L(u_r)})$ forms a supporting hyperplane of $-\risk{L}$ (and therefore $-\risk{\ell}$ as well) if and only if $u_r \in \Gamma(p)$ for $p \in \Gamma_{u_r}$. 
	As $(p, -\inprod{p}{L(u_r)})$ also forms a supporting hyperplane of $-\risk{\ell}$, we must then have $p \in \gamma_r$.
	As this is true for all $p \in \Gamma_{u_r}$, we must have $\Gamma_{u_r} = \gamma_r$.
	Losses match by considering the supporting hyperplanes to the facets of the epigraphs of the Bayes Risks are the same, and the loss values are uniquely determined by the final coordinate by this final coordinate of the supporting hyperplane.}
...
%  Let $\Gamma = \prop{L}$ and $\gamma = \prop{\ell}$.
  ...
  As $L(\varphi(\cdot)) = \ell(\cdot)$ by embedding condition (i), we have
  \begin{equation*}
  ...
  In what follows, we implicitly work in the affine hull of $\simplex$ by taking the appropriate projection into $\reals^{n-1}_+$, so that interiors are well-defined, and $\risk{\ell}$ may be differentiable on the interior of $\simplex$.
  (For intuition, one can consider the relative interiors in $\reals^n_+$, but we need to consider interiors in order to apply previous results.)
  Since $\ell$ is discrete, $-\risk{\ell}$ is polyhedral as the pointwise maximum of a finite set of linear functions.
%  Moreover, consider the linear transformation $P: \ell \mapsto [\mathbb{I}_{n-1}, \vec 0] \risk{\ell}$ projecting $\risk{\ell}$ down to $n-1$ dimensions.
  The projection of its epigraph $E_{\ell}$ onto $\simplex$ forms a power diagram by Theorem~\ref{thm:aurenhammer}, whose cells are full-dimensional (in $n-1$ dimensions) and correspond to the level sets $\gamma_r$ of $\gamma = \prop{\ell}$.
...
%  \btw{RF: This is on the informal side, mostly because of the ``working in the affine hull'' bit.  It can all be formalized though, probably should be for the journal version: just take an appropriate linear projection of $\reals^n$ to $\reals^{n-1}$, and work with the projected versions of everything, and then project back.}
%  For the reverse implication, assume that $\risk{L} = \risk{\ell}$.
%  In what follows, we implicitly work in the affine hull of $\simplex$ \proposedadd{by projecting into $\reals^{n-1}$}, so that interiors are well-defined, and $\risk{\ell}$ may be differentiable on the (relative) interior of $\simplex$.
%  \proposedadd{Formally, we work with $L' := P(L)$ and $\ell' := P(\ell)$, where $P:\reals^n \to \reals^{n-1}$ is the linear transformation given by $[\mathbb{I}_{n-1}, \vec 0]$, taking the first $n-1$ components of $L$ and $\ell$, respectively.}
%  Since $\ell$ is discrete,\proposedadd{ so is $\ell'$, and thus} $-\risk{\ell}$ is polyhedral as the pointwise maximum of a finite set of linear functions.
...
For example, this can be seen by a loss that is polyhedral in the convex hull of $\phi(\R)$, but smooth outside that region.
...
\end{proposition}
\begin{proof}
	\jessie{Intuition: $\Leftarrow$: There are a finite set of full-dimensional level sets, which we enumerate.  We then use these enumerated reports to define a finite property $\gamma$ such that $u_i \in \Gamma(p) \implies i \in \gamma(p)$, and take the discrete loss values equal to original loss values at the enumerated points.}
...
Note however that the number of dimensions $d$ required could be as large as $|\Y|$, which is particularly undesirable in structured prediction problems with exponentially many outcomes.
Recent work~\citep{ramaswamy2016convex,finocchiaro2020embedding} yield characterizations for bounding the convex calibration and embedding dimensions.
...
\proposedadd{Moreover, to construct an embedding of dimension $n-1$ instead of $n$ (see~\citet{finocchiaro2020embedding} for significance of dimension),  we use the same trick as in the proof of Proposition~\ref{prop:embed-bayes-risks}.
For $p \in \simplex$, consider $p_{-n}$ to be the first $n-1$ coordinates of $p$ identically projected into $n-1$ dimensional space subject to the affine constraint $\sum_y p_y = 1$, so that $p_n = 1 - \sum_{y=1}^{n-1} p_y$. 
Take $L' : \reals^{n-1} \to \reals^\Y$ defined by $u \mapsto (-\ell')^*(u) \ones - u$, where $\ell'$ is $\ell$ projected into $n-1$ dimensions as in Proposition~\ref{prop:embed-bayes-risks}, so that $\inprod{\ell}{p} = \ell'(p_{-n})$ for all $p \in \simplex$.
The Bayes risks $\risk{L}(p)$ and $\risk{L'}(p_{-n})$ are equal for all $p \in \simplex$; thus, the result holds in $n-1$ dimensions as well.}\jessiet{Leaving this change in the macro since we haven't made the change in proposition 1 yet.}
...
%\raf{Please fix refs, maybe by removing the duplicates from the appendix}
...
\jessiet{New to journal version}
\begin{lemma}\label{lem:embed-defs-equiv}
%  \raft{Introduce $L$}
	Let $L : \reals^d \to \reals^\Y_+$ be a loss function.
	If the property $\Gamma := \prop{L}$ embeds the finite property $\gamma : \simplex \toto \R$, then there is a discrete loss $\ell$ such that $\ell$ elicits $\gamma$ and $L$ embeds $\ell$.
\end{lemma}
\begin{proof}
  \raft{All the important pieces are here, but take it slower.  State $\ell$ as a definition (let $\ell$ be given by...) and show why $L$ embeds it.\jessie{Took another pass but changed technique, adding in this detail.  Now very similar to the proof of Prop 1 forward direction.}}
  	Consider a loss $\ell : \R \to \reals_+^\Y$, defined so that $\ell : r \mapsto L(\varphi(r))$, i.e. $\ell = L|_{\varphi(\R)}$, where $\varphi$ is the same embedding given by $\Gamma$ embedding $\gamma$.
    Now, we claim that $L$ embeds $\ell$.
%    To see this, consider that Condition (i.) of matching embedded loss values is met by construction of $\ell$.
    Using Proposition~\ref{prop:embed-bayes-risks}, showing $\risk{L} = \risk{\ell}$ suffices to show that $L$ embeds $\ell$.
    ...
    Consider that if we did not have $(1) \geq (2)$ for some $p \in \simplex$, then there would be no $r$ such that $\varphi(r) \in \Gamma(p)$.
    Therefore the property it embeds, $\gamma$ is degenerate at $p$, i.e. $\gamma(p) = \emptyset$, yielding a contradiction.
    This gives us $(1) = (2)$, so we have $\risk{L} = \risk{L|_{\varphi(\R)}} = \risk{\ell(r)}$ by construction of $\ell$.
    %
...
By Lemma~\ref{lem:embed-defs-equiv}, we can work directly with properties and set aside the losses which elicit them, which is often more convenient.
Moreover, we can assume that finite properties are non-redundant without much loss of generality.
...
When working with convex losses which are not strictly convex, one quickly encounters redundant properties: if $\inprod{p}{L(\cdot)}$ is minimized by a point where $\inprod{p}{L}$ is flat, then there will be an uncountable set of reports which also minimize the expected loss.
As results in property elicitation typically assume properties are non-redundant (e.g.~\cite{frongillo2014general,frongillo2015elicitation}), it is useful to consider a transformation which removes redundant level sets, captured by the \emph{trim} operation.
...
Before we state Proposition~\ref{prop:embed-trim}, which is needed to prove many of the statements in Section~\ref{sec:poly-loss-embed}, we will need to general lemmas about properties and their losses.
...
\begin{lemma}\label{lem:embedded-level-sets-trim}
	...
	If $\Gamma$ embeds a (non-redundant) finite property $\gamma : \simplex \toto \R$ by the injection $\varphi$, then $\{\gamma_r : r \in \R\} = \{\Gamma_u : u \in \varphi(\R)\} = \trim(\Gamma)$.
\end{lemma}
\begin{proof}
	Let $L$ elicit $\Gamma$.
	...
	As $\gamma$ is finite (and non-redundant by assumption), we know that each level set of $\gamma$ must be full-dimensional. 
	...
	%		
	%		\item[$\supseteq$] Take $u'$ so that $\Gamma_{u'} \not \in \{\Gamma_u : u \in \varphi(\R)\}$.
	%		We proceed in 3 cases: first, if there exists a $u$ such that $\Gamma_{u'} \subsetneq \Gamma_u$, then $\Gamma_{u'} \not \in \trim(\Gamma)$.
	%		If there is a $u \in \varphi(\R)$ such that $\Gamma_u = \Gamma_{u'}$, then we have $\Gamma_{u'}$ in the original set, as well as in $\trim(\Gamma)$ by Lemma~\ref{lem:fdls-in-trim}, since $\Gamma_{u'}$ must then be full-dimensional.
	%		In the final case, we have $\Gamma_{u'}$ not a subset of any $\Gamma_\varphi(r)$.
	%		Then by nondegeneracy of $\gamma$ (and in turn, of $\Gamma$) spans the interior of multiple level sets in $\{\Gamma_u : u \in \varphi(\R)\}$, which contradicts the elicitability of $\Gamma$ by Lemma~\ref{lem:elicitable-level-sets}.
	%		Thus, we cannot observe this case.
	%	\end{itemize}
\end{proof}
...
\end{proposition}
\begin{proof}\jessie{Consolidated; original below.}
	Let $L$ elicit $\Gamma$.
	
	1 $\Rightarrow$ 2:
        %\jessie{Loss intuition: Let $L$ elicit $\Gamma$.  If $\Gamma$ embeds a finite $\gamma'$, $L$ must embed a discrete loss $\ell$ such that $\ell(r) = L(\varphi(r))$.  Therefore, $\risk{L} = \risk{\ell}$ is polyhedral. Moreover, we have $\Gamma$ also embedding the finite property $\prop{\ell} := \gamma$ as a corollary of the definition of embedding properties. We claim $\trim(\Gamma) = \{\gamma_r : r \in \R\}$ (Lemma~\ref{lem:embedded-level-sets-trim}), and since the latter set is finitely generated (as $\R$ is finite), we have have $\trim(\Gamma)$ is finite.  Moreover, as $\gamma$ is nondegenerate (since it is defined over the simplex), we have that $\cup_{r \in \R} \gamma_r = \simplex$.}
  If $\Gamma$ embeds a finite $\gamma$, then Lemma~\ref{lem:embed-defs-equiv} says there is be a discrete loss $\ell$ embedded by $L$ such that $\ell$ elicits $\gamma$.
  \raft{Again, a bit too brief.  Consider adding some of this statement into the Lemma. \jessie{Is this better?}}
  By Lemma~\ref{lem:embedded-level-sets-trim}, we then observe that $\trim(\Gamma) = \{\Gamma_{\varphi(r)} : r \in \R \} = \{\gamma_r : r \in \R\}$.
%  \raft{Replace with ``As $\R$ is finite, we conclude $\trim(\Gamma)$ is finite.'' or similar}
  As $\R$ is finite, we conclude that $\trim(\Gamma)$ is also finite.
%  Since the latter set is finitely generated (as $\R$ is finite), we have have $\trim(\Gamma)$ is finite.
  \raft{Why is it nonempty though?  \jessie{Just above definition 2, we specify that properties are nonempty}}
  Moreover, as $\gamma$ is nondegenerate (recall that $\gamma: \simplex \toto \R$ is shorthand for $\gamma: \simplex \to 2^\R \setminus \emptyset$), we have that $\cup \trim(\Gamma) = \cup_{r \in \R} \gamma_r = \simplex$.

  \raft{These next two directions together seem like what we did before for $2\Rightarrow 3$, correct?  (Not as complete though; see the step where we justify the conditions of Lemma~\ref{lem:loss-restrict}.)  I'm actually happy for them to be separated, but let's make sure the argument is just as complete. \jessie{Added in the conditions of Lemma 3 to be a bit more thorough.}}
	2 $\Rightarrow$ 1:
	...
	We have $\U \subset \reals^d$ and $\Gamma(p) \cap \U \neq \emptyset$ for all $p$, since we have $p \in \Gamma_{u_i}$ for some $u_i \in \U$.
	Then we can apply Lemma~\ref{lem:loss-restrict} to observe that $L$ embeds $\ell$, and therefore $\Gamma$ embeds $\prop{\ell}$, which is finite as $\ell$ is discrete.
	
%	1 $\Rightarrow$ 3: \jessie{Intuition: Each $\theta \in \trim(\Gamma)$ is full dimensional by Lemma~\ref{lem:embedded-level-sets-trim} and equality of the set of embedded level sets of $\Gamma$ and set of level sets of $\gamma$.  Take $\Theta = \trim(\Gamma)$.}
%	Assume $\Gamma$ embeds a finite property $\gamma$.
%	Then for each $r \in \R$, we have $\Gamma_{\varphi(r)} = \gamma_r$, and therefore $\{\Gamma_{\varphi(r)} : r \in \R\} = \{\gamma_r : r \in \R\}$.
%	Chaining this with Lemma~\ref{lem:embedded-level-sets-trim}, we have $\trim(\Gamma) = \{\gamma_r : r \in \R\}$.
%	
%	\jessie{This is really the beginning of 2 $\Rightarrow 3$}
%	If there is a level set $\Gamma_{\varphi(\hat r)} \in \trim(\Gamma)$ that is not full-dimensional, then we can define an injection $\varphi'$ so that $\varphi' :\R \setminus \{\hat r\} \to \reals^d$ maps $r \mapsto \varphi(r)$.
%	(Iterate if there are multiple level sets that are not full-dimensional.)
%	Now take $\Theta = \{\Gamma_{\varphi'(r)} : r \in \R\}$.
%	We know each level set in $\Theta$ is full-dimensional by construction, and just need to show $\cup \Theta = \simplex$.
%	However, if there was a level set $\Gamma_{\varphi(\hat r)}$ that was not in $\trim(\Gamma)$ \jessie{\ldots}

	1 $\Rightarrow$ 3: 
	Let $\varphi: \R \to \reals^d$ by the embedding function, and take $\Theta = \{\Gamma_{\varphi(r)} : r \in \R\}$.
	The latter set is equal to $\{\gamma_r : r \in \R\}$ by $\Gamma$ embedding $\gamma$.
	Therefore, we know each $\theta \in \Theta = \gamma_r$ for some $r \in \R$, which is full-dimensional in the simplex.
	Moreover, since $\gamma$ is non-degenerate, we have $\cup \Theta = \cup_r \gamma_r = \simplex$.
%	\hrule
%	Since $\Gamma$ embeds the finite property $\gamma$, Lemma~\ref{lem:embed-defs-equiv} yields a loss $\ell$ eliciting $\gamma$ embedded by $L$, and therefore $\risk{L} = \risk{\ell}$.
%	As in the proof of Proposition~\ref{prop:embed-bayes-risks}, we observe that every facet of the epigraph of $-\risk{\ell}$ projects onto the simplex as a full-dimensional level set of $\gamma$.
%	As the risks are equal, this is also true for $\Gamma$.
%	Observe each cell in the simplex of $\gamma$ has some report $r$ so the cell is $\gamma_r$.
%	If $\varphi: r \mapsto u_r$, then we must have $\gamma_r = \Gamma_{u_r}$.
%	Take $\Theta$ to be the set $\{\Gamma_{u_r} : u_r \in \varphi(\R)\}$.
%	We have shown each $\Gamma_{u_r}$ is full-dimensional, and as the union of these cells forms a power diagram on $\simplex$, we have $\cup \Theta = \simplex$.

%	2 $\Rightarrow$ 3:
%	\jessie{New attempt: show that $\Gamma$ elicitable and $\trim(\Gamma)$ finite implies each $\Gamma_u \in \trim(\Gamma)$ is full-dimensional. }
%		
%	We claim every $\theta \in \trim(\Gamma)$ is full-dimensional, then take $\Theta = \trim(\Gamma)$ and the result holds.
%	Suppose there was a $u \in \reals^d$ so that $\Gamma_u \in \trim(\Gamma)$ and $\Gamma_u$ was not full-dimensional.
%	The only case in which this happens is if $\Gamma_u$ spans two level sets, as anything that did not would be a subset of a full-dimensional level set in $\trim(\Gamma)$.
%	However, if $\Gamma_u$ spans the relative interior of two full-dimensional level sets, we contradict the elicitability condition in Lemma~\ref{lem:elicitable-level-sets}.\jessiet{Need to be more careful here.}
%	(If $\Gamma_u$ spans only the boundary of one level set, then we have that $\Gamma_u$ is a subset of the full-dimensional level set by closure of level sets of elicitable properties.)
%	Therefore, we must have each $\Gamma_u \in \trim(\Gamma)$ full-dimensional if $\trim(\Gamma)$ is a finite set and $\Gamma$ is elicitable.
...
\subsection{Refining properties} \btw{JESSIE: This needs more narrative text.  I also don't know if this should go in the Appendix, given it doesn't really connect to the rest of the paper, though I think the point it makes (surrogates work for nearly any indirectly elicited property) is worth having in the body.}
...
%	\jessiet{Do we need to show the preimage of $\Gamma$ is $V$?}
	...
	For the second part, let $\gamma':\simplex\toto\R'$ be the finite elicitable property embedded by $L$, with embedding $\varphi:\R'\to\reals^d$, and let $\psi$ be a link to a non-redundant elicitable property $\gamma:\simplex\toto\R$.
...
\jessie{I am going to suggest cutting this; I don't think it adds much to the narrative and forward refs results from calibrated links.}
\begin{corollary}\label{cor:ie-sufficient} \jessiet{Added 7.24.20.  Probably too long to be a Corollary proof, but would rather start too long than too short.  Also the proof has a forward ref}
	If a polyhedral loss $L:\reals^d \to \reals^\Y$ indirectly elicits a property $\gamma': \simplex \toto \R'$, then for any loss $\ell'$ eliciting $\gamma'$, there exists a link $\bar \psi$ such that $(L, \bar \psi)$ is calibrated with respect to $\ell'$.
\end{corollary}
\begin{proof}
	By Theorem~\ref{thm:ie-iff-embeds-refinement}, we know that $\Gamma := \prop{L}$ embeds a property $\gamma\simplex \toto \R$ which refines $\gamma'$.
	Therefore, these exists a calibrated link $\psi : \reals^d \to \R$ from $\Gamma$ to $\gamma$. \jessie{Relies on results in the next section...}
	Moreover, as $\gamma$ refines $\gamma'$, take any link $\psi': r \mapsto r'$, where $\gamma_r \subseteq \gamma'_{r'}$. 
	(We know such a link exists by refinement.)
	Consider $\tilde \psi := \psi \circ \psi'$.
	
	Now, we show that $(L, \tilde \psi)$ is calibrated with respect to $\ell'$.
	Consider that since $\Gamma$ embeds $\gamma$, we have a link $\bar \psi$ such that $(L,\bar \psi)$ is calibrated with respect to $\ell$ eliciting $\gamma$.
	It then suffices to show
	\begin{align*}
	\inf_{u}\inprod{p}{L(u)} &< \inf_{u : \psi(u) \not \in \gamma(p)} \inprod{p}{L(u)}\\
	&\leq \inf_{u : \psi'(u) \not \in \gamma'(p)} \inprod{p}{L(u)}~.~
	\end{align*}
	This first statement is true as embedding implies calibration (Theorem~\ref{thm:eps-thick-calibrated}), and the second is true if $\{u : \psi'(u) \not \in \gamma'(p) \} \subseteq \{u : \psi(u) \not \in \gamma(p) \}$; we show the contrapositive.
	Take $u \in \reals^d$ so that $\psi(u) \in \gamma(p)$.
	\begin{align*}
	\psi(u) \in \gamma(p) &\iff \psi' \circ \psi(u) \in \psi' \circ \gamma(p)\\
	&\iff \tilde \psi(u) \in \gamma'(p)~,~
	\end{align*}
	where the last inequality follows by construction of $\psi'$.
\end{proof}
...
The examples in \S~\ref{sec:examples}, with the exception of the abstain surrogate given by~\citet{ramaswamy2018consistent}, all present a surrogate that use the link $\psi: u \mapsto \sgn(u)$.
...
\jessie{Some notes on dimension of this surrogate.}
