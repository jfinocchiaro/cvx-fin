...
A growing body of work seeks to design and analyze convex surrogates for given loss functions, and more broadly, understand when such (consistent) surrogates can and cannot be found.
...
Literature directly connecting property elicitation to consistency is sparse, with the main reference being~\citet{agarwal2015consistent}.
However, they consider single-valued properties in finite outcome settings, whereas finite properties elicited by general convex losses are necessarily set-valued.
...
\proposedadd{While the definition of ``optimal'' is unclear right now, but we shortly introduce loss functions that are associated with a given property.
In general, by optimal, we mean minimizing this associated loss function in expectation over $p$.}
...
\proposedadd{We will see that embedding implies the existence of a consistent surrogate and link, so this result tells us that, for discrete losses, the embedding framework is sufficient to understand the existence of consistent surrogates for the given loss.
...
\jessie{Commented out entire appendix section for the time being.  All that was there that isn't here is some commentary about positive normal sets.}
...
\begin{definition}\label{def:prop-embed}
	...
	\proposedadd{Moreover, we say a loss embeds a property when it embeds a loss eliciting the property.}
\end{definition}


By condition (ii.) of Definition~\ref{def:loss-embed}, we then have $L$ embedding $\ell$ implies $\prop{L}$ embeds $\prop{\ell}$.
\proposedadd{However, we should observe that this statement is not immediately an ``if and only if'' because property embedding does not capture the requirement of matching losses on embedded points; i.e. that $L(\varphi(r)) = \ell(r)$ for all $r \in \R$.  
However, Lemma~\ref{lem:embed-defs-equiv} shows an equivalence follows without loss of generality.}

\jessiet{Lemma new to journal version}
...
%    To see this, consider that Condition (i.) of matching embedded loss values is met by construction of $\ell$.
    ...
    To verify $\ell$ elicits $\gamma$, consider $r \in \gamma(p) \iff \varphi(r) \in \Gamma(p) \iff \varphi(r) \in \argmin_{u \in \reals^d} \inprod{p}{L(u)} \iff r \in \argmin_{r \in \R} \inprod{p}{\ell(r)}$.
...
Because of this Lemma, we can work directly with properties and set aside the losses which elicit them, which is often more convenient.
...
Before we state the Proposition needed to prove many of the statements in Section~\ref{sec:poly-loss-embed}, we will need to general lemmas about properties and their losses.\btw{Should this section be before or after embedding losses, since the results here are used in the proofs in the embedding losses section?}
...
\jessie{ADDITIONS HERE:}
\btw{Again: figures/an example might help clarify this}
\btw{Jessie: Can/should we move this Lemma above Prop 3 since it is used in the proof of Prop 3?}
...
\jessie{Added from COLT-19 but not discussed in COLT-20.  Suggest removing this for 1. clarity, and 2. lack of definitions.  If we do keep it, it should move sections though.}
...
%	\jessiet{Do we need to show the preimage of $\Gamma$ is $V$?}
	...
	\jessie{Expand this paragraph to 4-5 lines and remove word ``calibrated''}
...
We illustrate this formulaic approach in Section~\ref{sec:abstain}.
...
\proposedadd{The examples in Section~\ref{sec:examples}, with the exception of the abstain surrogate given by~\citet{ramaswamy2018consistent}, all present a surrogate that use the link $\psi: u \mapsto \sgn(u)$.
...
However, the property (Figure~\ref{fig:3-node-tree-prop}) elicited by tree distance for $H$ in Figure~\ref{fig:3-node-tree} is $1$-embeddable by the characterization of \cite[Section~3]{finocchiaro2020embedding}, so one can see that the dimension cascade surrogate does not yield a tight bound.
