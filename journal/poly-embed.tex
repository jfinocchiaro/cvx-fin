...
If this embedding can be extended to a calibrated link function, which maps approximately $L$-optimal reports to $\ell$-optimal reports, then consistency follows~\citep{agarwal2015consistent}.
...
\jessiet{Add note about precision vs recall~\citep{menon2019multilabel} and $F_\beta$ measure~\citep{zhang2020convex}.}
The literature on convex surrogates focuses mainly on smooth surrogate losses~\citep{crammer2001algorithmic,bartlett2006convexity,bartlett2008classification, duchi2018multiclass, williamson2016composite, reid2010composite}.
...
\citet{finocchiaro2020indirect}\jessiet{neurips 2020 paper; might need to throw on arxiv if we want this ref.} additionally relates indirect property elicitation to consistency in finite outcome settings when one is given either a target loss or property in both discrete and continuous prediction settings, assuming surrogates attain their infimum.
...
After introducing notation, and terminology from property elicitation, we study \emph{calibration} (Definition~\ref{def:calibrated}), which is equivalent to consistency in finite settings~\citep{bartlett2006convexity,tewari2007consistency} and depends solely on the conditional distribution over $\Y$.
...
\btw{Jessie: Should we add plots of the Bayes Risks for these since we don't (currently) have page restrictions?}
...
To motivate our focus on polyhedral losses, we echo~\citet[Section~1.2]{ramaswamy2018consistent}, who note that smooth surrogates often encode much more information than necessary, and in these cases non-smooth surrogates are the only candidates to achieve a low input dimension $d$ above.
...
Note that in the finite-outcome setting, calibration is necessary and sufficient for consistency from a generalization of~\cite{tewari2007consistency}.  
...
In this particular case, it is known $(\hinge,\psi)$ is calibrated for $\psi(u) = \sgn(u)$; in \S~\ref{sec:calibration} we show that, perhaps surprisingly, all embeddings lead to calibration with an appropriate link.
...
\begin{proof}
  \raft{$\impliedby$: First 2 sentences are good, but then it fogs up a bit.  3rd needs another careful pass, e.g., supporting hyperplane at what point?   4th cannot be right since you don't cover all reports $r$ \jessie{Took another pass}}
	...
	Moreover, the point $(p, -\inprod{p}{L(u_r)})$ forms a supporting hyperplane of $-\risk{L}$ (and therefore $-\risk{\ell}$ as well) if and only if $u_r \in \Gamma(p)$ for $p \in \Gamma_{u_r} = \gamma_r$.  
...
%  Moreover, consider the linear transformation $P: \ell \mapsto [\mathbb{I}_{n-1}, \vec 0] \risk{\ell}$ projecting $\risk{\ell}$ down to $n-1$ dimensions.
  The projection of its epigraph $E_{\ell}$ onto $\simplex$ forms a power diagram by Theorem~\ref{thm:aurenhammer}, whose cells are full-dimensional (in $n$ dimensions) and correspond to the level sets $\gamma_r$ of $\gamma = \prop{\ell}$.

  For each $r\in\R$, let $p_r$ be a distribution in the relative interior of $\gamma_r$, and let $u_r \in \Gamma(p)$.
...
The Bayes risks $\risk{L}(p)$ and $\risk{L'}(p_{-n})$ are equal for all $p \in \simplex$; thus, the result holds in $n-1$ dimensions as well.}
...
\jessiet{Commented out entire appendix section for the time being.  All that was there that isn't here is some commentary about positive normal sets.}
...
This statement is not immediately equivalent because property embedding does not capture the requirement of matching losses on embedded points; i.e. that $L(\varphi(r)) = \ell(r)$ for all $r \in \R$.  
...
\begin{proof}
  \raft{All the important pieces are here, but take it slower.  State $\ell$ as a definition (let $\ell$ be given by...) and show why $L$ embeds it.\jessie{Took another pass but changed technique, adding in this detail.  Now very similar to the proof of Prop 1 forward direction.}}
  	Consider a loss $\ell : \R \to \reals_+^\Y$, defined so that $\ell : r \mapsto L(\varphi(r))$, where $\varphi$ is the same embedding given by $\Gamma$ embedding $\gamma$.
...
%    To see this, consider that Condition (i.) of matching embedded loss values is met by construction of $\ell$.
    ...
    We aim to show that for all $p \in \simplex$, we have $(1) \inf_{u \in \reals^d}{p}{L(u)} = (2) \inf_{r \in \R}{p}{L(\varphi(r))}$, and the equality of risks follows.
    ...
    This gives us $(1) = (2)$, so we have $\risk{L} = \risk{L|_{\varphi(\R)}}$.
...
Before we state the Proposition needed to prove many of the statements in Section~\ref{sec:poly-loss-embed}, we will need to general lemmas about properties and their losses.\btw{Jessie: Should this section be before or after embedding losses, since the results here are used in the proofs in the embedding losses section?}
...
\begin{lemma}\label{lem:embedded-level-sets-trim}
	...
	If $\Gamma$ embeds a finite property $\gamma : \simplex \toto \R$ by the injection $\varphi$, then $\{\gamma_r : r \in \R\} = \{\Gamma_u : u \in \varphi(\R)\} = \trim(\Gamma)$.
\end{lemma}
\begin{proof}
	Let $L$ elicit $\Gamma$.
...
\begin{corollary}\label{cor:ie-sufficient} \jessiet{Added 7.24.20.  Probably too long to be a Corollary proof, but would rather start too long than too short.  Also the proof has a forward ref}
	If a polyhedral loss $L:\reals^d \to \reals^\Y$ indirectly elicits a property $\gamma': \simplex \toto \R'$, then $L$ is calibrated with respect to any loss $\ell'$ eliciting $\gamma'$.
\end{corollary}
\begin{proof}
	...
	Therefore, these exists a calibrated link $\psi : \reals^d \to \R$ from $\Gamma$ to $\gamma$.
