...
\pgfmathsetmacro\aa{0.5*(sqrt(1-\c)+1)}
\pgfmathsetmacro\bb{0.5*\c}
\node[inner sep=2pt] (levset1) at (0.45,0.55, -0.3) {\vphantom{$\Gamma_{0.1}$}};
\node (levset) at (0.5,0.5,-.15) {$\Gamma_{0.6}$};
...
(Q4) The level sets of the variance are curved and cannot fit a $1$-flat; from Theorem~\ref{thm:convex-flats-inf-dim} there is no $1$-dimensional convex surrogate consistent for the variance.
\raf{Tweaked the fig, FYI: removed some level set labels, changed $\Gamma_{0.05}$ to $\Gamma_{0.1}$ in Q3}
