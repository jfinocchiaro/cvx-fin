...
The abstain target loss is a well-studied variation of 0-1 loss that allows for an ``abstain'' report that gives a lesser punishment $1/2$ for abstaining, $r = \bot$ \raft{cites here}.
...
\citet{ramaswamy2016convex}.\raft{Jessie: see if you can match the style of the revised abstain example for the others.  In particular, I think we can leave the definition of $p$ to the figure, and avoid referencing $\bullet$ or anything -- the figure can make clear that $p=\bullet$.  Probably it's only Q2 that needs to change.}
