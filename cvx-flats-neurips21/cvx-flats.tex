...
A closely related property for any $\beta > 0$ is the (midpoint of the) modal interval of width $2\beta$, given by $\Gamma_\beta(p) = \argmax_{x\in\reals} p([x-\beta,x+\beta])$.
...
As observed in \citet[Corollary 1]{dearborn2020indirect}, the properties $\mode$ and $\Gamma_\beta$ coincide with the family of distributions needed in their Theorem 1, meaning the conclusion of Theorem~\ref{thm:mode} transfers to the modal interval as well.
\begin{corollary}\label{cor:modal-interval}
  For any $\beta > 0$, the modal midpoint, $\Gamma_\beta: \P_\beta \to \reals$, has  $\conscvx(\Gamma_\beta) = \eliccvx(\Gamma_\beta) = \infty$ (countably infinite) with respect to $\P_\beta$, the class of unimodal probability measures on $\Y=\reals$ which admit a smooth and bounded density, and such that $\mode$ and $\Gamma_\beta$ are single-valued.
\end{corollary}

Thus, while $\Gamma_\beta$ is elicitable, it does not have any consistent finite-dimensional convex surrogate.
