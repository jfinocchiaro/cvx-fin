...
\raf{Lemma~\ref{lem:consistent-loss-implies-prop} goes here}
...
\begin{proof}
  ...
  \btw{NOTE: the set $\P$ is not convex.  Take two univariance Gaussians centered at 0 and 1: they are each in $\P$ but their mixture is not.}
...
\begin{ack}
  ...
  This material is based upon work supported by the National Science Foundation under Grant Nos.\ IIS-2045347 and 2040434. \jessie{confirm GRFP number}\raf{I think it's usually stated differently for GRFPs, since it's not really a grant application, right?}
\end{ack}
