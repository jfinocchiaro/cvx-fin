...
Like abstain, we cannot fit a $1$-flat (line) entirely contained in the level set $\gamma_{\text{passive}}$ through the depicted $p$, so Theorem~\ref{thm:convex-flats-inf-dim} again gives $\conscvx(\gamma) = 2$.
...
In Appendix~\ref{app:omitted-examples}, we show $\eliccvx(\Gamma^\Var)=2$, meaning the lowest dimension of a convex loss to estimate conditional variance is 2. %~\citep{osband1985providing,lambert2018elicitation}.
...
In fact, the $d$-flats bound can be strictly tighter; in Appendix~\ref{sec:finite-calib} we show that the abstain example from Figure~\ref{fig:quadrants-simplices} (Q1) yields a $d$-flats lower bound of 2 and a feasible subspace dimension lower bound of 1.
...
See Figure~\ref{fig:fsd-flats-abstain} in Appendix~\ref{sec:finite-calib} for an illustration.


\section{Application: Risk Measures, Mode, and Modal Interval}\label{sec:contin-consis}

\raf{New intro... something along these lines}
...
Using our framework, we establish such bounds, resolving both open questions.
The central observation, from Theorem~\ref{thm:convex-flats-inf-dim}, is that the level sets of $d$-dimensional convex-elicitable properties, if not $d$-flats themselves, are \emph{unions} of $d$-flats.
Using this observation, we can modify the arguments of~\citet{frongillo2020elicitation} and~\citet{dearborn2020indirect} to give similar bounds.
In all cases, our lower bounds match the identifiable case, though we conjecture they are not tight in all cases.
