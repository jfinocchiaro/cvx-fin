...
\begin{abstract}
  ...
  \raf{Notes}
  The prediction dimension of a surrogate is the lowest dimension $d$ such that we can construct a convex surrogate $L : \reals^d \times \Y \to \reals$ that is consistent with respect to a given prediction task.
  We present a new tool based on property elicitation to study the prediction dimension of surrogate loss functions.
  Our tool unifies results on prediction dimension for various settings, including continuous vs discrete predictions.
  We apply our tool in continuous estimation settings to yield a new lower bound on prediction dimension in risk measures, resolving a stated open question in the literature~\citep{frongillo2015elicitation}.
  In discrete prediction settings, this tool also recovers previous lower bounds on prediction dimension attained by feasible subspace dimension~\citep{ramaswamy2016convex}.
...
These two scenarios typically involve different techniques for designing and analyzing surrogate losses.
We unify these settings using tools from property elicitation, and give a general lower bound on prediction dimension which is simultaneously stronger and broader than previous results.
To understand how our framework for producing lower bounds is stronger, we introduce ``four quadrants'' of problem settings, and our bound holds for all four; previous bounds only apply to one quadrant at a time.
To demonstrate the strength of our bounds, we show new results for the prediction dimension required to estimate entropy and norms, answering an open problem of~\citet{frongillo2015elicitation}.
...
%For continuous estimation, our lower bound resolves an open problem on estimating measures of risk and uncertainty.
%
%
%Given a prediction task, understanding when one can and cannot design a consistent convex surrogate loss, particularly a low-dimensional one, is an important and active area of machine learning research. 
...
In many type (1) settings, such as structured prediction and extreme classification, the prediction dimension can easily become intractably large, forcing one to sacrifice consistency for computational efficiency.
...
Challenges of type (2) include risk measures such as conditional value at risk (CVaR), with applications in financial regulation, robust engineering design, and algorithmic fairness.
Risk measures provably cannot be specified via a target loss, and thus we seek a surrogate loss of low (or at least finite) prediction dimension.
...
When given a \emph{target loss} $\ell$, we roughly define $L$ to be consistent if minimizing $L$, and applying a link function, minimizes $\ell$ (Definition \ref{def:consistent-ell})~\citep{zhang2004statistical,bartlett2006convexity,tewari2007consistency,steinwart2007compare,ramaswamy2016convex}.
When given a target statistic such as the conditional quantile or variance, but no target loss, we introduce a notion of consistency in line with classical statistics (Definition \ref{def:consistent-prop}) \citep{gyorfi2006distribution, fan1998efficient,ruppert1997local}.
Here we will define $L$ to be consistent if minimizing $L$ and applying a link function yields estimates converging to the correct value.
...
In fact, we observe that consistency for target losses is a special case of consistency for target statistics (\S~\ref{sec:consis-implies-indir}).
This observation suggests property elicitation (see \S~\ref{subsec:properties}) as a useful tool to study general lower bounds.
...
Particularly,~\citet{zhang2004statistical, lin2004note,bartlett2006convexity,tewari2007consistency,ramaswamy2016convex} show the equivalence of consistency and calibration in Quadrant 1, where one is given a target statistic and discrete prediction set.
...
\begin{table}[t]
	...
	\begin{tabular}{p{12ex}|l|l|} 
		...
		\emph{Discrete} & \textbf{Q1:} Classification  & \textbf{Q2:} Risk-averse classification\\ 
		{\em prediction} & & (Appendix~\ref{app:omitted-examples})\\ \hline 
...
\end{center}
\caption{\raf{Problem: ref quadrants before defining them, but don't want type I and II either}
...
While implicitly observed in some of the literature~\citep{steinwart2008support}\jessie{Add more}, we can observe that consistency implies indirect elicitation.
Formal definitions and the following proof are deferred to Appendix~\ref{sec:consis-implies-indir}, but we loosely mean that converging to the optimal surrogate loss in expectation over the data distribution $\D \sim \X \times \Y$ implies that we can apply a link to the convergent sequence and approach the optimal target loss or conditional statistical value.
...
An important caveat to the above elicitation definitions is that, since $\Gamma = \prop[\P]{L}$ is nonempty everywhere, we must have $L\in\L$, meaning that $\exploss{L}{\cdot}{p}$ always achieves a minimum.
...
%	\btw{Maybe take out of definition environment, but (a) I'd worry that people wouldn't be able to find the definition when we use it later, and (b) I'm hoping we can make the distinction very clear, which now is achieved by the parallel definitions}
\end{definition}
\emph{Consistency} is defined for a target loss in Definition~\ref{def:consistent-ell} and for a target property in Definition~\ref{def:consistent-prop}.
...
From Theorem \ref{thm:consistent-implies-indir-elic}, in Appendix~\ref{sec:consis-implies-indir}, given a target property $\gamma$ or loss $\ell$ with $\gamma = \prop{\ell}$, this task reduces to lower bounding the prediction dimension of a convex surrogate indirectly eliciting $\gamma$.
...
Lemma~\ref{lem:convex-flats-inf-dim}, crystallized from the proofs of \citet[Theorem 16]{ramaswamy2016convex} and \citet[Theorem~9]{agarwal2015consistent}, is to consider a particular distribution~$p$ and surrogate prediction $u \in \reals^d$ with is optimal for $p$.
...
\jessie{Edit this paragraph if Cors 2 and 3 move to the Appendix}
...
\jessiet{At this rate, doesn't need to be in a section header of its own?}
We can apply Lemma~\ref{lem:convex-flats-inf-dim} to construct lower bounds on convex consistency dimension for targets across all four quadrants by Corollaries~\ref{cor:Pcodim-flat-elic-relint-prop} (for discrete prediction problems; Q1, Q2) and~\ref{cor:Pcodim-flat-single-val-prop} (for continuous estimation problems; Q3, Q4).
This is demonstrated in Table~\ref{tab:quadrants-simplices}.\jessie{Ref is wrong}
These illustrations are simply intended to show how Lemma~\ref{lem:convex-flats-inf-dim} can be applied to results in all four quadrants of problem types-- making it the first lower bound to our knowledge to do so.

\paragraph{Q1: Abstain}
The abstain target loss is a well-studied variation of 0-1 loss that allows for an ``abstain'' report that gives a lesser punishment $\alpha \in (0,1)$ for abstaining, $r = \bot$.
Given the target loss $\ell^{\alpha}(r,y) := \Ind{r \not \in \{y, \bot\}} + \alpha\Ind{r = \bot}$,  we can consider the \emph{abstain property} $\gamma$ it elicits, where one predicts the most likely outcome $y$ if $Pr[Y=y|x] \geq 1-\alpha$ and ``abstain'' by predicting $\bot$ otherwise.
\citet{ramaswamy2016convex} present a convex surrogate for the abstain loss with $\alpha = 1/2$ that takes as input a prediction whose dimension is logarithmic in the number of outcomes, yielding new upper bounds on $\conscvx(\ell^{1/2})$ which are an exponential improvement over previous results, e.g.,~\citep{crammer2001algorithmic,rifkin2004defense}.

When $n =3$, the property $\gamma$ elicited by $\ell^{1/2}$ is depicted in Table~\ref{tab:quadrants-simplices} (Q1).
Upon closer inspection, the level set boundaries are found at the set of distribution where is a $y$ such that $p_y = 1/2 = 1-\alpha$.
For any distribution $p \in \relint{\gamma_\bot}$, we can derive a lower bound on convex consistency dimension by trying to fir a $1$-flat through this point; however, by the global geometry of the property and location of the abstain cell relative to other cells, we cannot fit a $1$-flat (line in the 2-d projection of the simplex here) fully contained in $\gamma_\bot$ that passes through $p = \bullet$.
...
While least squares regression is commonly used in machine learning and statistics for continuous estimation, it might not be immediately \emph{why} we do so until we study the property estimated by squared loss, which turns out to be the expected value.
...
See Corollary~\ref{cor:variance} in Appendix~\ref{app:omitted-examples} to see our framework applied to derive lower bounds for convex consistency dimension of the variance.
...
\begin{tabular}{c|c}
	...
	\begin{scope}
	\clip (h) -- (m) -- (l) -- (h);
	\draw[myblue,thin] (1/2,0,1/2) -- (0,1/2, 1/2);
	\def\e{1/16};
	\draw[myblue] (1/4,1/4,1/2) -- (1/2, 1/2, 0);
	\coordinate (point) at (0.3,0.4, 0.3);
...
\coordinate (centerdot) at (0.3,0.3, 0.4);
\draw[myorange, very thick] (C) parabola (A);
\draw[myorange, very thick] (C) parabola (B);
\draw[orange, dashed,shorten >= -3cm, shorten <=-3cm] (3/4,0,1/4) -- (centerdot);
...
%\\
%\hline
\end{tabular}\label{tab:quadrants-simplices}
\caption{Example properties for each of the quadrants. 
In (Q1), we fix $p$ uniform in the abstain level set $\Gamma_\bot$. 
...
\btw{for neurips and colt, we had commented out everything below this for space}
...
\jessie{If, instead of a target loss $\ell$, we have a target property as in Quadrants 2 and 4, we might want to equivalently define consistency with respect to a property.  
	Intuitively, this definition is similar, but is equipped with a metric-like function to measure equivalence to a target property value.  
	See Appendix~\ref{} for our precise definition, which we show is equivalent to consistency with respect to target loss.
	This allows us to ``translate'' results about target losses to be about the properties these losses elicit, and therefore apply our results to all four Quadrants.}\jessiet{Suggestion: replace following text up to Lemma 1 with this.}
...
\jessie{Move this statement to appendix, and modify above paragraph to give hand-waving description.}
...
We formalize this by letting $\propdis$ be the $\ell$-regret, yielding Lemma~\ref{lem:consistent-loss-implies-prop}, proven in Appendix~\ref{app:misc-omitted-proofs}.\jessiet{Move this proof if property consistency is moving to its own appendix as well.}
...
\jessie{Comment on how this lets us ``shift'' Q1 to Q2, and Q3 to Q4.  This is a subtlety missing in Table~\ref{tab:quadrants-simplices} too.}
...
\section{Implications of convex indirect elicitation bounds}
...
\section{Previous lower bounds and comparisons}\label{sec:finite-calib}
