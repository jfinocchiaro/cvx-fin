...
\section{Illustration: geometric intuition for the four quadrants}
...
This is demonstrated in Table~\ref{tab:quadrants-simplices}.
...
\begin{tabular}{|c|c|}
	...
	\coordinate[label=above:${y=0}$] (l) at (0,0,1);
...
\caption{Example properties for each of the 4 quadrants and how we can or cannot fit $1$-flats through the level sets containing $p = \bullet$. \jessie{TODO: ADD MORE}}
\end{table}

\section{Breadth of framework: Discrete-valued predictions}\label{sec:discrete-predictions-refactored}
In discrete prediction settings (Quadrants 1 and 2), we can apply Lemma~\ref{lem:convex-flats-inf-dim} to observe lower bounds on convex prediction dimension that subsume previous bounds given by~\citet{ramaswamy2016convex}.
Their lower bound 
%(the only non-trivial one to our knowledge) 
is given by the co-dimension of the \emph{feasible subspace dimension} $\Sc_\C(p)$ of a convex set $\C$ at a given distribution $p$ such that $p \in \C$, which is loosely the highest dimension subspace of $\C$ containing a neighborhood around $p$.
The feasible subspace dimension uses ``local'' information of the property to construct lower bounds, while $d$-flats in Lemma~\ref{lem:convex-flats-inf-dim} allow us to also use ``global'' information, while subsuming the previous results about local information.

\begin{proposition}\label{prop:discrete-bound-subsumes-ramaswamy}
	Suppose we are given a \jessie{elicitable?} property $\gamma: \simplex \toto \R$.
	Fix $p \in \simplex$ and take $r \in \R$ such that $p \in \gamma_r$.  
...
\jessie{Suggest cutting this for space, since it's in the table.}
