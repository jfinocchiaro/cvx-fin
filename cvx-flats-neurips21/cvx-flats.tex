...
As discussed above, notions of consistency have appeared in the literature with respect to target losses, and to target statistics or properties.
...
Lemma~\ref{lem:consistent-loss-implies-prop} in \S~\ref{sec:consis-implies-indir} shows that, in fact, one can capture consistency with respect to a target loss as a special case of consistency with respect to a target property.
Specifically, given a target loss $\ell$, one can take $\gamma = \prop{\ell}$ and define $\mu(r,p) := \E_p \ell(r,Y) - \min_{r'}\E_p \ell(r',Y)$ to be the $\ell$-regret of the report $r$.
