...
\begin{figure}[b!]
  ...
  \begin{tikzpicture}[yscale=0.6,xscale=0.46]
    ...
    (consistent) edge[bend left] node[above, sloped]{Thm.~\ref{thm:consistent-implies-indir-elic}} (indir-elic)    
    (calibrated) edge[bend right] (indir-elic)
    (indir-elic) edge node[above, sloped]{Lem.~\ref{lem:convex-flats-inf-dim}} (d-flats)
  ...
  \end{tikzpicture}
\caption{Flow and implications of our results. Compared to calibration, we suggest indirect elicitation as a simpler but almost-as-powerful necessary condition for consistency. In particular, we obtain a testable condition (Lemma~\ref{lem:convex-flats-inf-dim}), based on $d$-flats, for the existence of a $d$-dimensional consistent convex surrogate. This condition recovers and strengthens existing calibration-based results, while simultaneously applying to other settings (Q2,4).}
...
\begin{lemma}
  ...
  \jessiet{Change back to Theorem?}
  \raft{Good to discuss; let's leave as Lemma for now}
\end{lemma}
\begin{proof}
...
We now illustrate how to apply Lemma~\ref{lem:convex-flats-inf-dim} to construct lower bounds on convex consistency dimension for targets across all four quadrants of Table~\ref{tab:quadrants}.\raft{I once again cut mention of the Corollaries, hoping we can use the lemma directly for these examples.  But I'm thinking we'll want to bring them back actually, and state the Cor's below Lem 1... let's discuss on Monday}
...
Throughout the examples, we will have $|\Y|=3$ so that the probability simplex can be visualized in two dimensions (Table~\ref{tab:quadrants-simplices}).
...
To apply Lemma~\ref{lem:convex-flats-inf-dim}, we first consider the abstain property $\gamma$ elicited by $\ell^{1/2}$, where one predicts the most likely outcome $y$ if $Pr[Y=y] \geq 1/2$ and otherwise ``abstains'' by predicting $\bot$.
...
This property is shown in Table~\ref{tab:quadrants-simplices} (Q2), and we can observe the point that, like abstain, we cannot fit a $1$-flat (line) entirely contained in the level set $\gamma_{\text{snowy}}$ through $p$.
Therefore, we can apply Lemma~\ref{lem:convex-flats-inf-dim} to conclude that $\conscvx(\gamma) = 2$.
...
Since squared loss is a $1$-dimensional loss, we can verify that for all $p \in \simplex$, we can fit a $1$-flat (precisely equal to the level set) through $p$, as demonstrated in Table~\ref{tab:quadrants-simplices}.
