...
\definecolor{mygreen}{rgb}{0,0.8,0.2}
...
The convex consistency dimension of a supervised learning task is the lowest prediction dimension $d$ such that there exists a convex surrogate $L : \mathbb{R}^d \times \mathcal{Y} \to \mathbb{R}$ that is consistent for the given task.
  ...
  We use $d$-flats to obtain a new lower bound on the convex consistency dimension of risk measures, resolving an open question due to Frongillo and Kash (NeurIPS 2015).
...
There are two main reasons why a surrogate loss is necessary: (I) the target problem is to minimize a loss, the \emph{target loss}, that does not satisfy some desiderata such as continuity or convexity; or (II) the target problem is to estimate some \emph{target statistic} and some associated surrogate loss is required to do so, as in many continuous estimation problems.
...
Challenges of type (II) include estimating risk measures such as conditional value at risk (CVaR), with applications in financial regulation, robust engineering design, and algorithmic fairness~\cite{acerbi2014back-testing,fissler2016expected,rockafellar2010buffered,williamson2019fairness}.
...
Using this tool, we resolve the above open question for type (II), giving the first prediction dimension bounds for risk measures with respect to convex surrogates.
We also resolve a similar open question for the mode and modal interval, posed by~\citet{dearborn2020indirect}.
...
\btw{Old contributions commented out here}
...
In addition to the two possible targets, we may have one of two domains: a \emph{discrete} (i.e.\ finite) target prediction space, like a classification problem, or a \emph{continuous} one, like a regression or point estimation problem.
...
We give informal definitions of consistency in \S~\ref{sec:informal-consis-implies-indir}, with formal definitions in \S~\ref{sec:consis-implies-indir}.
%Def.~\ref{def:consistent-ell-informal} informally; Def.~\ref{def:consistent-ell} formally
%,bartlett2006convexity
% (Def.~\ref{def:consistent-prop-informal} informally; Def.~\ref{def:consistent-prop}, \S~\ref{sec:consis-implies-indir} formally)

When given a \emph{target loss} $\ell$, we roughly define $L$ to be consistent if minimizing $L$, and applying a link function, minimizes $\ell$~\citep{zhang2004statistical,tewari2007consistency,steinwart2007compare,ramaswamy2016convex}.
When given instead a target statistic such as the conditional quantile or variance,\!%terrible spacing hack
\begin{table}[h!]
  \vspace*{10pt}
  \begin{center}
    \aboverulesep=1ex
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[yscale=0.6,xscale=0.46]
    ...
    \path[fill=mygreen, fill opacity = 0.1] (0,-3) -- (0,3) -- (15.8,3) -- (15.8,-3) -- cycle;
    ...
    \node[vertex] (calibrated) at  (-11,-2) {Calibration (Q1)};
    \node[vertex] (indir-elic) at  (-5,0) {Indirect elicitation};


    \node[vertex] (d-flats) at  (5,0) {$d$-flats in level sets};
    \node[vertex, text width=30mm] (q1-bounds) at (12,2) {$\conscvx$ bounds for Q1, Q2. (Cor.~\ref{cor:fsd-bound})};
    \node[vertex, text width=30mm] (q4-bounds) at (12,-2) {$\conscvx$ bounds for Q3, Q4. (Thm.~\ref{thm:bayes-risk-lower-bound},~\ref{thm:mode})};

    % edges
    \path[->,bend angle=10]
    (consistent) edge[<->] node[left]{\cite{bartlett2006convexity,steinwart2008support}} (calibrated)
    (consistent) edge[bend left] node[above, sloped]{Prop.~\ref{prop:consistent-implies-indir-elic}} (indir-elic)    
...
The key observation which underpins our approach is that consistency for target losses is a special case of consistency for target statistics (Lemma~\ref{lem:consistent-loss-implies-prop}). 
%(\S~\ref{sec:consis-implies-indir}).
...
See \S~\ref{subsec:complexity} for other prior work on notions of prediction dimension. 
...
%, where one is given a target statistic \raf{loss?} and discrete prediction set
We discuss the additional relationship of elicitation and calibration in \S~\ref{app:calibration}, and re-derive Proposition~\ref{prop:consistent-implies-indir-elic} via calibration.
...
\label{sec:informal-consis-implies-indir}

\raft{Your edit was much better for that narrative -- I think this narrative might flow better though.}
As discussed above, notions of consistency have appeared in the literature with respect to target losses, and to target statistics / properties.
We give informal definitions of both notions here, with formal versions deferred to \S~\ref{sec:consis-implies-indir}.
Lemma~\ref{lem:consistent-loss-implies-prop} shows that, in fact, one can capture consistency with respect to a target loss as a special case of consistency with respect to a target property.
This observation allows us to translate consistency from Quadrant 1 to Quadrant 2, and from Quadrant 3 to Quadrant 4;
in particular, it will allow us to prove bounds for all four quadrants simultaneously.
...
Consistency with respect to a property follows similarly, but instead of converging to the optimal target loss, one should approach the optimal (conditional) property value.
...
We can now state Lemma~\ref{lem:consistent-loss-implies-prop}, with proof deferred to \S~\ref{sec:consis-implies-indir}.
...
\begin{restatable}{lemma}{consistentlossimpliesprop}\label{lem:consistent-loss-implies-prop}
	...
	$\mu(r,p) := \E_p \ell(r,Y) - \min_{r'}\E_p \ell(r',Y)$.
...
\end{restatable}
\jessiet{Note: this is the formal version- do we want an informal version here instead, or to just leave the details in here?}


As observed in the literature, e.g.~\citep{steinwart2008support,agarwal2015consistent},
both notions of consistency imply in particular that the link function must map exactly optimal surrogate reports to exactly optimal target reports.
In property elicitation, this condition is known as \emph{indirect elicitation}: for single-valued properties, $\Gamma$ and $\psi$ indirectly elicit $\gamma$ if $\gamma = \psi \circ \Gamma$.
The definition below covers the general set-valued case as well.
...
\end{definition}
\begin{restatable}{proposition}{consistencyimpliesindirelic}\label{prop:consistent-implies-indir-elic}
...
In light of Lemma~\ref{lem:consistent-loss-implies-prop}, we can use this fact to build prediction dimension lower bounds across all four quadrants.
...
From Proposition~\ref{prop:consistent-implies-indir-elic}, given a target property $\gamma$ or loss $\ell$ with $\gamma = \prop{\ell}$, this task reduces to lower bounding the convex consistency dimension of $\gamma$.
...
The following lemma yields consistency bounds when combined with Proposition~\ref{prop:consistent-implies-indir-elic}. %\raft{Minor writing point: we capitalize lemma when it's a proper noun, hence ``this lemma'' vs ``Lemma 1''.  I know, weird.}
...
\end{cases}~.
\end{align*} 
This ``T-shaped'' property, depicted in Figure~\ref{fig:quadrants-simplices} (Q2), falls under Quadrant 2, as it is not elicited by any target loss.%
\footnote{The cells of finite elicitable properties form power diagrams, a generalization of Voronoi diagrams, which disallow this ``T-shaped'' configuration~\cite{lambert2009eliciting,frongillo2014general}.}
...
\tikzstyle{vertexgrn} = [draw, minimum width=4em, inner sep=2pt, fill = green, fill opacity = 0.2, text=black, text opacity = 1.];
\tikzstyle{every node}=[font=\small];

% vertices
\node[vertex] (speech) at  (0,2) {Speech};
\node[vertexgrn] (active) at  (-2,1) {Active};
\node[vertex] (nonactive) at  (2,1) {Non-active};
\node[vertexgrn] (median) at  (1,0) {Median};
\node[vertexgrn] (passive) at  (3,0) {Passive};
...
In \S~\ref{app:variance}, we show $\eliccvx(\Var)=2$, meaning the lowest dimension of a convex loss to estimate conditional variance is 2. %~\citep{osband1985providing,lambert2018elicitation}.
...
\begin{SCfigure}
  \begin{tabular}{c|c}
    ...
    \begin{tikzpicture} [scale=\tikzfigscale, thick, tdplot_main_coords]
      ...
      \begin{scope}
	\clip (h) -- (m) -- (l) -- (h);
	\draw[mygreen] (1/2,1/2,0) -- (1/2, 0, 1/2);
	\draw[mygreen] (0,1/2,1/2) -- (1/2, 0, 1/2);
	\draw[mygreen] (0,1/2,1/2) -- (1/2,1/2, 0);
	\coordinate (point) at (0.3,0.4, 0.3);
	\draw[myblue, dashed,shorten >= -3cm, shorten <=-3cm] (1/4,3/4,0) -- (point);
	\draw[myblue, dashed,shorten >= -3cm, shorten <=-3cm] (0.7,0,0.3) -- (point);
	\node at (point) {$\bullet$};
	...
	\begin{scope}
          \clip (h) -- (m) -- (l) -- (h);
          \draw[mygreen,thick] (1/2,0,1/2) -- (0,1/2, 1/2);
          \def\e{1/16};
          \draw[mygreen,thick] (1/4,1/4,1/2) -- (1/2, 1/2, 0);
          \coordinate (point) at (0.3,0.4, 0.3);
          \draw[myblue, dashed,shorten >= -3cm, shorten <=-3cm] (0.4, 0.6, 0.) -- (point);
          \draw[myblue, dashed,shorten >= -3cm, shorten <=-3cm] (0,0.4,0.6) -- (point);
          \node at (point) {$\bullet$};
    ...
    \begin{tikzpicture} [scale=\tikzfigscale, thick, tdplot_main_coords]
      ...
      \begin{scope}
        ...
        \foreach \c in {0,0.05,...,1}
        {
          ...
          \draw[mygreen,thin] (A) -- (B);
        }
        \draw[mygreen] (0.45,0.55,0) -- (0.45,.55,2);
        \draw[myblue, very thick,dashed] (0.45,0.55,0) -- (0.45,.55,2);
        % \draw[orange, very thick] (0.7,0.3,0) -- (0.7,0.3,2);
      \end{scope}
      ...
      \begin{tikzpicture} [scale=\tikzfigscale, thick, tdplot_main_coords]
        ...
        \begin{scope}
          ...
          \foreach \c in {0,0.115,...,1}
          {
            ...
            \draw[mygreen,thin] (C) parabola (A);
            \draw[mygreen,thin] (C) parabola (B);
          }
          \def\c{0.69}
          \pgfmathsetmacro\aa{0.5*(sqrt(1-\c)+1)}
          \pgfmathsetmacro\bb{0.5*\c}
          ...
          \draw[mygreen, very thick] (C) parabola (A);
          \draw[mygreen, very thick] (C) parabola (B);
          \draw[myblue, dashed,shorten >= -3cm, shorten <=-3cm] (1/10,9/10,0) -- (centerdot);
...
\begin{proof}
  ...
  \btw{NOTE: the set $\P$ is not convex.  Take two univariance Gaussians centered at 0 and 1: they are each in $\P$ but their mixture is not. \jessie{Should we be concerned about this}}
...
% \raft{Recommend cutting for camera-ready.  Ultimately we just say ``beats me'' at the end, and the rest is not really about broader impacts :-]}
% \jessiet{You're not wrong...}
% Since our work is theoretical, it is impossible to forecast downstream applications of this work and their social impact.
...
\begin{ack}
  We thank Adam Bloniarz and Nishant Mehta for several key suggestions and insights.
  This material is based upon work supported by the National Science Foundation under Grant No.\ IIS-2045347. 
  Additionally, this material is based upon work supported by the National Science Foundation Graduate Research Fellowship under Grant No. 2040434. \jessie{confirm GRFP number}
\end{ack}
...
\begin{condition}[Covers]
	A set $\D \subseteq \Delta(\X \times \Y)$ \emph{covers} a convex set $\P \subseteq \simplex$ if, for all $p \in \P$, there exists $D \in \D$ and $x \in \X$ such that $D$ has a point mass on $x$ and $p = D_x$.
\end{condition}
...
\begin{definition}[Consistent: loss]\label{def:consistent-ell}
	...
	For a given convex set $\P \subseteq \simplex$, we simply say $(L,\psi)$ is \emph{consistent} if it is $\D$-consistent for some $\D$ covering $\P$.
	%satisfying the following: for all $p \in \P$, there exists $D \in \D$ and $x \in \X$ such that $D$ has a point mass on $x$ and $p = D_x$.
...
\begin{definition}[Consistent: property]\label{def:consistent-prop}
	...
	\end{equation}
	% where $D_x = \Pr[Y|X=x]$.
	We simply say $(L,\psi)$ is \emph{$\propdis$-consistent} if it is $(\propdis,\D)$-consistent for some $\D$ covering $\P$.
	%satisfying the following: for all $p \in \P$, there exists $D \in \D$ and $x \in \X$ such that $D$ has a point mass on $x$ and $p = D_x$.
...
We formalize this by letting $\propdis$ be the $\ell$-regret, $R_\ell := \E_p \ell(r,Y) - \min_{r'}\E_p \ell(r',Y)$, yielding Lemma~\ref{lem:consistent-loss-implies-prop}.
...
%\raf{Important: I think we need to move this lemma up to \S~\ref{sec:informal-consis-implies-indir}.  Probably using this exact same statement.  Related: do we define $R_\ell$ anywhere?  Not in this section, unless I missed it...}
%\jessie{Good catch; I think we removed it from COLT to NeurIPS or vice versa}

\begin{restatable}{lemma}{consistentlossimpliesprop}\label{lem:consistent-loss-implies-prop}
	Let a convex $\P \subseteq \simplex$ be given.
	Given a surrogate loss $L \in \L$, link $\psi$, and target loss $\ell$, set
	$\mu(r,p) := \E_p \ell(r,Y) - \min_{r'}\E_p \ell(r',Y)$ as the excess risk of $\ell$, $R_\ell$.
	Then there is a $\D$ covering $\P$ such that
	$(L, \psi)$ is $\D$-consistent with respect to $\ell$ if and only if $(L,\psi)$ is $(\propdis, \D)$-consistent with respect to $\gamma := \prop{\ell}$. 
	%	\btw{Note: don't actually need $\gamma$ nonempty here.}
\end{restatable}
\begin{proof}
...
This points to indirect elicitation as a natural necessary condition for consistency, as formalized in Proposition~\ref{prop:consistent-implies-indir-elic}.
...
Combining the two results, we can observe the result of Proposition~\ref{prop:consistent-implies-indir-elic} another way: \emph{through calibration}.
