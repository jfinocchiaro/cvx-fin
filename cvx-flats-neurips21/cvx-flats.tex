...
See Figure~\ref{fig:results-flow} for a roadmap of our main results.
\jessie{Re-write}
...
\jessie{Make restateable}
\begin{proposition}\label{thm:consistent-implies-indir-elic}
...
\begin{tabular}{|c|c|}
	...
	\draw[myorange, dashed] (1/2,0,1/2) -- (0,1,0);
	\draw[myorange, dashed] (2/3,1/3,0) -- (0,1/3,2/3);
	\node (uniformdot) at (1/3,1/3,1/3) {$\bullet$};
	%\draw[orange, very thick] (0.7,0.3,0) -- (0.7,0.3,2);
	\end{scope}
	\node[inner sep=2pt] (levset1) at (3/4,1/8,1/8) {$\Gamma_{1}$};
	...
	\begin{scope}
	\clip (h) -- (m) -- (l) -- (h);
	\draw[myblue,thin] (2/3,0,1/3) -- (0,2/3, 1/3);
	\def\e{1/16};
	\draw[myblue,thin] (\e + 1/3, 1/3- \e,1/3) -- (1/2 + \e, 1/2 - \e,0);
	\coordinate (point) at (1/3 - \e/2, 1/3 + \e, 1/3 - \e/2);
	\draw[orange, dashed,shorten >= -3cm, shorten <=-3cm] (1/4,3/4,0) -- (point);
	\draw[orange, dashed] (2/3 + \e/2,0,1/3 - \e/2) -- (0,2/3 + \e/2, 1/3 - \e/2);
	\node at (point) {$\bullet$};
	%\draw[orange, very thick] (0.3,0.7,0) -- (0.3,0.7,2);
	%\draw[orange, very thick] (0.7,0.3,0) -- (0.7,0.3,2);
	\end{scope}
	\node[inner sep=2pt] (levsetmin1) at (2/3, 1/6,1/6) {$\Gamma_{-1}\!\!$};
	\node[inner sep=2pt] (levset1) at (1/6,2/3,1/6) {$\Gamma_{1}\!\!$};
	%\draw[-latex,thin] (levset2) edge[in=270,out=90] (0.7,0.3,0);
	%\draw[-latex,thin] (levset1) edge[in=270,out=90] (0.3,0.7,0);
	\placefiglabel{(Q2)}
	\end{tikzpicture}
...
\draw[orange, very thick,dashed] (0.5,0.5,0) -- (0,0.,2);
%\draw[orange, very thick] (0.7,0.3,0) -- (0.7,0.3,2);
\end{scope}
\node (centerdot) at (1/3,1/3,1/3) {$\bullet$};
\node[inner sep=2pt] (levset1) at (0.5,0.5, -0.3) {$\Gamma_{0}\!$};
%\node[inner sep=2pt] (levset2) at (0.7,0.3,-.3) {$\Gamma_{\!\!-0.4}\!\!\!\!$};
%\draw[-latex,thin] (levset2) edge[in=270,out=90] (0.7,0.3,0);
\draw[-latex,thin] (levset1) edge[in=270,out=90] (0.5,0.5,0);
\placefiglabel{(Q3)}
\end{tikzpicture}
&
\begin{tikzpicture} [scale=\tikzfigscale, thick, tdplot_main_coords]
...
\jessie{Suggest cutting this for space if needed, since it's in the table.}
...
\begin{theorem}\label{thm:consistent-implies-indir-elic}
	For a surrogate $L \in \L$, if the pair $(L, \psi)$ is consistent with respect to a property $\gamma: \P \toto \R$ or a loss $\ell$ eliciting $\gamma$, then $(L, \psi)$ indirectly elicits $\gamma$.
	\jessie{Move to prop; give credit to literature}
\end{theorem}
