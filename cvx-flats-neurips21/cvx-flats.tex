...
Therefore, property elicitation -- which studies the relationship between target losses, target statistics, surrogate losses, and surrogate statistics, see \S~\ref{subsec:properties}  -- allows us to study general lower bounds that apply across all four quadrants.
...
We often abstract away $x$, working directly with a convex set of distributions over outcomes $\P \subseteq \simplex$.
...
\bo{It's not clear exactly why we mention this.}
...
\begin{proof}[Finite case]
  We will prove the result for the case of finite $\Y$, and defer the $\Y=\reals$ case to Appendix~\ref{app:implications-of-bounds}.
...
Proofs are deferred to Appendix~\ref{app:implications-of-bounds}.
...
We now illustrate how to apply Theorem~\ref{thm:convex-flats-inf-dim} to construct lower bounds on convex consistency dimension for targets across all four quadrants of Table~\ref{tab:quadrants}.\raft{I once again cut mention of the Corollaries, hoping we can use the lemma directly for these examples.  But I'm thinking we'll want to bring them back actually, and state the Cor's below Lem 1... let's discuss on Monday}
...
For each, we take $d=1$, and thus ask whether a $1$-flat (a line in the figures) exists through the point $p$ while staying within the corresponding level set.
...
For the depicted distribution $p \in \relint{\gamma_\bot}$, we can derive a lower bound on convex consistency dimension as we cannot fit a $1$-flat (line) fully contained in $\gamma_\bot$ that passes through $p$.
By Corollary~\ref{cor:Pcodim-flat-elic-relint-prop}, we can conclude $\conscvx(\gamma^{1/2}) \geq 2$ when $|\Y|=3$, which matches the upper bound from the convex surrogate of
\citet{ramaswamy2016convex}.\raft{Jessie: see if you can match the style of the revised abstain example for the others.  In particular, I think we can leave the definition of $p$ to the figure, and avoid referencing $\bullet$ or anything -- the figure can make clear that $p=\bullet$.  Probably it's only Q2 that needs to change. \jessie{Took a pass! Feel free to burn}}
%present a convex surrogate for the abstain loss yielding a logarithmic upper bound on $\conscvx(\ell^{1/2})$, so this bound is tight.% which are an exponential improvement over previous results, e.g.,~\citep{crammer2001algorithmic,rifkin2004defense}.

\paragraph{Q2: Variation of hierarchical classification.}
\jessiet{I don't like this phrasing, since it seems like we're pointing to Q1 then swerve to Q2}
\citet{ramaswamy2015hierarchical} studies a discrete target loss for hierarchical classification tasks~\citep{xiao2007hierarchical,burkhardt2005database}, such as the task in Figure~\ref{fig:hierarchical-tree}, in which one wishes to predict the deepest node in a tree that is ``likely enough.''
Consider the mild variation of this task where one wants to predict an outcome if it is likely enough, and otherwise proceed in a different direction and classify from another binary substructure as depicted in Figure~\ref{fig:quadrants-simplices} (Q2). 
Here, we take the reports and outcomes $\R = \Y$ to be the leaves of the tree in Figure~\ref{fig:hierarchical-tree}.
\citet{osband1985providing} notes that a property is elicitable if and only if its level sets form a power diagram projected onto the simplex, so there is no discrete target loss eliciting the described property, hence it belongs in Q2.
...
\begin{SCfigure}
	...
	\begin{tikzpicture}
\tikzstyle{vertex} = [draw, minimum width=4em, inner sep=4pt, fill = white];
\tikzstyle{vertexblue} = [draw, minimum width=4em, inner sep=4pt, fill = blue, fill opacity = 0.2, text=black, text opacity = 1.];
\tikzstyle{every node}=[font=\small];
...
In Figure~\ref{fig:quadrants-simplices} (Q2), we see that, like abstain, we cannot fit a $1$-flat (line) entirely contained in the level set $\gamma_{\text{passive}}$ through $p$.
...
Evaluating the derivative of squared loss, we observe that it elicits the expected value $\Gamma(p) = \E_p[Y]$.
With $\Y = \{-1,0,1\}$, we can visualize the level sets of the expected value as in Figure~\ref{fig:quadrants-simplices} (Q3).
Since squared loss is a $1$-dimensional loss, we can verify that for all $p \in \simplex$, we can fit a $1$-flat (precisely equal to the level set) through $p$, as demonstrated in Figure~\ref{fig:quadrants-simplices}.

\paragraph{Q4: Variance}
One might reasonably want to estimate the variance $\Gamma^\Var(p) = \E_p[Y^2] - \E_p[Y]^2$, though a loss function to elicit the variance is not immediately obvious.
This property is continuous in $\simplex$, and therefore belongs in Quadrant 4.
...
This lower bound will follow from Theorem~\ref{thm:bayes-risk-lower-bound} in Section~\ref{sec:contin-consis} using the fact that variance is the Bayes risk of squared loss $L(r,y) = (r-y)^2$, which elicits the mean $\Gamma(p) = \E_p Y$.
...
\begin{tabular}{c|c}
	...
	\coordinate (p) at (\ph,\pm,\pl);
	\coordinate (uniform) at (1/3,1/3,1/3);
	\coordinate[label=below:${\text{passive}}$] (h) at (1,0,0);
	\coordinate[label=below:${\text{median}}$] (m) at (0,1,0);
	\coordinate[label=right:${\text{active}}$] (l) at (0,0,1);
...
In each Quadrant, we take $\bullet$ to be the distribution $p = (0.3, 0.3, 0.4)$ according to the left, top, and right outcomes respectively. 
In (Q1) and (Q2), we cannot fit a $1$-flat (line) through $p$ without leaving the level set $\Gamma_\bot$.  
Therefore, there is no $1$-dimensional consistent convex surrogate for abstain by Theorem~\ref{thm:convex-flats-inf-dim}.  
In (Q3), we can fit a $1$-flat through any $p \in \simplex$ for least squares, eliciting expected value.
In (Q4), the level sets of the variance are curved, and therefore we cannot draw a $1$-flat contained in the level set, and conclude there is not a $1$-dimensional consistent convex surrogate w.r.t. the variance.
}\label{fig:quadrants-simplices}
\end{SCfigure}
...
Theorem~\ref{thm:convex-flats-inf-dim} subsumes the  bounds given by~\citet{ramaswamy2016convex} by showing that, if there is a $d$-flat fully contained in the level set (so we can apply Theorem~\ref{thm:convex-flats-inf-dim}) then the co-dimension of the subspace of feasible directions at the same $p \in \C := \gamma_r$ has co-dimension at most $d$.
...
The feasible subspace dimension bound uses ``local'' information of the property to construct lower bounds, while $d$-flats in Theorem~\ref{thm:convex-flats-inf-dim} allow us to also use ``global'' information, while subsuming the previous results about local information.
...
\subsection{Mode and modal interval}

\raf{Define the mode.}
\citet{dearborn2020indirect} recently showed that the mode....

\begin{corollary}\label{cor:mode}
  The mode has $\conscvx(\mode) = \eliccvx(\mode) = \infty$ (countably infinite) with respect to $\P$, the class of probability measures on $\Y=\reals$ which admit a smooth and bounded density and such that $\mode$ is single-valued.
\end{corollary}
\begin{proof}
...
\raf{Introduce modal interval}
... $\Gamma_\epsilon$ is elicited by the target loss $\ell_\epsilon(r,y) = \ones\{|r-y| > \epsilon\}$
... so $\Gamma_\epsilon$ could be thought of as being in Quadrant 3.

As observed in \citet[Corollary 1]{dearborn2020indirect}, the properties $\mode$ and $\Gamma_\epsilon$ coincide with the family of distributions needed in their Theorem 1, meaning the conclusion of Corollary~\ref{cor:mode} transfers to the modal interval as well.
\begin{corollary}\label{cor:modal-interval}
  For any $\epsilon > 0$, the modal midpoint, $\Gamma_\epsilon: \P_\epsilon \to \reals$, has  $\conscvx(\Gamma_\epsilon) = \eliccvx(\Gamma_\epsilon) = \infty$ (countably infinite) with respect to $\P_\epsilon$, the class of unimodal probability measures on $\Y=\reals$ which admit a smooth and bounded density, and such that $\mode$ and $\Gamma_\epsilon$ are single-valued.
\end{corollary}
