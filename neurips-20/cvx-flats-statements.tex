...
\begin{proof}
	...
	We then have $L(f_m(x); p) = L(u_m; p) \to L(\Gamma(p);p)$, so $\lim_{m \to \infty}\zeta(L(u_m;p) - L(\Gamma(p);p)) \to 0$.
	This gives $\ell(\psi(u_m);p) > \ell(\gamma(p);p) + \epsilon_m$ for all $m$, and $\epsilon_m \to 0$.
	This in turn implies $\ell(\psi(u_m); p) \not \to \ell(\gamma(p);p)$; thus, we do not have consistency.
