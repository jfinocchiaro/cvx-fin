...
\begin{definition}[Consistent: property]
	We say a loss $L:\reals^d \times \Y \to \reals$ is consistent with respect to a property $\gamma : \simplex \toto \R$ elicited by the loss $\ell$ if there is a link $\psi : \reals^d \to \R$ such that, for all distributions $p \in \simplex$, we have
	\begin{equation}
	\ell( \psi(u); p) - \ell(\gamma(p); p)  \leq \zeta \left(  L(u;p) - L(\Gamma(p); p) \right)
	\end{equation}
	for some increasing function $\zeta : \reals \to \reals$ that is continuous at $0$ with $\zeta(0) = 0$.
\end{definition}
...
\begin{proposition}\label{prop:consistent-implies-indir-elic}
	If a surrogate and link pair $(L, \psi)$ is consistent with respect to $\gamma$ or with respect to $\ell$ eliciting $\gamma$, then $L$ indirectly elicits $\gamma$. \jessie{Fix and adjust to include $\gamma$}
\end{proposition}
\begin{proof}
