...
\begin{definition}[Consistent: property]
	A loss $L : \reals^d \times \Y \to \reals$ is \emph{$d$-consistent with respect to a property} $\Gamma$ for a metric $d$ if, for all $D$ over $\X \times \Y$ and $p_x$ is the conditional distribution on $Y$ given $X = x$,  we observe
	\begin{equation}
	\E_{(X,Y) \sim D} L(\Gamma(p_X), Y) \to \inf_{u \in \reals^d} \E_{Y \sim p_x} L(u, Y) \implies d(\E_X \Gamma(p_X), \Gamma(p_x)) \to 0 
	\end{equation}
	for all $x \in \X$.
...
The \emph{convex calibration dimension} $\ccdim(\ell)$ of a loss $\ell$ is the minimum dimension $d$ such that there is a convex loss $L: \reals^d \times \Y \to \reals$ and link $\psi$ such that $L$ is calibrated with respect to $\ell$.
\end{definition}
...
\begin{proposition}\label{prop:consistent-implies-calibrated}
	If a loss $L$ is consistent with respect to a property $\Gamma$, then it is calibrated with respect to $\Gamma$.
...
\begin{definition}[Subspace of feasible directions]
	Define the \emph{subspace of feasible directions} $\S_\C(p)$ of a convex set $\C \subseteq \reals^n$ at a point $p \in \C$ as the subspace $\S_\C(p) = \{ v \in \reals^n : \exists \epsilon_0 \geq 0 $ such that $p + \epsilon v \in \C$ and $p - \epsilon v \in \C \; \forall \epsilon \in (0,\epsilon_0) \}$ .
\end{definition}
