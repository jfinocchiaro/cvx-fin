...
\begin{definition}[Consistent: property]
	...
	We simply say the pair is consistent with respect to $\gamma$ if there is a distance $\mu^\gamma$ such that $(L,\psi)$ are $\mu$-consistent with respect to $\gamma$.
\end{definition}
...
We show the contrapositive; suppose $(L, \psi)$ is not consistent with respect to $\gamma$, and take $\Gamma := \prop{L}$.
Then there is a distribution $p \in \simplex$ so that $u \in \Gamma(p)$ but $\psi(u) \not \in \gamma(p)$.
Consider the constant sequence $\{u_m\} = u$ for all $m$.

We then consider a distribution $D = D_x = p$ for some $x \in \X$ and any sequence $\{f_m\}$ so that $f_m(x) = u_m$ for all $m$.
...
Hence, we do not have consistency.
\end{proof}
...
\begin{enumerate}
	...
	\item [C] For all $u \in \reals^d$, $R_\ell(\psi(u); p) \leq \zeta(R_L(u;p))$.
...
\end{proposition}
\begin{proof}
	\jessie{Contrapositive; not calibration by def~\ref{def:calibrated-finite} implies $\inf_{u : \psi(u) \not \in \gamma(p)} L(u;p) = \inf_u L(u; p)$.
	Thus there is a sequence $\{u_m\}$ so that $\lim_{m \to \infty} \psi(u_m) \not \in \gamma(p)$ and $L(u_m; p) \to \risk{L}(p)$.  Now we contradict $D$ and by Lemma~\ref{lem:calib-converging-regrets}, we have not calibrated by Def~\ref{def:calibrated-general}. }
...
%%	\lim_{m \to \infty}\ell(\psi(u_m) ; p) &\leq \risk{\ell}(p) + \lim_{m \to \infty} \zeta(L(u_m; p) - \risk{L}(p)\\
...
\begin{lemma}\label{lem:calib-converging-regrets}
	A surrogate and link $(L,\psi)$ are calibrated (via definition~\ref{def:calibrated-general}) with respect to $\ell$ if and only if, for all $p \in \simplex$ and sequences $\{u_m\}$ so that $R_L(u_m; p) \to 0$, we have $R_\ell(\psi(u_m); p) \to 0$.
\end{lemma}
\begin{proof}
	...
	$\impliedby$ Consider $\zeta(c) := \inf_{u: R_L(u;p) \geq c} R_\ell(\psi(u); p)$.for nonnegative values of $c$, and $\zeta(c) = 0$ for $c < 0$.
	We will show $R_L(u_m; p) \to 0 \implies R_\ell(\psi(u_m); p) \to 0$ gives calibration via the function $\zeta$ constructed above. 
	
	\jessie{Outline: 1. $\zeta$ satisfies requirements (A).  2. Verify bound is true as approach $0$ by assumption on regrets. 3. Verify bound is true elsewhere by construction of $\zeta$.}
	
	First, we can verify that $\zeta$ as constructed maps is continuous at $0$ and $\zeta(0) = 0$.
	\jessie{continuous: TODO}
	As $\risk{L}$ and $\risk{\ell}$ are well-defined for all $p \in \simplex$, taking $u = \Gamma(p)$ yields $\zeta(0) = \inf_u (R_\ell(\psi(u); p))$, which is $0$ as long as there is some $u$ so that $\psi(u) = \gamma(p)$ for each $p \in \simplex$.
	
	Second, we want to verify that the bound in Equation~\ref{eq:calibrated-general} holds as regrets approach $0$.
	Fix $p \in \simplex$.
	If $\{u_m\}$ is a sequence so that $R_L(u_m; p) \to 0$, then $\zeta(R_L(u_m; p)) \to 0$.
	As $R_\ell(\psi(u_m); p) \to 0$, then $\zeta$ of surrogate regret upper bounds discrete regret as surrogate regret gets arbitrarily close to $0$.
	
	Last, we observe that the bound in Equation~\eqref{eq:calibrated-general} is satisfied for $\{u_m\} \not \to 0$ by construction of $\zeta$.	
	\jessie{TODO}
\end{proof}
