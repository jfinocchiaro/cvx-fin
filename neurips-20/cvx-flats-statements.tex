...
\begin{proof}
	...
	\jessie{Outline: 1. $\zeta$ satisfies requirements (A).  2. Verify bound is true elsewhere by construction of $\zeta$.}
	
	First, we can verify that $\zeta$ as constructed is continuous at $0$ and $\zeta(0) = 0$.
	As $c < 0 \implies \lim_{c \to 0^-} \zeta(c) = 0$, we need to show first that $\lim_{c \to 0^+} \zeta(c) = 0$ and $\zeta(0) = 0$.
	Consider $\{u_m\}$ so that $R_L(u_m;p) \to 0$. Since $R_\ell(\psi(u_m); p) \to 0$, we have the supremum of a function continuous (at $0^+$) is also continuous at $0^+$, so we have $\lim_{c \to 0^+} \zeta(c) = 0$. 
	
	As $\risk{L}$ and $\risk{\ell}$ are well-defined for all $p \in \simplex$, taking any $\{u_m\} \to u \in \Gamma(p)$ yields $\zeta(0) = \sup_{u':R_L(u;p) = 0} (R_\ell(\psi(u'); p)) = \lim_{m \to \infty} R_\ell(\psi(u_m); p) = 0$, with equality following since $\{u_m\}$ is arbitrary. \jessie{Not super happy with this, but I also think I'm making it more complicated than it needs to be.}
