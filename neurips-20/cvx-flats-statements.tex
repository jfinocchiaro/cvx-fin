...
%	}
	...
	Take $D$ to be the distribution with a point mass on one $x \in \X$ so that $p = D_x$, and consider any sequence of functions $\{f_m\} \to f$ with $f_m(x) = u_m$ for all $f_m$.
...
\begin{theorem}
  ...
  If $\lbar$ is non-constant on every non-singleton level set of $\Gamma$, then $\eliccvx(\lbar) \geq d+1$.
\end{theorem}
\begin{proof}
