...
\cite{ramaswamy2016convex} further show necessary and sufficient conditions for finite prediction problems to be consistent that can be applied to general discrete losses, i.e. those where both the report and outcome set are finite.
...
\begin{definition}[Calibrated]\label{def:calibrated-general}
	...
	\end{equation}
\end{definition}
In finite settings, the calibration is often given a more restrictive definition~\citet{bartlett2006convexity,tewari2007consistency,ramaswamy2016convex,ramaswamy2018consistent}.
Their definition is given below, and we show in the Proposition~\ref{prop:calib-defs-equiv} that the general definition implies the following definition in finite settings.
...
\begin{proposition}\label{prop:consistent-implies-indir-elic-loss}
	If a surrogate and link pair $(L, \psi)$ is consistent with respect to $\ell$, then $L$ indirectly elicits $\gamma := \prop{\ell}$.
\end{proposition}

\begin{proposition}\label{prop:consistent-implies-indir-elic-prop}
	If a surrogate and link pair $(L, \psi)$ is consistent with respect to $\gamma$, then $L$ indirectly elicits $\gamma$.
\end{proposition}
...
Unless otherwise mentioned, we will assume that $W$ is full rank so $f = n-d$.
