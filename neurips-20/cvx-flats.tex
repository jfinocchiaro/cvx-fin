...
\jessie{Mention calibration results are in the appendix if the reader wants that connection too.}
...
\jessie{Discussion connecting to \cite{ramaswamy2016convex}.}
...
\begin{proof}
	...
	Moreover, as $d$ is the maximal codimension of such a flat, we show in Lemma~\ref{lem:feas-sub-is-a-flat} that $\S_{\gamma_r}(p)$ is one such flat, and we have $\dim(\S_{\gamma_r}(p)) \geq \dim(F') \geq \|p\|_0 - d- 1$.
