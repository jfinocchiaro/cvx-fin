...
A basic requirement of surrogate losses $L$ is \emph{consistency}, \bo{classically, we have the necessary cond. of calibration. this expands it to the four quadrants and weakens to indirect}.

which we next formalize in two ways.
First (Definition \ref{def:consistent-ell}), if one is given an original loss $\ell$, then we say $L$ is consistent if optimizing $L$ implies optimizing $\ell$.
Second (Definition \ref{def:consistent-prop}), if one is given a property $\gamma$ (for example, the conditional mean), then we say $L$ is consistent if optimizing $L$ implies approaching, in some measurable sense, the correct statistic $\gamma(D_x)$ of the conditional distributions.
We will then show that consistency always implies $L$ indirectly elicits $\prop{\ell}$ or $\gamma$ respectively.

\bo{Mention how calibration fits in?}
...
\bo{Yeah, I skipped this since it seems like we won't bring in calibration definitions to the story...}
...
\begin{theorem}\label{thm:cvx-flats}
	...
	For all $r\in\gamma(p)$, if there is no flat $F$ of codimension at most $d$ containing $p$ so that $F \cap \simplex \subseteq \gamma_r$, then there is no convex surrogate loss $L : \reals^d \times \Y \to \reals$ that indirectly elicits $\gamma$.\jessiet{Do we still want to state it as the contrapositive?  That's where the bound comes from, but we use the original direction for all of the intuitive explanation.}
  \raft{I kind of see your point.  In fact, in the continuous setting, I use the original direction in the proof.  What do we do for FSD stuff?}
\end{theorem}
\begin{proof}[Proof]
	We prove the contrapositive: if there is a convex surrogate loss $L$ indirectly eliciting $\gamma$, then for all $p \in \simplex$ and $r \in \R$ such that $r \in \gamma(p)$, there is a flat $F$ of codimension at most $d$ such that $F \cap \simplex \subseteq \gamma_r$ and $p \in F$.
...
In this case, the bound by feasible subspace dimension yields $\ccdim(\ell^) \geq 3 - 2 - 1 = 0$, as the feasible subspace dimension is $2$ since we are on the relative interior of the level set and simplex, as shown in Figure~\ref{fig:fsd-bound}.
