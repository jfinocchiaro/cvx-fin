...
\begin{lemma}\label{lem:feas-sub-is-a-flat}
	Suppose we have the finite property $\gamma$ indirectly elicited by the convex loss $L : \reals^d \times \Y \to \reals$ and distribution $p \in \simplex$.
	The subspace $F := \F_\C(p) \cap (-\F_\C(p))$ is a flat of the loss $L$, contains $p$, and $F \cap \simplex \subseteq \gamma_r$.
\end{lemma}
\begin{proof}
	\jessie{TODO}
\end{proof}
