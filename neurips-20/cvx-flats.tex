...
%	So F is also in gamma_r.
  ...
  cf.~\cite[$\mathbf{A}^m$ in Theorem 16]{ramaswamy2016convex}.
	Now taking $F := \ker(W) \cap \affhull(\simplex)$, we have\botodo{justification needed for codim=rank, because of intersection with the simplex} $\codim(F) = \rank(W) \leq d$, and $p \in F$ by construction.
