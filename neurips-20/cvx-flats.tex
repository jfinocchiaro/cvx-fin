...
\bo{This will need some references!}
\paragraph{Background.}
A basic requirement of surrogate losses $L$ is \emph{consistency}, which roughly means that minimizing $L$-loss corresponds to solving the original problem of interest.
We can show a learning problem is hard by showing that consistency is not possible without a certain amount of complexity -- here, dimensionality $d$ of the prediction space.
But consistency, which we formalize below, is not very tractable, so we seek more useful necessary conditions.
Ideally, such a condition would apply to all of the following settings:
\begin{itemize}
...
Here a notable more-tractable condition is \emph{calibration}, which has been used to prove lower bounds on dimension of consistent surrogates.\bo{references, etc}
An example is the \emph{feasible subspace dimension} approach of \bo{TODO}.

\paragraph{Our approach: indirect elicitation.}
In this section, we show that indirect elicitation gives a single necessary condition for consistency that applies to all of the above settings.
This criterion is implied by both consistency and calibration (as fully explored in Appendix \bo{TODO}), so it might be viewed as a weak necessary condition for existence of a dimension-$d$ surrogate.
...
To begin, we formalize consistency in two ways.
...
\begin{definition}[Flat]
	A \emph{flat} $F \subseteq \affhull(\simplex) \subseteq \reals^n$ is a set $F = \ker W \cap \affhull(\simplex)$ for some $W\in\reals^{d\times n}$ where $d\leq n$.
...
%	
	...
	That would mean that $q \in F \implies \implies Wq = \vec 0 \implies \sum_y p_y x_y^i = 0$ for all $i \in [d]$, which in turn implies $\vec 0 \in \partial L(u;q)$, implying $q \in \Gamma_u \subseteq \gamma_{r}$.
\end{proof}
