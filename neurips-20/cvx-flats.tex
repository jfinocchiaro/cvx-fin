...
A natural tension arises between consistency and the prediction dimension, as consistency is always possible for a sufficiently large dimension, but a larger dimension can increase the computational and sample complexity of learning.
...
We now define flats formally. Recall $\affhull(\simplex) = \{\sum_{i=1}^n \alpha_i p_i : p_i \in \simplex, \alpha_i \in \reals, \sum_{i=1}^n \alpha_i = 1\}$, which has dimension $n-1$.
