...
In particular,~\cite{bartlett2006convexity} show that if a convex surrogate is differentiable, minimal at $0$, and nonnegative, there is a consistent surrogate for binary classification problems, and ~\cite{tewari2007consistency} generalizes this result for multiclass classification. \jessie{add \cite{ramaswamy2016convex, ramaswamy2018consistent} notes}

\cite{steinwart2007compare} generalizes the study of consistent losses from the classification setting \ldots \jessie{Need to come back to this.  area exam Jessie looked at this briefly, but current Jessie is a whole different person.}
...
\jessiet{Excess risk definition on its own}


\section{Characteristics of Consistent Convex Surrogates}\label{sec:char-convex}

Proposition~\ref{prop:consistent-implies-indir-elic} allows us to draw the connection between property elicitation and consistent losses.
...
\begin{definition}[Flat]
  ...
  \raft{Would suggest defining a bit more formally after, like ``In other words, $F\subseteq \reals^d$ can be written $F=\{x\in\reals^n : Wx + b = 0\}$ for some $W\in\reals^{d\times n}$ and $b\in\reals^d$ where $d\leq n$.  Then you can define the dimension of $F$ easily as $d$, assuming the matrix is full rank}
\end{definition}
...
\begin{theorem}\label{thm:cvx-flats}
	Suppose we are given a property $\gamma$.
  \raft{Made logic more direct}
	If, for any $p \in \simplex$, and $r\in\gamma(p)$,
	there is no $(n - d - 1)$-dimensional flat $F$ satisfying both $F \cap \simplex \subseteq \gamma_r$ and $p \in F$, then there is no convex surrogate loss $L : \reals^d \times \Y \to \reals$ that indirectly elicits $\gamma$.
\end{theorem}
\begin{proof}[Proof intuition]
  ...
  \raft{Nice way to start.}
	...
	Moreover, with $p$ fixed, we know there is a vector of witnesses $W = [x_1, \ldots, x_n]$ such that $\sum_y p_y x_y = \vec 0$.
  \raft{State the dimensions of $W$}
	
	\jessiet{This next paragraph is mostly me spelling it out for myself}
  \raft{Don't understand; this is necessary to complete the proof!  (So good thing you have it.)}
	Observe that $F := \ker(W)$ is at most $(n-d-1)$ dimensions in probability space and contains $p$ by construction.
  \raft{Justify the dimension.  Hint: $F$ contains $\vec 0$.  You'll probably want to define some $F'$ by adding a certain something to $W$ -- check out the FSD paper.}
	...
	To see this, consider $q \in F \cap \simplex$, but $q \not \in \gamma_r$.
	That would mean that $Wq = \vec 0$, which implies $\vec 0 \in \partial L(u;q)$, implying $q \in \Gamma_u$.
...
%%end Commented 4.16.20
	
%	We claim there exists a matrix $W \in \reals^{d+1 \times n}$ defining the flat $F = \{q \in \reals^n : Wq = \vec 0 \}$; $F$ is then $(n-(d+1))$-dimensional by the rank-nullity theorem, so we just need to show such a $W$ also keeps $F \cap \simplex \subseteq \gamma_r$.
%	As $\Gamma_u \subseteq \gamma_r$, it is sufficient to show $F \cap \simplex \subset \Gamma_u$.
%	 
%	Construct $W$ by taking each column to describe $\partial L(u, y)$ for each $y \in \Y$.
%	Since $\partial L(u, y)$ is $d$-dimensional, the affine subspace containing each subgradient can be described with $d+1$ constraints. \jessie{??}
%	We can see that $p \in F$ since $\vec 0 \in \oplus_y p_y \partial L(u, y) \implies Wp = \vec 0$ by construction of $W$.
%	Moreover, if there was a $\hat q \in \simplex \setminus \gamma_r$ such that $W \hat q = \vec 0$, then we would contradict the indirect elicitation of $\gamma$ since we would not have $\Gamma_{u} \subseteq \gamma_{\psi(u)}$ since we would have $\hat q \in \Gamma_u$ but $\hat q \not \in \gamma_{\psi(u)}$.
%	Therefore,  we have an $(n - d - 1)$-dimensional flat $F$ such that $F \cap \simplex \subseteq \gamma_r$ and $p \in F$ for all $p \in \simplex$ and $r \in \gamma(p)$.
\end{proof}


\section{Finite outcome settings and finite properties}\label{sec:finite-calib}
