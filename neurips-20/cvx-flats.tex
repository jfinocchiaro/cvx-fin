...
\begin{proof}
  ...
  Then \btw{todo: emphasize that it's target loss}
%  \raft{Discuss proof}
        \begin{align}
...
\raf{2}
Flats of a convex (expected) loss are complemented by its subgradients.\footnote{We define the subdifferential $\partial L(u,y) = \{g \in \reals^d : L(u',y) \geq L(u,y) +g^T (u' - u) \; \forall u' \in \reals^d  \}$, and members of this set subgradients.}  % of convex losses and their flats in order to understand convex elicitation complexity. 
\raft{What kind of zen is this}
While subgradients live in the prediction space and flats in probability space, the two can be connected considering a distribution $p \in \simplex$.
In a loose sense, this $p$ is a witness of a flat of the level set, and the flat must be of certain codimension (dependent on prediction dimension) so that, for this witness $p$, we can extract a witness $W$ of subgradient sets proving that $\vec 0 \in \partial \exploss{L}{u}{p}$; our desired optimality condition.
\jessiet{This is probably TMI but it at least helped me to write it out and think of it this way (p as a witness of the flat, in a loose sense)}
...
%	
	...
	Fix $p \in \simplex$ and $r \in \gamma(p)$, and take any $u \in \Gamma(p)$ such that $\psi(u) = r \in \gamma(p)$.\jessie{Does such a $u$ exist?}
  ...
  % (Observe that in order for $L$ to be convex, $L(\cdot, y)$ must be convex for all $y \in \Y$.) 
	...
	Expanding, we have $\oplus_y p_y \partial L(u,y) = \{p_1 x_1 + \ldots + p_n x_n : x_1 \in \partial L(u,1), \ldots, x_n \in \partial L(u,n) \}$.
%	In particular, $u \in \Gamma(p)$ if and only if $\vec 0$ is a $p$-convex combination of subgradients.
	Defining the matrix $W = [x_1, \ldots, x_n] \in \reals^{d\times n}$, we have $W p = \sum_y p_y x_y = \vec 0$
  (cf. $\mathbf{A}^m$ from~\cite[Theorem 16]{ramaswamy2016convex}).
  ...
  \raf{Old:}
	That would mean that $q \in F \implies Wq = \vec 0 \implies \sum_y q_y (x_y)_j = 0$ for all $j \in [d]$, which in turn implies $\vec 0 \in \partial L(u;q)$, implying $q \in \Gamma_u \subseteq \gamma_{r}$.
\end{proof}
...
\begin{restatable}[\citep{ramaswamy2016convex}]{theorem}{bayesrisklowerbound}\label{thm:bayes-risk-lower-bound}
