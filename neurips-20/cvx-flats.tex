...
%	So F is also in gamma_r.
  ...
  cf.~\cite[$\mathbf{A}^m$ in Theorem 16]{ramaswamy2016convex}.
	Now taking $F := \ker(W) \cap \affhull(\simplex)$, we have\botodo{justification needed for codim=rank, because of intersection with the simplex}\jessiet{Follows since codim is relative to $\affhull(\simplex)$, right?} $\codim(F) = \rank(W) \leq d$, and $p \in F$ by construction.
	...
	Apply Lemma~\ref{lem:set-valued-prop-flats} and observe that $p \in \gamma_r \cap \gamma_{r'}$, so $p \in F \cap \simplex \subseteq \gamma_{r'} \implies p \in F \cap \simplex \subseteq \gamma_r$.
\end{proof}
