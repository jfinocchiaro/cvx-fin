...
\raft{I think few ML people know anything about pinball loss... I think we should de-emphasize the continuous setting in the intro, and include it in passing.}\jessie{Removed the sentence, but probably want to at least allude to it?}
...
\raft{Cleaned up this prev sentence} \jessiet{Noted... much better how it is now.}
...
The goal is to produce a hypothesis $f: \X \to \R$, for some \emph{prediction space} (or prediction space) $\R$, which may be different from $\Y$.
...
\raf{Isn't that what we do next anyway?  I think it's nice in 2.1}
...
\bo{Maybe something here about how consistency means different things in different settings, so lower-bounding it with a single general technique seems tricky. Then the bullet points explain the different settings.} \jessie{Adding a few sentences to try to address this... probably needs to be pared down.}
Consistency, which we formalize in Section~\ref{sec:consis-implies-indir}, is not very tractable.
Throughout the literature, consistency is defined in many subtlely different ways; some papers (e.g.\,\cite{ramaswamy2016convex,ramaswamy2018consistent}) refer to consistent \emph{algorithms} while others (e.g.\,\cite{bartlett2006convexity,tewari2007consistency}) refer to consistent \emph{losses}.
Even more confusing is the slight difference between considering consistency based on the sequence of functions learned from trianing on $m$ samples as opposed to learning over any general sequence of functions.
In general, consistency relies on both a surrogate and target loss, and until now, it has been unclear how to define consistency without a target loss. 
...
While there are a few possible definitions of indirect elicitation, this one is the most applicable for our setting because when we consider set-valued properties, we do not have that \emph{all} optimal predictions for $\Gamma$ must be linked back to \emph{all} optimal predictions for $\gamma$.
Instead, we lighten this restriction to say that any optimal prediction for $\Gamma$ must be linked to \emph{an} optimal prediction for $\gamma$, and must be done deterministically. 
