...
In discrete predictions, it is well-known that a surrogate is consistent with respect to a target loss if and only if it is calibrated with respect to the target (e.g.\,~\citep[Theorem 1, part 3]{bartlett2006convexity}).
...
Second, given a property $\gamma$ (for example, the $\alpha$-quantile)\jessiet{changed example from conditional mean}, then we say $L$ is consistent if optimizing $L$ implies approaching, in some sense, the correct statistic $\gamma(D_x)$ of the conditional distributions (Definition \ref{def:consistent-prop}).
...
\raft{Reminder: give tons of credit to Hari's paper}\jessiet{Attempted.}
...
When given a discrete target loss,~\citet{ramaswamy2016convex} give lower bounds on the convex calibration dimension, or equivalently the dimension of a convex and consistent surrogate\jessiet{should probably cite the original calibration iff consistent in discrete setting result; have it earlier in the paper now.}, by constructing what they call the subspace of feasible dimensions and giving bounds in terms of its dimension.
