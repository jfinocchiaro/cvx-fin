...
\begin{proof}[Proof intuition]
	...
	We argue that there must be some $u \in \Gamma(p) \subset \reals^d$ such that $r = \psi(u)$.
	...
	Moreover, we must have $F \cap \simplex \subseteq \gamma_r$.
	To see this, consider $q \in F \cap \simplex$, but $q \not \in \gamma_r$.
	That would mean that $Wq = \vec 0$, which implies $\vec 0 \in \partial L(u;q)$, so $q \not \in \gamma_r$ would contradict $L$ indirectly eliciting $\gamma$.
