...
We consider supervised learning problems in the space $\X \times \Y$, for some \emph{feature space} $\X$ and \emph{label space} $\Y$.
...
\raft{FYI: moved $\supp$ to where we use it.}
...
In addition to the two possible targets, wee may have one of two domains: a \emph{discrete} target prediction space, like a classification problem, or a \emph{continuous} one, like a regression or estimation problem.
...
In prior work, most research on this problem focuses on the quadrant of target losses and discrete predictions.
\raft{Jessie: add cites here?}
...
To ensure consistency, the literature has embraced the notion of \emph{calibration} due to~\citet{tewari2007consistency}, and its generalizations to arbitrary discrete target losses\raft{Jessie: what are the best cites here? AA15?}.
...
Intuitively, calibration says that one cannot approach the optimal surrogate loss while linking to an supoptimal target prediction.
\raft{FYI Jessie: what you had was now pretty redundant to what we have earlier}
...
Arising from the statistics and economics literature, property elicitation is similar to calibration, but only places a restriction on the exact minimizers of a surrogate $L$.
...
\raft{FYI Jessie: I like this line but $D_x$ has not been defined yet}
...
It is straightforward to show that, for a discrete target loss $\ell$, if $(L,\psi)$ is calibrated with respect to $\ell$, then $(L,\psi)$ indirectly elicits $\gamma = \prop{\ell}$.
...
As we will see in Section~\ref{sec:consis-implies-indir}, indirect property elicitation is also a necessary condition for constructing a consistent surrogate loss.
...
\raf{Note: implied by calibration}
...
\botodo{wording/credit ok? \jessie{I think so}}
Theorem \ref{thm:cvx-flats} will show that if $d$ is small, then the level set $\{p : u \in \argmin_{u'} \exploss{L}{u'}{p}\}$ \bo{used comma instead of semicolon} must be large; in fact, it must roughly contain a high-dimensional affine subspace, or \emph{flat}.
...
\botodo{TODO: we have not yet introduced subgradients or the subdifferential!}\jessiet{Added as a footnote since I would hope most people know what a subgradient is, but also a good point to not assume.}
...
\bo{I didn't follow this very last part. Can revisit in the morning, or someone can clarify.}
...
\raft{FYI to Jessie: what you had was all reasonable/true, but was a dispersed.  Which is understandable---just explaining the motivation for the edits. \jessie{danke}}
