...
\begin{definition}[Flat]
	A \emph{flat} $F \subseteq \affhull(\simplex)$ of $\reals^n$ is an affine subspace of $\reals^n$.
	...
	The codimension of the flat $F$ relative to $\reals^n$ (relative to $\affhull(\simplex)$) is given by $\mathrm{rank}(W)$, and the dimension of $F$ is $n - \mathrm{rank}(W) - 1$.
\end{definition}
...
\begin{theorem}\label{thm:cvx-flats}
	...
	For all $r\in\gamma(p)$, if there is no flat $F \subseteq \affhull(\simplex)$ \jessie{affine hull or affine span?} of codimension at most $d+1$ containing $p$ so that $F \cap \simplex \subseteq \gamma_r$, then there is no convex surrogate loss $L : \reals^d \times \Y \to \reals$ that indirectly elicits $\gamma$.\jessiet{Do we still want to state it as the contrapositive?  That's where the bound comes from, but we use the original direction for all of the intuitive explanation.}
  \raft{I kind of see your point.  In fact, in the continuous setting, I use the original direction in the proof.  What do we do for FSD stuff?}\jessiet{The same.}
\end{theorem}
\begin{proof}
	...
	What we really show is that $F \subseteq \Gamma_u$, but as $\Gamma_u \subseteq \gamma_r$, the subset inclusion stacks to see the desired result.
	...
	Taking $W' := [W; \ones]^T$ and $b' = [\vec 0 ; -1]^T$, we now can take $F := \ker(W') + b'$, whose codimension is at most $d + 1$.

	We also claim that $F \in \affhull(\simplex)$ and $F \cap \simplex \subseteq \gamma_r$.
	To verify $F \in \affhull(\simplex)$, observe that the $\ones$ row of $W'$ and $-1$ element of $b$ implies $\sum_i p_i = 1$, enforcing $F$ is in the affine span of the simplex.
	Moreover, the nonnegativity constraints of $F$ are imposed by \jessie{??}
	
	To see $F \subseteq \gamma_r$, consider $q \in F$, but $q \not \in \gamma_r$.
	That would mean that $W'q + b = \vec 0$ where $b = [\vec 0 ; -1]^T$, which in turn implies $\vec 0 \in \partial L(u;q)$, implying $q \in \Gamma_u \subseteq \gamma_{r}$.
...
\begin{proof}
  ...
  By Theorem~\ref{thm:cvx-flats}, as $\hat L$ elicits $\hat\Gamma$ and indirectly elicits $\Gamma$, there is a level set $\hat\Gamma_{\hat r}$ and flat $\hat F$ containing $p$ such that $\codim(\hat F) \leq k$ and $S := \hat F \cap \simplex \subseteq \hat\Gamma_{\hat r} \subseteq \Gamma_r$.
  ...
  Let $F = \affhull(\Gamma_r)$ and recall $\codim(F)=d+1$.
  ...
  From the first two observations, Lemma~\ref{lem:affhull-relint} gives $\affhull(S) = \affhull(F'\cap \simplex) = F'$.
  Now as $S \subseteq \Gamma_r$, we also have $F' = \affhull(S) \subseteq \affhull(\Gamma_r) = F$,
  implying $\codim(F') \geq \codim(F)$.

  If $\Gamma_r = \{p\}$ is a singleton, then $F = \affhull(\Gamma_r) = \{p\}$, and in particular $n = \codim(F) \leq \codim(F')$, so we must have $k=d=n-1$.
  If $\Gamma_r$ is not a singleton, suppose for a contradiction that $k\leq d$.
  Then $\codim(F) = d+1 \geq k+1 \geq \codim(F')$, so we must have $\codim(F')=d+1=\codim(F)$; combining with $F' \subseteq F$ gives $F' = F$.
  We now have $S = F' \cap \simplex = F \cap \simplex = \Gamma_r$, and as $S \subseteq \hat\Gamma_{\hat r} \subseteq \Gamma_r$, we conclude $S = \hat\Gamma_{\hat r} = \Gamma_r$.
...
As a warm-up, let us see how to show the elicitation complexity of the variance of a random variable, with respect to convex losses, is exactly 2 whenever $|\Y|\geq 3$.
Let $\Y\subseteq\reals$ be a finite set with $n=|\Y|$.
Observe that the variance of $Y$ is the Bayes risk of squared loss $L:\reals\times\Y\to\reals$, $L(r,y) = (r-y)^2$.
We can see this using the fact that squared loss elicits the mean $\Gamma(p) = \E_p Y$, so that $\Var_p[Y] = \E_p (\E_pY - Y)^2 = \min_{r\in\reals} \E_p L(r,Y)$.
...
Here we taking $p = \tfrac 1 n \ones$ to be the uniform distribution, and let $r=\E_pY$.
...
While intuitively obvious, the lower bound of 2 is not trivial, as pathological examples illustrate~\citep{frongillo2018elicitation}.
...
To show the bound, recall that $G$ is the Bayes risk of a proper loss defined by $L(p,y) = G(p) + s_p \cdot (\delta_y - p)$, where $\delta_y(y') = \ones\{y=y'\}$ is the point distribution on $y$ and $-s_p \in \partial (-G)(p)$ is a subgradient of $-G$ at $p$~\citep{gneiting2007strictly}\raf{other refs}.
