...
\begin{definition}[Consistent]
	...
	For a distribution $D$ over input and label spaces $\X \times\Y$, we say $\{f_m\}$ is \emph{consistent} with respect to a loss $L: \R \times \Y \to \reals_+$ if
	\begin{align*}
...
\begin{theorem}
	Suppose the property $\Gamma$ is elicited by the convex loss $L : \reals^d \times \Y \to \reals_+$ such that the infimum $L(\cdot)_y$ is attained for all $y \in \Y$.
...
\begin{theorem}
	Suppose we are given a discrete loss $\ell:\R \times \Y \to \reals_+$ eliciting $\gamma:= \prop{\ell}$.
	Fix $p \in \simplex$, and let $r$ be the report such that $p \in \gamma_r$.  
	If there is not $(n - d - 1)$-dimensional flat $F$ such that $F \subseteq \gamma_r$ and $p \in F$, then there is no surrogate loss $L : \reals^d \times \Y \to \reals_+$ and link $\psi:\reals^d \to \R$ such that $(L, \psi)$ is calibrated with respect to $\ell$.
