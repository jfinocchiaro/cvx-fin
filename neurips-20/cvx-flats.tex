...
In supervised machine learning, one often makes predictions about future outcomes by training a classifier to minimize a surrogate loss.
...
When given a target loss, we want low surrogate risk to imply low target risk.
...
In continuous estimation settings, one often starts with a statistic of the data they would like to estimate instead of a target loss.
...
\cite{agarwal2015consistent} is the first to our knowledge to formally relate property elicitation to the consistency of a surrogate loss, although they only consider the discrete setting. \jessiet{right?}

\subsection{Complexity notions}\label{subsec:complexity}

In order to study the difficulty of a learning problem, various works have studied the minimum input dimension $d$ needed in order to construct a consistent surrogate loss $L: \reals^d \times \Y \to \reals$.
Previously this has been studied through proxies such as calibration and property elicitation without establishing their connection to consistency in a general manner; that is, while addressing all four quadrants.
...
In this section, we show that indirect elicitation gives a single necessary condition for consistency that applies to all of the above settings (the four quadrants).
In discrete predictions, it is well-known that a surrogate is consistent with respect to a target loss if and only if it is calibrated with respect to the target.
...
We delegate this proof to Appendix~\ref{app:calibration} and directly prove the general result, which does not rely on a target loss.
...
Since this creterion is implied by both consistency and calibration, it might be viewed as a weak necessary condition for existence of a consistent dimension-$d$ surrogate, yet it gives state-of-the-art lower bounds on $d$.
...
When one focuses on \emph{convex} surrogates, all minima are global, and we then have $\vec 0  \in \partial L(u, y)$ if and only if $u$ minimizes $L$ in its first component.
...
Fixing a distribution $p$, we construct a set of $n$ witnesses in $\partial L(u,y)$ for each $y$ whose $p$-weighted sum is $\vec 0$.
...
Now we consider specific characteristics of convex losses in order to understand the convex elicitation complexity of a given property.
...
In fact, their main proof tool, the subspace of feasible dimensions, flat that is a linear shift of a flat satisfying the requirements of Theorem~\ref{thm:cvx-flats}, which lets us deduce their result as Corollary~\ref{cor:fsd-bound}.
...
\begin{definition}[Subspace of feasible directions]
	Define the \emph{subspace of feasible directions} $\Sc_\C(p)$ of a convex set $\C \subseteq \reals^n$ at a point $p \in \C$ as the subspace $\Sc_\C(p) = \{ v \in \reals^n : \exists \epsilon_0 > 0 $ such that $p + \epsilon v \in \C \; \forall \epsilon \in (-\epsilon_0,\epsilon_0) \}$.
