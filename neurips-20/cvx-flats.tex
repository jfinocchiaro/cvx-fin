...
\title[Indirect elicitation necessary for consistent surrogates]{Indirect elicitation as a necessary condition for consistent surrogate losses}
\usepackage{times}
...
We consider supervised learning problems in the space $\X \times \Y$, for some \emph{feature space} $\X$ and a finite \emph{label space} $\Y$ of size $n$, with data drawn from a distribution $D$ over $\X \times \Y$.
...
Intuitively, calibration says one cannot approach the optimal surrogate loss while linking to an suboptimal target prediction.
...
In particular, our bounds imply those given via feasible subspace dimension (Corollary~\ref{cor:fsd-bound}).\jessiet{I might suggest cutting this paragraph for the sake of space?}
\raft{Kept it and squoze elsewhere}
...
\raft{Cut the last bits here; I bet AA15 thought of their result as being about consistency, though I could be wrong}
