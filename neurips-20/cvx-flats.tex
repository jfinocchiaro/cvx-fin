...
Without a target loss, one typically starts with a statistic of the data they would like to estimate instead of a target loss, such as the conditional expected value or an $\alpha$-quantile.
...
that are efficient in the prediction dimension, as this is one potential source of significant computational speedup.
...
\raf{While ugly, for now let's use:} ``\verb|\exploss{L}{u}{p}|''
...
\raf{Would also like to mention how our proof is shorter, even factoring in all the lemmas and theorem 2, since we don't have to deal with the infimum in calibration (the eplisons in their proof) and cut straight to the exact minimizers.}
...
Shifting away from discrete predictions, we turn our attention to continuous estimation settings.
More often, the continuous estimation setting does not start with a target loss, but instead with a statistic of the data that we want to estimate, such as the conditional mean, variance, or entropy.
We use Theorem~\ref{thm:cvx-flats} in conjunction with results from~\citet{frongillo2018elicitation} to prove Theorem~\ref{thm:bayes-risk-lower-bound}, which yields new bounds on the convex elicitation complexity of properties like the variance and entropy.
\botodo{The pinball stuff felt a bit too much of a digression, I think. Jessie, could you rewrite this paragraph to say what readers should expect in the upcoming section? Perhaps mention quadrants explicitly to help readers follow the flow from the previous section. We can save the pinball text for a better opportunity.\jessie{Attempting... old paragraph commented out.}}
