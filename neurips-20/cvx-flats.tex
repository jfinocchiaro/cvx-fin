...
A loss $\ell : \R \to \reals_+^\Y$ is a \emph{discrete loss} if $|\R| < \infty$, and in general, we use $\ell$ to denote our \emph{original loss} for which we want to construct a consistent surrogate.
...
\begin{definition}[Consistent]
	...
	\begin{align*}
	\E_D L(f_m(X), Y) &\to \inf_f \E_D L(f(X), Y)~.~
	\end{align*}
...
In particular, when $\R \subseteq \reals^k$ for some $k \in \mathbb{Z}$, \cite{frongillo2015elicitation} introduce the notion of \emph{Elicitation Complexity} and \emph{Identification Complexity}, which uses $k$ to measure the ``efficiency'' of a loss $L$ eliciting $\Gamma$.
...
In Proposition~\ref{prop:consistent-implies-indir-elic} below, we relate consistency of a surrogate and link to indirect elicitation.
...
\section{Characteristics of convex surrogates}\label{sec:char-convex}
\iffalse
...
If a property is vector-valued (i.e.$\, \Gamma: \simplex \to \reals^d$), then we can consider the notion of \emph{identifiability}.

\begin{definition}[$d$-identifiable]
...
Moreover, we define the \emph{identification complexity} of a property $\Gamma$ as the minimum dimension $d$ such that there is a link $phi$ such that $\Gamma = \phi \circ \hat \Gamma$ and $\hat \Gamma$ is $d$-identifiable.
...
When one restricts their focus to \emph{convex} surrogates, we know that all minima are global, and we then have $\nabla L(u, y) = \vec 0$ if and only if $u$ minimizes $L$ in its first component.

\begin{theorem}
	Suppose the property $\Gamma$ is elicited by the convex loss $L : \reals^d \to \reals^\Y_+$ such that the infimum $L(\cdot)_y$ is attained for all $y \in \Y$.
...
Now we can consider specific characteristics of convex losses in order to understand the convex elicitation complexity \jessie{define?} of a given property.
