...
%	However, these discrete losses are often hard to optimize, so one often seeks a surrogate losses yielding desirable statistical guarantees with respect to the original problem.
	...
	In this paper, we are specifically interested in the construction of \emph{consistent} surrogate losses; bonus points if they are ``efficient'' in the dimension of their input.
	...
	We also extend the notion of consistency to properties, so that we may understand when one can construct a convex, consistent surrogate without requiring an original loss.
\end{abstract}
...
\begin{definition}
  ...
  We write $\iden(\Gamma) = \min\{d \in \N : \Gamma\text{ is $d$-identifiable}\}$.
\end{definition}
...
\jessie{$D \implies A$, but actually $\lnot B \implies \lnot D$}
Fix $p \in \simplex$.
Suppose we have a sequence $\{\epsilon_m\}$ so that $\epsilon_m \to 0$, but $\zeta(\epsilon_m) \not \to 0$.
If $L$ is continuous over the reals \jessiet{Need this, right?}, then for each $m$, we we can construct a subsequence $\{u^j_m\}$ so that $R_L(u^j_m, p) \to \epsilon_m$ for all $m$.
