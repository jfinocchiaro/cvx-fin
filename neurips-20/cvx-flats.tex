...
While this condition is restrictive, and rules out several popular surrogates such as logistic and exponential loss for binary classification, these are easily rectified for example by thresholding the loss $L'(u,y) = \max(L(u,y),\epsilon)$ for sufficiently small $\epsilon>0$.
...
\begin{lemma}\label{lem:intersect-levelsets}
  If $\gamma: \simplex \toto \R$ is an elicitable property, then for any pair of predictions $r \neq r' \in \R$, there is a hyperplane $H = \{p : v \cdot p = 0\}$ that weakly separates both $\gamma_r$ and $\gamma_{r'}$ with $\gamma_r \cap \gamma_{r'} \subseteq H$.
\end{lemma}
\begin{proof}
  Because $\gamma$ is elicitable, its level sets are closed convex bounded sets.\bo{cite}
  Note that if the intersection $\gamma_r \cap \gamma_{r'} = \emptyset$, then the separating hyperplane theorem already gives an $H$ that strictly separates the two sets, in particular, $\gamma_r \cap \gamma_{r'}$.

  \bo{Not finished - Sketch next. I don't know if we can appeal to something, but here's my approach to proceed.}

  \bo{Approach 1}

  Each level set corresponds to a report $r$, which corresponds to a supporting hyperplane $h_r$ of the epigraph of the Bayes risk, where $\gamma_r$ is the set of points above which $h_r$ supports $\epi(\ellbar)$.\bo{cite...?}
  So $\gamma_r \cap \gamma_{r'}$ is the set of points at which both $h_r$ and $h_{r'}$ support $\epi(\ellbar)$.
  \bo{Idea: use properties of hyperplanes from here...not sure how exactly}

  \bo{Approach 2}

  First, we claim $\gamma_r$ and $\gamma_{r'}$ are either equal, or have disjoint relative interiors. \bo{True?}

  If not equal, then the intersection is on the relative boundary of both of them, which since they're convex should give a weakly separating hyperplane.
\end{proof}
...
\end{lemma}
\begin{proof}
  First, by Lemma \ref{lem:intersect-levelsets}, $\gamma_r \cap \gamma_{r'}$ is a closed convex subset of a hyperplane that weakly separates $\gamma_r$ and $\gamma_{r'}$.
  Let it be parameterized by the unit vector $v \in \reals^{\Y}$, where $\gamma_r \subseteq \{ q \in \simplex : v \cdot q \leq 0 \}$, while $\gamma_{r'} \subseteq \{ q \in \simplex : v \cdot q \geq 0\}$, and $q \in \gamma_r \cap \gamma_{r'} \implies v \cdot q = 0$.

  Now, we prove the equivalent claim $F \cap \simplex \subsetneq \gamma_r \iff F \cap \simplex \subsetneq \gamma_{r'}$.
  Suppose $F \cap \simplex$ has a point $q$ not in $\gamma_r$.
  If $v \cdot q < 0$, then $q$ is already not in $\gamma_{r'}$, QED.
  Otherwise, we claim that $p - \epsilon (q-p) \in F \cap \simplex$ for small enough $\epsilon$.
  ...
  But $v \cdot (p - \epsilon (q-p)) = -\epsilon (v \cdot q) < 0$, so $q$ is not in $\gamma_{r'}$, QED.
\end{proof}
...
\begin{proof}
	...
	Now if $\gamma$ is elicitable and $p \in \relint{\simplex}$, we know that $\psi(u) = r'$ for some $r' \in \R$.%suppose there is another $r' \in \R$ with $r \neq r'$ and $r' \in \gamma(p)$.
