...
\raft{To discuss: labels or outcomes?}
...
A basic requirement of surrogate losses $L: \reals^d \times \Y \to \reals$ is \emph{consistency}, which roughly means that minimizing $L$-loss corresponds to
\raft{I like this phrasing -- nicely captures all settings}
solving the target problem of interest.
We consider two kinds of consistency.
In the first, we are given a \emph{target loss} $\ell$ and we define $L$ to be consistent (Definition \ref{def:consistent-ell}) if minimizing $L$ and applying a link yields hypotheses minimizing $\ell$.
...
Flats of a convex (expected) loss are complemented by its subgradients\footnote{We define the subgradient $\partial L(u,y) = \{g \in \reals^d : L(u',y) \geq L(u,y) +g^T (u' - u) \; \forall u' \in \reals^d  \}$.}.% of convex losses and their flats in order to understand convex elicitation complexity. 
