...
\end{theorem}
\begin{proof}
  Suppose that we have some convex-elicitable property $\hat\Gamma:\simplex\to\reals^k$ and link $\psi : \reals^k \to \reals$ such that $\lbar = \psi \circ \hat\Gamma$.
  ...
  We conclude $\hat\Gamma$ must refine $\Gamma$, and in particular, we must have $p \in \hat\Gamma_{\hat r} \subseteq \Gamma_r$ for some $\hat r\in\hat\Gamma(\simplex)$.
  ...
  By Theorem~\ref{thm:cvx-flats}, as $\hat\Gamma$ is $k$-convex elicitable, $\hat\Gamma_{\hat r}$ is the union of flats of codimension at most $k+1$ interesected with $\simplex$; let $\hat F$ be such a flat containing $p$, and $S = \hat F \cap \simplex$, so that $\codim(\hat F) \leq k+1$ and $p \in \hat F\cap\relint\simplex$ and $S \subseteq \hat\Gamma_{\hat r} \subseteq \Gamma_r$.
  As $p$ shows $\relint\simplex\cap \hat F \neq \emptyset$, Lemma~\ref{lem:affhull-relint} gives $\affhull(S) = \affhull(\hat F\cap \simplex) = \hat F$.
  Now as $S \subseteq \Gamma_r$, we also have $\hat F = \affhull(S) \subseteq \affhull(\Gamma_r) = F$,
  implying $\codim(\hat F) \geq \codim(F)$.

  If $\Gamma_r = \{p\}$ is a singleton, then $F = \affhull(\Gamma_r) = \{p\}$, and in particular $n = \codim(F) \leq \codim(\hat F)$, so we must have $k=d=n-1$.
  If $\Gamma_r$ is not a singleton, suppose for a contradiction that $k\leq d$.
  Then $\codim(F) = d+1 \geq k+1 \geq \codim(\hat F)$, so we must have $\codim(\hat F)=d+1$ and $\hat F = F$.
  We now have $S = \hat F \cap \simplex = F \cap \simplex = \Gamma_r$, and as $S \subseteq \hat\Gamma_{\hat r} \subseteq \Gamma_r$, we conclude $\hat\Gamma_{\hat r} = \Gamma_r$.
  By assumption, $\lbar$ is non-constant on $\Gamma_r$, so we have distributions $p,p' \in\hat\Gamma_{\hat r}$ with $\lbar(p)\neq\lbar(p')$, which contradicts $\lbar = \psi \circ \hat\Gamma$.
\end{proof}
