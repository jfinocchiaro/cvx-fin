...
\begin{proof}
  ...
  Each level set is a set of $p$ having the same subgradient of the Bayes risk of $\ell$, e.g. for some $s$, we have $\gamma_r = \{ p : s \in \partial \ellbar (p) \}$.\bo{cite}
  So $\gamma_r \cap \gamma_{r'} = \{ p : s,s' \in \partial \ellbar(p)\}$, for some $s,s'$.
  \bo{TO DO}
\end{proof}
...
\end{lemma}
\begin{proof}
  First, we claim \botodo{separate into another lemma?} that $\gamma_r \cap \gamma_{r'}$ is a closed convex subset of a hyperplane that weakly separates $\gamma_r$ and $\gamma_{r'}$.
