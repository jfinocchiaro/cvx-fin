...
For discrete prediction problems, one often is able to intuitive start with a target loss $\ell$ in mind: for classification, 0-1 loss comes to mind; for rankings, a borda-like loss that assigns error based on one's difference between the winner's predicted ranking.
...
This notion of a correct surrogate is captured by two slightly distinct notions: \emph{consistency}, introduced formally in Definitions~\ref{def:consistent-ell} and~\ref{def:consistent-prop}, and \emph{calibration}.
...
Recently, the elicitation of multiple properties was studied by~\cite{frongillo2015vector-valued,fissler2015higher,frongillo2018elicitation}.
...
\begin{lemma}\label{lem:consistent-loss-implies-prop}
	...
	\bo{Change to if and only if. Given $L,\psi$, define $\mu$, then consistent iff consistent.}
\end{lemma}
\begin{proof}
