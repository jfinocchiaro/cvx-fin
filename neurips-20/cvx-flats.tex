...
\begin{proof}
	...
	Then for all $\epsilon > 0$, we have either $p \pm \epsilon v \not \in \gamma'_r$.
	Moreover, this must be because $p \pm \epsilon v$ leaves the simplex $\Delta_{\supp(p)}$, as we have $p$ on the relative interior of that such simplex.
	While $p \pm \epsilon v$ may be in $\simplex$, we must then have $p \mp \epsilon v$ outside the simplex.
	As $\gamma_r \subseteq \simplex$, we have $v \not \in \S_{\gamma_r}(p)$.
...
\begin{proof}
	...
	Now, consider the distribution $p_0$ over $\Y' := \supp(p)$ where each component of $p_0$ is equal to the corresponding entry of $p$.
	
	If we can show that $L$ is calibrated with respect to $\ell' := \R \times \Y' \to \reals$ with $\ell(r,y) = \ell'(r,y)$ for all $r \in \R$ and $y \in \Y'$, then we observe the existence of a flat $F^*$ of codimension (in $\reals^{\Y'}$) at most $d$ so that $\dim(F') = \|p\|_0 - d - 1$ by Theorem~\ref{thm:cvx-flats}.
	Then we can use Lemma~\ref{lem:p-boundary-fsd} to observe $\dim(\S_{\gamma'_r}(p_0)) = \dim(\S_{\gamma_r}(p))$, and thus the result holds.
	
	Now to see that $L$ is calibrated with respect to $\ell'$, consider that $\gamma(p) = \gamma'(p)$ for all $p \in \Delta^{\Y'}$, and thus the bound in Equation~\eqref{eq:calibration} is true for all $p \in \Delta_{\Y'}$ as the expected losses are equal for all $y \in \Y'$.
