...
Our result generalizes a calibration result of \citet{ramaswamy2016convex} to all four\botodo{Again we mention the four settings, but we haven't said what they are. probably should either say it explicitly, or change this to something like ``all the above settings''} settings, yielding to our knowledge the most general such bound.
...
\btw{TODO: Give readers a sense of difficulty; add a little discussion}
...
%	So F is also in gamma_r.
	...
	\btw{Bo: moved all $\ell$ stuff later.}
  ...
  cf.~\cite[$\mathbf{A}^m$ in Theorem 16]{ramaswamy2016convex}.
	Now taking $F := \ker(W) \cap \affhull(\simplex)$ (by Definition~\ref{def:flat}), we have $\codim(F) = \rank(W) \leq d$, and $p \in F$ by construction.
