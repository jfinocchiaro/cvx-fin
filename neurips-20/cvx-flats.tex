...
\begin{definition}[Consistent]
	...
	For all distributions $D$ over input and label spaces $\X \times\Y$, we say a loss and link $(L,\psi)$ are consistent with respect to an original loss $\ell$ if 
	\begin{align*}
...
However, one open question that remains from their work is if these bounds are equal. \jessie{\ldots} \jessie{MOVE}
...
\begin{theorem}\label{thm:cvx-flats}
	...
	If there is not $(n - d - 1)$-dimensional flat $F$ such that $F \subseteq \gamma_r$ and $p \in F$, then there is no convex surrogate loss $L : \reals^d \times \Y \to \reals_+$ that indirectly elicits $\gamma$.
...
\begin{proof}[Proof intuition]
	...
	Suppose we have $L:\reals^d \times \Y \to \reals$ eliciting $\Gamma$ and link $\psi : \reals^d \to \R$ such that for all $u \in \reals^d$, we have $\Gamma_u \subseteq \gamma_{\psi(u)}$.
	Fix any $p \in \simplex$, and take any $r \in \gamma(p)$.
	We argue that there must be some $u \in \reals^d \in \Gamma(p)$ so that $r = \psi(u)$, since $\Gamma(p) = \emptyset$ otherwise.
	...
	The weighted Minkowski sum is in $d$-dimensional space as $L$ is defined on $\reals^d$.
