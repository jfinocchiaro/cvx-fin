...
\begin{theorem}\label{thm:consistent-implies-indir-elic}
	\botodo{Assume L is minnable}If a surrogate and link pair $(L, \psi)$ is consistent with respect to a property $\gamma$ or loss $\ell$ eliciting $\gamma$, then $(L, \psi)$ indirectly elicits $\gamma$.
\end{theorem}
\begin{proof}
...
\begin{lemma}\label{lem:intersect-levelsets}
  If $\gamma: \simplex \toto \R$ is an elicitable property, then for any pair of predictions $r, r' \in \R$ where $\gamma_r \neq \gamma_{r'}$, there is a hyperplane $H = \{x \in \reals^{\Y} : v \cdot x = 0\}$, for some $v \in \reals^\Y$, that weakly separates both $\gamma_r$ and $\gamma_{r'}$ with $\gamma_r \cap H = \gamma_{r'} \cap H = \gamma_r \cap \gamma_{r'}$.
\end{lemma}
\begin{proof}
  ...
  Suppose $\gamma$ is elicited by $\ell$.
  Let $v = \ell(r, \cdot) - \ell(r', \cdot)$, interpreted as a vector in $\reals^\Y$.
  Let $H = \{ q : v \cdot q = 0 \}$.
...
\begin{proof}
  ...
  \bo{Also get that only points in hyperplane are in the intersection}
  Now take a point $q \in \gamma_r \setminus \gamma_{r'}$ (WLOG, swapping roles if necessay).
  By containment in $\gamma_r$, we have $v \cdot q \leq 0$.
  Then $v \cdot q < 0$, because if $q$ is in the hyperplane and in $\gamma_r$, then it is in $\gamma_{r'}$.
  Now we claim that $p - \epsilon (q-p) \in F \cap \simplex$ for small enough $\epsilon$.
...
%	So F is also in gamma_r.
	...
	Since $\Gamma$ is nonempty by assumption, we know there is some $u \in \Gamma(p)$.
	
	We will first show $F \cap \simplex \subseteq \Gamma_u$; then show $\Gamma_u \subseteq \gamma_r$.
  ...
  Let $r' = \psi(u)$; we then have $\gamma_{r'}$ containing $F \cap \simplex$.
  If $|\gamma(p)| = 1$, then we are done; otherwise we have $\gamma$ elictable and $p \in \relint{\simplex}$.
  Moreover, if $r = r'$, we are done.
  Otherwise, we can apply Lemma~\ref{lem:set-valued-prop-flats} to observe that $p \in \gamma_r \cap \gamma_{r'}$, and since $p \in F \cap \simplex \subseteq \gamma_{r'} \implies p \in F \cap \simplex \subseteq \gamma_r$.
