...
A natural tension arises between consistency and the prediction dimension, as consistency is always possible for a sufficiently large dimension, but a larger dimension typically increases the computational\botodo{and sample?}\raft{I think so, but not sure -- you'd probably know better} complexity of learning.
...
First, after introducing a general definition of consistency (\S~\ref{sec:consis-implies-indir}) which applies to all of the settings mentioned above, we show that indirect property elicitation is a necessary condition for consistency in all these settings (Theorem~\ref{thm:consistent-implies-indir-elic}).
...
Our result generalizes a calibration result of \citet{ramaswamy2016convex} to all of the above 
%\botodo{Again we mention the four settings, but we haven't said what they are. probably should either say it explicitly, or change this to something like ``all the above settings''} 
settings, yielding to our knowledge the most general such bound.
\botodo{added these last two sentences}\raft{LGTM!}
A caveat to these results is that, unlike \citet{ramaswamy2016convex}, we generally require surrogates to be ``minimizable'', i.e. functions that attain their global infimum.
Removing this requirement is a technically interesting and perhaps useful direction for future work.

\botodo{modified this last paragraph a bit}\raft{added ``essentially'' again... such a great word, isn't it?}
...
A property is \emph{single-valued} if $|\Gamma(p)| = 1$ for all $p\in\simplex$.%, in which case we write $\Gamma\to\R$.\botodo{We do?}\jessiet{Commenting out.  We only used once in the appendix, do I changed that instance.}
We use $\Gamma := \prop{L}$ to denote the unique property elicited by $L$ from eq.~\eqref{eq:elic}.\footnote{Note that $\prop{L}$ is well-defined if and only if $L \in \L$.}
...
As a result, indirect elicitation in a necessary condition for consistency in this case; \S~\ref{sec:consis-implies-indir} will extend this to all four quadrants mentioned above.
\botodo{Somewhere, we need to credit AA15 for Thm 9 and Cor 10. \jessie{Did this under elicitation complexity section}}
...
\raft{In particular, I'm torn about whether this statement is accurate; in the sense that we can restrict to $\P\subseteq\simplex$ I think it's fair, but in the sense that calibration does not imply indirect elicitation without it, perhaps not.}\jessiet{If we define the property only on $\P \subseteq \simplex$ where $\Gamma$ is nonempty, calibration over $\P$ should hold, though, right?}
...
\jessiet{Take out of definition environment?}
\raft{I think we could do it, but (a) I'd worry that people wouldn't be able to find the definition when we use it later, and (b) I'm hoping we can make the distinction very clear, which now is achieved by the parallel definitions}
...
In particular, we will derive bounds that imply those given by the feasible subspace dimension approach (Corollary~\ref{cor:fsd-bound}).\jessiet{I might suggest cutting this paragraph for the sake of space?}
...
In particular, we will often use $\affhull(\simplex) = \{\sum_{i=1}^n \alpha_i p_i : p_i \in \simplex, \alpha_i \in \reals, \sum_{i=1}^n \alpha_i = 1\}$, which has dimension $n-1$.\jessiet{Space suggestion: move terse version of this to footnote.}
...
We now state our elicitation lower bound, Theorem \ref{thm:cvx-flats}.
By combining with Theorem \ref{thm:consistent-implies-indir-elic}, it will be used to imply consistency bounds.
\botodo{Added comparison to AA15; please proofread!}
A result of a similar flavor is \citet[Theorem 9]{agarwal2015consistent}, which lower-bounds the affine dimension of level sets of $\prop{L}$.
Theorem \ref{thm:cvx-flats} gives a more powerful criterion, namely containment of flats of certain dimension in the level sets, and this is needed to give good bounds for some of our examples.
Theorem \ref{thm:cvx-flats} also gives a lower bound for any given target $\ell$ or $\gamma$, rather than a given surrogate $L$ (although an analogous implication of their Theorem 9 may have been known to \citet{agarwal2015consistent}).
...
\botodo{I suggest cutting this paragraph entirely if we keep the full proof in. We mention above that it takes inspiration from Ramaswamy.}\jessiet{agreed; cutting this paragraph}
...
The proof heavily builds around the ``limits of sequences'' structure of the definition of calibration.
We now show that, by restricting slightly to the broad class of minimizable losses $\L$, their bound follows relatively directly from Theorem \ref{thm:cvx-flats}.\footnote{We conjecture, as an intriguing open problem, that even this restriction can be lifted and the bound of \citet{ramaswamy2016convex} can be entirely recovered via indirect elicitation lower bounds; perhaps by generalizing the definition of indirect elicitation to apply to non-minimizable losses in some way.}
...
\raft{Revisit this sentence! \bo{Visited, had fun, bought the t-shirt.}}
...
\begin{proof}[Proof sketch]
	...
	Thus, we have $\dim(\Sc_{\gamma_r}(p)) \geq n - d - 1$, completing the proof for $p \in \relint{\simplex}$.
	
	\btw{Bo: also rewrote this a bit; I hope it still sounds good?}
	When $p \not \in \relint{\simplex}$, we can loosely ``project down'' to the subsimplex on the support of $p$ and modify $L$ and $\ell$ accordingly.
	Now $p$ is in the relative interior of this subsimplex, so the above result gives that $\dim(\Sc_{\gamma_r}(p)) \geq \|p\|_0 - d - 1$, where the feasible subspace dimension is relative to $\reals^{\supp(p)}$.
...
Of course, improvements in general machine learning algorithms can be put to many uses in many societal contexts, so a more specific analysis of potential broader impacts appears very difficult to foresee.
...
\begin{proof}
	...
	\botodo{reworked this part, hope you like the change.}
