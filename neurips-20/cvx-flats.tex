...
A natural tension arises between consistency and the prediction dimension, as consistency is always possible for a sufficiently large dimension, but a larger dimension typically increases the computational\botodo{and sample?} complexity of learning.
...
First, after introducing a general definition of consistency (\S~\ref{sec:consis-implies-indir}) which applies to all of the\btw{Bo: changed ``all four'' to ``all of the'' since I think it's not clear what the four are yet} settings mentioned above, we show that indirect property elicitation is a necessary condition for consistency in all these settings (Theorem~\ref{thm:consistent-implies-indir-elic}).
...
\btw{Bo: added these last two sentences}
...
In Appendix~\ref{app:calibration}, we give a definition of calibration which is equivalent to Definition~\ref{def:calibrated-finite} in discrete prediction settings, but generalizes to continuous estimation settings.\jessiet{Suggest trimming down if needed \bo{agree}}
...
A property is \emph{single-valued} if $|\Gamma(p)| = 1$ for all $p\in\simplex$, in which case we write $\Gamma\to\R$.\botodo{We do?}
We use $\Gamma := \prop{L}$ to denote the unique property elicited by $L$ from eq.~\eqref{eq:elic}.\footnote{\bo{added} Note that $\prop{L}$ is well-defined if and only if $L \in \L$.}
...
\botodo{Somewhere, we need to credit AA15 for Thm 9 and Cor 10.}
