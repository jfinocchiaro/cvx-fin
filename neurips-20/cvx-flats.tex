...
\raft{FYI Jessie: pretty good!  I condensed and reorganized slightly, but it was basically good to go as it was}
...
\raft{Loved this prev sentence!  Great way to frame the issue.}
We are therefore interested in finding consistent convex surrogates with a low prediction dimension, such as logarithmic in the number of labels, as is desired in many structured prediction problems~\cite{ramaswamy2018consistent,yu2018lovasz}\raft{any other cites?}.
...
While meaningful bounds on the prediction dimension have recently begun to appear, both for discrete and continuous predictions~\cite{lambert2008eliciting,ramaswamy2012classification,ramaswamy2016convex,ramaswamy2018consistent,frongillo2018elicitation}, these works have several drawbacks.
...
\raft{Added: $\Y$ is always finite for us.  Reminds me: we should state infinite $\Y$ as a major open problem}
...
\raft{FYI Jessie: well done!  Still a bit of a digression, but I think it's good to have for context and paying proper dues to our elders :-)}
...
\begin{definition}[Indirect Elicitation]\label{def:indirectly-elicits}
	A loss and link $(L, \psi)$ \emph{indirectly elicit} a property $\gamma:\simplex \toto \R'$ if $L$ elicits a property $\Gamma: \simplex \toto \U$ such that for all $u \in \U$, we have $\Gamma_u \subseteq \gamma_{\psi(u)}$ \jessie{and $\Gamma_u \neq \emptyset$?}.
...
Formally, if $(L,\psi)$ is calibrated with respect to $\ell$, then $(L,\psi)$ indirectly elicits $\gamma = \prop{\ell}$.\jessiet{If $\gamma(p) = \emptyset$ fir some $p$, then calibration doesn't hold, so our concerns don't apply to this statement.}
...
Various works have studied the minimum prediction dimension $d$ needed in order to construct a consistent surrogate loss $L: \reals^d \times \Y \to \reals$, typically through proxies such as calibration~\citep{ramaswamy2016convex}
\raft{no other cites?  AA15 sort of counts I think} and property elicitation~\citep{frongillo2015vector-valued,fissler2016higher,frongillo2018elicitation}.
...
\end{lemma}\jessiet{Slight trickiness here.  Seems like this should still go through if $\Gamma$ can be empty though.  Have $\propdis(r,p) \neq 0$ for all $r$, but could still approach $0$.}
\begin{proof}
	First, observe that $\propdis(r,p) = 0 \iff \exploss{\ell}{r}{p} = \min_{r' \in \R} \exploss{\ell}{r'}{p} \iff r \in \gamma(p)$.\jessie{changed inf to min... right?}
...
%  \raft{Maybe ref the consistency definition here explicitly, at the top and the end}
	By Lemma~\ref{lem:consistent-loss-implies-prop}, it suffices to show the result for consistency with respect to a property $\gamma$, setting $\gamma := \prop{\ell}$ if $\ell$ is given instead.\jessiet{flagging; but calls on lemma 1 for the technical backbone.}
...
\end{theorem}
\begin{proof}
	Let $\Gamma := \prop{L}$ and suppose $(L, \psi)$ indirectly elicits $\gamma$ for some link $\psi : \reals^d \to \R$, so that for all $u \in \reals^d$, we have $\Gamma_u \subseteq \gamma_{\psi(u)}$.
...
In the proof of Theorem~\ref{thm:bayes-risk-lower-bound}\jessiet{??} below, we modify the argument of~\citet[Corollary 7]{frongillo2018elicitation} to bound the convex elicitation complexity of the Bayes risk of the loss.
