...
\begin{definition}[Finite property, nonredundant]
  We call $\gamma$ a \emph{finite} property is $\R$ is a finite set and call it \emph{nonredundant} if no level set is a superset of another.
...
Therefore, we show that there is no $V \in \reals^{5 \times 2}$ such that $S_i := \{w : Vw \leq B_i \}$ is nonempty and for $j = 1, \ldots, 5$, there is a $w \in \reals^2$ such that $\langle v_j, w \rangle = B_{ij}$.
