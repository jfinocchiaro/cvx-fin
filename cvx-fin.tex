...
\begin{proof}
  ...
  Therefore, there will always be some $T_i$ such that $0 \not \in T_i$.
  Specifically, if, as before, we say $||v_1|| \geq ||v_j||$ for all $j \neq 1$, we know that $0 \not \in T_1$.
  ...
  That is, the existence of an $x \in S_i$ such that $\inprod{v_{j+3}}{x} = 1 \implies \theta_{j,j+3} > 180^o$.
  However, this implies $\theta_{j+3, j} < 180^o$, and therefore, there is no $x$ such that $\inprod{v_j}{x} = 1$. 
  
  Like the triangle observed in the previous proof, we observe that $T_1$ is a trapezoid with some constraints that are not tight.
\end{proof}
