...
One way to look at this result in light of Proposition~\ref{prop:embed-trim} is that polyhedral losses ``directly'' elicit finite properties in a convex way.
...
\begin{proof}
  ...
  For the second part, let $\gamma':\simplex\toto\R'$ be the finite elicitable property embedded by $L$, with embedding $\varphi:\R'\to\reals^d$, and let $\psi$ be a calibrated link $\psi$ to a non-redundant elicitable property $\gamma:\simplex\toto\R$.
...
\raf{role of orderable}
\raf{...} relationships between the elicitation complexity by various classes of loss functions, including indirect elicitation via an embedding.
