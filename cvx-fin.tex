...
\begin{proof}
  ...
  Take some $\theta \in \trim(\Gamma)$ and consider the report $u$ so that $\theta = \Gamma_u$.
  If $u \in \varphi(\R)$, then clearly the subset inclusion is true.
  If $u \not\in \varphi(\R)$ but there is some report $w \in \varphi(\R)$ so that $\Gamma_u = \Gamma_w$, then we are again done.
