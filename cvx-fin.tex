...
\jessie{Will do once there is food in my stomach... should be ready by the morning.}
\begin{itemize}
	\item On the Algorithmic Implementation of Multiclass Kernel-based Vector Machines (Crammer and Singer 2001?)
	\item \cite{bartlett2006convexity}
	\item Classification with a Reject Option using Hinge Loss (Bartlett Wegkamp 2008)
	\item \cite{ramaswamy2018consistent} (Originally in 2014... new version in 2018.)
	\item \cite{agarwal2015consistent}
	\item \cite{ramaswamy2016convex}
\end{itemize}
...
\raf{Glue needed...}
For many problems in machine learning, researchers have naturally converged on some notion of learning a finite property by ``embedding'' the possible labels into points in $\reals^d$ and minimizing a consistent convex surrogate loss.
Here, we formalize that notion of embedding and show some geometric and topological characteristics of such embeddable properties.
...
We then want to show there is no $V\in \reals^{5\times 2}$ so that for all $i \in \{1, 2, \ldots, 5\}$ that $(V, B_i)$ is a tight H-rep, following from Definition~\ref{def:d-construction}.
...
\jessie{This is from the PDF notes...}
...
\raf{  By Lemma~\ref{lem:finite-full-dim}, $G$ is polyhedral and therefore closed, and by~\citet[Theorem 19.2]{rockafellar1997convex}, its convex conjugate $C : \reals^\Y \to \reals$ is also polyhedral and closed.
}

\begin{theorem}
  Let $\gamma:\simplex\toto\R$ be a non-redundant elicitable property, elicited by a loss $L:\R\to\reals^\Y_+$ whose Bayes risk is a closed function.
...
\begin{proof}
  ...
  Let $C:\reals^\Y\to\reals\cup\{\infty\}$ be the convex conjugate of $G$, given by $C(u) = \sup_{p\in\simplex} p\cdot u - G(p)$.  
  ...
  Now define $L:\dom(C)\to\reals^\Y$ by $L(u) = C(u)\ones - u$ where $\ones\in\reals^\Y$ is the all-ones vector.
  ...
  \end{align*}
  Putting this together with eq.~\eqref{eq:1}, and recalling that $s$ is a bijection, we have for any $u \in \D$ and $p\in\simplex$ that
  \begin{equation}
    \label{eq:3}
    u \in \prop{L}(p) \iff p \in \partial C(u) \iff s^{-1}(u) \in \gamma(p)~,
  \end{equation}
  which implies that $L$ embeds $\gamma$ with embedding function $\varphi:\R\to\reals^\Y$ given by $\varphi(r) = s(r)$.
  \raf{TO CHECK: the finiteness of $C$}
\end{proof}
