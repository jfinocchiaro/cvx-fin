...
\begin{proof}
  ...
  The first $d$ rows of $V$ form a (not-necessarily-orthonormal, but normal) basis for $\reals^d$.
  ...
  Taking the convex combination, we obtain $\langle v_{d+1}, w \rangle \leq -\alpha_1$.
  If $\alpha_1$ is positive, this implies there is slack in the $d$th constraint, so the halfspace representation cannot be tight.
  If $\alpha_1$ is positive, then we pick some other pair of vectors $i \neq j$ rather than $1,d+1$, such that $v_j = \sum_{i'\neq j} \alpha_{i'} v_{i'}$ with $\alpha_{i'} > 0$.
  \bo{I claim there must exist two such vectors in this collection ... need to prove it though!}
  Choosing the $i+1$st column of $B$ as a constraint gives the contradiction as above.
\end{proof}
