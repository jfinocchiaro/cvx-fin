...
\begin{proof}
  ...
  Applying the same argument iteratively, we know that if $v_c$ is the second-longest normal, in order for $(V, T_c)$ to be tight, we must observe $\theta_{j-1, j} > 60^o$ and $\theta_{j,j+1} > 60^o$.
  ...
  For the normals $v_i$ and $v_{i+2}$ such that $\theta_{i,i+2} \leq 120^o$, there is no way for the H-representation $(V, T_{i+1})$ to be tight. \jessie{.....}
  
  Therefore, there is no $V$ yielding a $2$-construction for abstain and $\alpha > 1/2$.
