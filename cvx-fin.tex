...
\begin{definition}[Finite property, non-redundant]
  We call $\gamma$ a \emph{finite} property if $\R$ is a finite set and call it \emph{non-redundant} if no level set is a superset of another.
  For simplicity, when we say a property is finite, we also require it to be non-redundant.
\end{definition}
...
When working with convex losses which are not strictly convex, one quickly encounters redundant properties: if $\inprod{p}{L(\cdot)}$ is minimized by a point $u\in\reals^d$ where $p\cdot L$ is flat, then there will be an uncountable set of reports $u'$ which also minimize the loss.
...
This transformation is the trim operator presented below.
