...
\end{proposition}
\begin{proof}
  For $1 \implies 2,3$: given that $\Gamma$ embeds the finite property $\gamma$, we show that $\trim(\Gamma) = \{\Gamma_u : u \in \varphi(\R) \} = \{\gamma_r : r \in \R\}$.
  ...
  We claim $G$ is polyhedral. $\gamma$ is finite (and by definition non-redundant), so each level set $\gamma_r = \Gamma_{\varphi(r)}$ is full-dimensional\bo{need to prove this separately in a lemma}.
