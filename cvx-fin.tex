...
\begin{definition}\label{def:trim}
  Let $\Gamma:\simplex \toto\R'$ be a property.
...
%If the loss $L$ is polyhedral, then we can construct a $\psi$ so that the above holds.
...
\raf{But wait, what about dimension?}

\section{Embedding Dimension}

\begin{definition}
  The \emph{embedding dimension} of a finite property $\gamma$, written $\elicembed(\gamma)$, is the minimum $d\leq 0$ \jessie{$d \geq 1?$} such that $\gamma$ is $d$-embeddable.
\end{definition}

\subsection{Dimension 1: real-valued losses}

\raf{Note: move monotone and lemmas to appendix}
...
\begin{theorem}
  ...
  \raf{add $elic$ for continuous non-loc-const properties}
\end{theorem}
