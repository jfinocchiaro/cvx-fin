...
Thus, the set generated by indexing a finite set is finite.
For $2 \implies 3$, assume each level set $\Gamma_r$ in the given set is full-dimensional without loss of generality...
\jessie{TODO} 


For $3 \implies 1$, observe that for each level set $\theta \in \Theta$, there is a report $u \in \reals^d$ such that $\Gamma_u = \theta$.
Let $\R$ be the collection of such reports $u$ corresponding to each level set.
