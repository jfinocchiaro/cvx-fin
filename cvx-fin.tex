...
\begin{abstract}
  We formalize and investigate two approaches to designing convex surrogate loss functions for problems such as classification or ranking.
  First, one can attempt to ``embed'' each prediction (label, ranking, etc) as a point in $\reals^d$ and design a convex loss function on that space.
  Second, one can use a piecewise-linear convex, or polyhedral loss.
  We prove that these approaches are equivalent in power, giving several characterizations of each, and show that every loss with a finite number of possible labels has a surrogate of both forms.
  Then, we investigate the dimensionality of the surrogate hypothesis space.
  We give a complete characterization of the $1$-d case.
  In higher dimensions, we give necessary conditions and a novel lower bound for abstain loss.
\end{abstract}
