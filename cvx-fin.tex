...
\bo{I think a figure would be really great here. One showing the 5 vectors, and one showing the constraints we get from one column of $B$ (i.e. the triangle.}
...
In our case, each $S_y$ is the intersection of $T^*$ with a halfspace, $T_y = \{w \in T^* : \inprod{v_y}{w} \leq -1\}$.
...
In constrast to this lower bound, \cite{ramaswamy2018consistent} give a $2$-embedding of the abstain loss on $|\Y|=4$.
...
\end{theorem}
\begin{proof}
  Like Theorem~\ref{thm:abs-5outcome-not-2embed}, we will again look at the abstain cell.  
  First, note that when $\alpha \geq \frac{n-1}{n}$, the abstain property decomposes into the mode, as the abstain cell is empty, so we can restrict to $\frac{n-1}{n} > \alpha > 1/2$.
  ...
  We know that $\beta := 1 - \frac{1}{\alpha} \in (0,1)$ as the inequalities are strict.
  Consider two requirements of the polytope: first, for all $i \in \{1,2, \ldots, 6\}, ||v_i|| > 0$.
  Additionally, we know $-\beta < 0$.
  Therefore, there will always be some $S_i$ such that $0 \not \in S_i$. 
  
  By a similar argument to Theorem~\ref{thm:abs-5outcome-not-2embed}, we then must observe $\theta_{j, j+3} > 180^o$ in order to have some $w$ that satisfies our tightness constraint for $j+3$.
  That is, there is an $x \in S_i$ such that $\inprod{v_{j+3}}{x} = 1$.
  However, this implies $\theta_{j+3, j} < 180^o$, and therefore, there is no $x$ such that $\inprod{v_j}{x} = 1$. 
\end{proof}
