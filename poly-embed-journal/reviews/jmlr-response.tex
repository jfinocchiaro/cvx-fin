...
\begin{enumerate}
	...
	We have clarified this point with a concrete example; see margin comment 1 in orange.
	
	\item \emph{Proof of Lemma~\ref{lem:polyhedral-range-gamma}}
		
	The reviewer is correct on all the suggestions; we have made changes accordingly.
	See margin comment 2 in orange for how we rephrased the sentence with too many parentheticals.

	\item \emph{``Thus, for any $u \in \cup \mathcal{U}$''}
	
	Each element of $\mathcal{U}$ is a set, so the union $\cup \mathcal{U}$ is a  bit imprecise; we clarify that we are discussing $u \in \bigcup_{U \in \mathcal{U}} U$.
\end{enumerate}
...
\item \textbf{Related work and discussion:}\\
 We thank the reviewer for the references and questions about the connections to $\mathcal{H}$-consistency and calibration. We have added a discussion about possible extensions to Section~\ref{sec:conclusion}. See margin comment 1 in blue.
...
\item \textbf{Miscellaneous minor issues and suggestions:}
  ...
  We added a formal definition of consistency into Appendix~\ref{app:consistency} with discussion about this definition and its history.\raft{I'm actually wondering if we should just define it in the paper like they suggest... let's see how the appendix section turns out} \jessiet{based on first pass of appendix section, might be in favor of adding in to body.}
...
\begin{enumerate}
	...
	The choice of surrogate loss is of course important for consistency, but so is the choice of link function; see the discussion after Definition~\ref{def:indirect-elic}, marked by margin comment 1 in green.  
