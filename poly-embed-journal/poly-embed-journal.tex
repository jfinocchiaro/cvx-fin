...
\begin{theorem}\label{thm:polyhedron-uniquely-gen-facets}
  ...
  (iii) $\{H \cap P \mid H^+ \in \H^*, H = \partial H^+\}$ is the set of facets of $P$.
\end{theorem}
\begin{proof}
	...
	Thus, $F$ such that $F = P \cap H$, where $H = \partial H^+$, is a facet of $P$, and is of dimension $d-1$.
	This implies the facet $F$ can be defined by $d$ affinely independent points (contained in $P$), whose affine hull is $H$.
	As halfspaces are uniquely determined, so is the facet $F = P \cap H$.
	As polyhedron are uniquely determined by their facets (by Minkowski's uniqueness theorem), we must have $H^+ \in \H^*$. 
