...
\begin{definition}
  ...
  We further say $\Sc$ is a \emph{minimum representative set}\jessiet{Propogate change} if it has the smallest cardinality among all representative sets.
\end{definition}
Minimum representative sets are essentially the same as \emph{maximally informative sets} from~\citet{wang2020weston}, but we found the name minimum representative set to give more intuition about being the smallest such set.
...
\begin{proposition}\label{prop:SL-minimal}
...
\begin{proof}\jessiet{I don't like this proof, but meh.}
	First, we have to show that $\Sc(L)$ is a representative set.
	As facets are closed and the hyperplane $p_F \mapsto (p_F, \inprod{p_F}{L(u_F)})$ supports $E_L$ (epigraph of negative risk) over the entire facet (and therefore $\Gamma(p) \cap \Sc(L) \ni u_F$ for all $p$ such that the hyperplane supports $E_L$ at $p$), we have $\Sc(L)$ representative for $L$.
	
	
	If $\Sc(L)$ was not a minimal set, then we could remove some $s$ from $\Sc(L)$ such that $\Sc' := \Sc(L) \setminus \{s\}$ is also a representative set for $L$.
	By the construction of $\Sc(L)$ over facets of $E_L$, we have some distribution $p$ such that the hyperplane $p \mapsto (p, L(s))$ uniquely supports $E_L$.
	We particularly have $\Gamma(p) \cap \Sc' = \emptyset$, since any other element $s'$ of $\Sc'$ being in $\Gamma(p)$ would imply $p \mapsto (p, L(s'))$ also supports $E_L$, but this hyperplane supporting $E_L$ must be unique, so these are equal.
	By construction of $\Sc$, we must have only added at most one of these to the set.
	Therefore, $\Sc(L)$ is a minimal representative set.
\end{proof}
...
\proposedadd{ALTERNATE PROOF FOR REVERSE IMPLICATION: 
	For the reverse implication, assume $\risk{L} = \risk{\ell}$, which are polyhedral functions as $\ell$ is discrete.  
	Construct $\Sc := \Sc(\ell)$ as in Definition~\ref{cons:rep-set}, and $\U := \Sc(L)$, with the embedding $\varphi : r \mapsto u_r$ if the hyperplanes $p \mapsto (p, \inprod{p}{\ell(r)})$ and $p \mapsto (p, \inprod{p}{L(u_r)})$ are equal.
...
Matching optimality is true by construction, since the optimal reports support the epigraph of negative Bayes risk on exactly the same distributions, meaning a report and its embedding are optimal at exactly the same distributions.}\jessie{This feels like it's missing a detail about how $\Sc$ is minimal.}
\end{proof}
...
\begin{proof}
  ...
  \proposedadd{Form the finite set $\Sc := \Sc(L)$ as in Definition~\ref{cons:rep-set}.
...
\begin{proof}
	...
	By Lemma~\ref{lem:polyhedral-range-gamma}, $\U = \Gamma(\simplex)$ is finite. \jessie{Do we need Lemma 3 or can we just conclude $L$ has a finite representative set without it?}
...
\proposedadd{Moreover, to construct an embedding of dimension $n-1$ instead of $n$ (see~\citet{finocchiaro2020embedding} for significance of dimension),  we use the same technique as in the proof of Proposition~\ref{prop:embed-bayes-risks}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Minimal Representative Sets, Non-redundancy, and Tight Embeddings}
...
%\begin{itemize}
%	\item $L$ embeds discrete $\ell$ over set $\Sc$ (minimally representative for $\ell$) iff $\trim_\Sc(\prop{\ell}) = \trim_{\varphi(\Sc)}(\prop{L})$
...
\begin{proof}[Sketch]
	...
	Since $r \in \Gamma(p_{r'}^r)$, we have the hyperplane $H_L(p_{r'}^r; r) := \inprod{p_{r'}^r}{L(r)}$ supporting $\risk{L}$ at $p_{r'}^r$, and similarly $H_L(p_{r''}^r; r)$ supports $\risk{L}$ at $p_{r''}^r$.
	However, $H_L(p_{r'}^r; r'')$ does not support $\risk{L}$ at $p_{r'}^r$, so we have
	\begin{align*}
	\inprod{p_{r'}^r}{L(r)} &= \inprod{p_{r'}^r}{L(r')} \neq \inprod{p_{r'}^r}{L(r'')}\\
	\inprod{p_{r''}^r}{L(r)} &= \inprod{p_{r''}^r}{L(r')} \neq \inprod{p_{r''}^r}{L(r'')}
	\end{align*}
	However, this yields a contradiction since $H_L(\cdot; r)$ is linear in its first argument, and therefore $\risk{L}$ must be linear between $p_{r'}^r$ and $p_{r''}^r$.
%	This means that each $\Gamma_r \subseteq \Gamma_{r'}$ or $\Gamma_{r''}$; in either case, we contradict non-redundancy.
%	\jessie{This is not well-written... nor correct as written?}
...
\begin{proposition}\label{prop:trim-unique}
	Suppose $\R$ and $\R'$ are minimum representative sets for the loss $L$.
...
\begin{definition}[Trim]\label{def:trim-loss}
	Given a loss $L:\R \to \reals_+^\Y$, we define $\trim(L) = \{L(r) \mid r \in \R'\}$ given any minimum representative set $\R'$ for $L$.
\end{definition}

\begin{proposition}\label{prop:trim-loss-condition}
	Given an loss $L : \R \to \reals^\Y_+$, we have $\trim(L) = \{L(u) \mid u \in \R \textrm{ s.t. } \neg\exists u'\in\R,u'\neq u,\, \Gamma_u \subsetneq \Gamma_{u'}\}$.
\end{proposition}
\begin{proof}[Sketch]
...
As we define one loss embedding another, one can imagine an analagous definition where one property embeds another, and we can define $\trim(\Gamma)$ as the set of level sets generated by a minimum representative set.
...
\jessie{Just took old TFAE for now, but this is quite different from Prop 4, united by (c) in each}
\begin{proposition}\label{prop:embed-trim}
	...
	\begin{enumerate}\setlength{\itemsep}{0pt}
		...
		\item \jessie{Adding} $\R^*$ is a finite (minimum?) representative set for $\Gamma$.
	...
	\end{enumerate}
	Moreover, when any of the above hold, $\trim(\Gamma) = \Theta$, and $\gamma$ is elicitable.
\end{proposition}
...
\begin{lemma}\label{lem:embedded-level-sets-trim}
	...
	\proposedadd{RETRY: Let $\Gamma$ be an elicitable property.
		If $\Gamma$ embeds a finite property $\gamma : \simplex \toto \R$ by the injection $\varphi$, then for any minimal representative set $\Sc$ for $\gamma$, we have $\{\gamma_r : r \in \Sc\} = \{\Gamma_u : u \in \varphi(\Sc)\} = \trim(\Gamma)=\trim(\gamma)$.}
\end{lemma}
\begin{proof}
	Let $L$ elicit $\Gamma$.
...
\proposedadd{First, construct $\Sc := \Sc(L)$ as Definition~\ref{cons:rep-set}\jessiet{Argue there is a bijection from this set to any other minimal representative set}, which is a minimal representative set by Proposition~\ref{prop:SL-minimal}. 
...
\begin{proof}
	...
	As $\R$, and therefore $\Sc$ is finite, we conclude that $\trim(\Gamma)$ is also finite.
	%  Since the latter set is finitely generated (as $\R$ is finite), we have have $\trim(\Gamma)$ is finite.
	Moreover, as $\gamma$ is nondegenerate (recall that $\gamma: \simplex \toto \R$ is shorthand for $\gamma: \simplex \to 2^\R \setminus \{\emptyset\}$ and $\Sc$ is representative), we have that $\cup \trim(\Gamma) = \cup_{r \in \Sc} \gamma_r = \simplex$, since if this were not true, $\Sc$ would not be a representative set for $\gamma$.
