...
Thus, for all $p \in \simplex$, there exists $r \in \R^*$ such that $\risk L(p) = \risk L_+(p) = g_{\V^*}(p) = \inprod{L(r)}{p}$ and therefore $r \in \prop{L}(p)$\raft{Edited and fleshed out}.
