...
\begin{abstract}
  \raf{prev title: An Embedding Framework for Consistent Polyhedral Surrogates}
...
In supervised learning, one tries to learn a hypothesis which fits labeled data by minimizing a target loss function.
...
To ensure the surrogate and link properly correspond to the target problem, the surrogate must also be \emph{consistent}, meaning that minimizing it over enough data and applying the link solves the target problem.
...
A growing body of work seeks to design and analyze consistent convex surrogates for particular target loss functions.
Generally speaking, surrogate design has been ad-hoc, with few tools to construct consistent convex surrogates, and even sparser understanding of what types of surrogates are possible.
For example, in top-$k$ classification, there are many proposed ``hinge-like'' surrogates which are piecewise linear and convex (henceforth, \emph{polyhedral}), which were later proved to be inconsistent~\cite{yang2018consistency}.
...
We prove that such an embedding scheme is intimately related to the class of polyhedral (piecewise-linear and convex)\jessiet{need this again?} loss functions.
...
\end{restatable}
\noindent
Beyond consistency, we show that any calibrated link gives rise to a linear surrogate regret bound, which allows one to translate generalization bounds from the surrogate to the target \citep{frongillo2021surrogate}.
...
\paragraph{Intuition for polyhedral embeddings}
\jessiet{Do we need this paragraph? or do we want to move it later?}
...
\paragraph{Takeaways}
We view our contributions as both conceptual and practical.
...
While property elicitation has an extensive literature by now~\citep{savage1971elicitation,osband1985information-eliciting,lambert2008eliciting,gneiting2011making,steinwart2014elicitation,frongillo2015vector-valued,fissler2016higher,lambert2018elicitation}, these works are mostly concerned with point estimation problems, instead of the more general statistics elicited by polyhedral embeddings. 
...
\raft{Rewrote this section intro, and added a bunch here so the reader knows roughly how all these concepts connect.  Could use some eyes.}
...
We will frequently appeal to concepts and results from property elicitation.
...
For example, hinge loss is minimizable, whereas logistic loss is not (take $p=(0,1)$ or $(1,0)$)\raft{FYI, added last sentence}.
...
\raf{Could not understand this last bit; do we need it? ``, which requires that the excess loss of some report is bounded by (a constant times) the excess loss of the linked report.''}
...
\begin{definition}
  ...
  For $\P\subseteq\simplex$, we say $(L,\psi)$ is $\P$-\emph{calibrated} with respect to $\ell$ if
for all $p \in \P$,
  \begin{equation}
    \label{eq:calibrated}
  \inf_{u \in \reals^d : \psi(u) \not\in \prop{\ell}(p)} \inprod{p}{L(u)} > \inf_{u \in \reals^d} \inprod{p}{L(u)}~.
  \end{equation}
  If $\P = \simplex$, we simply say the pair is \emph{calibrated}.
  If $(L, \psi)$ is calibrated with respect to $\ell$, we call $\psi$ a \emph{calibrated link.}
\end{definition}
\jessiet{Added in $\P$-calibration since we now discuss restrictions to recover calibration.  burn if okay}
\raft{We only use the definition once though that I could see (right?), and the other time I saw it used we explained by saying eq.~\eqref{eq:calibrated} is satisfied only on $\P$.  So I'd say we either revert to the simpler one, or actually use it later.  Leaning toward the former, since someone will forget what it means by the time we use it anyway :-)}
...
\begin{definition}\label{def:indirect-elic}
  ...
  The pair $(L, \psi)$ indirectly elicits a property $\gamma : \simplex \toto \R$ if for all $u \in \reals^d$, we have $\Gamma_u \subseteq \gamma_{\psi(u)}$, where $\Gamma = \prop{L}$.
	Moreover, we say $L$ indirectly elicits $\gamma$ if such a $\psi$ exists.
\end{definition}
...
However, when restricting to (minimizable) polyhedral surrogates, embedding is necessary and sufficient for calibration, which we show in \S~\ref{sec:poly-ie-consistency}.
\jessiet{Added sentence w/ forward ref to \S~\ref{sec:poly-ie-consistency}.  Burn}
\raft{In what sense is this last sentence true?  That section is saying $L$ indirectly elicits $\ell$ $\implies$ $L$ (with some link, maybe a different one) is consistent for $\ell$.  Agreed it would be nice to say something about that here.}
...
\raft{Need to combine/unify the filler text here and next paragraph \jessie{attempted. burn?}}
...
While polyhedral surrogates do not directly elicit finite properties, in the sense that they often have infinitely many optimal reports, they do elicit properties with a finite range, and thus have finite representative sets.
...
Since polyhedral surrogates have finite representative sets, one simply can pick one point from each of the finitely many polyhedral optimal sets to obtain a finite representative set.
This observation, combined with Lemma~\ref{lem:loss-restrict}, yields Theorem~\ref{thm:poly-embeds-discrete}.
...
\jessiet{There was an attempt to rewrite this paragraph}
The proof of Theorem~\ref{thm:discrete-loss-poly-embeddable} uses a construction via convex conjugate duality which has appeared in several different forms in the literature. 
%(e.g.\ \cite{duchi2018multiclass,abernethy2013efficient,frongillo2014general}).
For example, the literature on minimax optimization in supervised learning settings \citep{asif2015adversarial,farnia2016minimax,fathony2016adversarial} poses the construction in Theorem~\ref{thm:discrete-loss-poly-embeddable} as a minimax optimization problem of the expected loss matrix over both players' strategies, which becomes a convex relaxation of the discrete optimization problem.
Moreover, \citet{reid2012convexity} construct what they deem the ``canonical link'' for proper losses for continuous estimation problems, which is the continuous setting analog of the conjugate construction below, requiring the (invertible projection of the) Bayes risk to be differentiable.
\citet[Proposition 3]{duchi2018multiclass} give the same construction as we do below, but only comment on the calibration of surrogates under such constructions for multiclass classification tasks given strictly concave losses, inherently excluding polyhedral surrogates.
\citet{abernethy2013efficient} use the construction below as a cost function serving as a market maker in settings where the outcome space is large and running a complete market is not feasible, so cost is computed by the following scoring rule. \jessie{??}
Finally, \citet{frongillo2014general} use the following convex conjugate construction to connect mechanism design with affine scoring rules to the properties elicited by such scoring rules.
\raft{Let's give a whole paragraph here, going into more detail about the results in these papers.  For reference on the style one could use here, see the paragraph after Theorem 3 of \url{https://arxiv.org/pdf/1506.07212.pdf}, the one starting ``We briefly mention various forms of Theorem 3 which have appeared in the literature.''.  To add: that top-k reviewer's reference.  I also think this paper of Bob's is relevant \url{https://icml.cc/2012/papers/367.pdf}; their definition of ``canonical link'' (which he mentioned in your defense BTW) maps target reports to subgradients of the Bayes risk, and in their continuous setting I think it's the same thing.}
...
Recalling that calibration is equivalent to consistency for discrete targets, this shows that an embedding can always yield consistency.

The key idea is showing calibration equivalent to \emph{$\epsilon$-separation}\jessie{is this clause missing words?}, a lower bound on the distance between points in $\reals^d$ that link to optimal reports for $\ell$ and points that link to non-optimal reports.
...
Theorem \ref{thm:calibrated-separated} shows that calibration is equivalent to $\epsilon$-separation; Theorem \ref{thm:thickened-separated} constructs separated links.
...
\begin{restatable}{theorem}{calibratedseparated} \label{thm:calibrated-separated}
  ...
  $\epsilon>0$.
\end{restatable}
\botodo{Added proof ideas}
\raft{Nice! Updated to clarify surrogate vs target}
Intuitively, a separated link is calibrated because of two facts.
...
On the other hand, if a link is not separated, then the same two facts imply that a sequence of surrogate reports that get arbitrarily close to an optimal report set while linking to a suboptimal target report; this sequence has expected loss approaching optimal, violating calibration.
...
To set the stage for Construction \ref{const:eps-thick-link}, we sketch the two main steps in proving Theorem \ref{thm:thickened-separated}: \emph{(a)}\jessiet{minor, but we use (i) and (ii) in the intro, not emphasized.  do with that info what you want.  burn.} showing that one can produce a link $\psi$ such that $(L,\psi)$ indirectly elicits $\ell$; \emph{(b)} ``thickening'' $\psi$ such that it is separated.
...
However, it is not clear that this linking is possible because a point $u$ may be in the intersection of several thickened sets $U_{\epsilon}, U'_{\epsilon}$, etc. corresponding to $\Gamma(p), \Gamma(p')$, etc.
Therefore, we need to take each possible collection $U,U'$, etc., show that their intersection (if nonempty) contains a legal choice for the link, then thicken their intersection in an analogous way.
...
\subsubsection{A converse to Theorem \ref{thm:thickened-separated}}
...
\begin{proposition}\label{prop:link-converse}
  ...
  Let $\epsilon>0$ be given, and let $\Psi$ be the link envelope from Construction~\ref{const:eps-thick-link} for $L,\ell,\epsilon,\|\cdot\|_\infty$.
  Then $\psi$ is $\epsilon$-separated if and only if $\psi(u) \in \Psi(u)$ for all $u\in\reals^d$.
\end{proposition}
\begin{proof}
  For $u\in\reals^d$, let $P_u$ consist of all $p\in\simplex$ such that $d_{\infty}(u,\Gamma(p)) < \epsilon$.
  By definition, $\Psi(u) = \cap_{p \in P_u} R_p$ where $R_p = \{r \in \R : \varphi(r) \in \Gamma(p)\}$, the set of reports that embed into $\Gamma(p)$.
  By definition of embedding, $R_p = \gamma(p)$.
  By definition of separation, a link $\psi$ is $\epsilon$-separated if and only if, for all $u$, $\psi(u) \in \cap_{p \in P_u} \gamma(p) = \Psi(u)$.
\raft{Looks great/close, but I got lost with the ``if and only if, then'' \bo{updated, how about now?}\raf{That part is great now, but I'm worried about $R_p = \gamma(p)$ and wonder if we should step through that slower. Actually maybe this is an issue with the construction itself: I think you only want to work with the representative set from the definition of embedding, right?}}
\end{proof}

\begin{corollary}
  A link $\psi$ is calibrated for a given discrete target $\ell$ and polyhedral surrogate $L$ if and only if there exists $\epsilon$ such that $\psi$ is produced by Construction \ref{const:eps-thick-link} for $L,\ell,\epsilon,\|\cdot\|_{\infty}$.
\end{corollary}

\subsubsection{Remarks}
Applying Construction \ref{const:eps-thick-link} enables one to verify the consistency of a proposed link $\psi^*$.
...
\botodo{In general, how do we deal with the $\epsilon$? How do we know what $\epsilon$ to test? I guess we can gradually shrink it and see...}
\raft{A guess for $\epsilon$ was pretty clear from the top-$k$ paper, so yeah I think ``shrink and see'' makes sense.  Also probably okay to not even mention $\epsilon$ here given the last line of the paragraph.}
See \S~\ref{sec:top-k} for an illustration of this test.
On the other hand, if $\psi^*$ cannot possibly be produced from Construction \ref{const:eps-thick-link}, then it cannot be a calibrated link.
...
\botodo{Not sure if we have anything useful to say on computing $\epsilon$}
\raft{Agreed}
...
\raf{Updated the above; worth a check!}\jessie{Looks good to me?}
...
From here one can further remove redundant reports until arriving at a tight embedding if desired, or re-label embedded reports to cotextualize the problem; call this embedded loss $\hat \ell$.
...
\begin{minipage}{0.7\linewidth}
	\caption{The BEP surrogate $L$ (\S~\ref{sec:abstain}) is not calibrated for multiclass classification, eliciting the mode.
	The level sets of $\Gamma := \prop{L}$ are given in solid black lines, while the level sets of $\mode$ are given in dashed blue lines.
	To demonstrate non-calibration, one can pick distributions $p, p'$ in the relative interior of the abstain cell (blue) but in different cells of the mode.
  These distributions will satisfy $\Gamma(p) = \Gamma(p')$ but $\mode(p) \neq \mode(p')$.
  In this case, fixing $u\in\Gamma(p)$, it is impossible to define $\psi(u)$ to satisfy calibration, as $\psi(u)$ cannot be equal to $\mode(p)$ and $\mode(p')$ simultaneously.
  ...
  For this example, the BEP surrogate is classification-calibrated if one restricts to the set of distributions where at least one label $y\in\Y$ has probability $p_y \geq \tfrac 1 2$.
	}
...
In particular, our results give novel surrogate regret bounds for several applications (\S~\ref{sec:lovasz-hinge},~\ref{sec:winge},~\ref{sec:top-k}).\jessiet{This sentence sounds like we're going to give the bounds, but we decided against that, right?}
...
%\raf{Proposal: define $\ell$ for $1/2$, $L$ for BEP, $\psi^\infty$ for their link (bake in 1/2), $\psi^1$ for ours \jessie{Burn}}
\citet{ramaswamy2018consistent} study the loss $\ell: \Y \cup \{\bot\} \to \reals^\Y_+$ defined by $\ell(r)_y = 0$ if $r=y$, $1/2$ if $r = \bot$, and 1 otherwise.
The report $\bot$ corresponds to ``abstaining'' for a constant loss regardless of outcome $y$. 
\citeauthor{ramaswamy2018consistent} provide a polyhedral surrogate $L$, which they call the \emph{binary encoded predictions (BEP)} surrogate, and link $\psi^\infty$ which is calibrated for $\ell$.
Letting $d = \ceil{\log_2 |\Y|}$, their surrogate $L : \reals^d \to \reals^\Y_+$ is given by
...
Observe that $L$ is exactly hinge loss when $|\Y|=2$ and thus $d=1$. 
...
\begin{equation}\label{eq:abstain-link}
  ...
  \end{cases}~,
\end{equation}
and they go on to establish linear surrogate regret bounds for $(L,\psi^{\infty})$.

Using our framework, one can show that $L$ (eliciting $\Gamma$) embeds (2 times) $\ell$ (eliciting $\hat \gamma$), with the embedding given by $\varphi$ above where we define $\varphi(\bot) = 0 \in \reals^d$.
(Following the general procedure outlined above, the regions where $L$ is affine all have vertices in the set $\{-1,1\}^d \cup \{0\}$, meaning it is representative, and $L$ restricted to that set is precisely $2\ell \circ \varphi^{-1}$.)
...
With the abstain loss $\ell$ in hand, it is straightforward to verify that the $\ell$-optimal report for any $p$ such that $\max_y p_y < 1/2$ is to ``abstain'' from predicting by reporting $\bot$.
When juxtaposed with multiclass classification in Figure~\ref{fig:abstain-intuition-inconsistent}, one can observe that the BEP surrogate $L$ does not indirectly elicit the mode, as the level sets $\Gamma_{\vec 0} = \hat \gamma_\bot$ are equal by the definition of embedding, yet one can observe $\prop{\ell}_\bot \not \subseteq \mode_r$ for any $r$. 
...
Moreover, as we illustrate in Figure~\ref{fig:abstain-links}(L), the link $\psi^{\infty}$ proposed by \citeauthor{ramaswamy2018consistent} can be recovered from Construction~\ref{const:eps-thick-link} by choosing the norm $\|\cdot\|_\infty$ for any sufficiently small choice of $\epsilon > 0$.
...
\begin{equation}\label{eq:abstain-link-1}
  ...
  \end{cases}~.
\end{equation}
This link is the result of Construction~\ref{const:eps-thick-link} for norm $\|\cdot\|_1$ and the choice $\epsilon=1$, which proves calibration of $(L,\psi^1)$ with respect to $\ell$.
...
\end{minipage}\hfill
\caption{Constructing links for the abstain surrogate $L$ with $d=2$. The embedding is shown in bold labeled by the corresponding reports. (L) The link envelope $\Psi$ resulting from Construction~\ref{const:eps-thick-link} using $\|\cdot\|_\infty$ and $\epsilon = 1/2$, and a possible link $\psi$ which matches eq.~\eqref{eq:abstain-link} from~\cite{ramaswamy2018consistent}.  (M) An illustration of the thickened sets from Construction~\ref{const:eps-thick-link} for two sets $U, U' \in \U$, using $\|\cdot\|_1$ and $\epsilon = 1$. (R) The $\Psi$ and $\psi$ from Construction~\ref{const:eps-thick-link} using $\|\cdot\|_1$ and $\epsilon = 1$.}
...
\begin{figure}
	\begin{center}
		...
		\end{minipage}\hfill
		\caption{Links $\psi^*$ and $\psi^\diamond$ such that $(L^f, \psi^*)$ and $\psi^\diamond$ are calibrated with respect to $\ellabs$ for all suitable $f$. Points in each region link to the embedding point contained in the region. Both are constructed via the link envelope $\Psi$ from Construction~\ref{const:eps-thick-link}, which yields possible choices for calibrated links.\raf{Labels are still a tad small, probably because the actual size of the image (before shrinking) is larger.  Also, to match Figure~\ref{fig:abstain-links}, we can use commas instead of ``OR'', which should give us some more space too.  (Besides, it's really ``AND'' in the sense that both are in $\Psi(\cdot)$...)}
			}
...
\raf{See if we can simplify / combine these paragraphs given the new 5.1}\jessie{Attempted}
%% one might make distributional assumptions to regain consistency with geometric drawing
Upon showing that $L^{WW}$ embeds $\ell^\OP$, \citeauthor{wang2020weston} to characterize $\prop{\ell^\OP}$ and use it to give sufficient distributions assumptions over labels in $\P \subseteq \simplex$ such that $L^{WW}$ and the canonical argmax link $\psi(u) : u \mapsto r \in \argmax_y \inprod{e_y}{u}$ are calibrated with respect to 0-1 loss on $\P \subseteq \simplex$ (i.e., that eq.~\ref{eq:calibrated} holds for all $p \in \P$).
...
Like Figure~\ref{fig:abstain-intuition-inconsistent}, Figure~\ref{fig:ordered-partition} gives the level sets $\gamma_T =\{p \in \simplex \mid T \in \prop{\ell^\OP}(p)\}$ for each $T \in \T$\jessie{notation} juxtaposed with the level sets of the mode.
...
%		The level sets of the mode (for which $L^{WW}$ is proposed as a surrogate) are given by the cells formed by the dashed blue lines.
      For the same reason as in Figure~\ref{fig:abstain-intuition-inconsistent}, the level sets of $\prop{\ell^\OP}$ whose relative interiors span multiple cells of the mode, colored blue, cannot be properly linked to the mode.
...
It also reveals some deeper structure of embeddings, even down to the geometry of the underlying properties, and the equivalence of various notions of non-redundant predictions.
...
\raft{TODO: Brief description of the result}\jessiet{Attempted. burn if okay}
...
\raf{Short parag on why we care about trim}
...
\section{Polyhedral Indirect Elicitation Implies Consistency\jessie{Calibration?}}
...
\raf{Jessie: sync up refines vs indirect elic; use the latter unless it's really awkward \jessie{burn}}
...
With this result, we can show that if a surrogate indirectly elicits a target property, then it is also calibrated with respect to any loss eliciting the target property.
...
\begin{proof}
  ...
  Furthermore, Lemma~\ref{lem:X}(\ref{item:X-min-Theta},\ref{item:X-redundant},\ref{item:X-tight-embed}) implies that $L$ (eliciting $\Gamma$) indirectly elicits $\prop \ell$ (eliciting $\hat \gamma$) for any discrete loss $\ell$ that $L$ tightly embeds.
  (To construct a link, construct $\hat \psi: u \mapsto r$ such that $\Gamma_u \subseteq \hat \gamma_r$ for all $u \in \reals^d$.)

  We claim that, for any property $\gamma$, and any loss $\ell$ that $L$ tightly embeds, $L$ indirectly elicits $\gamma$ if and only if $\ell$ indirectly elicits $\gamma$.
  ...
  For the other direction, Lemma~\ref{lem:X}(\ref{item:X-min-Theta},\ref{item:X-tight-embed}) shows that the level sets of $\hat \gamma$ are contained in the set $\{\Gamma_u \mid u\in\reals^d\}$.
  Thus, if $L$ indirectly elicits $\gamma$, then in particular $\ell$ indirectly elicits $\gamma$.
...
\begin{proof}
  ...
  By Lemma~\ref{lem:ie-iff-embeds-refinement}, $L$ tightly embeds a discrete loss $\ell^\emb:\R^\emb\to\reals^\Y_+$ such that $\ell^\emb$ (directly) elicits $\gamma^\emb$ and indirectly elicits $\gamma$.
  By indirect elicitation, there exists a function $\psi^\R: \R^\emb \to \R$ such that for all $r\in\R^\emb$ and $p\in\simplex$ we have $r\in\gamma^\emb(p) \implies \psi^\R(r) \in \gamma(p)$. 
  ...
  Moreover, Theorem~\ref{thm:link-main} gives a link function $\psi^\emb : \reals^d \to \R^\emb$ such that $(L,\psi^\emb)$ is calibrated with respect to $\ell^\emb$.

	
  Consider $\psi := \psi^\R \circ \psi^\emb$ and fix $p\in\simplex$.
	For any $u\in\reals^d$, if $\psi^\emb(u) \in \gamma^\emb(p)$, then $\psi(u) = \psi^\R(\psi^\emb(u)) \in \gamma(p)$ by definition of $\psi^\emb$ and $\psi^\R$.
  ...
  \end{equation*}
  Combined with the fact that $(L,\psi^\emb)$ is calibrated with respect to $\ell^\emb$, we have
	\begin{align*}
...
In this paper, we introduce the embeddings framework to obtain convex and consistent surrogates for discrete prediction tasks, showing consistency by providing a link function construction.  
We show that a convex surrogate embedding a target loss via the introduced framework is equivalent to the surrogate and target loss having matching Bayes risks and that their ``trimmed'' forms are equal as well.
Finally, we show examples from the literature which demonstrate the broad applicability of our framework, ranging from high-confidence classification to structured prediction and others. \jessiet{Sentence on surrogate regret bounds?}

\paragraph{Open directions}
\jessie{Dimensionality, geometry of losses, more applications}
Several open directions of remain.
...
Since polyhedral surrogates have a finite trimmed loss, it is an open line of work to relate embeddings to the geometry of losses~\citep{williamson2014geometry} through superprediction sets, as these sets encapsulate loss vectors that are dominated by elements of $\trim(L)$.

Finally, while we work through applications of the framework to different target losses, we can imagine more target losses for which the embedding framework might be useful, such as multilabel learning, ranking, and (multiclass) structured prediction problems.
Understanding the application of embeddings to these settings, as well as the empirical performance of algorithms optimizing embeddings remains to demonstrate the empirical use of the framework. \jessiet{I don't like this sentence}
...
%\item \btw{Modified from a conjecture Raf thought was false. Old version in comments - J, 15 Sept 21}
...
\begin{proof}
  ...
  \btw{In fact, we must also have $\Gamma(p) = \reals^d$ by Lemma~\ref{lemma:hoffman-polyhedral}, since every $u\in\reals^d$ would have $d_\infty(u,\Gamma(p)) = 0$ and $\Gamma(p)$ is closed.}
...
\begin{proof}
  ...
  By Lemma~\ref{lemma:calibrated-eps-sep}, $\psi$ is $\epsilon$-separated for some $\epsilon>0$; let $\epsilon_{\psi} = \epsilon$.
  By Lemma~\ref{lemma:separated-constant-p}, for each $q \in \mathcal{Q}$,
