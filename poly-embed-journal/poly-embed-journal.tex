...
\begin{proof}
  ...
  $x \in \H_\Y$ by previous Claim \jessie{ref}, so we just need to show that $(x,c) \in \cap \H_{\V'} \implies (x,c) \in G$.
...
\begin{claim}
	$\Theta$ is exactly equal to $\{\Gamma_r \mid r \in \R^*\}$ \jessie{Not sure if this is it.}
\end{claim}

\begin{claim}
	For any $r \in \R$ with $v = L(r)$, we have $\Gamma_r \subset \theta(v)$. \jessie{=?}
\end{claim}
