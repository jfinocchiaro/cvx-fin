...
\item $\trimnonred(\Gamma):\simplex \toto \U_\equiv$ where $\trimnonred(\Gamma)_{[u]} := \Gamma_u$, i.e., $\trimnonred(\Gamma)(p) = \{[u] \mid u \in \Gamma(p)\}$. 
\item $\trimnonred(L):\U_\equiv\to\reals^\Y_+$ where $L([u]) = L(u)$.  \raf{But well-defined?}
\end{itemize} 
\raf{END NOTES}
...
\begin{restatable}{proposition}{embedtrim}\label{prop:embed-trim}
	...
	\begin{enumerate}\setlength{\itemsep}{0pt}
		...
		\item There is a finite minimum representative set $\U$ for $\Gamma$. \raf{$\trimcover(\Gamma)$ is a finite set (but this is by definition currently)}
