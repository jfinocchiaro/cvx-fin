...
\begin{definition}[Embedding a loss]\label{def:loss-embed}
  A loss $L:\reals^d\to\reals^\Y$ \emph{embeds} a loss $\ell:\R\to\reals^\Y$ \emph{over a set} $\Sc$ if there exists some injective embedding $\varphi:\Sc\to\reals^d$ such that
...
\begin{proof}
  ...
  \proposedadd{Form the set $\Sc := \Sc(L)$ as in Definition~\ref{cons:rep-set}.
...
\begin{definition}[Tight embedding]\label{def:tight-embedding}
	We say that a loss $L : \reals^d \to \reals_+^\Y$ \emph{tightly embeds} a discrete loss $\ell : \R \to \reals_+^\Y$ via the injection $\varphi :\R \to \reals^d$ if $L$ embeds $\ell$ and $\R$ is non-redundant.
\end{definition}

\begin{definition}[Trim]\label{def:trim}
...
\begin{proposition}\label{prop:nonredundant-iff-trim}
	Suppose $L$ elicits $\Gamma$.
	If a  set $\R$ is non-redundant for $L$ then $\{\Gamma_r \mid r \in \R\} = \trim(\Gamma)$.
\end{proposition}
\begin{proof}
	\jessie{Not an iff because you can have duplicate reports, which is consolidated by trim}
	If $\R$ is non-redundant, then every $r \in \R$ uniquely optimizes $L$ for some $p_r \in \simplex$.
	First, we have $\trim(\Gamma) \subseteq \{\Gamma_r \mid r \in \R\}$ trivially.
	Consider $r \in \R$.
	Since there is a $p_r \in \simplex$ so that$\Gamma(p_r) = \{r\}$.
	Therefore, we cannot have $\Gamma_r \subseteq \Gamma_{r'}$ for any other $r' \neq r \in \R$, and therefore $\Gamma_r \in \trim(\Gamma)$.
\end{proof}


\begin{proposition}\label{prop:min-rep-iff-trim}
	Suppose $L$ elicits $\Gamma$.
	If a  set $\R$ is a finite minimum representative set then $\{\Gamma_r \mid r \in \R\} = \trim(\Gamma)$.
\end{proposition}
\begin{proof}
	\jessie{Not an iff because you can have duplicate reports, which is consolidated by trim}
\end{proof}
...
\begin{corollary}
	$\R$ is non-redundant for $L$ if and only if $\R$ is a minimum representative set for $L$.
\end{corollary}

\begin{corollary}
	If $\R$ is a minimum representative set for $\ell$ and $L$ embeds $\ell$ over $\R$, then $L$ tightly embeds $\ell$.
\end{corollary}

\jessie{TODO: Relate to trim}
