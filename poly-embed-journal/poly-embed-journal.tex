...
\begin{proposition}\label{prop:SL-minimum}
  A loss $L: \R \to \reals^\Y_+$ has a polyhedral Bayes risk if and only if it has a finite representive set.\raft{Chengaed to ``finite representive set''}
...
If $\Gamma$ embeds $\gamma$ with map $\varphi$, then $\Gamma$ also has a finite representative set $\U := \varphi(\Sc)$ and $\{\gamma_s \mid s\in\Sc\} = \{\Gamma_u \mid u\in\U\}$.
