...
We now define an embedding, which is a special case of indirect property elicitation introduced above.
...
However, when restricting to polyhedral surrogates, embedding is necessary and sufficient for calibration, which we show in \S~\ref{sec:poly-ie-consistency}.
\jessie{Added sentence w/ forward ref to \S~\ref{sec:poly-ie-consistency}}
...
For example, the literature on minimax optimization in supervised learning settings \citep{asif2015adversarial,farnia2016minimax} poses the construction in Theorem~\ref{thm:discrete-loss-poly-embeddable} as a minimax optimization problem of the expected loss matrix over both players' strategies, which becomes a convex relaxation of the discrete optimization problem.
...
\citet{abernethy2013efficient} use the construction below as a cost function serving as a market maker in settings where the outcome space is large and running a complete market is not feasible, so cost is computer by the following scoring rule. \jessie{??}
...
\section{Consistency and Linear Regret Transfer via Separated Links \raf{revisit title? \bo{revisited...}}}
...
Recalling that calibration is equivalent to consistency for discrete targets, this shows that an embedding can always be extended to yield consistency.\botodo{added the last sentence -- I think we sometimes talk about consistency and calibration interchangeably but should be clear why}

The key idea is showing calibration equivalent to \emph{$\epsilon$-separation}, a lower bound on the distance between points in $\reals^d$ that link to optimal reports for $\ell$ and points that link to non-optimal reports.
...
\begin{definition}[Separated Link]\label{def:sep-link}
  ...
  \footnote{\citet{frongillo2021surrogate} define $\epsilon$-separation with a strict inequality $d_\infty(u,\Gamma(p)) > \epsilon$; we adopt a weak inequality as it is more convenient in examples. \raf{I felt we should write something to this effect here, but I don't love my attempt..\bo{I think this is fine}}}
...
First, a polyhedral $L$ has only a finite number of ``optimal report sets'' $\{\Gamma(p) ~:~ p \in \simplex\}$.
Second for a given $p$, the expected loss for a suboptimal point scales with the distance from the optimal set $\Gamma(p)$ at some minimum linear rate $\alpha > 0$; this is related to \emph{Hoffman constants}.
Combining with the first fact gives a universal minimum constant $\alpha$.
Combining this with $\epsilon$-separation implies that any point linking to a suboptimal report has expected loss at least $\alpha \cdot \epsilon > 0$.
On the other hand, if a link is not separated, then the same two facts imply that a sequence of suboptimal points that get arbitrarily close to an optimal report set also have expected loss arbitrarily close to optimal, which implies that the link is not calibrated.
...
\raft{I'd suggest taking the last paragraph of the proof of Theorem~\ref{thm:thickened-separated} and making that one direction of the proof of this proposition, and then using the proposition to prove the theorem. \bo{didn't do this, for now at least}}
...
\begin{proposition}
  ...
  Let $\epsilon>0$ be given, and let $\Psi$ be the link envelope from Construction~\ref{const:eps-thick-link}$(L,\ell,\epsilon,\|\cdot\|_\infty)$.
...
\begin{proof}
  ...
  By definition, $\Psi(u) = \cap_{p \in P_u} R_p$ where $R_p = \{r \in \R ~:~ \phi(r) \in \Gamma(p)\}$, the set of reports that embed into $\Gamma(p)$.
...
\raft{Looks great/close, but I got lost with the ``if and only if, then'' \bo{updated, how about now?}}
\end{proof}

\begin{corollary}
  A link $\psi$ is calibrated for a given discrete target $\ell$ and polyhedral surrogate $L$ if and only if there exists $\epsilon$ such that $\psi$ is produced by Construction \ref{const:eps-thick-link}$(L,\ell,\epsilon,\|\cdot\|_{\infty})$ for some choice of tiebreakers.
\end{corollary}
...
Finally, we refer the reader to \citet{frongillo2021surrogate} a quadratic lower bound on the rate transfer for sufficiently non-polyhedral surrogates.
\raf{Updated the above; worth a check!}
...
From here one can further remove redundant reports until arriving at a tight embedding if desired.
...
If this problem $\hat\ell$ is not the desired target problem $\ell$, we can still derive restrictions on label distributions for which $L$ is consistent for $\ell$.
...
\begin{figure}
    ...
    \raf{Let's add $p,p'$ to the figure.  Otherwise, really nice job!}
\end{minipage}
...
In particular, our results give novel surrogate regret bounds for several applications (\S~\ref{sec:lovasz-hinge},~\ref{sec:winge},~\ref{sec:top-k}).
...
As we show in \raf{converse theorem}, Construction~\ref{const:eps-thick-link} fully characterizes the set of possible calibrated link functions for a polyhedral embedding via the link envelope $\Psi$, so $\psi$ is calibrated if and only if it is contained in $\Psi$ for some $\epsilon>0$.
...
\citeauthor{ramaswamy2018consistent} provide a polyhedral surrogate $L$, which they call the \emph{binary encoded predictions (BEP)} surrogate, and link $\psi^\infty$ which are calibrated for $\ell$.
...
When juxtaposed with multiclass classification in Figure~\ref{fig:abstain-intuition-inconsistent}, one can observe that the BEP surrogate $L$ does not indirectly elicit the mode, as the level set $\Gamma_{\vec 0} = \hat \gamma_\bot$ by embedding and one can observe $\prop{\ell}_\bot \not \subseteq \mode_r$ for any $r$. 
As embedding is a special form of indirect elicitaion, we additionally know the BEP surrogate does not embed the mode.
The intuition described by Figure~\ref{fig:abstain-intuition-inconsistent} additionally gives a procedure to validate inconsistency with respect to a target task.
...
\begin{figure}
	\begin{center}
		...
		\end{minipage}\hfill
		\caption{Links $\psi^*$ (L) and $\psi^\diamond$ (M) such that $(L^f, \psi^*)$ and $\psi^\diamond$ are calibrated with respect to $\ellabs$ for all suitable $f$. Points in each region link to the embedding point contained in the region. Both are constructed via the link envelope $\Psi$ (R) from Construction~\ref{const:eps-thick-link}, which yields possible choices for calibrated links.\jessie{New, simpler, caption.  Clear enough?}\raf{Looks great.  Maybe instead of (L),(M), etc, which look a bit like function applications, we could put the actual symbols in the top left of the figures.  Also, could we shrink the $u_1,u_2$ and bullets, and make the other labels larger, basically to match Figure~\ref{fig:abstain-links}?}
			}
...
Like Figure~\ref{fig:abstain-intuition-inconsistent}, Figure~\ref{fig:ordered-partition} gives the level sets $\gamma_T =\{p \in \simplex \mid T \in \prop{\ell^\OP}(p)\}$ for each $T \in \T$ juxtaposed with the level sets of the mode.
...
\begin{table*}
	...
	\caption{
		...
		Regions outlined in black that are filled in blue and cross the dashed blue lines suggest where deciding how to construct a link $\psi$ is ambiguous, as the top-$k$ elements of the optimal report $u$ are ambiguous.  \jessie{Thoughts on this caption?  Now walking through this geometric intuition for the third time.} \raf{Let's refer back to Figure~\ref{fig:abstain-intuition-inconsistent} instead of going into the same level of detail; see the changes I made in Figure~\ref{fig:ordered-partition}.}
	}
...
In this paper, we introduce and demonstrate the applicability of the embedding framework to construct calibrated polyhedral surrogates for finite prediction tasks.
...
Moreover, given an embedding, we give a link construction and show that a surrogate embedding a target loss implies the surrogate and a link given by our construction together are \emph{calibrated} with respect to the target loss.
Moreover, we show examples from the literature which demonstrate the broad applicability of our framework. \jessiet{Sentence on surrogate regret bounds?}

\paragraph{Open directions}
\jessie{Dimensionality, geometry of losses, more applications}
Several open directions of remain.
First, \citet{finocchiaro2020embedding} introduce preliminary lower bounds on embedding dimension (e.g., the lowest $d \in \mathbb{Z}$ such that $L : \reals^d \to \reals_+^\Y$ embeds $\ell$) based on optimality conditions that they derive.
However, it is unclear if these bounds are tight or if they can be improved by leveraging information about adjacent level sets of an embedded property.
...
Since polyhedral surrogates have a finite trimmed loss, it is an open line of work to relate embeddings to the geometry of losses~\citep{williamson2014geometry} through superprediction sets, as these sets encapsulate losses that are dominated by elements of $\trim(L)$.
...
Understanding the application of embeddings to these settings, as well as the empirical performance of algorithms optimizing embeddings remains to demonstrate the empirical use of the framework.
