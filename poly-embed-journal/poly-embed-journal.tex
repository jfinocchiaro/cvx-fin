...
In particular, consider the function $g$ so that $g$ and $f$ match on $\simplex$.

Define the function $\theta(v) = \{p \in \simplex \mid \inprod{v}{p} = f(p)\}$ for any $v \in \V$, and the set $\Theta := \{\theta(v) \mid v \in \V^*\}$.
...
\begin{proof}
  ...
  \begin{align*}
    p \in \theta(v)
    &\iff \inprod{v}{p} = \min_{v' \in \V}\inprod{v'}{p} & \text{Def of $\theta$}\\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \theta(v)
  \end{align*}
  Now, we have $p \in \Gamma_r$ if and only if,
  \begin{align*}
    p \in \Gamma_r
...
\begin{proof}
  Take $v = L(r)$.
  It suffices to show there is a $v^* \in \V$ such that $\theta(v) \subseteq \theta(v^*)$.
  We know $\theta(v) = \pi(F_v) \cap \simplex$ and similarly for $\theta(v^*)$ by Claim~\ref{claim:level-set-is-projected-face}.
  Moreover, we know that there is a $v^* \in \V^* \subseteq \V$ such that $\pi(F_v) \subseteq \pi(F_{v^*})$ by Claim~\ref{claim:exists-vstar-projected-face-subset}.
  The result follows.
  % Let $v = L(r) \in \V$.
