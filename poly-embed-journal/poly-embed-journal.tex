...
In \S~\ref{sec:poly-loss-embed}, we discussed the connection between embeddings and polyhedral losses, and a natural next question is to understand if polyhedral losses are necessary to embed a discrete loss --- the answer is negative.
...
Since surrogates with polyhedral Bayes risk have finite minimum representative sets, we introduce and study the notion of a trimmed loss, and connect this to tight embeddings and non-redundancy from property elicitation.
 

\raf{Add somewhere: initially explored by~\citet{wang2020weston} under the name \emph{embedding cardinality} }
\jessie{Feels like we should mention that earlier, when minimum representative set is introduced? -- mentioned it right under the definition of representative set}
