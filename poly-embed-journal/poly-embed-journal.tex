...
\item We define $H^+_v := H^+_{([v;-1], 0)}$ for a $v \in \reals^\Y_+$, where $[v; -1]\in\reals^\Y_+\times\reals$.
\item Additionally, denote $H_y^+ := H_{([e_y; 0], 0)}^+$. % \{ (x,c) \in \reals^\Y \times \reals \mid x_y \geq 0\}$.
\item Again, we let $H_v := H_{([v;-1], 0)}$ for $v \in \reals^\Y_+$. 
...
\begin{claim}
	Claim: $G$ is $(|\Y|+1)$-dimensional.  %\raf{$g$ is nonnegative on $\reals^\Y_+$, so $G$ contains $\{(x,c) \mid x\in\reals^\Y_+, c \leq 0\}$, which is full-dim.}
\end{claim}
\begin{proof}
...
\begin{proof}
	We argue $\H_\Y \subseteq \H^*$; if there was a $y \in \Y$ such that $H_y = \{(x,c) \mid x_y \geq 0\}$ was not in $\H^*$, then we would either have some $c_1 > 0$ such that $\{(x,c) \mid x_y \geq c_1\} \in \H^*$, or we have a point $x$ such that $x_y < 0$ but $g(x) > -\infty$.
	The first cannot happen as we take $f$ to be defined (and finite) on $p = \delta_y \in \simplex$, and $f$ and $g$ match on the domain of $f$.
	Moreover, the second cannot be true by construction of $g$.	
\end{proof}
...
\begin{claim}
	For all $x\in\reals^\Y_+$, there exists $v\in\V^*$ such that $H_v$ supports $G$ at $(x,g(x))$.
\end{claim}
\begin{proof}
We can write any face $F_v = H_v \cap G = \{x \in \reals_+^{d-1} \mid \inprod{x}{v} = g(x)\}$.
Since $H_v^+$ must be a valid inequality to define a face, we know that $G \subseteq H_v^+$.
If there was no $v \in \V^*$ such that $x \in F_v$, 
Since $\H_\V = \H_\Y \cup \H_{\V^*}$, if there was no $v \in \V^*$ such that $x \in F_v$, it must be the case that $\min_{H_v \in \H_\Y} \inprod{v}{x} = \min_{y \in \Y} x_y = 0 = g(x)$, and we must have $x \not \in \reals_{++}^\Y$.

\jessie{If $G$ is not locally constant}
Since $g$ is continuous (as it is convex), and not constant \jessie{??}, there must be some $x' \in B(x,\epsilon) \cap \reals_+^\Y$ such that $g(x') > 0$ and therefore no $y \in \Y$ would define a face $F_{v^*}$ supporting $G$ at $x'$.
That $v^*$ must then be in $\V^*$, and we claim $\inprod{v^*}{x} = 0$, which would imply $F_{v^*}$ also supports $G$ at $x$.
If $\inprod{v^*}{x} > 0 = g(x)$ (recall $g$ nonnegative on $\reals_+^\Y$) then we contradict continuity of $g$ on $\reals_+^\Y$. \jessie{Not the level of rigor that we want.  Also using lots of polyhedral structure here.}
...
%  Thus, for all loss vectors such that $v = L(r)$ we have $\inprod{v}{x} = \inprod{L(r)}{x} \geq c$, and therefore $(x,c) \in H^+_v$ for all $v \in L(\R)$, and is therefore in the intersection $H_v^+$.
  ...
  &\iff (x,c) \in \cap \H_\V \\ 
  \end{align*}
  Combining the two equalities, we have $G = \cap \H$.
...
\begin{claim}
	...
	\jessie{Is this true?  You could have hinge with a really bad loss vector that is never uniquely optimal (ex: $u = 2$) and $(3,0) \in L(\R)$ but not in $L(\{-1,1\})$.}
\end{claim}
...
Note: $\theta(v) = \pi(F_v) \cap \simplex$. \raf{But haven't def $\theta$ yet}
