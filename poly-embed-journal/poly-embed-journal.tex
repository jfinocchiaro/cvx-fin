...
This framework also provides a way to verify the consistency or inconsistency of a given polyhedral surrogate, by first determining the discrete loss $\ell_{\mathrm{embed}}$ it embeds and comparing $\prop{\ell_{\mathrm{embed}}}$ to the property elicited by the proposed target $\prop{\ell}$.
...
First, for each $y\in\Y$, divide $\reals^d$ into a finite number of polyhedral regions on which $L(\cdot)_y$ is an affine function (\S~\ref{sec:lovasz-hinge}, \ref{sec:top-k}).
...
Once the embedded discrete loss is known, the behavior of the surrogate becomes more clear: what problem it is solving, and for which restrictions on conditional label distributions is it consistent for the original problem (\S~\ref{sec:winge}, \ref{sec:top-k}).
...
A key step in Construction~\ref{const:eps-thick-link} is constructing the link candidate function $\Psi$, which yields the possible values any $\epsilon$-separated link $\psi$ could map to; this in turn provides a tool to verify the calibration of a proposed link (\S~\ref{sec:abstain}, \ref{sec:lovasz-hinge}, \ref{sec:top-k}).
\raft{We should discuss this a bit in the examples where appropriate, e.g. WW, and then forward ref in parens here to the relevant subsection(s)}

\jessiet{Commented out paragraph below; feels redundant now. - Mar 1, 22}
%For many target problems, the search for calibrated surrogates has been restricted to those accommodating a particular link function, such as $k$ largest coordinates of the surrogate report for the top-$k$ problem (\S~\ref{sec:top-k}).
%In these cases, as we observe in \S~\ref{sec:calibration}, our link construction can also be used to verify the calibration of a given proposed link.
%More generally, however, our results suggest that researchers could explore surrogates beyond the link function they had in mind (\S~\ref{sec:lovasz-hinge}).
%In particular, one can instead search for surrogates that embed the correct discrete loss, and use Construction~\ref{const:eps-thick-link} to construct the link.
...
In particular, \citet{ramaswamy2018consistent} study the loss $\ell^{\alpha} : [n] \cup \{\bot\} \to \reals^\Y_+$ defined by $\ell^{\alpha}(r)_y = 0$ if $r=y$, $\alpha$ if $r = \bot$, and 1 otherwise.
...
%\end{cases}~.$
%\end{align}
The report $\bot$ corresponds to ``abstaining'' if no one label is sufficiently likely: specifically, if no $y\in\Y$ has $p_y \geq 1-\alpha$.
\citeauthor{ramaswamy2018consistent} provide a polyhedral surrogate $L_\alpha$ for $\ell^{\alpha}$ when $\alpha=1/2$.
...
As we illustrate in Figure~\ref{fig:abstain-links}(L), the link function $\psi$ proposed by \citeauthor{ramaswamy2018consistent} can be recovered from Construction~\ref{const:eps-thick-link} by choosing the norm $\|\cdot\|_\infty$ and setting $\epsilon=1/2$. 
...
\begin{equation}\label{eq:abstain-link-1}
  ...
  \end{cases}~.
\end{equation}
Construction~\ref{const:eps-thick-link} immediately gives calibration of $(L_{1/2},\psi_1)$ with respect to $\ell^\alpha$. \jessie{does it?  do we know $\epsilon$ is small enough for higher $d$?}
...
\citeauthor{ourlovaszpaper} observe that the BEP surrogate $L_{1/2}$ in eq.~\eqref{eq:abstain-surrogate} is one example of a Lov\'asz hinge introduced in \S~\ref{sec:lovasz-hinge}, and that $\psi$ in eq.~\eqref{eq:abstain-link} is an example of a calibrated link given by Construction~\ref{const:eps-thick-link} for the embedded loss.
...
\citet{ourlovaszpaper} use our embedding framework to resolve the consistency of $L^f$, showing that it is inconsistent with respect to $\ell^f$ outside of the trivial case where $f$ is modular and $\ell^f$ is weighted Hamming loss.
...
To derive $\ellabs$, they show that the set $\V = \{-1,0,1\}^k$ is representative for $L^f$, and use our Proposition~\ref{prop:representative-embeds-restriction} to show that $L^f$ embeds $\ellabs := L^f|_{\V}$.
...
Thus, interestingly, the Lov\'asz hinge is consistent with respect to the structured abstain loss $\ellabs$.

\jessie{new: added a bit about links. not sure how I feel about it yet.}
Since $L^f$ embeds $\ellabs$, Theorem~\ref{thm:link-main} implies that, for each submodular and increasing $f : 2^{[k]} \to \reals_+$, there exists a link $\psi^f$ such that $(L^f, \psi^f)$ is calibrated with respect to $\ellabs$.
Perhaps surprisingly, \citeauthor{ourlovaszpaper} find two links $\psi^*$ and $\psi^\diamond$ such that $(L^f, \psi^*)$ and $(L^f, \psi^\diamond)$ are both calibrated with respect to $\ellabs$, regardless of $f$.
These links are visualized for $k = 2$ in Figure~\ref{fig:lovasz-links}.
...
\label{sec:top-k}
In settings like object recognition and information retrieval, the top-$k$ classification problem arises in which one predicts a set $S$ of size $k$, and if the true outcome is $y$, receives loss $\elltopk(S)_y = \ones\{y\notin S\}$~\citep{lapin2015top, lapin2016loss, lapin2018analysis,yang2018consistency,berrada2018smooth,rastegari2011scalable,reddi2019stochastic}.
...
Satisfying all three desiderata simultaneously had not been observed until their work.
The surrogates that are convex and calibrated but not polyhedral often elicit the entire distribution $p \in \simplex$, and take the link to be the top $k$ elements of $p$, as noted by~\citep{lapin2016loss}, which learns more information than desired and seems to require more samples to converge.\jessie{fact check; cite rates paper?}
...
\citet{finocchiaro2022consistenttopk} apply the embedding framework to these surrogates to understand the target problems for which they are consistent.
%$\psi_3$ from~\citet[Table 1]{yang2018consistency} elicits a similar property to $\prop{\ell^\OP}$ shown in Figure~\ref{fig:ordered-partition}, but it is proposed as a surrogate for a different task. \jessiet{Do we even want this sentence?}
...
None of the studied surrogates embed top-$k$ but use the properties elicited by the embedded losses to characterize (tighter\jessie{not sure how to say this.  better bounds than YK gave, which was just the abstain cell for all the surrogates}) sufficient assumptions on the data distribution in order be calibrated with respect to top-$k$, fixing the link to be the canonical argmax link $\psi^k$.
\jessie{do we want to drop the table here or nah?}

Moreover, \citeauthor{finocchiaro2022consistenttopk} uses Theorem~\ref{thm:discrete-loss-poly-embeddable} to derive the first calibrated polyhedral surrogate for $\elltopk$, given in eq.~\eqref{eq:topk-embedding}.
Here $u_{[i]}$ denotes the $i^{th}$ largest element of $u \in \reals^n$.
\begin{align}\label{eq:topk-embedding}
L^k(u)_y &= \max \left(u_{[1]}, \max_{m \in \{k+1, \ldots, m\}} \left[ 1 - \frac k m + \frac 1 m \sum_{i=1}^m u_{[i]}\right] \right)- u_y~.
\end{align}
While Theorem~\ref{thm:link-main} guarantees the existence of a link function from $L^k$ to $\elltopk$, we ideally would like to use the intuitive argmax link function 
$\psi^k(u)$ which returns the $k$ largest elements of $u$ (breaking ties arbitrarily).
\citeauthor{finocchiaro2022consistenttopk} Theorem 4.4 confirms that indeed $(L^k,\psi^k)$ is calibrated with respect to $\elltopk$ for this link $\psi^k$, by showing that $\psi^k$ is an $\epsilon$-separated link for $\elltopk$ with any $\epsilon < \frac 1 {2n}$ via the construction of the link envelope $\Psi$ in Construction~\ref{const:eps-thick-link}.
...
\end{equation}
While it was later shown to be inconsistent with respect to 0-1 loss~\citep{tewari2007consistency,liu2007fisher}, \citet{dogan2016unified} nonetheless advocate for the Weston-Watkins SVM to be the default for multiclass prediction problems. \jessie{do we want the ponit about dogan et al or do we not care?}
...
\begin{align*}
\T = \{ (T_0,\ldots,T_k) \mid k \geq 1, \emptyset = T_0 \subsetneq T_1 \subsetneq \ldots \subsetneq T_k = [n]\}~.
\end{align*}
\jessie{do we want $k$ or something else, given the previous section?}

The ordered partition target loss $\ell^{\OP} : \T \to \reals^\Y_+$ is then defined by
\begin{align*}
\ell^\OP(T)_y &= \sum_{i=1}^{k} \left(|T_{i}| \cdot \Ind{y \not \in T_{i-1}} \right) -1~.~
\end{align*}
...
\citeauthor{wang2020weston} then apply Theorem~\ref{thm:link-main} to conclude that there exists a link function $\psi$ such that $(L_{WW},\psi)$ is calibrated with respect to $\ell^\OP$; however, they do not construct the link. \jessie{want to omit this detail?}
Figure~\ref{fig:ordered-partition} gives the cells $\{p \in \simplex \mid T \in \prop{L_{WW}}(p)\}$ for each $T \in \T$, outlined in solid black.
These cells are juxtaposed with the cells $\{p \in \simplex \mid T \in \mode(p)\}$, outlined in dashed blue, for which $L_{WW}$ is proposed as a surrogate.
\jessie{do we want more detail or save it for the caption/the top-$k$ seciton above?}
\raf{Jessie, could you add something here about the figure and dotted lines, which show conditions under which WW is consistent for 0-1? \jessie{\#burn ?}}
...
Thus, for any halfspace $H^+$, we have that $H^+$ is one of the two halfspaces corresponding to the hyperplane $\partial H^+$.
...
\begin{definition}[Face, facet]
  ...
  A (non-trivial)\jessie{word choice} \emph{face} $F$ of the polytope $P$ is any set of the form
  \begin{equation*}
    F = P \cap H~,
  \end{equation*}
...
\cite{gallier2008notes} shows that every polyhedron $P$ has a unique and irredundant halfspace representation $\H^*$, and each $H^+ \in \H^*$ generates a facet of $P$.
...
\begin{proof}
	...
	We claim that $P$ must have some irredundant representation $P = \H^*$ for a finite set $\H^*$.
...
Given any $v \in \V \subseteq \reals^\Y_+$, define $H^+_v := H^+_{(v, -1)} = \{(x, c) \in \reals^{|\Y|} \times \reals \mid \inprod{v}{x} = c\}$.
...
Throughout, we will work with a function $g_\V$ generated by $\V$ of the following form.
...
We first observe that the region generated by the intersection of the $\H_y$ halfspaces restricts the hypograph of any $g_\V$ to the nonnegative orthant.
...
We proceed with some observations about facets and dimension of $\hyp(g_\V)$.
...
\begin{lemma}\label{lem:G-unique-facets-Hstar}
  ...
  Moreover, for each $H^+ \in\H^*$ and $H$ such that $H = \partial H^+$, the face $F = G \cap H$ is a facet.
\end{lemma}
\begin{proof}
  Since $G$ is full-dimensional by Lemma~\ref{lem:G-full-dimensional}, this follows immediately from Theorem~\ref{thm:polyhedron-uniquely-gen-facets}(i).
\end{proof}
...
\begin{lemma}\label{lem:HY-subset-Hstar}
  Given a finite set $\V \subset \reals^\Y_+$, consider the unique finite set $\H^*$ such that $\hyp(g_\V) = \cap \H^*$ and $\H^* \subseteq (\H_\V \cup \H_\Y)$.
  $\H_\Y \subseteq \H^*$.
\end{lemma}
\begin{proof}
  ...
  Moreover, the second cannot be true by construction of $g_\V$ including the $0-\infty$ indicator on $\reals^\Y_+$.	
\end{proof}

\begin{corollary}\label{cor:unique-set-loss-vectors-defining-facets}
  Suppose we are given a finite set $\V \subset \reals^\Y_+$, and consider the unique irredundant set $\H^*$ such that $\hyp(g_\V) = \cap \H^*$.
...
%
  ...
  Moreover, $H_v \in \H_{\V^*} \subseteq \H^*$ generates the facet $F_v$ of $\hyp(g_\V)$.
...
\begin{corollary}\label{cor:anything-gen-G-subset-Hstar}
  Let $\V \subset \reals^\Y_+$ be a finite set such that $\H := \H_\Y \cup \H_{\V}$ satisfies $\hyp(g_{\V}) = \cap \H$, and take $\V^*$ the unique set such that irredundant (wrt. $\hyp(g_\V)$) set $\H^* = \H_\Y \cup \H_{\V^*}$.  
  Then $\V^* \subseteq \V$.
\end{corollary}
\begin{proof}
...
\begin{assumption}\label{assum:V-star-exists-finite}
	Suppose we are given a finite set $\V \subset \reals^\Y_+$, and consider the unique irredundant set $\H^*$ such that $\hyp(g_\V) = \cap \H^*$.
	Moreover, $\V^* \subseteq \V$ is the unique finite set such that $\H^* = \H_\Y \cup \H_{\V^*}$ is irredundant.
\end{assumption}
...
\begin{lemma}\label{claim:g-gen-by-Vstar}
	Given a finite set $\V \subset \reals^\Y_+$, consider $\V^*$ as in Assumption~\ref{assum:V-star-exists-finite}.
...
We now define $\V = L(\R) \subseteq \reals^\Y_+$ and $\H_{\V} = \{H_v^+ \mid v\in\V\}$, and consider $g_\V$.  
Unlike \S~\ref{appsubsec:phase1}, these could be infinite sets.  
Now let $\H = \H_\Y \cup \H_{\V}$; again, this may be infinitely generated.
\begin{claim}
	Given a minimizable $L : \R \to \reals^\Y_+$ with polyhedral extended risk $\risk L_+$, consider $\V := L(\R)$ and $\H = \H_\Y \cup \H_{\V}$.
	Then $\hyp(g_\V) = \cap\H$.
\end{claim}
\begin{proof}
	...
	&\iff \risk L_+(x) \geq c & \text{$g_\V=\risk L_+$ for $x \in \reals^\Y_+$}\\
...
\jessie{I think we want to spell a little more out here before jumping to this corollary?}
\begin{corollary}
	Given minimizable $L : \R \to \reals^\Y_+$ such that $\risk L_+$ is polyhedral, there exists a (unique) finite $\V^* \subseteq \V := L(\R)$ such that \jessie{Check} $g_\V = g_{\V^*}$ and $\hyp(g_\V) = \hyp(g_{\V^*}) = \cap_{v \in \V^*} H^+_v \bigcap \cap_{y \in \Y} H^+_y = \cap(\H_{\V^*} \cup \H_\Y)$ is irredundant.
\end{corollary}

This paves the way for our second assumption, which parallels Assumption~\ref{assum:V-star-exists-finite}, but is contextualized when given a minimizable loss with polyhedral extended risk.

\jessie{Which parts of these two do we want? another assumption commented out.  I think current one is more than sufficient}
\begin{assumption}\label{assum:V-star-exists-infinite}
	$L : \R \to \reals^\Y_+$ is a minimizable loss function with $\V = L(\R)$.
	Moreover, $\risk \L_+ = g_\V$ is polyhedral.
	Finally, $\V^* \subseteq \V$ is a (unique) finite irredundant set such that $g_\V = g_{\V^*}$ and $\hyp(g_\V) = \hyp(g_{\V^*}) = \cap_{v \in \V^*} H^+_v \bigcap \cap_{y \in \Y} H^+_y = \cap(\H_{\V^*} \cup \H_\Y)$ is irredundant.
\end{assumption}

%\begin{assumption}\label{assum:V-star-exists-infinite-2}
%	Given $\V \subseteq \reals^\Y_+$ such that $\hyp(g_\V)$ is a polyhedron, and $\V^* \subseteq \V$ is the unique finite set such that $\hyp(g_\V) = \hyp(g_{\V^*})$.
%	Moreover, $\H^* = \cap(\H_{\V^*} \cup \H_\Y)$ is irredundant.
%\end{assumption}	



%\raf{Need to use minimizable}
If follows that for $\V = L(\R)$ with $\risk L_+$ polyhedral, that $\hyp(g_\V)$ is a polyhedron, and $(d+1)$-dimensional as $\{(x,c) \mid x \in \reals^\Y_+, c \leq 0\} \subseteq \hyp(g_\V)$.
...
Throughout this section, suppose that we are given a set $\V$ such that $\hyp(g_\V)$ is a polyhedron. 
Moreover, we denote the unique irredundant set $\V^* \subseteq \V$ such that $g_\V = g_{\V^*}$.
This is often established by Assumptions~\ref{assum:V-star-exists-finite} and \ref{assum:V-star-exists-infinite}.
We now consider projections from $\reals^d = \reals^\Y_+ \times \reals$ onto the positive orthant $\reals^\Y_+$.

\begin{claim}\label{claim:vstar-supporting-G}
	Given $\V$ and $\V^*$ satisfying Assumption~\ref{assum:V-star-exists-finite} or $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, for all $x\in\reals^\Y_+$, there exists $v^*\in\V^*$ such that $H_{v^*}$ supports $\hyp(g_\V)$ at $(x,g_\V(x))$.
\end{claim}
\begin{proof}
	...
	%
	By assumption, we have $g_\V(x) = g_{\V^*}(x) = \inf_{v \in \V^*}\inprod{v}{x} = \min_{v \in \V^*}\inprod{v}{x}$ for $x \in \reals^\Y_+$.
	In particular, consider the normal $v^* \in \argmin_{v \in \V^*}\inprod{v}{x}$; we claim that the hyperplane $H_{v^*}$ such that $H_{v^*} = \partial H^+_{v^*}$ supports $\hyp(g_\V)$ at $(x,g(x))$.
	...
	By definition of $g_\V$, we have $g_\V(x) = \inprod{v^*}{x}$, so $(x,g_\V(x)) \in H_{v^*}$.
...
\begin{corollary}\label{cor:projected-Vstar-faces-cover-pos-orthant}
	Consider $\V$ and $\V^*$ satisfying Assumption~\ref{assum:V-star-exists-finite} or $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}.
	For each $v \in \V^*$, let $F_v = H_v \cap \hyp(g_\V^*)$. \jessiet{redundant?}
...
\begin{claim}\label{claim:pi-preserves-dim}
	Consider $\V$ and $\V^*$ satisfying Assumption~\ref{assum:V-star-exists-finite} or $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}. %, let $F_v = H_v \cap \hyp(g_\V)$.
...
Given $\V$ and $\V^*$ satisfying Assumption~\ref{assum:V-star-exists-finite} or $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite},
	Fix any $x \in \reals^\Y_+$.
	For any $v \in \V$, the following are equivalent: 
...
\end{lemma}
\begin{proof}
	For $v \in \V$, define $F_v := F_v^{\hyp(g_\V)}$.
	\begin{align*}
...
\end{claim}
\begin{proof}
	\jessiet{Revisit... not tracking, but that could just be a slow morning on my end.}
	For any $v \in \V$, define $F_v := H_v \cap \hyp(g_\V)$.
	We would like to show
	$\cup_{v\in\V'} \pi(F_v) = \reals^\Y_+ \iff \cap(\H_\Y \cup \H_{\V'}) = \hyp(g_\V) := G$, then the result follows by a corollary of the assumption. %apply Corollary~\ref{cor:anything-gen-G-subset-Hstar}.
	...
	$(x,c) \in \cap \H_\Y$ for all $c \in \reals$ by Lemma~\ref{lem:x-nonneg-orthant-iff-intersection-HY}, so it is left to show that $(x,c) \in \cap \H_{\V'}$, which yields the desired result.
...
Taking $\V, \V^*$ as in Assumption~\ref{assum:V-star-exists-finite} or $L,\V,\V^*$ as in Assumption~\ref{assum:V-star-exists-infinite}, when taking $F_{v^*} = H_{v^*} \cap \hyp(g_\V)$ for $v^* \in \V^*$, the projection $\pi$ preserves full-dimensionality.
\begin{claim}\label{claim:projected-Vstar-faces-full-dim}
	Given $\V$ and $\V^*$ satisfying Assumption~\ref{assum:V-star-exists-finite} or $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, define $F_v := F^{\hyp(g_\V)}_v = H_v \cap \hyp(g_\V)$.
...
\end{claim}
\begin{proof}
	By Corollary~\ref{cor:unique-set-loss-vectors-defining-facets} \jessie{careful: corollary only state if $\V$ finite}, $F_v$ is a facet of $\hyp(g_\V)$ in $\reals^d_+$, meaning it is $(d - 1)$-dimensional.
...
Given $\V$ and $\V^*$ satisfying Assumption~\ref{assum:V-star-exists-finite} or $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, denote $\Lambda := \{\pi(F_v) \mid v \in \V^*\}$\jessie{To reduce notation, let's get rid of $\Lambda$?} as the set of projected facets generated by $\V^*$.
...
\begin{claim}\label{claim:Vprime-projected-faces-cover-iffprojected-faces-subsets}
	...
	For $\V' \subseteq \V$, we have $\cup_{v \in \V'} \pi(F_v) = \reals_+^\Y \iff \Lambda \subseteq \{ \pi(F_v) \mid v \in \V'\}$.
...
\begin{proof}
	...
	Explicitly, for all $v \in \V^*$ we also have $v \in \V'$, so, $\pi(F_v) \in \{\pi(F_v) \mid v \in \V^*\} \cap \{\pi(F_v) \mid v \in \V'\} = \{\pi(F_v) \mid v \in \V^*\}$.
...
Given $L, \V := L(\R)$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, consider $v = L(r)$ for any $r \in \R$.
As $H_{v}^+ \in \H$, it supports $\hyp(g_\V)$ or it is redundant in $\H$ (or both).

\begin{claim}\label{claim:faces-contained-in-facets}
	Given $L, \V := L(\R)$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, if some $H_v \in \H$ supports $\hyp(g_\V)$, then $F := H_{v} \cap \hyp(g_\V)$ is a nonempty face of $\hyp(g_\V)$, and thus a subset of a facet. 
\end{claim}
\begin{proof}
	Since $H_v$ supports $\hyp(g_\V)$, we know that $\hyp(g_\V) \subseteq H_v^+$ and $F$ is not empty.
	Moreover, $H^+_v$ is valid, and we have $F = H_{(v,-1)} \cap \hyp(g_\V)$ is a face of $\hyp(g_\V)$ by definition.
	
	% subset of facet
	Moreover, the faces of $\hyp(g_\V)$ are all convex polyhedra.
...
\end{claim}
\begin{proof}
	As $\hyp(g_{\V^*})$ is polyhedral, each of its faces are convex polyhedra, and is also a face of some facet of $\hyp(g_{\V^*})$; these facets are defined by $\V^*$ and $\Y$ via Corollary~\ref{cor:unique-set-loss-vectors-defining-facets}.
	...
	Recall from the definition of a face that $F_v = \hyp(g_\V) \cap H_{(v,-1)}$ and $F_y = \hyp(g_\V) \cap H_{(e_y, 0)}$.
...
\begin{corollary}\label{cor:projected-faces-subset-projected-facet}
	Consider $\V$ and $\V^*$ satisfying Assumption~\ref{assum:V-star-exists-finite} or $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, and $F_v := H_v \cap \hyp(g_\V)$.
...
\begin{corollary}\label{cor:exists-vstar-projected-face-subset}
	Consider $\V$ and $\V^*$ satisfying Assumption~\ref{assum:V-star-exists-finite} or $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}.
	For $v \in \V$, let $F_v := H_v \cap \hyp(g_\V)$/
...
\begin{lemma}\label{cor:f-matches-g-on-simplex}
  Consider $\V$ satisfying Assumption~\ref{assum:V-star-exists-finite} or  Assumption~\ref{assum:V-star-exists-infinite}.
...
\begin{claim}\label{claim:theta-is-pi-cap-simplex}
  ...
  For all $v \in \V$, denote the face $F_v := H_v \cap \hyp(g_\V)$.
...
If a minimizable $L$ has polyhedral $\risk L_+$ with $\V := L(\R)$, and $\Gamma := \prop{L}$, we can define the \emph{extended level set} $\bar \Gamma_r := \{x \in \reals^\Y_+ \mid \inprod{L(r)}{x} = \risk L_+(x)\}$.

\begin{lemma}\label{lem:levelset-to-extended-levelset}
	Consider a minimizable $L: \R \to\reals^\Y_+$ satisfying Assumption~\ref{assum:gV-andfV}.
...
\begin{lemma}\label{lem:extended-levelset-equals-projected-face}
	...
	For any $v \in \V$, define the face $F_v := H_v \cap \hyp(g_\V)$.
...
\begin{claim}\label{claim:projected-faces-cover-RY-iff-representative}
	Consider $L : \R \to \reals^\Y_+, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}.
	For any $v \in \V$, denote the face $F_v := H_v \cap \hyp(g_\V)$.
...
\begin{corollary}
	Consider $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, and $\R^*$ such that $\V^* = L(\R^*)$.
	$\Theta = \{\Gamma_r \mid r \in \R^*\}$.
\end{corollary}
Observe that $\R^*$ exists by Corollary~\ref{cor:finite-rep-set}.
