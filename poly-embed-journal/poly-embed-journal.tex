...
Since $G$ is a full-dimensional polyhedron, it has a $H$-rep $H'$ uniquely determined by the facets of $G$ by \jessie{Ziegler Theorem 2.15}.
...
Moreover, $F_v := H_v \cap G$ is a facet of $G$ for each $v\in\V$. \jessie{by analog of Ziegler Theorem 2.15; more directly by Gallier notes \url{https://arxiv.org/pdf/0805.0292.pdf} Prop 4.5}
...
Now for the other direction of subset inclusion, we can take any $x \in \reals_+^\Y$ and show there is some $v \in H'$ such that $\inprod{v}{x} = g(x) < \infty$.
Recall that $\inprod{v}{x} = \infty$ for all $v \in H' \setminus \V$\jessie{nah not true}, but is finite for any $v \in \V$, so we must have $\cup_{v \in \V}\pi(F_v) = \reals_+^\Y$.
Note that we can also take $\theta(v) = F_v \cap \simplex$.
\jessie{I don't like this, but getting out of paralysis}


\jessie{I think we can delete everything under this}
...
To see this, observe that any $F_v = H_v \cap G = \{(x,c) \mid \inprod{x}{v} \geq c\} \cap \reals^\Y_+$, so $\pi(F_v) = \{x \mid \inprod{x}{v} \geq c\}$.
Therefore, $\cup_{v\in\V} \pi(F_v) \subseteq \reals^\Y_+$.
