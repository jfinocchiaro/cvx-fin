...
\begin{proof}
	...
	Now for $p \not \in \int{\simplex}$, consider $\Y'\subsetneq \Y$, $\Y'\neq\emptyset$.
...
\end{theorem}
 \begin{proof}
 	\jessie{Lemma 4 is redundant with this proof, and we don't really need the proof.  We have $L$ polyhedral $\implies \risk{L}$ polyhedral (L4) $\iff$ $L$ embeds a discrete loss (P2).}
	The projection of the epigraph of $\risk{L}$ onto $\simplex$ forms a power diagram by Theorem~\ref{thm:aurenhammer} with finitely many cells $C_1,\ldots,C_k$, which we can index by $\R := \{1,\ldots,k\}$.
	Defining the property $\gamma:\simplex\toto\R$ by $\gamma_r = C_r$ for $r\in\R$, we see that the same construction gives us points $u_r \in\reals^d$ such that $u_r \in \Gamma(p) \iff r \in \gamma(p)$.
...
\begin{proof}
	...
	Now, we claim that $L$ embeds $\ell$, and by Proposition~\ref{prop:embed-bayes-risks}, we can simply show $\risk{L} = \risk{\ell}$.
