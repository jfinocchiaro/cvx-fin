...
Since $G$ is full-dimensional, all facets of $G$ are uniquely determined by some $H^+ \in \H^*$.
...
\begin{corollary}
  Let $\V \subseteq \reals^\Y_+$ be a finite set such that $\H = \H_\Y \cup \H_\V$ satisfies $P = \cap \H$.  Then $\V^* \subseteq \V$.
\end{corollary}
\begin{proof}
...
% . REP <=> V* subset L(R')
% . REP <=> Theta subset {Gamma_r | r in R'}
% . Theta == Full-dim
% . Lev sets subset theta
...
\begin{proof}
	...
	The first subset inclusion follows by construction of $\H_{\V^*}$, and the second follows from \jessie{cite} as $\H^*$ is the unique minimum halfspace representation of $G$, and is finite.
...
\end{claim}
\begin{proof}
	By assumption, $G$ is nowhere locally constant, meaning $([\vec 0; -1], b) \not \in \V^*$ for any $b \in \reals$.
	Therefore, the dimension of $\pi(F_v) = \pi(H_v \cap G) = \pi(\{(x,c) \in G \mid \inprod{v}{x} = c, c = g(x) \}) = \{x \in \reals^d_+ \mid \inprod{v}{x} = c\}$.
	Let $A := \{(x,c) \in G \mid \inprod{v}{x} = c, c = g(x) \}$
	If $\dim(A) = k$, then there is a set of $k\leq d$ vectors $\{z_i\}_{i=1}^k$ forming a basis for $\affhull(A)$.
	In particular, \jessie{....}
	 
	\jessie{TODO}
	Facts we can potentially use:
	$F_v = \H_v \cap G \neq \emptyset$ by assumption.
	$\dim(F_v) = \dim(\affhull(F_v))$
	$\dim(\pi(F_v)) = \dim(F_v) - 1$.
	...
	\jessie{Meeting notes:} just need to argue that you never needed $c$ (i.e., $c$ was always just $g(x)$).  Maybe: take an affine basis, project, and show it's an affine basis. Given $\Y+1$ affinely independent vectors $z_i$ in $F_v$, so we get $|\Y| = 1$ affinely independent vectors.  Claim: $\{\pi(z_i)\}$ are affinely independent.
	Let $a_1, \ldots, a_{\Y+1}$ sum to $0$, such that $\sum_i a_i \pi(z_i) = 0$.
	Want to conclude $a_i = 0$ for all $i$.
	
	Observe $z_i = (x_i, \inprod{v}{x_i})$ for all $i$ since $z_i \in F_v$, and therefore in $H_v$.
	So $0 = \sim_i a_i \pi(z_i) = \sum_i a_i x_i$.
	
	Moreover, the sum $\sum_i a_i z_i = \sum_i a_i (x_i, \inprod{v}{x_i}) = (\sum_i a_i x_i, \inprod{v}{\sum_i a_i x_i}) = (0,0) = \vec 0$.
	Thus, since the set $\{z_i\}$ is affinely independent, we have $a_i = 0$ for all $i$.
...
\begin{claim}
	...
	\jessie{I don't think this is true as stated.  I think this is true for non-facets, but this implies that any $H_y \in \H_\Y$ doesn't support $G$}
\end{claim}
\begin{proof}
Since every face of a convex polyhedron is a convex polytope, any $F_v$ is 
\end{proof}
...
\begin{claim}
	We may write $f(p) = \min_{v\in\V} \inprod{p}{v}$ for some finite set $\V \subseteq \reals^\Y_+$ \raf{show you can do this using the $h$ and $\delta$ representation from Rockafellar -- argue that because $p\in\simplex$ we can write $\inprod{p}{b_i}-\beta_i = \inprod{p}{b_i-\beta_i\ones}$.}
\end{claim}
\begin{proof}
