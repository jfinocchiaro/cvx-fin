...
\raft{We got some feedback from Anish and others that the way we write generic losses was confusing.  I tried to rewrite to emphasize that we use $L$ for generic losses, including surrogates, and $\ell$ for discrete losses, where we assume $\R$ is a finite set.  That way the idea that $\R$ is always finite is perhaps dispelled.\jessie{I like this re-write}}
