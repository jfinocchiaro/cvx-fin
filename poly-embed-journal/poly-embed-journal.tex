...
As $\risk L_+$ is the $1$-homogeneous extension of $\risk L$, we observe $\risk L(x)_+ = \risk L(\bar x)$ for all $x \in \reals^\Y_+$.
Therefore, we have $\risk L_+(x) = \risk L(\bar x) = \min_{(b, \beta) \in \B} \inprod{\bar x}{b - \ones \beta}$, which is polyhedral as the pointwise min over a finite set of linear functions.
\raft{You have to connect the display to the conclusion; one way is to write out what $\risk L_+$ is, but you have to justify it either way.}
\end{proof}
...
Moreover, there must be a finite set $\R^*$ such that $\V^* = L(\R^*)$ by construction. %, which is representative as $\risk L = \min_{v \in \V^*}\inprod{p}{v} - \delta\{p \mid \simplex\} = \min_{v \in L(\R^*)}\inprod{p}{v} - \delta\{p \mid \simplex\}$.
For all $p \in \simplex$, there is a $v \in \V^* = L(\R^*)$ and $r \in \R^*$ such that $v = \risk L(p) = L(r)$ and therefore $r \in \prop{L}(p)$ as $L$ is minimizable.
As this is true for all $p \in \simplex$, $\R^*$ is representative for $L$. \raft{``generates the Bayes risk'' not defined; need to slow down here I think and find the rigorous statement among the many things you proved! \jessie{Had a feeling this comment was coming when I wrote it.  :grimace: burn if okay now}}.
