...
\citet{gallier2008notes} shows that every polyhedron $P$ has a unique and irredundant halfspace representation $\H^*$, and each $H^+ \in \H^*$ generates a facet of $P$.
...
For now, we consider some $\V \subset \reals_+^\Y$.
...
To start, we will assume $\V \subset \reals^\Y_+$ is finite.
...
\begin{lemma}\label{lem:G-full-dimensional}
  Given a finite, nonempty set $\V \subset \reals^d$, $\hyp(g_\V)$ is $d$-dimensional.  %\raf{$g$ is nonnegative on $\reals^\Y_+$, so $G$ contains $\{(x,c) \mid x\in\reals^\Y_+, c \leq 0\}$, which is full-dim.} 
\end{lemma}
\begin{proof}
  Since $g_\V$ is nonnegative on $\reals_+^\Y$, $\hyp(g_\V)$ therefore contains $\{(x,c) \mid x\in\reals^\Y_+, c \leq 0\}$, which is $(|\Y| + 1)$-dimensional.
\end{proof}

This result allows us to apply Theorem~\ref{thm:polyhedron-uniquely-gen-facets}, which yields a unique set of halfspaces $\H^*$ generating $\hyp(g_\V)$.
...
\end{lemma}
\begin{proof}
  Since $\hyp(g_\V)$ is full-dimensional by Lemma~\ref{lem:G-full-dimensional}, this follows immediately from Theorem~\ref{thm:polyhedron-uniquely-gen-facets}(i).
\end{proof}
...
\begin{corollary}\label{cor:anything-gen-G-subset-Hstar}
  Let $\V \subset \reals^\Y_+$ be a finite set, and $\V^*$ the unique finite set from Corollary~\ref{cor:unique-set-loss-vectors-defining-facets}. %such that $\H := \H_\Y \cup \H_{\V}$ satisfies $\hyp(g_{\V}) = \cap \H$, and take $\V^*$ the unique set such that irredundant (wrt. $\hyp(g_\V)$) set $\H^* = \H_\Y \cup \H_{\V^*}$.  
  Then $\V^* \subseteq \V$.
\end{corollary}
\begin{proof}
...
Now suppose $L : \R \to \reals^\Y_+$ is a minimizable loss function.
...
Unlike \S~\ref{appsubsec:phase1}, $\V$ and $\H_\V$ could be infinite sets.  
...
\begin{claim}\label{claim:gV-equals-riskL}
	...
	Then $\hyp(g_\V) = \cap\H$.
	Moreover, there is a finite $\V' \subseteq \V$ such that $\hyp(g_\V) = \cap (\H_{\V'} \cup \H_\Y) = \hyp(g_{\V'})$ and $g_\V = g_{\V'}$. 
\end{claim}
\begin{proof}
	...
	&\iff (x,c) \in H^+_{v} \,\, \forall v \in \V & \text{By definition of each halfspace}\\
	...
	The existence of a finite $\V' \subseteq \V$ follows as $\risk L_+$ polyhedral implies $\hyp(\risk L_+)$ is a polyhedron, which has a finite representation by definition.
	Finally, $\hyp(\risk L_+) = \hyp(g_\V) = \hyp(g_{\V'})$ if and only if $\risk L_+ = g_\V = g_{\V'}$. \jessiet{Omitting details on why $\V' \subseteq \V$.}
\end{proof}
...
\begin{assumption}\label{assum:V-star-exists-infinite}
	...
	%$\V' \subseteq \V$ is a finite set such that $g_\V = g_{\V'}$.
...
Moreover, we denote the unique irredundant finite set $\V^* \subseteq \V$ such that $g_\V = g_{\V^*}$.
...
This projected faces generated by $\V^*$ cover the nonnegative orthant.
...
Since we preserve the dimension of these projected spaces, we can now study equivalence of projected faces of the hypograph and regions of support of $g_\V$ for any $v \in \V$.
...
\begin{claim}\label{claim:Vprime-projected-faces-cover-iffprojected-faces-subsets}
	...
	For $\V' \subseteq \V$, we have $\cup_{v \in \V'} \pi(F_v) = \reals_+^\Y \iff  \{ \pi(F_v) \mid v \in \V^*\} \subseteq \{ \pi(F_v) \mid v \in \V'\}$.
