...
\begin{definition}[Positive halfspace]
	For any $v\in\reals^\Y_+$, let $H_{v}^+ := \{ (x,c) \in \reals^\Y \times \reals \mid \inprod{x}{v} - c \geq 0\}$ be the \emph{linear positive halfspace}, and $H_{(w,b)}^+ := \{ x \in \reals^{\Y+1} \mid \inprod{x}{w} - b \geq 0\}$ be the \emph{affine positive halfspace}
\end{definition}
Abusing notation, we sometimes let $H_y^+ := \{ (x,c) \in \reals^\Y \times \reals \mid x_y \geq 0\}$.
\begin{definition}[Hyperplane]
	For any $v\in\reals^\Y_+$, let $H_v := \{ (x,c) \in \reals^\Y \times \reals \mid \inprod{x}{v} - c = 0\}$ be the hyperplane generated by $v$.
\end{definition} 
\begin{definition}[$\H$-polyhedron\jessie{Ziegler page 28}]
	An \emph{$\H$-polyhedron} $P$ describes an intersection of closed halfspaces presented in the form
	\begin{equation*}
...
\begin{definition}[Face, facet]
	...
	A linear inequality $\inprod{v}{x} \geq c$ is \emph{valid} for $P$ if it is satisfied for all $x \in P$.
	A \emph{face} of the polytope $P$ is any set of the form
	\begin{equation*}
	F = P \cap \{x \in \reals^d \mid \inprod{v}{x} = c\}~.~
	\end{equation*}
...
\begin{proof}
	...
	Therefore $(x,c) \in H^+_v$ for all $v \in \hat \V$ and in $H^+_y$ for all $y \in \Y$, so $(x,c) \in \cap \H$.
\end{proof}
\end{itemize}
...
\item \raf{This is the important general polyhedra result to cite/state.  Note: we'll have to work half-spaces of the form $H_{(w,\beta)}^+ = \{ z \in \reals^{n+1} \mid \inprod{z}{w} - \beta \geq 0\}$.}  Since $G$ is a full-dimensional polyhedron, with a unique H-rep by [cite].
  Hence, there is some unique $\H' \subseteq \H$ such that $G = \cap \H'$. \jessie{$\H \subseteq \H$ since $G = \cap \H' = \cap \H$; something about smallest cardinality here?}
  Moreover, each $H\in\H'$ defines a facet \raf{make formal}
  \begin{proof}
	\jessie{Incomplete}
...
\item Claim: $F_v$ is a facet of $G$ for each $v\in\V$.  \raf{Unpacking ``defines a facet''}
\begin{proof}
...
\item Claim: For all $x\in\reals^\Y_+$, there exists $v\in\V$ such that $H_v$ supports $G$ at $(x,g(x))$.
\item Corollary: $\cup_{v\in\V} \pi(F_v) = \reals^\Y_+$. \jessie{Not sure how to show this, but intuitively, this is saying that the projection of the facets union to $\reals^\Y_+$.}
