...
The key idea is to show that calibration is equivalent to a geometric feature of a link function $\psi$, which we call \emph{separation}.
Recall that for indirect elicitation, any point $u \in \Gamma(p)$ must link to a point $\psi(u) \in \gamma(p)$.
(In terms of losses, $u$ minimizing expected $L$-loss implies that $\psi(u)$ minimizes expected $\ell$-loss, with respect to $p$.)
The idea of separation is that points $u'$ in the neighborhood of $u$ must also link to $\psi(u)$.
...
The following construction produces one.
Below, we sketch an intuition for Construction \ref{const:eps-thick-link} and for the proof of Theorem \ref{thm:thickened-separated} together.
...
Recall that the goal of surrogate regret minimization is to learn a hypothesis $h$ that minimizes expected surrogate loss, then output the hypothesis $\psi \circ h$ that (hopefully) minimizes expected target loss.
Consistency is a minimal requirement: it implies that when surrogate regret\footnote{Regret in this context is the difference between the expected loss of a hypothesis and the expected loss of the Bayes optimal hypothesis that minimizes expected loss. We refer the reader to \cite{frongillo2021surrogate} for a formal definition.} of $h$ converges to zero, so does target regret of $\psi \circ h$.
A natural question is whether \emph{fast} convergence in surrogate regret implies fast convergence in target regret.
The recent paper \cite{frongillo2021surrogate} shows that, for polyhedral surrogates, this is always the case.
...
% [Raf:] Then Theorem 5 (constructive linear regret bound) from the rates paper, which essentially depends only on Lemmas 6 and 7 anyway!
...
Note that the construction given above in Construction~\ref{const:eps-thick-link} is not necessarily computationally efficient as the number of labels $n$ grows.
