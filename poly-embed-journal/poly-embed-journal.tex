...
Given feature space $\X$, \jessie{and any?}fix a distribution $D \in \Delta(\X\times\Y)$.
...
If $(L,\psi)$ is consistent, then for any sequence of surrogate hypotheses $H_1,H_2,\ldots$ whose $L$-loss limits to $L^*$, the $\ell$-loss of $\psi\circ H_1,\psi \circ H_2, \ldots$ limits to $\ell^*$.
\bo{edited above slightly. I believe a technical definition of consistency would have a ``for all $\D$'', but we can probably skip that.}
...
\end{minipage}
\caption{Bayes risks of 0-1, hinge, and logistic losses, respectively, in binary classification setting. \jessie{Move this down to when we actually discuss risks matching?}\raf{Let's keep but add detail / increase font size}}
...
\raf{Reference Theorem~\ref{thm:link-informal}}
...
Then, we impose a requirement that all points in this thickened set are linked to some element of $\gamma(p) = \argmin_r \inprod{p}{\ell(r)}$.
...
The examples below, with the exception of the abstain surrogate given by~\citet{ramaswamy2018consistent}, all present a surrogate that use the link $\psi: u \mapsto \sgn(u)$.
...
\raf{TODO: talk about generic approach: restrict to affine regions, take vertices}
...
As even the formal definition of the Lov\'asz hinge is quite involved, we defer the complete analysis to \S~\ref{app:lovasz} and here state the main results.
...
For example, a modified hinge loss $L(r)_y = \max(r^2-1,1-ry)$, \jessiet{Would a figure be helpful?}\raft{Yeah!} which matches hinge loss on the interval $[-1,1]$ but is smooth outside the interval $[-2,2]$, still embeds twice 0-1 loss.
...
\begin{proof}
  ...
  From Lemma~\ref{lem:X}(\ref{item:X-min-V}), we have some set $\V\subseteq\reals^\Y_+$ and minimum representative sets $\R^* \subseteq \R$ and $\Sc \subseteq \U$, for $\ell$ and $L$ respectively, such that $\ell(\R^*) = \V = L(\Sc)$.\jessiet{This is a different $\Sc$ than the first paragraph, right? The first $\Sc$ is representative for $\ell$.}\raft{They are both quantified separately, so not linked; are you saying it's confusing to use $\Sc$ here though because its role is slightly different?}
...
Here the minimum representative sets are $\{(-1,a),(1,b)\}$ for any $a,b\in\reals$. \jessiet{Do we want this as an example or is it too contrived? I don't have a better one though.  We could also point out that lots of surrogates have an extra dimension in practice (e.g., people use $n$ dimensions instead of $n-1$ for multiclass classification.)}\raft{I like the example for its simplicity.  We could add a footnote about this coming up in practice, but since it's not a major point I'd lean leaving it as-is. If you agree feel free to burn.}
...
\item
  Relation to geometry of losses / superprediction sets?  Say something about trim being finite...
  
\item
  For which other classes of surrogates / settings is indirect elicitation equivalent to consistency?
...
If this inequality is strict whenever $S$ and $T$ are incomparable, meaning $S\not\subseteq T$ and $T\not\subseteq S$, then we say $f$ is \emph{strictly submodular}.
A function is \emph{modular} if the submodular inequality holds with equality for all $S,T\subseteq N$.
The function $f$ is \emph{increasing} if we have $f(S\cup T) \geq f(S)$ for all disjoint $S,T\subseteq N$, and \emph{strictly increasing} if the inequality is always tight.

Let $\triangle$ be the symmetric difference operator, defined by $S\triangle T = (S\setminus T) \cup (T\setminus S)$.
To avoid additional parentheses, throughout we assume $\triangle$ has operator precedence over $\setminus$, $\cap$, and $\cup$.

We are interested in convex surrogates for the following discrete loss $\ell^f:\R\times\Y\to\reals$, with $\R=\Y=2^N$,
\begin{equation}
...
We begin our analysis by calculating $\hat\ell^f$.

We will show that the set $\U = \{-1,0,1\}^k$ is representative for $L^f$.
...
As a first step, we show that the filled hypercube $[-1,1]^k$ is representative.

\begin{lemma}
	\label{lem:lovasz-cube}
	For any increasing and normalized $f:2^N\to\reals_+$, the set $[-1,1]^k$ is representative for $L^f$.
...
\begin{lemma}
	...
	For any increasing and normalized $f:2^N\to\reals_+$, the finite set $\U$ is representative for $L^f$.
\end{lemma}
\begin{proof}
	We will construct polytopes $P^A_\pi \subseteq [-1,1]^k$ for every set $A\subseteq N$ and permutation $\pi \in N^N$, satisfying three conditions: (i) these polytopes cover the hypercube, meaning $\cup_{A,\pi} P^A_\pi = [-1,1]^k$, (ii) $P^A_\pi$ is the convex hull of points in $\U$, and (iii) for all $S\subseteq N$, $L(\cdot,S)$ is affine on $P^A_\pi$.
	The result will then follow, as $L(\cdot;p)$ will be also affine on each $P^A_\pi$, and thus minimized at a vertex.
	
	To begin, let us recall the polyhedra on which the Lov\'asz extension is affine; for any permutation $\pi$, define $P_\pi = \{u\in\reals^k : u_{\pi_1} \geq \cdots \geq u_{\pi_k}\}$.
	It is clear from the definition that $F$ is affine on $P_\pi$; see also Equation 3.1, and ``Linear interpolation on simplices'' (pg.\ 167) in \citet{bach2013learning}.
	We will use these polyhedra to identify the regions where $L(\cdot,S)$ is affine simultaneously for all outcomes $S\in\Y$.
	...
	As $F$ is affine on $P_{\pi'}$ for all permutations $\pi'$, we will conclude that for any fixed subset $S$ the loss $L(u,S) = F(\ones - u\odot\chi_S)$ will be affine in $u$ on $P^A_\pi$.
...
This statement proves the first part of Theorem~\ref{thm:lovasz-embed-main}; see \S~\ref{app:lovasz-tight} for when this embedding is tight.
...
From the form~\eqref{eq:lovasz-embeds}, we see that $\hat\ell^f$ matches $2\ell^f$ when $B=\emptyset$, just as hinge loss embeds twice 0-1 loss.
...
\begin{lemma}
	...
	Then $\bar f \geq f(N)/2$, and $\bar f = f(N)/2$ if and only if $f$ is modular.
\end{lemma}
\begin{proof}
...
We can now restate and prove Theorem~\ref{thm:lovasz-consistent-main}.
\begin{theorem}
	Let $f$ be submodular, normalized, and increasing.
...
\begin{proof}
	...
	Let $p^S = (1-\epsilon) \bar p + \epsilon \delta_S$, where again $\bar p$ is the uniform distribution, and $\delta_S$ is the point distribution on $S$.
  We begin with $p^\emptyset$.
	From Lemma~\ref{lem:2-bar-f}, for all $A\subseteq N$ we have
	\begin{align*}
	\hat\ell^f((A,\emptyset);p^\emptyset)
	&= (1-\epsilon) 2 \bar f + \epsilon \, \hat\ell^f((A,\emptyset),\emptyset)\\
	&= (1-\epsilon) 2 \bar f + \epsilon \, 2f(A)\\
	&\geq (1-\epsilon)2 \bar f > f(N) = \hat\ell^f((\emptyset,N);p^\emptyset)~.
	\end{align*}
	As we have some report with strictly lower loss than all reports of the form $(A,\emptyset)$, we conclude that we must have some $(A,B) \in \prop{\hat\ell^f}(p^\emptyset)$ with $B\neq\emptyset$.
	We can also see that $\prop{\ell^f}(p^\emptyset) = \{\emptyset\}$ by the second equality, since $\ell^f(A,\cdot) = \hat\ell^f((A,\emptyset),\cdot)$ and $\{\emptyset\} = \argmin_{S\subseteq N} f(S)$.
	
	Now let $\Gamma^f = \prop{L^f}$.
  From Lemma~\ref{lem:lovasz-u} and the map between $\U$ and $\hat\R$, we have some $u\in\Gamma^f(p^\emptyset)$ which we can write $u = \chi_A + \ones_B$ for disjoint $A,B\subseteq N$.
  ...
  Otherwise, $T=\emptyset$, so $A=\emptyset$ as well.
	In this case, we will shift to $p^B$, and show that $u\in\Gamma^f(p^B)$ as well, again violating calibration.

	To show optimality, let $c = L^f(u;p^\emptyset) = \risk{L^f}(p^\emptyset)$, and note that by symmetry of $L^f$, for any $S\subseteq N$ we have $c = \risk{L^f}(p^S)$ as well.
	%\raft{Too informal; should flesh out}
	In particular, we have $c = \risk{L^f}(p^B)$.
	By Lemma~\ref{lem:lovasz-u}, we have
	$L^f(u,B) = f(\emptyset\triangle B\setminus B) + f(\emptyset\triangle B\cup B) = f(\emptyset) + f(B) = f(\emptyset\triangle \emptyset\setminus B) + f(\emptyset\triangle \emptyset\cup B) = L^f(u,\emptyset)$.
  Thus, $L^f(u,p^B) = (1-\epsilon) L^f(u,\bar p) + \epsilon L^f(u,B) = (1-\epsilon) L^f(u,\bar p) + \epsilon L^f(u,\emptyset) = L^f(u,p^\emptyset) = c$.
  As $c=\risk{L^f}(p^B)$, we conclude $u\in\Gamma^f(p^B)$.
	Finally, as $\chi_B \neq \chi_T = \sgn(u)$, we are done.
\end{proof}

\subsection{Tightness of the embedding}
\label{app:lovasz-tight}

Finally, we show a sufficient condition for when the embedding is tight.
It appears from the proof that both assumptions on $f$ are required, though we leave open whether that is indeed the case.

\begin{lemma}
  Let $X,Y,Z\subseteq N$ such that $Y\cap Z = \emptyset$.
  Then $X\triangle (Y\cup Z) = X\triangle Y \triangle Z$.
\end{lemma}
\begin{proof}
  $X\triangle (Y\cup Z) = X\cup (Y\cup Z) \setminus (X \cap (Y\cup Z))$


  $X\triangle (Y\cup Z) = (X \setminus (Y\cup Z)) \cup ((Y\cup Z) \setminus X)$
\end{proof}

\begin{theorem}\label{thm:lovasz-embed-tight}
  Let $f$ be strictly submodular and strictly increasing.
  Then $L^f$ tightly embeds $\hat\ell^f$.
\end{theorem}
\begin{proof}
  Let $A,B\subseteq N$ be disjoint.
  We will show that $(A,B)$ is the unique $\hat\ell^f$-optimal report for the distribution $p$ given by
  \begin{equation}
    \label{eq:1}
    p(S) =
    \begin{cases}
      2^{-|B|} & S\setminus A \subseteq B \\
      0 & \text{otherwise}
    \end{cases}~.
  \end{equation}
  The condition $S\setminus A \subseteq B$ is equivalent to the existence of some $T\subseteq B$ such that $S = A \cup T$, and this $T$ must be disjoint from $A$ as $B$ is.
  The expected loss for $(A,B)$ under $p$ is therefore
  \begin{align*}
    \hat\ell^f((A,B);p)
    &= \sum_{T\subseteq B} 2^{-|B|} f(A\triangle (A\cup T)\setminus B) + f(A\triangle (A\cup T)\cup B)
    \\
    &= 2^{-|B|} \sum_{T\subseteq B} f(T\setminus B) + f(T\cup B)
    \\
    &= f(\emptyset) + f(B)~.
  \end{align*}

  Now suppose $A',B'\subseteq N$ are disjoint, with $(A',B') \neq (A,B)$.
  Once again we write the expected loss under $p$.
  We will use the observation that, as $A$ and $T$ are disjoint, $A\cup T = A\triangle T$.
  Defining $\hat A = A' \triangle A$, and using the associativity of $\triangle$, we then have $A' \triangle (A\cup T) = \hat A \triangle T$.
  \begin{align*}
    \hat\ell^f((A',B');p)
    &= 2^{-|B|} \sum_{T\subseteq B} f(A'\triangle (A\cup T)\setminus B') + f(A'\triangle (A\cup T)\cup B')
    \\
    &= 2^{-|B|} \sum_{T\subseteq B} f(\hat A\triangle T\setminus B') + f(\hat A\triangle T\cup B')
    \\
    &= \frac 1 2 \; 2^{-|B|} \sum_{T\subseteq B} \biggl( f(\hat A\triangle T\setminus B') + f(\hat A\triangle T\cup B')
    \\[-8pt]
    & \hspace*{85pt} + f(\hat A\triangle (B\setminus T)\setminus B') + f(\hat A\triangle (B\setminus T)\cup B') \biggr)~.
  \end{align*}
  
  We first suppose $B \subseteq B'$.
  Then $\hat A\triangle T\setminus B' = \hat A \setminus B'$ and $\hat A\triangle T\cup B' = \hat A\cup B'$ for any $T \subseteq B$.
  We conclude
  \begin{align*}
    \hat\ell^f((A',B');p)
    &= \frac 1 2 \; 2^{-|B|} \sum_{T\subseteq B} \biggl( f(\hat A\setminus B') + f(\hat A\cup B') + f(\hat A\setminus B') + f(\hat A\cup B') \biggr)
    \\
    &= f(\hat A\setminus B') + f(\hat A\cup B')~.
  \end{align*}
  We will show either (i) $\hat A\setminus B' \neq \emptyset$ or (ii) $\hat A\cup B' \supsetneq B$ hold, and then apply the fact that $f$ is strictly increasing.
  If (ii) fails, since $B \subseteq B'$, we must have $B = B'$.
  Then $B = B'$ is disjoint from both $A$ and $A'$, and therefore from $\hat A = A'\triangle A$.
  We therefore have $\hat A\setminus B' \neq \emptyset \iff \hat A \neq \emptyset \iff A \neq A'$, which must hold as $B = B'$ yet $(A,B) \neq (A',B')$ by assumption.
  Thus, at least (i) or (ii) hold.
  As $f$ is strictly increasing, we conclude $\hat\ell^f((A',B');p) > f(\emptyset) + f(B) = \hat\ell^f((A,B);p)$.

  Now suppose $B \not\subseteq B'$.
  We claim that there exists $T\subseteq B$ such that one of the following two cases hold:
  \begin{enumerate}
  \item[(i)]
    $\hat A\triangle T\setminus B'$ and 
    $\hat A\triangle (B\setminus T)\cup B'$
    are incomparable;
  \item[(ii)]
    $\hat A\triangle T\cup B'$
    and
    $\hat A\triangle (B\setminus T)\setminus B'$
    are incomparable. 
  \end{enumerate}
  We will then apply strict submodularity to conclude.
...
\raf{It seems we need to handle the case where $B\setminus B'$ is a singleton, since then we run into the same problem as when we used only $\emptyset$ and $B$.  If larger, we can take $T$ to fragment $B\setminus B'$ and I think we're good...}

  Let $i\in B\setminus B'$.
  If $i\in A'$, then $i\in\hat A$ (as $i\notin A$).
...
\section{General characteristics of polyhedral functions}
...
\begin{proof}
  ...
  \jessie{TODO} \url{https://arxiv.org/abs/0805.0292} Prop 4.5?
\end{proof}
...
We now introduce some of our notation.
\begin{itemize}
...
\item Similarly, we let $H_v := H_{([v;-1], 0)}$ for $v \in \reals^\Y_+$ and define $H_y := H_{([e_y;-1], 0)}$. 
...
\begin{proof}
  ...
  Since $G$ is full-dimensional, all facets of $G$ are uniquely determined by the hyperplanes $H$ whose halfspaces $H^+$ compose $\H^*$ by Claim~\ref{claim:G-unique-facets-Hstar}.
...
We now consider projections onto the positive orthant $\reals^\Y_+$.
...
\begin{proof}
  ...
  \jessie{Meeting notes: See \url{http://homepages.cae.wisc.edu/~linderot/classes/ie418/lecture11.pdf} for def of AI}
...
\jessie{Where we're going: (can be deleted)}
\begin{itemize}
...
\begin{claim}\label{claim:Vprime-projected-faces-cover-iff-Vstar-subset-Vprime}
  ...
  $\cup_{v\in\V'} \pi(F_v) = \reals^\Y_+ \iff \V^* \subseteq \V'$.\jessie{Will get us to REP $\iff$ $\V^* \subseteq L(R')$ (3); need to move later as we havne't defined $L$ yet in this section.}
\end{claim}
\begin{proof}
  Would like to show
...
\begin{claim}\label{claim:faces-contained-in-facets}
  If a $H_v \in \H$ does support $G$, then $F_{v} = H_{v} \cap G$ is a nonempty face of $G$, and thus a subset of a facet. \jessie{Do we use this?}
\end{claim}
\begin{proof}
...
\begin{claim}\label{claim:vface-subset-some-vstar-face}
  ...
  \jessie{Want to move this to the projections subsection, but technically we want this for the infinite version of $\V$.} 
\end{claim}
\begin{proof}
...
\end{corollary}
Given the function $f$, we consider $g$ to be its extension and $L$ such that $\risk L_+ = g$ so that we may use the tools in Phases 1 and 2 draw conclusions about the property $\Gamma$.
...
\begin{proof}
  ...
  \jessiet{Moved previous proof outline to comment}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \theta(v)
  \end{align*}
  \raf{Wait, isn't this just Claim 19 again?}
...
\begin{claim}\label{claim:any-levelset-contained-in-minlevelset}
...
Define the \emph{extended level set} $\bar \Gamma_r := \{x \in \reals^\Y_+ \mid \inprod{L(r)}{x} = \risk L_+(x)\}$.
...
\begin{proof}
  \begin{align*}
    ...
    &= \pi(F_v) & \text{Since $F_v = \{(x,c) \mid \inprod{v}{x} = c = g(x)\}$\jessie{Help on this justification?} }\\
  \end{align*}
\end{proof}
