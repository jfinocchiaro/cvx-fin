...
\begin{abstract}
  \raf{prev title: An Embedding Framework for Consistent Polyhedral Surrogates}\jessie{I like the new title}
...
For example, in multiclass and top-$k$ classification, several proposed surrogates were proposed and adopted but later proved to be inconsistent~\cite{yang2018consistency}\raft{need a good ref for multiclass; I'm thinking WW and others}\jessiet{Probably Crammer-Singer and One-vs-All as well?}.
...
\raft{Rewrote this section intro, and added a bunch here so the reader knows roughly how all these concepts connect.  Could use some eyes. \jessie{I like this second paragraph that wraps everything together}}
...
(This fact is non-trivial; see Lemma~\ref{lem:embed-implies-indirect-elic}.)
...
However, we show in \S~\ref{sec:poly-ie-consistency} that (minimizable) polyhedral indirect elicitation implies the existence of a calibrated link, and we leverage the fact that every polyhedral surrogate embeds a discrete loss as an intermediary step in the proof.
...
\jessiet{Thanks for catching that-- updated the sentence to be accurate, but I'm not actually sure if we want it here.  Feel free to delete if there isn't a smooth was to fit it in.}
\raft{TODO: Loop back}
...
The key idea behind our construction is a restatement of calibration equivalent for discrete prediction problems, to a condition we call \emph{separation}.
Roughly, a link is $\epsilon$-separated if the distance between any point in $\reals^d$ that links to an $\ell$-optimal report, and any point that links to a non-optimal report, is at least $\epsilon$.
...
To prove Theorem \ref{thm:link-main}, we now show how to construct a calibrated link from an embedding.
...
\begin{lemma}\label{lem:intersection-property-embedding}
  % Let polyhedral loss $L$ embed some discrete loss $\ell$.
  Let $\U'\subseteq\U$.  Then $\cap_{U\in\U'} U \neq \emptyset$ implies $\cap_{U\in\U'} R_u \neq \emptyset$.
\end{lemma}
\noindent
Thus, for any $u\in\cup\U$, we have a nonempty set of valid choices for $\psi(u)$.
(The statement of Lemma~\ref{lem:intersection-property-embedding} may appear similar to indirect elicitation; in fact the two conditions are equivalent, as we discuss in \S~\ref{sec:poly-ie-consistency}.)
...
This claim follows from a geometric result (Lemma~\ref{lem:thick-empty}) that, for all small enough $\epsilon$, a subset of thickenings $U_{\epsilon}$ intersect if and only if the $U$ sets themselves intersect.
When they do intersect, Lemma~\ref{lem:intersection-property-embedding} implies that there exists a permitted choice of link for the intersection of the thickenings.
...
\raft{I simplified this paragraph to reduce redundancy, and rewrote much of the figure caption so it's clearer how it applies more generally.  See what you think!}
\jessiet{I really like the caption now, but in the caption we're comparing $\prop{L}$ to $\prop{\ell}$ and in the paragraph we mention $\prop{\hat \ell}$, where $\hat \ell$ is the embedded loss.  I kinda like working through $\prop{\hat\ell}$ since it justifies this framework a bit more, but also the drawings look the exact same for $\BEP$ and $\ellabstain$...}
...
\end{equation}
This surrogate $\LWW$ was later shown to be inconsistent with respect to 0-1 loss~\citep{tewari2007consistency,liu2007fisher}.\raft{FYI: changed notation to match the style of our other surrogates.  Not sure I care enough to change the top-$k$ ones though... \jessie{Changed the macro for top-$k$... was that what you meant?}}
...
Using the losses each proposed surrogate embeds, the authors go on to derive constraints on the label distributions under which the proposed surrogates are actually consistent for top-$k$ classification which are tighter than previous constraints~\citep{yang2018consistency}.
\raft{I did not understand what ``which subsume previous constraints'' meant.  Do we mean the constraints are tighter than the previous constraints of YK? \jessie{Exactly; burn}}
...
\begin{construction}[general $\epsilon$-thickened link] \label{const:general-eps-thick-link}
  ...
  For all $U \in \U$, define $R_U := \{r \mid \Gamma_U \subseteq \gamma_r\}$, where $\Gamma_U := \{p\in\simplex \mid U = \Gamma(p)\}$.
...
In general, indirect elicitation is weaker than calibration, since it only depends on the loss through the property it elicits, i.e., its exact minimizers.
...
Fortunately, for polyhedral surrogates, our embedding framework shows that one can always design a separated link from Construction~\ref{const:eps-thick-link}.



%% first step to show iff: indirect polyhedral elicitation passes through embedding.
A important step is to show that, for polyhedral surrogates, indirect elicitation must always pass through an embedding.
...
From Lemma~\ref{lem:X}(\ref{item:X-tight-embed}), $L$ tightly embeds a discrete loss.
  ...
  If $\ell$ indirectly elicits $\gamma$ via the link $\psi$, then $L$ indirectly elicits $\gamma$ by transitivity of subset inclusion, as \jessiet{first subset is an equality, right?}\raft{don't think so} $\Gamma_u \subseteq \hat\gamma_{\hat\psi(u)} \subseteq \gamma_{\psi \circ \hat\psi(u)}$ for all $u \in \reals^d$.
...
With Lemma~\ref{lem:ie-iff-embeds-refinement}, we can show the main result of this section: if a polyhedral surrogate indirectly elicits $\prop{\ell}$, then it is also calibrated with respect to $\ell$ for some choice of link function.
...
Indeed, our framework has already been applied to submodular structured problems (\S~\ref{sec:lovasz-hinge}) and to max-margin losses\raft{Cite the two Nowak Rudi Bach papers; 2nd one came out now: \url{https://proceedings.mlr.press/v151/nowak22a.html}}.
...
We motivated the idea of an embedding as a way to ``convexify'' a discrete loss.
It is not clear, however, how embeddings relate to the convex envelope operation.
...
Conversely, we would like to know when the construction in Theorem~\ref{thm:discrete-loss-poly-embeddable} can be viewed as a convex envelope, and therefore a true ``embed and convexify'' approach.

\raf{Could add superprediction stuff here}


\paragraph{Polyhedral vs.\ smooth surrogates}
  \raf{Show that polyhedral losses are ``more efficient'' in a statistical sense?
  We know target rates are likely the same (or not far off) for smooth vs non-smooth, so difficulty of the learning problem may break the tie.
...
\begin{lemma} \label{lem:calibrated-pos}
  Let $\U' \subseteq \U$.
...
\end{proof}
Lemma \ref{lem:calibrated-pos} implies that there exists a $\psi$ such that $(L,\psi)$ indirectly elicits $\ell$: for each $u$, let $\U' = \{U\in\U : u \in U\}$ be the optimal sets that contain it; choose $r$ from the nonempty set $\cap_{U \in\U'} R_U$; and set $\psi(u) = r$.

\raf{ALTERNATIVE...}

Let $\gamma = \prop\ell$.
For all $U \in \U$, let $R_U = \{r: \Gamma_U \subseteq \gamma_r\}$, where $\Gamma_U = \{p\in\simplex : U = \Gamma(p)\}$.
\begin{lemma}
  If $L$ embeds $\ell$, then $L$ indirectly elicits $\gamma$.
\end{lemma}
\begin{proof}
...
\end{proof}
\begin{lemma}
  Suppose $L$ indirectly elicits $\gamma$.
  If $\cap_{U\in\U'} U \neq \emptyset$ then $\cap_{U\in\U'} R_U \neq \emptyset$.
\end{lemma}
\begin{proof}
  Observe that, as $\U$ is the range of $\Gamma$, we have for all $u\in\reals^d$ that $\Gamma_u = \cup_{U \ni u} \Gamma_U$.
  Suppose $\cap_{U\in\U'} U \neq \emptyset$, and let $u \in \cap_{U\in\U'} U$.
...
The main problem now is to prove a ``thickened'' analogue of Lemma \ref{lem:calibrated-pos} that extends this link to points $u$ that are up to $\epsilon$ far from an optimal set $U$.
Namely, Lemma \ref{lem:thick-empty} will show that if $\epsilon$ is small enough, then the $\epsilon$-thickenings of all $U \in \U'$ intersect if and only if the $U$ sets themselves intersect.
Thus, if $u \in \cap_{U \in \U'} B(U,\epsilon)$, then $u \in \cap_{U \in \U'} U$, and Lemma \ref{lem:calibrated-pos} gives some legal target report $\psi(u) = r \in \cap_{U \in \U'} R_U$.

The next few geometric results build to Lemma \ref{lem:thick-empty}.
...
\end{corollary}
\begin{proof}
  For each subset, Lemma \ref{lem:thick-empty} gives an $\epsilon_0 > 0$.
...
\begin{proof}
  ...
  By Lemma \ref{lem:calibrated-pos}, their corresponding report sets $\{R_j : j \in \mathcal{J}\}$ also intersect at some $r$, so $\Psi(u)$ is nonempty.
\end{proof}
...
While one traditionally considers $P$ to be a trivial face of itself, we exclude this case throughout by discussing nontrivial faces.
\raft{It was a bit confusing because you said to observe that $P$ was a face of itself when you defined face to exclude that possibility\jessie{Is this clearer?}}
...
\begin{definition}[{\citet{gallier2008notes}}]\label{def:irredundant}
	...
	If $\cap \H'$ is an irredundant decomposition of $P$, then we call $\partial H^+$ an irredundant face of $P$ for each $H^+ \in \H'$.
\end{definition}
...
\begin{proof}
	...
	As the smallest halfspace representation, $\H^*$ is irredundant by definition.
...
%	This can only iterate up to $|\H^*|$ times, so there must be some finite irredunant representation for $P$.\jessie{This feels way too spelled out for its lack of detail.}
  ...
  By (ii), each $H^+ \in \H^*$  determines a facet of $P$, and this facet is irredundant by (i)\raft{Not defined (I think) what it means for a facet to be irredundant.  \jessie{good catch; updated the definition. burn.}}.
...
Given any $v \in \reals^\Y_+$, define $H^+_v := H^+_{(v, -1)} = \{(x, c) \in \reals^{\Y}_+ \times \reals \mid \inprod{v}{x} = c\}$.
...
Given a set $\V \subseteq \reals^d$, we let $\H_{\V} = \{H_v^+ \mid v\in\V\}$ denote the set of halfspaces generated by $\V$, $\H_\Y = \{H_y^+ \mid y\in\Y\}$.
...
Throughout, we will work with a function $g_\V$ generated by a set $\V \subseteq \reals^\Y_+$ of the following form.
...
Using the polyhedral perspective above, we can characterize the hypograph of $\risk L_+ = g_{L(\R)}$ and extract a finite set $\V$ of ``active'' loss vectors, so that $\risk L_+ = g_\V$.
%Let $\H_{L(\R)} = \{H_v^+ \mid v\in L(\R)\}$.
Observe that $L(\R)$ and $\H_{L(\R)}$ may be infinite sets.  
\raft{FYI, I have seen tons of places where you defined things in the text, and then again in the claim/lemma/theorem statement, which actually makes it more confusing.  Probably a hold-over from when you only defined things out of statements}
...
\subsection{From a finite set of loss vectors to the unique smallest}

\jessie{We can work with finite $\V$.}
...
We will work with any $\V \subseteq \reals^\Y_+$ which is finite.
%Now, we can define $\hyp(g_\V)$ as the intersection of halfspaces generated by $\V$ on the nonnegative orthant.
%\jessie{This isn't reffed anywhere else in the paper.  Remove?}
...
\jessie{We want to get to $\V^*$.  We need to build up to it through $\V$ and working with extended BR.}
We proceed with some observations about facets and dimension of $\hyp(g_\V)$ in order to use the \emph{smallest} halfspace representation for $g_{L(\R)} = g_\V$.


%\jessiet{Move into next lemma?  it's only used that one time, and it's so short it might as well go in.- June 2}
...
\jessiet{move theorem~\ref{thm:polyhedron-uniquely-gen-facets} here?- June 2}

With a bit of preliminary work, we can apply Theorem~\ref{thm:polyhedron-uniquely-gen-facets} to observe a unique set of halfspaces $\H^*$ generating $\hyp(g_\V)$.

\jessie{Getting to $\V^*$ through $\H^*$, which exists (and contains $\H_\Y$)}
\begin{lemma}\label{lem:G-unique-facets-Hstar}
  ...
  There is some unique irredundant $\H^* \subseteq \H$ such that $\hyp(g_\V) = \cap \H^*$. 
  Moreover, for each $H^+ \in\H^*$ and $H$ such that $H = \partial H^+$, the face $\hyp(g_\V) \cap H$ is a facet. 
  Moreover, $\H_\Y \subseteq \H^*$. 
\end{lemma}
\begin{proof}
	First, we need to show that $\hyp(g_\V)$ is full-dimensional.
	Since $g_\V$ is nonnegative on $\reals_+^\Y$, $\hyp(g_\V)$ therefore contains $\{(x,c) \mid x\in\reals^\Y_+, c \leq 0\}$, which is $(|\Y| + 1)$-dimensional.
	
  Since $\hyp(g_\V)$ is full-dimensional, $\hyp(g_\V) \cap H$ being a facet follows immediately from Theorem~\ref{thm:polyhedron-uniquely-gen-facets}(i) and (iii).
  
  To see $\H_\Y \subseteq \H^*$, if there was a $y \in \Y$ such that $H^+_y = \{(x,c) \mid x_y \geq 0\}$ was not in $\H^*$, then we would either have some $c_1 > 0$ such that $\{(x,c) \mid x_y \geq c_1\} \in \H^*$, or we have a point $x$ such that $x_y < 0$ but $g(x) > -\infty$.
  The first cannot happen as $g_\V$ is concave and finite at $x = e_y \in \reals^\Y_+$.
  The second cannot be true by construction of $g_\V$ since $\V$ is finite including the convex indicator on $\reals^\Y_+$\raft{Couldn't follow this last line, since $\delta$ can be $\infty$}. 
\end{proof}
...
\begin{corollary}\label{cor:unique-set-loss-vectors-defining-facets}
  ...
  (ii)$F_v$ is a facet of $\hyp(g_\V)$ for each $v\in\V^*$.
  (iii) $\V^* \subseteq \V$.
\end{corollary}
\begin{proof}
...
%
  (i)Since $\hyp(g_\V)$ is full-dimensional, the facets of $\hyp(g_\V)$ are uniquely determined by the hyperplanes $H$ such that $H = \partial H^+$ and $H^+ \in \H^*$\raft{was ``$\H^* = \{H^+\}$''} by Lemma~\ref{lem:G-unique-facets-Hstar}.
  Any facet must then be some intersection of an $H_y \cap \hyp(g_\V)$ or $H_v \cap \hyp(g_\V)$.
  Consider $\H_{\V^*} := \H^* \setminus \H_{\Y}$, and $\V^*$ the unique set generating $\H_{\V^*}$, since $\H_\Y \subseteq \H^*$ by Lemma~\ref{lem:G-unique-facets-Hstar}.
  (ii) Moreover, $H_v \in \H_{\V^*} \subseteq \H^*$ generates the facet $F_v$ of $\hyp(g_\V)$ by Lemma~\ref{lem:G-unique-facets-Hstar}.
%
%  The moreover follows immediately since $\H_{\V^*} \subseteq \H^*$ and every $H^+ \in \H^*$ defines a facet by Lemma~\ref{lem:G-unique-facets-Hstar}.
  (iii)  $\H_\Y \cup \H_{\V^*} = \H^*$ by Corollary~\ref{cor:unique-set-loss-vectors-defining-facets}, and $\H^* \subseteq \H = \H_\Y \cup \H_{\V}$ by Lemma~\ref{lem:G-unique-facets-Hstar}, ergo $\V^* \subseteq \V$.
\end{proof}
...
We now show that we can equivalently construct $g_\V$ through the unique finite set $\V^*$ instead of the given set of vectors $\V$, and and later, extend this result to loss vectors $L(\R)$.
...
This paves the way for our primary assumption for the rest of this appendix, which we show is always satisfied by losses with polyhedral extended risk.
...
\begin{assumption}\label{assum:V-star-exists-infinite}
	...
	$\V^* \subseteq L(\R)$ is the unique finite set such that $g_{L(\R)} = g_{\V^*}$ and $\hyp(g_{L(\R)}) = \hyp(g_{\V^*}) = \cap(\H_{\V^*} \cup \H_\Y)$, and the last of these is irredundant.
\end{assumption}
...
\begin{proof}
	...
	Equality of the hypographs follows immediately from equailty of the graphs.
...
\subsection{Projecting from $\reals^d_+$ to $\reals^\Y_+$\jessie{extended Bayes risk to extended level sets}}\label{subsec:project-pi}
...
\begin{corollary}\label{cor:projected-Vstar-faces-cover-pos-orthant}
	...
	Then $\cup_{v\in\V^*} \pi(F_v^{\hyp(g_{\V^*})}) = \reals^\Y_+$.
\end{corollary}
...
\jessie{Projection $\pi$ preserves dimensions so we can move from extended risk to extended level set and back}
...
\begin{claim}\label{claim:projected-Vstar-faces-full-dim}
	Given $L, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, define $F_v := F^{\hyp(g_{\V^*})}_v = H_v \cap \hyp(g_{\V^*})$.
...
\jessie{Translated versions of lemma~\ref{lem:X} statements incoming}
...
\begin{proof}
	...
	Observe that $\V^*$ is unique and irredundant (by assumption) and $\pi(F_v)$ is full-dimensional in $\reals^\Y_+$ by Claim~\ref{claim:projected-Vstar-faces-full-dim}.
...
We now claim that a set of projected faces $\{F^{\hyp(g_{L(\R)})}_v\}_{v \in \V'}$ for some $\V'$ will cover $\reals^\Y_+$ if and only if $\V^* \subseteq \V'$.
Given a finite set $\V^* \subset \reals^\Y_+$, we denote $\Lambda_{S} := \{\pi(F^{\hyp(g_{L(\R)})}_v) \mid v \in S\}$ as the set of projected facets generated by $S$.
...
\begin{claim}\label{claim:vface-subset-some-vstar-face}
	Consider $L, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}.
	For any $v \in L(\R)$, the face $F^{\hyp(g_{\V^*})}_{v} \subseteq F^{\hyp(g_{\V^*})}_{v^*}$ for some $v^* \in \V^*$. \jessiet{Need to clarify that face of $\hyp(g_\V)$?} %must be a subset of one of the $\V^*$ facets $F_{v^*}$ (as opposed to the vertical ones)
\end{claim}
\begin{proof}
	\jessie{I'm not sure if this is clear, but this is a new approach to the proof that I generally like better. I can clarify if the wording is not clear though. Old version in comments}
...
\begin{claim}\label{claim:f-min-affine}
	...
	For any polyhedral concave function with domain on $\simplex$, there is some finite set $\V \subseteq \reals^\Y_+$ such that the function can be written $f_\V(p) = \min_{v\in\V} \inprod{p}{v} - \delta(p \mid \simplex)$.  %\raf{show you can do this using the $h$ and $\delta$ representation from Rockafellar -- argue that because $p\in\simplex$ we can write $\inprod{p}{b_i}-\beta_i = \inprod{p}{b_i-\beta_i\ones}$.}
\end{claim}
\begin{proof}
...
Moreover, let $\R^*$ be the set such that $\V^* := L(\R^*)$.

\begin{corollary}
	Consider $L, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, and $\R^*$ such that $\V^* = L(\R^*)$. 
	(Such a set exists as $\V^* \subseteq L(\R)$ in Assumption~\ref{assum:V-star-exists-infinite})
...
Since $\risk L$ is polyhedral and concave, there exists a finite set $\V$ such that $L$ and $\V$ satisfy Assumption~\ref{assum:V-star-exists-infinite} by Proposition~\ref{prop:poly-risk-assumption-satisfied}.\jessiet{Actually, we need to show that $\risk L_+$ is polyhedral, and only have $\risk L$ polyhedral...}
...
% for a finite set $\V$ by Claim~\ref{claim:f-min-affine}.
