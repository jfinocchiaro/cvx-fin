...
\raf{Make a corollary.  THus we'll focus on finite rep sets.}

\begin{proposition}\label{prop:SL-minimum}
  A minimizable loss $L: \R \to \reals^\Y_+$ has a polyhedral Bayes risk if and only if it has a finite representive set.
...
\begin{proof}
  ...
  Lemma~\ref{lem:X} gives the converse.
\end{proof}
...
\raf{Demote to corollary?}
\begin{proposition}\label{prop:embed-risk-poly}
  A minimizable loss $L$ embeds a discrete loss $\ell$ if and only if $\risk{L}$ is polyhedral.
\end{proposition}
\begin{proof}
  If $L$ embeds a discrete $\ell$, Proposition~\ref{prop:embed-bayes-risks} gives us $\risk{L} = \risk{\ell}$ and therefore $\risk{L}$ is polyhedral.
  For the converse, let $\risk{L}$ be polyhedral. %; we again examine the proof of Proposition~\ref{prop:embed-bayes-risks}.
  Lemma~\ref{lem:X} gives a finite representative set $\Sc$, and $L$ embeds $L|_\Sc$ by Proposition~\ref{prop:representative-embeds-restriction}.
\end{proof}

\raf{Moved; reorganize}
...
\begin{proposition}\label{prop:tfae-min-rep-nonredundant}
  Let $\U$ be a finite representative set for $L$.
...
%  Therefore, $\R$ is minimum.
  ...
  Since $\U$ is a finite representative set, we know $L|_\U$ is embedded by $L$ in Proposition~\ref{prop:representative-embeds-restriction}.
...
$\ell$ is a finite loss since $\U$ is finite, and by Proposition~\ref{prop:representative-embeds-restriction}, $L$ embeds $\ell$.
...
Since $\cup \Theta = \simplex$, we have $\U$ a finite representative set for $L$, so $L$ elicits $L|_\U$ by Proposition~\ref{prop:representative-embeds-restriction}, and therefore $\Gamma$ embeds $\gamma := \prop{L|_\U}$. 
...
%  We can see $\R'$ is representative since $\cup_{u \in \R'} \Gamma_u = \cup \theta = \simplex$, and therefore we have the result by an analog of Proposition~\ref{prop:representative-embeds-restriction}.
%  
...
%  %% If we show $ c \implies b$, we have $b \implies a$ as an version of Proposition~\ref{prop:representative-embeds-restriction}.
%  % For each $\theta \in \Theta$, there is some $u \in \R$ so that $\theta = \Gamma_u$; take $\R'$ to be the set of such $u$s.
%  % If $\R'$ is a minimum representative set, then we have the result by an analog of Proposition~\ref{prop:representative-embeds-restriction}.
%  % 
