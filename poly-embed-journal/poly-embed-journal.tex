...
\end{equation}
where $B:[n]\to\{-1,1\}^d$ is a arbitrary injection; let us assume $n = 2^d$ so that we have a bijection\raft{Do we actually need the bijection?  Might be simpler to leave this statement out otherwise}.
...
Moreover, as we illustrate in Figure~\ref{fig:abstain-links}(L), the link function $\psi$ proposed by \citeauthor{ramaswamy2018consistent} can be recovered from Theorem~\ref{thm:link-main} by choosing the norm $\|\cdot\|_\infty$ and setting $\epsilon=1/2$\raft{Double-check theorem ref; was eps-thick-calibrated.  Also below.}.
...
Given a tree $H = ([n], E,W)$ over finite class labels\raft{Let's clear up what we mean by the elements of this tuple, or cut it entirely.  What is $W$?}, they consider the discrete loss 
...
We can also verify that their surrogate is an embedding and recover their result through Theorem~\ref{thm:poly-ie-implies-consistent}\raft{Can we say more explicitly, as we did for BEP, how exactly our framework could have helped, and/or sheds light on it now?}.
...
\label{sec:winge}
While the hinge loss is the most common surrogate for binary support vector machines (SVMs), original extensions to $n$-class prediction involved a one-vs-all reduction to the binary problem and generating ${n \choose 2}$ hyperplanes via hinge loss.
Proposing a more efficient solution,~\citet*{weston1999support} give an alternate surrogate for multiclass SVM prediction, though originally its consistency with respect to 0-1 loss remained open.
Regardless,~\citet{dogan2016unified} advocate for the Weston-Watkins SVM to be the default for multiclass prediction problems.
