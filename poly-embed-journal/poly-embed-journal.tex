...
\begin{definition}\label{def:indirect-elic}
	A loss $L : \reals^d \to \reals^\Y_+$  and surrogate $\psi : \reals^d \to \R$ pair $(L, \psi)$ indirectly elicits a property $\gamma : \simplex \toto \R$ if $L$ elicits a property $\Gamma : \simplex \toto \reals^d$ such that, for all $u \in \reals^d$, we have $\Gamma_u \subseteq \gamma_{\psi(u)}$.
...
From the definition of refinement, it is almost immediate to observe that a loss $L$ indirectly elicits a property $\gamma$ if $\prop L$ refines $\gamma$.
...
%% giving intuition for new figure
\raf{Follow-up paragraph on prev sentence using abstain as an example} \jessie{Attempted}
For example, \citet{ramaswamy2018consistent} propose the BEP surrogate, which embeds the discrete abstain loss (\S~\ref{sec:abstain}).
With the abstain loss in hand, it is easy to verify that the abstain-optimal report for any $p$ such that $\max_y p_y < 1/2$ is to ``abstain'' from predicting.
When juxtaposed with multiclass classification in Figure~\ref{fig:abstain-intuition-inconsistent}, one can observe that the BEP surrogate does not indirectly elicit the mode, as the level set $\prop{L^{BEP}}_{\vec 0} = \prop{\ell^{\mathrm{abs}}}_\bot \not \subseteq \mode_r$ for any $r$. \jessie{Notation}
...
\begin{minipage}{0.55\linewidth}
	\caption{Example demonstrating the BEP surrogate $L^{BEP}$ (\S~\ref{sec:abstain}) is not calibrated for multiclass classification, eliciting the mode.
	The level sets of $\prop{L^{BEP}}$ are given in solid black lines, while the level sets of the mode are given in dashed blue lines.
	To demonstrate non-calibration, one can pick distributions $p, p'$ in the center cell (blue) such that $\vec 0 = \prop{L^{BEP}}(p) = \prop{L^{BEP}}(p')$, but $\mode(p) \neq \mode(p')$.  In this case, it is unclear how one should define $\psi(\vec 0)$, since it will not link to the mode for either $p$ or $p'$.  Thus, there is no calibrated link for the mode if the entire center cell is viable \jessie{word choice?}.  Instead, one may recover calibration by restricting to a subset of $\simplex$.}
...
Observe that $L$ is exactly hinge loss when $n=2$ and thus $d=1$. \jessiet{Remove this sentence?  or new paragraph below?}
...
Using our framework, one can show that $L$ embeds (2 times) $\ell$, with the embedding given by $\varphi$ above where we define $\varphi(\bot) = 0 \in \reals^d$.
