...
\begin{definition}[Supports]
	...
	(i) $P \subseteq H^+$ for a halfspace $H^+$ with $H = \partial H^+$
	(ii)$H \cap \partial P \neq \emptyset$.
	Moreover, $H$ supports $P$ at $x$ if $x \in H \cap \partial P$.
\end{definition}

\jessie{In definition 19, need a way to go uniquely from halfspace to hyperplane}
...
\begin{definition}[Face, facet]
  ...
  \begin{equation*}
    F = P \cap H~,
  \end{equation*}
  for any hyperplane $H$ supporting $P$.
...
\begin{proof}
	...
	By (i), we know that each $H^+_{(w,b)} \in \H^*$ uniquely determines a facet of $P$.
	...
	As polyhedron are uniquely determined by their facets (by Minkowski's uniqueness theorem), we must have $H^+_{(w,b)} \in \H^*$. 
...
\btw{Commented out a claim that has no label, so is not being explicitly used later, please double check that we don't use it.  The proof has some forward refs}
%\begin{claim}
%	$\H_{\V^*} \subseteq \H$
%\end{claim}
%\begin{proof}
