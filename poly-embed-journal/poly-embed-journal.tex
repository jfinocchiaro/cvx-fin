...
\begin{construction}[general $\epsilon$-thickened link] \label{const:general-eps-thick-link}
  ...
  Define $\Gamma = \prop L$ and $\U = \{\Gamma(p) \mid p \in \simplex\}$.
  For all $U \in \U$, define $R_U := \{r\in\R \mid \Gamma_U \subseteq \gamma_r\}$, where $\Gamma_U := \{p\in\simplex \mid U = \Gamma(p)\}$.
  The \emph{$\epsilon$-thickened link} $\psi$ is constructed as follows.
...
In this section, we prove Theorem \ref{thm:thickened-separated}, as discussed at the beginning of Appendix \ref{sec:equiv-sep-calib}: embeddings give rise to separated links.
We will prove this statement by combining three broader facts: (i) an embedding is a special case of indirect elicitation (Lemma~\ref{lem:embedding-implies-indirect-elic}), (ii) indirect elicitation is equivalent to eq.~\eqref{eq:intersection-property-embedding}, the intersection condition on optimal sets (Lemma~\ref{lem:intersection-equiv-indirect-elic}), and (iii) for polyhedral surrogates, this intersection condition implies separation (Proposition.......).
Combining these statements together will show that Construction~\ref{const:general-eps-thick-link}, a generalization of Construction~\ref{const:eps-thick-link}, gives a separated link.
...
\begin{proof}
  ...
  Observe that, as $\U$ is the range of $\Gamma$, we have for all $u\in\reals^d$ that $\Gamma_u = \cup\{\Gamma_U \mid U\in\U, u\in U\}$.
...
\begin{lemma}~\label{lem:thick-intersect}
  Let $\U$ be defined as in Lemma~\ref{lem:intersection-equiv-indirect-elic}.
...
\subsection{Separation for embeddings and beyond}
...
\begin{proposition}\label{prop:link-converse}
  ...
  Let $\epsilon>0$ be given, and let $\Psi$ be the link envelope from Construction~\ref{const:general-eps-thick-link} for $L,\ell,\epsilon,\|\cdot\|_\infty$.
  Then a link $\psi:\reals^d\to\R$ is $\epsilon$-separated if and only if $\psi(u) \in \Psi(u)$ for all $u\in\reals^d$.
\end{proposition}
\begin{proof}
  ...
  \begin{align*}
    ...
    \\
    &\iff \forall u\in\reals^d,\, p\in\simplex \text{ s.t. } d(u,\Gamma(p)) < \epsilon,\;\; \psi(u) \in \gamma(p)
    \\
...
From the equivalence of calibration and separation (Theorem~\ref{thm:calibrated-separated}), we now have the following.
\begin{corollary}\label{cor:link-char}
  A link $\psi$ is calibrated for a given discrete target $\ell$ and polyhedral surrogate $L$ if and only if there exists $\epsilon$ such that $\psi$ is produced by Construction \ref{const:general-eps-thick-link} for $L,\ell,\epsilon,\|\cdot\|_{\infty}$.
\end{corollary}
...
\begin{theorem}\label{thm:general-construction-eps-sep}
  Let $L:\reals^d\to\reals^\Y_+$ be polyhedral and $\ell:\R\to\reals^\Y_+$ be discrete.
  If $L$ indirectly elicits $\prop\ell$, then there exists $\epsilon_0 > 0$ such that, for all $0 < \epsilon \leq \epsilon_0$, Construction~\ref{const:general-eps-thick-link} for $L$, $\ell$, $\epsilon$, $\|\cdot\|_\infty$, yields an $\epsilon$-separated link with respect to $L$ and $\ell$.
\end{theorem}
\begin{proof}
  Fix a small enough $\epsilon_0$ as promised by Lemma~\ref{lem:thick-intersect}.
  Let $u\in\reals^d$ and $\U'_u = \{U\in\U \mid d(u,U) < \epsilon\}$.
  From Construction \ref{const:general-eps-thick-link}, we have $\Psi(u) = \cap \{R_U \mid U\in\U'_u\}$.
  Since $u\in\cap\{U\in\U'_u\} B(U,\epsilon)$ by definition, by Lemma~\ref{lem:thick-intersect}, because of our choice of $\epsilon$, we have $\cap\U'_u \neq \emptyset$.
  By Lemma~\ref{lem:calibrated-pos}, we have $\cap \{R_U \mid U\in\U'_u\} \neq \emptyset$, giving $\Psi(u) \neq \emptyset$.
  In particular, $\psi(u)$ is well-defined.
  Now Proposition~\ref{prop:link-converse} \raf{Corollary~\ref{cor:link-char}?} gives the result.
...
\raf{TODOS:}
\begin{enumerate}
\item corollary: indirect elicitation implies calibration
\item X prove general converse
\item show that construction 1 is a special case (really just the $R_U$ sets!)

  Let $\varphi:\Sc\to\reals^d$ be an embedding from $L$ to $\ell$.
  Let $U\in\U$.
  Then for $r\in\Sc$, we have $r \in R_U \iff \Gamma_U \subseteq \gamma_r \iff \Gamma_U \subseteq \Gamma_{\varphi(r)} \iff (\Gamma(p) = U \implies \varphi(r) \in \Gamma(p)) \iff \varphi(r) \in U$, which matches the original definition of $R_U$ \raf{careful about $\R$ vs $\Sc$}.
  
\item corollary: Theorem~\ref{thm:thickened-separated}
\item Once all the pieces are in place: where should they go?  What should we prove in section 7?  Maybe the link characterization goes there, not here?
\end{enumerate}
