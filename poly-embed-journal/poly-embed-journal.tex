...
This can be shown, for example, if there exists a point $u$ where $\Psi(u)$ is empty for all $\epsilon > 0$.\botodo{Is this a reasonable possibility?}
...
Once the embedded discrete loss is known, the behavior of the surrogate becomes more clear: what problem it is solving, and for which restrictions on label distributions is it consistent for the original problem $\ell$.
In particular, any level set of the embedded property $\hat\gamma$ which spans multiple level sets of the target property $\gamma=\prop{\ell}$ will lead to inconsistency for $\ell$ (Figure~\ref{fig:abstain-intuition-inconsistent}).
...
\begin{minipage}{0.5\linewidth}
	\caption{Example demonstrating the BEP surrogate $L$ (\S~\ref{sec:abstain}) is not calibrated for multiclass classification, eliciting the mode.
	...
	To demonstrate non-calibration, one can pick distributions $p, p'$ in the center cell (blue) such that $\vec 0 = \Gamma(p) = \Gamma(p')$, but $\mode(p) \neq \mode(p')$.  In this case, it is unclear how one should define $\psi(\vec 0)$, since it will not link to the mode for either $p$ or $p'$.  Thus, there is no calibrated link for the mode without distributional assumptions.
	}
	\label{fig:abstain-intuition-inconsistent}
	\end{minipage}
\end{figure}

%% also interesting to use Sec 4 for ``whole'' picture
\raft{Rework so emphasis is more on the general construction and less on testing a given link. \jessie{Attempted; old version in comments}}
With an embedding in hand, Construction~\ref{const:eps-thick-link} provides a calibrated link function.
% adding - May 10, 22
While the literature often ``guesses'' and verifies the calibration of propose links, Construction~\ref{const:eps-thick-link} removes the guesswork from the equation.
This is beneficial in when the most intuitive link functions are not calibrated and no known calibrated link is known; see \S~\ref{sec:lovasz-hinge} for a somewhat intricate example.
Moreover, as Construction~\ref{const:eps-thick-link} fully characterizes the set of possible calibrated link functions for a polyhedral embedding via the link envelope $\Psi$, one can also use $\Psi$ in Construction~\ref{const:eps-thick-link} to verify the calibration of proposed links, demonstrated in \S~\ref{sec:top-k}.
%% previous version - May 10 22
...
%Interestingly, our construction is also useful in this situation, where one wishes to verify the consistence of a given proposed link $\psi$. 
...
\citet{ramaswamy2018consistent} study the loss $\ell: [n] \cup \{\bot\} \to \reals^\Y_+$ defined by $\ell(r)_y = 0$ if $r=y$, $1/2$ if $r = \bot$, and 1 otherwise.
...
Letting $d = \ceil{\log_2(n)}$, their surrogate $L : \reals^d \to \reals^\Y_+$ is given by
...
Observe that $L$ is exactly hinge loss when $n=2$ and thus $d=1$. 
...
Hence, our framework could have simplified the process of finding $\psi^\infty$, and the corresponding proof of consistency and surrogate regret bounds (\S~\ref{subsec:regret-bounds}\raft{let's reference the appendix where we derive the bound here}).
...
\begin{equation}\label{eq:abstain-link-1}
  ...
  \end{cases}~.
\end{equation}
Construction~\ref{const:eps-thick-link} gives calibration of $(L,\psi^1)$ with respect to $\ell$. 
...
Like Figure~\ref{fig:abstain-intuition-inconsistent}, Figure~\ref{fig:ordered-partition} gives the level sets $\gamma_T =\{p \in \simplex \mid T \in \prop{\ell^\OP}(p)\}$ for each $T \in \T$ juxtaposed with the level sets $\mode_r = \{p \in \simplex \mid r \in \mode(p)\}$ for each $r \in \Y$.
...
\begin{table*}
	...
	\caption{
		...
		Regions outlined in black that are filled in blue and cross the dashed blue lines suggest where deciding how to construct a link $\psi$ is ambiguous, as the top-$k$ elements of the optimal report $u$ are ambiguous.  \jessie{Thoughts on this caption?  Now walking through this geometric intuition for the third time.}
	}
