...
For example, in multiclass and top-$k$ classification, several proposed surrogates were proposed and adopted but later proved to be inconsistent~\cite{yang2018consistency}\raft{TODO: ref for multiclass; I'm thinking WW and others}\jessiet{Probably Crammer-Singer and One-vs-All as well?}\raft{Yep, let's add those}.
...
For example, the literature on minimax optimization in supervised learning settings \citep{asif2015adversarial,farnia2016minimax,fathony2016adversarial} poses a minimax optimization problem of the expected loss matrix over both players' strategies, which becomes a convex relaxation of the discrete optimization problem\raft{TODO: We said ``poses the construction in Theorem~\ref{thm:discrete-loss-poly-embeddable}'' -- is it exactly the same?  We just have to say how it relates, like ``when viewing X as Y, one arrives at the same construction'', or maybe one is a special case of the other.}\raf{TODO: Update 6/12/2022: looks like this sentence changed, but now isn't tied to Theorem~\ref{thm:discrete-loss-poly-embeddable}...}.
...
based on optimality conditions \citep{finocchiaro2020embedding}\raft{Thoughts on using 1st person plural here for previous work?  It is the same authors, and same broader project... I think it's less awkward this way, but I'm okay with 3rd person too}.
...
See also \raf{TODO: cite Ziegler} and \citet{gallier2008notes}.
...
\begin{proof}
  ...
  The second cannot be true by construction of $g_\V$ since $\V$ is finite including the convex indicator on $\reals^\Y_+$\raft{TODO: Couldn't follow this last line, since $\delta$ can be $\infty$\jessie{does it help that it's minus the convex indicator?} 6/20/2022: still don't get it, sorry :-)}. 
\end{proof}
