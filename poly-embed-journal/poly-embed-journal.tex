...
We apply these tools to several polyhedral surrogates which have been proposed recently (\S~\ref{sec:examples}), showing that they are inconsistent for the desired target, but are consistent for a different target: the one embedded by the loss.
...
\subsection{Embedding and Main Results}
...
%  Therefore, $\R$ is minimum.
  ...
  Since $\Gamma|_\U$ is non-redundant by assumption, we can apply Proposition~\ref{prop:embed-trim}\jessiet{BTW: Forward ref}, $\U$ is a minimum representative set for $L$.
