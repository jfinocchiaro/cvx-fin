...
\end{definition}
Additionally, we sometimes let $H_y^+ := H_{e_y}^+$. % \{ (x,c) \in \reals^\Y \times \reals \mid x_y \geq 0\}$.
\begin{definition}[Hyperplane]
	For any $(w,b)\in\reals^\Y_+ \times \reals$, let $H_{(w,b)} := \{ x \in \reals^d \mid \inprod{x}{w} - b = 0\}$ be the hyperplane generated by $(w,b)$.
	Again, we let $H_v := H_{([v;-1], 0)}$ for a $v \in \reals^\Y_+$. 
\end{definition} 

\begin{definition}[Polyhedron - halfspace representation\jessie{Ziegler page 28}]
	An \emph{polyhedron} $P$ describes an intersection of a finite set of closed halfspaces $\H = \{H^+_{(w_i,b_i)}\}$ presented in the form $P = \cap \H$.
%	\begin{equation*}
...
\begin{definition}[Face, facet]
	...
	A \emph{face} of the polytope $P$ is any set of the form
	\begin{equation*}
	F = P \cap H_{(w,b)}~,~
	\end{equation*}
	for any valid halfspace $H^+_{(w,b)}$.
	The dimension of a face is the dimension of its affine hull $\dim(F) := \dim(\affhull(F))$.
...
\end{claim}
\begin{proof}
We can write any face $F_v = \{x \in \reals^d \mid \inprod{x}{v} = g(x)\}$.
...
\item Claim: for all $v\in\reals^\Y_+$, if $H_v$ supports $G$,  $\dim(\pi(F_v)) = \dim(F_v)-1$.
