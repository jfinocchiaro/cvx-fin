...
\begin{definition}[{\citet{gallier2008notes}}]\label{def:irredundant}
	...
	We say that $\cap \H$ is an \emph{irredundant decomposition} for $P$ (and $\H$ is irredundant for $P$) if $P$ cannot be expressed as $P = \cap \H'$ for some set of closed halfspaces $\H'$ such that $|\H'| < |\H|$.
\end{definition}
...
\jessie{Cor: from $\H^*$ we can get $\V^*$}
...
%
  Since $\hyp(g_\V)$ is full-dimensional, the facets of $\hyp(g_\V)$ are uniquely determined by the hyperplanes $H$ such that $H = \partial H^+$ and $H^+ \in \H^*$\raft{was ``$\H^* = \{H^+\}$''} by Lemma~\ref{lem:G-unique-facets-Hstar}.
...
\jessie{What's more, $\V^* \subseteq \V (\subseteq L(\R))$.  Not used anywhere else, so maybe delete anyways?}
\raf{It's actually used in Lemma~\ref{claim:g-gen-by-Vstar} but attributed to Corollary~\ref{cor:unique-set-loss-vectors-defining-facets}; that flow makes sense to me so let's combine the two (change $\V^* \subseteq \reals^\Y_+$ to $\V^* \subseteq \V$ above, and add the proof).}
...
\begin{corollary}
	Given minimizable $L : \R \to \reals^\Y_+$ such that $\risk L_+$ is polyhedral, there exists a (unique) finite $\V^* \subseteq L(\R)$ such that $g_{L(\R)} = g_{\V^*}$ and $\hyp(g_{L(\R)}) = \hyp(g_{\V^*}) = \cap(\H_{\V^*} \cup \H_\Y)$ is irredundant.
\end{corollary}
...
\raf{Given a polyhedron $P$, we denote the face $F^P_v := H_v \cap P$.
