...
\begin{definition}
  \raft{For Bo: added this def, changed def embedding}
  ...
  We further say $\Sc$ is \emph{minimally representative} or a \emph{minimal representative set} if it has the smallest cardinality among all representative sets.
\end{definition}

\begin{definition}[Embedding a loss]\label{def:loss-embed}
  A loss $L:\reals^d\to\reals^\Y$ \emph{embeds} a loss $\ell:\R\to\reals^\Y$ over a set $\Sc$ if there exists some injective embedding $\varphi:\Sc\to\reals^d$ such that
  (i) for all $r\in\Sc$ we have $L(\varphi(r)) = \ell(r)$, and (ii) for all $p\in\simplex,r\in\R$ we have
  ...
  \end{equation}
  We simply say $L$ \emph{embeds} $\ell$ if there is some representative set $\Sc$ such that $L$ embeds $\ell$ over $\Sc$.
\end{definition}
...
\raf{We should probably add the type of $\ell$ and $L$ in each lemma/prop/thm statement, or add something like: ``throughout this section, unless specified otherwise, we use $\ell:\R\to\reals^Y$ and $L:\reals^d\to\reals^\Y$''.}
...
\begin{proof}
  ...
  Suppose $L$ embeds $\ell$, so we have some $\Sc\subseteq \R$ which is representative for $\ell$ and an embedding $\varphi:S\to\reals^d$.
  Since $\Sc$ is representative, for all $p\in\simplex$ we have $\Sc \cap \gamma(p) \neq \emptyset$.
  ...
  Take $\U := \varphi(\Sc)$, and consider $\Sc$ is representative of $\ell$ if and only if $\U$ is representative of $L$, as $\gamma(p) \cap \Sc \neq \emptyset \iff \exists r \in \Sc$ such that $r \in \gamma(p) \iff \exists r$ such that $\varphi(r) \in \Gamma(p) \cap \U \iff \Gamma(p) \cap \U \neq \emptyset$.
  Therefore, we have $\U$ representative of $L|_\U$ since $\Gamma(p)$ is nonempty, and $\U$ is representative of $L$.
%  To see that $\gamma'(p) \neq \emptyset$ for all $p\in\simplex$, note that by the definition of $\gamma$ as the property elicited by $\ell$ we have some $r \in \gamma(p)$, and by the embedding condition~\eqref{eq:embed-loss}, $\varphi(r) \in \Gamma(p)$.
  By Lemma~\ref{lem:loss-restrict}, we see that $\risk{L} = \risk{L|_{\U}}$.
  ...
  \end{equation*}
  for all $p\in\simplex$.
  Moreover, the risks are polyhedral as $\risk{\ell}$ is the pointwise max of a finite set of linear functions.
...
\proposedadd{To verify matching losses, observe $\risk \ell (\delta_y) = \risk L (\delta_y)$ for each $y \in \Y$, and these risks are attained by some report $r$ and its embedding $u_r$, respectively.
Matching optimality is true by construction, since the optimal reports support the epigraph of negative Bayes risk on exactly the same distributions, meaning a report and its embedding are optimal at exactly the same distributions.}
\end{proof}
...
\begin{proof}
  ...
  \proposedadd{Form the set $\Sc$ by iterating over the facets of $E_L$, the epigraph of $-\risk{L}$.
  For each facet of $E_L$, which is full-dimensional, there must be some interior distribution $p$ such that $p \mapsto (p, \inprod{p}{L(u)})$ uniquely supports $E_L$, and such a $u$ must exist or else the Bayes risk is not attained.  Take $\Sc$ to be the collection of such reports $u$, and observe $\Sc$ is representative for $L$.  By Corollary~\ref{cor:representative-embeds-restriction}, $L$ embeds $L|_\Sc$, which is finite by construction.}
...
\begin{proof}
	...
	$\Sc$ is representative of $L$, so Lemma~\ref{lem:loss-restrict} gives us $\risk{L} = \risk{L|_{\Sc}}$, which is polyhedral as $\Sc$ is finite.
\end{proof}
...
\begin{definition}[Finite property]
	\jessie{Should be earlier}
	A property $\gamma:\simplex\toto\R$ %is \emph{redundant} if for some $r,r'\in\R$ with $r \neq r'$, we have $\gamma_r \subseteq \gamma_{r'}$, and \emph{non-redundant} otherwise.  $\gamma$ 
	is \emph{finite} if $\R$ is a finite set.
\end{definition}
...
\begin{lemma}\label{lem:embedded-level-sets-trim}
	...
	\proposedadd{RETRY: Let $\Gamma$ be an elicitable property.
		If $\Gamma$ embeds a finite property $\gamma : \simplex \toto \R$ by the injection $\varphi$, then there is a set $\Sc$ such that $\Sc$ is representative for $\gamma$\jessiet{Need to define this, technically} and $\{\gamma_r : r \in \Sc\} = \{\Gamma_u : u \in \varphi(\Sc)\} = \trim(\Gamma)=\trim(\gamma)$.}
\end{lemma}
\begin{proof}
	Let $L$ elicit $\Gamma$.
...
\proposedadd{First, construct $\Sc$ as in the proof of Proposition~\ref{prop:embed-bayes-risks}; this is a minimal representative set in the sense that removing any element $r$ corresponding to the facet $F_r$ from $\Sc$ leads to $\gamma(p) \cap \Sc = \emptyset$ for all $p \in \inter{F_r}$. Moreover, we have $r \in \gamma(p) \iff \varphi(r) \in \Gamma(p)$ by embedding, and therefore $\gamma_r = \Gamma_{\varphi(r)}$, yielding $\{\gamma_r \mid r \in \Sc\} = \{\Gamma_{\varphi(r)} \mid r \in \Sc\}$ as this is true for all $r \in \Sc$.}

\proposedadd{TO SHOW: $T := \{\gamma_r \mid r \in \Sc\} = \trim(\gamma)$. 
	...
	Therefore, for any $p \in \inter{\theta}$ (and one such $p$ must exist), the hyperplane $p \mapsto (p, \ell(r'))$ uniquely supports the hyperplane.
...
\begin{proof}
	...
	By Lemma~\ref{lem:embedded-level-sets-trim}, we then observe that there is a minimal representative set $\Sc$ such that $\trim(\Gamma) = \{\Gamma_{\varphi(r)} : r \in \Sc \} = \{\gamma_r : r \in \Sc\}$.
	...
	Moreover, as $\gamma$ is nondegenerate (recall that $\gamma: \simplex \toto \R$ is shorthand for $\gamma: \simplex \to 2^\R \setminus \{\emptyset\}$ and $\Sc$ is representative), we have that $\cup \trim(\Gamma) = \cup_{r \in \Sc} \gamma_r = \simplex$.\jessie{Needs more/different justification, since we union over $\Sc$ instead of $\R$ }
