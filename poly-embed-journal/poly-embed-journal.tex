...
\jessie{Adding new statements}
\begin{definition}
	Suppose a generic loss $L : \R \to \reals^\Y$ is given with polyhedral risk $-\risk{L}$.
...
\begin{proposition}
	If loss $L: \R \to \reals^\Y_+$ has polyhedral risk, $\Sc(L)$ is a minimal representative set for $L$.
\end{proposition}
\begin{proof}\jessiet{I don't like this proof, but meh.}
	If $\Sc(L)$ was not a minimal set, then we could remove some $s$ from $\Sc$ such that $\Sc' := \Sc \setminus \{s\}$ is also a representative set for $L$.
	By construction, we have some distribution $p$ such that the hyperplane $p \mapsto (p, L(s))$ uniquely supports $E_L$.
	We particularly have $\Gamma(p) \cap \Sc' = \emptyset$, since any other element $s'$ of $\Sc'$ being in $\Gamma(p)$ would imply $p \mapsto (p, L(s'))$ also supports $E_L$, but this hyperplane must be unique, and therefore either $s$ or $s'$ would not be in $\Sc$.
\end{proof}

\jessie{End additions}
...
\begin{proof}
  ...
  Suppose $L$ embeds $\ell$, so we have some $\Sc\subseteq \R$ which is representative for $\ell$ and an embedding $\varphi:\Sc\to\reals^d$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Minimal Representative Sets}
...
\raf{Some thoughts on this section:
  ...
  \item It seems nice to point out and explore the fact that our notion of embedding is, now, symmetric.
    ...
    \item Moreover, since we've already established (Prop~\ref{prop:embed-bayes-risks}) that if $L$ embeds $\ell$ over $\Sc$, then $\Sc$ being representative for $\ell$ implies $\varphi(\Sc)$ is representative for $L$, meaning $L$ and $\ell$ are equivalent under this definition.
...
\begin{lemma}\label{lem:embed-defs-equiv}
	%  \raft{Introduce $L$}
	Let $L : \reals^d \to \reals^\Y_+$ be a loss function \proposedadd{whose infimum is attained in expectation for all $p \in \simplex$}\raft{Ah, good call! \#burn}.
