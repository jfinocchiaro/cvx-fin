...
\begin{lemma}\label{lem:G-unique-facets-Hstar}
  ...
  There is a unique irredundant set $\H^*$ such that $\hyp(g_{L(\R)}) = \cap \H^*$. 
...
\begin{proof}
  ...
  Since $\hyp(g_{L(\R)})$ is full-dimensional, take $\H^*$ to be the unique irredundant and finite set of closed halfspaces such that $\hyp(g_{L(\R)}) = \H^*$ from Theorem~\ref{thm:polyhedron-uniquely-gen-facets}(i).
  Now, $\hyp(g_{L(\R)}) \cap H$ being a facet follows immediately from Theorem~\ref{thm:polyhedron-uniquely-gen-facets}(ii) and (iii).
  
  To see $\H_\Y \subseteq \H^*$, if there was a $y \in \Y$ such that $H^+_y = \{(x,c) \mid x_y \geq 0\}$ was not in $\H^*$, then we would either have some $c_1 > 0$ such that $\{(x,c) \mid x_y \geq c_1\} \in \H^*$, or we have a point $x$ such that $x_y < 0$ but $g_{L(\R)}(x) > -\infty$.
  The first cannot happen as $g_{L(\R)}$ is concave and finite (namely, nonnegative) at $x = e_y$.
  The second cannot occur as $g_{L(\R)}$ is only finite on $\reals^\Y_+$ by construction with the convex indicator. \raft{TODO: Couldn't follow this last line, since $\delta$ can be $\infty$\jessie{does it help that it's minus the convex indicator?} 6/20/2022: still don't get it, sorry :-)... \jessie{Yeah I changed the sentence entirely.  better? \# burn}}. 
\end{proof}
...
\begin{corollary}\label{cor:unique-set-loss-vectors-defining-facets}
  ...
  (iv) $g_{L(\R)}(x) = \min_{v \in \V^*}\inprod{v}{x} - \delta(x \mid \reals_+^\Y) = g_{\V^*}(x)$.
\end{corollary}
\begin{proof}
...
%
  ...
  (ii) Moreover, $H_v \in \H_{\V^*} \subseteq \H^*$ generates the facet $F_v = H_v \cap \hyp(g_{\V})$ of $\hyp(g_{L(\R)})$ by Lemma~\ref{lem:G-unique-facets-Hstar}.
%
%  The moreover follows immediately since $\H_{\V^*} \subseteq \H^*$ and every $H^+ \in \H^*$ defines a facet by Lemma~\ref{lem:G-unique-facets-Hstar}.
  ...
  (iv)   The result holds if $\hyp(g_{L(\R)}) = \hyp(g_{\V^*})$.
  By construction, $\hyp(g_{L(\R)}) = \cap (\H_\Y \cup \H_\V)= \cap \H^* = \cap (\H_\Y \cup \H_{\V^*}) = \{(x,c) \in \reals_+^\Y \times \reals \mid \inprod{v^*}{x} \geq c$ for all $v^* \in \V^* \}$ where the first equality follows as $\H^* \subseteq \H$.
...
\end{lemma}
\begin{proof}
	For $v \in L(\R)$, define $F_v := F_v^{\hyp(g_{L(\R)})}$.
	\begin{align*}
...
\begin{proof}
	...
	&= \{x \in \reals^\Y_+ \mid \inprod{L(r)}{x} = g_{L(\R)}(x)\} & \text{Assumption~\ref{assum:V-star-exists-infinite}: $\risk L_+(x) = g_{\V^*}(x)$}\\
