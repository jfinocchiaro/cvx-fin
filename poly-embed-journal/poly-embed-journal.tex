...
\begin{proof}
	...
	Observe that $\V^*$ is unique and irredundant (by assumption) and $\pi(F^{\hyp(g_\V)}_v)$ is full-dimensional in $\reals^\Y_+$ by Corollary~\ref{cor:anything-gen-G-subset-Hstar} and Claim~\ref{claim:pi-preserves-dim}.
	Moreover, $\pi(F_v^{\hyp(g_\V)}) \not \in \cup_{v' \in \V'} \pi(F_v^{\hyp(g_\V)})$, which implies $\cup_{v' \in \V'} \pi(F_v^{\hyp(g_\V)}) \neq \cup_{v^* \in \V^*} \pi(F_{v^*}^{\hyp(g_\V)}) = \reals^\Y_+$.
...
%	% ...$\leq \inprod{v}{x}$ for all $v \in \V$, so $(x,g(x)) \in \cap \H_\V \implies (x,g(x)) \in G = \cap (\H_\Y \cup \H_\V)$.
	...
	($\impliedby$)
	Since $\V^* \subseteq \V'$, we immediately have \jessie{todo...}
