...
\newcommand{\trimred}{\mathrm{trim}_{{\tt red}}}
...
\raft{I think good as-is, since in both cases these follow from more specific results}
...
A first observation is that a loss restricted to some representative set $\Sc$ elicits the same property restricted to $\Sc$.
...
For example, a modified hinge loss $L(r)_y = \max(r^2-1,1-ry)$, which matches hinge loss on the interval $[-1,1]$ but is smooth outside the interval $[-2,2]$, still embeds twice 0-1 loss.
...
We now present our main structural result, which will lay the foundation for the rest of this section.
\raf{TODO: Brief description of the result}
...
\begin{corollary}\label{cor:poly-risk-fin-rep}
  ...
  \end{enumerate}
\end{corollary}
As ``$L$ has a finite representative set'' is a more succinct condition than ``$L$ is minimizable and $\risk{L}$ is polyhedral'', we will use the former in the sequel.
(Recall that having a finite representative set already implies minimizability.)


\subsection{Equivalent condition: matching Bayes risks}
...
\begin{proof}
  ...
  From Lemma~\ref{lem:X}(\ref{item:X-min-V}), we have some set $\V\subseteq\reals^\Y_+$ and minimum representative sets $\R^* \subseteq \R$ and $\Sc \subseteq \U$, for $\ell$ and $L$ respectively, such that $\ell(\R^*) = \V = L(\Sc)$.
  As $\R^*$ and $\Sc$ are miniumum, they cannot repeat loss vectors, and thus $|\R^*|=|\ell(\R^*)|$ and $|L(\Sc)|=\Sc$.
  We conclude that $\R^*$ and $\Sc$ are both in bijection with $\V$.
  The map $\varphi :\R^* \to \reals^d$, given by $\varphi(r) = s \in \Sc$ where $\ell(r) = L(s)$, is therefore well-defined.
  Condition (i) of an embedding is immediate.
  From Proposition~\ref{prop:representative-embeds-restriction}, $\ell$ embeds $\ell|_{\R^*}$ and $L$ embeds $L|_\Sc$, both via the identity embedding.
  Using condition (ii) from both embeddings, for all $p\in\simplex$ and $r\in\R^*$, we have
  \begin{equation*}
    r \in \gamma(p) \iff r \in \prop{\ell|_{\R^*}}(p) \iff \varphi(r) \in \prop{L|_\Sc}(p)
...
\end{proposition}
\begin{proof}
  \raft{This is sort of a ``clever'' proof, probably not in a good way.  Something that used Lemma X more directly would probably be better than using Bayes risk matching.  But if correct I think it's not bad.}
  ...
  We conclude $\risk L = \risk{\ell_\trim} = \risk \ell$ from Proposition~\ref{prop:embed-iff-trims-equal}.
...
We now relate this condition to another natural notion from the property elicitation literature, that of non-redundancy~\cite{frongillo2014general}\raft{TODO: see if there are other references we should include here.  Maybe Jessie you could search?}.
...
\begin{proof}
  ...
  Now Lemma~\ref{lem:X}(\ref{item:X-redundant}) gives some $s\in\Sc$ such that $\Gamma_r \subseteq \Gamma_s$, which contradicts $L|_{\R'}$ being non-redundant.
  We conclude $\Sc=\R'$, meaning $\R'$ is a minimum representative set.
\end{proof}

\begin{corollary}\label{cor:tight-embed-min-rep}
  Let loss $L:\R\to\reals^\Y_+$ and finite representative set $\Sc$ be given.
  Then $L$ tightly embeds $L|_\Sc$ if and only if $L|_\Sc$ is non-redundant.
\end{corollary}


\begin{corollary}\label{cor:trim-loss-red}
  Let $L : \R \to \reals^\Y_+$ have a finite representative set.
  Then $\trimcover(L) = \{L(r) \mid r \in \R,\; \forall r'\in\R,\; \Gamma_r = \Gamma_{r'} \textnormal{ or } \Gamma_r \not\subseteq \Gamma_{r'}\}$.
  % $\mid u \in \R \textrm{ s.t. } \neg\exists u'\in\R,u'\neq u,\, \Gamma_u \subsetneq \Gamma_{u'}\}$
\end{corollary}
\begin{proof}
  \raft{I was a bit surprised how involved this proof ended up being.  Quite possibly there is a faster way, though at the same time, I do like how the proof shows something stronger.  Old version is in before-reorg-sec-3.tex.}
  Let $\A := \{r \in \R \mid \forall r'\in\R,\; \Gamma_r = \Gamma_{r'} \textnormal{ or } \Gamma_r \not\subseteq \Gamma_{r'}\}$.
  We will show the stronger statement, $\A = \bigcup\{\Sc\subseteq\R \mid \Sc \textnormal{ is minimum representative for } L\}$.
  The result will then follow from the definition of trim.

  Let $\Sc$ be a minimum representative set for $L$.
  Observe that $\Gamma_s \not\subseteq \Gamma_{s'}$ for all $s,s'\in\Sc$, $s\neq s'$, as otherwise $\Sc$ would not be minimum representative.
  Furthermore, as $\risk L$ is polyhedral (Corollary~\ref{cor:poly-risk-fin-rep}), Lemma~\ref{lem:X}(\ref{item:X-min-Theta},\ref{item:X-redundant}) together state that for all $r\in\R$, we have some $s\in\Sc$ such that $\Gamma_r \subseteq \Gamma_s$.
  Let $s\in\Sc$.
  Suppose for a contradiction that $\Gamma_s \subsetneq \Gamma_r$ for some $r\in\R$.
  From the above we have some $s'\in\Sc$ with $\Gamma_r \subseteq \Gamma_{s'}$.
  But now $\Gamma_s \subsetneq \Gamma_r \subseteq \Gamma_{s'}$, contradicting $\Sc$ being minimum representative.
  We conclude that, for all $r\in\R$, either $\Gamma_s = \Gamma_r$ or $\Gamma_s \not\subseteq \Gamma_r$.
  Thus $s\in\A$.

  For the reverse inclusion, let $r\in\A$.
  ...
  By definition of $\A$, we conclude $\Gamma_r = \Gamma_s$.
  ...
  As $r\in\Sc'$ we are done.
\end{proof}



\subsection{A property elicitation perspective}

We conclude this section with a similar structural result about the properties elicited or embedded by a surrogate.
...
When working with properties, it is perhaps more natural to define ``trim'' to remove any level sets which are redundant in the sense of set inclusion, as in Corollary~\ref{cor:trim-loss-red}.
Precisely, given a property $\Gamma:\simplex\toto\R$, define
\[ \trimred(\Gamma) := \{\Gamma_r \mid r \in \R \textrm{ s.t. } \neg\exists r'\in\R,r'\neq r,\, \Gamma_r \subsetneq \Gamma_{r'}\}~.\]
...
\begin{restatable}{proposition}{embedtrim}\label{prop:embed-trim}
  ...
  Moreover, when any of the above hold, $\trimred(\gamma) = \trimred(\Gamma) = \{\Gamma_u \mid u\in\U\} = \Theta$.
\end{restatable}

\begin{proof}
  Let $L$ elicit $\Gamma$.
  Observe $L$ is minimizable by definition of elicits.
  In each case, we will show that $\risk L$ is polyhedral, and thus Lemma~\ref{lem:X} will give us uniquely determined sets $\V$ and $\Theta$.

  In case 1, in particular $\Gamma$ has a finite representative set $\Sc$, and $\varphi(\Sc)$ is representative for $\gamma$.........
\end{proof}

We defer the proof, and precise definitions, to \S~\ref{app:embed-props}.
...
By Rockafellar, we have $f$ polyhedral if and only if it can be written as the max of a finite set of linear functions $\V$ over $\reals^\Y_+$. 
Therefore, $f(p) = \min_{v\in\V} \inprod{p}{v}$ over $p \in \reals_+^\Y$. 
We extend $f$ to $g(x) = \min_{v\in\V} \inprod{x}{v}$ for $x \in \reals^\Y_+$ and $g(x) = \infty$ otherwise, and define $g(0) = 0$. 
That is, $g(x) = \min_{v \in \V} \inprod{x}{v} + \delta(x \mid \reals^\Y_+)$, which is also polyhedral by Rockafellar. 
...
Abusing notation, we will say $H_y^+ := \{(x,c) \mid x_y \geq 0\}$, and $H_y := \{(x,c) \mid x_y = 0\}$ for $y \in \Y$. %\jessie{$H_y = H_{e_y}$ doesn't quite match up with what was here before; before, $H^+_y\{(x,c) \mid x_y \geq 0\}$, where this now has a $-c$ in that term.}
For any set of vectors, $Z$, let $\H_{Z} = \{H_z^+ \mid z\in Z\}$, and denote $\H = \H_{\V} \cup \H_\Y$\footnote{Slightly abusing notation, we denote $\H_\Y := \{H_y \mid y \in \Y\}$} as the set of halfspaces defining $G$; $\H_{\hat V}$ defines $G$ on $\reals^\Y_+$, and $\H_\Y$ determines $G$ outside  the nonnegative orthant.
...
%Consider the set $\V^* := H' \setminus e_\Y$\footnote{$e_\Y := \{e_i\}_{i=1}^n$}. \jessie{Doesn't quite type check, hence the comment out}
...
%\jessie{by analog of Ziegler Theorem 2.15; more directly by Gallier notes \url{https://arxiv.org/pdf/0805.0292.pdf} Prop 4.5}
...
\begin{definition}[Polyhedron - halfspace representation\jessie{Ziegler page 28}]
...
%	\begin{equation*}
%	P = P(A,z) = \{x \in \reals^d \mid Ax \leq z\} \qquad \textrm{ for some } A \in \reals^{m \times d}, z \in \reals^m~.~
%	\end{equation*}
\end{definition}
...
\begin{theorem}\label{thm:polyhedron-uniquely-gen-facets}
	Given a $d$-dimensional polyhedron $P \subseteq \reals^d$, there is a unique finite set of closed halfspaces $\H^*$ such that $P = \cap \H^*$, and for all sets of closed halfspaces $\H$ such that $P = \cap \H$, we have $\H^* \subseteq \H$.
	Moreover, $\{H^+\cap P \mid H^+ \in \H^*\}$ is the set of facets of $P$.
	% Given a $d$-dimensional polyhedron $P \subseteq \reals^d$, there is a unique finite set of closed halfspaces $\H = \{H^+_{(w_1, b_1)}, \ldots, H^+_{(w_k, b_k)}\}$ such that $P = \cap \H$, and for all sets of closed halfspaces $\H'$ such that $P = \cap \H'$, we have $\H \subseteq \H'$.
	% Moreover, for each $i \in [k]$, letting $F_i := H_{(w_i, b_i)} \cap P$, the set of $\{F_1, \ldots, F_k\}$ is the set of facets of $P$.
\end{theorem}
\begin{proof}
	\jessie{TODO}
\end{proof}
\jessie{Need to find a good reference.  Cite a result for polytopes and transform the polyhedra into a polytope. Could state the result for class of polyhedra containing $G$.
	Add two facets and now it's a polytope.
}


\subsection{Phase 1: construct $\V^*$}
Suppose we have a finite set $\V \subset \reals_+^d$.
...
\begin{definition}
	...
	taking $g(0) = 0$.
\end{definition}
%Extend to $g(x) = \min_{v\in\V} \inprod{x}{v}$ for $x \in \reals^\Y_+$ and $g(x) = -\infty$ otherwise.  Def $g(0) = 0$.


\begin{itemize}
\item Define $G = \{(x,c) \mid c \leq g(x)\} \subseteq \reals^d \times \reals$ to be the hypograph of $g$.
\item We define $H^+_v := H^+_{([v;-1], 0)}$ for a $v \in \reals^d_+$, where $[v; -1]\in\reals^d_+\times\reals$.
\item Additionally, denote $H_y^+ := H_{([e_y; 0], 0)}^+$ for any $y \in [d]$. % \{ (x,c) \in \reals^\Y \times \reals \mid x_y \geq 0\}$.
\item Again, we let $H_v := H_{([v;-1], 0)}$ for $v \in \reals^d_+$. 

\item Define $F_v := H_v \cap G$ for any $v\in\reals^d_+$.

%\item Let $H_y^+ := \{ (x,c) \in \reals^\Y \times \reals \mid x_y \geq 0\}$.
\item Let $\H_{\V} = \{H_v^+ \mid v\in\V\}$, $\H_\Y = \{H_y^+ \mid y\in\Y\}$, $\H = \H_{\V} \cup \H_\Y$.
\end{itemize}
...
\end{claim}
\begin{proof}
	$(x,c) \in G \iff g(x) - c \geq 0 \iff \min_{v \in \V}\inprod{v}{x} - c \geq 0$ and $x \in \reals_+^d$, which is true if and only if $\inprod{v}{x} - c \geq 0 \,\forall v \in \V$ and $x_y \geq 0$ for all $y$.
	In turn, this statement holds if and only if $(x,c) \in H^+_v$ for all $v \in \V$ and in $H^+_y$ for all $y \in \Y$, so $(x,c) \in \cap \H$.
\end{proof}

\begin{claim}
	$x \in \reals^d_+ \iff (x,c)\in \cap \H_\Y$ for all $c \in \reals$.
\end{claim}
\begin{proof}
	$\implies$ 
	Fix any $c \in \reals$.
	$x \in \reals^d_+ \iff x_y \geq 0$ for all $y \in \Y$.
...
Some statements about facets and H-rep
 
\begin{claim}
	Claim: $G$ is $(d+1)$-dimensional.  %\raf{$g$ is nonnegative on $\reals^\Y_+$, so $G$ contains $\{(x,c) \mid x\in\reals^\Y_+, c \leq 0\}$, which is full-dim.}
\end{claim}
\begin{proof}
...
\begin{claim}
	...
	%Since $G$ is a full-dimensional polyhedron, with a unique H-rep by [cite].
	There is some unique $\H^* \subseteq \H$ such that $G = \cap \H^*$. 
	%\jessie{$\H^* \subseteq \H$ since $G = \cap \H^* = \cap \H$; something about smallest cardinality here?}
	Moreover, for each $H_{(w_i, b_i)} \in\H^*$, the face $F_{(w_i, b_i)} = H_{(w_i, b_i)} \cap G$ is a facet. %\raf{make formal}
\end{claim}
  \begin{proof}
  	Since $G$ is full-dimensional, this follows immediately from Theorem~\ref{thm:polyhedron-uniquely-gen-facets}.
%	We know $G$ has a halfspace representation $G = G(H', \vec 0)$ uniquely determined by the facets of $G$ by \citep[Proposition 4.5]{gallier2008notes} \jessie{Another reference: Ziegler Proposition 2.15 analagous extension}.
%	Take $\H' := \H_{H'}$; we claim that $\H' \subseteq \H$ and $G = \cap \H'$. \jessie{$\H' \subseteq \H$ seems to follow from Proposition 8.5 from \url{https://people.inf.ethz.ch/fukudak/lect/pclect/notes2014/PolyComp2014.pdf}}
  \end{proof}
...
\begin{proof}
	...
	The first cannot happen as we take $g$ to be defined (and finite) on $x = e_y \in \reals^d_+$.
	Moreover, the second cannot be true by construction of $g$ including the $0-\infty$ indicator on $\reals^d_+$.	
\end{proof}
...
Since $G$ is full-dimensional, all facets of $G$ are uniquely determined by the hyperplanes $H$ whose halfspaces $H^+$ compose $\H^*$.
Any facet $F$ must then be some intersection of an $H_y \cap G$ or $H_v \cap G$.
Take $\V^*$ to be the set of such $H_v$ hyperplanes.
%Since $\H_\Y \subseteq \H^*$, we can consider $\H_{\V^*} := \H \setminus \H_{\Y}$, and $\V^*$ to be the set generating $\H_{\V^*}$.
...
%	First, observe that by construction, $H_v = \{(x,c) \mid \inprod{v}{x} - c \geq 0\}$ is a valid inequality for $G$, and thus $F_v$ is a face of $G$ by definition.
%	To see that $F_v$ is a facet, we must show it is $\dim(G) - 1 = |\Y|$-dimensional.
%	This holds true if we can fit a $|\Y|$-dimensional ball in $F_v$.
%	If we could not fit such a ball in $F_v$, \jessie{contradict $\V$ minimum cardinality}
\end{proof}
...
\begin{claim}
	$g(x) = \min_{v \in \V^*}\inprod{v}{x} - \delta(x \mid \reals_+^d)$
\end{claim}
\begin{proof}
	$G = \cap \H = \cap \H^* = \cap (\H_\Y \cup \H_{\V^*}) = \{(x,c) \in \reals_+^d \times \reals \mid \inprod{v^*}{x} \geq c$ for all $v^* \in \V^* \}$.
	Since $G$ is the hypograph of $g$, this means $g$ can be written as 
	$g(x) = \min_{v \in \V^*}\inprod{v}{x} - \delta(x \mid \reals^d_+)$. 
\end{proof}

\subsection{Projections to the positive orthant}
...
\end{claim}
\begin{proof}
%We can write any face $F = H \cap G = \{(x',c) \in \reals_+^{d} \mid \inprod{x'}{v} = c = g(x')\}$.
%Since $H_v^+$ must be a valid halfspace to define a face, we know that $G \subseteq H_v^+$.
%%If there was no $v \in \V^*$ such that $x \in F_v$, 
%Since $\H^* = \H_\Y \cup \H_{\V^*}$, if there was no $v \in \V^*$ such that $x \in F_v = F$, it must be the case that $\min_{H_v \in \H_\Y} \inprod{v}{x} = \min_{y \in \Y} x_y = g(x) = 0$ by construction of the halfspaces in $\H_\Y$. %, and we must have $x \not \in \reals_{++}^\Y$.
%
%\jessie{If $g$ is not locally constant}
%Since $g$ is continuous on $\reals^d_+$ (as it is convex), and not constant \jessie{??}, there must be some $x' \in B(x,\epsilon) \cap \reals_+^\Y$ such that $g(x') > 0$ and therefore no $y \in \Y$ would define a face $F_{v^*}$ supporting $G$ at $x'$.
%(We know such a face exists as $G$ is polyhedral and finite.)
%That $v^*$ must then be in $\V^*$, and we claim $\inprod{v^*}{x} = 0$, which would imply $H_{v^*}$ also supports $G$ at $x$.
%If $\inprod{v^*}{x} > 0 = g(x)$ (recall $g$ nonnegative on $\reals_+^\Y$) then we contradict continuity of $g$ on $\reals_+^\Y$. 
%Therefore, we must have $\inprod{v^*}{x} = 0$, and thus $H_{v^*}$ also supports $G$ at $x$.
%\jessie{Not the level of rigor that we want.  Also using lots of polyhedral structure here.}
%%First, $\inprod{v^*}{x} \geq 0$ by definition of $g$ nonnegative on $\reals_+^\Y$, and $\leq 0$
...
By Claim 7\jessie{make reference}, we know that $g(x) = \min_{v \in \V^*}\inprod{v}{x}$.
In particular, consider $v^* \in \argmin_{v \in \V^*}\inprod{v}{x}$; we claim that $H_{v^*}$ supports $G$ at $(x,g(x))$.
First, $G \subset H^+_{v^*}$ by definition of $G$ as the intersection of halfspaces.
...
%\jessie{Claim: $x$ must be on boundary of $\reals_+^\Y$ and therefore $g(x) = 0$, and we have a matchy matchy with a $v^* \in \V^*$.}

%\jessie{If $x \in \reals_{++}^\Y$, then}

%As these halfspaces are vertical, we have $g(x) = -\infty$, which does not support $G$ at $(x, g(x))$ since $x \in \reals_+^\Y$, yielding a contradiction.
%\jessie{Don't like this. Slash not true.  There's something implicit about all the normals being in $\R^d_-$ and $x \in \reals^d_+$.  Haven't proven the first claim}
%We claim $\cup_{v \in \V}\pi(F_v) = \reals_+^\Y$.
%If there was a $v \in \V$ such that $\pi(F_v) \not \subseteq \reals_+^\Y$, this would imply there is some $x \not \in \reals_+^\Y$ such that $\inprod{x}{v} = c < \infty = g(x)$, contradicting $F_v$ supporting $g$ at $x$.
%Thus, $\cup_{v \in \V} \pi(F_v) \subseteq \reals_+^\Y$.
%Now for the other direction of subset inclusion, observe that for all $x \in \reals^\Y_+$, we have $g(x) = f(x) = \min_{v \in \V}\inprod{v}{x}$, so there is a $v \in \V$ supporting $g$ at $x$, and therefore $x \in \pi(F_v)$ for some $v \in \V$, and is therefore in the union of projections.
...
\begin{corollary}
	...
	% \jessie{Not sure how to show this, but intuitively, this is saying that the projection of the facets union to $\reals^\Y_+$.}
\end{corollary}

\raf{From notes 6/21/2021 COMMENTED OUT}


\begin{itemize}
	\item REP $\iff$ $\V^* \subseteq L(R')$
	\item REP $\iff \Theta \subseteq \{\Gamma_r | r \in \R'\}$.
	\item $\Theta$ == Full-dim
	\item Lev sets subset $\theta$
\end{itemize}


\begin{claim}[6/21/2021]
  For $\V' \subseteq \V$, we have
  $\cup_{v\in\V'} \pi(F_v) = \reals^\Y_+ \iff \V^* \subseteq \V'$.\jessie{Will get us to REP $\iff$ $\V^* \subseteq L(R')$}
\end{claim}
\begin{proof}
  Would like to show
  $\cup_{v\in\V'} \pi(F_v) = \reals^\Y_+ \iff \cap(\H_\Y \cup \H_{\V'}) = G$ then apply Corollary 7\jessie{make reference}.

  Suppose $\cup_{v\in\V'} \pi(F_v) = \reals^\Y_+$.
  Take any $x\in\reals^\Y_+$.
  Then there exists $c \in \reals$ and $v\in\V'$ such that $(x,c) \in F_v$, and $c = \inprod{v}{x}$.
  ...
  $(x,c) \in \H_\Y$ for all $c \in \reals$ by previous Claim \jessie{ref}, so we just need to show that $(x,c) \in \cap \H_{\V'} \implies (x,c) \in G$.
  
  We know there exists a $v' \in \V'$ and $c \in \reals$ such that $(x,c) \in F_{v'}$, and $c = \inprod{v'}{x} = g(x)$.
  Therefore $\inprod{v'}{x} = c = g(x) \leq \inprod{v}{x}$ for all $v \in \V$, so $(x,c) \in \cap \H_\V \implies (x,c) \in G = \cap (\H_\Y \cup \H_\V)$.
  
  $\impliedby$
  $G = \cap (\H_\Y \cup \H_{\V'})$ if and only if $G = \cap \H_{\V'}$ for all $x \in \reals_+^\Y$.
...
\subsection{Phase 2: Now given a minimizable loss $L:\R\to\reals^\Y_+$ with $\risk L_+ = g$ (1-homog extension).}


For $x\in\reals^\Y_+$,
$\risk{L}_+(x) := \inf_{r\in\R} \inprod{x}{L(r)}$

\begin{itemize}\setlength{\itemsep}{0pt}
\item Define $\V = L(\R) \subseteq \reals^\Y_+$ and $\H_{\V} = \{H_v^+ \mid v\in\V\}$.  Unlike above, these could be infinite sets.
\item Let $\H = \H_\Y \cup \H_{\V}$.
\end{itemize}
\begin{claim}
	$G = \cap\H$ by definition of $\risk{L}_+$.
\end{claim}
\begin{proof}
%  \raf{Let $(x,c) \in G$.  Show $(x,c)\in\cap \H$.  Then vice versa.  Or maybe iff once you get the hang of it.}
%  $(x,c) \in G = \{(x,c) \mid g(x) \geq c\}$.
%  Since $g(x) = \risk L_+(x)$, we then have $\min_{v \in \V}\inprod{x}{v} - \delta(x \mid \reals^\Y_+) = \inf_{r \in \R} \inprod{x}{L(r)}$\jessie{minnable doesn't really come in here, but want to be careful}.
%  Thus, for all loss vectors such that $v = L(r)$ we have $\inprod{v}{x} = \inprod{L(r)}{x} \geq c$, and therefore $(x,c) \in H^+_v$ for all $v \in L(\R)$, and is therefore in the intersection $H_v^+$.
  
  Observe that $x \in \reals^\Y_+ \iff (x, c) \in \cap \H_\Y$.
  Moreover,
  \begin{align*}
  (x,c) \in G &\iff g(x) \geq c \\
  &\iff \inprod{v}{x} \geq c \,\, \forall v \in \V \\
  &\iff (x,c) \in H^+_{v} \,\, \forall v \in L(\R) \\
  &\iff \inprod{L(r)}{x} \geq c \,\, \forall r \in \R\\
  &\iff (x,c) \in H^+_{L(r)} \,\, \forall r \in \R \\
  &\iff (x,c) \in \cap \H_\V \\ 
  \end{align*}
  Combining the two equalities (e.g., $\cap \H = (\cap \H_\Y) \cap (\cap \H_\V)$), we have $G = \cap \H$.

%  $\implies$
%  Have $g(x) \geq c$... use $g=\risk{L}_+$... then for all loss vectors $v = L()$, $(x,c) \in H_v^+$.  Done.
%
%  Other dir: for any $(x,c) \in \cap \H$
\end{proof}

Need to use minimizable
Take $\V^*$ to be the unique finite set such that $\H^* = \H_\Y \cup \H_{\V^*}$, where $\H^*$ is the unique minimum halfspace representation for $G$ as in phase 1.

\begin{claim}
	$\H_{\V^*} \subseteq \H$
\end{claim}
\begin{proof}
	$\H_{\V^*} \subseteq \H^* \subseteq \H$.
	The first subset inclusion follows by construction of $\H_{\V^*}$, and the second follows from Theorem~\ref{thm:polyhedron-uniquely-gen-facets} as $\H^*$ is the unique minimum halfspace representation of $G$, and is finite.
	%%$G = \cap \H^*$ and $\H_{\V^*} \subseteq \H$.
	%since $\H_\Y \cup \H_{\V^*}$ is the unique min H-rep of $G$. \raf{not sure how to do this, since $\H$ could be infinite}
\end{proof}

\begin{corollary}
	There is a set $\R^* \subseteq \R$ such that $L(\R^*) = \V^*$ (without duplicates).
\end{corollary}

\begin{claim} 
For all $v\in\reals^\Y_+$, if $H_v$ supports $G$, then $\dim(\pi(F_v)) = \dim(F_v)$.
\end{claim}
\begin{proof}

	\jessie{Think of the projection as a linear map and affine hull as a subspace.   Pick any point in relint of the facet, at the linear span, ...}
	\jessie{$\affhull(F_v) = H_v$, $\dim(\affhull(\pi(F_v))) = \dim(\pi(\affhull(F_v)))$.   Show that when you project $H_v$, it gives you $\reals^\Y$.}
	
	\jessie{Meeting notes: See \url{http://homepages.cae.wisc.edu/~linderot/classes/ie418/lecture11.pdf} for def of AI} just need to argue that you never needed $c$ (i.e., $c$ was always just $g(x)$).  Maybe: take an affine basis, project, and show it's an affine basis. 
	Suppose we are given $|\Y|+1$ affinely independent vectors $z_i$ in $F_v$.  %, so we get $|\Y| + 1$ affinely independent vectors. 
	We claim $\{\pi(z_i)\}$ are affinely independent.
	Let $a_1 + \ldots + a_{\Y+1} = 0$, such that $\sum_i a_i \pi(z_i) = 0$.
	Want to conclude $a_i = 0$ for all $i$.
	
	Observe $z_i = (x_i, \inprod{v}{x_i})$ for all $i$ since $z_i \in F_v$, and therefore in $H_v$.
	So $0 = \sum_i a_i \pi(z_i) = \sum_i a_i x_i$.
	
	Moreover, the sum $\sum_i a_i z_i = \sum_i a_i (x_i, \inprod{v}{x_i}) = (\sum_i a_i x_i, \inprod{v}{\sum_i a_i x_i}) = (0,0) = \vec 0$.
...
\begin{claim}If a $H_v \in \H$ does support $G$, then $F_{v} = H_{v} \cap G$ is a face of $G$, and thus a subset of a facet.
\end{claim}
\begin{proof}
	Since $H_v$ supports $G$, we know that $G \subseteq H_v^+$ (WLOG) and $F_v$ is not empty.
	...
	\jessie{Gr\"unbaum Page 35, Section 3.1 statement 7}
\end{proof}
...
\end{claim}
\begin{proof}
$\{F_{v^*}\}_{v^* \in \V^*}$ is exactly the set of facets on $\reals_+^\Y$. \jessie{Cite}
Any $k$-face is contained in a facet (claim above).
Must therefore be one of these.

If it as a subset of a vertical facet, we would have $F_v = H_v \cap G \subseteq H_y \cap G$ for some $y \in \Y$.
Expanding, this is $\{(x,g(x)) \mid \inprod{v}{x} = g(x)\} \subseteq \{(x,g(x)) \mid x_y = 0 \}$.
While this can happen, it must be on the boundary of $\reals_+^d$, and must be at the intersection of $H_y$ and some other $H_{v^*}$ for $v^* \in \V^*$. \jessie{Justify}
\end{proof}

\begin{corollary}
	For $v,v^*$ such that $F_v \subseteq F_{v^*}$ and $v^*\in \V^*$, $\pi(F_{v}) \subseteq \pi(F_{v^*})$. \raf{This will show (3) once we translate back}
\end{corollary}
...
In particular, take $f$ to match $g$ on $\simplex$.

Define the function $\theta(v) = \{p \in \simplex \mid \inprod{v}{p} = f(p)\}$, and the set $\Theta := \{\theta(v) \mid v \in \V^*\}$.

\begin{claim}
  $\theta(v) = \pi(F_v) \cap \simplex$. \raf{But haven't def $\theta$ yet}
\end{claim}
\begin{proof}
Argue $\theta(v)$ is full-dim in $\simplex$ if and only if $\pi(F_v)$ is $|\Y|$-dim, if and only if $F_v$ is a facet. (Claim 12\jessie{make ref}) \raf{(2)}
\end{proof}

\begin{claim}
	For any $r \in \R$, take $v \in \V$ such that $v = L(r)$.
	$\theta(v) = \Gamma_r$.
\end{claim}
\begin{proof}
	\begin{align*}
	p \in \theta(v) &\iff \inprod{v}{p} = \min_{v' \in \V} \inprod{v'}{p}\\
	&\iff v \in \argmin_{v' \in \V} \inprod{v'}{p}\\
	&\iff r \in \argmin_{r' \in \R} \inprod{L(r')}{p}\\ 
	&\iff p \in \Gamma_r
	\end{align*}
\end{proof}

\begin{claim}
	$\cup_{v \in \V'} \pi(F_v) = \reals_+^\Y \iff \Theta \subseteq \{\Gamma_r \mid r \in \R'\}$.
\end{claim}
\begin{proof}
	\jessie{Ran out of time but mild speedbump to show the forward implication.  Reverse is in my notebook.}
\end{proof}

\jessie{Don't think this claim is actually necessary?}
\begin{claim}
	$\Theta = \{\Gamma_r \mid r \in \R^*\}$ 
\end{claim}
\begin{proof}
	Fix $v \in \V^*$.
	By Corollary 9, there is some $r \in \R^*$ so that $v = L(r)$.
	\begin{align*}
	\theta(v) \in \Theta &\iff \inprod{v}{p} = \min_{v' \in \V} \inprod{v'}{p}\\
	&\iff r \in \argmin_{r' \in \R} \inprod{L(r')}{p}\\
	&\iff r \in \argmin_{r' \in \R^*} \inprod{L(r')}{p}\\
	&\iff r \in \Gamma_r
	\end{align*}
...
\begin{proof}
	Let $v = L(r) \in \V$.
	By Claim 14\jessie{ref}, face $F_v$ must be a subset of some $F_{v^*}$ for $v^* \in \V^*$.
	We must then have $\pi(F_v) \cap \simplex \subseteq \pi(F_{v^*}) \cap \simplex$, which is precisely defined as $\theta(v) \subseteq \theta(v^*)$.
	Since $v = L(r)$, this can be re-written $\theta(v) = \Gamma_r$ by Claim 17.
	The result then follows $\Gamma_r \subseteq \theta(v^*)$.
\end{proof}
