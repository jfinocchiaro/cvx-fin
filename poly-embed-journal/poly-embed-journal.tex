...
\item Claim: we may write $f(p) = \min_{v\in\hat\V} \inprod{p}{v}$ for some finite set $\hat\V \subseteq \reals^\Y_+$ \raf{show you can do this using the $h$ and $\delta$ representation from Rockafellar -- argue that because $p\in\simplex$ we can write $\inprod{p}{b_i}-\beta_i = \inprod{p}{b_i-\beta_i\ones}$.}
...
\item For any $v\in\reals^\Y_+$, let $H_{v}^+ := \{ (x,c) \in \reals^\Y \times \reals \mid \inprod{x}{v} - c \geq 0\}$ and $H_v := \{ (x,c) \in \reals^\Y \times \reals \mid \inprod{x}{v} - c = 0\}$.
...
\begin{itemize}
\item Claim: $G$ is $(|\Y|+1)$-dimensional.  \raf{$g$ is nonnegative on $\reals^\Y_+$, so $G$ contains $\{(x,c) \mid x\in\reals^\Y_+, c \leq 0\}$, which is full-dim.}
...
\item Claim: $\H_\Y \subseteq \H'$.
\item Then $\H' \setminus \H_\Y = \{H_v^+ : v\in\V\} =: \H_\V$ for some $\V \subseteq \hat \V$; That's the $\V$ we are looking for.  (And we've argued that it's uniquely determined by $f$.) \jessie{Why is $\V$ minimum cardinality?}
