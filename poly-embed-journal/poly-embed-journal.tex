...
\begin{definition}[Trim]\label{def:trim}
	Given an elicitable property $\Gamma:\simplex \toto\R$, we define $\trim(\Gamma) = \{\Gamma_u : u \in \R \textrm{ s.t. } \neg\exists u'\in\R,u'\neq u,\, \Gamma_u \subsetneq \Gamma_{u'}\}$ as the set of maximal level sets of $\Gamma$.
\end{definition}

Observe that if a loss $L : \R \to \reals_+^\Y$ eliciting the property $\Gamma : \simplex \toto \R$ is non-redundant, then $\{\Gamma_r \mid r \in \R\} = \trim(\Gamma)$ by applying the definition of non-redundancy.
...
\begin{proposition}
	Let $L : \R' \to \reals^\Y_+$ be a given loss function and $\R$ a representative set for $L$.
