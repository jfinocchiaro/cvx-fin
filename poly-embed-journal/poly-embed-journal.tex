...
A first step is a simple lemma showing that restricting a loss to only those reports in some representative set preserves the Bayes risk.
We will use this lemma throughout the paper.
...
\end{theorem}
\begin{proof}
  \raf{Update notation here; been using $\U$ for a set of reports everywhere else}
  ...
  Then $\Sc$ is representative for $L$.
...
\end{theorem}
\begin{proof}
  \raf{We will apply Theorem (bayes-risk-matching), which we prove in \S~\ref{sec:min-rep-sets}}
...
\begin{proof}
  ...
  By Proposition~\ref{prop:representative-embeds-restriction}, $L$ embeds $L|_\Sc$, which is finite by construction.
