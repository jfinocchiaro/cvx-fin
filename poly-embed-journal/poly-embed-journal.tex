...
Consistency depends crucially on the choice of link function that maps surrogate reports (predictions) to target reports; see the discussion following Definition~\ref{def:indirect-elic}.
...
\reviewer{Discussion of polyhedral surrogate for $0-1$ loss}
...
based on optimality conditions \citep{finocchiaro2020embedding}\raft{Thoughts on using 1st person plural here for previous work?  It is the same authors, and same broader project... I think it's less awkward this way, but I'm okay with 3rd person too}\jessiet{So long as we're submitting to a single-blind venue, I'm fine with it.}.
