...
\begin{proof}
	...
	As facets are closed, for each facet $F$ and $p_F \in F$, there is a report $u_F \in \Sc(L)$ by construction and closedness of facets so that the hyperplane $p_F \mapsto (p_F, \inprod{p_F}{L(u_F)})$ supports $E_L$ (epigraph of negative risk) exactly on facet (and therefore $u_F \in \Gamma(p) \cap \Sc(L)$ for all $p$ such that the hyperplane supports $E_L$ at $p$), we have $\Sc(L)$ representative for $L$.
	Representativeness follows the facets of $E_L$ union to the simplex.
	
	
	If $\Sc(L)$ was not a minimum set, then we could remove some $s$ from $\Sc(L)$ such that $\Sc' := \Sc(L) \setminus \{s\}$ is also a representative set for $L$.
	By the construction of $\Sc(L)$ over facets of $E_L$, this report $s$ corresponds to a facet $F$ of $E_L$ since facets are full-dimensional and thus have well-defined nonempty interiors.
	...
	Therefore, $\Sc(L)$ is a minimum representative set.
\end{proof}
...
\begin{proof}
  ...
  Form the finite set $\Sc := \Sc(L)$ as in Definition~\ref{cons:rep-set}.
