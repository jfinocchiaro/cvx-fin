...
\begin{proof}
  ...
  As $\Gamma$ has at least on minimum representative set, we conclude $\trimred(\Gamma) = \{\Gamma_r \mid r\in \R\setminus\red(\Gamma)\} = \Theta$.  
\end{proof}
...
\begin{proof}
  ...
  In each case we will show that the relevant set of level sets is equal to $\Theta$, giving the result.
  ...
  Case 2:
  If $\trimred(\Gamma)$ is finite, then in particular we have a finite set of reports $\Sc \subseteq \R\setminus\red(\Gamma)$ such that $\trimred(\Gamma) = \{\Gamma_s \mid s\in\Sc\}$.
  As $\Gamma$ is elicitable, $\R$ is representative for $\Gamma$.
  By definition of $\trimred$, we have $\simplex = \cup_{r\in\R} \Gamma_r \subseteq \cup \trimred(\Gamma) = \cup_{s\in\Sc} \Gamma_s$, and therefore $\Sc$ is representative for $\Gamma$ and for $L$.
  From Lemma~\ref{lem:X}(\ref{item:X-rep-contain-min}), we have some minimum representative set $\U\subseteq\U'$ for $L$ and $\Gamma$, showing Case 3.
