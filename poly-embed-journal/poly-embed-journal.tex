...
We lack general tools to methodically construct consistent convex surrogates, much less an understanding of which surrogates will be consistent.
For example, in multiclass and top-$k$ classification, several proposed surrogates were proposed and adopted but later proved to be inconsistent~\cite{yang2018consistency}\raft{need a good ref for multiclass; I'm thinking WW and others}.
This state of affairs is even more dire for structured prediction, where in addition to convexity and consistency, one requires a low \emph{prediction dimension} (the dimension of the surrogate prediction space) as the label set can grow exponentially large.
...
In all of these settings, we lack a unifying framework that moves from a given target problem to a convex consistent surrogate and link.
...
We prove that such an embedding scheme is intimately related to the class of \emph{polyhedral} loss functions, i.e., those that are piecewise-linear and convex.
...
\raf{ADD: Beyond consistency, we show that any polyhedral surrogate achieves a linear surrogate regret bound, which allows one to translate generalization bounds from the surrogate to the target (\S~\ref{subsec:regret-bounds}).}
...
From Lemma~\ref{lem:X}(\ref{item:X-tight-embed}), $L$ tightly embeds a discrete loss.
  ...
  If $\ell$ indirectly elicits $\gamma$ via the link $\psi$, then $L$ indirectly elicits $\gamma$ by transitivity of subset inclusion, as \jessiet{first subset is an equality, right?} $\Gamma_u \subseteq \hat\gamma_{\hat\psi(u)} \subseteq \gamma_{\psi \circ \hat\psi(u)}$ for all $u \in \reals^d$.
...
In this work, we introduce the embeddings framework to obtain convex and consistent surrogates for discrete prediction tasks show construct a link function such that the embedding surrogate and constructed link pair are consistent for the target task.  
...
\paragraph{Implications and Applications}

As we illustrate in \S~\ref{sec:applications},
...
Finally, several open theoretical directions remain.
