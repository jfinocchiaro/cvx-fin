...
\begin{claim}
	$G = \cap\H$ by definition of $\risk{L}$ \raf{prove}
\end{claim}
\begin{proof}
  ...
  Other dir: for any $(x,c) \in \cap \H$
	\jessie{If $\H$ is redundant, there is still a finite $\H^*$ such that $\H^* \subseteq \H$... right?  weird if $\H$ is infinite.}
\end{proof}
\item Need to use minimizable
\begin{claim}
	Argue that $\H_{\V^*} \subseteq \H$, since $\H_\Y \cup \H_{\V^*}$ is the unique min H-rep of $G$. \raf{not sure how to do this, since $\H$ could be infinite}
\end{claim}
\item 
\begin{corollary}
	Conclude there is some set $\R^* \subseteq \R$ such that $L(\R^*) = \V^*$ (without duplicates).
\end{corollary}
\item 
\begin{claim} 
