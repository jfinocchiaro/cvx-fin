...
Using the losses each proposed surrogate embeds, the authors go on to derive constraints on the label distributions under which the proposed surrogates are actually consistent for top-$k$ classification which subsume previous constraints~\citep{yang2018consistency}.
\raft{I did not understand what ``which subsume previous constraints'' meant.  Do we mean the constraints are tighter than the previous constraints of YK?}
