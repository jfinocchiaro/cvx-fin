...
\begin{proof}
  ...
  Form the finite set $\Sc$ as the proof of Proposition~\ref{prop:SL-minimal}.
...
%  Therefore, $\R$ is minimum.
  ...
  Since $\U$ is a finite representative set, we know $L|_\U$ is embedded by $L$ nu Corollary~\ref{cor:representative-embeds-restriction}.
  Therefore, $\Gamma := \prop{L}$ embeds $\Gamma|_\U := \prop{L|_\U}$, and by assumption $\Gamma|_\U$ is non-redundant.
  By Proposition~\ref{prop:embed-trim}\jessiet{BTW: Forward ref}, $\U$ is a minimum representative set for $L$.
...
\begin{restatable}{proposition}{embedtrim}\label{prop:embed-trim}
	...
	The following are equivalent:
	\begin{enumerate}\setlength{\itemsep}{0pt}
		\item $\Gamma$ embeds a \jessie{nonredundant} finite property $\gamma:\simplex \toto \R$.
...
%	Then we can apply Lemma~\ref{lem:loss-restrict} to observe that $L$ embeds $L|_\U$, and therefore $\Gamma$ embeds $\prop{\ell}$, which is finite as $\ell$ is discrete.
	...
	3 $\Rightarrow$ 1:
	Since $\U$ is representative, we have $\Gamma(p) \cap \U$ nonempty for all $p \in \simplex$, and therefore, $\Gamma$ embeds $\Gamma|_\U$, which is finite as $\U$ is finite.
...
%	Moreover, since $\gamma$ is non-degenerate, we have $\cup \Theta = \cup_r \gamma_r = \simplex$.	
	...
	Since $\Gamma$ embeds a finite property $\gamma$, we have a finite minimum representative set $\Sc$ for $\gamma$ such that $\{\Gamma_u \mid u \in \varphi(\Sc)\} = \{\gamma_r \mid r \in \Sc\}$.
	...
	Since level sets of elicitable properties are closed, $\gamma_r$ must then be a face of some facet $\gamma_{r'}$.
	...
	4 $\Rightarrow$ 1: \jessie{Intuition: Take $\gamma : p \mapsto \Gamma(p) \cap \varphi(\R)$.}
