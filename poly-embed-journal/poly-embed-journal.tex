...
\begin{proposition}\label{prop:embed-bayes-risks}
  A loss $L$ embeds a discrete loss $\ell$ if and only if $\risk{L}=\risk{\ell}$ and the risks are polyhedral.
...
\begin{proof}
  ...
  Suppose $L$ embeds $\ell$ over $\Sc\subseteq \R$ via the embedding $\varphi:S\to\reals^d$.
