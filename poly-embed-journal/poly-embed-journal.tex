...
\begin{figure}
	\begin{center}
		...
		\end{minipage}\hfill
		\caption{Links $\psi^*$ and $\psi^\diamond$ such that $(L^f, \psi^*)$ and $\psi^\diamond$ are calibrated with respect to $\ellabs$ for all suitable $f$. Points in each region link to the embedding point contained in the region. Both are constructed via the link envelope $\Psi$ from Construction~\ref{const:eps-thick-link}, which yields possible choices for calibrated links.\jessie{New, simpler, caption.  Clear enough?}\raf{Looks great.  Maybe instead of (L),(M), etc, which look a bit like function applications, we could put the actual symbols in the top left of the figures.  Also, could we shrink the $u_1,u_2$ and bullets, and make the other labels larger, basically to match Figure~\ref{fig:abstain-links}?\jessie{Done}}
			}
...
\begin{table*}
	...
	\caption{
		...
		\jessie{Thoughts on this caption?  Now walking through this geometric intuition for the third time.} \raf{Let's refer back to Figure~\ref{fig:abstain-intuition-inconsistent} instead of going into the same level of detail; see the changes I made in Figure~\ref{fig:ordered-partition}.}
	}
