...
In particular, our results imply the existence of linear regret transfer bounds bounds for several applications where no such bounds were known (\S~\ref{sec:lovasz-hinge},~\ref{sec:winge},~\ref{sec:top-k}).\jessiet{This sentence sounds like we're going to give the bounds, but we decided against that, right?}
\raft{Yeah... rephrased.  Better?}
...
In particular, since the abstain report $\bot$ is $\ellabs$-optimal whenever $\max_{y\in\Y} p_y \leq 1/2$, by the definition of embedding, the origin $0\in\reals^d$ is $\BEP$-optimal for the same distributions.
...
\end{equation}
This surrogate $\LWW$ was later shown to be inconsistent with respect to 0-1 loss~\citep{tewari2007consistency,liu2007fisher}.\raft{FYI: changed notation to match the style of our other surrogates.  Not sure I care enough to change the top-$k$ ones though...}
