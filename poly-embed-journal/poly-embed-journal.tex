...
\raf{Old version:} Using tools from property elicitation, we show a tight relationship between such embeddings and the class of polyhedral (piecewise-linear convex) loss functions.
...
Moreover, given an existing polyhedral surrogate, we provide tools to find the discrete loss it embeds (Corollary~\ref{cor:representative-embeds-restriction}).
...
We apply these tools to several polyhedral surrogates which have been proposed recently (\S~\ref{sec:applications}), showing that they are inconsistent for the desired target, but are consistent for a different target: the one embedded by the loss.
...
As a consequence, polyhedral losses always have finite representative sets.
Aside from observations like these which we use to prove our main results, we provide several others through the lens of property elicitation which we view as conceptual contributions (\S~\ref{sec:min-rep-sets}).
These latter observations culminate in the following result: when restricting to the class of polyhedral surrogates, indirect elicitation is both necessary and sufficient for consistency (Theorem~\ref{thm:poly-ie-implies-consistent} in \S~\ref{sec:poly-ie-consistency}).
...
Several consistent convex surrogates in the literature can be thought of as \emph{embeddings}, wherein one maps the discrete reports to a vector space, and finds a convex loss which agrees with the original loss.
...
\begin{proposition}\label{prop:SL-minimum}
  A loss $L: \R \to \reals^\Y_+$ has a polyhedral Bayes risk if and only if it has a finite minimum representive set.
...
\end{proposition}
\begin{proof}
	Finite minimum representative implies polyhedral Bayes risk as a corollary of Lemma~\ref{lem:loss-restrict}.
	
	To show that $L$ has a polyhedral Bayes risk implies it has a finite minimum representative set, we construct the set $\Sc$ as follows:
...
\begin{proof}
  ...
  By Lemma~\ref{lem:loss-restrict}, we have $\risk{\ell} = \risk{\ell|_{\Sc}}$.
  Since $\Sc$ is representative for $\ell$, for all $p\in\simplex$ we have some $r \in \Sc \cap \gamma(p)$ and thus we have $\varphi(r) \in \U \cap \Gamma(p)$.
  In particular, $\U \cap \Gamma(p)\neq \emptyset$ for all $p\in\simplex$, so $\U$ is representative for $L$.
...
%  To see that $\gamma'(p) \neq \emptyset$ for all $p\in\simplex$, note that by the definition of $\gamma$ as the property elicited by $\ell$ we have some $r \in \gamma(p)$, and by the embedding condition~\eqref{eq:embed-loss}, $\varphi(r) \in \Gamma(p)$.
  By Lemma~\ref{lem:loss-restrict}, we have $\risk{L} = \risk{L|_{\U}}$.
...
\begin{proof}[Sketch]
  ...
  Finally, consider that $\trim(L) = \trim(\ell)$ implies $L$ embeds $\ell$, and if $\R^*$ is a minimum representative set for $\ell$ then $\trim(L) = \trim(\ell) = \trim(\ell|_{\R^*})$ so $L$ tightly embeds $\ell|_{\R^*}$ by Corollary~\ref{cor:tight-embed-min-rep} and composing the embeddings.
  % and $\ell|_{\R^*}$ is non-redundant by Proposition~\ref{prop:tfae-min-rep-nonredundant}.
  Therefore, $L$ tightly embeds $\ell|_{\R^*}$.
\end{proof}

As we define one loss embedding another, one can imagine an analogous definition where one property embeds another (Definition~\ref{def:prop-embed}), and we can define $\trim(\Gamma)$ as the set of level sets generated by a minimum representative set (Definition~\ref{def:trim-prop}).
...
\begin{restatable}{proposition}{embedtrim}\label{prop:embed-trim}
	...
	Moreover, when any of the above hold, $\trimnonred(\gamma) = \trimnonred(\Gamma) = \{\Gamma_u : u\in\U\} = \Theta$, and $\gamma$ is elicitable.\jessiet{At the moment, don't show anything about $\gamma$ elicitable.}\jessiet{Also, should this statement be $\trimcover(\gamma) = \trimnonred(\gamma) = \trimcover(\Gamma) = \trimnonred(\Gamma) = \{\Gamma_u \mid u \in \U\} = \Theta$? But also, can we pull it out into its own statement? It should follow as a corollary of this and an analogous version of Prop 6.}
\end{restatable}
\begin{proof}
	\jessie{Keep plus example in mind}
	\jessie{Use trim nonredundant and trim covering macros}
	\jessie{1 -> 2 -> 3 -> 1, 3 <=> 4}
	Let $L$ elicit $\Gamma$.
	...
	%
%	Since $\gamma$ is finite, it has a finite minimum representative set $\Sc$, and we can show $\trimnonred(\gamma) = \trimcover(\gamma) := \{\gamma_r \mid r \in \Sc\}$. 
%	\jessiet{Hand waving}
%	Moreover, since the embedding condition implies $\gamma_r = \Gamma_{\varphi(r)}$, we also have $\trimcover(\Gamma) = \trimcover(\gamma)$, which is finite as $\gamma$ is finite.
%	Now we can show that $\trimcover(\Gamma)$ finite implies $\trimcover(\Gamma) = \trimnonred(\Gamma)$ and the result follows.
%
	%Since $\Gamma$ embeds a finite $\gamma$ over some minimum representative set $\Sc$, and each for each $r \in \Sc$, we have $\gamma_r = \Gamma_{\varphi(r)}$.
	%
	Since $\R$ is finite, we have $\trimnonred(\Gamma) \subseteq \{\Gamma_{\varphi(r)} \mid r \in \R\} = \{\gamma_r \mid r \in \R\}$.
	As the last set is finite, so is $\trimnonred(\Gamma)$.
	Moreover, as $\trimnonred$ only removes redundant reports, we have $\cup \trimnonred(\Gamma) = \simplex$.
	

\btw{Commenting out 3 June 21 to use old $\trimnonred$ definition.}
%	If $\Gamma$ embeds a finite $\gamma$, then any representative set $\Sc \subseteq \R$ is finite, and so any minimum representative set is finite; $\trim$ is then finite as it is finitely generated.
%	$\cup \trim(\Gamma) = \simplex$ follows from representativeness.

\btw{Commenting out 2 $\Rightarrow$ 1 for now since I added 1-> 2 -> 3 ->1 for cyclicity.}
%	\raft{These next two directions together seem like what we did before for $2\Rightarrow 3$, correct?  (Not as complete though; see the step where we justify the conditions of Lemma~\ref{lem:loss-restrict}.)  I'm actually happy for them to be separated, but let's make sure the argument is just as complete. \jessie{Added in the conditions of Lemma 3 to be a bit more thorough.}}\btw{Not sure how old this comment is, but feel free to burn if you're happy with thoroughness.}
%	2 $\Rightarrow$ 1:
%	As $\trim(\Gamma)$ is finite, we know $\trim(\Gamma) = \{\Gamma_{u_i} \mid u_i \in \U\}$ for some minimum representative set $\U$ of finite cardinality $k$.
%	Then we can apply Lemma~\ref{lem:loss-restrict} to observe that $L$ embeds $L|_\U$, and therefore $\Gamma$ embeds $\prop{\ell}$, which is finite as $\ell$ is discrete.
	...
	2 $\Rightarrow$ 3: 
	%Suppose $\trimcover(\Gamma) = \{\Gamma_u \mid u \in \U\}$.
	%Then we know $\U$ is a minimum representative set, and since $\trimcover(\Gamma)$ is finite, so is $\U$; if it was not, then $\U$ would not be minimum.
	\jessie{Using the old definition of $\trim := \trimnonred$, mirroring ish previous proofs} 
	As a reminder, we have $\trimnonred(\Gamma) = \{\Gamma_u \mid \neg \exists u' \neq u $ s.t. $ \Gamma_u \subsetneq \Gamma_{u'}\}$.
	Take $\U^* = \{u_1, \ldots, u_k\}$ to be a finite set of reports forming $\trimnonred$.
	Since $\cup \trimnonred(\Gamma) = \cup_{u \in \U^*} \Gamma_u = \simplex$, we have $\U^*$ representative.
	We can now construct a minimum representative set $\U \subseteq \U^*$.
	Form $\U$ by iterating over $\U^*$ and removing any element $u'$ of $\U^*$ if there is another $u$ remaining in $\U^*$ such that $\Gamma_u = \Gamma_{u'}$.
	As we only remove exact duplicate level sets, we still have $\U$ representative, and proceed to show $\U$ is minimum.
	
	If $\U$ was not minimum, there would be some report $u' \in \U$ so that $\U' := \U \setminus \{u'\}$ is also representative.
	Since $\Gamma$ is elicitable and $\trimnonred$ is finite, we know that the level sets of $\Gamma$ form a power diagram intersected with the simplex (Theorem~\ref{thm:aurenhammer}).
	Therefore, every level set $\Gamma_u$ corresponds to a face of a convex polyhedron: we correspond in two cases, according to if $\Gamma_{u'}$ corresponds to a facet or some lower-dimensional face.
	
	First, suppose $\Gamma_{u'}$ does not correspond to a facet.
	Since $\Gamma_{u'}$ does correspond to a face of a convex polytope, it must be contained in a facet, and therefore there is some $u$ such that $\Gamma_{u'} \subsetneq \Gamma_u$.
	Thus, $\Gamma_u \not \in \trimnonred(\Gamma)$.
	If $\Gamma_{u'}$ corresponds to a facet, then it has a well-defined nonempty interior, and by removing exactly duplicate reports in the construction of $\U$, we have a $p \in \inter{\Gamma_{u'}}$ such that $\{u'\} = \Gamma(p)$.
	Therefore, $\U'$ would not be representative.
	We conclude $\U$ as constructed must be a minimum representative set.
	
	
	3 $\Rightarrow$ 1:
	Since $\U$ is representative, we have $\Gamma|_\U(p) := \Gamma(p) \cap \U$ nonempty for all $p \in \simplex$, and therefore, $\Gamma$ embeds $\Gamma|_\U$, which is finite as $\U$ is finite.
	Since $\U$ is minimum, the constructed $\Gamma_\U$ is additionally non-redundant.
	\jessiet{Want more details here.  It's the forward implication of Prop 4, but since this is used in the reverse implication of prop 4, don't want to create cyclic dependencies.}
	
	
	
	\btw{Previous version commented out above because it relies on old definitions - Jessie 1 June 21, commenting out 1 $\iff$ 4 to replace with 3 $\iff$ 4 - 3 June 21}
%	1 $\Rightarrow$ 4:
...
%	Suppose $\trimcover(\Gamma) = \{\Gamma_u \mid u \in \U\} = \simplex$.
%	(Embedding implies $\Sc$ is a minimum representative set for $\gamma$ iff $\varphi(\Sc)$ is minimum representative for $\Gamma$.)
%	Take $\Theta = \trimcover(\Gamma)$; since $\U$ is representative, we know $\cup \Theta = \simplex$.
%	Moreover, $\Theta$ is finite since $\U$ is finite.
%	It is just left to show each $\theta \in \Theta$ is full-dimensional.
%	Since $\Gamma$ is elicitable and has a finite minimum representative set, 
%	Since $\gamma$ is a finite property, its level sets form a power diagram in the simplex (Theorem~\ref{thm:aurenhammer}).
%	If there was a $\theta = \gamma_r$ that was not full-dimensional, it must therefore be the face of another level set.
	%Since level sets of elicitable properties are closed
	%\footnote{$\gamma_r = \Gamma_{\varphi(r)}$ for all $r \in \Sc$, we can use this characteristic about level sets of elicitable properties. \jessie{Adding this since we haven't shown $\gamma$ elicitable yet.}}, 
	%$\gamma_r$ must then be a face of some facet $\gamma_{r'}$.
	%Therefore, $\gamma_r \subseteq \gamma_{r'}$, contradicting $\Sc$ being minimum, so each $\theta \in \Theta$ must be full-dimensional.
%	
%	%	\hrule
...
3 $\Rightarrow$ 4:
It suffices to show that, for each $u \in \U$, $\Gamma_u$ is full-dimensional, so we can take $\Theta = \{\Gamma_u \mid u \in \U\}$.
Since $\Gamma$ has a finite representative set, we know its level sets are faces of convex polyhedron by equivalence to a power diagram intersected with the simplex.
If there is a $u \in \U$ such that $\Gamma_u$ is not full-dimensional, it must be a face of some facet corresponding to $\Gamma_{u'}$ for some $u' \in \U$, and hence we contradict $\U$ being minimum, as $\U \setminus \{u\}$ would be a representative set of strictly smaller cardinality.



4 $\Rightarrow$ 3:
We want to show that for each $\theta \in \Theta$, we cannot have $\Theta' := \Theta \setminus \{\theta\}$ be representative.
Observe we have no duplicate elements of $\Theta$.
Fix $\theta := \Gamma_u \in \Theta$.
Since $\Gamma_u$ is full-dimensional, there is some $p \in \inter{\Gamma_u}$ so that $\{u\} = \Gamma(p)$.
Therefore, $\Theta'$ is not representative, so $\Theta$ is a finite minimum representative set for $\Gamma$.


Observe now that when one of these is true, $\Gamma|_\U$ is elicited by $L|_\U$, and that there is a bijection from $\Gamma|_\U$ to $\gamma$, and therefore $\gamma$ is also elicitable. 
\end{proof}
