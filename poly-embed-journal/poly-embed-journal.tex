...
\begin{definition}\label{def:g-finite}
  Given a set $\V \subseteq \reals^\Y_+$, define the function $g_\V : \reals^\Y \to \reals_+ \cup \{-\infty\}$\raft{added $-\infty$ and changed the domain, since the definition implies that $x$ could be outside of $\reals^\Y_+$, as otherwise you wouldn't need the indicator} by
  \begin{align*}
...
\begin{proof}
    ...
    &\iff \inprod{v}{x} \geq c \,\, \forall v \in {L(\R)} & \text{definition of $\risk L_+$, minimizability} \\
...
\begin{corollary}\label{cor:unique-set-loss-vectors-defining-facets}
  ...
  (ii) $F_v$ is a facet of $\hyp(g_\V)$ for each $v\in\V^*$.\raft{Is $F_v$ defined?  Looks like it got cut (we were going to move it to this subsec maybe)}
  (iii) $\V^* \subseteq \V$.
  (iv) $g_\V(x) = \min_{v \in \V^*}\inprod{v}{x} - \delta(x \mid \reals_+^\Y) = g_{\V^*}(x)$.
\end{corollary}
\begin{proof}
...
%  The moreover follows immediately since $\H_{\V^*} \subseteq \H^*$ and every $H^+ \in \H^*$ defines a facet by Lemma~\ref{lem:G-unique-facets-Hstar}.
  ...
  (iv)   The result holds if $\hyp(g_\V) = \hyp(g_{\V^*})$.
  ...
  Thus $g_\V$ can be written as $g_\V(x) = \min_{v \in \V^*}\inprod{v}{x} - \delta(x \mid \reals^\Y_+) = g_{\V^*}(x)$. 
\end{proof}
...
%	For all $v \in L(\R)$, define $F_v$ as the face of $g_{L(\R)}=g_{\V^*}$ generated by $H_v$.
	Then $\dim(F_v^{\hyp(g_{\V^*})}) = \dim(\pi(F_v^{\hyp(g_{\V^*})}))$.
\end{claim}
\begin{proof}
	...
	Observe $z_i = (x_i, \inprod{v}{x_i})$ for all $i$; therefore, if $z_i \in F_v$ (e.g., $F_v$ supports $\hyp(g_\V)=\hyp(g_{\V^*})$ at $(x, \inprod{v}{x})$), then we also have $z_i \in H_v$.
