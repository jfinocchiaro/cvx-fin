...
\bo{This is pretty inaccessible to nonexperts. I think we should consider a rewrite that expands this intro and makes it much more accessible. For example what is a surrogate? It would help many readers a lot to spell things out more.}
...
For example, discrete losses have polyhedral Bayes risks (as the minimum of finitely many linear functions), and so do polyhedral losses (Lemma~\ref{lem:poly-loss-poly-risk})\jessiet{Flag: unresolved ref}.
...
We first discuss the conditional setting, with just labels $\Y$ and no features $\X$, and show in \S~\ref{subsec:calibration-links} how these notions relate to the usual $\X\times\Y$ setting considering features.
\bo{I don't expect the previous sentence to make sense to most readers. Suggest we add some explanation or detail -- or defer this comment to a bit later.}
...
\jessiet{Removed sentence saying we assume non-redundancy}
...
\begin{definition}[Polyhedral loss]
  A loss $L: \reals^d \to \reals^{\Y}_+$ is \emph{polyhedral} if $L(u)_y$ is a polyhedral convex\bo{removed parentheses which might be confusing} function of $u$ for each $y\in\Y$.
\end{definition}
%
...
In these cases, which capture several consistent surrogates in the literature\bo{cites/examples?}, one maps each report (prediction) of $\ell$ to a point in $\reals^d$, then constructs a convex loss on $\reals^d$ that agrees with $\ell$ at these points.

In fact, it is not always necessary to map \emph{all} reports to such points.
It turns out to often be convenient to allow $\ell$ to have reports that are ``redundant'' in some sense (explored in \S~\ref{sec:min-rep-sets}).
...
\end{lemma}
\bo{I didn't see us define $L|_\Sc$ anywhere yet! Although it's briefly mentioned in the intro.}
\bo{Notation in the lemma is confusing because $L$ is supposed to be a surrogate with report space $\reals^d$ and $\R$ is supposed to be the report space for $\ell$.}
\begin{proof}
...
Because $u$ minimizes expected loss for both $p$ and $p'$, this imposes the requirement on the link that $\psi(u) \in \gamma(p) \cap \gamma(p')$.
...
After repeating for each optimal report set, we will claim that there exists a small enough $\epsilon > 0$ so that the construction is successful, i.e. $\Psi(u)$ is nonempty.
...
For example, a modified hinge loss $L(r)_y = \max(r^2-1,1-ry)$, \jessie{Would a figure be helpful?} which matches hinge loss on the interval $[-1,1]$ but is smooth outside the interval $[-2,2]$, still embeds twice 0-1 loss.
...
\begin{proof}
  ...
  From Lemma~\ref{lem:X}(\ref{item:X-min-V}), we have some set $\V\subseteq\reals^\Y_+$ and minimum representative sets $\R^* \subseteq \R$ and $\Sc \subseteq \U$, for $\ell$ and $L$ respectively, such that $\ell(\R^*) = \V = L(\Sc)$.\jessie{This is a different $\Sc$ than the first paragraph, right? The first $\Sc$ is representative for $\ell$.}
...
We now relate this condition to another natural notion from the property elicitation literature, that of non-redundancy~\cite{frongillo2014general,lambert2018elicitation}\raft{TODO: see if there are other references we should include here.  Maybe Jessie you could search? \jessie{Quick search leads me to a couple of your papers and Nico's, but not much else.}}.
...
\begin{proposition}\label{prop:embed-trim}
  Let $\Gamma:\simplex\toto\reals^d$ be an elicitable property.\jessie{$\reals^d$ vs $\R$?}
  The following are equivalent:
  \begin{enumerate}\setlength{\itemsep}{0pt}
...
\begin{proof}
  ...
  Case 2:
  If $\trimred(\Gamma)$ is finite, then in particular we have a finite set of reports $\Sc \subseteq \R\setminus\red(\Gamma)$ such that $\trimred(\Gamma) = \{\Gamma_s \mid s\in\Sc\}$. \jessie{Technically, $\R$ not defined?}
  As $\Gamma$ is elicitable, $\R$ is representative for $\Gamma$.
  By definition of $\trimred$, we have $\simplex = \cup_{r\in\R} \Gamma_r \subseteq \cup \trimred(\Gamma) = \cup_{s\in\Sc} \Gamma_s$, and therefore $\Sc$ is representative for $\Gamma$ and for $L$. \jessie{I'm a bit confused by this- how do we get $\cup_{\R}\Gamma_r \subseteq \cup \trimred(\Gamma)$?}
  From Lemma~\ref{lem:X}(\ref{item:X-rep-contain-min}), we have some minimum representative set $\U\subseteq\U'$ \jessie{$\R$?} for $L$ and $\Gamma$, showing Case 3.
  ...
  Finally, Corollary~\ref{cor:trim-prop-red} gives $\trimred(\gamma) = \Theta$. \jessie{$\Gamma$?}
  %
...
\begin{proof}
  ...
  \begin{align*}
    p \in \theta(v)
    &\iff \inprod{v}{p} = f(p) & \text{Def of $\theta$}\\
    &\iff \inprod{v}{p} = \min_{v' \in \V}\inprod{v'}{p} & \text{$f$ matches $g$ on $\simplex$ (Cor.~\ref{cor:f-matches-g-on-simplex}.)}\\
    &\iff v \in \argmin_{v' \in \V}\inprod{v'}{p} & \text{Def of argmin}\\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\iff p \in \{p' \in \simplex \mid \inprod{v}{p'} = g(p')\} &\text {Since $f = g$ on simplex} \\
    &\iff p \in \{p' \in \reals_+^\Y \mid \inprod{v}{p'} = g(p')\} \cap \simplex &\text {Since $g$ defined over $\reals^\Y_+$} \\
    &\iff p \in \pi(F_v) \cap \simplex & \text{Following from definition of $F_v$}
  \end{align*}
  The result follows.
...
\begin{proof}
  ...
  \begin{align*}
    p \in \Gamma_r
    &= \{p' \in \simplex \mid r \in \argmin_{r' \in \R} \inprod{L(r')}{p'} \} & \text {Def of level set} \\
    ...
    &= \{p' \in \simplex \mid c \inprod{v}{p'} = g(p') \} & \text {Multiplying both sides by $c$} \\
    &= \{p' \in \simplex \mid  \inprod{v}{cp'} = g(cp') \} & \text {Linearity of inner product and Lemma~\ref{lem:g-1-homog}} \\
    \implies cp
