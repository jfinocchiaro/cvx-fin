...
For a fixed concave function $g : \reals^\Y_+ \to \reals_+$, define $G = \{(x,c) \mid c \leq g(x)\} \subseteq \reals^\Y_+ \times \reals$ to be the hypograph of $g$.

If $g$ takes the form of Definition~\ref{def:g-finite} below, define $H^+_v := H^+_{([v;-1], 0)}$ for a given $v$, where $[v; -1]\in\reals^\Y_+\times\reals$ define a halfspace for some $v \in \V$.
...
Throughout, we will work with a function $g$ generated by $\V$ of the following form.
...
For $x\in\reals^\Y_+$, consider the $1$-homogeneous extension of Bayes risk  $\risk{L}_+(x) := \inf_{r\in\R} \inprod{x}{L(r)}$ polyhedral.
...
\subsection{Projecting from $G$ to $g$}\label{subsec:project-pi}
