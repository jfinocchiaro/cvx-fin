...
\item Then $G = \cap \H$.
\item Since $G$ is a full-dimensional polyhedron, with a unique H-rep by [cite]
\item Hence, there is some unique $\H' \subseteq \H$ such that $G = \cap \H'$.
\item Argue $\H_\Y \subseteq \H'$.
\item Then $\H' \setminus \H_\Y = \{H_v^+ : v\in\V\} =: \H_\V$ for some $\V \subseteq \hat \V$.
\item That's the $\V$ we are looking for.  (And we've argued that it's uniquely determined by $f$.)
\item Moreover, $F_v := H_v^+ \cap G$ is a facet of $G$ for each $v\in\V$.
\item Define the projection $\pi:\reals^\Y\times \reals \to \reals^\Y, (x,c) \mapsto x$.
\item Show that $\cup_{v\in\V} \pi(F_v) = \reals^\Y_+$.
