...
%	This can only iterate up to $|\H^*|$ times, so there must be some finite irredunant representation for $P$.\jessie{This feels way too spelled out for its lack of detail.}
	...
	It is just left to show (iii).
  	...
  	For any halfspace $H^+ \in \H^*$, we want to show that $H^+ \in \H$ as well.
  	By (ii), each $H^+ \in \H^*$  determines a facet of $P$, and this facet is irredundant by (i).
  	We claim that if $H^+ \not \in \H$, then $\cap \H \neq P$ as convex polytopes are uniquely determined by their facets (cf.~\citep[pg. 455]{schneider2014convex}).
  	Thus, if $H^+ \not \in \H$, we contradict the assumption $\cap \H = P$.
...
\jessie{Defining extended Bayes risk}
...
\jessie{Context for Claim 2; translate from losses to $\V$ mentioned above}
...
\subsection{Structure of $g_\V$\jessie{Rename: ``From polyhedral extended Bayes risks to finite loss vectors'' ish}}
