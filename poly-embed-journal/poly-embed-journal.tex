...
Suppose $ p  = (0, 1)$ and consider the sequence $\{u_m\} = \{1 - \frac 1 {m+1}\}_{m=1}^\infty$, which limits to $1$, but $\psi(u_m) = -1$ for all $m$.
...
\reviewertwo{C}
\raft{I made some wording edits here.  One general point though is to be careful when adding to the paper at this stage, since the notation was actually slightly off from what we had been doing, e.g.\ ``$0-1$ loss'' instead of ``0-1 loss''  and $\ell^{0-1}$ instead of $\ellzo$.}
\jessiet{Added a clause so te Weston-Watkins reference doesn't seem so random}
...
\end{align*}
In particular, this surrogate is consistent with respect to 0-1 loss using the canonical argmax link, while the Weston-Watkins hinge is not consistent for any link (\S~\ref{sec:winge}), despite being proposed as a surrogate for 0-1 loss.
...
\section{Connection to \citet[Theorem 8]{ramaswamy2016convex}}
...
This construction is quite different from our Construction~\ref{const:eps-thick-link}, in that it operations in the loss vector space $\reals^\Y_+$ rather than the surrogate report space $\reals^d$.
...
One can see that this construction does achieve separation, but does not necessarily align well with any norm in the surrogate report space.
