...
\begin{proof}
  ...
  \jessiet{1. Show it's a facet. 2a. Show $H_y$ is a face. 2b. }
  ...
  First, observe that $F_y$ is a face of $\hyp(g_{L(\R)})$ as $H_y$ supports $\hyp(g_{L(\R)})$ by construction of $g_{L(\R)}$ with the convex indicator. \jessie{too much hand-waving?}
  Moreover, $F_y = H_{(e_y, 0)} \cap \hyp(g_{L(\R)}) = \{(x,c) \mid x_y \geq 0\} \cap \hyp(g_{L(\R)})$ is a $|\Y|$-dimensional face as it contains $\{(x,c) \mid x_y \geq 0, c \leq 0\}$, which is $|\Y|$-dimensional.
