...
\begin{definition}[Embedding a loss]\label{def:loss-embed}
  ...
  \end{equation}
\end{definition}
\proposedadd{Moreover, we say $L$ embeds $\ell$ over the set $S$ if $L$ embeds $\ell|_S$ and $\prop{\ell}(p) \cap S \neq \emptyset$ for all $p\in \simplex$.}
%
...
\begin{proposition}\label{prop:embed-bayes-risks}
  A loss $L:\reals^d \to \reals^\Y_+$ embeds discrete loss $\ell:\R \to \reals^\Y_+$ \proposedadd{for some set of reports $S \subseteq \R$} if and only if $\risk{L}=\risk{\ell}$.
  \jessie{This isn't quite right, since we really want $\risk{L} = \risk{\ell|_S}$; is this any set $S$ such that blah blah, by applying Lemma 1?}.  
...
\begin{proposition}\label{prop:embed-risk-poly}
  \proposedadd{There exists a set $S \subseteq \R$ such that the} loss $L$ embeds a discrete loss $\ell : \R \to \reals^\Y_+$ if and only if $\risk{L}$ is polyhedral.
\end{proposition}
\begin{proof}
  ...
  If $L$ embeds $\ell$ \proposedadd{on a set $S$}, Proposition~\ref{prop:embed-bayes-risks} gives us $\risk{L} = \risk{\ell}$, and its proof already argued that $\risk{\ell}$ is polyhedral since $\risk{\ell}$ is a pointwise max of a finite number of linear functions.
  ...
  Defining $\ell:\R\to\reals^\Y_+$ by $\ell(r) = L(u_r)$, the same proof shows that $L$ embeds $\ell$ \proposedadd{on the set $\R$}.
...
\end{theorem}
 \begin{proof}
 	\jessie{Why isn't this proof just:  We have $L$ polyhedral $\implies \risk{L}$ polyhedral (L4) $\iff$ $L$ embeds a discrete loss (P2).}
	The projection of the epigraph of $\risk{L}$ onto $\simplex$ forms a power diagram by Theorem~\ref{thm:aurenhammer} with finitely many cells $C_1,\ldots,C_k$, which we can index by $\R := \{1,\ldots,k\}$.
	Defining the property $\gamma:\simplex\toto\R$ by $\gamma_r = C_r$ for $r\in\R$, we see that the same construction gives us points $u_r \in\reals^d$ such that $u_r \in \Gamma(p) \iff r \in \gamma(p)$.
...
\begin{theorem}\label{thm:discrete-loss-poly-embeddable}
  For every discrete loss $\ell:\R \to \reals^\Y_+$ \proposedadd{there is a set $S \subseteq \R$ such that $\ell$} is embedded by a polyhedral loss \proposedadd{on $S$}.
\end{theorem}
\begin{proof}
...
\begin{definition}\label{def:prop-embed}
	A property $\Gamma : \simplex \toto \reals^d$ embeds a property $\gamma:\simplex \toto \R$ \proposedadd{for a set $S \subseteq \R$ such that $\gamma(p) \cap S \neq \emptyset$ for all $p \in \simplex$} if there exists some injective embedding $\varphi:\R \to \reals^d$ such that for all $p \in \simplex$ and $r \in \R$, we have $r \in \gamma(p) \iff \varphi(r) \in \Gamma(p)$.
...
\begin{lemma}\label{lem:embed-defs-equiv}
	%  \raft{Introduce $L$}
	Let $L : \reals^d \to \reals^\Y_+$ be a loss function \proposedadd{whose infimum is attained in expectation for all $p \in \simplex$}.
	If the property $\Gamma := \prop{L}$ embeds the finite property $\gamma : \simplex \toto \R$ \proposedadd{on $S$}, then there is a discrete loss $\ell:\R \to \reals^\Y_+$ such that $\ell$ elicits $\gamma$ and $L$ embeds $\ell$ \proposedadd{on $S$}.
\end{lemma}
\begin{proof}
	...
	Now, we claim that $L$ embeds $\ell$ \proposedadd{on the set $S$}, and by Proposition~\ref{prop:embed-bayes-risks}, we can simply show $\risk{L} = \risk{\ell}$.
	...
	Consider that if we had $(1) < (2)$ for some $p \in \simplex$, then there would be no $r$ such that $\varphi(r) \in \Gamma(p)$ \jessie{BUG: NEED THE INFIMUM TO BE ATTAINED}, and therefore, we would have $\gamma(p) = \emptyset$, yielding a contradiction. 
...
\begin{definition}[Finite property, non-redundant]
	\jessie{Feels like this should be earlier}
...
Before we state Proposition~\ref{prop:embed-trim}, which is needed to prove many of the statements in \S~\ref{sec:poly-loss-embed}, we will need to general lemmas about properties and their losses.
...
\begin{proposition}\label{prop:embed-trim}
	...
	\begin{enumerate}\setlength{\itemsep}{0pt}
		...
		\item There is a finite set of full-dimensional level sets $\Theta$ of $\Gamma$, and $\cup\,\Theta = \affhull(\simplex)$.
	\end{enumerate}
...
\begin{proof}
	...
	Then we can apply Lemma~\ref{lem:loss-restrict} to observe that $L$ embeds $\ell$, and therefore $\Gamma$ embeds $\prop{\ell}$, which is finite as $\ell$ is discrete and never empty.
	...
	By construction, we have $\gamma_i = \theta_i = \Gamma_{u_i}$ for all $i$, so letting $\varphi(i) = u_i$ we satisfy the definition of embedding, namely statement (i.).
