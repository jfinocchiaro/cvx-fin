...
\begin{definition}[Elicits]
  ...
  \end{equation}
  If $L$ elicits a property, it is unique and we write $\prop{L}$ to refer to the property elicited by a loss $L$.
\end{definition}

Observe that we have defined a property $\Gamma$ to be nonempty, so if the minimum of $\inprod{p}{L(\cdot)}$ is not defined for some $p \in \simplex$, then $L$ does not elicit a property.
...
\end{minipage}
\caption{Bayes risks $L : p \mapsto \inf_u \inprod{p}{L(u)}$ of 0-1, hinge, and logistic losses, respectively, in binary classification setting. Here, the value $x$ on the horizontal axis yields $p = (1-x, x)$ as the respective probabilities $(Pr[Y=-1], Pr[Y=1])$.
	Observe that the Bayes risks of 0-1 and hinge loss are both piecewise lienar and concave, while the Bayes risk of logistic loss is also concave, but not piecewise linear.  \raf{Add detail}\jessie{better?}}
