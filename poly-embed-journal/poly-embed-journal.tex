...
\raf{High-level point: the flow and purpose of our related work could use revisiting; at worst, what's there could even detract from our message above, depending on how one reads it.  We should be clear and careful about what we are saying and what connections are relevant and why.} \jessie{I took another pass on this, but still not sure how I feel about it.  It feels very top-k-esque.}
...
\raft{I got lost in this paragraph; what is the message? (Every paragraph should have a clear message)}
...
\raft{Tried to condense; old version commented out. \jessie{burn unless you want the reminder.  new version is good to me.}}
...
We assume that a given discrete prediction problem, such as classification, is given in the form of a discrete \emph{target loss} where the report set $\R$ is finite.\jessiet{Should we discuss the setting if one is given a discrete target statistic?  I'm fine with omitting, but I remember this as a growth in our thinking about this problem over the years.}
...
\jessiet{flag: I think we should move this figure down below and/or maybe replace it, but since we don't really discuss Bayes risks until much later... so maybe remove this sentence?}
...
For surrogate losses and properties, we will typically consider the report set $\reals^d$ \jessiet{Redundant with notation paragraph; not necessarily a bad thing, but worth thinking about if we want to keep this.}.
...
\jessiet{Commented out sentence below.  Thoughts?- Mar 24 22}
...
By results in \S~\ref{subsec:match-BR}, one could also show that $\hinge$ embeds $2\ellzo$ by the fact that their Bayes risks match (Figure~\ref{fig:bayes-risks-01}).\jessiet{I like this foreshadowing, but think we should move the figure down to \S 6.}
...
\caption{Bayes risks $\risk L : p \mapsto \inf_u \inprod{p}{L(u)}$ of 0-1, hinge, and logistic losses, respectively, plotted as a function of $p_1 = \Pr[Y=1]$.
	Observe that the Bayes risks of 0-1 and hinge loss are both piecewise linear and concave, while the Bayes risk of logistic loss is not piecewise linear.  Proposition~\ref{prop:embed-bayes-risks} states that embedding is equivalent to matching Bayes risks, confirming that hinge loss (M) embeds 0-1 loss (L), but logistic loss (R) does not. \raf{Take a step back and ask: what point do we (want to) make with this figure? Seems a shame not to mention the Bayes risk matching of the first two and how that relates to embedding.}\jessie{Big picture: there's something eerily similar about Bayes risks when embedding is in the picture.  In fact, Bayes risks match.  This was originally up here when the BR matchinf result was way earlier.  I think it provides some nice intuition if you understand what's going on, but it might be more helpful closer to that result.  Otherwise, we're just introducing more things}}
...
\jessiet{Commented out next setence since it didn't really vibe with rest of paragraph to me - Mar 24 22}
%We first explore in \S~\ref{sec:poly-loss-embed} the connection between embeddings and polyhedral surrogates.

%% note that embedding is sufficient, but not necessary
\jessiet{Does this location for this point make sense?}
...
A first observation is that if a loss $L$ elicits a property $\Gamma$, then $L$ restricted to some representative set $\Sc$, denoted $L|_\Sc$, elicits $\Gamma$ restricted to $\Sc$. \jessiet{Already in the notation... delete this?}
...
\jessiet{Do we want this paragraph?- May 3, 22}
...
In this section, we prove Theorem~\ref{thm:link-main}, restated below, which gives a construction to generate calibrated links for \emph{any} polyhedral surrogate. \jessiet{Haven't defined calibrated yet, so maybe want to say consistent? idk what is best - May 3, 22}
\jessie{in link state: calibrated vs consistent?}
...
For any $p \in \simplex$, consider the optimal surrogate report set, i.e. set of the form $U = \Gamma(p) = \argmin_u \inprod{p}{L(u)}$. \jessiet{Adding ``for any $p$'' quantification - May 3, 22 \# burn}
...
To warm up, we study the abstain surrogate given by~\citet{ramaswamy2018consistent}, and show how to rederive their link function and surrogate regret bounds (\S~\ref{sec:abstain}). \jessiet{regret bounds?}
...
In structured binary classification and top-$k$ classification, our framework also gives new consistent surrogates and link functions which would likely have been extremely challenging to derive without our framework.
...
Letting $d = \ceil{\log_2(n)}$, their surrogate $L^{1/2} : \reals^d \to \reals^\Y_+$ given by
...
\begin{equation}\label{eq:abstain-link-1}
  ...
  \end{cases}~.
\end{equation}
Construction~\ref{const:eps-thick-link} gives calibration of $(L^{1/2},\psi')$ with respect to $\ell^{1/2}$. 
...
Moreover, they show that $L^f$ embeds a variant $\ellabs$ of $\ell^f$ where one is allowed to abstain on a set of indices $A \subseteq [k]$, which they call the \emph{structured abstain problem}.\jessiet{Changing $S$ to $A$ to not override notation from the submodular function.}
The inclusion of abstain options is natural when observing that the BEP surrogate $L^{1/2}$ from \S~\ref{sec:abstain} coincides with $L^f$ for the function $f(S) = \ones\{S \neq \emptyset\}$, so the multiclass abstain problem must be a special case of the Lov\'asz hinge.
...
Perhaps surprisingly, by deriving a link envelope $\Psi$ which is contained in the envelopes for $L^f$ for all submodular and increasing $f$, they prove that $\psi^*(u) \subseteq \Psi(u)$ and $\psi^\diamond(u) \subseteq \Psi(u)$ for all $u \in \reals^d$.
...
\raft{was citet*; what does that even do? \jessie{lists all authors (e.g., instead of Finocchiaro et al., goes to Finocchiaro, Frongillo, and Waggoner) \# burn}}
...
\raft{I am not sure what all Yutong and Scott show, but if the main point is restrictions on the conditional label distributions rendering the surrogate consistent, I'd say we should move this subsection ahead of top-$k$, and flesh out a bit more what exactly we mean by these restrictions and why we can conclude consistency under them (at a high level).  Then when we mention the same idea for top-$k$, we don't have to go into detail (and currently we don't). \jessie{they give some distributional constraints for $L^{WW}$ with the argmax link $\psi : u \mapsto \argmax_i u_i$ to be consistent w.r.t. 0-1 loss.}}
%% one might make distributional assumptions to regain consistency
Upon showing that $L^{WW}$ embeds $\ell^\OP$, \citeauthor{wang2020weston} proceed to use their characterization of $\prop{\ell^\OP}$ to give sufficient distributions assumptions over labels in $\P \subseteq \simplex$ such that $L^{WW}$ and the canonical argmax link $\psi^1(u) : u \mapsto r \in \argmax_y \inprod{e_y}{u}$ are calibrated with respect to 0-1 loss on $\P \subseteq \simplex$ (i.e., that eq.~\ref{eq:calibrated} holds for all $p \in \P$).

%% how to understand level set simplex drawings
Sufficient constraints to recover consistency are characterized by comparing $\prop{\ell^\OP}$ and $\mode$.
...
To recover consistency on $\P \subseteq \simplex$, it suffices that $\P$ excludes these cells.
\jessiet{Next sentence redundant?  Can be massaged in to rest of paragraph?}
...
\begin{figure}[H]
	...
	\begin{minipage}{0.5\linewidth}
		\caption{Level sets of $\prop{\ell^\OP}$, the property elicited by the ordered partition loss and embedded by $L^{WW}$.
...
%\raft{Not sure you can use ``observed'' like that (came up a few times; see original)}
...
\begin{table*}
	...
	\caption{
		...
		\jessie{I honestly think this might be more clear in this paper if we remove the labels on the cells.  Thoughts?}
	}
...
\raf{TODO: definition mismatch for indirectly elicits (link specified or not?).  Suggestion: use ``refines'' only, since $\epsilon$-separation already implies the correct link.}
...
\end{lemma}
\begin{proof}
  This statement can be deduced from the embedding framework of \cite{finocchiaro2019embedding}.
...
\begin{proof}
  ...
  \raf{Inserted from nonconstructive proof:}
  ...
  Write $p = \sum_{q \in \mathcal{Q}_u} \beta(q) q$ for some probability distribution $\beta$.
