...
\raf{TODO: update section with formal theorem statements and new result about tight embedding}
...
\inf_{u\in\reals^d : \psi(u) \not \in \gamma(p)} \inprod{p}{L(u)} \geq	\inf_{u\in\reals^d : \psi^\emb(u) \not \in \gamma^\emb(p)} \inprod{p}{L(u)} > \inf_{u\in\reals^d}\inprod{p}{L(u)}~,
  ...
  showing calibration of $\psi$.
	Consistency follows as calibration and consistency are equivalent in this setting~\citep{bartlett2006convexity}\raft{I think we need a better reference, since I believe that's only for binary classification...}.
\end{proof}
