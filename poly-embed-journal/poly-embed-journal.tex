...
%The embedding framework also provides a way to verify the consistency or inconsistency of a given polyhedral surrogate, by first determining the discrete loss $\ell_{\mathrm{embed}}$ it embeds and comparing $\prop{\ell_{\mathrm{embed}}}$ to the property elicited by the proposed target $\prop{\ell}$.
...
\raft{We should discuss this a bit in the examples where appropriate, e.g. WW, and then forward ref in parens here to the relevant subsection(s) \jessie{burn?}}
...
As we illustrate in Figure~\ref{fig:abstain-links}(L), the link function $\psi$ proposed by \citeauthor{ramaswamy2018consistent} can be recovered from Construction~\ref{const:eps-thick-link} by choosing the norm $\|\cdot\|_\infty$ and setting $\epsilon=1/2$ and choosing $\psi^{1/2}(u) = \bot$ for all $u$ such that $\bot \in \Psi(u)$. 
(For all $u$ such that $\bot \not \in \Psi(u)$, there is only one choice to link, e.g., $|\Psi(u)| = 1$.)
Hence, our framework can be used to verify the calibration of the prposed link $\psi^{1/2}$. \jessiet{changed this sentence.  Thoughts?}%would have simplified the process of finding such a link, and the corresponding proof of consistency.
...
\begin{equation}\label{eq:abstain-link-1}
  ...
  \end{cases}~.
\end{equation}
Construction~\ref{const:eps-thick-link} immediately gives calibration of $(L_{1/2},\psi_1)$ with respect to $\ell^{1/2}$. \jessie{does it?  do we know $\epsilon$ is small enough for higher $d$?}
...
Thinking of the abstain loss $\ell^{1/2}$ as a binary structured prediction problem, \citeauthor{ourlovaszpaper} observe that the BEP surrogate $L_{1/2}$ in eq.~\eqref{eq:abstain-surrogate} is one example of a Lov\'asz hinge introduced in \S~\ref{sec:lovasz-hinge}. %, and that $\psi^{1/2}$ in eq.~\eqref{eq:abstain-link} is an example of a calibrated link given by Construction~\ref{const:eps-thick-link} for the embedded loss.
...
Thus, the Lov\'asz hinge is consistent with respect to the structured abstain loss $\ellabs$.
...
\citet{finocchiaro2022consistenttopk} apply the embedding framework to these surrogates to understand the target problems for which previously proposed polyhedral surrogates are consistent.
...
Each of the examined surrogates embeds a discrete loss which can be viewed as a variant of the top-$k$ problem, allowing for varying levels of weight on the outcomes or predictions of sets smaller than $k$ elements.
For example, one of the surrogates $L^{(4)}(u)_y = \left(1 - u_y + \frac 1 k \sum_{i=1}^k (u_{\setminus y}_{[i]})\right)_+$ is minimized in expectation by predicting the set of labels which are ``likely enough,'' up to $k$ labels.
The loss embedded by $L^{(4)}$ might be preferable to top-$k$ in certain settings: for example, if a search engine chooses $c < k$ web results to display for a query by optimizing $L^{(4)}$, the additional $k-c$ slots may be opened to advertisements.


Moreover, \citeauthor{finocchiaro2022consistenttopk} uses the construction from the proof of Theorem~\ref{thm:discrete-loss-poly-embeddable} to derive the first calibrated polyhedral surrogate for $\elltopk$, given in eq.~\eqref{eq:topk-embedding}.
...
\citep[Theorem 4.4]{finocchiaro2022consistenttopk} confirms that indeed $(L^k,\psi^k)$ is calibrated with respect to $\elltopk$ for this link $\psi^k$, by showing that $\psi^k$ is an $\epsilon$-separated link for $\elltopk$ with any $\epsilon \leq \frac 1 {2n}$ via the construction of the link envelope $\Psi$ in Construction~\ref{const:eps-thick-link}.
\jessiet{COmmenting out reference to old figure}
...
\end{equation}
The Weston-Watkins hinge was later shown to be inconsistent with respect to 0-1 loss~\citep{tewari2007consistency,liu2007fisher}. %, \citet{dogan2016unified} nonetheless advocate for the Weston-Watkins SVM to be the default for multiclass prediction problems. 
...
%\citeauthor{wang2020weston} then apply Theorem~\ref{thm:link-main} to conclude that there exists a link function $\psi$ such that $(L_{WW},\psi)$ is calibrated with respect to $\ell^\OP$; however, they do not construct the link. 
\jessiet{Commented out a sentence above; do we want [a modification] of it included?}
...
These cells are juxtaposed with the cells $\{p \in \simplex \mid T \in \mode(p)\}$, outlined in dashed blue, for which $L_{WW}$ was originally proposed as a surrogate.
