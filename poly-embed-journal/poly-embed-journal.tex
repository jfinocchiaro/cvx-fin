...
If there was no $v \in \V^*$ such that $x \in F_v$, then $g(x) = \min_{H_v \in \H_\Y} \inprod{v}{x}$.
...
\end{claim}
\begin{claim}
	Thus, $\pi(F_{\hat v}) \subseteq \pi(F_v)$. \raf{This will show (3) once we translate back}
\end{claim}
\begin{claim}
	$\R'$ is a rep set for $L$ if and only if $\V \subseteq L(\R')$.
\end{claim}
\begin{corollary}
	$\R^*$ is a min rep set for $L$ if and only if $\V^* = L(\R^*)$. \raf{(1)}
\end{corollary}
