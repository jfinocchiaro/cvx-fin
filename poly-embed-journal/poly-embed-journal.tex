...
We show some additional structure of embeddings, such as a reducing embedding to matching Bayes risks.
Finally, we show that when restricting to polyhedral surrogates, indirect property elicitation is equivalent to consistent.
...
\raft{Note to self: revisit abstract}
\end{abstract}
...
For discrete target losses and properties, we will take $\R$ to be any finite set, and use lowercase notation $\ell$ and $\gamma$, respectively.
Any property $\gamma:\simplex\toto\R$ for a finite set $\R$ is called a \emph{finite property}.
\raft{Rewrote to (hopefully) make our notational conventions clearer \jessie{Re-arranged slightly to match the re-write of loss notation, but feel free to undo if more confusing. - 16 Sept 21}}
...
Applying the $\epsilon$-thickened link construction additionally enables one to verify the consistency of a proposed link.
...
Studying existing surrogates using our framework generally proceeds as follows.
Let a surrogate $L:\reals^d \to \reals^\Y_+$ be given.
...
\citeauthor{ramaswamy2018consistent} provide a polyhedral surrogate for $\ellabs{\alpha}$, which we present here for $\alpha=1/2$.
...
\end{equation}
where embeds outcomes to corners of the $\pm 1$ hypercube, and the abstain report to the origin. %; let us assume $n = 2^d$ so that we have a bijection
%\raft{Do we actually need the bijection?  Might be simpler to leave this statement out otherwise}.
...
Aside from its simplicity, one possible advantage of $\psi_1$ is that it appears to yield the same constant in generalization bounds as $\psi$, yet assigns $\bot$ to much less of the surrogate space $\reals^d$\raft{Let's check this statement with Bo; I thought when we actually worked it out that the bound seemed to be worse.  In that case, we could simply say ``Aside from its simplicity, one possible advantage of $\psi_1$ is that it assigns $\bot$ to much less of the surrogate space $\reals^d$.''}.
It would be interesting to compare the two links in practice.

\raft{Let's clear up what we mean by the elements of this tuple, or cut it entirely.  What is $W$?}
...
While the authors provide theoretical justification and experiments, consistency of the Lov\'asz hinge is left open, which we resolve. \jessiet{Want to cite the preprint instead of saying we resolve this?}
...
In \S~\ref{app:lovasz} \jessiet{Change to citation?}, we generalize this observation for all $k$.
...
This result casts doubt on the practical effectiveness of the Lov\'asz hinge as a surrogate for $\ell^f$.\jessie{We might have a reviewer fight us on that unless we add something about ``without low-noise assumptions''}
...
We show in \S~\ref{app:lovasz}\jessie{CITE} that the answer is a variant of abstain loss where one can abstain on a set of labels $B\subseteq[k]$.
...
On the other hand, of course, this revelation suggests a positive result as well: under an appropriate link from our construction in Theorem~\ref{thm:link-main}, the Lov\'asz hinge could be fruitfully applied to set-valued prediction problems when one allows the learning algorithm to abstain on a subset of labels.
...
\citet{wang2020weston} show that $L_{WW}$ embeds $\ell^\OP$, and conclude that $L_{WW}$ is calibrated with respect to $\ell^\OP$\raft{Do they construct a link, or start with the existing one?  If the latter, I'm curious how they show consistency; using our framework, as we now describe in 4, or something else?}\jessiet{They just show embedding and don't provide a link, citing our construction and leaving it to existence}.
Specifically, they use the embedding
% \citet[Definition 2.3]{wang2020weston}
$\varphi : \T \to \reals^n$ given by $\varphi(T)_j = -(i-1)$ for all $j \in T_i \setminus T_{i-1}$.
\raf{Here would be a good place to define the link $\psi$, even if informally.}\jessie{They don't define $\psi$- they just take its existence for granted as a corollary of showing embedding.}
...
However, this seems to encapsulate more information than necessary and therefore presumably requires more data to learn the Bayes optimal hypothesis than learning the less granular top-$k$ property.

\raf{Take one more pass?  Define the brackets notation.  Feel free to guess at what our results are going to be, and then add margin comments that we should revisit.}
Other hinge-like surrogates are shown to be non-consistent by~\citet{lapin2016loss} and~\citet{yang2018consistency}.
For example, $\psi_2$ from~\citet[Table 1]{yang2018consistency} elicits essentially the same property $\prop{WW}$ shown in Figure~\ref{fig:ordered-partition} before the link is applied for certain values of $n$ and $k$.
...
Moreover, \citeauthor{ourtopkpaper} presents the embedding in Eq.~\eqref{eq:topk-embedding} for top-$k$ loss that looks unlike the commonly studied surrogates in the literature.
In Equation~\eqref{eq:topk-embedding}, $u_{[i]}$ denotes the $i^{th}$ largest element of $u \in \reals^n$.

\begin{align}\label{eq:topk-embedding}
L(u,y) &= \max \left(u_{[1]}, \max_{m \in \{k+1, \ldots, m\}} \left[ 1 - \frac k m + \frac 1 m \sum_{i=1}^m u_{[i]}\right] \right)- u_y
\end{align}

Perhaps the most natural link function for a proposed top-$k$ surrogate is the argmax link 
$\psi^{\tt argmax}(u) := \argmax_{S \subseteq \Y : |S| = k} \sum_{i \in S} u_i$ linking to the $k$ highest elements of $u$.
While the embeddings framework guarantees the existence of a calibrated link, we cannot immediately conclude the pair $(L, \psi^{\tt argmax})$ is calibrated with respect to the top-$k$ loss.
\citeauthor{ourtopkpaper} use the $\epsilon$-thickening link construction as a tool to verify this natural conjecture in the following way:
...
\end{proposition}
\begin{proof}
  \raft{This is sort of a ``clever'' proof, probably not in a good way.  Something that used Lemma X more directly would probably be better than using Bayes risk matching.  But if correct I think it's not bad. \jessie{It's a little subtle in parts, but checks out to me.  Feel free to \# burn}}
...
\btw{JOURNAL: cool to point out that when $L$ is polyhedral, $\prop{L}$ has a finite range (this result) and so does its (multivalued map) inverse (trim result)! \jessie{Think this is a bit outdated.  at least pre lemma X.  Feel free to burn}}
...
It is straightforward to verify that consistency, and therefore calibration, implies indirect elicitation~\citep{finocchiaro2021unifying,gyorfi2002distribution,fan1998efficient}\raft{Add cites \jessie{Any more?}}.
...
\jessie{I don't think we want all of these... I feel like it sends a signal about the work being incomplete.  Also, I think we should take out of bullet format}
...
Given any $v$ in a given set $\V$, define $H^+_v := H^+_{([v;-1], 0)}$, where $[v; -1]\in\reals^\Y_+\times\reals$ define a halfspace.
Similarly, we denote $H_y^+ := H_{([e_y; 0], 0)}^+$ for any $y \in \Y$; the latter will help us restrict our $\hyp(g_\V)$ to the nonnegative orthant. 
Similarly, we let $H_v := H_{([v;-1], 0)}$ for $v \in \reals^\Y_+$ and define $H_y := H_{([e_y;0], 0)}$. 

%Finally, we define the face generated by $v \in \V$ with $F_v := H_v \cap G$ for any $v\in\V$.
Finally, we let $\H_{\V} = \{H_v^+ \mid v\in\V\}$ denote the set of halfspaces generated by $\V$, $\H_\Y = \{H_y^+ \mid y\in\Y\}$, $\H = \H_{\V} \cup \H_\Y$.
...
Throughout this section let $G:= \hyp(g_V)$ for a fixed finite set $\V$.
...
\end{corollary}
\begin{proof}
  $\H_\Y \cup \H_{\V^*} = \H^* \subseteq \H = \H_\Y \cup \H_{\V}$, ergo $\V^* \subseteq \V'$. \jessiet{Add refs}
\end{proof}

We now show that we can equivalently construct $g$ through the unique finite set $\V^*$ instead of the given set of vectors $\V$.
...
\begin{proof}
	...
	\begin{align*}
	(x,c) \in \hyp(G)
	&\iff g(x) \geq c & \text{ Definition of hypograph}\\
	...
	\end{align*}
	Combining the two equalities (e.g., $\cap \H = (\cap \H_\Y) \cap (\cap \H_\V)$), we have $\hyp(g) = \cap \H$.
	
\end{proof}

%\raf{Need to use minimizable}
We can now use the results from \S~\ref{appsubsec:phase1} to conclude that $\hyp(g)$ has a unique minimum halfspace representation $\cap \H^*$, where $\H^* = \H_\Y \cup \H_{\V^*}$ for some unique finite set $\V^*$.
Moreover, we have $g = g_{\V^*}$, as $\H^*$ is the unique minimum halfspace representation for $\hyp(g)$.
...
Suppose that we are given a set $\V$ and finite minimum representative set $\V^*$ such that $g_\V = g_{\V^*}$.\jessie{Too much assumption}
...
For the set $\V^*$, we have each of these projected facets being full-dimensional in the projected space as well.
...
\end{claim}
\begin{proof}
	By Corollary~\ref{cor:unique-set-loss-vectors-defining-facets}, $F_v$ is a facet of $G$ in $\reals^d_+$, meaning it is $(d - 1)$-dimensional.
...
Given a loss $L$ with $\risk L_+$ polyhedral, take $v = L(r)$ for any $r \in \R$.
...
\begin{proof}
  ...
  Moreover, since $f$ is polyhedral, it is finitely generated \citep[Proposition 19.1.2]{rockafellar1997convex} and can be written 
	...
	\end{align*}
\end{proof}
Thi allows us to project $g$ from $\reals^\Y_+$ to the simplex $\simplex$.
...
\end{corollary}
Given the function $f$, we consider $g$ to be its extension and $L$ such that $\risk L_+ = g$ so that we may use the tools in \S~\ref{appsubsec:phase1} and~\ref{appsubsec:phase2} to draw conclusions about the property $\Gamma := \prop{L}$ defined on the simplex.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \theta(v)
  \end{align*}
  The result follows from Claim~\ref{claim:theta-is-pi-cap-simplex}.
...
Now that we have translated from $\reals^d_+$ to $\reals^\Y$ in \S~\ref{subsec:project-pi} and from $\reals^\Y_+$ to $\simplex$ in \S~\ref{subsec:project-f}, we can take the final steps to prove Lemma~\ref{lem:X}.
