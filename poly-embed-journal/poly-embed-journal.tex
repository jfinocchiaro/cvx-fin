...
\usepackage[natbib,preprint]{jmlr2e-hack} %% FIX BEFORE SUBMITTING
...
In work on $\H$-consistency, realizability assumptions often imply assumptions on the conditional distribution that exclude many of the regions causing inconsistency issues in \S~\ref{sec:applications}. \jessiet{Raf, I know you said something about the conditional distribution being a point mass, but I'm not sure I fully understand/buy the argument.}
Under these assumptions (or others), it would be interesting to understand if polyhedral embedding still implies $\H$-consistency.
Since $\H$-consistency is not equivalent to the corresponding analogue of calibration (c.f., \citep{awasthi2021calibration,awasthi2021finer}) it is unclear if our results can be immediately extended.
\citet{awasthi2022hconsistency,awasthi2022multi} additionally study surrogate regret bounds for $\H$-consistent surrogates for adversarial classification and multi-class classification, though it is unclear how such bounds generalize to general finite prediction tasks.
