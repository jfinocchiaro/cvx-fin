...
\begin{theorem}\label{thm:polyhedron-uniquely-gen-facets}
  ...
  (iii) $\{H\cap P \mid H^+ \in \H^*\}$ is the set of facets of $P$.
\end{theorem}
\begin{proof}
	Since $P$ is $d$-dimensional in $\reals^d$, it therefore has nonempty interior.	
	\citet[Proposition 4.5(i)]{gallier2008notes} can then be applied to yield the existence of a unique finite set of closed halfspaces $\H^*$ uniquely determining $P$. (i)
	...
	By (i), we know that $H_{(w,b)}$ uniquely determines a facet of $P$.
	The result then follows if there is no set of $\H' := \{H^+_{(w_i, b_i)}\}_{i=1}^l \subseteq \H \setminus \H^*$ such that $\cap \H' = H_{(w,b)}$.
	As halfspaces are uniquely determined, this is only true if some $H_{(w_i,b_i)} \in \H'$ is precisely equal to $H^+_{(w,b)}$, contradicting the assumption that $\H^* \not \subseteq \H$.
	\jessiet{This is super hand-wavy, and not nearly as precise as I'd like, but it's a first pass.}
...
Given any $v$ in a given set $\V$, define $H^+_v := H^+_{(v, -1)}$, where $(v, -1)\in\reals^\Y_+\times\reals$ define a halfspace.
...
We can now use the results from \S~\ref{appsubsec:phase1} to conclude that $\hyp(g_\V)$ has a unique minimum halfspace representation $\cap \H^*$, where $\H^* = \H_\Y \cup \H_{\V^*}$ for the unique finite set $\V^*$.
...
\begin{corollary}\label{cor:finite-rep-set}
	Given a minimizable loss $L : \R \times \Y \to \reals$ with polyhedral extended Bayes risk $\risk L_+$, take $L(\R) = \V$ and $\V^*$ to be the unique minimum set such that $\hyp(g_{\V}) = \cap (\H_\Y \cup \H_{\V^*})$.
...
Suppose that we are given a set $\V$ and the unique finite set $\V^* \subseteq \V$ such that $g_\V = g_{\V^*}$.
...
\begin{proof}
	...
	%
	By Claim~\ref{claim:g-gen-by-Vstar}, we know that $g_\V(x) = g_{\V^*}(x) = \min_{v \in \V^*}\inprod{v}{x}$.
...
\begin{claim}\label{claim:Vprime-projected-faces-cover-iffprojected-faces-subsets}
	...
	For $\V' \subseteq \V := L(\R)$, we have $\cup_{v \in \V'} \pi(F_v) = \reals_+^\Y \iff \Lambda \subseteq \{ \pi(F_v) \mid v \in L(\R') \}$.
...
\begin{proof}
	...
	Since $H^+_v$ is valid, we have $F_v = F_{(v,-1)}$ is a face by definition.
