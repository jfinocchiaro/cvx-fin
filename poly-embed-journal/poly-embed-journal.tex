...
\begin{proof}
	...
	Observe that for any report $r \in \R$, the hyperplane $H^L_r : p \mapsto (p, -\inprod{p}{L(r)})$ supports $E_L$ on exactly the level set.
	...
	Observe that any $r^* \in \R^*$ must be a full-dimensional cell of the power diagram, since if it was a lower-dimensional, it would be a subset of some full-dimensional cell, and we would contradict that $\R^*$ is minimum.
	...
	If $\Gamma_r$ is not full-dimensional, it must be a face of some full-dimensional level set $\Gamma_{r^*}$, and therefore $\Gamma_r \subset \Gamma_{r^*}$.
\end{proof}
...
$1 \Rightarrow 2$:
Since $\Gamma$ embeds a finite property $\gamma$ with minimum representative set $\Sc$, $\Gamma$ also has a finite minimum representative set $\U := \varphi(\Sc)$.
We claim $\trimnonred(\Gamma) = \{\Gamma_u \mid u \in \U\} = \{\gamma_r \mid r \in \Sc \}$, and the result follows from $\U$ being a finite minimum representative set.
Since $\U$ is a finite minimum representative set, $\trimnonred(\Gamma) \supseteq \{\Gamma_u \mid u \in \U\}$ follows immediately.
We now show $\trimnonred(\Gamma) \subseteq \{\Gamma_u \mid u \in \U\}$.
Consider any $\Gamma_{u'} \in \trimnonred(\Gamma)$.
Since $\Gamma$ has the finite minimum representative set $\U$, if $u' \not \in \U$, there is some $u \in \U$ such that $\Gamma_{u'} \subseteq \Gamma_u$ by Lemma~\ref{lem:finite-minrep-weakly-dominate}. 
If $\Gamma_{u'} \subsetneq \Gamma_u$, then $u' \not \in \U_\equiv$, and $\Gamma_{u'} \not \in \trimnonred(\Gamma)$ nor is $u' \in \U$.
If $\Gamma_{u'} = \Gamma_u$, then$u' \equiv u$ so $\Gamma_{u'} \in \trimnonred(\Gamma)$ as $[u'] = [u] \in \U_\equiv$.
Thus, we have $\trimnonred(\Gamma) = \{\Gamma_u \mid u \in \U\}$, so $\trimnonred(\Gamma)$ is finite and unions to the simplex.

$2 \Rightarrow 3$:
Since $\trimnonred(\Gamma)$ is finite and covers the simplex, we have $\U_\equiv$ finite.
Consider any set $\U = \{u \mid [u] \in \U_\equiv\}$ to be a selection of elements of $\U_\equiv$ from each equivalence class.
We claim $\{\Gamma_u \mid u \in \U_\equiv\} = \{\Gamma_u \mid u \in \U\}$, and the result follows immediately.
...
To see $\{\Gamma_u \mid u \in \U_\equiv\} \subseteq \{\Gamma_u \mid u \in \U\}$, consider any $u \in \U_\equiv$.
If $u \in \U$, the result follows immediately.
If $u \not \in \U$, then we know there is no report in $u' \in \U$ such that $\Gamma_{u} \subsetneq \Gamma_{u'}$ by virtue of $u \in \U_\equiv$, so we must have some $u' \in \U$ such that $\Gamma_u = \Gamma_{u'}$.
Therefore, $u \equiv u'$, so $\Gamma_u = \Gamma_{u'} \in \{\Gamma_{u'} \mid u' \in \U\}$.
Thus, there is a finite minimum representative set $\U$ such that $\trimnonred(\Gamma) = \{\Gamma_u \mid u \in \U\}$.
...
$3 \Rightarrow 4$:
Since level sets of elicitable properties are closed, the set $\Theta = \{\Gamma_u \mid u \in \U\}$ yields a finite covering of the simplex.\jessiet{How much effort do we need to put into showing each level set is full-dimensional?}

$4 \Leftarrow 3$:
Since $\Theta$ is a finite set of full-dimensional level sets, and these level sets are affinely equivalent to the epigraph of negative risk $\mathrm{epi}(-\risk L)$, it must follow that $-\risk L$ is polyhedral (convexity follows as a property of $-\risk L$). 
...
From Proposition~\ref{prop:SL-minimum}, we also know there is a bijection between the facets of $\mathrm{epi}(-\risk L)$ (corresponding to the level sets $\theta \in \Theta$) and the level sets $\{\Gamma_u \mid u \in \U\}$, so the generated sets are equal.
