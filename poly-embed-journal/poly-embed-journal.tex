...
\begin{proof}
	...
	Now, to construct a finite $\V \subseteq L(\R)$ such that $g_\V = g_{L(\R)}$, iterate over each of the (finitely many) faces of $g_{L(\R)}$.
	...
	\jessiet{Added construction on how to get all the faces.}
...
\begin{corollary}\label{cor:unique-set-loss-vectors-defining-facets}
  ...
  (ii) $F_v$ is a facet of $\hyp(g_\V)$ for each $v\in\V^*$.\raft{Is $F_v$ defined?  Looks like it got cut (we were going to move it to this subsec maybe) define in line}
  (iii) $\V^* \subseteq \V$.
