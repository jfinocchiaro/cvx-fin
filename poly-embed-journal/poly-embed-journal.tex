...
We will later consider losses over a finite set of outcomes $\Y$; for now, we use $\reals^\Y_+$ to denote $\reals^{|\Y|}_+$, and let $d := |\Y|+1$.
Suppose we have a finite set $\V \subset \reals_+^\Y$.
...
\begin{itemize}
\item Define $G = \{(x,c) \mid c \leq g(x)\} \subseteq \reals^\Y_+ \times \reals$ to be the hypograph of $g$.
\item We define $H^+_v := H^+_{([v;-1], 0)}$ for a given $v$, where $[v; -1]\in\reals^\Y_+\times\reals$.
\item Additionally, denote $H_y^+ := H_{([e_y; 0], 0)}^+$ for any $y \in \Y$. % \{ (x,c) \in \reals^\Y \times \reals \mid x_y \geq 0\}$.
\item Similarly, we let $H_v := H_{([v;-1], 0)}$ for $v \in \reals^\Y_+$ and define $H_y$ analogously. 
\item Define $F_v := H_v \cap G$ for any $v\in\reals^\Y_+$.
...
\begin{claim}\label{claim:x-nonneg-orthant-iff-intersection-HY}
	$x \in \reals^\Y_+ \iff (x,c)\in \cap \H_\Y$ for all $c \in \reals$.
\end{claim}
\begin{proof}
	$\implies$ 
	Fix any $c \in \reals$.
...
\begin{claim}\label{claim:G-full-dimensional}
	$G$ is $(|\Y| + 1)$-dimensional.  %\raf{$g$ is nonnegative on $\reals^\Y_+$, so $G$ contains $\{(x,c) \mid x\in\reals^\Y_+, c \leq 0\}$, which is full-dim.}
\end{claim}
\begin{proof}
...
\begin{claim}\label{claim:G-unique-facets-Hstar}
...
\begin{claim}\label{claim:HY-subset-Hstar}
	$\H_\Y \subseteq \H^*$.
\end{claim}
\begin{proof}
	...
	The first cannot happen as we take $g$ to be defined (and finite) on $x = e_y \in \reals^\Y_+$.
...
Since $G$ is full-dimensional, all facets of $G$ are uniquely determined by the hyperplanes $H$ whose halfspaces $H^+$ compose $\H^*$ by Claim~\ref{claim:unique-set-loss-vectors-defining-facets}.
...
\end{claim}
\begin{proof}
	$G = \cap \H = \cap \H^* = \cap (\H_\Y \cup \H_{\V^*}) = \{(x,c) \in \reals_+^\Y \times \reals \mid \inprod{v^*}{x} \geq c$ for all $v^* \in \V^* \}$.
...
First, $G \subset H^+_{v^*}$ by definition of $G$ as the intersection of halfspaces including $H^+_{v^*}$.
...
\begin{lemma}\label{lem:projected-faces-iff-support-iff-argmin}
	$x \in \pi(F_v) \iff (x,g(x)) \in F_v \iff v \in \argmin_{v' \in \V} \inprod{v'}{x}$
\end{lemma}
\begin{proof}
	\jessie{Don't like this in align environment}
	\begin{align*}
	x \in \pi(F_v) &\iff (x,g(x)) \in \{(x',c') \mid \inprod{v}{x'} = g(x) \} \\
	&\iff (x,g(x)) \in F_v \\
	&\iff (x,g(x)) \in \{(x',c') \mid \inprod{v}{x'} = \min_{v' \in \V} \inprod{v'}{x'}\} \\
...
\begin{claim}\label{claim:Vprime-projected-faces-cover-iff-Vstar-subset-Vprime}
  ...
  $\cup_{v\in\V'} \pi(F_v) = \reals^\Y_+ \iff \V^* \subseteq \V'$.\jessie{Will get us to REP $\iff$ $\V^* \subseteq L(R')$ (3)}
\end{claim}
\begin{proof}
  Would like to show
  $\cup_{v\in\V'} \pi(F_v) = \reals^\Y_+ \iff \cap(\H_\Y \cup \H_{\V'}) = G$ then apply Corollary~\ref{cor:anything-gen-G-subset-Hstar}\jessie{make reference}.

  Suppose $\cup_{v\in\V'} \pi(F_v) = \reals^\Y_+$.
  Take any $x\in\reals^\Y_+$.
  By Lemma~\ref{lem:projected-faces-iff-support-iff-argmin}, we have $(x,g(x)) \in F_v$.
  Let $c := g(x)$.
  ...
  $(x,c) \in \cap \H_\Y$ for all $c \in \reals$ by Claim~\ref{claim:x-nonneg-orthant-iff-intersection-HY}, so we just need to show that $(x,c) \in \cap \H_{\V'} \implies (x,c) \in G$.
  We know there exists a $v' \in \V'$ such that $(x,g(x)) \in F_{v'}$, and $\inprod{v'}{x} = g(x)$.
  Therefore $\inprod{v'}{x} = g(x) \leq \inprod{v}{x}$ for all $v \in \V$, so $(x,g(x)) \in \cap \H_\V \implies (x,g(x)) \in G = \cap (\H_\Y \cup \H_\V)$.
...
\begin{claim}\label{claim:projected-Vstar-faces-full-dim}
	...
	\jessie{$\Lambda$ is exactly the set of FDLS; Will lead to $\Theta$ are exactly the full dimensional level sets over simplex (6).}
\end{claim}
\begin{proof}
	By Claim~\ref{claim:unique-set-loss-vectors-defining-facets}, $F_v$ is a facet of $G$ in $\reals^d_+$, meaning it is $((|\Y| + 1) - 1)$-dimensional.
...
\begin{proof}
	...
	\jessie{Need to be more careful?}
...
\begin{claim}\label{claim:Vprime-projected-faces-cover-iffprojected-faces-subsets}
	...
	\jessie{Will lead to rep iff $\Theta$ subset of level sets generated by $\R'$.}
\end{claim}
\begin{proof}
	$\implies$
...
\end{claim}\label{claim:faces-contained-in-facets}
\begin{proof}
...
\begin{corollary}\label{cor:projected-facets-subset-projected-facet}
...
\begin{claim}\label{claim:exists-vstar-projected-face-subset}
	...
	\jessie{Will show 7 on $\reals^\Y_+$}
\end{claim}
\begin{proof}
	This is exactly the two previous results chained together.
\end{proof}
...
\begin{corollary}\label{claim:level-set-is-projected-face}
	\jessie{Once start talking about properties}
	For all $r \in \R$, $\Gamma_r = \pi(F_{L(r)}) \cap \simplex$.
\end{corollary}
\begin{proof}
...
\begin{claim}
	For any $r \in \R$, there exists a $v^* \in \V^*$ such that $\Gamma_r \subseteq \theta(v^*)$.\jessie{(7)}
\end{claim}
\begin{proof}
	Let $v = L(r) \in \V$.
