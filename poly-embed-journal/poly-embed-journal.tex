...
\end{claim}
\begin{proof}
We can write any face $F_v = H_v \cap G = \{x \in \reals_+^{d-1} \mid \inprod{x}{v} = g(x)\}$.
...
Since $\H^* = \H_\Y \cup \H_{\V^*}$, if there was no $v \in \V^*$ such that $x \in F_v$, it must be the case that $\min_{H_v \in \H_\Y} \inprod{v}{x} = \min_{y \in \Y} x_y = g(x) = 0$ by construction of the halfspaces in $\H_\Y$. %, and we must have $x \not \in \reals_{++}^\Y$.
...
\begin{claim} 
For all $v\in\reals^\Y_+$, if $H_v$ supports $G$,  $\dim(\pi(F_v)) = \dim(F_v)-1$.
\end{claim}
\begin{proof}
	...
	Any $k\leq d$ vectors $\{z_i\}_{i=1}^k$ forming a basis for $\{(x,c) \in G \mid \inprod{v}{x} = c, c = g(x) \}$ \jessie{....}
...
\end{claim}
\begin{proof}
%	Suppose $F_v \subseteq F_{v^*}$.
%	If $v' \not \in \V^*$, then $v' = [e_y; 0]$ for some $y \in \Y$.
%	By construction of $G$, the polyhedron shoots off to $-\infty$, so the only subfaces of $F_{v'}$ are also adjacent 
%	\jessie{Very informal...} 
\end{proof}
