...
Traditionally, one considers $P$ to be a trivial face of itself; we exclude this case throughout.
\raft{It was a bit confusing because you said to observe that $P$ was a face of itself when you defined face to exclude that possibility}
...
\begin{definition}[{\citet{gallier2008notes}}]\label{def:irredundant}
	...
	If $\cap \H'$ is an irredundant decomposition of $P$ and $\H = \H' \cup \{H^+\}$ for $H^+\ not \in \H'$, then we call $\partial H^+$ a redundant face of $P$.
\end{definition}
...
%	This can only iterate up to $|\H^*|$ times, so there must be some finite irredunant representation for $P$.\jessie{This feels way too spelled out for its lack of detail.}
  ...
  By (ii), each $H^+ \in \H^*$  determines a facet of $P$, and this facet is irredundant by (i)\raft{Not defined (I think) what it means for a facet to be irredundant}.
...
A first observation is that the region generated by the intersection of the $H^+_y$ halfspaces restricts the hypograph of any $g_\V$ to be finite only on the nonnegative orthant.
...
We will work with any $\V \subseteq L(\R) \subseteq \reals^\Y_+$ which is finite.
...
\end{lemma}
\begin{proof}
  Since $\hyp(g_\V)$ is full-dimensional by Lemma~\ref{lem:G-full-dimensional}, $\hyp(g_\V) \cap H$ being a facet follows immediately from Theorem~\ref{thm:polyhedron-uniquely-gen-facets}(i) and (iii).
...
\jessie{Cor 9 justifies the use of assumption 2 since for every $L$ wiht polyhedral extended risk, such a $\V$ and $\V^*$ exist.}
...
Given a finite set $\V^* \subset \reals^\Y_+$, we denote $\Lambda_{S} := \{\pi(F^{\hyp(g_{\V^*})}_v) \mid v \in S\}$ as the set of projected facets generated by $S$.
...
\begin{claim}\label{claim:Vprime-projected-faces-cover-iffprojected-faces-subsets}
	...
	Let $F_v := F^{\hyp(g_{\V^*})}_v$ denote the face of $\hyp(g_{\V^*})$ generated by $v \in L(\R)$.
	%For $\V' \subseteq \V$, 
...
\begin{claim}\label{claim:vface-subset-some-vstar-face}
	Consider $L, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}.
	For any $v \in L(\R)$, the face $F_{v} \subseteq F_{v^*}$ for some $v^* \in \V^*$. \jessiet{Need to clarify that face of $\hyp(g_\V)$?} %must be a subset of one of the $\V^*$ facets $F_{v^*}$ (as opposed to the vertical ones)
\end{claim}
\begin{proof}
...
Since $\risk L$ is polyhedral and concave, there exists a finite set $\V$ such that $L$ and $\V$ satisfy Assumption~\ref{assum:V-star-exists-infinite} by Proposition~\ref{prop:poly-risk-assumption-satisfied}.
