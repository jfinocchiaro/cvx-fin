...
\begin{definition}[Adapted from {\citet{gallier2008notes}}]\label{def:irredundant}
	...
	If $\cap \H'$ is an irredundant decomposition of $P$, then we call $\partial H^+$ an irredundant face of $P$ for each $H^+ \in \H'$.\jessiet{Added this sentence but not sure if it's necessary anymore.}
\end{definition}
...
Throughout, we will work with a function $g_\V$ generated by a set $\V \subseteq \reals^\Y_+$ of the following form, often polyhedral (but concave).
...
We proceed with some observations about facets and dimension of $\hyp(g_\V)$ in order to use the \emph{smallest} halfspace representation $\H^*$ for $g_{L(\R)} = g_\V$.
...
With a bit of preliminary work, we can apply Theorem~\ref{thm:polyhedron-uniquely-gen-facets} to observe a unique irredundant halfspace representation $\H^*$ generating $\hyp(g_\V)$.
...
\begin{proof}
  ...
  Since $\hyp(g_\V)$ is full-dimensional, $\hyp(g_\V) \cap H$ being a facet follows immediately from Theorem~\ref{thm:polyhedron-uniquely-gen-facets}(ii) and (iii).
  ...
  The second cannot be true by construction of $g_\V$ since $\V$ is finite including the convex indicator on $\reals^\Y_+$\raft{Couldn't follow this last line, since $\delta$ can be $\infty$\jessie{does it help that it's minus the convex indicator?}}. 
\end{proof}
...
\begin{claim}\label{claim:Vprime-projected-faces-cover-iff-Vstar-subset-Vprime}
	...
	$\cup_{v\in\V'} \pi(F^{\hyp(g_{\V^*})}_v) = \reals^\Y_+ \iff \V^* \subseteq \V'$.\btw{Will get us to REP $\iff$ $\V^* \subseteq L(R')$ (3).}
\end{claim}
\begin{proof}
...
We now claim that a set of projected faces $\{F^{\hyp(g_{L(\R)})}_v\}_{v \in \V'}$ \jessiet{Diff between $\{F^{\hyp(g_{L(\R)})}_v\}_{v \in \V'}$ and $\{F^{\hyp(g_{\V^*})}_v\}_{v \in \V'}$?}for some $\V'$ will cover $\reals^\Y_+$ if and only if $\V^* \subseteq \V'$.
...
\begin{proof}
	...
	It then suffices to show, for $v \in L(\R)$ that if $F_v := H_v \cap \hyp(g_{\V^*})$ is a face of the facet $F_y := H_y \cap \hyp(g_{\V^*})$ for some $y\in \Y$, then it must also be a face of $F_{v^*}$ for a $v^* \in \V^*$; it suffices to show $F_v$ is not a facet, and thus $F_v \neq F_y$.
...
Moreover, let $\R^*$ be a set such that $\V^* := L(\R^*)$.
