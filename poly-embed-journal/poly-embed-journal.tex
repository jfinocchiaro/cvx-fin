...
\begin{proposition}\label{prop:tfae-min-rep-nonredundant}
	...
	\begin{enumerate}
		...
		\item[c.] $\Gamma_r$ is full-dimensional for all $r \in \R$, the multiset $\{\Gamma_r : r \in \R\}$ has no duplicate elements, and $\cup_{r\in\R} \Gamma_r = \simplex$.
	\end{enumerate}
\end{proposition}
\begin{proof}[Sketch]
	$(a \implies b)$  
	...
	$(c \implies a)$
	As $\Gamma_r$ is full-dimensional and the multiset $\{\Gamma_r : r \in \R\}$ contains no duplicate elements (that is, there are no $r, r' \in \R$ such that $\Gamma_r = \Gamma_{r'}$), for each $r \in \R$ there must be some distribution $p_r \in \simplex$ so that $\Gamma(p_r) = \{r\}$.
...
\end{corollary}
\jessie{I think we want a little bit more than this, as we want flexibility for bijections of the reports.  Testing}
\begin{corollary}
	If $\R$ is a (finite) minimum representative set for $\ell$ and $L$ embeds $\ell$, then $L$ tightly embeds $\ell$.
\end{corollary}


\begin{definition}[Trim]\label{def:trim-loss}
	Given an loss $L : \R \to \reals^\Y_+$, we define $\trim(L) = \{L(u) : u \in \R \textrm{ s.t. } \neg\exists u'\in\R,u'\neq u,\, \Gamma_u \subsetneq \Gamma_{u'}\}$.
\end{definition}

Observe that if a loss $L : \R \to \reals_+^\Y$ is non-redundant, then $\{L(r) \mid r \in \R\} = \trim(L)$ by applying the definition of non-redundancy.

\begin{corollary}
	For the loss $L: \R \to \reals^\Y_+$, $\trim(L)$ is unique.
\end{corollary}
This follows from Proposition~\ref{prop:tfae-min-rep-nonredundant}, as $\trim(L)$ moves from $L$ to remove any non-redundant reports.
$\trim$ removes redundancy of strict subsets (that is, $\Gamma_r \subseteq \Gamma_{r'}$) by the set comprehension, while it removes fully redundant losses ($\Gamma_r = \Gamma_{r'}$) by definition of a set.

\begin{proposition}
	Let $\ell$ be a discrete loss.
