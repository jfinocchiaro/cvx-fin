...
Plugging this finite representative set into Lemma~\ref{lem:loss-restrict} then yields an embedding.
...
For example, the literature on minimax optimization in supervised learning settings \citep{asif2015adversarial,farnia2016minimax,fathony2016adversarial} poses a minimax optimization problem of the expected loss matrix over both players' strategies, which becomes a convex relaxation of the discrete optimization problem\raft{We said ``poses the construction in Theorem~\ref{thm:discrete-loss-poly-embeddable}'' -- is it exactly the same?  We just have to say how it relates, like ``when viewing X as Y, one arrives at the same construction'', or maybe one is a special case of the other.}.
...
\begin{construction}[$\epsilon$-thickened link] \label{const:eps-thick-link}
  Given a polyhedral $L$ that embeds some $\ell$ via the embedding $\varphi: \Sc \to \reals^d$ for a representative set $\Sc$; and given an $\epsilon > 0$ and a norm $\|\cdot\|$, the \emph{$\epsilon$-thickened link} $\psi$ is constructed as follows.
  Define $\U = \{\Gamma(p) : p \in \simplex\}$.
  ...
  First, initialize the \emph{link envelope} $\Psi: \reals^d \to 2^{\Sc}$ by setting $\Psi(u) = \Sc$ for all $u$.\botodo{$\Psi$ maps to $2^{\Sc}$ because $\Psi(u) = \emptyset$ sometimes}
  Then for each $U \in \U$, for all points $u$ such that $\inf_{u^* \in U} \|u^*-u\| < \epsilon$, update $\Psi(u) = \Psi(u) \cap R_U$, where $R_U = \{r \in \Sc : \varphi(r) \in U\}$ is the set of target reports that embed into $U$.
...
\begin{proof}
  ...
  By definition, $\Psi(u) = \cap_{p \in P_u} R_p$ where $R_p = \{r \in \Sc : \varphi(r) \in \Gamma(p)\}$, the set of reports that embed into $\Gamma(p)$.
  By definition of embedding, $R_p = \{r \in \Sc : r \in \gamma(p)\} = \gamma(p) \cap \Sc$.
  By definition of separation, a link $\psi$ is $\epsilon$-separated if and only if, for all $u$, $\psi(u) \in \cap_{p \in P_u} \left(\gamma(p) \cap \Sc\right) = \Psi(u)$.
\bo{There is one problem here. For points that are not within $\epsilon$ of any optimal set, can a separated link map them to $\R \setminus \Sc$? If so, we need to slightly modify the argument.}
