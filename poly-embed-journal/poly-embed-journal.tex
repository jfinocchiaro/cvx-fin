...
Several authors consider a variant of classification, with the addition of a ``reject'' or \emph{abstain} option\raft{Let's dump a few citations here, starting with Bartlett~\cite{bartlett2008classification}}.
...
\end{equation}
While it was later shown to be inconsistent~\citep{tewari2007consistency,liu2007fisher}, \citet{dogan2016unified} nonetheless advocate for the Weston-Watkins SVM to be the default for multiclass prediction problems.

\citet*{wang2020weston} use our embedding framework to construct and prove the calibration of the Weston-Watkins surrogate for the \emph{ordered partition} loss, and in turn, show inconsistency with respect to 0-1 loss\raft{But it was already known, right?  If so, it's good to use phrases like ``recover the result that''}.
...
\begin{align*}
\ell^\OP(T,y) &= \sum_{i=1}^{k} |T_{i}| \cdot \Ind{y \not \in T_{i-1}}-1~.~
\end{align*}
\raf{Jessie, could you check that the newer form is correct?  I messed with a couple things, and decided not to clutter the tex with the old stuff -- maybe check the diff if it would help, or go straight to the original version.}

This loss can be interpreted as a variation of 0-1 loss where one reports a nested sequence of sets, and the punishment for the outcome $y$ is the cardinality of the first set containing $y$, plus the cardinality of all smaller sets.
\citet{wang2020weston} use our framework to show that $L_{WW}$ embeds $\ell^\OP$, and thus is calibrated with respect to it\raft{Do they construct a link, or start with the existing one?  If the latter, I'm curious how they show consistency; using our framework, as we now describe in 4, or something else?}.
...
\raf{Here would be a good place to define the link $\psi$, even if informally.}
...
Moreover, \citet{ourtopkpaper} presents an embedding (Eq.~\eqref{eq:topk-embedding}) for top-$k$ loss that looks unlike the commonly studied surrogates in the literature.
Moreover, the most common link $\psi^{\tt argmax}(u) := \argmax_{S \subseteq \Y : |S| = k} \sum_{i \in S} u_i$ is calibrated for the surrogate in Equation~\eqref{eq:topk-embedding}.\jessie{TODO... confirm this is true.}
Their proof uses the $\epsilon$-thickened link construction from \S~\ref{sec:calibration} to understand which choices of linked values are feasible and align these choices with the argmax link.
