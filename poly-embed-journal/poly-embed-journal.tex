...
Many of the surrogate losses we consider will be \emph{polyhedral}, meaning piecewise linear and convex; we therefore briefly recall the relevant definitions.
...
\subsection{(Direct) Property Elicitation \jessie{Name?}}
...
\begin{definition}[Directly Elicits]
  ...
  \end{equation}
  If $L$ directly elicits a property, it is unique and we denote it $\prop{L}$.
\end{definition}
...
\begin{definition}
  ...
  We say $(L,\psi)$ is $\P$-\emph{calibrated} with respect to $\ell$ if
for all $p \in \P$,
  \begin{equation}
    \label{eq:calibrated}
