...
\begin{proposition}\label{prop:SL-minimal}
  ...
  \raft{Jessie: prove :-].  Note: min rep $\implies$ poly risk is immediate from Lemma~\ref{lem:loss-restrict}.  Move the construction into the proof.}
\end{proposition}
...
\jessie{This feels like it's missing a detail about how $\Sc$ is minimal and how level sets are full-dimensional.}
\end{proof}
...
\begin{lemma}
	...
	Then the range of $\Gamma$, $\U = \Gamma(\simplex) = \{\Gamma(p) \subseteq \reals^d : p\in\simplex\}$, is a finite set of closed polyhedra.
        \raft{Leave this statement and add a proof sketch; move other dependencies to appendix}
\end{lemma}
\begin{proof}
...
\begin{proof}[Sketch]
  ...
  %	Observe that the hyperplane $H_L(p_{r'}^r; r) := p_{r'}^r \mapsto \inprod{p_{r'}^r}{L(r)}$ supports $\risk{L}$ at $p_{r'}^r$ if and only if $r \in \Gamma(p_{r'}^r)$.
  %	Similarly, we can conclude $H_L(p_{r''}^r; r)$ supports $\risk{L}$ at $p_{r''}^r$.
...
\begin{proof}[Sketch]
  ...
  % If there was a level set $\Gamma_u$ for some $u \in \varphi(\R')$ that was not full-dimensional, it would have to be a subset of some other level set $\Gamma_{u'}$, but this contradicts that $\varphi(\R')$ is minimal. \jessie{Hand waving; make the argument you can't span relints of other level sets.}
  % 
  % 
