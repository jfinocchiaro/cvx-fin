...
Observe that while the Weston-Watkins hinge loss was originally designed as a surrogate for $0-1$ loss in multiclass settings, there is no link such that $(L^{WW}, \psi)$ is consistent with respect to $\ell^{0-1}$. 
However, we can construct a polyhedral surrogate and use the canonical argmax link for $\ell^{0-1}$ by simply using the $L^k$ surrogate with $k=1$. 
The $L^k$ surrogate simplifies to 
\begin{align*}
	L^1(u)_y &= \left(\max_m \frac{m-1}{m} + \frac 1 m \sum_{i=1}^m u_{[i]} \right) - u_y~.
\end{align*}
\jessie{Does this simplify further? I don't think so}
