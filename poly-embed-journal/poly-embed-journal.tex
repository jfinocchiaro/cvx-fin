...
\begin{lemma}
	...
	Define $\theta(v) = \{p \in \reals^n_+$\jessiet{$\simplex?$}$ \mid \inprod{v}{p} = \risk L(p)\}$, and $\Theta := \{\theta(v) \mid v \in \V\}$.
...
\begin{proof}
	Since $\risk L$ is a concave polyhedral function, it is the pointwise min of a finite set of linear functions by a corollary of~\citet[Theorem 19.1]{rockafellar1997convex}. %and therefore, the set of linear functions of minimum cardinality is well-defined and unique\jessiet{unique should be cite-able, but not sure where to find it.  My copy of Rockafellar is in COS right now.}.
	By the same result, we know that $E_L := \epi(\risk L)$ is closed and has finitely many faces.
