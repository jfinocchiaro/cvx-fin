...
While this problem is well-studied, \citep{lapin2015top, lapin2016loss, lapin2018analysis,yang2018consistency}\jessie{Add more} a class of surrogates for top-$k$, most of which are not convex and consistent.
...
\citet{TOPKPAPER} presents an embedding (Eq.~\eqref{eq:topk-embedding}) for top-$k$ loss that looks unlike the 
...
\raf{Add: explanation for why $B$ cannot be a singleton if we want a tight embedding... or maybe prove this first and then prove the stronger one --- downside with that is needing to define yet another discrete loss.}

\begin{lemma}
	\label{lem:lovasz-u}
	Let $\hat\ell^f:\hat\R\times\Y\to\reals_+$, where $\hat\R = \{(A,B) \in 2^N \times 2^N: A\cap B=\emptyset, |B|\neq 1\}$, be given by
	\begin{equation}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= 2^{-|B|} \sum_{T\subseteq B} f(\hat A\triangle T\setminus B') + f(\hat A\triangle T\cup B')
      \numberthis\label{eq:tightness-loss-rewrite}
  \end{align*}
  
  We first suppose $B \subseteq B'$.
  Then $\hat A\triangle T\setminus B' = \hat A \setminus B'$ and $\hat A\triangle T\cup B' = \hat A\cup B'$ for any $T \subseteq B$, giving the simpler form $\hat\ell^f((A',B');p) = f(\hat A\setminus B') + f(\hat A\cup B')$.
  Consider two cases (i) $\hat A\setminus B' \neq \emptyset$, and (ii) $\hat A\cup B' \supsetneq B$.
  We will show either (i) or (ii) hold, and then apply the fact that $f$ is strictly increasing.
  ...
  Now suppose $B \not\subseteq B'$.
  In this case, we will apply strict submodularity.
  Let us rewrite the expected loss,
  \begin{align*}
    \hat\ell^f((A',B');p)
    &= 2^{-|B|} \sum_{T\subseteq B} f(\hat A\triangle T\setminus B') + f(\hat A\triangle T\cup B')
    \\
    ...
    \\[-12pt]
    & \hspace*{85pt}
      + f(\hat A\triangle (B\setminus T)\setminus B') + f(\hat A\triangle (B\setminus T)\cup B') \biggr)
      \numberthis\label{eq:tightness-double-counting}
    \\
    &\geq \frac 1 2 \; 2^{-|B|} \sum_{T\subseteq B} \biggl(
      f\Bigl((\hat A\triangle T\setminus B') \cup (\hat A\triangle (B\setminus T)\cup B')\Bigr)
    \\[-10pt]
    & \hspace*{85pt}
      + f\Bigl((\hat A\triangle T\setminus B') \cap (\hat A\triangle (B\setminus T)\cup B')\Bigr)
    \\[2pt]
    & \hspace*{85pt}
      + f\Bigl((\hat A\triangle (B\setminus T)\setminus B') \cup (\hat A\triangle T\cup B')\Bigr)
    \\[-2pt]
    & \hspace*{85pt}
      + f\Bigl((\hat A\triangle (B\setminus T)\setminus B') \cap (\hat A\triangle T\cup B')\Bigr)
      \biggr)
      \numberthis\label{eq:tightness-apply-submodularity}
    \\
    &= \frac 1 2 \; 2^{-|B|} \sum_{T\subseteq B} \biggl(
      f\bigl(\hat A\cup B\cup B'\bigr) + f\bigl(\hat A \setminus (B\cup B')\bigr)
    \\[-10pt]
    & \hspace*{85pt}
      f\bigl(\hat A\cup B\cup B'\bigr) + f\bigl(\hat A \setminus (B\cup B')\bigr)
      \biggr)      ~,
      \numberthis\label{eq:tightness-simplify}
    \\
    &= f\bigl(\hat A\cup B\cup B'\bigr) + f\bigl(\hat A \setminus (B\cup B')\bigr)
    \\
    &\geq f(B) + f(\emptyset)
      ~,
      \numberthis\label{eq:tightness-weak-increasing}
  \end{align*}
  where in eq.~\eqref{eq:tightness-apply-submodularity} we applied submodularity twice, to the first and fourth terms and to the second and third terms, and in eq.~\eqref{eq:tightness-weak-increasing} we used the fact that $f$ is increasing.
  We will show that for at least one $T\subseteq B$, the first and fourth terms in eq.~\eqref{eq:tightness-apply-submodularity} correspond to incomparable sets, meaning the inequality is actually tight.
  
  As $|B|\neq 1$, we must have $|B|\geq 2$.
  Let $i,j\in B$ be distinct elements such that $i\notin B'$.
  We therefore have two cases for $i$, and three for $j$, and for each we claim there exists $T\subseteq B$ such that $\hat A\triangle T\setminus B'$ and $\hat A\triangle (B\setminus T)\cup B'$ are incomparable:
  \begin{center}
    \begin{tabular}{R|CC}
      & i\in A' & i\notin A'
      \\
      ...
      j\in A' & T=\{j\} & T=\{i,j\}
      \\
      j\in B' & T=B\setminus\{i\} & T=\{i\}
      \\
      j\notin A'\cup B' & T=\emptyset & T=\{i\}
    \end{tabular}
  \end{center}
  In particular, these choices give $i\in\hat A\triangle T$ and $i\notin\hat A\triangle (B\setminus T)$:
  in the first column, $i\in A'\setminus A \implies i\in \hat A$, so we ensure $i\notin T$, whereas we ensure $i\in T$ in the second column.
  Similarly, these choices give $j\notin\hat A\triangle T\setminus B'$ and $j\in\hat A\triangle (B\setminus T)\cup B'$:
  in the first row, $j\in \hat A$, so we ensure $j\in T$;
  in the second row, $j\in B'$, so we automatically have the condition;
  in the third row, $j\notin \hat A$, so we ensure $j\notin T$.
\end{proof}


\raf{Show also that $A,\{i\}$ is always weakly dominated by $A\cup\{i\},\emptyset$ or $A,\emptyset$.}


\raf{Other thoughts if we split this off into its own paper:}

\begin{enumerate}
\item 
  If modular on a sub lattice, what does that mean about abstentions wrt that sub lattice?  Maybe it’s not so bad to characterize the redundant reports as a function of f...
\item
  Related: what are examples of f functions in the wild?  Are they strict?
\item
  The link.  Would be super interesting to understand how to link properly when some reports are redundant.  Already not totally obvious for strictly modular and k=3... visualize!
\item
  Data set where Lh performs poorly wrt original target?
\end{enumerate}
...
\begin{proof}
  ...
  \jessiet{\url{https://arxiv.org/abs/0805.0292} Prop 4.5?}
\end{proof}
