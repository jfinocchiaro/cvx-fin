...
In Proposition~\ref{prop:embed-bayes-risks}, we show an equivalent condition to embedding, whose proof relies on the following Lemma about the Bayes Risk of a ``restricted'' surrogate.
...
\begin{proof}
  ...
  If $\Gamma$ embeds a finite $\gamma$, then Lemma~\ref{lem:embed-defs-equiv} says there is a discrete loss $\ell$ embedded by $L$ such that $\ell$ elicits $\gamma$.
...
\end{proof}
\jessiet{03.01.21 - Old version moved to comment}
...
\begin{theorem}\label{cor:ie-sufficient}
	If a polyhedral loss $L:\reals^d \to \reals^\Y$ indirectly elicits a property $\gamma': \simplex \toto \R'$, then for any loss $\ell'$ eliciting $\gamma'$, there exists a link $\bar \psi$ such that $(L, \bar \psi)$ is calibrated, and therefore consistent, with respect to $\ell'$.
\end{theorem}
\begin{proof}
	...
	Consistency now follows as calibration and consistency are equivalent in this setting.
\end{proof}
...
\btw{New (March 2 2021): adding Yutong's paper}
\subsection{Embedding Ordered Partitions}
\citet*{wang2020weston} use the embeddings to construct and prove the calibration of the Weston-Watkins surrogate for the \emph{Ordered Partition} loss.
...
The ordered partition target loss $\ell^{\OP}$ is defined, for $i \in [n]$ and $S = (S_1, S_l) \in \OP_n$ by
\begin{align*}
...
\btw{TODO: Add simplex diagrams for this!}
