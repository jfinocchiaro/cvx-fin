...
\raf{Theme: expanding our results from polyhedral losses to polyhedral Bayes risks. We can start by observing that the latter is a weaker condition.}
...
\begin{lemma}\label{lem:X}
  ...
  Then $L$ has a finite minimum representative set.
  ...
  \item $L$ tightly embeds $\ell:\R'\to\reals^\Y_+$ if and only if $\ell$ is injective and $\ell(\R') = \V$.
  \end{enumerate}
\end{lemma}
...
\begin{proof}
  ...
  Thus for all $p\in\simplex$ and $r\in\R^*$, we have
  \begin{equation*}
...
Previous work from~\citet[Proposition 4]{duchi2018multiclass} realized the significance of matching Bayes risks for calibration with respect to the 0-1 loss, but their result relies the Bayes risk of the surrogate being strictly concave.  As polyhedral functions are never strictly concave, Proposition~\ref{prop:embed-bayes-risk} extends their result to the embedding setting.
...
Motivated by this observation, let us define the ``trim'' of a loss to be this unique set of loss vectors induced by any minimum representative set.
...
Using this notion of trimming a loss, we can again recast our embedding condition: a loss embeds another if and only if they have the same $\trim$.
...
\begin{proposition}\label{prop:embed-iff-trims-equal}
  ...
  Then $L$ embeds a discrete loss $\ell$ if and only if $\trimcover(L) = \trimcover(\ell)$.
  Furthermore, $L$ tightly embeds $\ell$ if and only if $\ell$ is injective and $\trimcover(L) = \ell(\R)$.
\end{proposition}
\begin{proof}
  As $L$ has a finite representative set, $\risk{L}$ is polyhedral, and Lemma~\ref{lem:X} applies; let $\V$ be the set of loss vectors uniquely determined by $\risk{L}$.
  If $L$ embeds $\ell$, then we have $\risk{L} = \risk{\ell}$ from Proposition~\ref{prop:embed-bayes-risks}.
  Thus, Lemma~\ref{lem:X}(\ref{item:X-min-V}) gives $\trim(L) = \trim(\ell)$.

  \raf{Fill in the rest...}
\end{proof}
...
\begin{corollary}\label{prop:trim-loss-condition}
  ...
  Then $\trimcover(L) = \{L(u) \mid u \in \R \textrm{ s.t. } \neg\exists u'\in\R,u'\neq u,\, \Gamma_u \subsetneq \Gamma_{u'}\}$. 
\end{corollary}
\begin{proof}
  \raf{Didn't get to this proof}
...
As we define one loss embedding another, one can imagine an analogous definition where one property embeds another (Definition~\ref{def:prop-embed}), and we can define $\trimcover(\Gamma)$ as the set of level sets generated by a minimum representative set (Definition~\ref{def:trim-prop}).
...
Since embedding also means matching Bayes risks, the matching of $\trimred$ follows since the subgradients also trivially match.
...
\raf{Would suggest $r$ not $u$ here.}
Define $\trimred(\Gamma) = \{\Gamma_u \mid u \in \R \textrm{ s.t. } \neg\exists u'\in\R,u'\neq u,\, \Gamma_u \subsetneq \Gamma_{u'}\}$. 
