...
\end{claim}
\begin{proof}
  $G = \cap \H = \cap \H^* = \cap (\H_\Y \cup \H_{\V^*}) = \{(x,c) \in \reals_+^\Y \times \reals \mid \inprod{v^*}{x} \geq c$ for all $v^* \in \V^* \}$ where the first equality follows from Corollary~\ref{cor-anything-gen-G-subset-Hstar}.
...
\begin{proof}
  ...
  Want to conclude $a_i = 0$ for all $i$, meaning they are affinely independent.
...
\begin{proof}
  ...
  While this can happen, it must be on the boundary of $\reals_+^d$, and must be at the intersection of $H_y$ and some other $H_{v^*}$ for $v^* \in \V^*$.
...
\begin{corollary}\label{cor:f-matches-g-on-simplex}
  For all concave $f : \reals^{|\Y| + 1}_+ \to \reals_+ \cup \{-\infty\}$ with $\dom(f) = \simplex$, there is a polyhedral concave function $g : \reals^\Y_+ \to \reals_+$ with $\dom(g) = \reals^\Y_+$ so that $f(p) = g(p)$ for all $p \in \simplex$.
\end{corollary}
Given the function $f$, we consider $g$ to be such its extension and $L$ such that $\risk L_+ = g$ so that we may use the tools in Phases 1 and 2 draw conclusions about the property $\Gamma$.
...
\begin{proof}
  ...
  \jessie{Revisit; feels like it's missing something.}
\end{proof}


\begin{claim}\label{claim:level-set-is-projected-face}
  For all $r \in \R$, take $v \in \V$ so that $v = L(r)$.
  $\Gamma_r = \theta(v) = \pi(F_{v}) \cap \simplex$.
\end{claim}
\begin{proof}
...
\item REP $\iff \jessie{\Lambda := \{\pi(F_v) \mid v \in \V^*\}; \Lambda \subseteq \{ \pi(F_{v}) \mid v \in L(\R')\}}$.
  \jessie{Claim~\ref{claim:Vprime-projected-faces-cover-iffprojected-faces-subsets}}
\item $\Lambda$ == Full-dim \jessie{Claim~\ref{claim:projected-Vstar-faces-full-dim}.}
\item Lev sets subset $\theta$ \jessie{Just work with $\pi(F_{v})$ where $v \in L(\R)$}\jessie{Claim~\ref{claim:any-levelset-contained-in-minlevelset}}
\end{itemize}
...
\begin{claim}
  \jessie{(6)}
  The set of full-dimensional level sets of $\Gamma$ is exactly $\Theta$.
\end{claim}
\begin{proof}
