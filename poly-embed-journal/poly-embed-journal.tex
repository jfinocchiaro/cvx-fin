...
Often these regions are straightforward to identify based on the form of $L$.\jessie{tbh this sentence feels like a lie lol}
...
A key step in Construction~\ref{const:eps-thick-link} is constructing the link candidate function $\Psi$, which yields the possible values any $\epsilon$-separated link $\psi$ could map to; this in turn provides a tool to verify the calibration of a proposed link (\S~\ref{sec:abstain}, \ref{sec:top-k}).
...
\citeauthor{ramaswamy2018consistent} provide a polyhedral surrogate $L_\alpha$ for $\ell^{\alpha}$, which we present here for $\alpha=1/2$.
...
\begin{equation}\label{eq:abstain-link-1}
  ...
  \end{cases}~.
\end{equation}
Theorem~\ref{thm:link-main} immediately gives calibration of $(L_{1/2},\psi_1)$ with respect to $\ell^\alpha$.
...
\citet{ourlovaszpaper} use our embedding framework to resolve the consistency of $L^f$, showing that it is inconsistent with respect to $\ell^f$ outside of the trivial case where $f$ is modular and $L^f$ reduces to weighted Hamming loss.
...
\jessie{TODO: add a bit about the link construction}
...
\jessie{Natural link to test is a rounding link, yeah?}
...
\label{sec:top-k}
In settings like object recognition and information retrieval, the following top-$k$ classification problem arises, in which one predicts a set $S$ of size $k$, and if the true outcome is $y$, receives loss $\elltopk(S)_y = \ones\{y\notin S\}$~\citep{lapin2015top, lapin2016loss, lapin2018analysis,yang2018consistency,berrada2018smooth,rastegari2011scalable,reddi2019stochastic}.
In evaluating many of the existing surrogates for top-$k$ consider the following three desiderata: convexity, calibration, and piecewise linear (``hinge-like'') structure.
The surrogates that are convex and calibrated but not polyhedral often elicit the entire distribution $p \in \simplex$, and take the link to be the top $k$ elements of $p$, as noted by~\citep{lapin2016loss}, which learns more information than desired and seems to require more samples to converge.\jessie{fact check}
...
\citeauthor{finocchiaro2022consistenttopk} consider the polyhedral surrogates proposed by \citet[Table 1]{yang2018consistency} (cf. $\psi_2, \psi_3, \psi_4$) and evaluate \emph{which discrete losses these surrogates embed}, if not top-$k$.
They find that none of the surrogates embed top-$k$ but do characterize the discrete losses embedded, and use these characterizations to characterize (tighter\jessie{not sure how to say this.  better bounds than YK gave, which was just the abstain cell for all the surrogates}) sufficient assumptions on the data distribution in order be calibrated with respect to top-$k$, fixing the surrogate to be the canonical top-$k$ surrogate.
...
\citeauthor{finocchiaro2022consistenttopk} Theorem 4.4 confirms that indeed $(L,\psi)$ is calibrated with respect to $\elltopk$ for this link $\psi$, by showing that $\psi$ is an $\epsilon$-separated link for top-$k$ with any $\epsilon < \frac 1 {2n}$.\jessie{General note in this section: notation should get cleaned up}
