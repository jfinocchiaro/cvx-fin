...
\begin{proposition}\label{prop:SL-minimum}
  A loss $L: \R \to \reals^\Y_+$ has a polyhedral Bayes risk if and only if it has a finite minimum representive set.\raft{Let's change to ``finite representive set''}
...
%
	...
	We show $\trimnonred(\gamma) = \trimnonred(\Gamma)$, and the result will follow, since $\gamma$ is a finite property, and therefore $\trim(\gamma)$ is also finite, but its level sets still union to the simplex since $\trimnonred(\cdot)$ only removes strictly redundant level sets.
		
	Suppose the finite loss $\ell$ elicits $\gamma$; the negative Bayes risk of $\ell$ is a convex polyhedral function, and the epigraph of negative risk $E_\ell$ is polyhedral.
	Each face of $E_\ell$ corresponds to a level set of $\gamma$ as the hyperplane $H^\ell_r : p \mapsto (p, \inprod{p}{-\ell(r)})$ supports $E_\ell$ on exactly the level set $\gamma_r$.
	Since the faces of $E_\ell$ are closed and convex polyhedron and $E_\ell$ is affinely equivalent to a power diagram (Theorem~\ref{thm:aurenhammer}), each each level set $\gamma_r$ is a face of a convex polyhedron.
	Moreover, each $\gamma_r \in \trimnonred(\gamma)$ is a facet, since any lower-dimensional face is contained in a facet.
	
	By embedding, we have $H^L_{\varphi(r)} = H^\ell_r$ for all $r \in \R$, and similarly, we have every $\Gamma_u$ is a region of support for the hyperplane $H^L_u$ and is therefore a face of $E_L$.
	
	Since $\Gamma$ embeds $\gamma$ and $\gamma$ is elicitable, we have $\gamma_r = \Gamma_{\varphi(r)}$ for all $r \in \Sc$ and therefore $E_L$ is also  polyhedral since $\varphi(\Sc)$ is representative.\jessiet{Do we need to worry about the technicality of embedding over $\Sc \subset \R$ or are we fine with elicitability of $\gamma$?}
	Observe that for any $s \in \R \setminus [\Sc]$\jessiet{$\{s' : s' \equiv s \in \Sc\}$}, $\gamma_{s} \not \in \trimnonred(\gamma)$ since $\Sc$ is representative\jessiet{hand waving}, which gives $\trimnonred(\Gamma) \supseteq \trimnonred(\gamma)$, and we just need to show the other direction of subset inclusion.
	
	Consider 2 cases: (a) $u \in \varphi(\R), u \in \U_\equiv$, (b) $u \not \in \varphi(\R), u \in \U_\equiv$.
	For (a), given $u = \varphi(r)$, we have $\gamma_r = \Gamma_u$ by the embedding condition, so $\Gamma_u$ also supports $E_L$ on a facet and is therefore not a strict subset of any other level set.
	Thus, $\Gamma_u \in \trimnonred(\gamma)$.
	(b) follows similarly, except that there has to be some $u'$ such that $u \equiv u'$ such that $u' = \varphi(r)$; we conclude $\gamma_r = \Gamma_{u'} = \Gamma_u$ by embedding then equivalence of reports.
	Therefore, $\Gamma_u \in \trimnonred(\gamma)$.
	%For (c), if $u \not \in \U_\equiv$, then we have $\Gamma_u \not \in \trimnonred(\Gamma)$, and the statement follows vacuously.
	We conclude $\trimnonred(\Gamma) = \trimnonred(\gamma)$, and (2) follows.
	
	
%	Suppose for contradiction that there was a $u \in \U_\equiv$ (e.g., $\Gamma_u \in \trimnonred(\Gamma)$) such that $\Gamma_u \neq \gamma_r$ for any $r \in \R_\equiv$.
%	We know that $\Gamma_u$ is not a subset of any other level set $\Gamma_{u'}$ for $u' \in \U_\equiv$.
%	However, for any $p \in \Gamma_u$, there is some $r \in \R_\equiv$ such that $p \in \gamma_r \cap \Gamma_{\varphi(r)}$ by properties being nonempty and the embedding condition.
%	Since we claim $u \not\equiv u'$, 
...
%	Then we can apply Lemma~\ref{lem:loss-restrict} to observe that $L$ embeds $L|_\U$, and therefore $\Gamma$ embeds $\prop{\ell}$, which is finite as $\ell$ is discrete.
	...
	The result follows if we show $\U_\equiv$ is minimum.
	Recall $\trimnonred(\Gamma) = \{\Gamma_u \mid u \in \U_\equiv\}$.
	Since $\trimnonred(\Gamma)$ is finite and it is generated by $\U_\equiv$, so must $\U_\equiv$ by its construction containing no duplicate elements.
	Take any $\U = \{u \mid [u] \in \U_\equiv\}$ to be any finite set of reports forming $\trimnonred(\Gamma)$.
	Since $\cup \trimnonred(\Gamma) = \cup_{u \in \U} \Gamma_u = \simplex$, we have $\U$ representative, and just need to show $\U$ is minimum.
	
	If $\U$ was not minimum, there would be some report $u \in \U$ so that $\U \setminus \{u\}$ is also representative.
	...
	Therefore, every level set $\Gamma_u$ is a face of a convex polyhedron: we correspond in two cases, according to if $\Gamma_{u}$ corresponds to a facet or some lower-dimensional face.
	If $\Gamma_{u}$ is not a facet (but must still be a face) of a convex polytope, it must be contained in a facet, and therefore there is some $u'$ such that $\Gamma_{u} \subsetneq \Gamma_{u'}$, so $\Gamma_u \not \in \trimnonred(\Gamma)$.
	If $\Gamma_{u}$ corresponds to a facet, then it has a well-defined nonempty interior, and since there is no $u' \in \U$ such that $u' \equiv u$ by construction of $\U$, we have a $p \in \inter{\Gamma_{u}}$ such that $\{u\} = \Gamma(p)$.
	Therefore, $\U \setminus \{u\}$ would not be representative.
	We conclude $\U$ as constructed must be a minimum representative set.
	
	
	3 $\Rightarrow$ 1:
	Since $\U$ is representative, we have $\gamma(p) := \Gamma(p) \cap \U$ nonempty for all $p \in \simplex$, and therefore, $\Gamma$ embeds $\Gamma|_\U$ via the identity, which is finite as $\U$ is finite.
	Since $\U$ is minimum, we additionally have $\{\gamma_u \mid u \in \U\} =  \{\Gamma_u \mid u \in \U\} =\trimnonred(\gamma)$ by construction both of $\gamma$ and of $\U$.
...
Since $\Gamma$ has a finite representative set, we know its level sets are faces of convex polyhedron by equivalence to a power diagram intersected with the simplex (Theorem~\ref{thm:aurenhammer}).
...
4 $\Rightarrow$ 3:
We want to show that for each $\theta \in \Theta$, we cannot have $\Theta \setminus \{\theta\}$ be representative.
...
Therefore, $\Theta \setminus \{\Gamma_u\}$ is not representative, so $\Theta$ is a finite minimum representative set for $\Gamma$.
