...
\begin{definition}
  ...
  We say $S \subseteq \R$ is \emph{representative for $L$} if we have $\prop{L}(p) \cap S \neq \emptyset$ for all $p\in \simplex$.
\end{definition}

\begin{definition}[Embedding a loss]\label{def:loss-embed}
  \raft{Alternative: define ``embed over $S$'' first, and then ``embed'' means over some representative $S$.  Let's see if that would work.  Then we can call out $S$ to specify it if we want, but we don't need to carry it around everywhere and change all the statements if we don't want to call out $S$.}
  ...
  \end{equation}
\end{definition}
\proposedadd{Moreover, we say $L$ embeds $\ell$ over the set $S$ if $L$ embeds $\ell|_S$ and $S$ is representative for $\ell$.}
%
...
\begin{lemma}\label{lem:loss-restrict}
  Let $L$ elicit $\Gamma:\simplex\toto\R_1$, and let $\R_2\subseteq\R_1$ \proposedadd{be representative for $L$}.
  % such that $\Gamma(p) \cap \R_2 \neq \emptyset$ for all $p\in\simplex$.
  Then $L|_{\R_2}$ elicits $\gamma:\simplex\toto\R_2$ defined by $\gamma(p) = \Gamma(p)\cap \R_2$, and $\risk{L}=\risk{L|_{\R_2}}$.
\end{lemma}
\begin{proof}
...
\end{proposition}
\begin{proof}
  \raft{I'm going ahead with the proposed definition regarding $S$, since I think we can't avoid that for this result at least.}
  \raft{BTW, maybe we want $\mathcal S$ instead of $S$, not sure}
  Define $\Gamma = \prop{L}$ and $\gamma = \prop{\ell}$.
  Suppose $L$ embeds $\ell$ over $S\subseteq \R$ via the embedding $\varphi:S\to\reals^d$.
  Since $S$ is representative, for all $p\in\simplex$ we have $S\cap \gamma(p) \neq \emptyset$.
  \raf{Can we remove $\gamma'$ and show more directly that $\U$ is representative for $\Gamma$?  We only use the second part of Lemma~\ref{lem:loss-restrict} anyway.}
  Letting $\U := \varphi(S)$, define $\gamma':\simplex \toto \U$ by $\gamma' : p \mapsto \Gamma(p)\cap \U$.
...
\begin{corollary}
  ...
  If a finite set $\U \subseteq \reals^d$ is representative for $L$, then $L$ embeds the discrete loss $L|_\U$.
\end{corollary}
...
\raft{Oh, saw this last: didn't realize ``this'' meant the proposition (right?).  If so maybe we should demote the proposition to a corollary or just an observation.}
...
\begin{proof}
  ...
  \raft{Don't need power diagrams here either.  Also, I think once we have a nice clean proof of Proposition~\ref{prop:embed-bayes-risks} we can resolve the redundancy of these proofs easily, perhaps by proving the stronger statement in Proposition~\ref{prop:embed-bayes-risks}.}
