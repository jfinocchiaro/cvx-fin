...
Motivated by this observation, let us define the ``trim'' of a loss to be this unique set of loss vectors induced by any minimum representative set, as guaranteed by Lemma~\ref{lem:X}(\ref{item:X-min-V}).
...
Precisely, given a property $\Gamma:\simplex\toto\R$, define\raft{I think the ``$r'\neq r$'' is not needed, right?}
