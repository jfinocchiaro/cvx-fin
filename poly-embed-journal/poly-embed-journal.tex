...
\jessie{ TODO Items 01 Mar 2021
	\begin{itemize}
		...
		\item see if you can isolate what exactly we need from PDs and state it as a new lemma (which is just about Bayes risks)
		\item rethink proofs of Prop 1 and 2, see if we resolve the redundancy
		\end{itemize}
...
Our first main result, Proposition~\ref{prop:embed-bayes-risks}, focuses on matching the values of Bayes Risks, while the following result from~\citet{aurenhammer1987power} allows us to move towards understanding the projection of the Bayes Risk onto the simplex $\simplex$.
...
%  Let $\Gamma = \prop{L}$ and $\gamma = \prop{\ell}$.
  ...
  \begin{equation*}
    \risk{\ell}(p) = \min_{r \in \R}\inprod{p}{\ell(r)} = \min_{r \in \R}\inprod{p}{L(\varphi(r))} = \min_{u \in \U}\inprod{p}{L(u)} = \risk{L|_\U}(p) = \risk{L}(p)~,
  \end{equation*}
  for all $p\in\simplex$.
  ...
  In what follows, we implicitly work in the affine hull of $\simplex$ by taking an appropriate invertible linear projection $A : \reals^n_+ \to \reals^{n-1}_+$, so that interiors are well-defined, and $\risk{\ell}$ may be differentiable on the interior of $\simplex$.
...
\jessiet{Can remove this; took the part of the proof used in Thm 2 and moved there.  It's not a dependency anywhere else.}
...
\begin{proof}
  ...
  If $L$ embeds $\ell$, Proposition~\ref{prop:embed-bayes-risks} gives us $\risk{L} = \risk{\ell}$, and its proof already argued that $\risk{\ell}$ is polyhedral since $\risk{\ell}$ is a pointwise max of a finite number of linear functions.
...
This allows us to reason about the range of the property elicited by a polyhedral loss; in particular, it is finite.
...
\end{theorem}
 \begin{proof}
	The projection of the epigraph of $\risk{L}$ onto $\simplex$ forms a power diagram by Theorem~\ref{thm:aurenhammer} with finitely many cells $C_1,\ldots,C_k$, which we can index by $\R := \{1,\ldots,k\}$.
	Defining the property $\gamma:\simplex\toto\R$ by $\gamma_r = C_r$ for $r\in\R$, we see that the same construction gives us points $u_r \in\reals^d$ such that $u_r \in \Gamma(p) \iff r \in \gamma(p)$.
	Define $\ell:\R\to\reals^\Y_+$ by $\ell(r) = L(u_r)$.
	Since $L$ is polyhedral and $\gamma(p)\cap \R$ is nonempty for all $p \in \simplex$, we have $\risk{L} = \risk{L|_{\R}}$ polyhedral by Lemma~\ref{lem:poly-loss-poly-risk}.
 \end{proof}
...
Note however that the number of dimensions $d$ required could be as large as $|\Y|$, which is particularly undesirable in structured prediction and information retrieval problems with exponentially many outcomes.
Recent work~\citep{ramaswamy2016convex,finocchiaro2020embedding,finocchiaro2021unifying} yield characterizations for bounding the convex calibration and embedding dimensions.
...
Consider some invertible linear transformation $A : \reals^n \to \reals^{n-1}$ that reduces to an affine subspace.
Take $L' : \reals^{n-1} \to \reals^\Y$ as $x \mapsto L(A^{-1}(x))$ and $L$ defined by $u \mapsto (-\ell)^*(u) \ones - u$ where $\ell$ is that same as it is in Proposition~\ref{prop:embed-bayes-risks}, so that $\inprod{\ell(r)}{p} = \inprod{\L(\varphi(r))}{p} = \inprod{L'(A(\varphi(r)))}{p} $ for all $p \in \simplex$.
The Bayes risks $\risk{L}(p)$ and $\risk{L'}(p)$ are equal for all $p \in \simplex$; thus, the result holds in $n-1$ dimensions as well.}\jessiet{Leaving this change in the macro since we haven't made the change in proposition 1 yet.}
