...
A growing body of work seeks to design and analyze convex surrogates for particular target loss functions, and more broadly, understand the best empirical risk minimization bounds that can found for a surrogate, for which consistency is a necessary condition.
...
We assume that a given discrete prediction problem, such as classification, is given in the form of a discrete \emph{target loss} $\ell:\R\to\reals^\Y_+$, which maps a report (prediction) $r$ from a finite set $\R$ to the vector of loss values $\ell(r) = (\ell(r)_y)_{y\in\Y}$ for each possible outcome $y\in\Y$.
...
\bo{This slightly clashes with statement above in 2.1 that $L$ means a surrogate loss.}
...
Because of this, a key notion to define an embedding is that of a \emph{representative set}: a set of reports $\Sc$ such that, for all label distributions, at least one report $r\in\Sc$ minimizes expected loss.
\begin{definition}[Representative set]
  ...
  % We further say $\Sc$ is a \emph{minimum representative set} if it has the smallest cardinality among all representative sets.
\end{definition}
\bo{Removed a paragraph here and folded it into the above. It's good to explain why we need representative sets -- i.e. the first sentence was great. But I don't think we should go into any details of various ways in which reports can be redundant. It doesn't matter, we can forward ref. Instead though, I think we should give some intuition for why it will be convenient.}
...
\end{minipage}
\caption{Bayes risks of 0-1, hinge, and logistic losses, respectively, in binary classification setting. \jessie{Move this down to when we actually discuss risks matching?}}
...
\end{lemma}
\botodo{I didn't see us define $L|_\Sc$ anywhere yet! Although it's briefly mentioned in the intro.}\raft{Yeah, maybe recall the def here?}
\botodo{Notation in the lemma is confusing because $L$ is supposed to be a surrogate with report space $\reals^d$ and $\R$ is supposed to be the report space for $\ell$.}\raft{We should discuss this point}
\begin{proof}
...
In light of this negative result, together with the fact that every polyhedral surrogate embeds some target loss (Theorem~\ref{thm:poly-embeds-discrete}), we now turn to the question: what target problem, then, \emph{is} the Lov\'asz hinge consistent for?
...
\begin{theorem}\label{thm:lovasz-embed-main}
	...
	\begin{equation}
	\hat\ell^f((A,B),S) = f(A\triangle S\setminus B) + f(A\triangle S\cup B)~.
	\end{equation}
	Then the Lov\'asz hinge $L^f$ embeds $\hat\ell^f$.
  Moreover, the embedding is tight whenever $f$ is strictly submodular.
\end{theorem}
\raf{Something about how this could actually be a useful target and surrogate, provided you actually link from the surrogate to the target and not ignore $B$.}
