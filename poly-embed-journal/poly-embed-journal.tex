...
To set the stage for Construction \ref{const:eps-thick-link}, we sketch the two main steps in proving Theorem \ref{thm:thickened-separated}: (i) showing that one can produce a link $\psi$ such that $(L,\psi)$ indirectly elicits $\ell$; (ii) ``thickening'' $\psi$ such that it is separated.

For (i), begin by linking each embedding point back to its original report.
...
Indirect elicitation implies that they do;
we use the definition of embedding and elicitation results, discussed in \S~\ref{sec:min-rep-sets}, to show that $L$ must indirectly elicit $\ell$, giving a value of $\psi(u)$ for non-embedding points.
In particular, for each such $u$ there exists $r \in \R$ such that any $p$ satisfying $u \in \Gamma(p)$ also satisfies $r \in \gamma(p)$.
Thus if $u \in \Gamma(p) \cap \Gamma(p')$, then there exists $r \in \gamma(p) \cap \gamma(p')$, so we may safely choose $\psi(u) = r$.

\begin{lemma}\label{lem:embed-implies-indirect-elic}
  Let polyhedral loss $L$ embed some discrete loss $\ell$.
  Then $L$ indirectly elicits $\prop\ell$.
\end{lemma}
...
\begin{proposition}\label{prop:poly-risk-assumption-satisfied}
	Given a minimizable loss $L : \R \to \reals^\Y_+$ such that $\risk L_+$ is polyhedral, there exists a finite $\V$ and $\V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}.
\end{proposition}
\begin{proof}
	\jessie{TODO}
\end{proof}
...
Given $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, denote $\Lambda_{S} := \{\pi(F_v) \mid v \in S\}$ as the set of projected facets generated by $\V^*$.
...
\begin{lemma}\label{lem:any-levelset-contained-in-minlevelset}
  Consider $L : \R \to \reals^\Y_+, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, and $\R^* \subseteq \R$ the set such that $\V^* = L(\R^*)$.
