...
\begin{proof}
  ...
  We have $F_y = H_{(e_y, 0)} \cap \hyp(g_{L(\R)}) = \{(x,c) \mid x_y = 0, c \leq g_{L(\R)}(x)\} = \{(x,c) \mid x\in\reals^\Y_+, x_y=0, c \leq g_{L(\R)}(x)\} \supseteq \{(x,c) \mid x\in\reals^\Y_+, x_y=0, c \leq 0\}$ as $g_{L(\R)}(x) \geq 0$.
...
\end{align*}
Therefore, it is positively $1$-homogeneous, and its extension $\risk L_+$ is polyhedral immediately\raft{Is the ``it'' $\risk L$?  Since then it's not $1$-homog, its extension is.  You have to connect the display to the conclusion; one way is to write out what $\risk L_+$ is, but you have to justify it either way.}.
\end{proof}
...
Moreover, there must be a finite set $\R^*$ such that $\V^* = L(\R^*)$ by construction, which is representative as some element of $\R^*$ generates the Bayes risk for every $p \in \simplex$ as $\risk L = \min_{v \in L(\R^*)}\inprod{p}{v} - \delta\{p \mid \simplex\}$\raft{``generates the Bayes risk'' not defined; need to slow down here I think and find the rigorous statement among the many things you proved!}.
