...
\jessie{do we want more detail or save it for the caption?}
...
We first observe that the region generated by the intersection of the $H^+_y$ halfspaces restricts the hypograph of any $g_\V$ to the nonnegative orthant.
...
Observe that $L(\R)$ and $\H_{L(\R)}$ could be infinite sets.  
Now let $\H = \H_\Y \cup \H_{L(\R)}$; again, this may be infinitely generated.
We find the existence of a finite $\V \subseteq L(\R)$ such that $g_\V = g_{L(\R)}$.
...
\begin{assumption}\label{assum:V-star-exists-infinite}
	$L : \R \to \reals^\Y_+$ is a minimizable loss function such that $\risk L_+ = g_\V$ is polyhedral.
	$\V \subseteq L(\R)$ is a finite set such that $g_\V = g_{L(\R)}$.
	Finally, $\V^* \subseteq \V \subseteq L(\R)$ is the (unique) finite irredundant set such that $g_{L(\R)} = g_\V = g_{\V^*}$ and $\hyp(g_{L(\R)}) = \hyp(g_\V) = \hyp(g_{\V^*}) = \cap(\H_{\V^*} \cup \H_\Y)$ is irredundant.
\end{assumption}
...
This is often established by Assumption~\ref{assum:V-star-exists-infinite}.
...
\begin{claim}\label{claim:pi-preserves-dim}
	...
	For all $v \in L(\R)$, define $F_v$ as the face of $g_\V$ generated by $v$.
	For all $v\in L(\R)$, $\dim(F_v) = \dim(\pi(F_v))$.
\end{claim}
\begin{proof}
...
Now we can observe a set of normals $\V'$ generates faces whose projections cover $\reals^\Y_+$ if and only if the set contains $\V^*$.
...
%	% ...$\leq \inprod{v}{x}$ for all $v \in \V$, so $(x,g(x)) \in \cap \H_\V \implies (x,g(x)) \in G = \cap (\H_\Y \cup \H_\V)$.
	...
	As $g_\V$ is only defined on $\reals^\Y_+$, equality follows.
...
Taking $L,\V,\V^*$ as in Assumption~\ref{assum:V-star-exists-infinite}, when taking $F_{v^*} = H_{v^*} \cap \hyp(g_\V)$ for $v^* \in \V^*$, the projection $\pi$ preserves full-dimensionality.
...
%Given $L, \V, \V^*$ satisfying Assumption~\ref{assum:V-star-exists-infinite}, denote $\Lambda := \{\pi(F_v) \mid v \in \V^*\}$\jessie{To reduce notation, let's get rid of $\Lambda$?} as the set of projected facets generated by $\V^*$.
...
Define the function $\theta(v) = \{p \in \simplex \mid \inprod{v}{p} = f_\V(p)\}$ as the level sets of the loss vector $v \in \V$. %, and the set $\Theta_{\V^*} := \{\theta(v) \mid v \in \V^*\}$ to be the set of level sets uniquely defined by a given set of loss vectors.
...
Moreover, consider the follow set of level sets of $f_{\V^*}$, $\Theta = \{\theta(v) \mid v \in \V^*\}$.
