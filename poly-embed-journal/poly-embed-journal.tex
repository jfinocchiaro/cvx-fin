...
\begin{proof}
	...
	$\Sc$ is representative of $L$\raft{let's use ``representative for'' to be consistent; can always find/replace to change later \#burn}, so Lemma~\ref{lem:loss-restrict} gives us $\risk{L} = \risk{L|_{\Sc}}$, which is polyhedral as $\Sc$ is finite.
\end{proof}
