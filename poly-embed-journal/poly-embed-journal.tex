...
\raft{Added this paragraph since we needed it in \S~\ref{sec:poly-ie-consistency}}
...
\end{lemma}
\begin{proof}
  \raft{Clarified this proof in a bunch of places because of the refinement to indirect elicitation changes; worth a double-check}
  Let $\Gamma = \prop L$.
...
\begin{proof}
  ...
  By Lemma~\ref{lem:ie-iff-embeds-refinement}, $L$ tightly embeds a discrete loss $\ell^\emb:\R^\emb\to\reals^\Y_+$ such that $\ell^\emb$ indirectly elicits $\gamma$; let $\psi^\R: \R^\emb \to \R$ be the corresponding link function.
  Let $\gamma^\emb := \prop{\ell^\emb}$ be the property that $\ell^\emb$ directly elicits.
  Then for all $r\in\R^\emb$ and $p\in\simplex$ we have $r\in\gamma^\emb(p) \implies \psi^\R(r) \in \gamma(p)$. 
