...
\begin{figure}
  \raf{Let's make into a single figure with 3 subfigures}\\
...
We now present a concept called the $\trim$ of a loss (see Definition~\ref{def:trim-prop} for an analogous definition for properties in \S~\ref{app:embed-props}) which lets us focus on a unique set of loss vectors (resp., property values) induced by any minimum representative set.
...
\begin{restatable}{proposition}{embedtrim}\label{prop:embed-trim}
	...
	The following are equivalent:
	\begin{enumerate}\setlength{\itemsep}{0pt}
		\item $\Gamma$ embeds a \jessie{non-redundant} finite property $\gamma:\simplex \toto \R$.
	...
	Moreover, when any of the above hold, $\trim(\gamma) = \trim(\Gamma) = \{\Gamma_u : u\in\U\} = \Theta$, and $\gamma$ is elicitable.\jessiet{At the moment, don't show anything about $\gamma$ elicitable.}
\end{restatable}
\begin{proof}
	Let $L$ elicit $\Gamma$.
...
%	Then we can apply Lemma~\ref{lem:loss-restrict} to observe that $L$ embeds $L|_\U$, and therefore $\Gamma$ embeds $\prop{\ell}$, which is finite as $\ell$ is discrete.
	...
	2 $\Rightarrow$ 3: 
	Suppose $\trim(\Gamma) = \{\Gamma_u \mid u \in \U\}$.
	Then we know $\U$ is a minimum representative set, and since $\trim(\Gamma)$ is finite, so is $\U$; if it was not, then $\U$ would not be minimum.
	\jessie{Using the old definition of $\trim$, mirroring ish previous proofs: $\trim(\Gamma) = \{\Gamma_u \mid \neg \exists u' \neq u $s.t.$ \Gamma_u \subseteq \Gamma_{u'}\}$.
	Take $\U' = \{u_1, \ldots, u_k\}$ to be the finite set of reports forming $\trim$.
	Since $\cup \trim(\Gamma) = \cup_{u \in \U'} = \simplex$, we have $\U'$ representative.
	If $\U'$ is not minimum, there is some $\U \subseteq \U'$ that is minimum with $\card(\U) \leq \card(\U') = k$, which is finite.}
...
%	Moreover, since $\gamma$ is non-degenerate, we have $\cup \Theta = \cup_r \gamma_r = \simplex$.	
	\btw{Previous version commented out above because it relies on old definitions - Jessie 1 June 21}
	Since $\Gamma$ embeds a finite property $\gamma$, there is an embedding $\varphi$ and a finite minimum representative set $\Sc$ for $\gamma$ such that $\trim(\Gamma) = \{\Gamma_u \mid u \in \varphi(\Sc)\} = \{\gamma_r \mid r \in \Sc\}$.
	(Embedding implies $\Sc$ is a minimum representative set for $\gamma$ iff $\varphi(\Sc)$ is minimum representative for $\Gamma$.)
	Take $\Theta = \trim(\Gamma)$; since $\U$ is representative, we know $\cup \Theta = \simplex$.
	Moreover, $\Theta$ is finite since $\Sc$ is finite.
	...
	Since level sets of elicitable properties are closed\footnote{$\gamma_r = \Gamma_{\varphi(r)}$ for all $r \in \Sc$, we can use this characteristic about level sets of elicitable properties. \jessie{Adding this since we haven't shown $\gamma$ elicitable yet.}}, $\gamma_r$ must then be a face of some facet $\gamma_{r'}$.
...
\begin{definition}\label{def:trim-prop}
  Given an elicitable property $\Gamma:\simplex \toto\R$, we define $\trim(\Gamma) = \{\Gamma_u : u \in \U \}$ as the set of maximal level sets of $\Gamma$ for any minimum representative set $\U \subseteq \R$.
\end{definition}
