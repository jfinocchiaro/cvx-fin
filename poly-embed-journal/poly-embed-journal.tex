...
\begin{proof}
  ...
  From refinement, we can define a function $\psi^\R: \R^\emb \to \R$, where such that $\gamma^\emb_r \subseteq \gamma_{\psi^\R(r)}$ for all $r\in\R^\emb$. 
  ...
  We next show $\{u\in\reals^d \mid \psi'(u) \not \in \gamma(p) \} \subseteq \{u\in\reals^d \mid \psi^\emb(u) \not \in \gamma^\emb(p) \}$; we show the contrapositive, e.g., $\psi^\emb(u) \in \gamma^\emb(p) \implies \psi'(u) \in \gamma(p)$.
	...
	\begin{align*}
	\inf_{u\in\reals^d}\inprod{p}{L(u)} &< \inf_{u\in\reals^d : \psi^\emb(u) \not \in \gamma^\emb(p)} \inprod{p}{L(u)}\\
	&\leq \inf_{u\in\reals^d : \psi'(u) \not \in \gamma(p)} \inprod{p}{L(u)}~.~
	\end{align*}
	Consistency now follows as calibration and consistency are equivalent in this setting~\citep{bartlett2006convexity}.

  \hrule
  
  By Lemma~\ref{lem:ie-iff-embeds-refinement}, $\Gamma := \prop{L}$ embeds a finite elicitable property $\gamma^\emb:\simplex \toto \R'$ which refines $\gamma$.
...
\begin{lemma}\label{lem:projected-faces-iff-support-iff-argmin}
  ...
  $v \in \argmin_{v' \in \V} \inprod{v'}{x}$
\end{lemma}
\begin{proof}
  \jessie{Don't like this in align environment}
  \begin{align*}
    x \in \pi(F_v)
    &\iff \exists c \in \reals \text{ s.t. } (x,c) \in F_v\\
    &\iff \exists c \in \reals \text{ s.t. } (x,c) \in H_v \cap G\\
    & ...\\
    &\iff (x,g(x)) \in \{(x',c') \mid \inprod{v}{x'} = g(x) \} \\
    &\iff (x,g(x)) \in F_v \\
...
\end{claim}
\begin{proof}
  \raf{... by definition of $\risk{L}_+$.}
  
  Observe that $x \in \reals^\Y_+ \iff (x, c) \in \cap \H_\Y$.
Let $x \in \reals^\Y_+$.
  \begin{align*}
    (x,c) \in G
    &\iff g(x) \geq c & \text{ explain stuff here}\\
    &\iff \risk L_+(x) \geq c & \text{ as $g=\risk L_+$}\\
    &\iff \inprod{v}{x} \geq c \,\, \forall v \in \V \\
    &\iff (x,c) \in H^+_{v} \,\, \forall v \in L(\R) & \raf{let's stick with V here}\\
