...
\begin{proof}
  ...
  If and only if $\psi$ is $\epsilon$-separated, then for all $u$, $\psi(u) \in \cap_{p \in P_u} \gamma(p)$.
  Meanwhile by definition, $\Psi(u) = \cap_{p \in P_u} R_p$ where $R_p = \{r \in \R ~:~ \phi(r) \in \Gamma(p)\}$, the set of reports that embed into $\Gamma(p)$.
  By definition of embedding, $R_p = \gamma(p)$, completing the proof. \botodo{Ok?}
\raft{Looks great/close, but I got lost with the ``if and only if, then''}
\end{proof}
...
This impossibility can be shown, for example, if there exists a point $u$ where $\Psi(u)$ is empty for all $\epsilon > 0$.\botodo{Is this a reasonable possibility?}\raft{Yeah, sounds reasonable to me}
