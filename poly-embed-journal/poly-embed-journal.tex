...
\begin{proposition}\label{prop:tfae-min-rep-nonredundant}
	...
	\begin{enumerate}
		...
		\item[c.] For all $r \in \R$, $\Gamma_r$ is full-dimensional and there is no $r' \in \R'$ such that $\Gamma_{r'}= \Gamma_r$, and $\cup_{r\in\R} \Gamma_r = \simplex$.
	\end{enumerate}
\end{proposition}
\begin{proof}[Sketch]
	$(a \implies b)$  
...
\end{corollary}
\jessie{Add text about how this lets us think about general targets}
...
\begin{definition}[Trim]\label{def:trim-loss}
	Given a loss $L:\R \to \reals_+^\Y$, we define $\trim(L) = \{L(r) \mid r \in \R\}$ given any minimal representative set $\R$ for $L$.
\end{definition}

\begin{proposition}\label{prop:trim-loss-condition}
	Given an loss $L : \R \to \reals^\Y_+$, we have $\trim(L) = \{L(u) : u \in \R \textrm{ s.t. } \neg\exists u'\in\R,u'\neq u,\, \Gamma_u \subsetneq \Gamma_{u'}\}$.
\end{proposition}
\begin{proof}[Sketch]
...
\begin{proposition}\label{prop:embed-iff-trims-equal}
	Let $\ell$ eliciting $\gamma$ be a discrete loss and $\R$ a minimum representative set for $\ell$.
	$L$ (eliciting $\Gamma$) embeds $\ell$ if and only if $\trim(L) = \trim(\ell)$.
\end{proposition}
\begin{proof}[Sketch]
	$\implies$
	The optimality condition of embedding means $\Gamma_{\varphi(r)} = \gamma_r$ for all $r \in \R$.
	$\R$ a minimum representative set then implies $\varphi(\R)$ is a minimum representative set for $L$.
	Importantly, we have
	\begin{equation*}
	...
	\begin{align*}
	r \in \argmin_u \inprod{p}{\ell(u)} &\iff \ell(r) = \min_u \inprod{p}{\ell(u)}\\
	&\iff \ell(r) = \min_{u \in \R} \inprod{p}{\ell(u)} \\
