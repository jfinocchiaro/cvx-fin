...
\begin{definition}[Closed halfspace]
	For any $w\in\reals^\Y_+, b \in \reals$, let $H_{(w,b)}^+ := \{ x \in \reals^d \mid \inprod{x}{w} - b \geq 0\}$ be the closed halfspace defined by $(w,b)$, where $d = |\Y| +1$.
	Abusing notation, we sometimes write $H^+_v := H^+_{([v;-1], 0)}$ for a $v \in \reals^\Y_+$.\jessie{Is notation clear?}
\end{definition}
Additionally, denote $H_y^+ := H_{([e_y; -1], 0)}^+$. % \{ (x,c) \in \reals^\Y \times \reals \mid x_y \geq 0\}$.
\begin{definition}[Hyperplane]
	For any $(w,b)\in\reals^\Y_+ \times \reals$, let $H_{(w,b)} := \{ x \in \reals^d \mid \inprod{x}{w} - b = 0\}$ be the hyperplane generated by $(w,b)$, and the boundary of $H^+_{(w,b)}$.
	Again, we let $H_v := H_{([v;-1], 0)}$ for $v \in \reals^\Y_+$. 
\end{definition} 

\begin{definition}[Polyhedron - halfspace representation\jessie{Ziegler page 28}]
	An \emph{polyhedron} $P$ describes an intersection of a finite set of closed halfspaces $\H = \{H^+_{(w_i,b_i)}\}_{i=1}^k$ presented in the form $P = \cap \H$.
%	\begin{equation*}
%	P = P(A,z) = \{x \in \reals^d \mid Ax \leq z\} \qquad \textrm{ for some } A \in \reals^{m \times d}, z \in \reals^m~.~
%	\end{equation*}
\end{definition}
%\jessie{Comment the above out, and define as a finite intersection of closed halfspaces}
\begin{definition}[Face, facet]
	...
	A $(\dim(P) - 1)$-dimensional face is called a facet.
\end{definition}


\begin{theorem}\label{thm:polyhedron-uniquely-gen-facets}
	Given a $d$-dimensional polyhedron $P \subseteq \reals^d$, there is a unique finite set of halfspaces $\H = \{H^+_{(w_1, b_1)}, \ldots, H^+_{(w_k, b_k)}\}$ such that $P = \cap \H$, and for all sets of closed halfspaces $\H'$ such that $P = \cap \H'$, we have $\H \subseteq \H'$.
...
Set up notation and define $g,G$
\begin{itemize}\setlength{\itemsep}{0pt}
\item Let $f:\simplex\to\reals_+$ be a polyhedral concave function.
\begin{claim}
	We may write $f(p) = \min_{v\in\hat\V} \inprod{p}{v}$ for some finite set $\hat\V \subseteq \reals^\Y_+$ \raf{show you can do this using the $h$ and $\delta$ representation from Rockafellar -- argue that because $p\in\simplex$ we can write $\inprod{p}{b_i}-\beta_i = \inprod{p}{b_i-\beta_i\ones}$.}
\end{claim}
\begin{proof}
	Since $f$ is defined over the simplex, and therefore $\sum_i p_i = 1$, we can write any inner product $\inprod{p}{b} - \beta = \inprod{p}{b} - \inprod{p}{\beta \ones} = \inprod{p}{b - \beta \ones}$.
	Moreover, since $f$ is polyhedral, it is finitely generated \jessie{Rockafellar 19.1.2} and can be written 
	\begin{align*}
	f(p) &= h(p) - \delta(p \mid C)\\
		 &= \min(\inprod{p}{b_1} - \beta_1, \ldots, \inprod{p}{b_k} - \beta_k) - \delta(p \mid \simplex\jessie{?})\\
		 &= \min(\inprod{p}{b_1 - \beta_1 \ones}, \ldots, \inprod{p}{b_k - \beta_k \ones}) - \delta(p \mid \simplex)~.~
	\end{align*}
\end{proof}

\item We now extend $f$ from the simplex to the nonnegative reals.
\begin{definition}
	Define the function $g : \reals_+^\Y \to \reals_+$ by extending $-f$
	\begin{align*}
	g(x) = \min_{v\in\hat\V} \inprod{x}{v} - \delta(x \mid \reals_+^\Y)~,~
	\end{align*}
	taking $g(0) = 0$.
\end{definition}
%Extend to $g(x) = \min_{v\in\hat\V} \inprod{x}{v}$ for $x \in \reals^\Y_+$ and $g(x) = -\infty$ otherwise.  Def $g(0) = 0$.
...
\end{claim}
\begin{proof}
	$(x,c) \in G \iff g(x) - c \geq 0 \iff \min_{v \in \hat \V}\inprod{v}{x} - c \geq 0$ and $x \in \reals_+^\Y$, which is true if and only if $\inprod{v}{x} - c \geq 0 \,\forall v \in \hat \V$ and $x_y \geq 0$ for all $y$.
	Again, this is if and only if $(x,c) \in H^+_v$ for all $v \in \hat \V$ and in $H^+_y$ for all $y \in \Y$, so $(x,c) \in \cap \H$.
\end{proof}
\end{itemize}
...
\begin{claim}
	...
	There is some unique $\H' \subseteq \H$ such that $G = \cap \H'$. \jessie{$\H \subseteq \H$ since $G = \cap \H' = \cap \H$; something about smallest cardinality here?}
...
\item 
\begin{corollary}
	Then $\H' \setminus \H_\Y = \{H_v^+ : v\in\V\} =: \H_\V$ for some $\V \subseteq \hat \V$; That's the $\V$ we are looking for.  (And we've argued that it's uniquely determined by $f$.) \jessie{Why is $\V$ minimum cardinality?}
\end{corollary}

\item Define $F_v := H^+_v \cap G$ for any $v\in\reals^\Y_+$.
\item 
\begin{claim}
	$F_v$ is a facet of $G$ for each $v\in\V$.  \raf{Unpacking ``defines a facet''}
\end{claim}
\begin{proof}
This follows immediately since $\H_\V \subseteq \H'$ and every $H^+ \in \H'$ defines a facet by Theorem~\ref{thm:polyhedron-uniquely-gen-facets}.
...
Projections to the positive orthant
\begin{itemize}
...
\end{corollary}
\item Note: $\theta(v) = \pi(F_v) \cap \simplex$. \raf{But haven't def $\theta$ yet}
\end{itemize}

\subsection{Phase 2: Now given a minimizable loss $L:\R\to\reals^\Y_+$ with $\risk L = f$.}
\begin{itemize}\setlength{\itemsep}{0pt}
