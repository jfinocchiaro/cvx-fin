...
\begin{theorem} \label{thm:thickened-separated}
...
Because $u$ minimizes expected surrogate loss for both $p$ and $p'$, we must have $\psi(u) \in \gamma(p) \cap \gamma(p')$.
It is not \emph{a priori} clear that these sets even have a nonempty intersection.
We use elicitation results, discussed in \S~\ref{sec:min-rep-sets}, to show that for each $u \in \reals^d$ there exists $r \in \R$ such that $\Gamma_u \subseteq \gamma_r$, i.e. any $p$ satisfying $u \in \Gamma(p)$ also satisfies $r \in \gamma(p)$.
...
For $\epsilon > 0$, this directly implies calibration.
...
This claim (Lemma \ref{lemma:thick-empty}) follows because each $U$ is a closed convex set.
If all of their thickenings $U_{\epsilon}$ intersect, then so do the $U$ sets themselves, in which case indirect elicitation implies that there exists a legal choice.
Importantly, we show that a thickening of their intersection (using a larger $\epsilon$) contains the intersection of their thickenings.
It is also important that, by Lemma~\ref{lem:polyhedral-range-gamma}, there are only finitely many sets of the form $U = \Gamma(p)$.
...
For a given $\epsilon$ and norm $\|\cdot\|$, suppose one follows the routine of Construction~\ref{const:eps-thick-link} until the last step in which values for the link are selected.
...
\botodo{Commented out Proposition: converse, that every calibrated link from a polyhedral surrogate can be produced by our construction.}
...
\end{proof}
Lemma \ref{lemma:calibrated-pos} implies that there exists a $\psi$ such that $(L,\psi)$ indirectly elicits $\ell$: for every $u$ that is optimal for some $p$, let $\U' = \{U\in\U : u \in U\}$ be the optimal sets that contain it; choose $r$ from the nonempty set $\cap_{U \in\U'} R_U$; and set $\psi(u) = r$.

The main problem now is to prove a ``thickened'' analogue of Lemma \ref{lemma:calibrated-pos}.
...
Theorem \ref{thm:thickened-separated}, that Construction \ref{const:eps-thick-link} is a separated link function for small enough $\epsilon$, follows immediately from well-definedness (Theorem \ref{thm:small-eps-thick}) and the definitions.
