...
\raft{Changed; previous statement was not correct I think: ``Roughly, a link is $\epsilon$-separated if the distance between any point in $\reals^d$ that links to an $\ell$-optimal report, and any point that links to a non-optimal report, is at least $\epsilon$.''}
...
\begin{minipage}{0.72\linewidth}
    ...
    To exhibit non-calibration, take distributions $p, p'$ in the relative interior of the blue cell $\gamma_{\hat r}$ (here $\hat r=\bot$ for $\ellabstain$) but in different cells of $\gamma$.
...
\raft{Good call -- that was the whole point after all!  I updated to include the embedded property.}
...
\begin{corollary}\label{cor:unique-set-loss-vectors-defining-facets}
  ...
  (ii) $F_v := H_v \cap \hyp(g_{\V})$ is a facet of $\hyp(g_\V)$ for each $v\in\V^*$.
  (iii) $\V^* \subseteq \V$.
...
%
  (i) Since $\hyp(g_\V)$ is full-dimensional, the facets of $\hyp(g_\V)$ are uniquely determined by the hyperplanes $H$ such that $H = \partial H^+$ and $H^+ \in \H^*$ by Lemma~\ref{lem:G-unique-facets-Hstar}.
  ...
  (ii) Moreover, $H_v \in \H_{\V^*} \subseteq \H^*$ generates the facet $F_v = H_v \cap \hyp(g_{\V})$ of $\hyp(g_\V)$ by Lemma~\ref{lem:G-unique-facets-Hstar}.
%
...
\begin{proof}
  ...
  \raft{FYI, some of the steps were out of order I think}
