...
\raf{We then go on to prove surrogate regret bounds...}
...
\begin{corollary}
  A link $\psi$ is calibrated for a given discrete target $\ell$ and polyhedral surrogate $L$ if and only if there exists $\epsilon$ such that $\psi$ is produced by Construction \ref{const:eps-thick-link}$(L,\ell,\epsilon,\|\cdot\|_{\infty})$.
\end{corollary}
