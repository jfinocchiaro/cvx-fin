...
\jessie{ TODO Items 04 Mar 2021
	\begin{itemize}
		...
		\item Flesh out and expand on Ordered Partition Example - DONE
...
\begin{proof}
  ...
  \jessiet{Proof intuition in comment}
...
\section{Polyhedral losses: indirect elicitation implies consistency}\label{sec:embed-props}
...
\btw{JESSIE: This needs more narrative text.}
...
\end{theorem}
\begin{proof}
	\jessiet{17 Mar 21: REVISIT THIS... only one direction of the proof now.}
...
%	\jessiet{Do we need to show the preimage of $\Gamma$ is $V$?}
	...
	This suffices to prove both directions of the equality since there is a subtle use of the statement ``$\gamma'_{r'} = \prop{L}_{\varphi(r')}$ follows as a corollary of Proposition~\ref{prop:embed-trim}, and each direction of the statement follows from a subset inclusion of one of these two level sets. 
...
\begin{theorem}\label{thm:poly-ie-implies-consistent}
	If a polyhedral loss $L:\reals^d \to \reals^\Y$ indirectly elicits a property $\gamma': \simplex \toto \R'$, then for any loss $\ell'$ eliciting $\gamma'$, there exists a link $\hat \psi$ such that $(L, \hat \psi)$ is calibrated, and therefore consistent, with respect to $\ell'$.
\end{theorem}
\begin{proof}
	By Theorem~\ref{thm:ie-iff-embeds-refinement}, we know that $\Gamma := \prop{L}$ embeds a finite elicitable property $\gamma:\simplex \toto \R$ which refines $\gamma'$.
	Therefore, these exists a calibrated link $\psi : \reals^d \to \R$ from $\Gamma$ to $\gamma$ by Theorem~\ref{thm:eps-thick-calibrated}. \jessiet{Forward ref}
	...
	Consider $\hat \psi := \psi \circ \psi'$.
	
	Now, we show that $(L, \hat \psi)$ is calibrated with respect to $\ell'$.
	Consider that since $\Gamma$ embeds $\gamma$, we have a link $\psi$ such that $(L,\psi)$ is calibrated with respect to $\ell$ eliciting $\gamma$ (Theorem~\ref{thm:eps-thick-calibrated}) \jessiet{Forward ref}.
	We have
	\begin{align*}
	\inf_{u}\inprod{p}{L(u)} &< \inf_{u : \psi(u) \not \in \gamma(p)} \inprod{p}{L(u)}\\
	&\leq \inf_{u : \psi'(u) \not \in \gamma'(p)} \inprod{p}{L(u)}~.~
	\end{align*}
	The second statement is true if $\{u : \psi'(u) \not \in \gamma'(p) \} \subseteq \{u : \psi(u) \not \in \gamma(p) \}$; we show the converse for their compliments, e.g., $\psi(u) \in \gamma(p) \implies \psi'(u) \in \gamma'(p)$.
	Take $u \in \reals^d$ so that $\psi(u) \in \gamma(p)$.
	
	\begin{align*}
	\psi(u) \in \gamma(p) &\iff \psi' \circ \psi(u) \in \psi' \circ \gamma(p)\\
	&\implies \hat \psi(u) \in \gamma'(p)~,~
	\end{align*}
...
\btw{Summer 2020: New example using hierarchical surrogate}
