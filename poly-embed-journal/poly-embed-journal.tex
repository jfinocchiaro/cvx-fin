...
\begin{keywords}%
  Statistical consistency, surrogate loss functions, calibration, property elicitation, embeddings\jessiet{do we want embeddings?}
\end{keywords}
...
A surrogate and link pair $(L,\psi)$ is consistent with respect to $\ell$ if, for all data distributions $\D \in \Delta(\X \times \Y)$, and all sequences of surrogate hypotheses $h_1,h_2,\ldots$ whose $L$-loss limits to the optimal surrogate loss $L^*$ (in expectation over $D$), the $\ell$-loss of the sequence the linked hypotheses limits to the optimal target loss $\ell^*$.
...
\begin{definition}\label{def:consistency}
	...
	\begin{align*}
		\E_{(X,Y) \sim \D} L(h_m(X), Y) \to \inf_h  \E_{(X,Y) \sim \D} L(h(X), Y) &\implies \E_{(X,Y) \sim \D} \ell(\psi \circ h_m(X), Y) \to \inf_f  \E_{(X,Y) \sim \D} \ell(\psi \circ h(X), Y)~.
	\end{align*}
\end{definition}

When the hypothesis class $\H$ is not the set of all measurable functions and Definition~\ref{def:consistency} is updated accordingly, calibration and consistency are not equivalent~\citep{long2013consistency,kuznetov2014multiclass,zhang2020convex}.
See \S~\ref{sec:conclusion} for further discussion about extending our results to this setting.
\jessiet{Not sure if we want this paragraph... at least here?}
...
Finally, a similar construction also appears in the design of prediction markets \citep{abernethy2013efficient} and in connections between proper losses and mechanism design \citep{frongillo2014general}\jessiet{Does this citation need to be updated?}.
...
\subsection{Separation \jessie{Rename: separation and calibration}}
...
\subsection{Consistency \jessie{rename: constructing a calibrated link implies consistency}}
...
Let $\Gamma = \prop L$ and define $\U = \{\Gamma(p) \mid p\in\simplex\}$\jessiet{use $\Gamma(\simplex)$ earlier} to be the ``range'' of $\Gamma$, i.e., all possible optimal sets.
...
Together, these observations yield a single uniform smallest\jessiet{largest?} $\epsilon$ such that the key claim is true for all $u \in \reals^d$.
