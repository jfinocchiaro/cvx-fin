...
For example, the literature on adversarial prediction yields a special use case of this construction~\citep{asif2015adversarial,farnia2016minimax,fathony2016adversarial,fathony2018consistent}.
The equality of the constructions comes when one observes that $-\risk \ell$ is a maximum of affine functions, and the conjugate takes the infimum (which is always attained) so $C = (-\risk \ell)^*$ used in the proof of Theorem~\ref{thm:discrete-loss-poly-embeddable} lines up with their min-max objective.  
...
\jessie{Remove subsection and merge with above?}
