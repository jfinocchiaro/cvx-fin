...
\hypersetup{urlcolor=darkblue,
            anchorcolor=darkblue,citecolor=darkblue, hidelinks}
\usepackage{wrapfig}
...
\newcommand{\proposedadd}[1]{\mynote{orange}{#1}} % RF: I always read this as "propose dadd"
\newcommand{\reviewerone}[1]{\reviewerresponse{orange!20!white}{R1: #1}}
\newcommand{\reviewertwo}[1]{\reviewerresponse{blue!20!white}{R2: #1}}
\newcommand{\reviewerthree}[1]{\reviewerresponse{green!20!white}{R3: #1}}
...
\fi
\ifnum\CommentsR=1               % fix margins for todonotes
\setlength{\marginparwidth}{0.8in}
\fi
...
\usepackage{lastpage}
\jmlrheading{25}{2024}{1-\pageref{LastPage}}{6/22; Revised
1/24}{2/24}{22-0743}{Jessie Finocchiaro, Rafael M. Frongillo, Bo Waggoner}
\ShortHeadings{Embeddings for Consistent Polyhedral Surrogates}{Finocchiaro Frongillo Waggoner}
...
\begin{document}
\title{An Embedding Framework for the Design and Analysis of Consistent Polyhedral Surrogates}
\author{\name Jessie Finocchiaro\thanks{Work done while a student at University of Colorado Boulder} \email jefi8453@colorado.edu \\
...
Specifically, we say a convex surrogate $L$ embeds a discrete target loss $\ell$ if there is an injective function, which we call an \emph{embedding}, from the target reports (predictions) to $\reals^d$ such that (i)~the surrogate loss values match the target at the embedded reports, and (ii)~a target report is $\ell$-optimal if and only if its embedded report is $L$-optimal.
...
For example, while we know that the inconsistent top-$k$ polyhedral surrogates mentioned above are not consistent for top-$k$ classification, our framework illuminates the problems for which they \emph{are} consistent; it also yields restrictions on the underlying distribution which would render these surrogates top-$k$ consistent (\S~\ref{sec:top-k}).
...
% This terminology comes from the information elicitation \jessiet{information vs property... do we care}\\
\raft{we could also say economics literature; not ``property'' though since that doesn't offer an explanation of the word ``elicits''} literature~\citep{savage1971elicitation,osband1985information-eliciting,lambert2008eliciting}, in which a report $r$ is elicited from some forecaster by scoring her with a loss on the observed outcome $y$.
...
It is well-known in finite-outcome settings that calibration is equivalent to \emph{consistency}, as defined next (cf.~\citet{bartlett2006convexity,zhang2004statistical,agarwal2015consistent}).
...
When working with a restricted hypothesis class $\H$, as opposed to the set of all measurable functions in Definition~\ref{def:consistency}, then the corresponding notion of consistency (called \emph{$\H$-consistency}) is no longer equivalent to calibration~\citep{long2013consistency,zhang2020convex,kuznetsov2014multi};
...
Suppose $p = (0, 1) \in \simplex$ is the distribution putting all weight on $y=1$, and consider any sequence $u_m \to 1$ with $u_m < 1$ for all $m$.
Then the loss approaches the Bayes optimal, $L(1)_1 = 0$, but $\psi(u_n) = -1$ for all $n$, violating calibration.
\citet{agarwal2015consistent} were the first to formally connect property elicitation to calibration.
...
This approach captures several surrogates proposed in the literature (e.g.,~\citet{ramaswamy2015hierarchical,ramaswamy2016convex,lapin2015top,wang2020weston}; see \S~\ref{sec:applications}).
...
%		\label{fig:0-1-br}
	...
	\hfill
	\begin{minipage}{0.3\linewidth}
	\centering		\input{tikz/BRhinge.tex}
...
Combining Theorems~\ref{thm:poly-embeds-discrete} and~\ref{thm:discrete-loss-poly-embeddable}, we have Theorem~\ref{thm:embed-poly-main}.

\embedpolyinformal*
% \begin{proof}
%   Theorem~\ref{thm:discrete-loss-poly-embeddable} proves the first part of the statement, and Theorem~\ref{thm:poly-embeds-discrete} shows the second.
...
Finally, a similar construction also appears in the design of prediction markets \citep{abernethy2013efficient} and in connections between proper losses and mechanism design \citep{frongillo2014general,frongillo2021elicitation}.
...
The \emph{surrogate regret} $\Reg_L$ of $h$ and the \emph{target regret} $\Reg_{\ell}$ of the implied hypothesis $\psi \circ h$, are given by
\begin{align*}
  \Reg_L(h;\D) &= \E_{(X,Y)\sim\D} L(h(X))_Y - \inf_{h':\X\to\reals^d} \E_{(X,Y)\sim\D} L(h'(X))_Y~,
  \\
  \Reg_\ell(\psi\circ h;\D) &= \E_{(X,Y)\sim\D} \ell(\psi(h(X)))_Y - \inf_{g':\X\to\R} \E_{(X,Y)\sim\D} \ell(g'(X))_Y~,
\end{align*}
% $\Reg_L(h;\D) = \E_{(X,Y)\sim\D} L(h(X))_Y - \inf_{h':\X\to\reals^d} \E_{(X,Y)\sim\D} L(h'(X))_Y$,
...
% Given $h$, the \emph{target regret} of the implied hypothesis $\psi \circ h$ is $\Reg_\ell(\psi\circ h;\D) = \E_{(X,Y)\sim\D} \ell(\psi(h(X)))_Y - \inf_{h':\X\to\R} \E_{(X,Y)\sim\D} \ell(h'(X))_Y$.

Consistency (Definition~\ref{def:consistency}) means that if the surrogate regret of $h$ converges to zero, then the target regret of $\psi \circ h$ does as well; in other words, $\Reg_L(h;\D) \to 0$ implies $\Reg_{\ell}(\psi \circ h;\D) \to 0$.
...
A \emph{regret transfer bound}, also called a \emph{surrogate regret bound}, gives a guarantee on the relationship between the rates of convergence of $\Reg_L$ and $\Reg_{\ell}$.
For example, a surrogate with a fast rate of convegence $\Reg_L \to 0$ as $m \to \infty$ is not very useful if we nevertheless have a slow rate $\Reg_{\ell} \to 0$.
...
\begin{theorem}
  ...
  Then there exists $c > 0$ such that, for all measurable hypotheses $h$ and data distributions $\D$, we have $\Reg_{\ell}(\psi \circ h;\D) \leq c \cdot \Reg_L(h;\D)$.
\end{theorem}
...
As \citeauthor{ourlovaszpaper} observe, actually deteriving a calibrated link function in this case is nontrivial.
...
\citet{wang2020weston} use our embedding framework to show that the Weston--Watkins hinge embeds an \emph{ordered partition} loss $\ellOP$, as defined below.
...
\footnote{To recover the partition of~\citet{wang2020weston}, take $S_i = T_i \setminus T_{i-1}$.}
\begin{align*}
...
\begin{table*}[t]
	...
	\begin{tabular}{cc@{}c@{}c@{}c}
		& $\prop{\Li{2}}$ & $\prop{\Li{3}}$ & $\prop{\Li{4}}$ & $\prop{L^k}$\\[4pt]
		% \hline
...
%; we later expand on this relationship in Propositions~\ref{prop:embed-bayes-risks} and \ref{prop:embed-iff-trims-equal}.
%As a finite representative set implies a polyhedral Bayes risk by Lemma~\ref{lem:loss-restrict}, Lemma~\ref{lem:X} shows that polyhedral Bayes risks are equivalent to having finite representative sets, which in turn gives an embedding by Proposition~\ref{prop:representative-embeds-restriction}.
...
\begin{proof}
  ...
  But now $\Gamma_s \subsetneq \Gamma_r \subseteq \Gamma_{s'}$, contradicting $\Sc$ being a minimum representative set.
  Thus $\Sc \subseteq \R \setminus \red(\Gamma)$, which implies $\R' \subseteq \R \setminus \red(\Gamma)$.
...
Indeed, our framework has already been applied to submodular structured problems (\S~\ref{sec:lovasz-hinge}) and to max-margin losses~\citep{nowak2022consistency}.

Beyond applying our framework, we see several interesting directions for theoretical research.
...
It can be important for applications to understand the minimum prediction dimension $d$ of a consistent convex surrogate $L:\reals^d\to \reals^\Y_+$ for a given target problem, also called its convex elicitation complexity~\citep{frongillo2021elicitation}.
...
One reason this approach may be fruitful is that embeddings have much more structure than general convex losses, such as the fact that calibrated links arise automatically (Theorem~\ref{thm:thickened-separated}).
...
It is well-known that consistency is equivalent to calibration (Definition~\ref{def:calibrated}) for discrete target problems.
As calibration is a much easier condition to work with, and in particular only involves the conditional distributions, calibration is the main tool in the literature on consistency for discrete target problems.
...
In \S~\ref{sec:poly-ie-consistency}, we show that, when restricting to the class of polyhedral surrogates, indirect elicitation is actually \emph{equivalent} to calibration, and therefore consistency.
As indirect elicitation is an even simpler condition than calibration, an important line of future work is to identify other classes of surrogates for which this equivalence holds.


\paragraph{Extensions to $\H$-consistency}
\reviewertwo{A}
Throughout, we rely heavily on the fact that calibration is equivalent to consistency when the hypothesis class $\H$ in question is the set of all measurable functions.
...
In classification, calibration can fail to be sufficent for $\H$-consistency even in the realizable setting~\citep{long2013consistency,kuznetsov2014multi,awasthi2021calibration,awasthi2021finer}.
Realizability and similar assumptions often rule out the conditional label distributions causing inconsistency issues in \S~\ref{sec:applications}.
Under such assumptions, $\H$-consistency essentially reduces to an alignment of the surrogate hypothesis class and the target class $\H$, as mediated by the link function~\citep{zhang2020bayes}.
We expect that our link construction (\S~\ref{subsec:link-consistency}) and characterization (\S~\ref{sec:gener-thick-link}) could help extend these results beyond classification when using polyhedral surrogates.
Work has also begun to derive surrogate regret bounds for restricted classes $\H$, called \emph{$\H$-consistency bounds} \citep{awasthi2022hconsistency,awasthi2022multi}.
% additionally study surrogate regret bounds for $\H$-consistent surrogates for adversarial classification and multi-class classification, though it is unclear how such bounds generalize to general finite prediction tasks.
Here we expect that the linear surrogate regret bounds we derive (\S~\ref{subsec:regret-bounds})
could be applied.
...
and Dhamma Kimpara for observations that led to \S~\ref{app:ccdim-construction}.
We thank Drona Khurana, Nishant Mehta, Enrique Nueve, and Anish Thilagar for other suggestions.
\raft{others?}
...
\begin{theorem}[Aurenhammer~\citep{aurenhammer1987power}]\label{thm:aurenhammer}
...
the \emph{surrogate regret} $\Reg_L(u,p) = \inprod{p}{L(u)} - \risk{L}(p)$, and similarly the \emph{target regret} $\Reg_{\ell}(r,p) = \inprod{p}{\ell(r)} - \risk{\ell}(p)$.
...
\begin{lemma}\label{lem:hoffman-polyhedral}
  ...
  Then for any fixed $p$, there exists some smallest constant $H_{L,p} \geq 0$ such that $d_{\infty}(u,\Gamma(p)) \leq H_{L,p} \Reg_L(u,p)$ for all $u \in \reals^d$.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  Fix $p\in\simplex$.
  To show calibration, it suffices to find a positive lower bound for $\Reg_L(u,p)$ that holds for all $u\in\reals^d$ with $\psi(u) \notin \gamma(p)$.
  
  Applying the definition of $\epsilon$-separated and Lemma~\ref{lem:hoffman-polyhedral}, $\psi(u) \notin \gamma(p)$ implies
  \begin{align*}
    \epsilon &\leq    d_{\infty}(u,\Gamma(p)) \leq H_{L,p} \Reg_L(u,p) \implies 1 \leq \frac{H_{L,p}}{\epsilon} \Reg_L(u,p)~.
  \end{align*}
  Let $C_{\ell} = \max_{r,p} \Reg_{\ell}(r,p)$.
  Then $\Reg_{\ell}(\psi(u),p) \leq C_{\ell} \leq \frac{C_{\ell} H_{L,p}}{\epsilon} \Reg_L(u,p)$.

  If $H_{L,p} = 0$, then for all $u\in\reals^d$ we have $\Reg_\ell(\psi(u),p) = 0$, so calibration for this $p$ is trivial.
  ...
  Similarly, if $C_\ell = 0$, then $\Reg_\ell(r,p) = 0$ for all $r\in\R$, so again $\Reg_\ell(\psi(u),p) = 0$ for all $u\in\reals^d$.

  Now assume $C_\ell > 0$ and $H_{L,p} > 0$.
  Let $C'_{\ell,p} \doteq \min_{r \notin \gamma(p)} \Reg_\ell(r,p) > 0$.
  (As we assume $C_\ell > 0$, we must have $\gamma(p) \neq \R$, so the minimum is attained.)
  Then for all $u$ such that $\psi(u) \notin \gamma(p)$, we have $\Reg_\ell(\psi(u),p) \geq C'_{\ell,p}$.
  Rearranging, we have
  \[ \psi(u) \notin \gamma(p) \implies \Reg_L(u,p) \geq \frac{C'_{\ell,p} \epsilon}{C_\ell H_{L,p}} > 0~.\]
...
% The following is a direct result of Jensen's inequality, using the fact that $\Reg_L(h;\D) = \E_X \Reg_L(h(X),p_X)$ where $p_X$ is the conditional distribution on $Y$ given $X$.
...
\begin{lemma}\label{lem:linear-on-levelset}
  ...
  Then for any fixed $u,u^* \in \reals^d$ and $r \in \R$, the functions $\Reg_L(u,\cdot)$ and $\Reg_{\ell}(r,\cdot)$ are linear in their second arguments on $\Gamma_{u^*}$.
\end{lemma}
\begin{proof}
  Let $u^* \in \reals^d$ and $p \in \Gamma_{u^*}$.
  By definition, for all $p \in \Gamma_{u^*}$, $\risk{L}(p) = \inprod{p}{L(u^*)}$.
  So for fixed $u$,
    \[ \Reg_L(u,p) = \inprod{p}{L(u)} - \inprod{p}{L(u^*)} = \inprod{p}{L(u) - L(u^*)} , \]
  ...
  By the same argument, for fixed $r$, $\Reg_{\ell}(r,p) = \inprod{p}{\ell(r) - \ell(r^*)}$, a linear function of $p$ on $\gamma_{r^*}$ and thus on $\Gamma_{u^*}$.
\end{proof}
...
\begin{lemma}\label{lem:separated-constant-p}
  ...
  If $(L,\psi)$ indirectly elicit $\ell$ and $\psi$ is $\epsilon$-separated, then for all $u$ and $p$,
    \[ \Reg_{\ell}(\psi(u),p) \leq \frac{C_{\ell} H_{L,p}}{\epsilon} \Reg_L(u,p) . \]
\end{lemma}
\begin{proof}
  If $\psi(u) \in \gamma(p)$, then $\Reg_{\ell}(u,p) = 0$ and we are done.
  Otherwise, applying the definition of $\epsilon$-separated and Lemma~\ref{lem:hoffman-polyhedral},
  \begin{align*}
    \epsilon &<    d_{\infty}(u,\Gamma(p))  \\
             &\leq H_{L,p} \Reg_L(u,p) .
  \end{align*}
  So $\Reg_{\ell}(\psi(u),p) \leq C_{\ell} \leq \frac{C_{\ell} H_{L,p}}{\epsilon} \Reg_L(u,p)$.
\end{proof}
...
\begin{theorem}[Theorem~\ref{thm:linear-regret-bound}]
  ...
  If $(L,\psi)$ are consistent for $\ell$, then there exists constants $\epsilon_\psi, H_L > 0$ such that
    \[ (\forall h,\D) \quad \Reg_{\ell}(\psi \circ h ; \D) \leq \frac{C_{\ell} H_L}{\epsilon_{\psi}} \Reg_L(h ; \D) ~. \]
\end{theorem}
\begin{proof}
  Let $\Gamma = \prop{L}$.
  ...
  \[ \Reg_{\ell}(\psi(u),q) \leq \frac{C_{\ell} H_{L,q}}{\epsilon_{\psi}} \Reg_L(u,q) \leq \frac{C_{\ell} H_L}{\epsilon_{\psi}} \Reg_L(u,q)\]
  ...
  By Lemma~\ref{lem:linear-on-levelset}, $\Reg_L$ and $\Reg_{\ell}$ are linear in the second argument on $\Gamma_u$, so for any $u'\in\reals^d$,
  \begin{align*}
    \Reg_{\ell}(\psi(u'),p)
    &=    \sum_{q \in \mathcal{Q}_u} \beta(q) \Reg_{\ell}(\psi(u'), q)  \\
    &\leq \sum_{q \in \mathcal{Q}_u} \beta(q) \frac{C_{\ell} H_{L}}{\epsilon_{\psi}} \Reg_L(u', q) \\
    &\leq \frac{C_{\ell} H_L}{\epsilon_{\psi}} \sum_{q \in \mathcal{Q}_u} \beta(q) \Reg_L(u', q)  \\
    &= \frac{C_{\ell} H_L}{\epsilon_{\psi}} \Reg_L(u', p) .
  \end{align*}
...
Let us consider the tightest possible constant $c^*$ for which $\Reg_{\ell}(\psi \circ h;\D) \leq c^* \Reg_L(h;\D)$ for all $h$ and $\D$.
In general, for a fixed $p$, there is some smallest $c_p^*$ such that $\Reg_{\ell}(\psi(u),p) \leq c_p^* \Reg_L(u,p)$ for all $u$.
...
The rate at which $L$ grows is at least $H_{L,p}$, so the surrogate loss at $u$ may be as small as $\frac{\epsilon_{\psi}}{H_{L,p}}$, while the target regret may be as high as $C_{\ell} = \max_{r',p'} \Reg_{\ell}(r',p')$.
The ratio of regrets is therefore bounded by $\frac{H_{L,p} C_{\ell}}{\epsilon_{\psi}}$.

The tightest possible bound, on the other hand, is $c_p^* = \sup_{u: \psi(u) \not\in \gamma(p)} \frac{\Reg_{\ell}(\psi(u),p)}{\Reg_L(u,p)}$.
...
Meanwhile, $\Reg_{\ell}(r,p) \leq 1$, giving us an upper bound $c^*_p \leq \frac{(1)(1)}{1/2} = 2$.
...
\begin{theorem}[{\citet[Theorem 8]{ramaswamy2016convex}}]
...
Their construction is quite different from our Construction~\ref{const:eps-thick-link}, in that it operates in the loss vector space $\reals^\Y_+$ rather than the surrogate report space $\reals^d$.
...
\begin{proof}
	...
	If $x \in \reals^\Y_+$, then $g(cx) = \min_{v \in \V^*}\inprod{v}{cx} = c \min_{v \in \V^*}\inprod{v}{x} = c g(x)$ for any $c > 0$ by linearity of the inner product.
\end{proof}
