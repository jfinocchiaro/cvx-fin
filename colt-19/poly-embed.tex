...
\begin{abstract}
  We formalize and investigate the natural approach of designing convex surrogate loss functions via embeddings, for problems such as classification or ranking which involve a finite number of possible predictions.
  Specifically, we consider surrogate losses given by embedding each prediction (label, ranking, etc) as a point in $\reals^d$, and convexifying the loss.
  We prove that this approach is equivalent to working with polyhedral (piecewise linear convex) losses, in several strong senses.
  We then investigate the dimensionality of the surrogate hypothesis space required for such an embedding, giving a complete characterization of the $1$-d case, and several necessary conditions in higher dimensions.
  We illustrate our results with new lower bounds for abstain loss.
\end{abstract}
...
%
  \cite{agarwal2015consistent} and ~\cite{ramaswamy2016convex} study the existence of convex surrogates; the latter proves lower bounds on dimensionality of hypothesis spaces for e.g. \jessiet{take out of math mode} $0-1$ loss via their notion of feasible subspace dimension.
...
The expected loss of report $r$ when $y \jessie{Y} \sim p$ is written $\inprod{p}{L(r)}$.
...
\begin{definition}[Finite property, non-redundant]
  ...
  $\Gamma$ is \emph{finite} if it is non-redundant and $\R$ is \jessiet{delete: a} a finite.
...
This terminology comes from the information elicitation \jessiet{information vs property... do we care?} literature~\citep{savage1971elicitation,osband1985information-eliciting,lambert2008eliciting}, in which a report $r$ is elicited from some forecaster by scoring her with a loss on the observed outcome $y$.
...
\begin{definition}[Calibrated Link]\label{def:links}
  ...
  \emph{calibrated} if for all $p\in\simplex$, $r'\in \Gamma(p) \implies \psi(r') \in \gamma(p)$; in other words, if for all $r'\in\R'$, $\Gamma_{r'} \subseteq \gamma_{\psi(r')}$.
  It is \emph{separated}, with respect to a loss $L$ eliciting $\Gamma$, if for all $p \in \simplex$,
  \begin{align*}
...
Often one is given a property $\gamma$ (typically via a discrete loss $\ell$) and \jessiet{delete: one} one seeks a surrogate loss $L$ with certain attributes, such as convexity, smoothness, etc.
...
In particular, if both $\gamma$ and $\Gamma$ are single-valued, these works simply require $\gamma = f \circ \Gamma$. \jessiet{What is $f$?  Not defined...}
When $\Gamma$ is set-valued, one still straightforwardly requires $\gamma(p) = f(u)$ for all $u \in \Gamma(p)$.
The notion of a calibrated link from Definition~\ref{def:links} is only required when both $\gamma$ and $\Gamma$ could be set-valued, as is the case in general for convex surrogates of finite properties.
...
\jessiet{Replace ''However'' with some other filler?}
...
\begin{definition}\label{def:d-construction}
  ...
  \jessiet{Should we mention the known results that elicitable properties have convex level sets, and such a convex polytope exists (and in fact describes the level set) before the definition?}
\end{definition}
...
\jessiet{Is there any reason why $p \in \simplex$ would be implicit and not be required in $B$?  Asking more for the }
...
\jessiet{Does the original property refine of the restricted property?  If so, that might be worth mentioning.}
...
\jessiet{Throughout: find/replace 0.5 with 1/2.}
...
\jessiet{Would it be helpful to have drawings of the [3-outcome] simplex for abstain(1/2) and mode to see the difference?}
...
\begin{theorem}\label{thm:abstain-alphaghalf-not-2e}
  Abstain with $\alpha > 1/2$ and $|\Y|=6$ is not $2$-embeddable.
\end{theorem}
...
\begin{theorem}\label{thm:general-duality-embedding}
  ...
  \jessiet{\url{https://en.wikipedia.org/wiki/Closed_convex_function} so the simplex $\simplex$  being closed should suggest the Bayes Risk and $G$ are closed.}
\end{theorem}
\begin{proof}
...
\begin{proof}[Proof of Corollary~\ref{cor:d-embed-implies-d-construction}]
  ...
  Define the $y$th support function as $H_y(v) = \max_{w \in \partial L(u)_y} \langle w, v \rangle$ to obtain that $w \in \partial \langle p, L(u) \rangle$ if and only if, for all unit vectors $v$, \jessiet{Do the vectors have to be unit vectors?}
  ...
  \jessiet{... be the $i$th column of $B$, as given in Definition 24...}
  So, letting $B_i$ be the $i$th column of $B$, we have
    \[ \Gamma_r = \{p : \sum_y p_y B_y \geq 0\} = \{p : \sum_y p_y H_y(v) \geq 0 ~ (\forall v, \|v\|=1)\} . \]
  These sets are equal and full-dimensional as polytopes.
  By standard results (e.g.~\cite{grunbaum2013convex}), their H-representations are unique, so we must have $k$ vectors $v_1,\dots,v_k \in \reals^d$ such that $H_y(v_i) = B_{iy}$.
  ...
  \jessiet{For this proof, do we want to use a variable besides $w$ since it is used earlier in the inequality $\inprod{v}{w} \leq \sum p_y H_y(v)$?}
\end{proof}

\bo{A whole section of the following proof can be cut out, but it's trying to be a tutorial. Pairs well with the figure}
\begin{proof}[Proof of Theorem~\ref{thm:abs-5outcome-not-2embed}]
...
Because this equals $\{p : (\forall y) 2p_y \geq \sum_{y'} p_{y'}$\} \jessiet{$\sum_{y' \neq y} p_{y'}$?}, we can write the level set as $\{p : Bp \geq 0\}$ for
  \[ B = \begin{pmatrix}
       -1 &  1 &  1 & \cdots & 1  \\
       1  & -1 &  1 & \cdots & 1  \\
          &    &    & \vdots &    \\
...
\jessiet{Do we need a different symbol for the ones matrix since $\ones$ is typically used for the vector?  We could also write $\ones \ones^T$}
...
\jessiet{Equal or a  nonnegative scalar multiple of each other, in which case, we have a redundant constraint.}
\jessiet{Fix label for $T_5$ on image}
\begin{center}
...
\begin{proof}[Proof of Theorem~\ref{thm:abstain-alphaghalf-not-2e}]
  ...
  We know that $\beta := 1 - \frac{1}{1-\alpha} \in (0,1)$ as the inequalities describing $\alpha$ are strict.
  Observe that for all $i \in \{1,2, \ldots, 6\}, ||v_i|| > 0$. \jessiet{Give a reason.}
  ...
  \jessiet{Chance $o$ to $\circ$ for degrees}
  ...
  \jessiet{cut the first ``argument''}
  ...
  \jessiet{Add in tikz graphic for this image as well.}
\end{proof}
