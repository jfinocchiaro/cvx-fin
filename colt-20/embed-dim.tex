...
An embedding dimension significantly below $(n-1)$, such as $O(\log n)$ as with abstain loss, can lead to faster downstream optimization, as rates and computation times can both depend on the surrogate loss dimension.
...
\raf{Orphaned; add back:} necessary conditions for $d$-embeddability by satisfying optimality conditions, yielding new lower bounds for embedding dimension in Corollary~\ref{cor:d-embeddable-char}.
...
\begin{proof}
  ...
  Now consider that $\gamma_r = D(\T^r) = \{p \in \simplex : \vec 0 \in \oplus_p T_y \} = \{p \in \simplex : 0 \in \oplus_p \partial L(\varphi(r))_y \} = \{p : r \in \Gamma(p) \} =  \Gamma_{\varphi(r)}$.
  Therefore, as this holds for all $r \in \R$, and we have matching losses on the embedding points by monotonicity.
  \cite{frongillo2014general} shows that there is a finite set of reports suchthat there is a bijection between the level sets $\gamma_r$ and the subgradients of the Bayes Risk $\risk{\ell}$.
  Since the level sets match at the embedding points and we match losses, we have that the Bayes Risks of the two losses match.
  By~\cite[Proposition 1]{finocchiaro2019embedding}, we have $L$ embedding $\ell$.\jessiet{Hand waving, but not as much as before.} 
\end{proof}
...
Our program will consist of variables

are a set of normal vectors $\{v_i \in \reals^d : i \in [k]\}$ and a set of vertices $\{x^j_y \in \reals^d : j \in [\ell], y \in \Y\}$.
...
To accomplish our goal, we construct a quadratic feasibility program (QFP, Definition \ref{def:qfp}) that takes as input a polytope $C \subseteq \simplex$ and a parameter $d \in \mathbb{N}$ and (roughly) attempts to construct $\T$.
