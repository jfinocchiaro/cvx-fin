...
\citet{zhang2004convex,bartlett2006convexity} study when a convex, calibrated surrogate exists for a given discrete loss, and in a finite-outcome setting,~\citet{tewari2007consistency} show that calibration (Definition~\ref{def:calibration}) is necessary and sufficient for consistency.\jessiet{Do we want more of the canonical consistency papers?}
...
In the more specific case of convex calibrated surrogates for discrete losses, \citet{ramaswamy2016convex} propose the notion of \emph{convex calibration dimension}, which is the minimum dimension of a convex surrogate loss (not necessarily polyhedral) that is calibrated with respect to a given discrete loss.
...
\begin{definition}[(Finite) property, level set, elicited]\jessiet{Bo in FIXME: Change $\R$ since this is the only place $\R$ is not finite?}\jessiet{Changed $\R$ to $\R'$?}
...
\citet{tewari2007consistency} show that, in a finite-outcome setting, the presence of calibration and consistency are equivalent conditions.
...
Many surrogates in the literature provide upper bounds; we highlight in particular the \emph{abstain loss}~\citep{ramaswamy2018consistent} and seen in Equation~\eqref{eq:abstain}, in which one wants to predict the most likely outcome \emph{only if} confident in the outcome, and otherwise abstain.
...
\begin{proposition} \label{prop:orderable-iff-path}
  A finite property $\gamma$ is \emph{orderable} if and only if its intersection graph is a path, i.e. a connected graph\jessiet{tree?} where every vertex has either one or two neighbors.\jessiet{Bo in FIXME: ``Proposition 10: why is the incorrect 'definition' of path still there?'' I don't see why this is wrong?}
\end{proposition}
...
\begin{definition}\label{def:monotone-prop}
  ...
  \end{enumerate}
\end{definition}
\jessie{Can we explain in words the how orderable and monotone are different?}
\jessie{It is not immediately obvious that a property is orderable if and only if it is monotone.  Both definitions require an ordering on the reports, but it is not clear that the intersections of level sets on the relative interior of the simplex form hyperplanes from the definition of monotonicity.}
...
\jessiet{This seems like it should be a corollary of Theorem 12 now by observing that indirect elicitation implies calibrated surrogate.}
