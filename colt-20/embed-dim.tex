...
\begin{theorem}
  ...
  \item For all $y\in\Y$, the sequence $\sgn(v(i)_y)$ is monotone in
    $i\in\{1,\ldots,k-1\}$,
  \item For all $i\in\{2,\ldots,k-1\}$, letting
    \begin{align*}
    ...
    \end{align*}
    we have $\lambda^-(i) \geq \lambda^+(i)$. \jessie{Do we need to be more careful defining these when the sets are empty?}
  \end{enumerate}

  \raf{Still need to double check, but it will be something like this:}
  ...
  \begin{align*}
    ...
    L(u) &= \begin{cases}
      ...
      \ell(r_k) + (u - \varphi(r_k))\cdot K \ones & u \geq \varphi(r_k)
    \end{cases}~,
  \end{align*}
  where $\lambda(i) = \tfrac 1 2 (\lambda^-(i)+\lambda^+(i))$, $\Lambda_i := \prod_{j=2}^i \lambda(j)$, and $K=1+\max_{r\in\R,y\in\Y} |\ell(r)_y|$.
  \jessie{In the cases: what if $\varphi(r_1) < 0$?}\raf{I set $\varphi(r_1)=0$; updated to something hopefully clearer}
\end{theorem}
\begin{proof}
  Actually, the easiest proof given what we have is:
...
\begin{theorem} \label{thm:general-char}
  ...
  \begin{enumerate}
    ...
    \item (Monotonicity) There exists an injective embedding function $\varphi : \R \to \reals^d$ and loss functions $\{L_y : \reals^d \to \reals^\Y_{\geq 0}\}_{y \in \Y}$ such that for all $y \in \Y$, we have $T_y = \partial L_y(\varphi(r))$.
  \end{enumerate}
\end{theorem}
\begin{proof}
  \jessie{An attempt:}
  ...
  \jessiet{This is a hole, since the subgradients of the level sets match, but the values do not.  Need to bake matching losses into monotonicity condition}
\end{proof}
...
\subsection{Abstain, $\alpha \leq 1/2$, $d=1$}\label{subsec:abstain-d1}
...
The property $\gamma = \abstain{\alpha}$ can be verified:
...
However, it is unclear if this bound is tight, as the previously studied lower bounds of~\cite{ramaswamy2016convex} do not work well for this property, failing to give any lower bound tighter than the trivial $1$.
...
\bo{Am assuming that we will have to move almost everything to the appendix, wrote this blurb instead.}
...
\end{quotation}
The second proof is a direct geometrical one utilizing the same observations.
We show that packing too many normal vectors into a half-circle yields contradictions, implying that a $5$-outcome embedding into $2$ dimensions is impossible (and incidentally characterizing the structure of all possible $4$-dimensional embeddings).

\bo{Assuming everything after this will go to the appendix.}
...
\begin{proof}[Matrix rank proof]
  ...
  \jessie{Fill in explanation} \bo{attempted}
