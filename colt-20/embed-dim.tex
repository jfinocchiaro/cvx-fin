...
We have been omitting it because of the guarantee of Theorem \ref{thm:embed-iff-poly}, from \cite{finocchiaro2019embedding}, which guarantees its existence.
But in the case of $d=1$, we observe that one can simply use the midpoints between the embedding points as cutoffs for the link function, i.e. $\psi(u) = \argmin_{r \in \R} |u - \phi(r)|$, breaking ties arbitrarily.
...
\begin{theorem} \label{thm:general-char}
  ...
  \begin{enumerate}
    ...
    \item (Monotonicity) There exists an injective embedding function $\varphi : \R \to \reals^d$ and loss functions $\{L_y : \reals^d \to \reals^\Y_{\geq 0}\}_{y \in \Y}$ such that for all $r \in \R$ and $y \in \Y$, we have $T^r_y = \partial L_y(\varphi(r))$ and for all $r \in \R$, we have $L_y(\varphi(r)) = \ell_y(r)$.
  \end{enumerate}
\end{theorem}
\begin{proof}
  \jessie{An attempt:}
