...
\begin{theorem}
  ...
  \begin{align*}
    ...
    L(u) &= \begin{cases}
      ...
      \ell(r_k) + \Lambda_k \cdot (u - \varphi(r_k))\cdot K \ones & u \geq \varphi(r_k)
    \end{cases}~,
  \end{align*}
  \jessiet{In the cases: what if $\varphi(r_1) < 0$?}\raft{I set $\varphi(r_1)=0$; updated to something hopefully clearer}
  where $\lambda(i) = \min(\lambda^+(i),\max(\lambda^-(i),1))$, $\Lambda_i := \prod_{j=2}^i \lambda(j)$, $\Lambda_1=1$, and $K=$ \raf{TBD; was $1+\max_{r\in\R,y\in\Y} |\ell(r)_y|$}.
\end{theorem}
\begin{proof}
  ...
  \begin{align*}
    ...
    \\
    \inprod{w}{p} &= \frac 1 {1+c} \left(v(i+1)_y + \tfrac 1 2 (|v(i+1)_y| / v(i+1)_{y'} + |v(i)_y|/v(i)_{y'})v(i)_{y'}\right)
  ...
  Thus, $\Lambda_i > 0$ for all $i$, and it is then clear from our construction that $L(\varphi(r_i)) = \ell(r_i)$ for all $i$.
  It remains therefore to show convexity of $L$ and the optimality conditions.

  For convexity, note that $L$ is piecewise linear with nondifferentiable points at precisely the embedding points $\varphi(r_1),\ldots,\varphi(r_k)$.
  ...
  By construction, we have $\partial^-\ell(\varphi(r_1)) = -K\ones$ and $\partial^+\ell(\varphi(r_1)) = \Lambda_k K\ones$, and for $i\in\{1,\ldots,k-1\}$ we have $\partial^+\ell(\varphi(r_i)) = \partial^-\ell(\varphi(r_{i+1})) = \Lambda_i v(i+1)$.  
  \raf{Take $K > \max_{i\in\{1,\ldots,k-1\}} \Lambda_i\|v(i+1)\|_\infty$, show why it works for endpoints.}
  ...
  For optimality, consider a distribution $p\in\Delta(\Y)$, and the subgradient of $\inprod{p}{L(\varphi(r_i))}$ for some $i\in\{2,\ldots,k-1\}$.
  We have
  \begin{align*}
  ...
  \end{align*}
  \raf{MISSING: corner cases (easy?) and why we only need to look at two reports at once.}
