...
\begin{theorem}
  ...
  where $\lambda(i) = \min(\lambda^+(i),\max(\lambda^-(i),1))$\jessie{Is this backwards on the min/max?}, $\Lambda_i := \prod_{j=2}^i \lambda(j)$, $\Lambda_1=1$, and $K=$ \raf{TBD; was $1+\max_{r\in\R,y\in\Y} |\ell(r)_y|$}.
\end{theorem}
\begin{proof}
