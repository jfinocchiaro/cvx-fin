...
A contribution of this work is to provide the first nontrivial lower bounds for this loss.
...
\begin{theorem}
  ...
  \item For all $y\in\Y$, the sequence $\sgn(v(i)_y)$ is monotone in
    $i\in\{1,\ldots,k-1\}$,
  \item For all $i\in\{2,\ldots,k-1\}$\jessie{why 2 to k-1?}, letting
    \begin{align*}
...
\end{theorem}
\begin{proof}
  Actually, the easiest proof given what we have is:\jessie{change this wording?}
  ...
  By construction, we have $\partial^-\ell(\varphi(r_1)) = -K\ones$ and $\partial^+\ell(\varphi(r_1\jessie{r_k?})) = \Lambda_k K\ones$, and for $i\in\{1,\ldots,k-1\}$ we have $\partial^+\ell(\varphi(r_i)) = \partial^-\ell(\varphi(r_{i+1})) = \Lambda_i v(i+1)$.  
