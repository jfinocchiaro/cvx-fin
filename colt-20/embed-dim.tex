...
\newcommand{\discuss}[1]{\mytodo{cyan!20!white}{#1}}
...
Calibrated surrogate losses yields desirable statistical guarantees such as consistency and excess risk bounds~\citep{tewari2007consistency}.
...
The question therefore becomes: Given a discrete loss $\ell$, how do we design calibrated surrogates $L$? 
...
\citet{finocchiaro2019embedding} show that polyhedral surrogates are in a strong sense equivalent to an \emph{embedding}: % \emph{embedding} the discrete loss $\ell$ into $\reals^d$ by 
a mapping of each discrete prediction $r$ to some point $\varphi(r) \in \reals^d$, which optimizes $L$-loss if and only if $r$ optimizes $\ell$-loss.
...
\begin{theorem}[\citet{finocchiaro2019embedding}] \label{thm:embed-iff-poly}
...
\begin{definition}[Embedding dimension]
  We say a discrete loss $\ell: \R \to \nonnegreals^{\Y}$ is \emph{$d$-embeddable} if there exists a polyhedral surrogate $L: \reals^d \to \nonnegreals^{\Y}$ that embeds it.
...
We now give an equivalent condition to orderability which may be more intuitive: the adjacency graph of the level sets of $\gamma$, formed by connecting reports if their level sets intersect, must be a path.
...
\begin{definition}[Intersection graph]\label{def:intersection-graph}
  Given a discrete loss $\ell$ and associated finite property $\gamma: \simplex \to 2^{\R}$ elicited by $\ell$, the \emph{intersection graph} has vertices $\R$ with an edge $(r,r')$ if $\gamma_r \cap \gamma_{r'} \cap \relint(\simplex) \neq \emptyset$, where $\relint(\simplex)$ is the relative interior of $\simplex$.
  %(distributions with full support).
\end{definition}

If one can visualize level sets of a property, constructing the intersection graph yields an intuitive way to conceptualize orderability by Proposition~\ref{prop:orderable-iff-path}, proven in Appendix~\ref{app:1-dim}.


\begin{restatable}{proposition}{orderableiffpath} \label{prop:orderable-iff-path}
  A finite property $\gamma$ is \emph{orderable} iff its intersection graph is a path, i.e. a connected graph where two nodes have degree $1$ and every other node has degree $2$.% neighbors, and two nodes have degree one.
\end{restatable}
...
We give an example of a direct application in \S~\ref{sec:examples}.
By visualizing the level sets of $\gamma$ as a power diagram (generalization of Voronoi diagram) in the simplex like~\citet{lambert2009eliciting}, we can also use Proposition~\ref{prop:orderable-iff-path} to perform a visual test for orderability, and thus 1-embeddability (Figures~\ref{fig:intersection-graph-ex} and~\ref{fig:abstain-alpha-half}).
...
We can easily construct a link function in the case of $d=1$, by taking the midpoints between the embedding points as cutoffs: $\psi(u) = \argmin_{r \in \R} |u - \varphi(r)|$, breaking ties arbitrarily.

\subsection{Including general convex surrogates}
\label{sec:1d-proof-discuss}

We summarize the above results in the following theorem, together with one additional result, whose proof is in Appendix~\ref{app:1-dim}: if $\ell$ has any calibrated convex surrogate at all, it must have a polyhedral one.
(See Conjecture~\ref{conj:embed-equals-ccdim} regarding higher dimensional generalizations.)
...
\end{theorem}
\jessiet{If I understand correctly, different orderings of the reports may yield different embeddings, and these different embeddings should elicit different properties.  How to we reconcile this in the theorem statement?  Or where am I wrong?}
\raft{I think you are right, but I don't see a contradiction (do you?)}
\jessiet{I just wanted to make sure that it was clear that we get different surrogates with different orderings, although those would elicit different properties, so I see why it's fine.}
...
In particular, $\varphi(r)$ must minimize $\inprod{p}{L(\cdot)}$ if and only if $r$ minimizes $\inprod{p}{\ell(\cdot)}$.
...
\btw{The ``essentially'' is because we need loss values to match.  It's not clear if that comes for free, i.e., if you embed the correct property, can you always modify the loss to match $\ell$?}
...
\begin{definition}[$\T$, $D(\T)$]\label{def:D-T}
...
\begin{condition}[Vertex condition]\label{cond:V-condition}
	A collection of polytopes $\T$ and a polytope $C\subseteq\simplex$ defined by $C = \conv(\{p^1, \ldots, p^l\})$ \emph{satisfy the vertex condition} if for all $j \in [l]$, $0 \in \oplus_{p^j} \T$. %% Equivalent: there exist $\{x_{jy} \in T_y : j \in [\ell]\}$ such that, for all $j \in [\ell]$, $\sum_{y \in \Y} p^j_y x_{jy} = \vec 0$.
\end{condition}

\begin{theorem} \label{thm:vertex-halfspace-opt}
  Let the polytopes $\T = \{T_y \subseteq \reals^d : y \in \Y\}$ and $C$ be given, with $C = \conv(\{p^1,\ldots,p^{l}\}) = \{p: Bp \geq \vec 0\}$ for $B \in \reals^{k \times n}$.
...
Our program will consist of variables for the normal vectors $\{v_i \in \reals^d : i \in [k]\}$ for the (relaxed) halfspace condition, as described above, and variables for vertices $\{x^j_y \in \reals^d : j \in [l], y \in \Y\}$ which witness $0\in\oplus_{p^j} \T$ for the vertex condition, where the vector $x^j_y$ is the $y^{th}$ column of $X^j$.
...
\begin{definition}[Quadratic Feasibility Program] \label{def:qfp} ~ \\
  \indent 
  \textbf{Given:} $d \in \mathbb{N}$, a polytope $C = \{p \in \simplex : Bp \geq \vec 0\} = \conv(\{p^1, \ldots, p^l\}) \subseteq \simplex$, where $B \in \reals^{k \times n}$ has a minimum number of rows.

  \textbf{Variables:} $V \in \reals^{k \times d}$ with rows $\{v_i\}$; $X^1,\ldots,X^{l} \in \reals^{d \times n}$, where $X^j$ has columns $\{x^j_y\}$.

  \textbf{Constraints:}

  \vspace{-3.2em}
    \begin{align}
      V X^j                     &\leq B    & \text{(pointwise, $\forall j \in [l]$)}  \label{eqn:qp-constr-1} \\
      \sum_{y=1}^n p^j_y x^j_y &= \vec 0  & \text{($\forall j \in [l]$)}    \label{eqn:qp-constr-2}
    \end{align}
\end{definition}
...
\begin{proof}
  ...
  ($\implies$)
  By the vertex condition, for each $j \in [l]$, there exist witnesses $\{x^j_y \in T_y : y \in \Y\}$ satisfying the second constraint of the quadratic program (Inequality \ref{eqn:qp-constr-2}).
  ...
  ($\impliedby$)
  We construct $T_y = \conv(\{x^1_y, \ldots, x^{l}_y\})$.
  ...
  Any point $x \in T_y$ is a convex combination of $x^1_y,\ldots,x^{l}_y$, so it satisfies $\inprod{v_i}{x} \leq B_{iy}$.
...
\acks{Jessie Finocchiaro was funded by NSF Graduate Research Fellowship under grant No.\ DGE 1650115.  This material is based upon work supported by the National Science Foundation under Grant No.\ 1657598.  We also thank Peter Bartlett, Nishant Mehta, and Hari Ramaswamy for helpful comments.}
...
\begin{proof}[Proof of Proposition~\ref{prop:orderable-iff-path}]
	...
	If three level sets intersected on $\relint(\simplex)$, then the level boundary for any two would not be a halfspace (this follows because the level sets form a power diagram, e.g.~\citep{lambert2009eliciting}).
	This yields a path for the intersection graph.
	
	$(\impliedby)$
	\bo{Done with my pass. Not very happy with it but good enough I hope.}\jessiet{Come back to this once we verify definition of intersection graph}
...
%  \jessiet{I don't see the resolution of our conversation re: $\inprod{p}{\ell(r_i)} \leq \inprod{p}{\ell(r_j)}$ for all $j \leq i$ and vice versa?}

  \noindent\textbf{Embedding $\implies$ Monotone:}

  We trivially satsify the conditions of Definition~\ref{def:monotone-prop} by taking $a(r_i) = \partial^-L(\varphi(r))$ and $b(r_i) = \partial^+L(\varphi(r))$.
  ...
  Let $\gamma:\simplex\to 2^\R \setminus \{\emptyset\}$ be finite and monotone.
...
Throughout the rest of this section, we let $\prop{L}$ be the unique property elicited by the loss $L$.
\jessiet{Not sure how y'all feel about reordering the following proof statements, but they're almost in reverse order.  It makes sense to have the punchline at the front, but the Lemmas are also forward referencing and I'm not sure how to feel about it.  (Not going into crazy detail on proofs of Lemmas because of statement below.)}
...
\end{proposition}
\begin{proof}
  Let $\gamma:\simplex\to 2^\R \setminus \{\emptyset\}$.
  From Lemma~\ref{lem:prop-L-monotone} below, $\Gamma := \prop{L}$ is monotone.
...
The proof of Proposition~\ref{prop:indirect-orderable} uses the following results.

\raft{The following statement is true I believe, but low priority: ``An elicitable property $\Gamma:\simplex\to 2^\reals \setminus \{\emptyset\}$ is convex elicitable (elicited by a convex $L : \reals \to \reals^\Y$) if and only if it is monotone.''  Start of the proof commented out.  Just need to show that $b$ is the upper limit of $a$ and $a$ the lower of $b$; should follow from elicitability of $\Gamma$.}
...
\begin{lemma}\label{lem:pbar}
  Let $\gamma:\simplex\to 2^\R \setminus \{\emptyset\}$ be a finite elicitable property, and suppose there is a calibrated link $\psi$ from an elicitable $\Gamma$ to $\gamma$.
...
\begin{theorem}[\citep{aurenhammer1987power}]\label{thm:aurenhammer}
...
\begin{theorem}[{\citep[Theorem 3.1.2]{weibel2007minkowski}}] \label{thm:unique-face-decomp}
...
\iffalse 
\begin{corollary}[{\citep[Corollary 3.1.3]{weibel2007minkowski}}]\label{cor:face-decomp-normal-cones}
...
\begin{corollary}[{\citep[Corollary 3.1.4]{weibel2007minkowski}}]
...
\begin{theorem}[{\citep[Theorem 3.1.6]{weibel2007minkowski}}]\label{thm:support-minksum}
...
\begin{proof}
	...
	\jessie{I feel like this should be way simpler: ex: $\oplus_p \T$ is a convex polytope, which is defined as the intersection of a finite number of halfspaces, each defined by a normal $w_i$.  As scaling the polytopes by a nonnegative scalar does not require changing the normal defining the halfspace, it is simply the support that gets weighted.}
\end{proof}
