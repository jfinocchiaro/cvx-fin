...
\begin{lemma} \label{lemma:unique-spot-xiji}
  For $d=2$, any pair of linearly independent $v_i,v_j$ rule out all except for a unique set of points $x^{ij}_i$ and $x^{ij}_j$ from being feasible.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  WLOG suppose $v_1$ points vertically ``up'', as in Figure \ref{bo:todo}.
  ...
  This implies $x^{13}_1$ is left of $x^{12}_1$ on the line.
  But the symmetric constraints $\inprod{v_3}{x^{13}_1} \leq 1$ and $\inprod{v_3}{x^{12}_1} \leq 1$ imply that $x^{12}_1$ is left of $x^{13}_1$ on the line.
...
\begin{proof}
  ...
  $v_1,v_2,v_3$ cannot lie in a cone of strictly less than $180^{\circ}$, as this would contradict \bo{the previous}.
  ...
  This contradicts \bo{the previous}.
\end{proof}
