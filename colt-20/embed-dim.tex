...
\begin{theorem}
  ...
  Moreover, when these conditions hold, the loss $L:\reals\to\reals^\Y$ embeds $\ell$ with $\varphi:\R\to\reals$, \jessie{Have been using $\phi$ instead of $\varphi$ for embeddings in this paper so far. I have a slight preference for $\varphi$ since it matches the last paper, but I don't care too much... just need to pick one.}where
  \begin{align*}
    \varphi(r_i) &= \begin{cases}
      0 & i=1 \\
      \sum_{j=2}^i 1/\Lambda_j & i\in\{2,\ldots,k\}
    \end{cases}~,
    \\
    L(u) &= \begin{cases}
      \ell(r_1) - u K \ones & u \leq 0 \\
      \ell(r_i) + \Lambda_i \cdot (u - \varphi(r_i)) \cdot  (\ell(r_{i+1})-\ell(r_i)) & u \in [\varphi(r_i),\varphi(r_{i+1})] \\
      \ell(r_k) + u K \ones & u \geq \varphi(r_k)
    \end{cases}~,
  \end{align*}
  where $\Lambda_i := \prod_{j=2}^i \lambda(j)$ and $K=1+\max_{r\in\R,y\in\Y} |\ell(r)_y|$.
  \jessie{In the cases: what if $\varphi(r_1) < 0$?}
\end{theorem}
