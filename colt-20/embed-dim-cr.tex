...
\cite{finocchiaro2019embedding} show that polyhedral surrogates are in a strong sense equivalent to an \emph{embedding}: % \emph{embedding} the discrete loss $\ell$ into $\reals^d$ by 
...
\end{equation}
Here $\R = \Y \cup \{\bot\}$.
For $\alpha \leq 1/2$, \cite{ramaswamy2018consistent} give an elegant embedding of this loss on $n$ outcomes into $d = \lceil \log_2 (n) \rceil$ dimensions, where each $y \in \Y$ is embedded at a corner of the Boolean hypercube $\{-1,1\}^d$ while $\bot$ is embedded at the origin.

In general (e.g. \cite{finocchiaro2019embedding}\jessiet{Change citation probably.  Possibly to \cite[Corollary 13]{ramaswamy2016convex}}), a known convex-conjugate construction generically embeds any discrete loss on $\Y = [n]$ into $d = n-1$ dimensions, giving a flat upper bound of $n-1$ on embedding dimension.
...
The following condition of~\citet[Theorem 3]{lambert2018elicitation}\jessiet{Should we cite or citet?}, that a finite property is \emph{orderable}, states that any two level sets intersect in a hyperplane, or not at all.
...
\jessiet{Promised to give reviewers more intuition for this $\Lambda$ notation}
...
\jessiet{Cut next two lines for space}
...
\jessiet{Promised more intuition on $\T$ vs $\T^r$.}
...
Note that $\T = \T^r$ for some $r \in \R$; here, $r$ is understood, so we omit it for clarity.
...
\jessiet{Don't really introduce the $x^j_y$ notation in relationship to $X^j$...}
...
\begin{proof}
    ...
    The support of the Minkowski sum for a given normal is the sum of the normals~\cite[Theorem 3.1.6]{weibel2007minkowski}, and so we we can take $x \in \oplus_p \T \iff \inprod{V^*}{x} \leq E^*p$.
