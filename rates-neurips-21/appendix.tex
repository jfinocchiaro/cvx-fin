...
\begin{proof}
  ...
  Lemma 5 states that if $L$ is polyhedral, then the negative of its Bayes risk, $G(p) = -\risk{L}(p)$, is polyhedral.
  (In other words, the Bayes risk is the concave equivalent of a polyhedral function.)
  Recall that by definition, $\Gamma(p) = \argmin_{u \in \reals^d} \inprod{p}{L(u)}$.
\end{proof}
