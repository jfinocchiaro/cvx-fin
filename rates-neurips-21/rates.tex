...
\begin{abstract}
  ...
  Surrogate regret bounds are a common tool to prove generalization or excess risk rates for surrogate risk minimization.
  ...
  \raft{need to add keyword and/or mention: ``excess risk bounds'' (more common in the literature actually) \bo{Did above}}
\end{abstract}
...
After presenting the main results (Sections \ref{sec:upper} and \ref{sec:lower}), we make it constructive in Section \ref{sec:constant}.
...
\footnote{These works express surrogate regret bounds with $\zeta^{-1}$ appearing on the left-hand side of eq.~\eqref{eq:surrogate-regret-bound-informal}, and thus the terms ``upper bound'' and ``lower bound'' are opposite from their terminology.}
...
Another example is Huber loss\bo{cite?}, which is strongly convex when the prediction is ``close'' to the observation in some sense and linear when ``far''.
For classification with $\Y = \{\pm 1\}$ and prediction space $\reals$, Huber loss can be defined by letting the hinge loss be $t = \max\{0, 1-ry\}$ and setting $L(r)_y = t^2$ if $t \leq 2$, else $4(t-1)$.
