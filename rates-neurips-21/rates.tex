...
In this section, we make the result constructive by unpacking the implications of consistency (or more precisely, calibration) for polyhedral surrogates.
...
\cite{finocchiaro2020} showed that in fact, if $L$ indirectly elicits $\ell$, then there always exists an $\epsilon$-separated link $\psi$ such that $(L,\psi)$ is consistent for $L$.
...
Given Lemma \ref{lemma:lem:calibrated-eps-sep}, we can make the following definition.
\begin{definition}[$\epsilon_{\psi}$]
  If $(L,\psi)$ are calibrated for $\ell$, then let $\epsilon_{\psi} = \sup \{ \epsilon ~:~ \text{$\psi$ is $\epsilon$-separated}\}$.
\end{definition}
...
\bo{We could have $C_{\ell} = 0$ but then everything is optimal and the regret transfer is zero.}
