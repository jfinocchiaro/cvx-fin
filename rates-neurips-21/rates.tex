...
\begin{enumerate}
  ...
  \item Apply this result to a finite subset $P$ of $\simplex$, namely the vertices of the level sets of $\Gamma$.
        ...
        This implies a linear conditional transfer rate with constant $\max_{p' \in P} \alpha_{p'}$.
\end{enumerate}
We view both parts as relatively surprising.
The first part will actually hold for any calibrated surrogate.
The second part relies on a crucial fact about polyhedral losses $L$: their properties $\Gamma$ have only a finite number of full-dimensional level sets.
