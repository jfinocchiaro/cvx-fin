...
Formally, for a surrogate $L$, the regret of $h$ with respect to data distribution $\D$ is given by $R_L(h;\D) = E_{(X,Y)\sim\D} L(h(X), Y) - \inf_{h':\X\to\reals^d} E_{(X,Y)\sim\D} L(h'(X),Y)$, where the minimum is taken over all measurable functions.
(One may equivalently assume the Bayes optimal hypothesis is in the function class.)
The target regret of $h$ is thus $R_\ell(\psi\circ h;\D) = E_{(X,Y)\sim\D} \ell(\psi(h(X)), Y) - \inf_{h':\X\to\R} E_{(X,Y)\sim\D} \ell(h'(X),Y)$, where we recall that $\psi$ is the link function.

A surrogate regret bound answers the following question: if we can minimize the true surrogate regret at a rate $f(n)$, given $n$ data points, how fast does the corresponding hypothesis $h_n$ minimize the target regret?
