...
It is clear that $\alpha^* = \max_{p \in \mathcal{Q}} \alpha_p^*$ for the finite set $\mathcal{Q}$ used in the proof, i.e. the vertices of the full-dimensional level sets of $\Gamma = \prop{L}$.
...
The rate at which $L$ grows is at least $H_{L,p}$, so the surrogate loss at $u$ may be as small as $\frac{\epsilon_{\psi}}{H_{L,p}}$, while the target loss may be as high as $C_{\ell}$.
...
For example, $u$ may only be as close to the optimal set as $\epsilon_{\psi}$ when it links to reports $\psi(u)$ with lower target regret; or $L$ may have a smaller slope in the direction where the link's separation is larger than $\epsilon$.

To illustrate with a concrete example, consider the REB surrogate of \cite{ramaswamy2018} for the abstain loss, $\ell(r,y) = \frac{1}{2}$ if $r = \bot$, otherwise $\ell(r,y) = \ones[r \neq y]$.
\bo{TODO finish}
...
\bo{Make this more positive: we will broadly impact the world to be happier.}
