...
It requires that $L$ be \emph{minimizable:} for all $p \in \simplex$, there exists $u$ such that $\inprod{p}{L(u)} = \inf_{u' \in \reals^d} \inprod{p}{L(u')}$.
Note that if $(L,\psi)$ indirectly elicits $\ell$ for any $\psi$ and $L$, then $L$ is minimizable.
