...
Some references by setting.  P = polyhedral.  Surrogate regret bound: L = linear, Q = quadratic (sqrt).  LB = gives a ``lower bound'' for the surrogate regret too.
\begin{itemize}
...
\item Structured prediction (covering lots of target problems simultaneously)
  \begin{itemize}
  \item[Q,LB] \citet[Theorem 7]{osokin2017structured} -- Prove excess risk bounds for a broad family of structured prediction problems, using a specific quadratic surrogate.  Also give an upper bound on the calibration function, i.e. a lower bound in the sense that we prove (Theorem 8).
  ...
  \item[Q,LB] \citet[Theorem 4.4]{nowak2019general} -- Even more general than the previous Bach paper \cite{osokin2017structured} (e.g. includes ordinal regression).  Quadratic risk bound for a proper loss for the mean from a strongly-convex potential/entropy.
...
\item Fast rates
  ...
  \item Tim et al paper than Anish referenced!
  \end{itemize}
...
While much is known about surrogate regret bounds for particular problems, such as binary classification and bipartite ranking, general results which span multiple target problems are sparse.
Lacking such results, authors instead often prove bounds for specific surrogate losses from scratch, e.g. \raf{abstain, others?}.
...
We prove a general surrogate regret bound for the broad class of \emph{polyhedral} surrogates, those that can be written as maxima over finitely many affine functions (i.e., piecewise-linear and convex).
