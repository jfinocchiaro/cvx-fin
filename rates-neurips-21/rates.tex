...
\footnote{Many of these polyhedral surrogates are actually known to be \emph{inconsistent} for their intended target problems.
...
Without placing further assumptions on the data, therefore, \citet{mahdavi2014binary} observe the same general phenomenon as we do: while smoothness may help surrogate generalization rates, it does so as much as it hurts the regret transfer.
...
This holds roughly because expected surrogate loss is strongly convex in some neighborhood of $u_0$ and is strongly smooth elsewhere.%
