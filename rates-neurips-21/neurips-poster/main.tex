\documentclass[t,mathserif,12pt,aspectratio=1610,handout]{beamer}
...
\newcommand{\enumitem}[1]{{\usebeamercolor[bg]{item projected}%
  \raisebox{1.5pt}{\colorbox{bg}{\color{fg}#1}}}}%
...
\begin{frame}{The short version}{}
  ...
  \pause
  \begin{itemize}[<+->]
    \item Goal: optimize \redem{discrete ``target''} loss $\ell(r,y)$  \hintright{classification, structured prediction}  \\
          \hint{$r = $ prediction, $y = $ label}
...
%          \hint{$u = $ prediction in $\reals^d$}
  ...
  \vskip1em
  \textbf{Q:} When does (quick) \blueem{surrogate} convergence imply (quick) \redem{target} convergence?  \\
...
\begin{frame}{~}{}
  \vskip-1em
  \centerline{\includegraphics[width=0.65\linewidth]{figs/hinge}}
  \vskip1em

  \enumitem{\bf Theorem 1:} All polyhedral surrogates have \textbf{linear} regret transfer rates!

  \smallskip
  \textbf{Polyhedral:} \blueem{surrogate regret} $\leq \epsilon$ $\implies$ \redem{target regret} $\leq O(\epsilon)$.
  

  \vskip1em

  \enumitem{\bf Theorem 2:} Sufficiently ``non-polyhedral'' transfers are \textbf{quadratically slower}.

  \smallskip
  \textbf{Non-polyhedral:} Can have \blueem{surrogate regret} $\leq \epsilon$ yet \redem{target regret} $\geq \Omega(\sqrt{\epsilon})$.
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Background: surrogate risk, polyhedral losses}{}
  ...
  \hint{Classification ($0-1$ loss), ranking, top-$k$}
  ...
  \hint{Hinge loss for classification, BEP surrogate (Ramaswamy et al. 2018)}

  \vskip1em
  \blueem{Polyhedral surrogate} $L: \reals^d \times \Y \to \reals_{\geq 0}$: \\
  ...
  \begin{itemize}[<+->]
    \item Finocchiaro, Frongillo, Waggoner 2019, 2020; applications e.g. Wang, Scott 2020.
%    \begin{itemize}
...
\begin{frame}{~}{}
  ...
  \begin{itemize}
    \item Optimizable.  \hintright{Convex, strongly convex, etc.}
    \item Generalization bound, convergence rate.
%  \textbf{2.} Generalization bound; convergence rate. \\
%  \hint{e.g. with $n$ data points, $\Reg_L(h;\D) \leq O(\sqrt{1/n})$.}
    \item \textbf{Connection to \redem{target}...}
  \end{itemize}

  \vskip1em
  \textbf{Regret transfer function} (\emph{calibration function}) $\zeta: \reals_{\geq 0} \to \reals_{\geq 0}$:  \hintright{continuous at zero, $\zeta(0) = 0$}
...
\begin{frame}{Results}{}
  ...
  \begin{itemize}[<+->]
    ...
    \item Polyhedral losses have a \textbf{finite structure}: \hintright{related to embeddings framework}
    \begin{itemize}
...
\begin{frame}{~}{}
  ...
  \begin{itemize}
    ...
    \item \redem{Target regret} shrinks linearly as $\lambda \to 0$, but \blueem{surrogate regret} shrinks with $\sqrt{\lambda}$.
  \end{itemize}

  \pause
  Examples: exponential loss, Huber loss \hintright{via strengthenings}
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Investigating the constant in Theorem 1}{}
