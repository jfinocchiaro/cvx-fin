...
\jessie{R2 asked for more intuition on this definition... not sure what more to add}


\proposedadd{We focus on polyhedral losses in this context because, as Ramaswamy et al.~\cite[Section 1.2]{ramaswamy2018consistent} point out, smooth surrogates that estimate the conditional probability generally do much more than necessary, or practical in many cases.  In minimizing a polyhedral loss, they make the argument that one only learns what is necessary, in accordance with Vapnik's dictum~\cite[Section 1.9]{vapnik1999nature}.}
...
\proposedadd{In general, we are not overly concerned with the computational complexity of computing the surrogate and link, since these calculations only need to be computer once.  They are generally derived mathematically, rather than computationally.  These symbolically calculations typically tend to be straightforward, but can sometimes be tricky.}
