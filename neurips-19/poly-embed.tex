...
\bo{The following is not ideal because everywhere else in the paper, we always use $\R$ with $\gamma$ and $\reals^d$ with $\Gamma$}
...
The idea will be to map points in $U$, as well as nearby  to something in $R_U$
...
\begin{definition} \label{def:eps-thick-link}
  Given a polyhedral $L$ embedding some $\ell$, an $\epsilon > 0$, and a norm $\|\cdot\|$, the \emph{$\epsilon$-thickened link} $\psi$ is constructed as follows.
...
We write the surrogate $L_2(u,y) := \max\left( \frac{1}{k} \sum_{i=1}^k (u + \indopp(y))_{[i]} - u_y \right)_+ $, where $\indopp(y) = \ones - e_y$, and $(x)_{[i]}$ is the $i^{th}$ largest component of $x$.

When embedding, we consider the discrete loss
\begin{align*}
...
While of course several surrogates are known for this problem~\cite{pires2013cost}\raf{Jessie, could you cite \url{http://proceedings.mlr.press/v28/avilapires13.pdf} here}, they typically work in $n$ dimensions.
