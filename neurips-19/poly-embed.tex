...
In some cases we simplify existing proofs, while in others our results we give new results, such as the inconsistency of the recently proposed Lov\'asz hinge.
...
\subsection{Inconsistency of top-$k$ loss}
...
\raf{NEW: no consistency results known previously}

\raf{Will not even define the hinge, just mention informal, and give the expression explicitly for the increasing k=2 case: max f(1)... whatever. (That way it can fit in a few paragraphs, and justifies the delegation of the full result to the full version.)}
