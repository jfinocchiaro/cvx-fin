...
We omit the details connecting this definition to consistency, and instead refer the reader to Ramaswamy et al.~\cite{ramaswamy2018consistent}. 
(See the proof of Theorems 8.1-8.3.).
...
\begin{proof}
  ...
  We want to show that for all $p \in \simplex$ that $\inf_{u\in\reals^d} L(u,p) = \min_{r \in \R} \ell(r, p)$.
...
\begin{proof}
  ...
  (Note that this $u$ will be unique regardless of the choice of $p\in \inter f$, since $\risk{L}$ is differentiable here.)
  ...
  Thus, $L$ embeds the discrete $\ell$ as constructed.
...
\begin{proof}
  ...
  \jessie{Can also show that $L$ embeds $L|_\R$ like the original paper and use a modification of COLT Lemma 11.}
\end{proof}
...
\begin{proposition}
  If $\psi$ is a separated link between losses $L$ and $\ell$, then \raf{how do we want to talk about consistency?}
\end{proposition}
\begin{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Proving (in)consistency via embeddings}
\raf{Notes about the story, partially to myself, partially to Jessie: I think we want to emphasize the proofs of consistency and inconsistency (as the section title reflects), but we also want to illustrate our constructive approach to finding a link.  Basically, most of these examples (except abstain) have the link built in (usually as the sign function, though slightly different for top-k), and they just work around it.  For abstain, we can give alternate link functions, and hopefully show that the L1-based link has a better calibration function.  For the others, it may suggest that it's worth thinking ``outside the box'' (HAH, genious...) for embeddings which are not the hypercube.}

The above techniques allow us to evaluate and simplify some previous results in the literature.
\subsection{Top-$k$ loss}
Yang and Koyejo~\cite{yang2018consistency} prove the inconsistency of a family of surrogates for the top-$k$ classification problem.

The top $k$ classification problem takes as input a vector $u \in \reals^n$, and observes outcome $y \in [n]$.
The input is sorted in descending order, and the function $t_k(u)$ yields a set of outcomes associated to the first $k$ elements of this sorted input.
For example, if $u = (4,1,2)$, then $t_k(u) = \{0,2\}$ for the first and third outcomes.

We say the top-$k$ loss is
\begin{align}
...
In particular, Yang and Koyejo evaluate two losses given by Lapin et al.~\cite{lapin2016loss, lapin2018analsis}, one of which is shown to be convex, but its consistency is left as an open question.

After our discussion of embeddings, we now have a way of proving consistency: a surrogate $L$ is consistent with respect to a discrete loss $\ell$ \jessie{we still haven't defined consistency... here I'm going by Yang's consistency.  I suppose we still need a proof of this.} if and only if $L$ embeds $\ell$.
...
\subsection{Lov\'asz Hinge}
\jessie{Lovasz hinge paper~\cite{yu2018lovasz}}
\raf{I'm tempted to do the following.  We give the abstain counter-example for $k=2$ (I've been using $k$ for the number of elements in the set constructions, but maybe $n$ is better; we can sync up notation later), and then characterize the $k=2$ case informally, showing that the only thing that works is when $f(\{1,2\}) = f(\{1\}) + f(\{2\})$.  Then we can say ``In fact, in full version of the paper, we show a much tighter result: essentially the only Lov\'asz hinge that is consistent is weighted Hamming loss'' or something to that effect.  This leaves room for us to send a journal version to JMLR (for which we'll need some new results), which seems like a good idea for a paper like this: We seem to have useful tools here, so why not let them marinate a bit in NeurIPS, collect feedback, and then publish a more polished/mature version.}


\subsection{Abstain loss}
Our final example of an embedded loss is given by Ramaswamy et al.~\cite{ramaswamy2018consistent}.
