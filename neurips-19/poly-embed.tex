...
\begin{proof}
  ...
  \]
  Next, we observe that $L'$ is invariant in the $\ones$ direction, so without loss of generality, we can restrict our consideration to reports so that $u_{[n]} = 0$.
  To see this,
  \begin{align*}
  L'(u + \alpha \ones)_y &=\left(\frac{1}{k} \sum_{i=1}^k ((u+\alpha \ones) + \ones - e_y)_{[i]} - (u_y + \alpha) \right)_+\\
  &= \left(\frac{1}{k} \left[k\alpha + \sum_{i=1}^k (u + \ones - e_y)_{[i]}\right] - (u_y + \alpha) \right)_+\\
  &= \left(\alpha + \frac{1}{k} \left[\sum_{i=1}^k (u + \ones - e_y)_{[i]}\right] - (u_y + \alpha) \right)_+\\
  &= \left(\frac{1}{k} \left[\sum_{i=1}^k (u + \ones - e_y)_{[i]}\right] - u_y \right)_+\\
  &= L(u)_y
  \end{align*}
  ...
  Observe that if $u_{[1]} > 1$, the value of the loss gets ``cropped'' and will be the same by reporting $u_{[1]} = u_{[n]} + 1 = 1$. \jessiet{Flesh this out later.}
  ...
  Moreover, for this optimal $u$, observe $\|u\|_{1} \leq k$ since we have $u_{[k+1]} = u_{[k+2]} = \ldots = u_{[n]} = 0$, so we observe that $u \in \U$ as well.
\end{proof}
