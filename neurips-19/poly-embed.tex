...
For condition (ii), let us compute the property each loss elicits:
\[
...
They also show that Lapin's surrogates are not consistent, but without too much intuition as to why.
...
We show that $L_2$ embeds some other discrete loss $\ell_2:\{0,1\}^n \to \reals^\Y_+$, which has a different Bayes Risk than $\ell_{0-1}$.

In order to do this, we want to show that for all $p \in \simplex$, $\inf_{u' \in \reals^n} \inprod{p}{L_2(u')} = \min_{u' \in \{0,1\}^n} \inprod{p}{L_2(u')}$. (Lemma~\ref{lem:top-k-optimal-corners}.)
Then, we show there is a discrete loss $\ell_2 : \{0,1\}^n \to \reals^\Y_+$ that is embedded by $L_2$.  (Lemma~\ref{lem:top-k-surrogate-embeds}.)

We conclude with a concrete example; $k=1$.
One can see that the $\ell_{top-1}$ reduces to 0-1 loss, and we characterize the distributions $p \in \simplex$ where $\risk{L_2}(p) = \risk{\ell_2}(p) \neq \risk{\ell_{top-k}}(p)$.
...
L_2(u,y) := \left( \frac{1}{k} \sum_{i=1}^k (u + \ones - e_y)_{[i]} - u_y \right)_+
\end{align}
where $x_{[i]}$ is the $i^{th}$ largest component of $x$.
By Lemma~\ref{lem:top-k-optimal-corners}\jessie{in the Appendix... remove ref for camera ready version}, we can see that for all $p \in \simplex$, there is some $u \in \{0,1\}^n$ so that $u \in \argmin_{u' \in \reals^d} \inprod{p}{L_2(u)}$.
With this, we show that $L_2$ actually embeds a discrete loss $\ell_2: \{0,1\}^n \to \reals^\Y_+$ defined on the corners of the hypercube below:
\begin{align}\label{eq:ell-2}
\ell_2(r,y) &= \Ind{y \not\in r} + \min\left(\frac{|r \setminus \{y\}|}{k}, 1\right)
\end{align}
We show that $L_2$ embeds $\ell_2$ in Lemma~\ref{lem:top-k-surrogate-embeds} \jessie{in the Appendix... remove ref for camera ready version}.
Moreover, by Proposition~\ref{prop:embed-bayes-risks}, we know $\risk{L_2} = \risk{\ell_2}$.

While $L_2$ given by~\citet{lapin2018analysis} is not consistent with respect to top-$k$ loss, we do show a discrete loss $\ell_2$ so that $L_2$ is consistent with respect to $\ell_2$.

Taking $k = 1$, we see that $\vec{0} \in \argmin_{r' \subseteq [n]} \inprod{p}{\ell_2(r')}$ for any $p$ with $\max_y p_y \leq 1/2$, while $\risk{\ell_{top-1}}(p) = 1 - \max_y p_y < 1$.
First, we see that $\ell(\vec{0}, \bar{p}) = 1$ for all $p \in \simplex$.
...
Therefore, we have that $\vec{0} \in \argmin_{r' \subseteq [n]} \inprod{p}{\ell_2(r')}$ for any $p$ such that $\max_y p_y \leq 1/2$.
...
\section{Top-$k$}
Throughout this section, consider the surrogate and discrete loss $L_2(u,y)~:=~\left(\frac{1}{k} \sum_{i=1}^k (u + \ones - e_y)_{[i]} - u_y \right)_+$ and $\ell_2(r,y) = \Ind{y \not\in r} + \min\left(\frac{|r \setminus \{y\}|}{k}, 1\right)$ given in Equations~\ref{eq:L-2-surrogate} and~\ref{eq:ell-2}, respectively.


Observe that $L_2$ is a polyhedral loss, since it can be written as the pointwise max of $\binom{n}{k} +1$ terms, where the $\binom{n}{k}$ terms are selecting the $k$ elements of $u + \indopp(y)$ with highest weight.
...
\begin{lemma}\label{lem:top-k-optimal-corners}
	For all $p \in \simplex$, there is a $u \in \{0,1\}^n$ such that $u \in \argmin_{u'} \inprod{p}{L_2(u)}$.
\end{lemma}
\begin{proof}
...
$L_2:\reals^n \to \reals^\Y_+$ embeds $\ell_2:2^{[n]} \to \reals^\Y_+$.
\end{lemma}
\begin{proof}
  We can verify $\ell_2(r) = L_2(\varphi(r))$ for all $r \subseteq [n]$ and $\varphi(r)$ being its conversion from a set to binary vector, so $\varphi(\R) = \{0,1\}^n$.
  Moreover, by Lemma~\ref{lem:top-k-optimal-corners} and the above equality, respectively, we know that $\inf_{u \in \reals^n} \inprod{p}{L_2(u)} = \min_{u \in \{0,1\}^n} \inprod{p}{L_2(u)} = \min_{r \subseteq}\ell_2(r)$, and so we conclude that $L_2$ embeds $\ell_2$.
\end{proof}
