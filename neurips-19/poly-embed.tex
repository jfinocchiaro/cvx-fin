...
For example, 0-1 loss is a discrete loss with $\R = \Y = \{1,2,\ldots,n\}$ given by $\ell(r)_y = \Ind[r \neq y]$, and has Bayes risk $\risk{\ell}(p) = 1-\max_{y\in\Y} p_y$. \jessiet{Isn't that the Score $G$?  Don't we want $\risk{\ell} = 1 - \min_{y \in \Y} p_y$?}\raft{did you mean $1-\max$?}
...
For finite properties (those with $|\R|<\infty$) and discrete losses, we will use lowercase notation $\gamma$ and $\ell$, respectively, while $\Gamma$ and $L$ will be used for surrogate or general properties.
...
To ensure consistency, we will seek ``calibration''  of the surrogate and link function.
...
%  $\gamma'(p) := \Gamma(p) \cap \U$ is nonempty for all $p \in \simplex$ because $r \in \gamma(p) \implies \varphi(r) \in \Gamma(p)$ for all $p \in \simplex$, and by definition $\varphi(r) \in \U$, so $\varphi(r) \in \gamma'(p)$ for all $p \in \simplex$.
  $\gamma'(p) := \Gamma(p) \cap \U$ is nonempty for all $p \in \simplex$ as $r \in \gamma(p) \implies \varphi(r) \in \gamma'(p)$ by the definition of embedding and nondegeneracy of $\gamma$.
  By Lemma~\ref{lem:loss-restrict}, we know that $L|_{\U}$ elicits $\gamma'$ and $\risk{L} = \risk{L|_{\U}}$.
...
%  As $|\R| = |\R'|$, we know there is a bijection from one to the other defined by the embedding $\varphi$ on a restricted domain.
  ...
  The projection of their epigraphs onto $\simplex$ forms a power diagram by \ref{thm:aurenhammer}, and since the risks are the same, we know the corresponding power diagrams are equal.  %, corresponding to the level sets of the properties each loss elicits.
...
\end{theorem}
\begin{proof}
  The result is a Corollary of Theorem~\ref{thm:polyhedral-embed-prop} \jessie{and statement on matching losses.}.
\end{proof}
...
\begin{proof}
  ...
  From standard results in convex analysis, $C$ is polyhedral as $-\risk{\ell}$ is, and $C$ is finite on all of $\reals^\Y$ as the domain of $-\risk{\ell}$ is bounded~\citet[Corollary 13.3.1]{rockafellar1997convex}.
...
\jessie{Do we want this at all?}
To be useful in applications, a loss which embeds a property must be accompanied by a calibrated \jessie{separated?}link to that property.
...
In most cases, devising a separated link is straightforward.
Giving a construction that yields a separated link for all polyhedral losses is a nuanced problem described in the following section.
\raf{Replace this paragraph with gloating about our result if we actually give this construction.}
...
Rather than formally define the Lov\'asz hinge, we defer the full analysis to the original version of the paper~\cite{yu2015lovaszarxiv}, and focus here on the $k=2$ case.
