...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \left( 1 -u_y - \alpha + \alpha + \frac{1}{k}\left[ \sum_{i=1}^k (u - e_y)_{[i]} \right] \right)_+\\
  ...
  Since $u_{[n]} = 0 \implies u_{[1]} \in [0,1]$, we can then observe for all $p \in \simplex$, there is some $u \in \prop{L'}(p)$ so that $u \in [0,1]^n$.
