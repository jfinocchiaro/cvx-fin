...
\begin{proof}
  ...
  \begin{enumerate}
    \item Each $G_{u^*,v}$ can be associated with at least one of the finitely many linear functions $f$ as follows: $f(u) = \hat{L}(u;p)$ for all $u \in G_{u^*,v}$.
    \item There exists a constant $c_{u^*,v} > 0$ such that $f(u) = L(u^*;p) + c_{u^*,v} \cdot d(U,u)$ for all $u \in G_{u^*,v}$.
    \item The set of $(u^*,v)$ associated with $f$ is compact (closed and bounded), and the map $(u^*,v) \mapsto c_{u^*,v}$ is continuous.
    \item Hence, there is an infimum $c > 0$ such that $f(u) \geq L(u^*;p) + c \cdot d(U,u)$ for all $u$ in all $G_{u^*,v}$ associated with $f$.
    \item Taking the minimum such $c$ over these finitely many $f$, we obtain $L(u;p) \geq \hat{L}(u;p) \geq inf_{u^*} L(u^*;p) + c \cdot d(U,u)$ for all $u \not\in U$.
  \end{enumerate}

  (1) Follows because $G_{u^*,v}$ is contained in a cell of the power diagram, so it lies below the function where $\hat{L}(\cdot;p) = f(\cdot)$. \bo{Should have more justification!}

  (2) Let $\nabla f$ denote the gradient of the linear function $f$.
  Note that $f(u^*) = \hat{L}(u^*;p) = L(u^*;p)$.
  So we can write
  \begin{align*}
    f(u) &= f(u^*) + (\nabla f)\cdot (d(u^*,u) v)  \\
         &= f(u^*) + c_{u^*,v} \cdot d(U,u)
  \end{align*}
  where $c_{u^*,v} = (\nabla f)\cdot v$ and we used that $d(u^*,u) = d(U,u)$ by Lemma \ref{lemma:exposed-shortest}.
  We must have $c_{u^*,v} > 0$ because the set $U$ minimizes $L(\cdot;p)$, which is upper-bounded by $f$, so $f(u) > f(u^*) = L(u^*;p)$.

  (3) \bo{This one needs more details}
  This follows because the set of $u^*$ such that $f(u^*) = \hat{L}(u^*;p)$ is closed, and for fixed $u^*$, the set of $v$ such that $G_{u^*,v}$ is in the cell of the power diagram is also closed.
  The set is also bounded \bo{is there a corner case to consider here?} and the map $(u^*,v) \mapsto (\nabla f)\cdot v =: c_{u^*,v}$ is continuous for fixed $f$ (it is linear in $v$ only).

  Applying (4) and (5) prove the theorem.
\end{proof}
\bo{Improving the above: may suffice to only consider unit vectors $v$. Claim is that it's associated with $\argmax_i (\nabla f_i)\cdot v$. Now show the set of $v$ associated with $f_i$ is compact, and that for each $v$, it's true that for all $u^*$ exposed by $v$ and all $u = u^* + \alpha v$ for some $\alpha > 0$, we have $f(u) \geq f(u^*) + c_v \cdot d(U,u)$.}
