...
\begin{proof}[Sketch]
  (See Appendix~\ref{app:calibration} for the full proof.)
  \emph{Well-defined:}
...
\btw{The story we discussed: YK show $L_2$ is inconsistent.  Our framework lets us ask: with what discrete loss \emph{is} $L_2$ consistent?  (It also assures us that $L_2$ is consistent for something...) Oh look, $L_2$ embeds something which is $\elltopk$ plus some other term, so (a) we can see very clearly what $L_2$ ``is doing'', and (b) we can see where to look for distributions yielding inconsistency, namely ones for which we actually prefer to report a set with $|r|<k$.}
...
\end{align}
where $e_y$ is $1$ in compnonent $y$ and 0 elsewhere.
