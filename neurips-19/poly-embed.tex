...
\begin{proof}
  ...
  For each facet $F$ of $\risk{L}$, consider the report $u = \arginf_{u'} L(u', p)$ for some $p \in \inter F$.
  (Note that this $u$ will be unique regardless of the choice of $p\in \inter F$, since $\risk{L}$ is differentiable here.)
  Since $\risk{L}$ has a finite set of facets, the set $\R = \{u\}$ \jessie{notate} is finite.
