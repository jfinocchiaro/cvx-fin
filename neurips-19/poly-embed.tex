...
\end{theorem}
\jessie{Cut the definition of refines, so maybe we just want the first part of this.}
\begin{proof}[Proof of Theorem~\ref{thm:polyhedral-embed}]
  ...
  % Let $v'\in\reals^{d+1}$ be a vertex on such a face, and $v\in \F_\Y$ its projection onto $\reals^d$.
  ...
  % (When $\Y' = \{y\}$ is a singleton, we simply take the faces of $D_{\{y\}}projected vertices of $L(\cdot)_y$.)

  Thus, taking $\F = \bigcup\{\F_{\Y'} | \Y'\subseteq\Y, \Y'\neq\emptyset\}$, we have for all $p\in\simplex$ that $\Gamma(p) \in \F$.
  This implies that $\trim(\Gamma) \subseteq \F$, which is finite, so Proposition~\ref{prop:embed-trim} now gives the conclusion.
  \jessie{Do we need to show the preimage of $\Gamma$ is $\F$?}
  \raf{I'm not sure what you mean.  I also realized that I proved the wrong thing -- and $\trim(\Gamma) \subseteq \F$ does not make any sense.  Will have to think about this more...}
