...
\begin{proof}
  ...
  \jessie{Alluded to in COLT Lemma 11 and Prop 12 (Lemma~\ref{lem:finite-full-dim} and Prop~\ref{prop:embed-trim} here), but I don't see us prove this in the original paper.}
  ...
  Now, to see that $\risk{\ell} = \risk{L} \implies L$ embesds $\ell$, consider $F$ to be the set of facets of the loss of $\risk{L}$.
  For each facet $f \in \F$, we know by nonredundancy that there is some $p \in \inter{f}$ so that $\risk{L}$ is differentiable at $p$.
  Moreover, by Frongillo and Kash~\jessie{CITE... 2014 paper? Theorem 24?} we know that both $\ell$ and $L$ have a unique minimizer at $p$, and since the subgradients of the risks are the same (by their equality), we know the minimizers of the losses are also the same.
  Therefore, we can see (with more elaboriating) that $L$ embeds $\ell$.
...
\begin{proof}[Proof of Theorem~\ref{thm:polyhedral-embed}]
  ...
  From Lemma~\ref{lem:polyhedral-pd-same}, we have that the power diagram induced by the projection of $P(p)$ onto $\reals^d$ is constant over $p$ whenever $p\in\inter\simplex$.
