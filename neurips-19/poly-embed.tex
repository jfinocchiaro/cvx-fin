...
From Lemma~\ref{lem:loss-restrict}\raft{was lem:top-k-surrogate-embeds}, then, $L'$ must embed $L'|_\U$, which gives us $\ell':\R'\to\reals^\Y_+$ via the natural embedding from sets $\R' = \{r \subseteq [n] : |r| \leq k\}$ to their indicator vectors:
...
\begin{proof}
  ...
  For any $u^*\in\partial U$, $v\in V$ such that $v$ exposes $u^*$, let\raft{I'm not actually sure what definition of ``exposes'' we are using here; I think we just mean that $v$ is normal to $U$ at $u^*$, right?}
    \[ G_{u^*,v} = \left\{ u^* + \beta v : \beta \geq 0 \right\} . \]
  %Note that $\{ u : u \not\in U\} \subseteq \cup_{u^*,v} G_{u^*,v}$.
  We will prove the following steps:
  \begin{enumerate}
    \item For all $u^*\in\partial U$, $v\in V$ such that $v$ exposes $u^*$, there is some $f\in\F$ such that $G_{u^*,v}\subseteq U_f$.
    \item For all $v\in V$, there exists a constant $c_{v} > 0$ such that $L(u) = \risk{L}(p) + c_{v} \cdot d(U,u)$ for all $u \in G_{u^*,v}$ and all $u^*$ exposed by $v$.
    \item For all $f\in \F$, the set $V_f = \{v\in V : \exists u^*\in\partial U\: G_{u^*,v} \subseteq U_f\}$ is compact (closed and bounded), and the map $v \mapsto c_{v}$ is continuous on $V_f$.
    \item Hence, there is an infimum $c_f > 0$ such that $f(u) \geq \risk{L}(p) + c_f \cdot d(U,u)$ for all $u\in U_f$.
    \item Taking $c = \min \{c_f : f\in\F, \nabla f \neq 0\}$, we obtain $L(u;p) \geq \hat{L}(u;p) \geq \risk{L}(p) + c \cdot d(U,u)$ for all $u \not\in U$.
  \end{enumerate}
  ...
  \begin{align*}
    f(u) &= f(u^*) + (\nabla f)\cdot (u - u^*)  \\
    f(u) &= f(u^*) + (\nabla f)\cdot (d(u^*,u) v)  \\
         &= \risk{L}(p) + c_{v} \cdot d(U,u)  \\
  \end{align*}
  where $c_{v} = (\nabla f)\cdot v$.
  We must have $c_{v} > 0$ because the set $U$ minimizes $L(\cdot;p)$, so $f(u) > f(u^*) = \risk{L}(p)$.

  (3) The intersection $U_f \cap U$ is a face of $U$.
  $V_f$ is simply the union of normal cones $N_U(u^*)$ intersected with $V$ for all $u^* \in U_f \cap U$; as normal cones are identical in the relative interiors of faces, and $U_f\cap U$ is the union of finitely many such relative interiors of faces, $V_f$ is union of finitely many closed convex sets, and thus closed.
  \raft{This could stand a more formal treatment}
  Boundedness follows as $V$ is bounded.
  
  Finally, steps (4) and (5) are immediate.
\end{proof}
