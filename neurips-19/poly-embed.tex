...
\newcommand{\elltop}[1]{\ell_{\text{top-$#1$}}}
...
Surrogates for this problem commonly take reports $u\in\reals^n$, with the link $\psi(u) = \{u_{[1]},\ldots,u_{[k]}\}$, where $u_{[i]}$ is the $i^{th}$ largest component of $u$, with ties broken arbitrarily.
...
L_k(u)_y := \left( 1-u_y + \tfrac{1}{k} \textstyle\sum_{i=1}^k (u - e_y)_{[i]} \right)_+~,
\end{align}
where $e_y$ is $1$ in component $y$ and 0 elsewhere.

For concreteness, consider the case with $k = 2$ over $n=3$ outcomes.
%Without loss of generality, suppose $u$ is sorted in decreasing order, i.e. $u_1 \geq u_2 \geq u_3 = 0$ by invariance in the $\ones$ direction, shown in the Appendix.
We can re-write $L_2(u)_y = \left(1 - u_y + \tfrac 1 2 (u_{[1]}+ u_{[2]} - \min(1,u_y))\right)_+$.
One can verify that for every distribution $p \in \Delta_3$, $\inprod{p}{L}$ has a minimizer in the set $\R'$ composed of the tuples $i$ given by $\Gamma_i$ in Figure~\ref{fig:convex-surrogate-top-k}.
...
\begin{minipage}{0.3\linewidth}
	...
	\caption{Minimizers of $\elltop{2}$ as a function of $p$.}
	\label{fig:original-top-k}
\end{minipage}
\hfill
\begin{minipage}{0.3\linewidth}
	\centering
	\includegraphics[width=\linewidth]{tikz/convex-surrogate-top-k}
	\caption{Minimizers of $L_2$ as a function of $p$.}
	\label{fig:convex-surrogate-top-k}
\end{minipage}
\hfill
\begin{minipage}{0.3\linewidth}
	\centering
	\includegraphics[width=\linewidth]{tikz/finite-surrogate-top-k}
	\caption{Minimizers of $\ell$ as a function of $p$.}
	\label{fig:finite-surrogate-top-k}
\end{minipage}
\end{figure}

Figure~\ref{fig:original-top-k} gives a map of the property values of $\gamma_{top-2} := \prop{\ell_{top-2}}$ over the domain, $\Delta_3$, where $\ell_{top-2}(r)_y = \Ind{y \not\in r}$.
(Each cell in this simplex is a level set $\gamma_r$ and corresponds to a flat piece of the Bayes Risk $\risk{\ell_{top-2}}$.)
Moreover, Figure~\ref{fig:convex-surrogate-top-k} maps the property $\Gamma := \prop{L_2}$ over $\Delta_3$.
By visual inspection, we can observe there is no ``refining'' mapping from $\gamma_{top-2}$ to $\Gamma$.
By refining, we loosely mean that, for some $j \in \R$, there is no $I \subseteq \reals^n$ such that $\cup_{i \in I}\Gamma_i = \gamma_{top-2,j}$.
If such a mapping existed, it would allude to one possible consistent link $\psi$ by which $L_2$ embeds $\ell_{top-2}$.

However, by Theorem~\ref{thm:poly-embeds-discrete}, we do know that $L_2$ embeds some discrete loss $\ell_2 := L_2|_\R$ since it is polyhedral; the level sets of $\prop{\ell_2} $ are shown in Figure~\ref{fig:finite-surrogate-top-k}.
\jessie{I can't find a nice reduction of this loss for reports with a $2$ in the vector.}
At first glance, these properties seem indentical, but $\prop{L_2}$ has an infinite number of level sets at each distribution, not shown in Figure~\ref{fig:convex-surrogate-top-k} for simplicity.
We can ``refine'' $\prop{L_2}$ down to $\prop{\ell_2}$ by the axiom of choice.

\jessie{Not sure if this is too much detail...}
For intuition, since $\prop{\ell}$ does not refine $\prop{L}$, then $L$ does not embed $\ell$.
For example, we can take two distributions $p_1 \in \gamma_{top-2, \{1,1,0\}}$ and $p_2 \in \gamma_{top-2, \{1,0,1\}}$ such that $p_1, p2 \in \Gamma_{(1,0,0)}$.
Regardless of the choice of $\psi((1,0,0))$, for either $p_1$ or $p_2$, $\psi((1,0,0)) \not \in \argmin_u \inprod{p_i}{\ell_{top-2}(u)}$.
Thus, we see that $L_2$ is not a consistent surrogate for $\ell_{top-2}$.
...
\jessie{Can scrap Lemma 13... not sure the point of Lemma 12 on its own, but it might be worth keeping.  Thoughts?}
