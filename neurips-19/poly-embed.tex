...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item There exists a constant $c_{v} > 0$ such that $L(u) = \risk{L}(p) + c_{v} \cdot d(U,u)$ for all $u \in G_{u^*,v}$ and all $u^*$ exposed by $v$.
    \item The set of unit vectors $v$ that can be associated with $f$ is compact (closed and bounded), and the map $v \mapsto c_{v}$ is continuous on this set.
    \item Hence, there is an infimum $c > 0$ such that $f(u) \geq \risk{L}(p) + c \cdot d(U,u)$ for all $u$ in all $G_{u^*,v}$ associated with $f$.
    \item Taking the minimum such $c$ over these finitely many $f$, we obtain $L(u;p) \geq \hat{L}(u;p) \geq \risk{L}(p) + c \cdot d(U,u)$ for all $u \not\in U$.
  \end{enumerate}
  ...
  (3) The set of $u^*$ where $f(u^*) = \risk{L}(p)$ is the intersection of two cells of the power diagram, i.e. the boundary of $U$ with the cell associated with $f$.
  Hence it is closed and convex.
  The set of unit vectors $v$ that expose a closed, convex set of boundary points is also closed and convex.
  \bo{That should be justified/rechecked.}
