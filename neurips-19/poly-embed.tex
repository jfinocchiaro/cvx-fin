...
Moreover, the link function $\psi$ is exactly the link obtained from Theorem\raf{ref Bo's stuff} with the norm being $\|\cdot\|_\infty$ and $\epsilon=1/2$.
...
To illustrate this point further, we give an alternate link $\psi_1$ generated from Theorem\raf{ref Bo's stuff} for $\|\cdot\|_1$ and $\epsilon=1$, shown in Figure\raf{ref}:
\begin{equation}\label{eq:abstain-link}
  \psi_1(u) = \begin{cases}
	\bot & \|u\|_1 \leq 1\\
	B^{-1}(\sgn(-u)) &\text{otherwise}
  \end{cases}~.
\end{equation}
In particular, Theorem\raf{ref Bo's stuff} gives us calibration of $(L_{1/2},\psi_1)$ with respect to $\ellabs{1/2}$.
...
\raf{Jessie: stuff to update / change:}
  ...
  \item You keep saying $L(r,y)$ not $L(r)_y$ for all losses
  \item Let's use our property notation; $\prop{L_2}$ instead of argmins
  ...
  \item Domain of $\ell_2$ should not be $\{0,1\}^n$ anymore
  \end{itemize}
...
Then, we show there is a discrete loss $\ell_2 : \{0,1\}^n \to \reals^\Y_+$ that is embedded by $L_2$.  (Lemma~\ref{lem:top-k-surrogate-embeds}.)\jessiet{Appendix ref will be broken}
...
By Lemma~\ref{lem:top-k-optimal-corners}\jessiet{in the Appendix... remove ref for camera ready version}, we can see that for all $p \in \simplex$, there is some $u \in \{0,1\}^n$ so that $u \in \argmin_{u' \in \reals^d} \inprod{p}{L_2(u)}$.
With this, we show that $L_2$ actually embeds the discrete loss $\ell_2: \R \to \reals^\Y_+$, where $\R := \{r \subseteq 2^{[n]} : |r| \leq k \}$, given below:
...
Now to see a set of distributions where $\risk{L_2}(p) \neq \risk{\ell_{top-1}}(p)$, we claim that $\vec{0}~\in~\argmin_{r' \subseteq [n]} \inprod{p}{\ell_2(r')} \implies \risk{\ell_2}(p) = 1$ for any $p$ with $\max_y p_y \leq 1/2$.
Additionally $\risk{\ell_{top-1}}(p) = 1 - \max_y p_y < 1$.

First, $\ell(\vec{0}, \bar{p}) = 1$ for all $p \in \simplex$.
Taking the set $\{y\}$ for any $y \in \Y$, the expected loss $\ell_2(\{y\}, p) = 2 (1-p_y) \geq 1$ by assumption on $p$. 
...
\begin{proof}[Sketch]
  ...
  We first claim that for all $y \in \Y$, the minimum of $L(u,y)$ is achieved by some $u \in [0,1]^n$.
  ...
  Since $u_{[n]} = 0 \implies u_{[1]} \in [0,1]$, we can then observe there is some $u \in \argmin_{u' \in \reals^d} L_2(u',y)$ so that $u \in [0,1]^n$.
