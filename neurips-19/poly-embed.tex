...
Fix a fixed distribution $D \in \Delta(\X\times\Y)$, and let $L^*$ be the best possible expected $L$-loss achieved by any hypothesis $H:\X\to\reals^d$, and $\ell^*$ the best expected $\ell$-loss for any hypothesis $h:\X\to\R$, respectively.
...
\begin{proof}
  ...
  By Lemma~\ref{lemma:U-convex-closed}, $\U = \Gamma(\simplex)$ is finite.
...
Using a construction via convex conjugate duality, which has appeared several times in the literature (e.g.\ \cite{duchi2018multiclass,frongillo2014general})\raft{CITE: DKR, frongillo-kash (WINE 2014), probably one of Bob's papers?, others?}, we see that the answer is yes, although the number of dimensions $d$ required could be as large as $|\Y|$.
...
\end{proof}
While we sketch the intuition here, the full proof is given in Section~\ref{app:calibration} of the Appendix.
...
\end{tikzpicture}
\end{minipage}\hfill
\caption{Left: Possible optimal reports given by our $\psi$ constructed using $\|\cdot\|_\infty$ and $\epsilon = 1/2$.  Middle: Two examples of $U \in \U$ and their thickened regions for $\|\cdot\|_1$ and $\epsilon = 1$. Right: Link values given by our $\psi$ constructed using $\|\cdot\|_1$ and $\epsilon = 1$}
...
\item The next part of the story: oh look, $L_2$ embeds something which is $\elltopk$ plus some other term, so (a) we can see very clearly what $L_2$ ``is doing'', and (b) we can see where to look for distributions yielding inconsistency, namely ones for which we actually prefer to report a set with $|r|<k$. \jessie{Fixed with question}
\end{itemize}
...
Surrogates for this problem commonly take reports $u\in\reals^n$, with the link $\psi(u)$ mapping $u$ to the set of indices for the largest $k$ elements of $u$, with ties broken arbitrarily.
...
\begin{align}\label{eq:L-2-surrogate}
  L'(u)_y := \left( \frac{1}{k} \sum_{i=1}^k (u + \ones - e_y)_{[i]} - u_y \right)_+~,
\end{align}
where $e_y$ is $1$ in compnonent $y$ and 0 elsewhere, and $x_{[i]}$ is the $i^{th}$ largest component of $x$.
...
\ell'(r)_y &= \Ind{y \not\in r} + \tfrac 1 k |r \setminus \{y\}|~.
\end{align}
We now see that $\ell'$ is essentially $\elltopk$ (extended to smaller sets) plus an additional cardinality term.
...
Writing the expected loss, we have $\inprod{p}{\ell'(r)} = 1 - p(r) + \tfrac 1 k (|r|-p(r)) = 1 + \tfrac 1 k |r| - (1 + \tfrac 1 k) p(r)$, where $p(r) = \sum_{i\in r} p_i$.
...
we have $\inprod{p}{\ell'(r)-\ell'(r\setminus\{i\})} = \tfrac 1 k - p_i$, meaning we will drop elements from $r$ until they all have weight at least $\tfrac 1 k$.
...
More generally, as $k < n$, the set $P = \{p\in\simplex : \max_{r\in\R'} p(r) < n/k\}$ is full-dimensional and guarantees that at least one of the top $k$ labels has probability $< \tfrac 1 k$.

% We now focus on the case where $k=1$; the top-$k$ loss reduces to 0-1 loss in this case.
% To show a set of distributions where $\prop{\ell_2}(p) \neq \prop{\elltopk}(p)$, we claim that $\vec{0}~\in~\prop{\ell_2} \implies \risk{\ell_2}(p) = 1$ for any $p$ with $\max_y p_y \leq 1/2$.
...
%Since we can see that $\ell_2 \neq \elltopk$, we can then see that $L_2$ is not consistent with respect to $\elltopk$.
...
\begin{proof}
  ...
  \bo{Change $\underbar{L}$ to something like $\hat{L}$.}\jessiet{Done}
...
Throughout this section, consider the surrogate and discrete loss $L_2(u)_y~:=~\left(\frac{1}{k} \sum_{i=1}^k (u + \ones - e_y)_{[i]} - u_y \right)_+$ and $\ell_2(r)_y = \Ind{y \not\in r} + \frac{|r \setminus \{y\}|}{k}$ given in Equations~\ref{eq:L-2-surrogate} and~\ref{eq:ell-2}, respectively.
...
\begin{proof}[Sketch]
  ...
  Now, since we can restrict to the closed hypercube, we claim that we can rewrite the expected loss of $L_2$ as 
  \begin{align*}
