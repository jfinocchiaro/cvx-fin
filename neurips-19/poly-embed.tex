...
For example, \citet{ramaswamy2018consistent} study the loss $\ellabs{\alpha} : [n] \cup \{\bot\} \to \reals^\Y_+$ defined by $\ellabs{\alpha}(r) = 0$ if $r=y$, $\alpha$ if $r = \bot$, and 1 otherwise.
...
\begin{proof}
  ...
  This follows from the observation that if $u_{[1]} > 1$, the the loss only decreases by reporting $u_{[1]} = u_{[n]} + 1 = 1$.
