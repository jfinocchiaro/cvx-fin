...
One can see that the $\ell_{top-1}$ reduces to 0-1 loss, and we characterize the distributions where $\risk{L_2}(p) = \risk{\ell_2}(p) \neq \risk{\ell_{top-k}}(p)$.
...
We write the surrogate 
\begin{align*}
L_2(u,y) := \left( \frac{1}{k} \sum_{i=1}^k (u + \ones - e_y)_{[i]} - u_y \right)_+
\end{align*}
where $x_{[i]}$ is the $i^{th}$ largest component of $x$.
By Lemma~\ref{lem:top-k-optimal-corners}, we can see that for all $p \in \simplex$, there is some $u \in \{0,1\}^n$ so that $u \in \argmin_{u' \in \reals^d} \inprod{p}{L_2(u)}$.
With this, we show that $L_2$ actually embeds a discrete loss defined on the corners of the hypercube.

When embedding, we consider the discrete loss $\ell_2: \{0,1\}^n \to \reals^Y_+$
\begin{align*}
\ell_2(r,y) &= \Ind[y \not\in r] + \min\left(\frac{|r \setminus \{y\}|}{k}, 1\right)~,~
\end{align*}
and we show that $L_2$ embeds $\ell_2$ in Lemma~\ref{lem:top-k-surrogate-embeds}.
...
First, we see that $\ell(\vec{0}, \bar{p}) = 1$.
...
So while $L_2$ given by~\citet{lapin2018analysis} is not consistent with respect to top-$k$ loss, we do show a discrete loss $\ell_2$ so that $L_2$ is consistent with respect to $\ell_2$.
...
\section{Top-$k$}
Throughout this section, consider the surrogate and discrete loss\jessie{fill in}
