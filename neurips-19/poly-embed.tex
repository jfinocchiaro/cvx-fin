...
\begin{theorem}\label{thm:polyhedral-embed-prop}
...
\begin{enumerate}
	\item \jessie{TODO-- taking a quick break.}
	...
	Moreover, by part (1.) and the above verification, respectively, we know that $\inf_{u \in \reals^n} \inprod{p}{L_2(u)} = \min_{u \in \{0,1\}^n} \inprod{p}{L_2(u)} = \min_{r \subseteq}\ell_2(r)$, and so we conclude that $L_2$ embeds $\ell_2$.
