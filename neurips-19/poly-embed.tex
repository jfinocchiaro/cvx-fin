...
\jessie{If the convex loss is bounded from below by $M$, we can say $\reals^\Y_+$ WLOG since $L$ elicits $\Gamma$ implies $(L +c)$ elicits $\Gamma$ for any constant $c$-- in particular, $M$.  We might also assume our discrete loss is nonnegative?}
...
\begin{proof}
  ...
  Since $\ell$ is discrete, we know that $\risk{\ell}$ is polyhedral. \jessie{Need to prove?}
  Therefore, $\risk{L}$ is polyhedral.
  
  For the reverse implication, suppose $\risk{L}$ is polyhedral. 
  For each facet $f$ of $\risk{L}$, consider the report $u_f = \arginf_{u'} L(u', p)$ for some $p \in \inter f$.
  (Note that this $u$ will be unique regardless of the choice of $p\in \inter f$, since $\risk{L}$ is differentiable here.)
  Since $\risk{L}$ has a finite set of facets, the set $\R = \{u_f\}_{f \in F}$ \jessie{add that $F$ is full set of facets} is finite.
  Now define the discrete loss $\ell(r,y) = L(r,y)$ for all $r \in \R$.
  ...
  \begin{align*}
  	\risk{L} = \inf_{u \in \reals^d} L(u,p) = \min_{r \in \R} L(r,p) = \min_{r \in \R} \ell(r, p) = \risk{\ell}
  \end{align*}
  \jessie{Probably want to rigorously flesh out argument for the second equality.}
...
\begin{proof}
  ...
  \jessie{$L$ polyhedral implies $L^* = -\risk{L}$ polyhedral (cite?) implies $\risk{L}$ polyhedral.}
\end{proof}
...
The above techniques allow us to evaluate and simplify some previous historical results.
For example, Yang and Koyejo~\cite{yang2018consistency} prove the inconsistency of a family of surrogates for the top-$k$ classification problem.
They then create a new surrogate loss that is consistent for the top-$k$ problem, although it should be noted that their surrogate is not convex.
In particular, they evaluate two losses given by Lapin et al.~\cite{lapin2016loss, lapin2018analysis}, one of which is shown to be convex, but they leave its consistency as an open question.

After our discussion of embeddings, it is worth noting that we now have a way of proving consistency: a surrogate $L$ is consistent with respect to a discrete loss $\ell$ if and only if $L$ embeds $\ell$.
Therefore, we can then prove that the surrogates given by Lapin et al.~\cite{lapin2018analysis} are not consistent by showing the discrete loss that their loss embeds is not the discrete loss measuring top-$k$ error.

In particular, we measure top-$k$ error by 
\begin{align}
\ell_{top-k}(r,y) &= 
\begin{cases}
0 & r_y = 1 \text{ and } \|r \|_{\infty} \leq k\\
1 & \text{otherwise}
\end{cases}
\end{align}
Where $r \in 2^n$ can be thought of as a binary vector representing set inclusion and $k \leq n$ is a constant.
...
Following the numbering of Yang and Koyejo, we call the surrogate $L_2(u,y) := \max\left( \frac{1}{k} \sum_{i=1}^k (u + \indopp(y))_{[i]} - u_y \right) $, where $\indopp(y) = \ones - e_y$.

It can be verified that $\ell_2(r,y) = \begin{cases}
\min\left(\frac{HD(r, e_y)}{k}, 1\right) & s_y = 1 \\
1 + \min \left(\frac{HD(s, \vec{0})}{k}, 1\right) & s_y = 0
\end{cases}$, where $HD(\cdot, \cdot)$ is the Hamming distance between two vectors.
