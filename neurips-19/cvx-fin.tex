...
\usepackage[submission]{neurips_2019}
...
We augment this literature by studying a particularly natural approach for finding convex surrogates, wherein one ``convexifies'' a discrete loss.
Specifically, we say a convex surrogate $L$ \emph{embeds} a discrete loss $\ell$ if there is an injective embedding from the discrete reports to a vector space such that (i) the original loss values are recovered, and (ii) a report is $\ell$-optimal if and only if the embedded report is $L$-optimal.
...
Often one is given a property $\gamma$ (typically via a discrete loss $\ell$) and seeks a surrogate loss $L$ with certain attributes, such as convexity, smoothness, etc.
...
\begin{itemize}
\item Def: ``discrete loss'' should connote some regularity as well, just like we did with finite properties: every report should correspond to a full-dimensional cell.
...
\item Note: DKR (section 3.1) realized the importance of matching Bayes risks, but they could only give general results for strictly convex (concave I should say) risks, in part because they fixed the link function to be a generalization of $\sgn$.  In contrast, we focus exclusively on non-strictly-convex risks.
...
\item Converse via DKR construction: every discrete $\ell$ is embeddable.
    \\
  {\tiny Proof: already have it.}
\item Corollary: our current Theorem~\ref{thm:finite-elicit-embed}.
\item Prop: ``separated'' link implies blah blah ERM.  (Modify Arpit--Shivani Theorem 3, since it's not quite general enough.)
\item Theorem: If $L$ is polyhedral and embeds $\ell$, there is a separated link.  (TO PROVE)
\item Note: This prop and theorem give an excellent reason to focus on embeddings, since other techniques do not necessarily give you separated links for free.  Since we know we get them for free, we can just study elicitation complexity and not worry about how fancy the link is (calibrated vs separated).
\item Note: we should comment in the discussion section that we probably can show that *any* loss embedding $\ell$ must be polyhedral-ish, meaning polyhedral except for stuff that is never opetimal.  This theorem would then not need the ``polyhedral'' part.
\end{itemize}
...
\begin{definition}
  ...
  Finally, a loss $L:\reals^d\to\reals^\Y$ \emph{embeds} a loss $\ell:\R\to\reals^\Y$ if $\prop{L}$ embeds $\prop{\ell}$ via $\varphi$ and for all $r\in\R$ we have $L(\varphi(r)) = \ell(r)$.
\end{definition}

\raf{We could potentially shortcut much of this.
  \\[4pt]
  A loss $L:\reals^d\to\reals^\Y$ \emph{embeds} a loss $\ell:\R\to\reals^\Y$ if there exists some injective embdedding $\varphi:\R\to\reals^d$ such that
  (1) for all $r\in\R$ we have $L(\varphi(r)) = \ell(r)$, and (2) for all $p\in\simplex,r\in\R$ we have \[r \in \argmin_{r'\in\R} \inprod{p}{\ell(r')} \iff \varphi(r) \in \argmin_{u\in\reals^d} \inprod{p}{L(u)}\]
  \\[4pt]
Maybe even simplify further?  This could help us introduce the concept before we get into properties and elicitation at all.}
