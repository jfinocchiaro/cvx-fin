...
\begin{definition}
  ...
  Forming the matrix $V \in \reals^{k \times d}$ whose rows are the unit vectors, we must have for each $y$ that the polytope $\{ w : Vw \leq B_y \}$ is nonempty for each $y$ (where $B_y$ is the $y$th column).
\end{definition}
...
\begin{proof}
  ...
  Collecting $v_1,\dots,v_k$ as the rows of a matrix $V$, it is a $d$-representation of the level set, since the polytopes are nonempty, in particular:
    \[ \partial \ell_y(r) \subseteq \{ w : \langle v_i, w \rangle \leq f_{vy} ~ (\forall i) \}. \]
\end{proof}

We use this to prove lower bounds as follows:
\begin{enumerate}
  \item A level set is given to us as $\{ p : Bp \geq \vec{0}\}$ for a matrix $B \in \reals^{k \times n}$.
  \item We show there is no set of $k$ vectors $v_1,\dots,v_k$ that can form a $d$-representation.
  \item In particular, we show that however you pick $v_1,\dots,v_k$, for one of the outcomes $y$, the polytope $\{w : Vw \leq (B_{1y},\dots,B_{ky}) \}$ is empty (infeasible).
\end{enumerate}
