...
\begin{proof}
	\jessie{Direct}
		...
		Let $V_{u,p} : \Y \to \reals^d, y \mapsto W_y$ be the function encoding the columns of $W$\raft{Simplified prev line}.
...
\begin{lemma}\label{lem:finite-relint-dim}
  Let the $d$-flat $F\subseteq \P$ contain some $p\in\relint{\P}$.
  Then 
  \begin{enumerate}
  	\item[(i)] $p \in \relint{F}$; 
  	\item[(ii)] $\dim(\spn(F - \{p\})) \geq \dim(\spn(\P - \{p\})) - d$;
  	\item[(iii)] Given a discrete elicitable property $\gamma: \P \toto \R$ and $r \in \gamma(p)$, we have $F - \{p\}$ contained in $\Sc_{\gamma_r}(p)$.
  \end{enumerate}
\end{lemma}
\begin{proof}
	...
	Observe $p \in F \implies \E_p V(Y) = \vec 0$, and $q \in F \implies \E_q V(Y) = \vec 0$.
	By linearity of expectation, we then have $\E_{q_\alpha} V(Y) = \E_p V(Y) - \alpha(\E_q V(Y) - \E_p V(Y)) = \vec 0 - \alpha(\vec 0 - \vec 0) = \vec 0$.
	...
	By (i), we know $p \in \relint{F}$, and therefore, $\vec 0 \in \relint{F_p}$.
	Consider $v \in F_p$.
	By $F_p$ being convex and $\vec 0 \in \relint{F_p}$, we know there exists an $\alpha > 0$ so that $-\alpha v \in F_p$.
	This implies $p - \alpha v \in F \implies p - \alpha v \in \gamma_r \implies v \in \Sc_{\gamma_r}(p)$ since $F$ and $\Sc_{\gamma_r}(p)$ are convex and by construction of $F$ as the kernel space of a subgradient of a convex loss\jessiet{We need the loss setup here and the loss to be convex... makes me think this last part should be pulled out into its own statement.}
	This yields the desired result, $F_p \subseteq \Sc_{\gamma_r}(p)$.
\end{proof}
