...
Suppose we are given a target property $\gamma:\P \toto \R$, where $\P\subseteq\simplex$ and convex loss $L \in \L_d$.
If there is some $p \in \P$ with $|\gamma(p)| = 1$, and $\gamma(p) = \{r\}$ such that there is no flat $F$ of \Pcodimension at most $d$ with $p \in F \subseteq \gamma_r$, then $L$ does not indirectly elicit $\gamma$.
...
Suppose we are given a elicitable target property $\gamma:\P \toto \R$, where $\P\subseteq\simplex$ is defined over a finite set of outcomes $\Y$, and convex loss $L \in \L_d$.
If there is a $p \in \relint{\P}$ such that there is no flat $F$ of \Pcodimension at most $d$ with $p \in F \subseteq \gamma_r$, then $L$ does not indirectly elicit $\gamma$.
Moreover, if there is some $p \in \relint{\P}$ so that there is no flat $F \subseteq \gamma_r$ of \Pcodimension at most $d$ then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\gamma$.
%
