...
\begin{proof}
	...
	We first construct the flat $F$ and show $F \cap \P \subseteq \Gamma_u$, which implies $F \cap \P \subseteq \gamma_r$ by indirect elicitation if $u = \psi(r)$. \jessie{though this needs more detail since we don't necessarily have $u = \psi(r)$.}. 
	...
	Moreover, we have that $\codim(F) = \dim(\P \setminus F) = \dim(P) - \dim(F) \leq d$.
	To see this, observe $\dim(\P) \leq d + \dim(F)$.
	As $\dim(\nu) \leq d$ and $F$ is the restriction of $\nu$ to $p \in \P$, we have $\dim(F) = \dim(\P \cap \ker(\nu) ) \geq \dim(\P) - d$, giving the desired result.
