...
\begin{table}
	...
	\begin{tabular}{|l||l|l|} 
		...
		\emph{Discrete prediction} & \textbf{Q1:} High-confidence classification \S~\ref{sec:finite-calib}  & \textbf{Q2:} Hierarchical prediction App.~\ref{app:omitted-examples}\\ \hline 
		\emph{Continuous estimation} & \textbf{Q3:} AUC Optimization \jessie{??} & \textbf{Q4:} Variance, entropy, and norms \S~\ref{sec:contin-consis} \\ 
		& \citet{gao2012consistency} & \\
		\hline
	\end{tabular}
...
\subsection{Consistency \jessie{and calibration}} \label{subsec:consistency}
...
Given a target loss, calibration~\citep[Chapter 3]{steinwart2008support} is typically the tool used to study consistency.
Particularly,~\citet{zhang2004statistical, lin2004note,bartlett2006convexity,tewari2007consistency,ramaswamy2016convex} show the equivalence of consistency and calibration in the first quadrant, where one is given a target statistic and discrete prediction set.
We discuss the additional relationship of elicitation to calibration in Appendix~\ref{app:calibration}, and derive Theorem~\ref{thm:consistent-implies-indir-elic} via calibration.
...
As a result, indirect elicitation is a necessary condition for consistency in this case, and in fact, we show this is true for all four quadrants (\S~\ref{sec:consis-implies-indir}). \jessiet{quadrant speak vs ``in both cases'' (discrete target loss vs continuous estimation)?}
...
For discrete target losses, \citet{ramaswamy2016convex} introduce \emph{convex calibration dimension}, the minimum prediction dimension yielding a convex calibrated surrogate.\jessiet{Discussion about how this is really about consistency}
...
\jessiet{Simplify; merge with Sec 4?  Narrative here reads to me as ``hey this general result through calibration is our main contribution'' which I'm not sure if we particularly want.}


In this section, we show that indirect elicitation is necessary for consistency (Theorem \ref{thm:consistent-implies-indir-elic}) for losses in $\L$, and this is the first result to our knowledge that can be applied to all four quadrants.
...
we delegate this proof to Appendix~\ref{app:calibration} and directly prove the general result for all four quadrants.\jessiet{If we move calibration to the Appendix, this paragraph can be mostly removed}
...
\begin{definition}[Consistent: loss]\label{def:consistent-ell}
	Let \jessie{a convex} $\P \subseteq \simplex$ be given.
	A loss and link $(L,\psi)$ are $\D$-consistent for a set $\D$ of distributions over $\X \times \Y$ with respect to a target loss $\ell$ if, for all distributions $p \in \P$, there exists an $x \in \X$ and marginal distribution $D_x \in \D$ such that for all sequences of measurable hypothesis functions $\{f_m : \X \to \R\}$,\jessiet{in a given hypothesis class?}
	\begin{align*}
	\E_D L(f_m(X), Y) \to \inf_f \E_D L(f(X), Y) &\implies \E_D \ell((\psi \circ f_m)(X), Y) \to \inf_f \E_D \ell((\psi \circ f)(X), Y)~.~
	\end{align*}
If such a $\D$ exists, then we simply say $(L,\psi)$ is consistent.
\end{definition}
...
\begin{lemma}\label{lem:consistent-loss-implies-prop}
	Let \jessie{convex} $\P \subseteq \simplex$ be given.
...
\jessiet{Feels like this section got cut a lot for NeurIPS, but seems like some more narrative in this section might be helpful.}
...
\begin{minipage}{0.45\linewidth}
	...
	\caption{%Since the uniform distribution is in the relative interior of the simplex and the level set $\gamma_\bot$, 
          $\dim(\Sc_{\gamma_\bot}($\textbullet$)) = 2$ and $\dim(\Sc_{\gamma_\bot}({\color{blue}\scalebox{0.5}{$\bigstar$}})) = 1$.
...
These bounds address an open question of~\citet{frongillo2020elicitation}\jessiet{Is this the same as~\cite{frongillo2020elicitation}?  Do we need to change that citation.}\raft{Yeah, all should be the 2020 now, and eventually 2021.  Burn after reading :-)}, that of developing a theory of elicitation complexity for with respect to convex-elicitable properties.
...
\begin{lemma}[\citep{frongillo2020elicitation}]
...
\begin{proof}
	...
	The proof of \citet[Theorem 4]{frongillo2020elicitation} argues that $\hat\Gamma$ must refine $\Gamma$, in the sense that every level set of $\hat\Gamma$ is contained in a level set of $\Gamma$; for completeness we give the argument here.
	...
	% The proof of \cite[Theorem 4]{frongillo2020elicitation} argues that $\hat\Gamma$ must refine $\Gamma$, in the sense that for all $\hat r \in \hat\Gamma(\simplex)$ we have $\hat\Gamma_{\hat r} \subseteq \Gamma_r$ for some $r\in\reals^k$.
