...
\paragraph{Our ``four quadrants'' of problem types}
...
When $L\in\L$, we use $\Gamma := \prop[\P]{L}$ to denote the unique property elicited by $L$ (over distributions in $\P$) from eq.~\eqref{eq:elic}. 
...
As a result, indirect elicitation in a necessary condition for consistency in this case, and in fact, we show this is true for all four quadrants mentioned above (\S~\ref{sec:consis-implies-indir}). \jessiet{quadrant speak vs ``in both cases'' (discrete target loss vs continuous estimation)?}
...
\begin{definition}[Consistent: loss]\label{def:consistent-ell}
	A loss and link $(L,\psi)$ are consistent with respect to a target loss $\ell$ if, for all distributions $D$ over input and label spaces $\X \times\Y$, and for all sequences of measurable hypothesis functions $\{f_m : \X \to \R\}$,\jessie{in a given hypothesis class?}
	\begin{align*}
...
\begin{lemma}\label{lem:consistent-loss-implies-prop}
  ...
  $\mu(r,p) := R_\ell(r,p)$.
  Then
	$(L, \psi)$ is consistent with respect to $\ell$ if and only if $(L,\psi)$ is $\propdis$-consistent with respect to $\gamma := \prop[\simplex]{\ell}$. \jessie{We can restrict to $\P$ right? Is $\P \supseteq \cup_{x \in \X} D_x$ required?}
...
\begin{proof}[Proof sketch]
	...
	Moreover, since $p \in \relint{\simplex}$, the flat $F$ satisfies the requirements of Lemma~\ref{lem:finite-relint-dim}, and the bound $\dim(\Sc_{\gamma_r}(p)) \geq \spn(F - \{p\}) \geq n - d- 1$ is given by chaining ~\ref{lem:finite-relint-dim} (ii) and Lemma~\ref{lem:Fp-subset-feasible-subset}.
...
However, consider any $1$-flat containing $p$.
...
See Figure~\ref{fig:flats-bound} for intuition; a $1$-flat would be a line in such a figure.
Therefore, we have no $1$-flat containing $p$ staying in $\gamma_\bot$, so we obtain better lower bound, $\ccdim(\ell^{abs}) \geq 2$.
...
\begin{minipage}{0.45\linewidth}
	...
	\caption{Any $1$-flat $F$ through \textbullet \, also leaves the cell $\gamma_\bot$.\vspace*{-5pt} %However, for the distribution $p = (1/4, 1/4, 1/2)$, there is a $1$ dimensional flat containing $p$ that is a subseteq of $\gamma_\bot$ and $\gamma_3$.
	}
...
\begin{proof}
	...
	(Such a function must exist by $F$ being a $d$-flat.)
	We aim to show $\dim(\spn(F_p)) = \dim(\ker(T_W)) \geq \dim(\spn(P_p)) - d$.

	The first equality follows if we show $\spn(F_p) = \ker(T_W)$.
	Let $v \in \ker(T_W) = \{v \in \spn(\P_p) : T_W(v) = \vec 0\}$.
	As $p \in \relint{\P}$ implies $\vec 0 \in \relint{F_p}$ as a Corollary of (i), there must exist an $\epsilon > 0$ so that $\epsilon v \in F_p$ and $-\epsilon v \in F_p$.
	...
	Now, $\dim(\ker(T_W)) \geq \dim(\spn(P_p)) - d$ follows from the rank nullity theorem, as $\P$ is defined as a set of probability distributions over a finite outcome set $\Y$.
	The rank-nullity theorem states $\dim(\im(T_W)) + \dim(\ker(T_W)) = \dim(\spn(\P_p))$.
	As $\dim(\im(T_W)) \geq d$, this yields the desired inequality.
	
	Chaining the results together yields $\dim(\spn(F_p)) \geq \dim(\spn(\P_p)) - d$.
...
If $\gamma$ is indirectly elicited by a convex $L \in \L_d$, then there is a $d$-flat $F$ such that $p \in F$ and $F_p := F - \{p\} \subseteq \Sc_{\gamma_r}(p)$.	
\end{lemma}
\begin{proof}
	Construct $F$ as in Theorem~\ref{thm:convex-flats-inf-dim}; this flat $F$ is a $d$-flat, contains $p$, and we have $F \subseteq \gamma_r$.
	We aim to show $v \in F_p \implies v \in \Sc_{\gamma_r}(p)$.
	By~\ref{lem:finite-relint-dim} (i), we know $p \in \relint{F}$, and therefore, $\vec 0 \in \relint{F_p}$.
	Consider $v \in F_p$.
...
Let $q \in F$.
Since, by Lemma~\ref{lem:finite-relint-dim} (i), we have $p \in \relint{F}$, there is a $\epsilon > 0$ so that $q' = p - \epsilon (q-p) \in F$

Now, suppose for contradiction that $q \not\in \gamma_{r'}$.\jessiet{I think this count be made more concise, but is clearest this way; it's a bit tricky with the movement between discussing flats and hyperplanes and being subsets of level sets, etc.}
...
\begin{proof}
	...
	As $L'$ is calibrated (and consistent) with respect to $\ell'$ and indirectly elicits $\gamma' := \prop{\ell'}$, then by Corollary~\ref{cor:Pcodim-flat-elic-relint-prop}, we know there exists a $d$-flat $F$ with $p \in F \subseteq \gamma'_r$.
