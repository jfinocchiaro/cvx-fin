...
\newcommand{\Pcodim}{\mathcal{P}-\mathrm{codim}}
...
\begin{definition}[$\P$-codimension]
	The \emph{$\P$-codimension} of a convex set $F \subseteq \P$ is the minimum $d$ such that there exists a measurable function $V : \Y \to \reals^d$ such that $F = \zeros{V}$, and is denoted $\Pcodim(F)$.
\end{definition}
...
\begin{corollary} \jessie{Indirect}
	Given target property $\gamma:\P \to\to \reals^d$, if there is some $p \in \P$ such that $|\gamma(p)| = 1$ with $\gamma(p) = \{r\}$, and there is not a flat $\zeros{\hat V_{u,p}} \subseteq \gamma_r$ of $\P$-codimension at most $d$ then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\hat \Gamma$.
\end{corollary}

\begin{corollary} \jessie{Indirect}
	Given target property $\gamma:\P \to\to \reals^d$, where $\P$ is defined over a finite set of outcomes $\Y$, if there is some $p \in \relint{\P}$ and there is not a flat $\zeros{\hat V_{u,p}} \subseteq \gamma_r$ of $\P$-codimension at most $d$ then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\gamma$.
\end{corollary}
...
Theorem~\ref{thm:bayes-risk-lower-bound} below yields new bounds on the convex elicitation complexity of properties quantifying risk or uncertainty such as variance and entropy.
