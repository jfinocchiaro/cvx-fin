...
This BEP loss is consistent with respect to the target 0-1 modification $\ell(r,y) = \Ind{r \not \in \{y,\bot\}} + (1/2) \Ind{r = \bot}$, and takes $\Theta(\log n)$-dimensional surrogate predictions, instead of $\Theta(n)$, as was required of previous consistent surrogates.
...
We consider supervised learning problems in the space $\X \times \Y$, for some \emph{feature space} $\X$ and a finite \emph{label space} $\Y$ of size $n$, with data drawn from a distribution $D$ over $\X \times \Y$.\footnote{Throughout this paper, we assume $\Y$ is finite for simplicity, though some results are extended to infinite outcome settings in Appendix~\ref{app:infinite-Y}.}
...
As we focus heavily on conditional distributions $p := D_x = \Pr[Y|X=x]$ over $\Y$ given some $x \in X$, we often abstract away $x$, working directly with probability simplex $\simplex$, the set of all distributions over $\Y$, through tools such as calibration~\cite[Chapter 3]{steinwart2008support}.
...
We write $\L_d^{inf} $\jessie{Add conditions on $\L$ for infinite $\Y$} for the set of minimizable, $\mathcal{B}(\reals^d) \otimes \Y$-measurable and lower semi-continuous surrogates $L : \reals^d \times\Y \to \reals$ such that $\E_p L(u,Y) < \infty$ for all $u \in \reals^d, p \in \P$; we will use this class of loss functions with $\Y$ such that $|\Y| = \infty$ in mind. \jessie{Sufficient condition for $\A$-normal convex integrand: $L$ is lower semi-continuous and $\mathcal{B}(\A) \otimes \Y$-measurable; $\E_p L(u,Y)$ finite for all $u, p$, there exists a $u_0$ for each $p$ so that $\E_p L(u_0, Y)$ is finite and continuous for Rockefellar's corollary, though it's stricter than Ioffe and Tokhimorov IIRC}
...
Moreover, we write $\L_d = \L_d^{inf} \cup \L_d^{fin}$ and $\L = \cup_{d \in \N} \L_d$.
%We write $\L_d = \L_d^{inf} \cup \L_d^{fin}$\jessie{Add conditions on $\L$ for infinite $\Y$} for the set of surrogates $L: \reals^d \times \Y \to \reals$ that are always minimizable, in the sense that $\argmin_{u} \exploss{L}{u}{p}$ is nonempty for all $p\in\simplex$, and write $\L = \cup_{d \in \mathbb{N}} \L_d$.
A loss $\ell$ is \emph{discrete} if $\R$ is a finite set.
A surrogate $L$ is \emph{convex} if $L(\cdot,y)$ is convex in $u$ for every label $y \in \Y$.
...
\jessie{Aiming for a unified definition here}
\begin{definition}[\jessie{Unified definition: }Flat]\label{def:flat-general}
	A \emph{flat} $F \subseteq \P$ $\subseteq \reals^n$ is a nonempty set $F = \ker_\P(W) = \{p \in \P : \E_p W = \vec 0\}$ for some $W:\Y \to \reals^d$.
...
\end{definition}
\jessie{There's a bit of nuance about $W$ being a matrix vs function}
...
Theorem \ref{thm:cvx-flats} also gives a lower bound for any given target $\ell$ or $\gamma$, rather than a given surrogate $L$. \jessie{Does AA15 do this?  Seems out of the blue without that context.}
...
\begin{lemma}
	  \label{thm:convex-flats-inf-dim}\jessie{Edit to be about $\Gamma$}
	Let $\Gamma\in\C^*_d$ for some $d\in\mathbb{N}$.
	For all $u\in\range\Gamma$ and $p\in\Gamma_r$, there is some $V_{u,p}:\Y\to\reals^d$ such that $p \in \zeros{V_{u,p}} \subseteq \Gamma_r$.
\end{lemma}
\begin{proof}
	\jessie{Direct}
		Let $\Gamma := \prop{L}$.
		...
		Let $V_{u,p} : \Y \to \reals^d$ be the function mapping column indices of $W$ to columns of $W$, and taking their inner product with $p$.
		That is, $V_{u,p}(y) := W_y p$.
				
		
		\emph{Infinite $\Y$: }	
		$L \in \L^{inf}_d$ implies $L$ satisfies the assumptions of~\cite[Corollary 1]{rockafellar1982interchange}, we can interchange subdifferentiation and expectation to observe $\vec 0 \in \partial \E_p L(u,Y) \iff \vec 0 \in \E_p \partial L(u,Y) := \cl\{\int V(y)dp(y) : V \mathrm{msbl}, V(y) \in \partial L(u,y) a.s.\}$.
		We can now take $V_{u,p}$ to be such a $V: \Y \to \reals^d$, which exists as the subgradient is nonempty.
		
		
		In both cases, we take the flat $F := \zeros{V_{u,p}}$, and have $\codim(F) \leq d$ by Definition~\ref{def:flat}, and $p \in F$ by construction. \jessie{Flesh out}
		To see $F \subseteq \Gamma_u$, from the chain of equivalences above, we have for any $q\in\P$ that $q \in \zeros{V_{u,p}} \implies u \in \Gamma(q) \implies q \in \Gamma_u$.
\end{proof}

\begin{lemma}
	Let $\gamma$ be indirectly elicited by some $\Gamma\in\C^*_d$ for some $d\in\mathbb{N}$.
	For all $r\in\range\gamma$ and $p$ such that $|\gamma(p)| = 1$ with $\{r\} = \gamma(p)$, there is some $V_{u,p}:\Y\to\reals^d$ such that $p \in \zeros{\hat V_{u,p}} \subseteq \gamma_r$.
\end{lemma}

\begin{lemma}
	Let $\gamma$ be indirectly elicited by some $\Gamma\in\C^*_d$ for some $d\in\mathbb{N}$, and suppose $\gamma$ is elicitable on $\P$ defined over finite $\Y$.
	For all $r\in\range\gamma$ and $p \in \relint{\P}$ such that $p \in \gamma_r$, there is some $V_{u,p}:\Y\to\reals^d$ such that $p \in \zeros{V_{u,p}} \subseteq \gamma_r$.
\end{lemma}

\begin{corollary} \jessie{Indirect}
	Given target property $\gamma:\P \to\to \reals^d$, if there is some $p \in \P$ such that $|\gamma(p)| = 1$ with $\gamma(p) = \{r\}$, and there is not a flat $\zeros{\hat V_{u,p}} \subseteq \gamma_r$ of \Pcodimension at most $d$ then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\hat \Gamma$.
\end{corollary}

\begin{corollary} \jessie{Indirect}
	Given target property $\gamma:\P \to\to \reals^d$, where $\P$ is defined over a finite set of outcomes $\Y$, if there is some $p \in \relint{\P}$ and there is not a flat $\zeros{\hat V_{u,p}} \subseteq \gamma_r$ of \Pcodimension at most $d$ then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\gamma$.
\end{corollary}
...
Then if a convex $L: \reals^d \times \Y \to \reals$ \jessie{in $\L_d$} indirectly elicits $\gamma$, there is a flat $F$ with $p\in F \cap \simplex \subseteq \gamma_r$ and $\codim(F) \leq d$.
% COmmented 06.03.20
...
It turns out that the subspace of feasible directions is essentially a special case of a flat described by Theorem~\ref{thm:cvx-flats}.
