...
\raft{To discuss: add brief explanation for $\E_p$ notation, that $Y\sim p$?  Not sure if it's clear.}
We write $\L_d $ for the set of $\mathcal{B}(\reals^d) \otimes \Y$-measurable surrogates $L : \reals^d \times\Y \to \reals$ such that $\E_p L(u,Y) < \infty$ for all $u \in \reals^d, p \in \P$, and are minimizable, in the sense that $\argmin_{u} \exploss{L}{u}{p}$ is nonempty for all $p\in\P$.
...
\raft{To discuss next meeting: maybe it's better to say $\eliccvx(\gamma) > d$ instead of the indirectly elicits bit.  Both here and the next Cor.}
