...
\begin{definition}[Consistent: property]\label{def:consistent-prop}
  ...
  \end{equation}
  %where $D_x = \Pr[Y|X=x]$.
  Suppose there is a set $\D$ such that, for all $p \in \P$, there is an $x \in \X$ and $D \in \D$ with a point mass on $x$ such that $p = D_x$ and a $\propdis$ satisfying the requirements above.
...
If there is no $d$-flat $F$ with $p \in F \subseteq \gamma_r$, then no $L\in\Lcvx_d$ indirectly elicits $\gamma$\raft{To discuss next meeting: maybe it's better to say $\eliccvx(\gamma) > d$ instead of the indirectly elicits bit.  Both here and the next Cor.}, and in particular, there is no surrogate $L \in \Lcvx_d$ that is  consistent with respect to $\gamma$.
