...
Moreover, we write $\L_d = \L_d^{inf} \cap \L_d^{fin}$ and $\L = \cup_{d \in \N} \L_d$.
...
\begin{proof}
	\jessie{Direct}
		...
		\emph{Infinite $\Y$: }	
		$L \in \L^{inf}_d$ and convex \jessie{Need to be explicit in our definition of $\C^*$ to imply $L \in \L^{inf}_d$} implies $L$ satisfies the assumptions of~\cite[Corollary 1]{rockafellar1982interchange}, we can interchange subdifferentiation and expectation to observe $\vec 0 \in \partial \E_p L(u,Y) \iff \vec 0 \in \E_p \partial L(u,Y) := \cl\{\int V(y)dp(y) : V \mathrm{msbl}, V(y) \in \partial L(u,y) a.s.\}$.
...
\begin{corollary} 
	Given target property $\gamma:\P \toto \reals^d$, if there is some $p \in \P$ such that $|\gamma(p)| = 1$ with $\gamma(p) = \{r\}$, and there is not a flat $F \subseteq \gamma_r$ of \Pcodimension at most $d$ then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\hat \Gamma$.
\end{corollary}
...
\begin{corollary} 
	Given target property $\gamma:\P \toto \reals^d$, where $\P$ is defined over a finite set of outcomes $\Y$, if there is some $p \in \relint{\P}$ and there is not a flat $F \subseteq \gamma_r$ of \Pcodimension at most $d$ then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\gamma$.
\end{corollary}
...
By restricting slightly to the broad class of minimizable losses $\L$ \jessie{$\L^{fin}$}, we show their bound follows relatively directly from Theorem \ref{thm:cvx-flats}.
...
\begin{proof}[Proof sketch]
	...
	Theorem~\ref{thm:cvx-flats} guarantees the existence of a flat $F$ of dimension at least $n-d-1$ with $p \in F$ and $F \cap \simplex \subseteq \gamma_r$. \jessie{Need to be careful to relate \Pcodimension to dimension}
