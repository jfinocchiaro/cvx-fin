...
\begin{lemma}\label{lem:finite-relint-dim}
  ...
  \begin{enumerate}
  	...
  	\item[(ii)] $\Scr_F(p) = \spn(F_p)$ and $\dim(\Scr_F(p)) \geq \dim(\spn(\P_p)) - d$.
  \end{enumerate}
\end{lemma}
...
\begin{proof}
	...
	Taking $\P = \simplexp$, we know $p' \in \relint{\simplexp}$ by construction, so we can apply Lemma~\ref{lem:finite-relint-dim}(ii), which gives $\dim(\Scr_F(p')) \geq \dim(\spn(\P_{p'})) - d$.
	...
	\begin{align*}
	\dim(\Sc_{\gamma_r}(p)) \geq \dim(\Sc_{\gamma'_r}(p'))  &\geq \dim(F_{p'}) = \dim(\spn(F_{p'})) \geq \|p\|_0 - 1 - d~.~
	\end{align*}
%
