...
\begin{definition}[\jessie{Updated definition: }Flat]\label{def:flat-general}
	...
	%recover this later
	\btw{Jessie: Suggest delete}
	If $n$ is finite, the codimension of $F$ is given by $\codim(F)=\mathrm{rank}(W)$.
	If $n$ is infinite, consider $\codim(F) = \dim(\{p + F : p \in \P\}) \leq d$.
\end{definition}
...
\begin{corollary}\label{cor:Pcodim-flat-single-val-prop}
	Given target property $\gamma:\P \toto \R$, if there is some $p \in \P$ such that $|\gamma(p)| = 1$ with $\gamma(p) = \{r\}$, there is some flat $F$ of \Pcodimension at most $d$ such that $p \in F \subseteq \gamma_r$
	Moreover, if there is not a flat $F \subseteq \gamma_r$ of \Pcodimension at most $d$ then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\gamma$.
\end{corollary}
\begin{proof}
	...
	Now, if we do not have a flat satisfying the above conditions, then we do not have $\prop{L}$ indirectly eliciting $\gamma$, and in turn do not have a link such that $L$ is consistent with respect to $\gamma$ by Theorem~\ref{thm:consistent-implies-indir-elic}.
\end{proof}
...
\begin{corollary}\label{cor:Pcodim-flat-elic-relint-prop} 
	Given target property $\gamma:\P \toto \reals^d$, where $\P$ is defined over a finite set of outcomes $\Y$.
...
\begin{proof}
	...
	As above, if we do not have a flat satisfying the above conditions, then we do not have $\prop{L}$ indirectly eliciting $\gamma$, and in turn do not have a link such that $L$ is consistent with respect to $\gamma$ by Theorem~\ref{thm:consistent-implies-indir-elic}.
\end{proof}
...
\begin{proof}
	...
	We can observe that $Wp = \vec 0 \iff \E_p V = \vec 0$ \jessie{Check} by definition of expectation and its linearity, so $F = \{p \in \P : \E_p V = \vec 0\} = \{p \in \P : Wp = \vec 0\}$. 
...
\begin{restatable}[\cite{ramaswamy2016convex} Theorem 18]{corollary}{hariresult}\label{cor:fsd-bound}
	Let $\ell:\R \times \Y \to \reals$ be a discrete loss eliciting $\gamma$, and let $L \in \L_d$ be a minimizable consistent convex surrogate for $\ell$.
...
\begin{proof}[Proof sketch]
	...
	By Lemma~\ref{lem:Pcodim-codim-finite}, this implies the existence of a matrix $W$ such that $p \in \{q \in \P : Wq = \vec 0 \} = F \subseteq \gamma_r$ and $\codim(W) \leq d$.
	%dimension at least $n-d-1$ with $p \in F$ and $F \cap \simplex \subseteq \gamma_r$. 
	For $p \in \relint{\simplex}$, this implies that $\Sc_{\gamma_r}(p) \supseteq \{q \in \P : Wq = \vec 0\} - p$.
	Moreover, $\dim(\Sc_{\gamma_r}(p)) \geq n - d - 1$, completing the proof for $p \in \relint{\simplex}$.\jessie{This is a bit convoluted, but I didn't want to override the kernel notation}
...
However, consider any flat of \jessie{\Pcodimension} codimension $1$ (a line in the two dimensions; a plane in three) containing $p$.
...
Therefore, we have no flat of \jessie{\Pcodimension} codimension $1$ containing $p$ staying in $\gamma_\bot$, so we obtain better lower bound, $\ccdim(\ell^{abs}) \geq 3 - 0 - 1 = 2$.
...
\begin{minipage}{0.4\linewidth}
	...
	\caption{Any flat $F$ with $\codim(F) = 1$ \jessie{$\Pcodim(F) = 1$} through \textbullet \, also leaves the cell $\gamma_\bot$.\vspace*{-5pt} %However, for the distribution $p = (1/4, 1/4, 1/2)$, there is a $1$ dimensional flat containing $p$ that is a subseteq of $\gamma_\bot$ and $\gamma_3$.
	}
...
For continuous settings, it would be especially interesting to extend to the case $\Y=\reals$, which would require a more careful analysis in Theorem~\ref{thm:bayes-risk-lower-bound}.
...
\begin{lemma}\label{lem:feas-sub-is-a-flat}
	...
	If $F$ is a flat in $\spn(\P)$ containing $p$ such that $F \cap \simplex \subseteq \gamma_r$, then $F - p$ is a subspace contained in $\Sc_{\gamma_r}(p)$.
