...
Throughout, given a distribution $p$ and set $P$ \jessiet{not necessarily a subspace, since $F$ isn't a subspace, but $P$ not necessarily convex...} $P$, we defined $P_p := P - \{p\}$ and $\Scr_P(p) := \spn(P_p)$.
\jessiet{Get rid of this notation?}
\jessiet{One extra step to show $\Sc_F(p) = \spn(F_p)$ maybe?}
...
\begin{lemma}\label{lem:finite-relint-dim}
  ...
  \begin{enumerate}
  	...
  	\item[(ii)] $\dim(\Scr_F(p)) \geq \dim(\Scr_\P(p)) - d$.
  \end{enumerate}
\end{lemma}
\begin{proof}
  ...
  Define $T_W:\Scr_\P(p)\to\reals^d, v\mapsto \E_v W$.
...
%  
  ...
  As $\Y$ is a finite set, $\Scr_\P(p)$ is a finite-dimensional vector space.
  The rank-nullity theorem states $\dim(\im(T_W)) + \dim(\ker(T_W)) = \dim(\Scr_\P(p))$.
  As $\dim(\im(T_W)) \leq d$, and we have shown above that $\Scr_F(p) = \ker(T_W)$, the conclusion follows.\raft{Worth checking the diff for what I changed. Inequality was backwards :-)} \jessiet{Lol saw this comment in the diff before I pulled...}
  % 
...
\jessiet{Are you suggesting getting rid of this Lemma?  This is mostly important for project down to subsimplices of lower support.  The result isn't super obvious to me, but that could just be me. :shrugs:}
