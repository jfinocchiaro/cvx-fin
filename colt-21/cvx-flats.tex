...
\begin{abstract}
  ...
  The prediction task may be given as a target loss, as in problems like classification or structured prediction, or simply as a (conditional) statistic of the data, as in risk measure estimation.
  Techniques to design and analyze surrogate losses in these scenarios seem differ
...
In both of these settings, we minimally require \emph{consistency}, a precursor to excess risk bounds and rates.
Roughly speaking, consistency means that minimizing surrogate risk corresponds to solving the target problem of interest:
the target risk is also minimized, or the continuous prediction approaches the true conditional statistic.
...
(1) In structured prediction and extreme classification, the prediction dimension can easily become intractably large, and often one sacrafices consistency for lack of better surrogates.
...
In (1), we recover the feasible subspace dimension result of \citet{ramaswamy2016convex}, and give an example where our bound is even tighter.
In (2), we give the first prediction dimension bounds for risk measures with respect to convex surrogates, addressing the open question above.
Our framework rests on property elicitation, a weaker condition than calibration, as a tool to understand consistency across a wide variety of domains.

\raf{\hrule Old version below:}
...
We require surrogate losses to be \emph{consistent}, which informally means the surrogate loss ``corresponds'' to the desired learning problem, and this condition is a prerequisite for deriving excess risk bounds.
...
The characterizations and constructions of efficient surrogates for a given task would allow for us to easily test the consistency of a surrogate for a given task. \jessiet{Not quite what we want to say- can someone refine this?}
...
The dimensionality of this surrogate prediction space often scales linearly with the number of outcomes unless sacrifices are made in terms of consistency if one is not careful.
...
Tight bounds on the dimension of the prediction space for a consistent convex surrogate remain an open question, though we tighten currently known bounds in this work.
...
Reducing surrogate prediction dimension while maintaining consistency is important to improve the computational complexity of the learning algorithm, as it decreases the dimension of the ERM optimization problem.
Given a target loss or statistic, we provide a new framework for constructing lower bounds on this prediction dimension that supersedes previous results in the machine learning literature (Corollary~\ref{cor:fsd-bound}) and answers open questions of the elicitation literature (\cite{frongillo2020elicitation}). 
...
\jessiet{This paragraph a little redundant}
...
In prior work, most research on this problem focuses on the quadrant of target losses and discrete predictions (Quadrant 1)~\citep{zhang2004statistical,bartlett2006convexity,tewari2007consistency,ramaswamy2015hierarchical,ramaswamy2016convex,ramaswamy2018consistent}.
...
\jessiet{Proposed paragraph about calibration to replace \S 2.2}
As definitions of consistency are relatively intractable to apply directly, the literature often focuses on a weaker condition called calibration~\citep[Chapter3]{steinwart2008support}, which only applies when given a discrete target loss.
...
% vertices
\node[vertex] (consis-ell) at  (-4,3) {Consistency w.r.t. $\ell$};
\node[vertex] (consis-gam) at  (2,3) {Consistency w.r.t. $\gamma$};
\node[vertex] (indir-elic) at  (-4,-3) {Indirect elicitation};
\node[vertex] (d-flats) at  (2,-3) {$d$-flats};
\node[vertex] (conscvx-bd) at (8,0) {$\conscvx(\gamma) > d$};
\node[vertex] (q1-bounds) at (14,3) {$\conscvx$ bounds for Q1};
\node[vertex] (q4-bounds) at (14,-3) {$\conscvx$ bounds for Q4};
...
Moreover, $\Lcvx_d \subseteq \L_d$ is the set of convex losses in $\L_d$.
...
Typically, we notate finite report sets $\R$ and continuous prediction spaces $\reals^d$.
...
A property is \emph{single-valued} if $|\Gamma(p)|=1$ for all $p\in\P$, in which case we may write $\Gamma:\P\to\R$ and $\Gamma(p) \in \R$\raft{added, since sec 6 gets confusing without this single-arrow notation}.
...
\jessiet{I think it's okay to remove this paragraph completely.  We discuss \cite{agarwal2015consistent} other places.}
...
\begin{definition}[Convex Consistency Dimension]\label{def:cvx-consistency-dim}
  ...
  \raft{Why not use $\Lcvx$ here?  Then you don't need the qualifier below.}
%	\btw{Maybe take out of definition environment, but (a) I'd worry that people wouldn't be able to find the definition when we use it later, and (b) I'm hoping we can make the distinction very clear, which now is achieved by the parallel definitions}
\end{definition}
...
\begin{definition}[Consistent: loss]\label{def:consistent-ell}
  ...
  \raft{Touched up to clarify the quantification over $\D$}
\end{definition}
...
\begin{definition}[Consistent: property]\label{def:consistent-prop}
  ...
  \end{equation}
  % where $D_x = \Pr[Y|X=x]$.
  We simply say $(L,\psi)$ is \emph{$\mu$-consistent} if it is $(\mu,\D)$-consistent for some $\D$ satisfying the following: for all $p \in \P$, there exists $D \in \D$ and $x \in \X$ such that $D$ has a point mass on $x$ and $p = D_x$.
  \raft{Ditto; hopefully this is equivalent to what was here before (commented out below)}
...
\begin{lemma}\label{lem:consistent-loss-implies-prop}
	...
	\jessiet{Defer to appendix?  Thoughts?}
...
\end{theorem}
for all $p\in\P$, a $D\in\D$ with $\Pr_D[Y]=p$ being the marginal distribution on $\Y$.
...
\begin{proof}
  ...
  By definition, we have consistency with respect to some distribution $D$ on $\X\times\Y$ with a point mass on some $x\in\X$ and $D_x = p$.
  ...
  \raft{$u_m$ not defined; just cut out that term perhaps?}
...
In this case, the bound by feasible subspace dimension yields $\conscvx(\ell^{abs}) \geq 3 - 2 - 1 = 0$, as the feasible subspace dimension is $2$ since we are on the relative interior of the level set and simplex, as shown in Figure~\ref{fig:fsd-bound}.
...
\begin{minipage}{0.45\linewidth}
	...
	\caption{%Since the uniform distribution is in the relative interior of the simplex and the level set $\gamma_\bot$, 
          ...
          \raf{I had a feeling my comment wasn't 100\% clear -- fine/useful to have the color, but good to have the symbol in case of colorblindness/grayscale}
          \vspace*{-5pt}        }
	\label{fig:fsd-bound}
\end{minipage}
\hfill
