...
\begin{corollary}\label{cor:Pcodim-flat-single-val-prop}
  Let target property $\gamma:\P \toto \R$ be given.
  \raf{You are missing $d$ here, and the indirect elicitation via convex surrogate.}
  If there is some $p \in \P$ with $|\gamma(p)| = 1$, then letting $\gamma(p) = \{r\}$, there is some flat $F$ of \Pcodimension at most $d$ such that $p \in F \subseteq \gamma_r$.
  In particular, if there is no flat $F$ of \Pcodimension at most $d$ such that $p \in F \subseteq \gamma_r$, then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\gamma$.
\end{corollary}
\begin{proof}
...
\begin{corollary}\label{cor:Pcodim-flat-elic-relint-prop} 
  Given target property $\gamma:\P \toto \reals^d$, \raf{$\R$ right?} where $\P\subseteq\simplex$ for a finite set of outcomes $\Y$.
  \raf{Missing $d$ and ind elic again}
