...
\begin{definition}[Unified definition: Flat]\label{def:flat-general}
	A \emph{flat} $F \subseteq \P$ $\subseteq \reals^n$ is a nonempty set $F = \ker_\P(W) = \{p \in \P : \E_p W = \vec 0\}$ for some $W:C \to \reals^d$.\jessie{??} $W\in\reals^{d\times n}$ where $d\leq n$.
	If $n$ is finite, we assume $C = \reals^n$, in which case, the codimension of $F$ is given by $\codim(F)=\mathrm{rank}(W)$.
	If $n$ is infinite, we consider $C = \Y$, with $\codim(F) = \dim(\{p + F : p \in \P\}) \leq d$.
\end{definition}
...
\jessie{TODO: Introduce $\C^*_d$ notation}
\begin{lemma}
	  \label{thm:convex-flats-inf-dim}\jessie{Edit to be about $\Gamma$}
	Let $\hat\Gamma\in\C^*_d$ for some $d\in\mathbb{N}$.
	For all $r\in\range\hat\Gamma$ and $p\in\hat\Gamma_r$, there is some $V_{u,p}:\Y\to\reals^d$ such that $p \in \zeros{\hat V_{u,p}} \subseteq \Gamma_\psi(r)$.
\end{lemma}
\begin{proof}
	\jessie{Direct}
		Let $\Gamma := \prop{L}$ and suppose $\Gamma \in \mathcal{E}^*_d(\P)$ is refined by $\hat \Gamma$ via the link $\psi$.
		We know such a $\Gamma$ exists by definition of $\C^*_d(\P)$.
		For all $r \in \range(\hat \Gamma)$, we then have $\hat\Gamma_r \subseteq \Gamma_{\psi(r)}$.
		Let $u := \psi(r)$.
		...
		In both cases, we take $F := \zeros{V_{u,p}}$, and have $\codim(F) \leq d$ by Definition~\ref{def:flat}, and $p \in F$ by construction.
		To see $F \subseteq \hat \Gamma_u$, from the chain of equivalences above, we have for any $q\in\P$ that $q \in \zeros{V_{u,p}} \implies u \in \Gamma(q) \implies q \in \Gamma_u$.
\end{proof}

\begin{lemma}
	Let $\hat\Gamma\in\C^*_d$ for some $d\in\mathbb{N}$.
	For all $r\in\range\hat\Gamma$ and $p$ such that $\{r\} = \hat\Gamma(p)$, there is some $V_{u,p}:\Y\to\reals^d$ such that $p \in \zeros{\hat V_{u,p}} \subseteq \hat \Gamma_r$.
\end{lemma}

\begin{lemma}
	Let $\hat\Gamma\in\C^*_d$ for some $d\in\mathbb{N}$ and suppose $\hat \Gamma$ is elicitable and defined over finite $\Y$.
	For all $r\in\range\hat\Gamma$ and $p \in \relint(\P)$ such that $p \in \hat \Gamma_r$, there is some $V_{u,p}:\Y\to\reals^d$ such that $p \in \zeros{\hat V_{u,p}} \subseteq \hat \Gamma_r$.
\end{lemma}

\begin{corollary} \jessie{Indirect}
	Let $\hat \Gamma \in \C^*_d$ for some $d \in \mathbb{N}$.
	If, for some $p \in \P$, we observe $|\hat \Gamma(p)| = 1$, and there is not a flat $\zeros{\hat V_{u,p}}$ of codimension at most $d$ then there is no $d$-dimensional convex surrogate $L$ consistent with respect to $\hat \Gamma$.
\end{corollary}

\begin{corollary} \jessie{Indirect}
	Let $\hat \Gamma \in \C^*_d$ for some $d \in \mathbb{N}$ and $\hat \Gamma$ be (directly) elicitable with $|\Y| < \infty$.
	If, for some $p \in \relint(\P)$, we observe there is no flat $\zeros{\hat V_{u,p}}$ of codimension at most $d$, then there is a $d$-dimensional convex surrogate $L$ consistent with respect to $\hat \Gamma$.
\end{corollary}
