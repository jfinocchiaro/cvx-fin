...
\end{lemma}
\begin{proof}
  \raf{Moved; see margin comment:  for the function $W : \Y \to \reals^d$ such that $F = \zeros{W}$.
    (Such a function must exist by $F$ being a $d$-flat.)}
  
  (i)
  Since $p \in \relint{\P}$, there is some small enough $\epsilon > 0 $ so that for $q \in \P$\raft{You want the quantification order of $\epsilon$ and $q$ swapped I think; also, add ``for all'' for clarity}, $\alpha \in (-\epsilon,\epsilon)$, the point $q_\alpha := p - \alpha(q - p)$ is still in $\P$.
  ...
  As $p,q \in F$, we have $\E_pV = \E_qV = \vec 0$.
  By linearity of expectation, we then have $\E_{q_\alpha} V = \vec 0$.
  % (Y) = \E_p V(Y) - \alpha(\E_q V(Y) - \E_p V(Y)) = \vec 0 - \alpha(\vec 0 - \vec 0) = \vec 0$.
  This implies $q_\alpha \in F$, and therefore $p \in \relint{F}$.
  
  (ii)
  Define $F_p := F - \{p\}$, $\P_p := \P - \{p\}$ and $T_W:\spn(\P_p)\to\reals^d, v\mapsto \E_v W$.
  We first show $\spn(F_p) = \ker(T_W)$.
  ...
  As $\dim(\im(T_W)) \leq d$, and we have shown above that $\spn(F_p) = \ker(T_W)$, the conclusion follows.\raft{Worth checking the diff for what I changed. Inequality was backwards :-)}
  % 
...
\end{lemma}
\begin{proof}
	Construct $F$ as in Theorem~\ref{thm:convex-flats-inf-dim}\raft{Lemma?}; $F$ is a $d$-flat, contains $p$, and we have $F \subseteq \gamma_r$.
...
\begin{proof}
	...
	Taking $\P = \simplexp$ in Lemma~\ref{lem:finite-relint-dim}(ii) gives $\dim(\spn(F_p)) \geq \|p\|_0 - 1 - d$\raft{Might be nicer on the reader to add one more step for $\|p\|_0$. Also, I just realized we are using $p\in\simplex$ here, but then $p$ can't be in $F\subseteq\Delta_{\Y'}$ ($p$ is an $n$-vector). So maybe we need a $p'$ too.}.
	Moreover, $F_p \subseteq \Sc_{\gamma'_r}(p)$ since $p \in \relint{F}$ by Lemma~\ref{lem:finite-relint-dim} part (3.)\raft{update ref}.
