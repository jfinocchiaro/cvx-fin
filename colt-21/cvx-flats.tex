...
\begin{proof}
	\jessie{Direct}
		...
		\emph{Infinite $\Y$: }	
		$L \in \L_d$ and convex implies $L$ satisfies the assumptions of~\cite[Corollary 1]{rockafellar1982interchange}, we can interchange subdifferentiation and expectation to observe $\partial \E_p L(u,Y) = \E_p \partial L(u,Y) := \cl\{\int V(y)dp(y) : V \mathrm{measurable}, V(y) \in \partial L(u,y) \textrm{a.s. in } p\}$.\jessie{Cite~\cite{ioffe1969minimization} instead; they don't use closure and it's the same result.}
		...
		\jessie{What happens if $\vec 0$ not in the set, but is in the closure}
...
\begin{proof}
	...
	\jessie{TODO: Pull in consistency}
\end{proof}
...
\begin{corollary} 
	...
	For all $p \in \relint{\P}$, there is some flat $F$ \jessie{of \Pcodimension at most $d$} such that $p \in F \subseteq \gamma_r$.
...
\begin{proof}
	...
	\jessie{TODO: Add in consistency bit}
\end{proof}
