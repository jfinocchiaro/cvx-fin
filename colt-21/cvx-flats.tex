...
\node[draw=none] at (3,3.5) {{\Huge$\Lcvx$}};
\node[draw=none] at (-3,3.5) {{\Huge$\L$}};
...
\end{center}
\caption{Flow and implications of results \jessie{Change}\raf{Suggested tweaks: larger font, lower vertical distance, newlines as needed (see what I did in Bartlett et al.\ to make that happen)}}
...
We consider supervised learning problems in the space $\X \times \Y$, for some \emph{feature space} $\X$ and a \emph{label space} $\Y$, with data drawn from a distribution $D$ over $\X \times \Y$\raft{Jessie, FYI, I cut $\Y$ ``of size $n$'' since that was a relic of the past.  Worth checking if you use $n$ without explanation anywhere.}
