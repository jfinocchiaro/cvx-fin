...
\tikzset{vertex/.style = {ellipse,draw,minimum width=10em, fill = white, font=\Large}}
...
\draw[dashed, opacity = 0.3] (0,-3) -- (0,3);
...
\node[vertex, text width=45mm] (q4-bounds) at (12,-2) {$\conscvx$ bounds for Q3,4. (Theorem~\ref{thm:bayes-risk-lower-bound})};
...
\end{center}
\caption{Flow and implications of results \jessie{Change}\raf{Suggested tweaks: larger font, lower vertical distance, newlines as needed (see what I did in Bartlett et al.\ to make that happen)} \jessie{Attempted. Feel free to modify from here.}}
...
\begin{definition}[Convex Consistency Dimension]\label{def:cvx-consistency-dim}
  Given target loss $\ell:\R \times\Y \to \reals$ or statistic $\gamma: \P \toto \R$, its \emph{convex consistency dimension} $\conscvx(\cdot)$ is the minimum dimension $d$ such that $\exists L \in \Lcvx_d$ and link $\psi$ such that $(L,\psi)$ is consistent with respect to $\ell$ or $\gamma$.
%	\btw{Maybe take out of definition environment, but (a) I'd worry that people wouldn't be able to find the definition when we use it later, and (b) I'm hoping we can make the distinction very clear, which now is achieved by the parallel definitions}
\end{definition}
...
\begin{lemma}
	...
	When $\Y = \reals$, we assume each $p\in\P$ admits a Lebesgue density supported on the same set for all $p\in\P$.%
        \footnote{\raf{This assumption is for technical convenience...}}
...
\begin{proof}
  ...
  We proceed in two cases, depending on $|\Y|$. \jessiet{Seems really pedantic, but do we need to specify $\Y$ finite or $\Y = \reals$?  Since, for example, $\Y = \mathbb{Z}$ doesn't fit in either case, but isn't ruled out by our Lemma statement.}
  ...
  \jessiet{Fleshing this out: $\exists E \subseteq \mathcal{B}(\Y)$ so that $p(E) = 0$ and $V(y) \in \partial L(u,y) \forall y \in E^c$.  Claim $q(E) = 0$, which would complete the claim that $V(y) \in \partial L(u,y) q$-a.s.  For contradiction, if $q(E) > 0$, then $\exists x \in E$ and open neighborhood $N_x \ni x$ with $q(N_x) > 0$.  Since $\supp(p) = \supp(q)$, we then have $p(N_x) >0$, contradicting $p(E) = 0$, and hence $V(y) \in \partial L(u,y)$ not $p$-a.s.  Therefore, we must have $q(E) = 0$, so $V(y) \in \partial L(u,y) q$-a.s.}
  For any $q\in\P$, as by assumption $q$ is supported on the same set as $p$, we have $V_{u,p}(y)\in\partial L(u,y)$ $q$-a.s., so that $V_{u,p}\in\V_{u,q}$\raft{I'm a bit too tired to see exactly how this follows, but it seems intuitively obvious. I'm okay to punt on it for the submission, but if someone has time to check, that would be great! \jessie{See above margin comment.  I think it's too much detail for the conference submission, but feel free to change my mind.}}.
  Thus, $\E_q V_{u,p} = 0$ implies $0\in\partial\E_q L(u,Y)$ by the above.
  \raft{For posterity, the bug: we want to conclude $\E_q V_{u,p} = 0 \implies q \in \Gamma_u$.  But the ``a.s.\ in $p$'' means we may not have $0 \in \partial \E_q L(u,Y)$, since maybe $V_{u,p}(y)$ is not in $\partial L(u,y)$ a.s.\ in $q$...}
...
%	Moreover, since $p \in \relint{\simplex}$, the flat $F$ satisfies the requirements of Lemma~\ref{lem:finite-relint-dim}, and the bound $\dim(\Sc_{\gamma_r}(p)) \geq \dim(\Scr_F(p)) \geq n - d- 1$ is given by Lemma~\ref{lem:finite-relint-dim} (ii).
        \raft{should give intuition instead of (or in addition to) lemmas, since that's more the point of a sketch \jessie{Good point... taking a pass below. Commented out original first paragraph above.}}
        
        \jessiet{Adding an attempt at first paragraph of sketch below, but also my brain isn't functioning.}
    If $\conscvx(\gamma) \geq d$, then there is a $L \in \Lcvx_d$ so that $L$ is consistent with respect to $\gamma$, and in turn, indirectly elicits $\gamma$.
    Lemma~\ref{lem:convex-flats-inf-dim} says that there is some $d$-flat $F = \zeros{V}$ contained in $\gamma_r$.
    In particular, we can verify $\Sc_{\gamma_r}(p) + \{p\}$ is one such $d$-flat.
    If $p \in \relint{\simplex}$, then the feasible subspace  (shifted by $p$) spans\jessiet{not the word I want...} $\relint{\simplex}$, and we can observe that the shifted feasible subspace is full dimensional relative to $\affhull(\simplex)$.
    We then use the rank-nullity theorem with a bound on $\rank(V) \leq d$ to obtain the bound $d \geq n-1-\dim(\Sc_{\gamma_r(p)})$.
...
%	Moreover, this implies $\dim(\Sc_{\gamma_r}(p)) \geq \codim(W) = n - d - 1$, completing the proof for $p \in \relint{\simplex}$.
	...
	Now $p$ is in the relative interior of this subsimplex, so the above result gives $\conscvx(\gamma) \geq \|p\|_0 - 1 - \dim(\Sc_{\gamma_r}(p))$, where the feasible subspace dimension is relative to $\reals^{\supp(p)}$.
...
It's worth noting that bounds provided by $d$-flats appear to work well at distributions complimentary to those providing previous bounds via feasible subspace dimension.
In some rough sense, this is because flats allow us a ``global'' view of the property we are eliciting, while the feasible subspace method only permits a ``local'' look at the property, so we find our method works better for distributions in $\relint\simplex$.
\jessie{Can someone check this paragraph?}
...
Furthermore, we introduce a new lower bound (Corollaries~\ref{cor:Pcodim-flat-single-val-prop} and~\ref{cor:Pcodim-flat-elic-relint-prop}) on convex consistency dimension that is generally applicable and extends previous results from both the discrete (Corollary~\ref{cor:fsd-bound}) and continuous (Corollaries~\ref{cor:variance} and \ref{cor:spectral-risks}) estimation settings.
...
\hariresult*
\begin{proof}
  Let $L \in \Lcvx_d$ elicit $\Gamma$ be a calibrated surrogate for $\ell$, and consider $\Y^{\proj(q)} := \supp(q)$ for any $q \in \simplex$ and what happens when we restrict $L$ and $\ell$ to only the outcomes in $\Y^{\proj(q)}$.
