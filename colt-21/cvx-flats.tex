...
In the case where we are given a \emph{target loss} $\ell$, we roughly define $L$ to be consistent if minimizing $L$, and applying a link function, minimizes $\ell$ (Definition \ref{def:consistent-ell})~\citep{zhang2004statistical,bartlett2006convexity,tewari2007consistency,steinwart2007compare,ramaswamy2016convex}.
In Definition \ref{def:consistent-prop}, we introduce a notion of consistency for the case of a \emph{target statistic}, such as the conditional variance or quantile, as in classical statistics~\citep{gyorfi2006distribution, fan1998efficient,ruppert1997local}.
...
\begin{table}
	...
	\begin{tabular}{p{12ex}|l|l|} 
		...
		{\em prediction} & & (Appendix~\ref{app:omitted-examples})\jessie{Discretized variance?}\\ \hline 
...
\end{center}
\caption{Flow and implications of results. Compared to calibration, we suggest that indirect elicitation is a simpler but almost-as-powerful necessary condition for consistency. In particular, for convex surrogates $\Lcvx$, we obtain a testable necessary condition, $d$-flats, for whether there exists a $d$-dimensional consistent convex surrogate. \jessie{Change}\raf{Suggested tweaks: larger font, lower vertical distance, newlines as needed (see what I did in Bartlett et al.\ to make that happen)} \jessie{Attempted. Feel free to modify from here.}}
...
\begin{lemma}
	...
	When $\Y = \reals$, we assume each $p\in\P$ admits a Lebesgue density supported on the same set for all $p\in\P$.%
        \footnote{This assumption is for technical convenience, though we suspect it is strictly stronger than necessary.\raf{This assumption is for technical convenience...}}
...
\begin{proof}
  ...
  \emph{$\Y=\reals$: }
  \jessiet{Note: this should work for any infinite $\Y$ so long as you have a good $\sigma$-algebra... working at least with $\Y \subseteq \reals$ just makes things easier since we can take the Borel $\sigma$-algebra for granted.}
...
%	Moreover, this implies $\dim(\Sc_{\gamma_r}(p)) \geq \codim(W) = n - d - 1$, completing the proof for $p \in \relint{\simplex}$.
	...
	When $p \not \in \relint{\simplex}$, we can ``project down'' to the subsimplex on the support of $p$ and modify $L$ and $\ell$ accordingly.
...
\jessiet{Can someone check this paragraph?}
