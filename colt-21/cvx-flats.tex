...
A surrogate loss function is an error measure that that is related but not identical to one's target problem of interest.
...
There are two main reasons why a surrogate loss is necessary: (1) the target loss does not satisfy some desiderata, such as convexity, or (2) the goal is to estimate some target statistic and there is no target loss, as in many continuous estimation problems.
...
Roughly speaking, consistency means that minimizing surrogate risk corresponds to solving the target problem of interest, i.e. in (1) the target risk is also minimized, or in (2) the continuous prediction approaches the true conditional statistic.
...
In many type (1) settings, such as structured prediction and extreme classification, the prediction dimension can easily become intractably large, and often one sacrifices consistency for lack of better surrogates.
...
Challenges of type (2) include risk measures such as conditional value at risk (CVaR).
These have applications in financial regulation, robust engineering design, and algorithmic fairness, yet provably cannot be specified via a target loss.
These applications therefore require a surrogate loss, and preferrably one of low (or at least finite) prediction dimension.
Recent work~\citep{fissler2016higher,frongillo2020elicitation} gives prediction dimension bounds for some of these risk measures, but without the requirement that the surrogate be convex: bounds for convex surrogates are a major open question.

We present a unification of existing techniques to bound the prediction dimension of consistent convex surrogates in both settings above.
...
For type (2), we give the first prediction dimension bounds for risk measures with respect to convex surrogates, addressing the open question above.
Our framework rests on \emph{property elicitation}, a weaker condition than calibration, as a tool to understand consistency across a wide variety of domains.

\raft{two bits saved here (commented out): ad-hoc-ness of surrogate design, and BEP success story}
...
For further examples, see Appendix~\ref{app:omitted-examples}.\jessiet{Don't actually have any Q3 examples in there at the moment}
...
We focus on the construction of consistent surrogate losses $L: \reals^d \times \Y \to \reals$, roughly meaning that minimizing $L$-loss corresponds to solving the target problem of interest.
Typically, we are given a \emph{target loss} $\ell$, and we roughly define $L$ to be consistent if minimizing $L$ and applying a link, minimizes $\ell$ (Definition \ref{def:consistent-ell})~\citep{zhang2004statistical,bartlett2006convexity,tewari2007consistency,steinwart2007compare,ramaswamy2016convex}.
We introduce a notion of consistency in which one is given a \emph{target statistic}, such as the conditional variance or quantile, as in classical statistics, but no target loss~\citep{gyorfi2006distribution, fan1998efficient,ruppert1997local}.
Here we will define $L$ to be consistent if minimizing $L$ and applying a link function yields estimates converging to the correct value (Definition \ref{def:consistent-prop}).


\jessiet{This paragraph feels a little redundant}
...
We observe that, in fact, target losses are a special case of target statistics (\S~\ref{sec:consis-implies-indir}), which suggests property elicitation (see \S~\ref{subsec:properties}) is well-suited to study general lower bounds.
Most prior work on convex consistency dimension focuses on the quadrant of target losses and discrete predictions (Quadrant 1)~\citep{zhang2004statistical,bartlett2006convexity,tewari2007consistency,ramaswamy2015hierarchical,ramaswamy2016convex,ramaswamy2018consistent}.

As definitions of consistency are relatively intractable to apply directly, the literature often focuses on a weaker condition called calibration, which only applies when given a discrete target loss, e.g. Quadrants 1 and 3. 
Particularly,~\citet{zhang2004statistical, lin2004note,bartlett2006convexity,tewari2007consistency,ramaswamy2016convex} show the equivalence of consistency and calibration in the Quadrant 1, where one is given a target statistic and discrete prediction set.
...
\tikzset{vertex/.style = {ellipse,draw,minimum width=10em, fill = white}}
\tikzset{edge/.style = {->,> = triangle 45, thick}}
\tikzset{node/.style = {anchor=above, sloped}}

\path[fill=blue, fill opacity = 0.1] (0,-4) -- (0,4) -- (14,4) -- (14,-4) -- cycle;
\draw[dashed, opacity = 0.4] (0,-4) -- (0,4);
\node[draw] at (3,3.5) {{\Huge$\Lcvx$}};
\node[draw] at (-3,3.5) {{\Huge$\L$}};

% vertices
\node[vertex] (consistent) at  (-9,3) {Consistency};
\node[vertex] (calibrated) at  (-9,-3) {Calibration (Q 1,3)};
\node[vertex] (indir-elic) at  (-3,0) {Indirect elicitation};


\node[vertex] (d-flats) at  (3,0) {$d$-flats in level sets};
\node[vertex] (q1-bounds) at (9,3) {$\conscvx$ bounds for Q1,2- Cor.~\ref{ref}};
\node[vertex] (q4-bounds) at (9,-3) {$\conscvx$ bounds for Q3,4- Theorem~\ref{ref}};

%edges
\draw[edge, <->] (consistent) to node[above,sloped]{\cite{bartlett2006convexity}} (calibrated);
...
\end{center}
\caption{Flow and implications of results \jessie{Change}}
\label{fig:results-flow}
\end{figure}

\paragraph{Contributions}
\jessiet{Thoughts on this vs bullet points?}
...
Finally, we use this framework to derive new bounds for well-studied problems such as abstain loss (\S~\ref{sec:finite-calib}) and variance, entropy, and norms (\S~\ref{sec:contin-consis}).
In Figure~\ref{fig:results-flow}, we show the implications of some of our main results.

\section{Preliminaries \jessie{Setting?}}\label{sec:related-work}
...
As we focus heavily on conditional distributions $p := D_x = \Pr[Y|X=x]$ over $\Y$ given some $x \in X$, we often abstract away $x$, working directly with a convex set of distributions over outcomes $\P \subseteq \simplex$, through tools such as property elicitation.
When we have $p$ over outcomes $\Y$, we often write the expectation of a function $\E_{Y \sim p} L(\cdot,Y) := \E_p L(\cdot,Y)$.
...
\raft{To discuss: add brief explanation for $\E_p$ notation, that $Y\sim p$?  Not sure if it's clear. \jessie{Added line in prev. paragraph- feel free to burn if happy.}}
We write $\L_d $ for the set of $\mathcal{B}(\reals^d) \otimes \Y$-measurable and lower semi-continuous surrogates $L : \reals^d \times\Y \to \reals$ such that $\E_{Y \sim p} L(u,Y) < \infty$ for all $u \in \reals^d, p \in \P$, and are minimizable, in the sense that $\argmin_{u} \exploss{L}{u}{p}$ is nonempty for all $p\in\P$.
...
\bo{Should give at least one example? Squared loss?}
...
\raf{Check me!}\jessie{<< From NeurIPS.  I'm good with this.  If we wanted to trim, we can probably remove mentions of rays and adding $\epsilon$.}
...
\begin{definition}[Convex Consistency Dimension]\label{def:cvx-consistency-dim}
  Given a target loss $\ell:\R \times\Y \to \reals$ or statistic $\gamma: \P \toto \R$, its \emph{convex consistency dimension} $\conscvx(\cdot)$ is the minimum dimension $d$ such that there is a $L \in \Lcvx_d$ and link $\psi$ such that $(L,\psi)$ is consistent with respect to $\ell$ or $\gamma$.
%	\btw{Maybe take out of definition environment, but (a) I'd worry that people wouldn't be able to find the definition when we use it later, and (b) I'm hoping we can make the distinction very clear, which now is achieved by the parallel definitions}
\end{definition}
A priori, this definition does not really make sense given a target statistic.
In Definition~\ref{def:consistent-prop}, we formalize consistency with respect to a property so that we can study $\conscvx(\gamma)$. \jessiet{Do we want to just talk about $\ell$ here?}\raft{I didn't think about the target property vs target loss point.  Let's discuss.}
...
We show indirect elicitation gives state-of-the-art lower bounds on the prediction dimension of consistent convex surrogates.
...
\begin{definition}[Consistent: loss]\label{def:consistent-ell}
  A loss and link $(L,\psi)$ are \emph{$\D$-consistent} for a set $\D$ of distributions over $\X \times \Y$ with respect to a target loss $\ell$ if, for all $D \in \D$ and all sequences of measurable hypothesis functions $\{f_m : \X \to \R\}$,
  \begin{align*}
...
\begin{definition}[Consistent: property]\label{def:consistent-prop}
	Suppose we are given a loss $L \in \L_d$, link function $\psi: \reals^d \to \R$, and property $\gamma:\P \toto \R$.
  ...
  \raft{Ditto; hopefully this is equivalent to what was here before (commented out below)\jessie{Looks good to me \#burn}}
...
\begin{lemma}
	...
	When $\Y = \reals$, we assume $\P$ is a set of Lebesgue densities supported on the same set for all $p \in \P$.
...
\begin{proof}
		...
		Any $L \in \Lcvx_d$ satisfies the assumptions of~\cite{ioffe1969minimization}, so we may interchange subdifferentiation and expectation.
                Specifically, letting $\V_{u,p} = \{V:\Y\to\reals^d \mid V \text{ measurable}, V(y) \in \partial L(u,y) \text{ a.s.\ in } p\}$, we have
		...
		% \jessie{What happens if $\vec 0$ not in the set, but is in the closure}\jessie{Cite~\cite{ioffe1969minimization} instead; they don't use closure and it's the same result. (Instead of ~\cite[Cor 1]{rockafellar1982interchange})}
                ...
                \raft{Rewrote.  But I see a problem, hopefully not a big one: we want to conclude $\E_q V_{u,p} = 0 \implies q \in \Gamma_u$.  But the ``a.s.\ in $p$'' means we may not have $0 \in \partial \E_q L(u,Y)$, since maybe $V_{u,p}(y)$ is not in $\partial L(u,y)$ a.s.\ in $q$...}
		...
		\jessie{Footnote about how we can probably lessen the assumption on $\P$}
\end{proof}
...
If there is no $d$-flat $F$ with $p \in F \subseteq \gamma_r$, then $\eliccvx(\gamma) \geq d + 1$, 
%no $L\in\Lcvx_d$ indirectly elicits $\gamma$
\raft{To discuss next meeting: maybe it's better to say $\eliccvx(\gamma) > d$ instead of the indirectly elicits bit.  Both here and the next Cor.\jessie{Changed.  I don't have strong opinions here, but lean towards $\eliccvx$ for the sake of unifying narrative.  Feel free to burn.}}
...
\begin{proof}
	...
	Thus, we have $p \in F \subseteq \gamma_r$.
        ...
        If no flat $F$ satisfies the above conditions, then no $L\in\Lcvx_d$ indirectly elicits $\gamma$, and hence no such that $L$ is consistent with respect to $\gamma$ by Theorem~\ref{thm:consistent-implies-indir-elic}.
\end{proof}
...
\begin{proof}[Proof sketch]
	...
	Moreover, since $p \in \relint{\simplex}$, the flat $F$ satisfies the requirements of Lemma~\ref{lem:finite-relint-dim}, and the bound $\dim(\Sc_{\gamma_r}(p)) \geq \dim(\Scr_F(p)) \geq n - d- 1$ is given by Lemma~\ref{lem:finite-relint-dim} (ii).
        \raft{should give intuition instead of (or in addition to) lemmas, since that's more the point of a sketch \jessie{Good point... taking a pass below}}
        
        \jessie{Adding an attempt at sketch below, but also my brain isn't functioning.}
    If $\conscvx(\gamma) = d$, then there is a $L \in \Lcvx_d$ so that $L$ is consistent with respect to $\gamma$, and in turn, indirectly elicits $\gamma$.
    Lemma~\ref{lem:convex-flats-inf-dim} says that there should be some $d$-flat $F = \zeros{V}$ contained in $\gamma_r$.
    In particular, we can verify $\Sc_{\gamma_r}(p) + \{p\}$ is one such $d$-flat.
    If $p \in \relint{\simplex}$, then the feasible subspace  (shifted by $p$) spans\jessiet{not the word I want...} $\relint{\simplex}$, and in particular, we can observe that the shifted feasible subspace is full dimensional relative to $\affhull(\simplex)$.
    We then use the rank-nullity theorem with a bound on $\rank(V) \leq d$ to obtain the bound $d \geq n-1-d$.
...
\begin{figure}[htb]
	...
	\caption{(Left) Feasible subspace dimension $\Sc_{\gamma_\bot}(\bullet) = 2$ and $\Sc_{\gamma_\bot}({\color{blue}\star}) = 1$, giving the bound $\conscvx(\ell^{abs}) \geq 3- 1-1 = 1$.
          (Right) No $1$-flat through $\bullet$ (a line since $\bullet \in \relint{\simplex})$  stays fully contained in $\gamma_\bot$, so $\conscvx(\ell^{abs}) > 1$.}
...
But a $d$-dimensional identifiable property's level sets are all precisely $d$-flats, so this yields a lower bound.

However, properties elicited by non-smooth convex losses are generally not identifiable.
For example, the properties elicited by hinge loss and the abstain surrogate are not identifiable, as their level sets are not flats (see Figure~\ref{fig:fsd-flats-abstain}). % (L), we can see that any affine space containing a level set of the abstain property would contain the entire simplex.
It might appear that entirely new ideas are needed.
But in fact, our framework for such properties is closely related to ~\citet{frongillo2020elicitation}.
Lemma~\ref{lem:convex-flats-inf-dim} states that the level sets of $d$-dimensional convex-elicitable properties are unions of $d$-flats.
...
\begin{condition}\label{cond:v-interior}
  For some $r\in\range\Gamma$, the level set $\Gamma_r = \zeros{V}$ is a $d$-flat presented by some $V:\Y\to\reals^d$ such that \raft{I think we only need $\Gamma_r \subseteq \zeros{V}$.  I think I'll update the proof at some point to reflect this, since it actually gets a bit easier to follow.} $0\in\interior\{\E_pV : p\in\P\}$.
\end{condition}
...
\begin{restatable}{theorem}{bayesrisklowerbound}\label{thm:bayes-risk-lower-bound}\btw{Old version for finite $\Y$ is in the NeurIPS 2020 submission; especially lots more detail on variance / norm / entropy examples}
  ...
  \raft{Chain $\conscvx(\cdot)$ result}
\end{restatable}
...
This lower bound will follow from Theorem~\ref{thm:bayes-risk-lower-bound} using that variance is the Bayes risk of squared loss $L(r,y) = (r-y)^2$, which elicits the mean $\Gamma(p) = \E_p Y$.
...
\begin{corollary}
  \label{cor:variance}
  Let $\P\subseteq\simplex$ for $\Y = \reals$, such that $\P$ is a set of Lebesgue densities with all $p \in \P$ having the same support, and there exists $p,q,q'\in\P$ with $\E_p Y = \E_q Y \neq \E_{q'} Y$ and $\Var(p) \neq \Var(q)$.
...
We conjecture $\eliccvx(\CVaR_\alpha) \geq 3$, which would be an interesting gap between elicitation complexity for identifiable and convex-elicitable properties.
...
\begin{proof}
  ...
  \raft{I'm not getting the $\proj(q)$ stuff.  It would help if you defined $\proj(q)$, but even so, I suspect there is a more direct approach.  E.g. you define all these $\proj(q)$ versions and then just take the primed versions to be equal.  I'd suggest starting from scratch and thinking through what exactly you need.}
  Take $L^{\proj(q)} := L|_{\Y^{\proj(q)}}$ and $\ell^{\proj(q)} := \ell|_{\Y^{\proj(q)}}$.
  Observe that for all $q \in \simplex$, we have $\gamma(q) = \gamma^{\proj(q)}(B(q))$.
...
\subsection{Proving the lower bound for spectral risks}
...
Then for all properties $\gamma$, if $\elic_{\C^*}(\gamma) < \infty$, we have $\elic_{\C^*}(\gamma) = \eliccvx(\gamma)$, a fact we use tacitly in the proof.\raft{When we add the stuff on entropy and norms, we'll need to actually define $\Lcvx_{\infty}$, which is a whole other can of worms... but totally fine actually.  Fun times for later :-]}
...
\paragraph{Discrete problem with no target loss (Quadrant 2).}
