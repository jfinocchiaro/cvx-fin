...
\begin{lemma}\label{lem:finite-relint-dim}
  Let flat $F\subseteq \P$ contain some $p\in\relint{\P}$.
  Then $p \in \relint{F}$ and \jessie{Originally $\Pcodim(F) = \codim(F)$? Also; $F$ vs $F'$?}.
\end{lemma}
\begin{proof}
	...
	When we substitute $P = \simplex$, this yields the bound $\dim(\spn(F')) \geq n - 1 - d$.
\end{proof}
...
\begin{lemma}\label{lem:p-boundary-fsd}
	...
	Define $\gamma' : \P \toto \R$ with $\gamma' : q \mapsto \gamma(q) \cap \simplexp \cap \P$.
...
\begin{proof}
	...
	The first case can be seen easily by the definition of $\gamma'$, and the latter can be seen because leaving $\simplexp \cap \P$ means one of $q^\pm$ also is not in $\P$, and therefore not in $\gamma_r$ as $\gamma_r \subseteq \P$.
...
\begin{proof}
	...
	As $L'$ is calibrated with respect to $\ell'$ and indirectly elicits $\gamma' := \prop{\ell'}$, then by Corollary~\ref{cor:Pcodim-flat-elic-relint-prop}, we know there exists a flat with $p \in F' \subseteq \gamma'_r$ \jessie{NOTATION- this $F'$ isn't $F - \{p\}$} of \Pcodimension at most $d$.\jessie{To my understanding, we still don't relate \Pcodimension to dimension? Or did I just miss that.}
	Moreover, as $d$ is the maximal codimension of such a flat, we show in Lemma~\ref{lem:feas-sub-is-a-flat} that $\Sc_{\gamma_r}(p)$ is one such flat, and we have $\dim(\Sc_{\gamma_r}(p)) \geq \dim(F - \{p\}) \geq \|p\|_0 - d- 1$ by Lemma~\jessie{??}.
