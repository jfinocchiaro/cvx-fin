...
\title{A Unification of Lower Bounds on Prediction Dimension of Consistent Convex Surrogates}
...
Surrogate risk minimization is one of the most widespread optimization heuristics in supervised machine learning.
...
the target risk should also be minimized, or the average deviation from the true conditional statistic function converges to 0.\jessie{Not sure if I like these sentences here, or in general}
...
This linear growth of prediction dimension (in the number of outcomes) leads to computationally expensive or intractable optimization problems, but it is unknown when it is possible to design a surrogate in a lower-dimensional prediction space for many tasks that have large outcome spaces.
...
\citet{ramaswamy2016convex} present a convex surrogate, called the BEP loss, for the abstain target loss that takes as input a prediction logarithmic in the number of outcomes.
...
As we focus heavily on conditional distributions $p := D_x = \Pr[Y|X=x]$ over $\Y$ given some $x \in X$, we often abstract away $x$, working directly with distributions over outcomes $\P \subseteq \simplex$, through tools such as calibration~\cite[Chapter 3]{steinwart2008support}.
...
\jessie{Sufficient condition for $\A$-normal convex integrand: $L$ is lower semi-continuous and $\mathcal{B}(\A) \otimes \Y$-measurable; $\E_p L(u,Y)$ finite for all $u, p$, there exists a $u_0$ for each $p$ so that $\E_p L(u_0, Y)$ is finite and continuous for Rockefellar's corollary, though it's stricter than Ioffe and Tokhimorov IIRC}
...
A surrogate $L:\U \times \Y \to \reals$ is \emph{convex} if $L(\cdot,y)$ is convex in $u$ almost surely in $\Y$.
...
In the second notion of consistency, we are given a \emph{target statistic}, such as the conditional variance or quantile, as in classical statistics, but no target loss.\jessie{citation for stats def}
...
\jessie{How does this paragraph change?}
...
which suggests property elicitation (see \S~\ref{subsec:properties}) as being uniquely \jessie{not nec. uniquely?} suited to studying general lower bounds.
...
In particular, as definitions of consistency are relatively intractable to apply directly, the literature often focuses on a weaker condition called calibration, which only applies when given a discrete target loss. \jessie{Target doesn't have to be discrete, but $\Y$ finite, right?}
\jessie{Make this section more concise?}
...
\begin{definition}[Calibrated]\label{def:calibrated-finite}
	...
	\forall p \in \simplex: \inf_{u \in \U : \psi(u) \not \in \argmin_r \E_p\ell(r,Y)} \exploss{L}{u}{p} > \inf_{u \in \U} \exploss{L}{u}{p}~.~
	\end{equation}
\end{definition}
...
When $L\in\L$, we use $\Gamma := \prop{L}$ to denote the unique property elicited by $L$ from eq.~\eqref{eq:elic}. \jessie{Small detail: probably want to specify the domain in this notation at least once, since we move away from $\P = \simplex$}
Finally, we typically denote the target property by $\gamma$, and the surrogate by $\Gamma$.
...
\begin{definition}[Indirect Elicitation]\label{def:indirectly-elicits}
	A loss and link $(L, \psi)$ \emph{indirectly elicit} a property $\gamma:\P \toto \R$ if $L$ elicits a property $\Gamma: \P \toto \U$ such that for all $u \in \U$, we have $\Gamma_u \subseteq \gamma_{\psi(u)}$.
...
As a result, indirect elicitation in a necessary condition for consistency in this case, and in fact, we show this is true for all four quadrants mentioned above (\S~\ref{sec:consis-implies-indir}). \jessie{quadrant speak vs ``in both cases'' (discrete target loss vs continuous estimation)?}
...
\jessie{Narrative here reads to me as ``hey this general result through calibration is our main contribution'' which I'm not sure if we particularly want.  Also want to double check how different our calibration definition is from Steinwart}

In this section, we show that indirect elicitation is necessary for consistency (Theorem \ref{thm:consistent-implies-indir-elic}) for losses in $\L$, while simultaneously applying to all of the above settings (our four quadrants). \jessie{Tagging quadrant speak}
...
we delegate this proof to Appendix~\ref{app:calibration} and directly prove the general result for all four quadrants. \jessie{Quadrant speak}
...
\begin{definition}[Consistent: property]\label{def:consistent-prop}
	Suppose we are given a loss $L : \U \times \Y \to \reals$, link function $\psi: \U \to \R'$, and property $\gamma:\simplex \toto \R'$.
...
\begin{definition}[Flat]\label{def:flat}\btw{Jessie: Suggest delete}
	...
	The codimension of $F$ is given by $\codim(F)=\mathrm{rank}(W)$, and the dimension of $F$ (relative to $\affhull(\simplex)$) is $n - \mathrm{rank}(W) - 1$.
\end{definition}

\jessie{Differentiate between $\ker$ and $\zeros$}
\begin{definition}[\jessie{Updated definition: }Flat]\label{def:flat-general}
	A \emph{flat} $F \subseteq \P$ $\subseteq \reals^n$ is a nonempty set $F = \ker_\P(W) = \{q \in \P : \E_q W = \vec 0\}$ for some $W:\Y \to \reals^d$.
	Furthermore, we define $\codim(F) := \dim(\spn(\P) / \spn(F))$.
...
\begin{definition}[$\P$-codimension]
	The \emph{\Pcodimension} of a convex set $F \subseteq \P$ is the minimum $d$ such that there exists a measurable function $V : \Y \to \reals^d$ such that $F = \zeros{V}$, and is denoted $\Pcodim(F)$.
\end{definition}


\begin{lemma}
	  \label{thm:convex-flats-inf-dim}
	Let $\Gamma$ be (directly) elicited by a convex $L \in \L_d$ for some $d\in\mathbb{N}$.
...
\begin{proof}
	\jessie{Direct}
		...
		Let $V_{u,p} : \Y \to \reals^d$ be the function mapping column indices of $W$ to columns of $W$.
		That is, $V_{u,p}(y) := W_y$.
				
		
		\emph{Infinite $\Y$: }	
		$L \in \L_d$ and convex implies $L$ satisfies the assumptions of~\cite{ioffe1969minimization}, we can interchange subdifferentiation and expectation to observe $\partial \E_p L(u,Y) = \E_p \partial L(u,Y) := \{\int V(y)dp(y) : V \mathrm{measurable}, V(y) \in \partial L(u,y) \textrm{a.s. in } p\}$.
		...
		We can now take $V_{u,p}$ to be such a $V: \Y \to \reals^d$, which exists as $\vec 0 \in \partial \E_p L(u,Y)$, and therefore, the set is nonempty.
...
If there is no flat $F$ of \Pcodimension at most $d$ with $p \in F \subseteq \gamma_r$, then no $L\in\L_d$ indirectly elicits $\gamma$, and in particular, there is no convex surrogate $L \in \L_d$ consistent with respect to $\gamma$.
...
\begin{proof}
	...
	Moreover, by construction of such a flat, we know the \Pcodimension is at most $d$.
	By definition of indirect elicitation, we additionally have $\Gamma_u \subseteq \gamma_r$.
	Thus, we have $p \in F \subseteq \gamma_r$.
	
	Now, if we do not have a flat satisfying the above conditions, then we do not have any $L$ so that $\prop{L}$ indirectly elicits $\gamma$, and in turn do not have a link such that $L$ is consistent with respect to $\gamma$ by Theorem~\ref{thm:consistent-implies-indir-elic}.
\end{proof}
...
Then if a convex $L \in \L_d$ indirectly elicits $\gamma$, there is a flat $F$ with $p\in F \cap \simplex \subseteq \gamma_r$ and $\codim(F) \leq d$.
% COmmented 06.03.20
...
%	So F is also in gamma_r.
	...
	Moreover, $\partial$ represents the subdifferential $\partial f(x) = \{z : f(x') - f(x) \geq \inprod{z}{x'-x}\; \forall x' \}$.
...
%\raft{Revisit this sentence! \botodo{Visited, had fun, bought the t-shirt.}}\jessiet{lol I don't want to delete this...}
...
\begin{proof}[Proof sketch]
	...
	Lemma~\ref{thm:convex-flats-inf-dim} guarantees the existence of a flat $F$ of \Pcodimension at most $d$ so that $p \in F \subseteq \gamma_r$.
...
However, consider any flat of \Pcodimension codimension $1$ containing $p$.
...
See Figure~\ref{fig:flats-bound} for intuition; a flat of \Pcodimension $1$ would be a line in such a figure.
Therefore, we have no flat of \Pcodimension $1$ containing $p$ staying in $\gamma_\bot$, so we obtain better lower bound, $\ccdim(\ell^{abs}) \geq 3 - 0 - 1 = 2$.

For a case where our bounds match that of~\citep{ramaswamy2016convex}, consider the distribution $q = (1/4, 1/4, 1/2)$, shown in blue in Figures~\ref{fig:fsd-bound} and~\ref{fig:flats-bound}.
...
The same line segment defines a flat contained in both $\gamma_\bot$ and $\gamma_3$, so we have $\ccdim(\ell^{abs}) \geq 3 - 1 - 1 = 1$ by Corollary~\ref{cor:Pcodim-flat-elic-relint-prop}, matching the feasible subspace dimension bound.
...
\begin{minipage}{0.45\linewidth}
	...
	\caption{Any flat $F$ with $\Pcodim(F) = 1$ through \textbullet \, also leaves the cell $\gamma_\bot$.\vspace*{-5pt} %However, for the distribution $p = (1/4, 1/4, 1/2)$, there is a $1$ dimensional flat containing $p$ that is a subseteq of $\gamma_\bot$ and $\gamma_3$.
	}
...
In this setting, Theorem~\ref{thm:cvx-flats} \jessie{Change ref} gives lower bounds on the prediction dimension of convex losses with a link to the desired conditional statistic, i.e., the convex elicitation complexity.
In particular,
...
The lower bounds of that work are essentially all with respect to identifiable properties, that is, properties $\gamma$ for which there is some $d$ where $\gamma_r$ is a flat of \Pcodimension $d$ for all $r\in\R$.
...
Surprisingly, we show that the central techniques of~\citet{frongillo2020elicitation} also apply to convex-elicitable properties, via \raf{MAIN THM}.
...
\jessie{Keep this}
As a warm-up, let us see how to show $\eliccvx(\Var)=2$ whenever $|\Y|\geq 3$, meaning the lowest dimension of a convex loss to estimate conditional variance is 2~\citep{osband1985providing,lambert2018elicitation}.
...
For continuous settings, it would be especially interesting to extend to the case $\Y=\reals$, which would require a more careful analysis in Theorem~\ref{thm:bayes-risk-lower-bound}.\jessie{Flagging this nc we take care of it I hope}
...
For general settings, we introduce a notion of calibration that is a special case of calibration as introduced by~\cite[Definition 2.7]{steinwart2007compare} and~\citet[Chapter 3]{steinwart2008support}. \jessie{I think we need to be careful how we present this; I'm not sure it's a new definition, though we seem to prove new results about it.}
...
\begin{definition}[Calibrated]\label{def:calibrated-general}
	A loss $L:\reals^d \times \Y \to \reals$ is \emph{calibrated} with respect to a loss $\ell : \R \times \Y \to \reals$ eliciting the property $\gamma$ \jessie{$\gamma$ isn't used here...?} if there is a link $\psi : \reals^d \to \R$ such that, for all distributions $p \in \simplex$, there exists a function $\zeta : \reals_+ \to \reals_+$ with $\zeta$ continuous at $0^+$ and $\zeta(0) = 0$ such that for all $u \in \reals^d$, we have
...
\begin{lemma}\label{lem:finite-relint-dim}
  Let flat $F\subseteq \P$ \jessie{of \Pcodimension at most $d$?} contain some $p\in\relint{\P}$.
  Then $p \in \relint{F}$ and $\dim(\spn(F - {p})) \geq \dim(\spn(P - \{p\})) - d$.\jessie{Originally $\Pcodim(F) = \codim(F)$? Also; $F$ vs $F'$?}.
\end{lemma}
\begin{proof}
	\jessie{TODO: write out def of relint (algebraic definition) to be explicit}
	Since $p \in \relint{\P}$, there is some small enough $\epsilon > 0 $ so that for $q \in \P, \alpha \in (-\epsilon,\epsilon)$, the point $q_\alpha := p - \alpha(q - p)$ is still in $\P$.
	In particular, if $q \in F$, then $q_\alpha \in F$, as it is a linear combination of elements of $F$, which is a convex set as it is a flat.
	Therefore, $p \in \relint{F}$.
		
		\bigskip
	\jessie{Where does this belong? Cause it's not here.}
	Now let $T_V : \spn(\P) \to \reals^d$ be the function mapping $q \mapsto \E_q V$ for the function $V$ given by Threorem~\ref{thm:convex-flats-inf-dim}.
	Let $p \in \relint{\simplex} \cap F$, and define $F' := F - \{p\}$, and similarly define $P' := P - \{p\}$ and $T'_V(v) = \E_{Y \sim v} V(Y)$.
	
	Now let $v \in \ker(T'_V)$ (so that $q = p + v$).
	There must exist an $\epsilon > 0$ so that $\epsilon v \in F'$ since, by Lemma~\ref{lem:finite-relint-dim}, we have $p \in \relint{\simplex}$, and therefore $\vec 0 \in \relint{F'}$.
	Moreover, we have $\epsilon v \in \ker(T'_V)$ by construction, since both $v$ and $\vec 0$ are in the kernel; this implies that $\spn(F') = \ker(T'_V)$.
	
	
	Moreover, we want to observe $F' = P' \cap \ker(T'_V)$.
	In order to see this, consider $v \in F' \iff v + p \in F$, and similarly for $\P'$ and $\ker(T'_V)$.
	Now, as $F \subseteq P$ by construction, we have $F' \subseteq P'$, and since $\spn(F') = \ker(T'_V)$, we additionally have $F' \subseteq \ker(T'_V)$, yielding $F' \subseteq P' \cap \ker(T'_V)$.
	Now consider $v \in \P' \cap \ker(T'_V)$; we know that $\E_v V = \vec 0$, and therefore, $\E_{q} = \vec 0 $, so $v \in F'$, thus showing $F' = P' \cap \ker(T'_V)$.
	
	
	Therefore, we have $\dim(\spn(F')) = \dim(\ker(T'_V)) \geq \dim(\spn(P')) - d$ by \jessie{isomorphism theorem} since $F' = \ker(T'_V) \cap P'$.
	When we substitute $P = \simplex$, this yields the bound $\dim(\spn(F')) \geq n - 1 - d$. \jessie{Too soon}
\end{proof}
...
\begin{proof}
	...
	Let $q \in F$.
	%By Lemma~\ref{lem:finite-relint-dim}, we also have $p \in \relint{F}$
	We claim that for small enough $\epsilon$, the point $q' = p - \epsilon (q-p)$ is in $F$ as well.
	Containment in $F$ follows because both $p$ and $q$ are in $F$ and it is a flat, while containment in $\P$ follows because $p \in \relint\P$.
...
\begin{lemma}\label{lem:feas-sub-is-a-flat}
	...
	If $F$ is a flat so that $p \in F \subseteq \gamma_r$, then $F' := F - \{p\}$ is a subspace contained in $\Sc_{\gamma_r}(p)$.
...
\begin{proof}
	...
	As $p \in \relint{\simplex}$, by Lemma~\ref{lem:finite-relint-dim}, we additionally have $p \in \relint{F}$, and in turn $\vec 0 \in \relint{F'}$.\jessie{Last part irrelevant?}
...
The next result helps us generalize Lemma~\ref{lem:feas-sub-is-a-flat} to any $p$; not just to $p \in \relint{\P}$.
...
\begin{proof}
	...
	As $L'$ is calibrated and consistent with respect to $\ell'$ and indirectly elicits $\gamma' := \prop{\ell'}$, then by Corollary~\ref{cor:Pcodim-flat-elic-relint-prop}, we know there exists a flat $F$ with $p \in F \subseteq \gamma'_r$ of \Pcodimension at most $d$.\jessie{To my understanding, we still don't relate \Pcodimension to dimension? Or did I just miss that.}
	Moreover, as $d$ is the maximal codimension of such a flat \jessie{Current phrasing: need to also show \Pcodimension of the feasible subspace is at most $d$}, we show in Lemma~\ref{lem:feas-sub-is-a-flat} that $\Sc_{\gamma_r}(p)$ is one such flat.
	When we substitute $P = \simplexp$ for Lemma~\ref{lem:finite-relint-dim}, this yields the bound $\dim(\spn(F-\{p\})) \geq \|p\|_0 - 1 - d$.
	Chaining inequalities, we then have $\dim(\Sc_{\gamma'_r}(p)) \geq \dim(F - \{p\}) \geq \|p\|_0 - d- 1$ as Lemma~\ref{lem:feas-sub-is-a-flat} states $F - \{p\}$ is a subspace of $\Sc_{\gamma'_r}(p)$.
...
\section{Extensions to infinite $\Y$}\label{app:infinite-Y}

The proof for Theorem~\ref{thm:cvx-flats} only requires slight modifications for infinite $\Y$; we restate such definitions here and generalize the proof.

\begin{definition}[Flat; infinite $\Y$]
	A flat $F \subseteq \spn(\P)$ is a nonempty set $F := \{p \in \spn(\P) : \int \nu(y) d(py) = 0\}$, for some measurable $\nu : \Y \to \reals^d$.
	The codimension of $F$ is given by $\codim(F) = \dim(\spn(P) / F) = \dim(\{p + F : p \in \spn(\P)\}) \leq d$ by design of $\nu$.
\end{definition}

We additionally have to generalize the definition of $\E_p \partial L(u,Y)  := \cl(\{\int \nu(y) p(dy) : \nu \in \L^1_d(\A), \nu(y) \in \partial L(u,Y)\,\, a.s.\})$, where $\L^1_d(\A)$ is the set of Lebesgue measurable functions defined on the $\sigma$-algebra $\A$.

\begin{theorem}[Theorem ~\ref{thm:cvx-flats} with infinite $\Y$]
	Let a single-valued property $\gamma : \P \toto \R$ be given, and let $p \in \P$, and $r \in \gamma(p)$.
	Let $L \in \L_d$ be a convex loss\footnote{$\E_p L(u,Y)$ must be a $\A$-convex normal integrand for all $p \in P$} such that such that $\E_p L(\cdot,Y)$ is finite for all $u \in \reals^d$ and $p \in \P$.
	If $L$ indirectly elicits $\gamma$, there is a flat $F$ with $p \in F \cap \P \subseteq \gamma_r$ with $\codim(F) \leq d$.
\end{theorem}
\begin{proof}
	Consider the probability space $(\Y, \A, p)$ so that $\E_p L(u,Y) = \int L(u,y) d(py)$ is well-defined and finite for all $u \in \reals^d$, and $p \in \P$.
	Moreover, let $\Gamma := \prop{L}$ and suppose $(L, \psi)$ indirectly elicits $\gamma$.
	By definition of properties, there is some $u \in \Gamma(p)$. \jessie{Address if we get rid of minnability}
	
	
	We first construct the flat $F$ and show $F \cap \P \subseteq \Gamma_u$, which implies $F \cap \P \subseteq \gamma_r$ since we must have $u = \psi(r)$ by indirect elicitation and $\gamma$ being single-valued. 
	
	As $L$ is convex and elicits $\Gamma$, we have $u \in \Gamma(p) \iff \vec 0 \in \partial \E_p L(u,Y)$.
	Since, for each $p \in \P$, we have $L$ satisfying the assumptions of~\cite[Corollary 1]{rockafellar1982interchange}, we can interchange subdifferentiation and expectation to observe $\vec 0 \in \partial \E_p L(u,Y) \iff \vec 0 \in \E_p \partial L(u,Y)$.
	As $\vec 0$ is in the expectation of the subgradient, such a $\nu$ exists, so we take $F := \{p \in \spn(\P) : \int \nu(y) d(py) = \vec 0\} \cap \P$.
	\jessie{Want $F$ to be an affine space; intersect with an affine hull}
	
	By construction we have $p \in F \implies \int \nu(y) d(py) = \vec 0 \implies \vec 0 \in \E_p \partial L(u,Y) \implies p \in \Gamma_u$.
	Moreover, we $\codim(F) \leq d$ by definition.
%	
%
%	\bigskip
%	
	%Now, it is just left to show that we must have $\Gamma_u \subseteq \gamma_r$.
	%Let $r' = \psi(u)$; by definition of indirect elicitation, we must have $\gamma_{r'} \supseteq \Gamma_r \supseteq F \cap \P$.
	%If $|\gamma(p)| = 1$, then we are done, as we must have $r' = r$ by indirect elicitation.
	%Now if $|\gamma(p)| > 1$ there are two cases.
	%If $r = \psi(u)$, we are trivially done.
	%If $r' = \psi(u) \neq r$, then we seek to show \jessie{help?}	
\end{proof}
