...
\begin{definition}[$d$-representation]
	The flat $F \subseteq \P$ is a $d$-representation if there exists a measurable function $V : \Y \to \reals^d$ such that $F = \zeros{V}$.
\end{definition}
...
If there is no flat $F$ that is a $d$-representation with $p \in F \subseteq \gamma_r$, then no $L\in\L_d$ indirectly elicits $\gamma$, and in particular, there is no convex surrogate $L \in \L_d$ consistent with respect to $\gamma$.
...
\begin{proof}
	...
	Moreover, by construction of such a flat, we know it is a $d$-representation.
...
Let $p \in \relint{\P}$.
If there is no flat $F$ of that is a $d$-representation with $p \in F \subseteq \gamma_r$, then no $L\in\L_d$ indirectly elicits $\gamma$, and in particular, there is no convex surrogate $L \in \L_d$ consistent with respect to $\gamma$.
%
...
\begin{lemma}\label{lem:finite-relint-dim}
  Let flat $F\subseteq \P$ that is a $d$-representation containing some $p\in\relint{\P}$.
  Then 
  \begin{enumerate}
  	\item $p \in \relint{F}$; 
  	\item $\dim(\spn(F - \{p\})) \geq \dim(\spn(\P - \{p\})) - d$;
  	\item Given a discrete elicitable property $\gamma: \P \toto \R$ and $r \in \gamma(p)$, we have $F - \{p\}$ contained in $\Sc_{\gamma_r}(p)$.
  \end{enumerate}
\end{lemma}
\begin{proof}
	...
	In particular, for $q \in F$, then $q_\alpha \in F$, as it is a linear combination of elements of $F$, which is a convex set as it is a flat.
	Therefore, $p \in \relint{F}$.
		
	\bigskip
	
	(2.)
	Let $p \in \relint{\P} \cap F$, and define $F_p := F - \{p\}$, $\P_p := \P - \{p\}$ and $T_W(v) = \E_{Y \sim v} W(Y)$ for that function $W : \Y \to \reals^d$ such that $F = \zeros{W}$.
	(Such a function must exist by $F$ being a $d$-representation.)
	
	
	Now let $v \in \ker(T_W) = \{v \in \spn(\P_p) : T_W(v) = \vec 0\}$.
	There must exist an $\epsilon > 0$ so that $\epsilon v \in F_p$ and $-\epsilon v \in F_p$ since, by (1.), we have $p \in \relint{\P}$, and therefore $\vec 0 \in \relint{F_p}$.
	...
	We want to show $F_p = \P_p \cap \ker(T_W)$, then derive the result from there.
	...
	Since $\spn(F_p) = \ker(T_W)$, we have $\dim(\spn(F_p)) = \dim(\ker(T_W)) \geq \dim(\spn(\P_p)) - d$ by \jessie{isomorphism theorem} since $F_p = \ker(T_W) \cap \P_p$.
	
	\bigskip
	
	(3.)
	We aim to show $v \in F_p \implies v \in \Sc_{\gamma_r}(p)$.
	First, observe $v \in F_p \iff p + v \in F$.
	As $p \in \relint{\P}$, by part (1.), we additionally have $p \in \relint{F}$, and in turn $\vec 0 \in \relint{F_p}$.
	
	Suppose $v \in F_p$, and since $p \in \relint{F}$, there is an $\epsilon > 0$ so that for all $\alpha \in (-\epsilon, \epsilon)$, we have $p + \alpha v \in F \implies p + \alpha v \in \gamma_r$, since $F$ is convex.%, as are level sets of elicitable properties~\cite{lambert2009eliciting}.
...
\begin{proof}
	...
	As $L'$ is calibrated (and consistent) with respect to $\ell'$ and indirectly elicits $\gamma' := \prop{\ell'}$, then by Corollary~\ref{cor:Pcodim-flat-elic-relint-prop}, we know there exists a flat $F$ with $p \in F \subseteq \gamma'_r$ that is a $d$-representation.
	
	When we substitute $P = \simplexp$ into Lemma~\ref{lem:finite-relint-dim}, we yield $\dim(\spn(F_p)) \geq \dim(\spn(P_p)) - d$.
	Moreover, we can observe that $F_p \subseteq \Sc_{\gamma'_r}(p)$ since $p \in \relint{F}$, and in turn $\vec 0 \in \relint{F_p}$, and applying the definition of feasible subspace dimension.
	By Lemma~\ref{lem:p-boundary-fsd}, we additionally have $\Sc_{\gamma_r}(p) = \Sc_{\gamma'_r}(p)$.
	Chaining these results, we obtain $\dim(\Sc_{\gamma_r}(p)) = \dim(\Sc_{\gamma'_r}(p)) \geq \dim(\spn(F_p)) \geq \|p\|_0 - 1 - d$.
