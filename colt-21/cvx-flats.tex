...
\tikzset{vertex/.style = {ellipse,draw,minimum width=10em}}
\tikzset{edge/.style = {->,> = triangle 45, thick}}
\tikzset{node/.style = {anchor=above, sloped}}

\path[fill=blue, fill opacity = 0.1] (0,-6) -- (0,6) -- (14,6) -- (14,-6) -- cycle;
\draw[dashed, opacity = 0.4] (0,-6) -- (0,6);
\node[draw] at (3,5) {{\Huge$\Lcvx$}};
%\draw (3,5)\node[above] {$\Lcvx$};

% vertices
\node[vertex] (consistent) at  (-9,4) {Consistency};
\node[vertex] (calibrated) at  (-9,-4) {Calibration (Q 1,3)};
\node[vertex] (indir-elic) at  (-3,0) {Indirect elicitation};


\node[vertex] (d-flats) at  (3,0) {$d$-flats in level sets};
\node[vertex] (q1-bounds) at (9,4) {$\conscvx$ bounds for Q1,2- Cor.~\ref{ref}};
\node[vertex] (q4-bounds) at (9,-4) {$\conscvx$ bounds for Q3,4- Theorem~\ref{ref}};
...
Our contributions are summarized in Figure~\ref{fig:results-flow}.
...
\begin{definition}[Convex Consistency Dimension]\label{def:cvx-consistency-dim}
  ...
  \raft{Why not use $\Lcvx$ here?  Then you don't need the qualifier below. \jessie{$\Lcvx$ implies minimizability, do we want that baked in here?}}
%	\btw{Maybe take out of definition environment, but (a) I'd worry that people wouldn't be able to find the definition when we use it later, and (b) I'm hoping we can make the distinction very clear, which now is achieved by the parallel definitions}
\end{definition}
...
\end{definition}
In particular, $\eliccvx(\prop{\ell}) \leq \conscvx(\ell)$.
...
\begin{definition}[Consistent: loss]\label{def:consistent-ell}
  ...
  For a given set $\P \subseteq \simplex$, we simply say $(L,\psi)$ is \emph{consistent} if it is $\D$-consistent for some $\D$ satisfying the following: for all $p \in \P$, there exists $D \in \D$ and $x \in \X$ such that $D$ has a point mass on $x$ and $p = D_x$.
...
\begin{corollary}
	Given a property $\gamma : \P \toto \R$, we have $\eliccvx(\gamma) \leq \conscvx(\gamma)$.
\end{corollary}
...
\begin{proof}
	...
	Otherwise, by Lemma~\ref{lem:set-valued-prop-flats}\jessiet{Moving to miscellaneous omitted proofs}, for elicitable properties with $p \in \gamma_r \cap \gamma_{r'}$, we observe $p \in F\subseteq \gamma_r \iff p \in F \subseteq \gamma_{r'}$.
...
\jessie{Remind reader we restrict to minnable}
...
\begin{proof}[Proof sketch]
	...
	Moreover, since $p \in \relint{\simplex}$, the flat $F$ satisfies the requirements of Lemma~\ref{lem:finite-relint-dim}, and the bound $\dim(\Sc_{\gamma_r}(p)) \geq \dim(\Scr_F(p)) \geq n - d- 1$ is given by Lemma~\ref{lem:finite-relint-dim} (ii).
        \raft{should give intuition instead of (or in addition to) lemmas, since that's more the point of a sketch}
