...
\begin{proof}
	...
	Since, for each $p \in \P$, we have $L$ satisfying the assumptions of~\cite[Corollary 1]{rockafellar1982interchange}, we can interchange subdifferentiation and expectation to observe $\vec 0 \in \partial \E_p L(u,Y) \iff \vec 0 \in \E_p \partial L(u,Y)$, which is defined to be $ \cl(\{\int \nu(y) p(dy) : \nu \in \L^1_d(\A), \nu(y) \in \partial L(u,Y)\,\, a.s.\})$, where $\L^1_d(\A)$ is the set of Lebesgue measurable functions defined on the $\sigma$-algebra $\A$.
	As $\vec 0$ is in the expectation of the subgradient, such a $\nu$ exists, so we take $F := \ker(\nu) \cap \P$, where $\ker(\nu) := \{p \in \P : \int \nu(y) d(py) = 0\}$.
	
	By construction we have $p \in F = \ker(\nu) \cap \P \implies p \in \ker(\nu) \implies \vec 0 = \E_p \nu(Y) \implies \vec 0 \in \E_p \partial L(u,Y) \implies p \in \Gamma_u$.
	
	Moreover, we want to show that $\codim(F) = \dim(\P \setminus F) = \dim(\P) - \dim(F) \leq d$.
	%To see this, observe $\dim(\P) \leq d + \dim(F)$.
	As $\dim(\nu) \leq d$ (by construction, as it is a subgradient of a $d$-dimensional loss), we must have $\dim(\ker(\nu)) \geq \dim(\P) - d$.
	Thus, we have $\dim(F) = \dim(\P \cap \ker(\nu) ) \geq \dim(\P) - d$, giving the desired result.
	\jessie{This is pretty fuzzy to me, but hopefully the edit distance is small.}
	\jessie{\url{https://www.youtube.com/watch?v=2Q_ZzBGPdqE}}
%	
%
%	\bigskip
%	
