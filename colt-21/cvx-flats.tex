...
\begin{lemma}\label{lem:finite-relint-dim}
  ...
  Then $p \in \relint{F}$ and $\Pcodim(F) = \codim(F)$.
\end{lemma}
\begin{proof}
	\jessie{TODO}
	First, to observe $p \in \relint{F}$, since $p \in \relint{\P}$, we must have some ball $B(\epsilon, p) \subseteq \P$ so that $B(\epsilon, p) \cap F \subseteq F$ and is also a ball, and thus $p \in \relint{F}$.
	
	Moreover, to observe $\Pcodim(F) = \codim(F)$, \jessie{TODO}
\end{proof}
...
\begin{lemma}\label{lem:feas-sub-is-a-flat}
	...
	If $F$ is a flat so that $p \in F \subseteq \gamma_r$, then $F - p$ is a subspace contained in $\Sc_{\gamma_r}(p)$.
	\jessie{Is this what we want to show? I feel like we want $\Sc_{\gamma_r}(p)$ is a flat containing $p$.}  
	%  \raf{I think you want $p$ in the interior of the simplex for now}
\end{lemma}
\begin{proof}
	First, observe $F-p$ is a subspace as it is a linear shift of $F$, which is a linear subspace by definition of a flat and the fact that it contains $\vec 0$ (since $F$ contains $p$, which we subtract from).
	Moreover, by Lemma~\ref{lem:finite-relint-dim}, we have $p \in \relint(F)$, so $\vec 0 \in \relint{F-p}$.
