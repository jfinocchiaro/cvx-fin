...
\begin{definition}[$d$-representation]
	The flat $F \subseteq \P$ hass a $d$-representation if there exists a measurable function $V : \Y \to \reals^d$ such that $F = \zeros{V}$.
\end{definition}
...
\begin{proof}
	...
	Moreover, by construction of such a flat, we know it has a $d$-representation.
...
Let $p \in \relint{\P}$.
If there is no flat $F$ of that has a $d$-representation with $p \in F \subseteq \gamma_r$, then no $L\in\L_d$ indirectly elicits $\gamma$, and in particular, there is no convex surrogate $L \in \L_d$ consistent with respect to $\gamma$.
%
...
\begin{proof}[Proof sketch]
	...
	Lemma~\ref{thm:convex-flats-inf-dim} guarantees the existence of a flat $F$ with a $d$-representation so that $p \in F \subseteq \gamma_r$.
	Moreover, since $p \in \relint{\simplex}$, the flat $F$ satisfies the requirements of Lemma~\ref{lem:finite-relint-dim}, and the bound $\dim(\Sc_{\gamma_r}(p)) \geq \spn(F - \{p\}) \geq n - d- 1$ is given by chaining parts (2.) and (3.) of the Lemma together.
...
However, consider any flat of with a $1$-representation containing $p$.
...
See Figure~\ref{fig:flats-bound} for intuition; a flat with a $1$-representation would be a line in such a figure.
Therefore, we have no flat with a $1$-representation containing $p$ staying in $\gamma_\bot$, so we obtain better lower bound, $\ccdim(\ell^{abs}) \geq 3 - 0 - 1 = 2$.
...
\begin{minipage}{0.45\linewidth}
	...
	\caption{Any flat $F$ with a $1$-representation through \textbullet \, also leaves the cell $\gamma_\bot$.\vspace*{-5pt} %However, for the distribution $p = (1/4, 1/4, 1/2)$, there is a $1$ dimensional flat containing $p$ that is a subseteq of $\gamma_\bot$ and $\gamma_3$.
	}
...
The lower bounds of that work are essentially all with respect to identifiable properties, that is, properties $\gamma$ for which there is some $d$ where $\gamma_r$ is a flat with a $d$-representation for all $r\in\R$.
...
\begin{lemma}\label{lem:finite-relint-dim}
  Let flat $F\subseteq \P$ be a $d$-representation containing some $p\in\relint{\P}$.
  Then 
  \begin{enumerate}
  	\item $p \in \relint{F}$; 
...
\begin{proof}
	...
	In particular, for $q \in F$, we have $q_\alpha \in F$.
	Therefore, $p \in \relint{F}$. \jessie{Feels insufficient}
		
	\bigskip
	
	(2.)
	Let $p \in F$, and define $F_p := F - \{p\}$, $\P_p := \P - \{p\}$ and $T_W(v) = \E_{Y \sim v} W(Y)$ for the function $W : \Y \to \reals^d$ such that $F = \zeros{W}$.
	...
	The result follows by applying the definition of the subspace of feasible directions.	
\end{proof}
...
\begin{proof}
	...
	Let $q \in F$.
	%By Lemma~\ref{lem:finite-relint-dim} part (1.), we also have $q \in \relint{F}$
...
The next result helps us generalize Lemma~\ref{lem:finite-relint-dim} to any $p$; not just to $p \in \relint{\P}$.\jessie{Just for their corollary though...}
...
\begin{proof}
	...
	When we substitute $\P = \simplexp$ into Lemma~\ref{lem:finite-relint-dim} part (2.), we yield $\dim(\spn(F_p)) \geq \|p\|_0 - 1 - d$.
	Moreover, we can observe that $F_p \subseteq \Sc_{\gamma'_r}(p)$ since $p \in \relint{F}$ by Lemma~\ref{lem:finite-relint-dim} part (3.).
	By Lemma~\ref{lem:p-boundary-fsd}, we additionally have $\Sc_{\gamma_r}(p) = \Sc_{\gamma'_r}(p)$.
	Chaining these results, we obtain 
	\begin{align*}
	\dim(\Sc_{\gamma_r}(p)) = \dim(\Sc_{\gamma'_r}(p))  &\geq \dim(F_p) = \dim(\spn(F_p)) \geq \|p\|_0 - 1 - d~.~
	\end{align*}
%
