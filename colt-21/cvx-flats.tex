...
\begin{table}
	...
	\begin{tabular}{|p{12ex}||l|l|} 
		\hline
		& \emph{Target loss}  & \emph{Target statistic}\\ \hline \hline
...
In Definition~\ref{def:consistent-prop}, we formalize consistency with respect to a property so that we can study $\conscvx(\gamma)$. \jessiet{Do we want to just talk about $\ell$ here?}
...
\begin{definition}[Consistent: loss]\label{def:consistent-ell}
	A loss and link $(L,\psi)$ are $\D$-consistent for a set $\D$ of distributions over $\X \times \Y$ with respect to a target loss $\ell$ if, for all $D \in \D$ and all sequences of measurable hypothesis functions $\{f_m : \X \to \R\}$,
	\begin{align*}
...
\begin{proof}
		...
		We can now take $V_{u,p}$ to a $V: \Y \to \reals^d$ so that $\E_p V = \vec 0$\raft{I think you're skipping a step or two here, so I'd suggest spelling this out.  You want $\E_p V = 0$ after all, so not any $V$ in that set. \jessie{Attempted to spell it out. Feel free to burn if happier with this}}, which exists as $\vec 0 \in \partial \E_p L(u,Y)$, and therefore, the set contains some $V$ so that $\E_p V = \vec 0$.
...
\begin{proof}
	...
	Since $\gamma$ is single-valued at $p$, we have $r = \psi(u)$; by Lemma~\ref{lem:convex-flats-inf-dim}, we know there is a flat $F = \zeros{V_{u,p}}$ so that $p \in F \subseteq \Gamma_u$.
...
\begin{minipage}{0.45\linewidth}
	...
	\caption{%Since the uniform distribution is in the relative interior of the simplex and the level set $\gamma_\bot$, 
          ...
          %\raf{Jessie: for color reasons, maybe we could make the blue one a different symbol?  Here's my attempt... could be touched up :-)}
          \vspace*{-5pt}        }
	\label{fig:fsd-bound}
\end{minipage}
\hfill
\begin{minipage}{0.45\linewidth}
	...
	However, there is a $1$-flat containing $\star$ that is a subset of $\gamma_\bot$ and $\gamma_3$.
	}
