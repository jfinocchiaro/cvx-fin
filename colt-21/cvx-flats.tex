...
%	\btw{Maybe take out of definition environment, but (a) I'd worry that people wouldn't be able to find the definition when we use it later, and (b) I'm hoping we can make the distinction very clear, which now is achieved by the parallel definitions}
\end{definition}
See Definition~\ref{def:consistent-prop} for consistency with respect to a property.
...
\begin{proof}
  ...
  Define $h:\Delta_{\Y'} \to \Delta_\Y$ such that $h(q') = q$ such that $q_y = q'_y$ for $y\in\Y'$ and $q_y = 0$ otherwise.
...
%  This same observation can be used to observe that $L'$ is also calibrated with respect to $\ell'$ as the calibration bound holds for all $q \in \simplex$, and .
  ...
  Let $\{v_i\}_{i=1}^k$ form a basis for $\Sc_{\gamma_r}(p))$.
  ...
  In particular, consider $q := h(v' + p')$; we have $q \in \gamma_r$, and $q - p \in \Sc_{\gamma_r}(p)$.
  However, as $v'$ cannot be written $\sum_{i=1}^k \alpha_i v_i$, neither can $q$, so we have $\{v_i\}_{i=1}^k$ is not a basis for $\Sc_{\gamma_r}(p)$.\jessiet{So much hand waving}
