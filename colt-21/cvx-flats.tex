...
\begin{theorem}\label{thm:consistent-implies-indir-elic}
	For a convex surrogate $L \in \L$, if the pair $(L, \psi)$ is $\D$-consistent with respect to a property $\gamma$ or a loss $\ell$ eliciting $\gamma$, then $(L, \psi)$ indirectly elicits $\gamma$.\jessiet{How do we want to handle consistency here- we want it to be specific to the distribution}\jessie{Require $\P \subseteq \cup_{D \in \D}\cup_x D_x$ and convex?  - This requirement works, but may be strictly tighter than we need.  With our assumptions on the loss (namely, the expected loss is finite everywhere for all $p$), we should be able to apply Fubini-Tonelli, so that $\P \subseteq \cup_D \cup_x \D_x$ is sufficient, as any $\D_X$ will be covered by this since the joint expectation is the same as $\E_X \E_Y L(u;D_X)$}\jessiet{Need $\X$ and $\Y$ to be $\sigma$-finite, which I think is immediate from them being probability measures}
\end{theorem}
\begin{proof}
...
\jessie{Here, we need to mention $\D$-consistency in the ``in particular''s.  And specify $\P \subseteq \cup_{D \in \D} \cup_x D_x$ or whatever condition we settle on.}
